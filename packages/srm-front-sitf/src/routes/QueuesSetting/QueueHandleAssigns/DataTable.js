/**
 * DataTable - 消息队列定义 - 消息队列处理分配定义 - 待分配数据
 * @date: 2018-9-13
...
export default class DataTable extends PureComponent {
   ...
   */
  componentDidMount() {
    const { getDataTableRef } = this.props;
    if (lodash.isFunction(getDataTableRef)) {
      getDataTableRef(this);
    }
  }
   ...
   */
  @Bind()
  fetchHandleData(pageData) {
    const { form, onFetchHandleAssign } = this.props;
    const { pageCache } = this.state;
    form.validateFields((err, fieldsValue) => {
      if (!err) {
        if (onFetchHandleAssign) {
          onFetchHandleAssign({
            ...fieldsValue.option,
            ...pageCache,
            ...pageData,
  ...
  fetchAssignByCondition() {
    ...
    this.fetchHandleData(data);
  }

  /**
   * 勾选框
   * @param {null} _ 占位符
   * @param {object} rows 勾选行数据
   */
  @Bind()
  ...
  render() {
    const { data = {}, dataColumns = [], loading } = this.props;
    const { selectedRows } = this.state;
    const rowSelection = {
      selectedRowKeys: selectedRows.map(n => n.queueHandlerId),
      onChange: this.handleSelectRows,
    };
    return (
      <Row>
        <div className="assign-table-title">
          {intl.get('sitf.queuesSetting.view.message.title.data').d('消息队列处理数据')}
        </div>
        <div className="table-list-search">{this.renderForm()}</div>
        <Table
          bordered
          loading={loading}
          rowKey="queueHandlerId"
