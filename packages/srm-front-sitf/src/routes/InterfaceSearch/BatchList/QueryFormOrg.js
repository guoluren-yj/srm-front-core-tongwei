/**
 * InterfaceSearch - 接口查询 - 接口列表 - 查询表单 - 平台级
 * @date: 2018-9-18
...
@CacheComponent({ cacheKey: '/sitf/interface-search-org/list' })
  ...
  queryValue() {
    ...
    form.validateFields((err, fieldsValue) => {
      if (!err) {
        queryValue({
          ...fieldsValue,
          creationDateFrom: fieldsValue.creationDateFrom
            ? fieldsValue.creationDateFrom.format(DEFAULT_DATETIME_FORMAT)
            : '',
          creationDateTo: fieldsValue.creationDateTo
            ...
            <Col span={18}>
              ...
              <Row style={{ display: expand ? 'none' : 'block' }}>
                ...
                <Col span={8}>
                  <FormItem
                    {...formItemProps}
                    label={intl.get('sitf.common.data.runningErrorFlag').d('运行时出错')}
                  >
                    {getFieldDecorator(
                      'runningErrorFlag',
                      {}
