...
const ReferencingComponents = props => {
  ...
  const fetchNavData = () => {
    referencingTreeNavDs.setQueryParameter('extItfId', id);
    referencingTreeNavDs.setQueryParameter('tenantId', tenantId);
    referencingTreeNavDs.setQueryParameter('source', 'RESPONSE');
  ...
  const fetchData = (tenantIdp, fieldUniqueCodeProps) => {
    if (!isNil(id) && !isNil(tenantIdp) && !isNil(fieldUniqueCodeProps)) {
      eferencingObjectDs.setQueryParameter('extItfId', id);
      eferencingObjectDs.setQueryParameter('tenantId', tenantIdp);
      eferencingObjectDs.setQueryParameter('source', 'RESPONSE');
  ...
  const handleLineDisabled = useCallback(async (record) => {
    ...
    eferencingObjectDs.setQueryParameter('source', 'RESPONSE');
    const response = enabledFlag ? await fetchObjectDisabled(id, {tenantId, source: 'RESPONSE'}, [currentData]) : await fetchObjectEnabled(id, {tenantId, source: 'RESPONSE'}, [currentData]);
  ...
  const handleLineDelete = useCallback((record) => {
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        const response = await fetchObjectDelete(id, {tenantId, source: 'RESPONSE'}, record.toData())
        const resp = getResponse(response);
        if(resp) {
          notification.success({});
          if(flag) {
  ...
  const handleAddField = useCallback(() => {
    ...
    eferencingObjectDs.find((item:any) => {
      ...
      if(flag) {
        record = eferencingObjectDs.create({sourceNode: uniqueCode}, 0);
      }
      return flag;
    })
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        if(!isEmpty(filterData)) {
          const response = await fetchDeleteLine(id, {tenantId, source: 'RESPONSE'}, filterData)
          const resp = getResponse(response);
          if(resp) {
            notification.success({});
            fetchData(tenantId, navKey)
  ...
  const handleObjectSave = useCallback(async () => {
    ...
    if(validateFlag) {
      ...
      const response = await fetchObjectSave(id, {tenantId, source: 'RESPONSE'}, currentData);
      const resp = getResponse(response);
      if(resp) {
        notification.success({});
        fetchData(tenantId, navKey);
    ...
    Modal.open({
      ...
      onOk: async () => {
        ...
        if(validateFlag) {
          ...
          const response = await fetchImportJson(id, {tenantId, source: 'RESPONSE'}, jsonData);
          const resp = getResponse(response);
          if(resp) {
            notification.success({});
            flag = true;
  ...
  const handleViewJson = useCallback(async () => {
    const response = await fetchViewJson(id, {tenantId, source: 'RESPONSE'});
    const resp = getResponse(response);
    if(resp) {
  ...
  const buttons = useMemo(() => [
    ...
    <div className='buttonsUploadStyle'>
      ...
      accept={['.xls', '.xlsx']}
      name="excel"
      action={`${API_HOST}/sifc/v1/ext-itf-field-infos/${id}/import?tenantId=${tenantId}&fieldSource=RESPONSE`}
  ...
  return (
    <div className="enterStyle">
      <TreeShow
        referencingTreeNavDs={referencingTreeNavDs}
        eferencingObjectDs={eferencingObjectDs}
