...
export default class ChangePwdForm extends React.Component {
  ...
  @Bind()
  okHandle() {
    const { form, saveNewPwd, tableRecord } = this.props;
    const { _token, frontEndSystemsId, clientId } = tableRecord;
    form.validateFields((err, fieldsValue) => {
      if (!err) {
        if (saveNewPwd) {
          saveNewPwd({ _token, frontEndSystemsId, clientId, ...fieldsValue });
        }
      }
    });
  }
  ...
  render() {
    ...
    return (
      <Modal
        ...
        title={intl.get('sitf.frontComputerDef.view.title.modal.changePwd').d('密码修改')}
              ...
              rules: [
                {
                  ...
                  message: intl.get('hzero.common.validation.notNull', {
                    name: intl
                      .get('sitf.frontComputerDef.model.frontComputerDef.password')
                      .d('密码'),
                  }),
                },
                {
