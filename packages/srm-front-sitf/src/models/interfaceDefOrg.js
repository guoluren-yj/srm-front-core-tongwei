/**
 * interfaceDefinition - 接口定义 - model 租户级
 * @date: 2018-09-09
...
import {
  ...
  updateInterfaces,
  quoteInterface,
} from '@/services/interfaceDefOrgService';
import {
  fetchKeywordConfig,
...
} from '@/services/interfaceTableDefOrgService';

export default {
  namespace: 'interfaceDefOrg',
  state: {
    list: {},
    pagination: {},
    code: [],
  ...
  effects: {
    ...
    *deleteInterFaceTable({ payload }, { call }) {
    ...
    // 引用云级接口数据
    *quoteInterface({ payload }, { call }) {
      const response = yield call(quoteInterface, payload);
      return getResponse(response);
    },
