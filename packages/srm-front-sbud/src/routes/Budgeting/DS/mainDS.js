  ...
  events: {
    update: ({ name, value, record }) => {
      ...
      if (name.includes('LOV') && value) {
        const bugetCode = name.split('LOV')[0];
        const mapList = record.getField(`${bugetCode}MapList`).get('defaultValue');
        if (!isEmpty(mapList) && record.get(name)) {
          mapList.forEach(item => {
            // 存在targetDimensionCode，目标维度编码
            if (item.mappingItemCode) {
              // lov对象中的字段赋值到targetDimensionCode
              record.set(item.mappingItemCode, record.get(name)[item.valueField]);
            }
          });
        }
        ...
        if (!isEmpty(mapList)) {
          mapList.forEach(item => {
            // 存在targetDimensionCode，目标维度编码
            if (item.mappingItemCode) {
              record.set(item.mappingItemCode, undefined);
            }
          });
        }
      }
...
const queryFields = () => [
  ...
  },
  {
    name: 'budgetStatus',
    type: 'string',
    lookupCode: 'SBUD.BUDGET_STATUS',
    label: intl.get('sbud.budgeting.model.budgeting.budgetStatus').d('预算状态'),
  },
  {
    name: 'createdByName',
    type: 'string',
  ...
  },
  {
    name: 'year',
    type: 'year',
    label: intl.get('sbud.budgeting.model.budgeting.year').d('年份'),
    format: 'YYYY',
    transformRequest: value => (value ? getMomentDate(value, 'YYYY') : null),
  },
  {
    name: 'creationDate',
    type: 'dateTime',
  ...
  },
  {
    ...
    transformRequest: value =>
      value && (value.startDate || value.endDate)
        ? {
            startDate: value.startDate ? getMomentDate(value.startDate, getDateTimeFormat()) : null,
            endDate: value.endDate ? getMomentDate(value.endDate, getDateTimeFormat()) : null,
          }
        : {},
  },
  {
    ...
    defaultValue: '0',
    label: intl.get('sbud.budgeting.model.budgeting.abolished').d('是否显示已作废数据'),
  },
];

export { mainTableDs, queryFields };
