...
export default class CommentImport extends PureComponent {
  ...
  getTemplate() {
    ...
    loadTemplate({ code, prefixPatch, tenantId, cusFlag: true })
      .then((res) => {
  ...
  async handleDownloadTemplateExcel() {
    ...
    const api = `${SRM_SPRM}/v1/${organizationId}/budget-import/template/${code}/excel`;
  ...
  handleTemporaryDataExportClick(e) {
    ...
    if (['excel', 'csv'].includes(e.key)) {
      const api = `${API_HOST}${prefixPatch}/v1/${organizationId}/budget-import/export/excel`;
      downloadFile({
        requestUrl: api,
        queryParams: [
