...
import { fetchHistory, unlock, copy, enable, disable } from '@/services/budgetStrategyService';
...
const Index = function Index(props) {
  ...
  const handleToEditDetail = record => {
    if (record) {
      dispatch(
        routerRedux.push({
          pathname: `/sbud/budget-strategy/detail/${record.get('budgetStrategyId')}`,
        })
      );
    } else {
      dispatch(
        routerRedux.push({
          pathname: `/sbud/budget-strategy/detail/new`,
        })
      );
    }
  };
  ...
  const fetchHistoryList = (record, hidden) => {
    ...
    if (!hidden) {
      ...
      fetchHistory(record.get('budgetStrategyId'))
        .then(res => {
          if (getResponse(res)) {
            setHistoryList(res);
          } else {
  ...
  const toReadOnlyDetail = record => {
    const type =
      ...
      record.get('strangeStatus') === 'NEW' &&
      (!record.parent || record?.parent?.get('strangeStatus') === 'RELEASED')
        ? 'edit'
        : 'read';
    dispatch(
      routerRedux.push({
        pathname: `/sbud/budget-strategy/read-only/${record.get('budgetStrategyId')}`,
        search: `?type=${type}`,
      })
    );
  };
    ...
    const buttonList = [
      {
        ...
        showFlag: ['DISABLED', 'RELEASED'].includes(record.get('strangeStatus')),
      },
      {
        name: 'edit',
      ...
      },
      {
        ...
        showFlag:
          record.get('enabledFlag') === 1 &&
          record.get('strangeStatus') === 'RELEASED' &&
          !record?.children,
      },
      {
        name: 'copy',
      ...
      },
      {
        ...
        showFlag:
          ...
          ['DISABLED', 'RELEASED'].includes(record.get('strangeStatus')),
      },
    ];
  ...
  const columns = useMemo(() => {
    return [
      ...
      //   renderer: ({ record }) => (
      //     <div>
      //       <CheckBox record={record} name="enabledFlag" />
  ...
  return (
    <Fragment>
      <Header title={intl.get(`${commonPrompt}.budgetStrategyTemplate`).d('预算策略')}>
      ...
      <Content>
        {customizeTable(
          {
            code: 'SBUD_BUDGET_STRATEGY.TABLE',
            dataSet: tableDs,
          },
          <SearchBarTable
            mode="tree"
            style={{ maxHeight: 'calc(100vh - 190px)' }}
            searchCode="SBUD_BUDGET_STRATEGY.SEARCH"
            dataSet={tableDs}
            cacheState
            columns={columns}
            queryFieldsLimit={3}
            ...
            searchBarConfig={{
              editorProps: {
                strangeStatus: {
                  optionsFilter: options => !['INVALID'].includes(options.get('value')),
                },
              },
            }}
          />
...
export default compose(
  ...
  withCustomize({
    unitCode: ['SBUD_BUDGET_STRATEGY.SEARCH', 'SBUD_BUDGET_STRATEGY.TABLE'],
  }),
  withProps(
    () => {
