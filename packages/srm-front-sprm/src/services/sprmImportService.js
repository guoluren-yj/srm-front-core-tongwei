...
export async function loadTemplate(params) {
  const { prefixPatch, code, tenantId, query = {} } = params;
  const site = isTenantRoleLevel();
  const reqUrl = prefixPatch
    ? `${prefixPatch}/v1/${tenantId}/import/template/${code}/info`
    : `${HZERO_IMP}/v1/${site ? `${tenantId}/` : ''}template/${code}/info${!site ? `?tenantId=${getCurrentOrganizationId()}` : ''
    }`;
...
export async function validateData(params) {
  const { sync, templateCode, prefixPatch, batch, tenantId, ...other } = params;
  const reqUrl = sync
...
export async function importData(params) {
  const { sync, prefixPatch, templateCode, batch, tenantId, ...other } = params;
  const reqUrl = sync
