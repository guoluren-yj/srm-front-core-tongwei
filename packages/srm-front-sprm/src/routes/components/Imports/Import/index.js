...
import {
  ...
  updateOne,
  validateData,
} from '@/services/sprmImportService';
...
export default class CommentImport extends PureComponent {
  ...
  @Bind()
  getTemplate() {
    const { prefixPatch = '', tenantId, code, args } = this.state;
    this.showLoading('loadTemplateLoading');
    loadTemplate({ code, prefixPatch, tenantId, query: { ...args } })
      .then(res => {
  ...
  @Bind()
  validateData() {
    const { prefixPatch, batch, sync, tenantId, code, args } = this.state;
    if (batch) {
