...
export default class UploadExcel extends React.Component {
  ...
  beforeUpload(file) {
    ...
    if (prefixPatch && templateCode) {
      ...
      const url = auto
        ? `${API_HOST}${prefixPatch}/v1/${tenantId}/import/data/sync/auto-import`
        : sync
        ? `${API_HOST}${prefixPatch}/v1/${tenantId}/import/data/sync/data-upload`
        : `${API_HOST}${prefixPatch}/v1/${tenantId}/import/data/data-upload`;
      this.setState({
        uploadLoading: true,
      });
      request(url, {
