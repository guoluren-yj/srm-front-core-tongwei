...
const line = (prHeaderId) => ({
    ...
    queryFields: [
        {
            ...
            noCache: true,
            type: 'string',
            valueField: 'processTypeCode',
            lookupCode: 'SPRM_111',
            textField: 'processTypeCodeMeaning',
            dynamicProps: {
                lookupAxiosConfig: () => ({
                    url: `${SRM_SPRM}/v1/${organizationId}/purchase-requests/${prHeaderId}/actions/lov`,
                    params: {
                    ...
                    transformResponse(data) {
                        const paymentArr = typeof data === 'string' ? JSON.parse(data) : data;
                        return paymentArr;
                    },
                }),
            },
    ...
    transport: {
        read: ({ data }) => {
            const { processedDateRange } = data || {};
            const [processDateStart, processDateEnd] = processedDateRange?.split(',') || [];
            return {
                url: `${SRM_SPRM}/v1/${organizationId}/purchase-requests/${prHeaderId}/actions/all`,
                method: 'GET',
                data: { ...data, processedDateRange: null, processDateStart, processDateEnd },
            };
        },
    },
});

const approval = (prHeaderId) => ({
    selection: false,
    fields: [
        {
            name: 'endTime',
    ...
    transport: {
        read: () => {
            return {
                url: `${SRM_SPRM}/v1/${organizationId}/pr-approval-history/record`,
                method: 'GET',
                data: { prHeaderId },
            };
        },
