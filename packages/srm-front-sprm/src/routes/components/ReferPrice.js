...
const ReferPrice = ({ currentRecord, customizeTable, cusCode, cuxLable, cuxShowLink }) => {
  const openModal = () => {
    const referLineDs = new DataSet(referPiceDs({ data: currentRecord?.toData(), cusCode }));
    const cols = [
  ...
  const cuxShowFlag = isFunction(cuxShowLink) ? cuxShowLink({ record: currentRecord }) : flag;
