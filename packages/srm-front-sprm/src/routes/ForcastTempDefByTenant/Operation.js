...
const Index = ({ templateHeaderId, operateLineDs }) => {
  ...
  const changeFieldMeaning = {
    ...
    feedbackApprovalMethod: intl
      .get(`${commonPrompt}.feedbackApprovalMethod`)
      .d('已反馈状态变更配置'),
    feedbackSyncFlag: intl.get(`${commonPrompt}.feedbackSyncFlag`).d('反馈完成是否同步外部系统'),
    ...
    if (changeField && typeCode === 'UPDATE') {
      return (
        <div>
          ...
          {intl.get('sprm.forecastMgt.model.common.jiang').d('将')}
          {tableCode && tableCode !== 'sprm_fcst_template_header' && (
            <span className="status gray">
              ...
              {intl.get('sprm.forecastMgt.model.common.de').d('的')}
            </span>
          )}
          <span className="status gray">{changeFieldMeaning[changeField]}</span>
          {intl.get('sprm.forecastMgt.model.common.from').d('由')}
          <span className="status gray">{oldValueMeaning || oldValue}</span>
          {intl.get('sprm.forecastMgt.model.common.to').d('更新为')}
  ...
  const allTypeIconObj = {
    UPDATE: 'mode_edit',
    NEWLINE: 'mode_edit',
    QUOTE: 'done_all',
    COPY: 'baseline-file_copy',
  };
            ...
            {actionData?.map((ele, index) => {
              if (isArray(ele) && ele.length > 1) {
                ...
                return (
                  ...
                  >
                    <Icon
                      type={allTypeIconObj[processTypeCode] || 'publish2'}
                      style={{ fontSize: 14 }}
                    />
