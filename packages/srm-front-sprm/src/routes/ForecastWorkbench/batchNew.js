...
export default class PriceLibBatchCreate extends PureComponent {
  ...
  async queryImportTemplate() {
    ...
    const result = getResponse(await queryImportTemplate({ templateHeaderId }));
  ...
  @Bind()
  renderBackPath() {
    const url = `/sprm/forecast-workbench/list`;
  ...
  render() {
    const {
      ...
      historyVisible,
      dataImportStatus = [],
      routerParams: { offlineRecord, templateHeaderId },
    } = this.state;
    const {
      match: { params },
    } = this.props;
