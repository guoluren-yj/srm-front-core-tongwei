...
const getDefaultFieldsConfig = (): FieldsConfig<OutputProps> => {
  return {
    versionNumber: {
      ...
      renderer: ({ text }) => intl.get('sbsm.common.view.message.versionVNumber', { versionNumber: text }).d('版本v{versionNumber}'),
    },
    userName: {
    ...
    <div className={styles['version-wrapper']}>
      {isEmpty(dataSet) ? (
        <div className={styles[`empty-version-wrapper`]}>
          {intl.get('hzero.common.button.historyEmpty').d('暂无历史版本信息')}
        </div>
      ) : (
        dataSet.map((record) => (
