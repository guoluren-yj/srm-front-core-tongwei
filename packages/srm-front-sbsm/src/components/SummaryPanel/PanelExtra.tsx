    ...
    return showExtra ? (
      <div className={styles[prefixCls]} onClick={() => setStickyFlag(!stickyFlag)}>
        <Tooltip
          ...
          title={
            stickyFlag
              ? intl.get('sbsm.common.view.message.pinAreaCancel').d('取消固定此区域')
              : intl.get('sbsm.common.view.message.pinArea').d('固定此区域')
          }
        >
          <img
            alt=""
          ...
          />
          {stickyFlag
            ? intl.get('sbsm.common.view.message.cancelPin').d('取消钉住')
            : intl.get('sbsm.common.view.message.onTheTop').d('钉在顶部')}
        </Tooltip>
      </div>
    ) : null;
  }
