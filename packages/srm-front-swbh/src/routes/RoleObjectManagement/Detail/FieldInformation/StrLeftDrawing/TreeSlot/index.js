...
import { Tooltip, OverflowTip } from 'choerodon-ui/pro';
...
const Index = ({ propsItem }) => {
  const [showJumObj, setShowJumpObj] = useState();
  const getTitle = (type) => {
    let iconName = '';
    let title = '';
    let thisClass = '';
      ...
      <div className={styles['model-source-tree-slot-con']} style={{ alignItems: 'normal' }}>
        <div>
          ...
          {showJumObj?.businessObjectId === propsItem?.businessObjectId ? (
            <a className={styles['link-to-bo']} href="###">
              {intl.get('swbh.roManagement.view.message.header.businessObject').d('单据对象')}
              <ImgIcon name="boChoose.svg" size={10} />
            </a>
          ) : null}
        </div>
        <span className={styles['relate-bo-code']}>
          <OverflowTip>{getTitle('text')}</OverflowTip>
          {!isNull(propsItem?.relateBusinessObjectCode) ? (
            <OverflowTip>
              <span className={styles['bo-code']}>{propsItem?.relateBusinessObjectCode}</span>
            </OverflowTip>
          ) : null}
        </span>
      </div>
