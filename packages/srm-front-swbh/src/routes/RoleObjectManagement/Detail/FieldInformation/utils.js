  ...
  const _mapTree = (_data, _callback) =>
    _data.map((item, index, arr) => {
      ...
      if (item?.businessObjectRelationFieldList && item?.businessObjectRelationFieldList?.length > 0) {
        newItem = {
          ...newItem,
          businessObjectRelationFieldList: _mapTree(item.businessObjectRelationFieldList, _callback),
        };
      }
...
export const dealCreateIdAndParentId = (data = [], onLyChild) => {
  let fieldList = [];
  let relationList = [];
  const originData = data?.map((item) => {
    const _uuid = uuid();
    ...
    // 处理字段
    if (item?.businessObjectRelationFieldList) {
      const arr = item?.businessObjectRelationFieldList?.map((i) => {
        return {
          ...i,
    ...
    if (item?.businessObjectRelationList) {
      const arr = item?.businessObjectRelationList?.map((i) => {
        const _uuid1 = uuid();
        return {
          ...i,
          id: _uuid1,
...
export const getChildList = (data, id, flag) => {
  const objList = data.filter((item) => item.parentId === id);
  if (flag) {
 ...
 */
export function treeFindPath(tree = [], func1, func2, children = 'businessObjectRelationList', path = []) {
  if (!tree) return [];
 ...
 * @param path 路径上的节点数据 以{}形式返回
 */
export function treeFindNode(tree = [], func, children = 'businessObjectRelationList') {
  if (!tree) return [];
  for (const data of tree) {
    if (func(data)) {
      return data;
