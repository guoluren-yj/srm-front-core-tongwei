...
import { Modal, Row, Col, Spin, Upload } from 'hzero-ui';
...
export default class Attachment extends PureComponent {
  ...
  @Bind()
  queryAttachmentList() {
    const { supplierAttachmentUuid } = this.props;
  ...
  render() {
    ...
    const { receivedFileList, fileList, previewVisible, previewImages, accessToken } = this.state;
    const headers = {};
    if (accessToken) {
