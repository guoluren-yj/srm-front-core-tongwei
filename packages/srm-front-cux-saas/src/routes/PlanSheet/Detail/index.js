...
export default class Detail extends PureComponent {
  ...
  handleImport() {
    ...
    const option = {
      pathname: '/scux/qwkj/plan-sheets/data-import/SSCH.SCHEDULE_LINE_DETAIL',
      search: stringify({
        action: intl.get('hzero.common.viewtitle.batchImport').d('批量导入'),
        backPath: `/scux/qwkj/plan-sheets/detail/${match.params.id}`,
      }),
    };
  ...
  handleValidateSaveOrRelease(submitFlag, modelName) {
    ...
    // const newDataSource = [{ ...planDetailList[0], scheduleDetailMap: { APLAN: aPlan } }];
    // const newDataSource = planDetailList.map((i, index) => ({
    //   ...planDetailList[index],
    //   scheduleDetailMap: { APLAN: aPlan[index] },
    // }));
    // planDetailList
        ...
        if (planDetailHeader.planningCycle) {
          const validDataSource = planDetailList.map((item) => {
            ...
            for (const i in scheduleDetailMap) {
              temp.push(...scheduleDetailMap[i]);
            }
            let validateNum = 0;
            const scheduleList = temp.map((elementItem, index) => {
          ...
          if (flag) {
            ...
            dispatch({
              ...
              payload: {
                ...
                scheduleLines: validDataSource.map((item) => omit(item, ['scheduleDetailMap'])),
              },
            }).then((res) => {
              if (res) {
                notification.success();
                if (submitFlag) {
                  history.push({ pathname: '/scux/qwkj/plan-sheets/list' });
                } else {
                  this.queryPlanDetailHeader();
                }
              }
  ...
  /**
   * 接口需要的数据
   * @param {Aarry} renderData 渲染数据
   * @param {Aarry} sourceData 接口初始数据
   * @returns {Aarry}
   */
  @Bind()
  transformOrigin(renderData = [], sourceData = []) {
    if (sourceData.length > 0) {
      const copySourceData = JSON.parse(JSON.stringify(sourceData));
      copySourceData.map((item, idx) => {
        const { scheduleDetailMap, ...rest } = item;
        const renderItem = renderData[idx];
        let element = {};
        // eslint-disable-next-line guard-for-in
        for (const planType in scheduleDetailMap) {
          const planArr = scheduleDetailMap[planType];
          ...
          planArr.forEach((plan) => {
            const planKey = `${planType}$$${plan.planDate}`;
            const planValue = renderItem[planKey];
            if (renderItem && planValue) {
              // eslint-disable-next-line no-param-reassign
              plan.planQuantity = planValue.planQuantity;
            }
          });
        }
        element = {
      ...
      // console.log('transformOrigin', copySourceData);
    ...
    return (
      <div>
        <Header
          ...
          backPath="/scux/qwkj/plan-sheets/list"
        >
          <Button
            icon="save"
            type="primary"
