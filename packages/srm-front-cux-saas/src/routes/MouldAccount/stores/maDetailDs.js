...
const maDetailDs = ({ maHeaderId, source = null, maExpandDs, tableDS, read }) => ({
  paging: false,
  autoCreate: true,
  dataToJSON: 'all',
  ...
  transport: {
    read: ({ data }) => {
      ...
      return {
        ...
        data: {
          customizeUnitCode:
            'SIEC.MOULD_PLATFORM.DETAIL.HEADER,SIEC.MOULD_PLATFORM.DETAIL.LIST,SIEC.MOULD_PLATFORM.DETAIL.EXPAND_LINE',
        },
      };
    },
  },
  children: {
    maLineList: tableDS,
    mouldAccountLineExpandList: maExpandDs,
  },
  events: {
    update: ({ name, value, record }) => {
      if (name === 'mouldLov' && value) {
        ...
        mouldMasterDataDetail(mouldId).then(res => {
          if (res && !res.failed) {
            const { _token, objectVersionNumber, creationDate, ...others } = res;
            record.set({
              ...others,
            });
          }
...
function tableLineDS(selection = 'multiple') {
  return {
    paging: false,
    dataToJSON: 'all',
    autoQuery: false,
    ...
    read: ({ data }) => {
      ...
      return {
        ...
        transformResponse: res => {
          ...
          const { maChangeModify, reason } = dealData;
          return { ...maChangeModify, reason };
        },
      };
    },
  },
...
export { maDetailDs, tableLineDS, maDetailModifyDs, maExpandLine };
