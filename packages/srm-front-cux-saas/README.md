...
React是用于构建用户界面的JavaScript库,本项目采用全新的react v16.12.0,其中包含一些全新的特性.且本项目会持续同步react版本.
...
```shell
git clone -b micro-1.x https://open-gitlab.going-link.com/operation-srm/srm-front-code.git
```
### 初始化本项目
...
```bash
yarn config set registry https://nexus.going-link.com/repository/zhenyun-npm-group/
```
...
* `build`: 构建生产包，此命令依赖父模块，即父项目`srm-front-core` 不存在时，该命令`nothing to do`
* `build:ext`: 独立构建，不依赖父模块
