...
export const supplierListDS = (nominationHeaderId, type, getCompanyId?): DataSetProps => ({
  ...
  fields: [
    ...
    { name: 'technologyReviewResult', type: FieldType.string, label: intl.get(`${prefix}.field.technologyReviewResult`).d('技术评审结果'), lookupCode: 'SCUX_TWNF_REVIEW_RESULTS', },
    { name: 'businessReviewResult', type: FieldType.string, label: intl.get(`${prefix}.field.businessReviewResult`).d('商务评审结果'), lookupCode: 'SCUX_TWNF_REVIEW_RESULTS', },
    { name: 'financeReviewResult', type: FieldType.string, label: intl.get(`${prefix}.field.financeReviewResult`).d('财务评审结果'), lookupCode: 'SCUX_TWNF_REVIEW_RESULTS', },
    { name: 'summaryReviewResult', type: FieldType.string, label: intl.get(`${prefix}.field.summaryReviewResult`).d('综评结果'), lookupCode: 'SCUX_TWNF_REVIEW_RESULTS', },
...
export const technicalReviewCaseDS = (nominationHeaderId, nominationSupLineId): DataSetProps => ({
  ...
  fields: [
    ...
    { name: 'caseStatus', type: FieldType.string, label: intl.get(`${prefix}.field.caseStatus`).d('案例状态'), lookupCode: 'SCUX_TWNF_CASE_STATUS', required: true },
    { name: 'caseTime', type: FieldType.year, format: 'YYYY', label: intl.get(`${prefix}.field.caseTime`).d('案例时间'), required: true },
    { name: 'contractAmount', type: FieldType.number, label: intl.get(`${prefix}.field.contractAmount`).d('合同金额（元）'), required: true },
    { name: 'employerContact', type: FieldType.string, label: intl.get(`${prefix}.field.employerContact`).d('发包联系人'), required: true },
    { name: 'contactPhone', type: FieldType.string, label: intl.get(`${prefix}.field.contactPhone`).d('联系电话'), required: true },
    { name: 'remark', type: FieldType.string, label: intl.get(`${prefix}.field.remark`).d('备注') },
