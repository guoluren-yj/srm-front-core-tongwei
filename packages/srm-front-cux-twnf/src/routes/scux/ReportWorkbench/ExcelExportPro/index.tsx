...
const ExcelExport: React.FC<IExcelExportProps> = observer(props => {
  ...
  const queryArea = useMemo(() => new DataSet(baseQueryDS(formProps, exportAsync)), [formProps]);
  ...
  const openExportModal = () => {
    ...
    if (!isEmpty(defaultConfig)) {
      ...
      if (!isEmpty(data)) {
        ...
        handleExport({ fillerType: 'single-sheet', ...others, async: 'true' });
      } else {
        queryColumnData({ fillerType: 'single-sheet', ...others, async: 'true' });
      }
    } else {
