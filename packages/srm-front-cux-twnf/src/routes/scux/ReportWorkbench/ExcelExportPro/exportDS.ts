...
const baseQueryDS = (formProps, exportAsync): DataSetProps => {
  let fields = [
    {
      name: 'fileName',
    ...
    },
    {
      ...
      computedProps: {
        defaultValue: ({ dataSet }) => {
          if(exportAsync === true || exportAsync === "true") {
            return "true";
          }else {
...
const exportTreeDS = (): DataSetProps => ({
  id: 'id',
  autoQuery: false,
  idField: 'id',
  ...
  transport: {
    read: ({ data, dataSet }) => {
      const { requestUrl, method, defaultSelectAll, templateCode, templateType, ...otherData } =
        data;
...
const templateListDS = (): DataSetProps => ({
  id: 'id',
  autoQuery: false,
  idField: 'id',
  parentField: 'parentId',
...
const exportHistoryDS = (): DataSetProps => ({
  autoQuery: true,
  queryFields: [
    {
      name: 'taskCode',
