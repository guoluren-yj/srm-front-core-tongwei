...
export default class Editor extends React.Component {
  ...
  handleOk() {
    ...
    form.validateFields((err, fieldsValue) => {
      if (!err) {
        const { agentAppId, miniAppId, extParam = {}, ...others } = fieldsValue;
        onOk({ ...others, extParam: JSON.stringify({ ...extParam, agentAppId, miniAppId }) });
      }
    });
  }
  ...
  channelRender(code) {
    ...
    switch (code) {
      ...
      case 'wxpay': {
        ...
        const { agentAppId = '', miniAppId = '' } = extParam;
        const signTypeList = [
            ...
            <Row>
              ...
              {getFieldValue('channelCode') && (
                <Col span={12}>
                  <FormItem
                    {...MODAL_FORM_ITEM_LAYOUT}
                    label={intl.get('hpay.payConfig.model.payConfig.payMethod').d('支付方式')}
                  >
                      ...
                      rules: [
                        {
                          ...
                          message: intl.get('hzero.common.validation.notNull', {
                            name: intl
                              .get('hpay.payConfig.model.payConfig.payMethod')
                              .d('支付方式'),
                          }),
                        },
                      ],
