 ...
 * 3. 将 scripts/icon.js 中的 aliUrl 属性替换为新的图标样式链接；
 * 4. 执行 yarn downIcon 命名下载图标文件
 ...
 * <Icons type="edit" />
 */
export default class mIcons extends React.Component {
  render() {
    const {
      type,
      title,
        ...
        {title ? (
          <Tooltip title={title} {...tipProps}>
            <i
              className={classNames({
                ...
                [className]: !!className,
                [iconFont[`${ORIGIN_ICON_NAME}-${type}`]]: !loading && !!type,
              })}
              style={{
                fontSize: `${size}px`,
                color,
          ...
          <i
            className={classNames({
              ...
              [className]: !!className,
              [iconFont[`hzero-icon-${type}`]]: !loading && !!type,
            })}
            style={{
              color,
              fontSize: `${size}px`,
