...
import { stylePrefix, FieldFlag, noop, getTempFieldName } from '../../util';
...
export default class Selector extends Component<SelectorProps, any> {
  constructor(props) {
    super(props);
    this.state = {
      searchText: '',
  ...
  @Bind()
  handleAllSelected(field: fieldProperties[]) {
    const { optionalFields = [] } = this.state;
    const { onAllSelected = noop, queryDs } = this.props;
    const action = () => {
    ...
    const flag = optionalFields.some((item: fieldProperties) => !isEmpty(queryDs.current?.get(item.multipleFlag === 1 ? getTempFieldName(item.name) :
      item.name)));
    if (flag) {
  ...
  handleSelectField(field: fieldProperties) {
    const { onSelectField = noop, onVisibleChange = noop, queryDs } = this.props;
    const { optionalFields = [] } = this.state;
    const action = () => {
      this.setState({
        optionalFields: optionalFields.map((item: fieldProperties) => {
    ...
    const targetField = optionalFields.find((item: fieldProperties) => item.name === name);
    if (targetField && !isEmpty(queryDs.current?.get(multipleFlag === 1 ? getTempFieldName(name) :
      name))) {
      this.props.onAction(action);
    } else {
      action();
  ...
  @Bind()
  handleClearSelected() {
    const { onClearSelected = noop, queryDs } = this.props;
    const { optionalFields = [] } = this.state;
    const flag = optionalFields.some((item: fieldProperties) => !isEmpty(queryDs.current?.get(item.multipleFlag === 1 ? getTempFieldName(item.name) :
      item.name)));
    const action = () => {
  ...
  render() {
    ...
    let selectableFields: fieldProperties[] = optionalFields;

    if (searchText) {
      selectableFields = selectableFields.filter(
        item => item.label && (item.label as string).includes(searchText)
      );
    }
    // 非固定字段
      ...
      <div>
        <div className={`${stylePrefix}-field-selector-header`}>
          ...
          <TextField
            ...
            onInput={e => this.handleSelectorSearch((e.target as any).value)}
        ...
        <div className={`${stylePrefix}-field-selector-list`}>
          <Menu>
            {selectableFields.map(item => (
              ...
              >
                <CheckBox checked={Boolean(item[FieldFlag.DISPLAY])} />
