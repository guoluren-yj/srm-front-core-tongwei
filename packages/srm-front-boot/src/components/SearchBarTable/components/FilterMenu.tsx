...
import type { filterProperties } from '../util';
import { stylePrefix, FilterType, FilterStatus, noop } from '../util';

interface FilterMenuProps {
  filter: filterProperties; // 筛选器属性
  onSaveFilter: (filter: filterProperties, callback?: Function) => void; // 保存筛选器回调函数
  onRenameFilter: (filter?: object) => void; // 保存筛选器回调函数
}

export default class FilterMenu extends PureComponent<FilterMenuProps, any> {
  constructor(props) {
    super(props);
    this.state = {
      menuVisible: false,
  ...
  setDefaultFilter(filter) {
    const newFilter = {
      ...
      defaultFlag: 1,
      isNewDefault: true, // 标识是新设置的默认筛选器
    };
      ...
      >
        {filter.defaultFlag !== 1 && (
          <Menu.Item key="default">
            {intl.get('srm.filterBar.button.setDefault').d('设为默认')}
          </Menu.Item>
        )}
        {filter.type !== FilterType.SYSTEM && (
          <Menu.Item key="rename">{intl.get('srm.filterBar.button.rename').d('重命名')}</Menu.Item>
        )}
        {filter.type !== FilterType.SYSTEM && (
  ...
  handleMenuVisibleChange(menuVisible) {
    ...
    const menuList = document.getElementsByClassName('c7n-pro-table-search-bar-filter-menu');
    if (!isEmpty(menuList)) {
      for (let i = 0; i < menuList.length; i++) {
        (menuList[i] as HTMLElement).style.display = menuVisible ? 'block' : 'none';
      }
    }
  }
