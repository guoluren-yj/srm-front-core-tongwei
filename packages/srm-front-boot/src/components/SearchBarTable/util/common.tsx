...
import { isString, isEmpty, isNil, isEqual, isArray, keys, isObject } from 'lodash';
...
export interface fieldProperties extends FieldProps {
  ...
  num?: number; // 排序
  customComparisonSet?: string[]; // 查询关系符列表
  ...
  defaultValueCon?: ConValid; // 条件默认值
  ...
  lovEnhanceFlag?: 0 | 1; // 值集字段是否开启高级筛选
  defaultValueJsonFlag?: 0 | 1; // 值集字段是否保存的是json类型默认值
  queryFieldList?: any[]; // 值集字段查询字段数据源
  defaultQueryField?: string; // 值集字段查询字段数据源
}
...
export interface templateConfig {
  cuszTplTemplateCode: string;
  cuszTplStageCode: string;
  cuszTplPageCode: string;
  cuszTplVersion: number;
}
...
export interface searchBarConfigProperties {
  ...
  closeFilterSelector?: boolean; // 关闭筛选器切换功能，默认为false，不关闭
  cuxDefaultFilterFlag?: boolean; // 华创集团专用：部分功能界面不走配置项，只使用默认筛选器
  defaultExpand?: boolean; // 默认展开或收起，关闭筛选器切换功能时默认收起，否则默认展开
  expand?: boolean; // 展开收起状态控制
  ...
  parseUrlParamsType?: ParseUrlParamsType; // 解析url参数的类型
  onlyModelField?: boolean; // 只处理模型字段
  onlySiteField?: boolean; // 只处理平台标准字段
  refreshButton?: boolean; // 是否显示刷新按钮
  fieldDefaultValueType?: FieldDefaultValueType; // 字段默认值类型,standrad-以代码位置,custom-以配置为准
  checkParamLength?: boolean; // get请求时校验参数长度
  paramMaxLength?: number; // get请求参数最大长度
  checkDataSetStatus?: boolean; // 查询前校验ds状态，默认为true
  isTemplate?: boolean; // 是否使用单据样式，默认为false
  loading?: boolean; // 显示loading，默认为false
  templateConfig?: templateConfig; // 单据样式配置
  customizeDsAutoCreate?: boolean; // 自定义组件dataSet自动创建record
  manualQuery?: boolean; // 手动查询
}
export interface searchBarLeftProperties {
  render?: (filter: filterProperties, dataSet: DataSet) => ReactNode; // 自定义渲染函数
}
...
// 解析url参数的类型
export enum ParseUrlParamsType {
  BASE64 = 'base64',
  DECODE = 'decode',
}

export enum FieldDefaultValueType {
  STANDARD = 'standard',
  CUSTOM = 'custom',
}
...
export interface CalculateConfig {
  ...
  searchInputDsRecord: Record;
  queryDsFields: fieldProperties[];
  searchInputDsFields: fieldProperties[];
  ctxParams: CtxParams;
...
export function getLovProFieldName(fieldName) {
  return `${fieldName}_lov`;
}
...
export function getRelatedFilterFields(
  ...
  // num, display, proDefaultFlag 属性保留
...
export const isObjectEqual = (targetObj, sourceObj) => {
  const targetObjTmp = filterNullValueObject(targetObj);
  const sourceObjTmp = filterNullValueObject(sourceObj);
  const objKeys = keys(targetObjTmp);
  if (!isEqual(objKeys, keys(sourceObjTmp))) {
    return false;
  }
  if (objKeys.length < 1) {
    return true;
  ...
  return objKeys.every(item => {
    const targetValue = isNil(targetObjTmp[item]) ? undefined : targetObjTmp[item];
    const sourceValue = isNil(sourceObjTmp[item]) ? undefined : sourceObjTmp[item];
    if (isObject(targetValue) && isObject(sourceValue)) {
      return isObjectEqual(targetValue, sourceValue);
    }
...
export const checkComparsionWithNull = comparsion => {
  return ['NOTNULL', 'ISNULL'].includes(comparsion);
};
...
// 计算带中文的字符串长度 1个中文算两个字符
export const getStringLength = (str) => {
  const reg = /[\u4e00-\u9fa5]/g; // unicode编码中文字符
  return str.replace(reg, 'xx').length;
};

export const getFieldName = ({ record, dataSet, field }: { record?: Record, dataSet?: DataSet, field: any }) => {
  const { name, fieldWidget, multipleFlag, modelCode, fieldCode } = field;
  if (fieldWidget === 'DATE_PICKER' && modelCode && fieldCode) {
    const currentRecord = record || (dataSet && dataSet.current);
    return currentRecord && DATE_RANGE_COMPARISON.includes(currentRecord.get(getComparsionFieldName(name))) ? getTempFieldName(name) : name;
  }
  return multipleFlag === 1 ? getTempFieldName(name) : name;
};

export const DATE_RANGE_COMPARISON = [
  'PAST_ONE_MONTH',
  'PAST_TWO_MONTH',
  'PAST_THREE_MONTH',
  'PAST_SIX_MONTH',
