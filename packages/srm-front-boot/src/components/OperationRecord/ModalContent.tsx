...
function ModalContent(props: ModalContentProps) {
  ...
  const operaTableDs = useMemo(() => new DataSet(operaTableDS({lookupCode, lovParams})), [tableName, tablePk]);
  useEffect(() => {
    if (templateCode) {
        modal.update({
          footer: (okBtn) => {
  ...
  const handleQuery = ({ params = {} }) => {
    try {
      ...
      queryActions({tableName, tablePk, ...params}).then(res => {
        if(getResponse(res)) {
            ...
            if(li.oldValue || li.newValue) {
              return (
                <div className='actionTab-detail-ul'>
                  ...
                  {action.fieldValue !== "CHANGE_REFUSE" &&<span>{intl.get('component.operationRecord.view.action.to').d('改变为')}</span>}
                  {action.fieldValue === "CHANGE_REFUSE" &&<span>{intl.get('component.operationRecord.view.returns.to').d('返回至')}</span>}
  ...
  const renderTabPane = () => {
    ...
    return (
      ...
      >
        <Spin spinning={actionTabLoading || false}>
          <FilterBar
            dataSet={[operaTableDs]}
            onQuery={handleQuery}
            onClear={handleClear}
