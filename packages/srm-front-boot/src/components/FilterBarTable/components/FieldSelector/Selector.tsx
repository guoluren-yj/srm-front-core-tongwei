...
import { stylePrefix, FieldFlag, noop } from '../../util';
...
export default class Selector extends Component<SelectorProps, any> {
  ...
  handleAllSelected(field: fieldProperties[]) {
    ...
    const flag = optionalFields.some((item: fieldProperties) => !isEmpty(queryDs.current?.get(item.name)));
    if (flag) {
  ...
  handleSelectField(field: fieldProperties) {
    ...
    if (targetField && !isEmpty(queryDs.current?.get(name))) {
      this.props.onAction(action);
    } else {
      action();
    }
