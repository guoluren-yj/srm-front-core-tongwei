...
import { Col, DatePicker, Form, Icon, Input, InputNumber, Row, Select } from 'hzero-ui';
...
import { DEFAULT_DATE_FORMAT, EDIT_FORM_ITEM_LAYOUT, EDIT_FORM_ROW_LAYOUT } from 'utils/constants';
...
function FlexSelect({
  ...
  const [selectOptionsDataSource, setSelectOptionsDataSource] = useState(dataSource);
  const [loading, setLoading] = useState(false);
  useEffect(() => {
    if (isString(selectOptionsDataSource)) {
      setLoading(true);
      queryCode({ lovCode: dataSource })
        .then((res = []) => {
          setSelectOptionsDataSource(res && !res.failed ? res : []);
        })
        .finally(() => {
          setLoading(false);
        });
  ...
  return (
    <FormItem {...formItemProps}>
      {getFieldDecorator(`FLEX_${fieldName}`, fieldDecoratorOptions)(
        <Select allowClear {...fieldItemProps} style={{ width: '100％' }}>
          {loading || isString(selectOptionsDataSource) ? (
            <Option key="loading">
              <Icon type="loading" />
            </Option>
          ) : (
            selectOptionsDataSource.map(n => (
...
function FlexLov({
  ...
  const onChange = (...rest) => {
    const { tenantName } = rest[1] || {};
    getFieldDecorator(`FLEX_${fieldName}Meaning`);
    setFieldsValue({
      [`FLEX_${fieldName}Meaning`]: tenantName,
    });

    if (isFunction(fieldItemProps.onChange)) {
      fieldItemProps.onChange(...rest);
    }
  };
  ...
  return (
    <FormItem {...formItemProps}>
      {getFieldDecorator(`FLEX_${fieldName}`, fieldDecoratorOptions)(
        <Lov {...fieldItemProps} onChange={onChange} />
      )}
    </FormItem>
  );
}
...
export function getFlexFormItemComponent(flexFormItemName, form = {}) {
  ...
  const defaultFlexFormItems = {
    ...
    DATE: ({ fieldName, formItemProps = {}, fieldDecoratorOptions = {}, fieldItemProps = {} }) => {
      const momentDate = moment(fieldDecoratorOptions.initialValue, DEFAULT_DATE_FORMAT);
      ...
      return (
        <FormItem {...formItemProps}>
          {getFieldDecorator(`FLEX_${fieldName}`, {
            ...
            getValueFromEvent: dateMoment => {
              return dateMoment && dateMoment.format(DEFAULT_DATE_FORMAT);
            },
            initialValue: valid ? fieldDecoratorOptions.initialValue : undefined,
          })(<DatePicker {...fieldItemProps} format={DEFAULT_DATE_FORMAT} />)}
        </FormItem>
      );
    },
  };
...
export function withFlexFieldsQueryParams(params = {}) {
  return (flexRuleCode, tenantId) =>
    omit(
      {
        ...params,
        flexRuleCode: isArray(flexRuleCode) ? flexRuleCode.join(',') : flexRuleCode,
        tenantId: isNumber(tenantId) ? tenantId : params.tenantId,
      },
...
export function withFormDataFlex(data = {}) {
  ...
  const flex = {};
  flexFieldsKeys.forEach(n => {
    flex[n.split('_')[1]] = !isEmpty(data[n]) || isNumber(data[n]) ? `${data[n]}` : '';
  });
...
export function getFormItemsLayout(formComponentObject = null) {
  ...
  const result = {};
  if (!isEmpty(((formComponentObject && formComponentObject.props) || {}).children)) {
    recurveFormObject(((formComponentObject && formComponentObject.props) || {}).children);
...
export function getCheckedDetailConfigList(config = [], trigger = {}, dataSource = {}) {
  const { getFieldsValue } = trigger;
  const checkedConfig = config.filter(
    o =>
      o.enabledFlag === 1 &&
      isCheckedByFlexCondition(o.flexCondition, {
        ...dataSource,
        ...(isFunction(getFieldsValue) ? getFieldsValue(o.fieldList) || {} : trigger),
      })
  );
...
export function formItemsFactory(
  ...
  const checkedDetailConfigList = getCheckedDetailConfigList(config, triggerForm, dataSource) || [];
  ...
  return checkedDetailConfigList.map((o = {}) => ({
    ...
    component: getFlexFormItemComponent(o.fieldType, targetForm)({
      ...
      fieldItemProps: {
        disabled: o.readableFlag === 1,
        ...(o.fieldType === 'LOV'
          ? { code: o.valueSource, textValue: dataSource[`${o.fieldName}Meaning`] }
          : {}),
        ...(o.fieldItemProps || {}),
      },
...
export function withFlexFieldsFormRender(formComponentObject = null) {
  ...
  return ({
    ...
    if (!isEmpty(((newFormComponentObject && newFormComponentObject.props) || {}).children)) {
      const layoutActionMap = {
        default: () => {
          ...
          if (
            ...
            newFormComponentObject.props &&
            isArray(newFormComponentObject.props.children)
          ) {
...
export function withFlexFieldsTableColumnsRender({
  ...
  let checkedDetailConfigList = [];
  if (isArray(trigger)) {
    trigger.forEach(o => {
      const result = getCheckedDetailConfigList(config, {}, o);
      if (!isEmpty(result)) {
        checkedDetailConfigList = result;
      }
    });
  } else {
    checkedDetailConfigList = getCheckedDetailConfigList(config, trigger) || [];
  }
  // checkedDetailConfigList = checkedDetailConfigList.filter(o => o.readableFlag === 0);
      ...
      ? {
          ...
          render: (value, record) => {
            ...
            if (record._status === 'create') {
              return getFlexFormItemComponent(fieldType, record.$form)({
                fieldName,
                formItemProps,
                fieldDecoratorOptions,
                fieldItemProps,
