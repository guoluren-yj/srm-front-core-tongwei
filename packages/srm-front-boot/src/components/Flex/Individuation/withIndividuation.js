...
import { groupBy, isArray, isBoolean, isEmpty, isNumber, isString, omit } from 'lodash';
...
function getViewFormComponentObject(formComponentObject, flatIndividualizedFormConfig = {}) {
      ...
      ((schema || {}).propsKeys || []).forEach(n => {
        if (!isEmpty(fieldProps, ['row', 'col'])) {
          ...
          if (fieldProps.required) {
            if (isArray(item.props['data-__meta'].rules)) {
              if (item.props['data-__meta'].rules.every(o => !o.required)) {
  ...
  if (
    !isEmpty(
      ((newDefaultFormComponentObject && newDefaultFormComponentObject.props) || {}).children
    )
  ) {
    ...
    if (!isEmpty(flatIndividualizedFormConfig) && !isEmpty(formLayout)) {
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        for (let j = 0; j < rowItems.length; j += 1) {
          ...
          if (
            (newDefaultFormComponentObject.props || {}).children[i] &&
            (newDefaultFormComponentObject.props || {}).children[i].props &&
            isArray((newDefaultFormComponentObject.props || {}).children[i].props.children)
          ) {
...
export default function withIndividuation(options = {}) {
  ...
  return ({ setToolboxDrawerVisible }) => {
    ...
    const newActiveIndividuationPanelTabKeys = [];
    form.forEach(n => {
      if (
        activeIndividuationPanelTabKey.formKeysArray.indexOf(n.individuationCode) > -1
          ? activeIndividuationPanelTabKey.form.indexOf(n.individuationCode) > -1
          : activeIndividuationPanelTabKey.form.some(o => o === `$DEFAULT_${n.individuationCode}`)
      ) {
        newActiveIndividuationPanelTabKeys.push(n.individuationCode);
      }
    });

    table.forEach(n => {
      if (
        activeIndividuationPanelTabKey.tableKeysArray.indexOf(n.individuationCode) > -1
          ? activeIndividuationPanelTabKey.table.indexOf(n.individuationCode) > -1
          : activeIndividuationPanelTabKey.table.some(o => o === `$DEFAULT_${n.individuationCode}`)
      ) {
        newActiveIndividuationPanelTabKeys.push(n.individuationCode);
      }
    });
    ...
    const openIndividuationPanelByTableBtns = (type, code) => {
      ...
      setIndividuationPanelVisible(true);
      setToolboxDrawerVisible(false);
      if (newActiveIndividuationPanelTabKeys.some(o => o === code)) {
        setIndividuationPanelTabKey(code);
        setIndividuationPanelDataType(type);
    ...
    const toolsConfig = {
      panel: (
        <IndividuationPanel
          ...
          activeIndividuationPanelTabKeys={newActiveIndividuationPanelTabKeys}
        ...
        <Collapse bordered={false}>
          {form.map(
            n =>
              // (activeIndividuationPanelTabKey.formKeysArray.indexOf(n.individuationCode) > -1
              //   ? activeIndividuationPanelTabKey.form.indexOf(n.individuationCode) > -1
              //   : activeIndividuationPanelTabKey.form.some(
              //       o => o === `$DEFAULT_${n.individuationCode}`
              //     ))
              newActiveIndividuationPanelTabKeys.some(o => o === n.individuationCode) && (
                <Panel
                  key={n.individuationCode}
                  showArrow={false}
                  forceRender
    ...
    toolsConfig.assignWrapComponentProps = wrapComponentProps => {
      ...
      if (!isEmpty(form)) {
        activedWrapComponentProps.withIndividuationForm = ({
          formComponentObject = null,
          code,
        }) => {
          // if (!componentObjectMap.has(code)) {
          const defaultObject = formComponentObject;
          const defaultSchema = composeFormItems(
            ((defaultObject && defaultObject.props) || {}).children
          );
          const defaultKey = `$DEFAULT_${code}`;
          if (!activeIndividuationPanelTabKey.form.some(o => o === code || o === defaultKey)) {
            const newForm = activeIndividuationPanelTabKey.form.concat(defaultKey);
            setActiveIndividuationPanelTabKey({
              ...activeIndividuationPanelTabKey,
              form: newForm,
            });
          }
          componentObjectMap.set(code, {
            component: ({ individuationSchema, ...rest }) => (
              <PreviewForm
                {...(defaultObject && defaultObject.props)}
      ...
      if (!isEmpty(table)) {
        activedWrapComponentProps.withIndividuationTable = (
          ...
          return TableComponent => {
            ...
            const defaultKey = `$DEFAULT_${individualizedTabelCode}`;
            if (
              !activeIndividuationPanelTabKey.table.some(
                o => o === individualizedTabelCode || o === defaultKey
              )
            ) {
              const newTable = activeIndividuationPanelTabKey.table.concat(defaultKey);
              setActiveIndividuationPanelTabKey({
                ...activeIndividuationPanelTabKey,
                table: newTable,
              });
      ...
      activedWrapComponentProps.withIndividuationByActiveKey = (type, activeKey, keys = []) => {
        const newActiveIndividuationPanelTabKey = {
          ...activeIndividuationPanelTabKey,
          [type]: activeIndividuationPanelTabKey[type]
            .filter(o => keys.indexOf(o) < 0 || o.indexOf('$DEFAULT_') === 0)
            .concat(activeKey),
        };
        if (isEmpty(newActiveIndividuationPanelTabKey[`${type}KeysArray`])) {
          newActiveIndividuationPanelTabKey[`${type}KeysArray`] = keys;
        }
        setActiveIndividuationPanelTabKey(newActiveIndividuationPanelTabKey);
      };
    };
