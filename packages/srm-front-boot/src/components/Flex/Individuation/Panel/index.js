...
export default class Panel extends PureComponent {
  ...
  @Bind()
  footerRender() {
    const { savePersonalityDetailsLoading, clearIndividuationDetailsLoading } = this.state;
    return (
      <Fragment>
        <Button
          onClick={this.clearIndividualizedConfigData}
          loading={clearIndividuationDetailsLoading}
        >
  ...
  clearIndividualizedConfigData() {
    ...
    const actionMap = {
      form: () => {
        this.setState({
          clearIndividuationDetailsLoading: true,
        });
        saveIndividuationFormDetails(individuationPanelKey, individuationPanelDataType, [])
          ...
          .finally(() => {
            this.setState({
              clearIndividuationDetailsLoading: false,
            });
          });
      },
      table: () => {
        ...
        deleteTableConfig(newTableConfigDataSource)
          .then(res => {
            if (res && res.failed) {
              throw res;
            } else {
  ...
  render() {
    const {
      ...
      individuationPanelKey,
      activeTableProps = {},
      activeIndividuationPanelTabKeys,
    } = this.props;
        ...
        >
          {tabPanes.map(o => {
            ...
            return (
              <TabPane
                style={{ minHeight: 700 }}
                disabled={activeIndividuationPanelTabKeys.indexOf(o.key) < 0}
                tab={o.title}
                key={o.key}
              >
