...
function MoreVersion(props = {}) {
  ...
  const { relTableSelectVersion = {} } = props;

  useEffect(() => {
    if (props.lineId && isEmpty(relTableSelectVersion)) {
      optionPageDs.setQueryParameter('lineId', props.lineId); // this.props.lineId
      optionPageDs.query().then((res) => {
        optionPageDs.loadData(res); // 异步
  ...
  const selectionRelTableDs = new DataSet({
    ...
    fields: [
      ...
      {
        name: 'scriptContent',
        bind: `historyDs.${relTableSelectVersion?.textObj?.name}`,
      },
    ],
  });
  ...
  const changeSelect = (record) => {
    ...
    const scriptCode =
      record.get(
        currentDsType !== 'relTable' ? 'scriptContent' : relTableSelectVersion?.textObj?.name
      ) || '';
    const currentCode =
