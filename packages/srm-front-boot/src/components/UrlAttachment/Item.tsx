...
import { Button, Tooltip, Picture, Icon, Progress } from 'choerodon-ui/pro';
...
import type { PictureForwardRef, PictureProps } from 'choerodon-ui/pro/lib/picture/Picture';
...
export interface ItemProps {
  attachment: AttachmentFile & { oriExt?: string; };
...
const Item: FunctionComponent<ItemProps> = function Item(props) {
  const {
    ...
    bucketDirectory, storageCode, isCard, provided, readOnly, restCount, draggable, index, hidden, isPublic, showSize, disabled,
    attachmentLimit: propsAttachmentLimit, previewData, downloadData,
  } = props;
  ...
  const attachmentConfig: any = getConfig('attachment');
  ...
  if (attachmentLimit) {
    limitConfig = { ...limitConfig, ...(attachmentLimit({attachment, isPublic}) || {}) };
  }
  let previewUrl: string | Function | undefined;
  let downloadUrl: string | Function | undefined;
  if (limitConfig.preview) {
    previewUrl = getPreviewUrl ? getPreviewUrl({ attachment, bucketName, bucketDirectory, storageCode, isPublic, previewData }) : url;
  }
  ...
  const handleOpenPreview = useCallback(async () => {
    if (isFunction(previewUrl)) {
      ...
      if (!(previewUrl as any).isPicture && isString(result)) {
        const target =
          props.previewTarget ||
          (globalPreviewTarget === "_blank" ? undefined : globalPreviewTarget);
  ...
  const renderImagePreview = (): ReactNode => {
    if (listType === 'text') {
      ...
      if (isPicture || isSrcIcon) {
        const pictureProps: PictureProps & { promiseSrc? } = {};
  ...
  const renderButtons = (): ReactNode => {
    ...
    if (!readOnly && !disabled && limitConfig.remove) {
      const rmProps = {
        className: classnames(`${prefixCls}-icon`),
