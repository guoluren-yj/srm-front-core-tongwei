...
export default class CustomTable extends React.Component {
  ...
  render() {
    ...
    const { columns, tableCode, customizeTable = (code, comp) => comp, ...restProps } = this.props;

    // 用户个性化 Table 菜单的国际化
    const filterBarLocale = {
      display: intl.get('hzeo.common.userUI.view.filterBar.display').d('显示'),
      field: intl.get('hzeo.common.userUI.view.filterBar.field').d('字段'),
      fixedLeft: intl.get('hzeo.common.userUI.view.filterBar.lock').d('lock'),
      orderSeq: intl.get('hzeo.common.userUI.view.filterBar.orderSeq').d('排序'),
    };
    return init
      ? customizeTable(
          { code: tableCode },
