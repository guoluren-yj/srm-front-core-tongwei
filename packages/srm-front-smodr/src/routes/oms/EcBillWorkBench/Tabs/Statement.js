...
function Statement(props) {
  ...
  function renderColor(record) {
    ...
    if (status.match('SUCCESS')) {
      return 'rgba(71,184,129,0.10)'; // 绿
    } else {
      return 'rgba(245,99,73,0.10)'; // 红
    }
  }
  ...
  return (
    <div style={{ height: 'calc(100vh - 260px)' }}>
      <SearchBarTable
        ...
        dataSet={props.stateDS}
        columns={columns}
        customizedCode="SMODR.EC.BILL.WORKBENCH.STATE.QUERY"
        searchCode="SMOP.EC.RECORD.EC_BILL_BAR"
        searchBarConfig={{
          left: {
            render: () => (
              <TextFieldPro
                ds={props.stateDS}
                placeholder={intl
                  .get('smodr.ecBill.model.searchBillId')
                  .d('请输入电商对账单编码查询')}
                name="orderQuery"
                onRef={(ref) => {
