...
export default class FilterForm extends React.Component {
  ...
  handleSearch(params, name, lovParam) {
    ...
    } else if (name === 'freightTypeCode') {
      this.props.form.setFieldsValue({ freightTypeCode: params });
  ...
  render() {
    const { form, orderTypes, freightTypes, companyList, purchaseCompanyList = [] } = this.props;
      ...
      <React.Fragment>
        <div style={{ display: 'flex', marginBottom: '12px' }}>
          <Form>
            <Form.Item>
              {getFieldDecorator('aggregateQuery')(
                <Input
                  style={{ width: '200px' }}
                  placeholder={intl.get('smodr.acceptOrder.model.newTips').d('订单编码｜下单人')}
          ...
          <Form.Item>
            ...
            >
              {intl.get('smodr.acceptOrder.view.query').d('查询')}
            </Button>
            <Button style={{ borderRadius: '1px' }} onClick={() => this.setState({ flag: !flag })}>
              {intl.get('smodr.acceptOrder.view.advanceQuery').d('高级筛选')}
        ...
        {flag && (
          <Form className={styles['search-form']}>
              ...
              {getFieldDecorator('companyId')(
                ...
                >
                  ...
                  {companyList.length > 3 && (
                    ...
                    >
                      {intl.get('smodr.acceptOrder.view.more').d('更多')}
                    </Lov>
                  )}
                </Radio.Group>
              )}
            ...
            <Form.Item
              label={intl.get('smodr.frightLine.model.purchaseCompanyName').d('采购方')}
              {...formLineLayout}
            >
            ...
            <Form.Item
              label={intl.get('smodr.acceptOrder.model.freightTypeCode').d('运费类型')}
              {...formLineLayout}
            >
              {getFieldDecorator('freightTypeCode')(
                <Radio.Group
                  defaultValue="1"
                  buttonStyle="solid"
                  onChange={(e) => this.handleSearch(e.target.value || '', 'freightTypeCode')}
                >
                  <Radio.Button>{intl.get('smodr.acceptOrder.view.all').d('全部')}</Radio.Button>
                  {freightTypes &&
            ...
            <Form.Item
              label={intl.get('smodr.frightLine.model.cecFromMeaning').d('运费来源')}
              {...formLineLayout}
            >
            ...
            {/* <Form.Item label="规则类型" {...formLineLayout}>
              {getFieldDecorator('orderTypeCode')(
                <Radio.Group
                  buttonStyle="solid"
                  onChange={e => this.handleSearch(e.target.value || '', 'orderTypeCode')}
            ...
            <Row>
              <Col span={12}>
                <Form.Item
                  label={intl.get('smodr.acceptOrder.model.queryDateCode').d('创建时间')}
                  {...formDateLineLayout}
                >
                  {getFieldDecorator('queryDateCode')(
                    ...
                    >
                      <Radio.Button>
                        {intl.get('smodr.acceptOrder.view.all').d('全部')}
                      </Radio.Button>
                      <Radio.Button value="withinOneWeek">
                        {intl.get('smodr.acceptOrder.view.oneWeek').d('一周内')}
                      </Radio.Button>
                      <Radio.Button value="withinOneMonth">
                        {intl.get('smodr.acceptOrder.view.oneMonth').d('一个月内')}
                      </Radio.Button>
                      <Radio.Button value="withinThreeMonth">
                        {intl.get('smodr.acceptOrder.view.threeMonth').d('三个月内')}
                      </Radio.Button>
                      <Radio.Button value="withinOneYear">
                        {intl.get('smodr.acceptOrder.view.oneYear').d('一年内')}
                      </Radio.Button>
                      <Radio.Button value="all">
                        {intl.get('smodr.acceptOrder.view.custom').d('自定义')}
                      </Radio.Button>
                    </Radio.Group>
                  )}
                </Form.Item>
              ...
              {!!dateFlag && (
                <Col span={12}>
                  <div className="date-line">
                    <Form.Item
                      label={intl.get('smodr.acceptOrder.view.from').d('从')}
                      {...formLayout}
                    >
                    ...
                    <Form.Item
                      label={intl.get('smodr.acceptOrder.view.to').d('至')}
                      {...formLayout}
                    >
                    ...
                    <Button style={{ margin: '6px 12px' }} onClick={this.resetData}>
                      {intl.get('smodr.acceptOrder.view.reset').d('重置')}
                    </Button>
