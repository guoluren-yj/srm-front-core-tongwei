...
const TreeShow = (props) => {
  const { dataSource = [], models, treeShowRef, rightFieldsRef } = props;
  ...
  const renderContent = ({ element }) => {
    const { tableName, pageType } = props;
    ...
    return (
      <div
        ...
        style={{
          border: tableName === element.dstTable ? '2px solid rgba(54,194,207,1)' : '',
          borderRadius: '2px',
          background:
            (element && element.subscribed) || pageType === 'platform' ? '#fff' : '#f8f8f8',
        }}
      >
        <TreeSlot {...treeSlot} />
      </div>
  ...
  const setCellStyle = ({ element }) => {
    const heightSum = element.srcColumn && element.dstColumn ? cellHeight : 66;
    // 单元样式
    return {
    ...
    return (
      <div
        style={{
          width: lineWidth,
          height: `${
            findYaxisNumber(element) * (cellHeight + cellMarginTop + cellPaddingTop) -
            cellHeight * 0.5 +
            2
          }px`,
          backgroundColor: lineColor,
  ...
  const getElement = (_dataSource, level) => {
    const arr = [];

    (_dataSource || []).forEach((element, i) => {
      if (level > 1) {
        element.parentId = element.businessObjectId;
      }
      arr[i] = (
        <div
          key={`${element?._token}`}
          ...
          style={{
            ...
            // background: element.subscribed ? '#fff' : 'transparent',
          }}
        >
          {/* 关联关系名称 */}
          ...
          element.relBusinessObjectFieldName ? (
            <Tooltip
              placement="top"
              title={`${intl.get('sdps.boComposition.fieldInfo.relation.prefix').d('关系名：')}${
                element?.relBusinessObjectFieldName
              }`}
            >
          ...
          {/* 递归遍历子节点 */}
          {element?.businessObjectRelationList?.length > 0
            ? getElement(element?.businessObjectRelationList, level + 1)
            : null}
        </div>
      );
    });
  ...
  const renderTree = ({ _dataSource = [] }) => {
    ...
    const arrList = getElement(_dataSource, 1);
    // 先遍历多个视图数组 在遍历视图内模型树
    ...
    return (
      <>
        {(arrList || []).map((item, index) => {
          return (
            <div
              key={`tree-out${_dataSource[index].relBusinessObjectId}`}
              style={{
            ...
            >
              <p style={{ fontSize: '14px', fontWeight: 600 }}>
                {_dataSource[index].businessObjectName}
              </p>
              <div>{item}</div>
            </div>
          );
...
export default formatterCollections({ code: ['sdps.boComposition'] })(TreeShow);
