...
import { queryRuleManageConfig, saveRuleManageConfig } from '@/services/ruleManagesService';
import {
  getBasicParamDs,
  getParamDs,
  getAddParamLovDs,
...
function Detail(props = {}) {
  ...
  useEffect(() => {
    if (metaDefinitionId) {
      queryData(metaDefinitionId, tenantId);
    }
      ...
      .then((response) => {
        if (response) {
          ...
          saveRuleManageConfig(payload)
            ...
            .finally(() => handleSpinning(false)); // catch会导致后端回传的错误无法终止加载事件，应使用finally
        } else {
          handleSpinning(false);
        }
      })
