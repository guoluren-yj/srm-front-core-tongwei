...
import { initIndexDimesion } from '@/services/ruleManagesService';
...
function Detail(props = {}) {
  const { tenantId, ruleManagementHeaderId: id, activeKey: backKey } = qs.parse(
    props.location.search.substr(1)
  ); // 截取url上面传递参数
  const [currentTabKey, handleCurrentTabKey] = useState('basic'); // 控制tab页的当前key
  const [ruleManagementHeaderId, setRuleManagementHeaderId] = useState(id || undefined); // 主键
  ...
  useEffect(() => {
    if (ruleManagementHeaderId) {
      ...
      // 对于平台级查看租户级的情况，添加指标的表格需要传参租户ID
      indexAddDs.setQueryParameter('tenantId', tenantId !== '0' ? tenantId : undefined);
      Promise.all([
        basicParamDs.query(1, { ruleManagementHeaderId }).then((res) => {
          ...
          handleType(basicParamDs.current.get('type')); // 规则查询成功则将type值存入state
        }),
    ...
    basicParamDs.validate().then((flag) => {
            ...
            basicParamDs
              ...
              .then((result) => {
                ...
                if (getResponse(result)) {
                  setRuleManagementHeaderId(
                    (result?.content?.length ?? 0) !== 0 &&
                      result?.content[0]?.ruleManagementHeaderId
                  );
                }
              })
              .catch(() => {
      ...
      basicParamDs
        ...
        .then((res) => {
          ...
          if (getResponse(res)) {
            setRuleManagementHeaderId(
              (res?.content?.length ?? 0) !== 0 && res?.content[0]?.ruleManagementHeaderId
            );
  ...
  const handleSelectIndex = (modal) => {
    ...
    const promiseArr = [];
    indexAddDs.selected.forEach((item) => {
      const { serviceCode, indexType } = item.get(['serviceCode', 'indexType']);
      // 对于非转换参数需要进行指标维度初始化
      if (indexType !== 'transform_parameter') {
        // 使用服务编码初始化维度
        promiseArr.push(
          initIndexDimesion({ serviceCode }).then((res) => {
            // 查询成功则存入dimensionality字段,否则就没有指标
            item.set('dimensionality', getResponse(res) ? JSON.stringify(res) : '[]');
            // 取得其指标编码
            const indexCode = item.get('indexCode');
            // 将记录加入指标DS,并添加计算指标字段
            indexMessageDs.create({
  ...
  const handlePageBack = () => {
    ...
    basicParamDs.setState('ruleManagementHeaderId', undefined);
  };
