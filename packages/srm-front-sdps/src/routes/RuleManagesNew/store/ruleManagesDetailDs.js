...
function getBasicParamDs() {
  return {
    ...
    fields: [
      ...
      {
        name: 'emptyRetStrategy',
        type: 'string',
        lookupCode: 'SDPS.RULE_MANAGE.EMPTY_RET_STRATEGY',
        label: intl.get(`${detailPrompt}.ruleManagesDetail.emptyRetStrategy`).d('空值默认返回'),
        required: true,
      },
      {
        name: 'description',
        type: 'intl',
        label: intl.get(`${detailPrompt}.ruleManagesDetail.ruleDescription`).d('参数描述'),
        pattern: /^(.){0,180}$/,
        defaultValidationMessages: {
          patternMismatch: intl
            .get(`${detailPrompt}.ruleManagesDetail.overMaximum`)
    ...
    transport: {
      read: ({ data }) => {
        if (data?.ruleManagementHeaderId) {
          return {
            url: `${SRM_DATA_PROCESS}/v1/rule-management-headers/${data.ruleManagementHeaderId}`,
            method: 'GET',
          };
        }
      },
  ...
  return {
    ...
    transport: {
      read: ({ dataSet }) => {
        ...
        return {
          url: `${SRM_DATA_PROCESS}/v1/rule-management-lines/headerId/${ruleManagementHeaderId}`,
          method: 'GET',
        };
      },
      submit: ({ data, dataSet }) => {
        ...
        const postData = cutData.map((item) => {
          return { ruleManagementHeaderId, tenantId, ...item };
        });
        return {
          url: `${SRM_DATA_PROCESS}/v1/rule-management-lines`,
          method: 'POST',
          data: postData,
        };
      },
      destroy: ({ data, dataSet }) => {
        ...
        return {
          url: `${SRM_DATA_PROCESS}/v1/rule-management-lines`,
          method: 'DELETE',
          data: { ruleManagementLineId, tenantId, ruleLineCode },
        };
      },
    },
  };
...
function getIndexAddDs() {
  return {
    ...
    transport: {
      read: () => {
        return {
          url: `${SRM_DATA_PROCESS}/v1/data-service/indexInfo`,
          method: 'GET',
        };
      },
    },
  ...
  return {
    ...
    transport: {
      read: ({ dataSet }) => {
        ...
        const code = dataSet.getState('code');
        const fullPathCode = dataSet.getState('fullPathCode');
        return {
      ...
      submit: ({ data, dataSet }) => {
        return {
          url: `${SRM_DATA_PROCESS}/v1/cnf-actions?tenantId=${dataSet.getState('currentTenantId')}`,
          method: 'POST',
          data: data[0],
        };
      },
      destroy: ({ data, dataSet }) => {
        return {
          ...
          data: {
            ...data[0],
            tenantId: dataSet.getState('currentTenantId'),
          },
        };
      },
    },
