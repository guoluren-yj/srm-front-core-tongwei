...
export default function IndexDimensionTable(props = {}) {
  const { tableDs, buttons, dataSet, defaultIndex, onPrev = () => {}, onNext = () => {} } = props;
  ...
  useEffect(() => {
    ...
    if (dataSet?.length >= 0 && defaultIndex === dataSet.length - 1) {
  ...
  const handleNext = () => {
    const indexNum =
      dataSet?.length >= 0 && recordIndex >= dataSet.length - 1
        ? dataSet.length - 1
        : recordIndex + 1;
    ...
    if (dataSet?.length >= 0 && indexNum >= dataSet?.length - 1) {
      setNextCanClick(true);
    } else {
      setNextCanClick(false);
    }
