...
import { fetchAudit, fetchAuditLine } from '@/services/auditCenterService';
...
const AuditCenter = (props) => {
  ...
  useEffect(() => {
    queryIdpValue('SDPS.AUDIT_STATUS').then((res) => {
      if (res && res.length) {
        setStatusList(res);
      }
    });
   ...
   */
  const handleInput = (e) => {
    rejectReson = e?.target?.value?.trim() ?? '';
    setWords(rejectReson);
  };
      ...
      <Content>
        ...
        >
          <Table
            ...
            dataSet={listDS}
            customizable
            customizedCode="SDPS.AUDIT_CENTER_LIST"
