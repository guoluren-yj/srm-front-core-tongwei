...
const MetadataDS = () => ({
  transport: {
    read: ({ data, params }) => {
      return {
        url: `${SRM_DATA_PROCESS}/v1/meta-table/${data.metaId}`,
        params: {
          ...data,
          ...params,
        },
  ...
  fields: [
    {
      name: 'mapObj',
      type: 'object',
      lovCode: 'SDPS.DATA_TABLE_COLUMNS',
      textField: 'name',
      noCache: true,
      dynamicProps: {
        lovQueryAxiosConfig: ({ record }) => {
          return {
            url: `${SRM_DATA_PROCESS}/v1/meta-table/columns?metaId=${record.get('sourceTableId')}`,
            method: 'GET',
          };
        },
      },
...
const ColumnPropDS = () => ({
  transport: {
    read: ({ data, params }) => {
      return {
        url: `${SRM_DATA_PROCESS}/v1/meta-table/columns`, // /${data.metaId}
        params: {
          ...data,
          ...params,
        },
...
const TenantSubscripDS = () => ({
  transport: {
    read: ({ data, params }) => {
      return {
        url: `${SRM_DATA_PROCESS}/v1/data-table-manages/link-tenants`,
        params: {
          ...data,
          ...params,
        },
...
const SubHistoryDS = () => ({
  ...
  transport: {
    read: ({ data, params }) => {
      return {
        url: `${SRM_DATA_PROCESS}/v1/data-table-manages/table-op-history`,
        params: {
          ...data,
          ...params,
        },
