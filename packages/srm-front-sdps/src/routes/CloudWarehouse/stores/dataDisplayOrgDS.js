...
const DataDisplayDS = () => ({
  transport: {
    read: ({ data, params }) => {
      return {
        url: `${SRM_DATA_PROCESS}/v1/${organizationId}/monitor-analysis/task-details-list`,
        params: {
          ...data,
          ...params,
        },
  ...
  fields: [
    {
      ...
      name: 'status',
      type: 'string',
      lookupCode: 'SDPS.DATA_SYNC_STATUS',
    },
    {
      ...
      type: 'string',
      // help:
      //   '增量同步：每天将SRM数据库中的新增及变化数据同步到客户指定的数据库。\n全量同步：每天将SRM数据库中的全部数据同步一份到客户指定的数据库',
    },
    {
...
const FileSyncDS = () => ({
  transport: {
    read: ({ data, params }) => {
      return {
        url: `${SRM_DATA_PROCESS}/v1/${organizationId}/monitor-analysis/seep-invoke/${data.ruleCode}`,
        params: {
          ...data,
          ...params,
        },
  ...
  fields: [
    ...
    {
      label: intl.get(`sdps.cloudWarehouse.model.fileSyncMode`).d('文件同步方式'),
      name: 'sync_type',
      type: 'string',
    },
    {
...
export { DataDisplayDS, FileSyncDS };
