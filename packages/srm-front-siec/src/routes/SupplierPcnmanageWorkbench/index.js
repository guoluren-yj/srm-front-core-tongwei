...
@withCustomize({
  unitCode: [
    'SIEC.SUPPIER_WORKSPACE_LIST.ALL_LIST',
    'SIEC.SUPPIER_WORKSPACE_LIST.LIST',
    'SIEC.SUPPIER_WORKSPACE_LIST.BTNS',
    'SIEC.SUPPIER_WORKSPACE_LIST.ALL.BTNS',
  ],
})
@connect(({ // loading = {},
  pcnmanageWorkbench }) => ({
  // batchApprovePassLoading: loading.effects['pcnmanageWorkbench/batchApprovePass'],
  // batchApproveRefusedLoading: loading.effects['pcnmanageWorkbench/batchApproveRefused'],
  pcnmanageWorkbench,
}))
...
export default class PcnmanageWorkbench extends Component {
  ...
  fetchTabData = (statusConfigId) => {
    fetchSupTabDataList(statusConfigId).then((res) => {
      if (getResponse(res)) {
        this.setState({
          tabClause: {
  ...
  async handleHeaderBtnAffairHandle(operationItem) {
    ...
    if (initialFlag) {
      this.props.history.push({
        pathname: `/siec/supplier-pcnmanage-workbench/detail`,
        search: `statusConfigId=${statusConfigId}&operationCode=${operationCode}`,
      });
    } else {
  ...
  render() {
          ...
          },
          {
            ...
            child: (name) => (
              <ExcelExportPro
                ...
                templateCode="SRM_C_SRM_SIEC_PCN_HEADER_SUPPLIER_WAITING_EXPORT"
                requestUrl={`${SRM_SIEC}/v1/${organizationId}/supplier/pcn-headers/exportHeaderInfo/new`}
                queryParams={{
                  pcnHeaderIds:
                    selectedRows.length > 0 ? selectedRows.map((n) => n?.pcnHeaderId) : [],
                  statusConfigId,
                  customizeUnitCode: 'SRM_C_SRM_SIEC_PCN_HEADER_SUPPLIER_WAITING_EXPORT',
          ...
          },
          {
            ...
            child: (name) => (
              <ExcelExport
                ...
                requestUrl={`${SRM_SIEC}/v1/${organizationId}/supplier/pcn-headers/exportHeaderInfo`}
                queryParams={{
                  pcnHeaderIds:
                    selectedRows.length > 0 ? selectedRows.map((n) => n?.pcnHeaderId) : [],
                  statusConfigId,
                  customizeUnitCode: 'SIEC.SUPPIER_WORKSPACE_LIST.WAIT_QUERY',
                  ...filterNullValueObject(tableDs.queryDataSet?.current?.toData()),
                }}
              />
            ),
          ...
          },
          {
            ...
            child: (name) => (
              <ExcelExportPro
                ...
                templateCode="SRM_C_SRM_SIEC_PCN_HEADER_SUPPLIER_ALL_EXPORT"
                requestUrl={`${SRM_SIEC}/v1/${organizationId}/supplier/pcn-headers/exportDetail/new`}
                queryParams={{
          ...
          },
          {
            ...
            child: (name) => (
              <ExcelExport
                ...
                requestUrl={`${SRM_SIEC}/v1/${organizationId}/supplier/pcn-headers/exportDetail`}
                queryParams={{
                  pcnLineIds: selectedRows.length > 0 ? selectedRows.map((n) => n?.pcnLineId) : [],
                  statusConfigId,
                  customizeUnitCode: 'SIEC.SUPPIER_WORKSPACE_LIST.WAIT_QUERY',
      ...
      return (
        <Fragment>
          {activeKey !== 'all'
            ? customizeBtnGroup(
                {
                  code: 'SIEC.SUPPIER_WORKSPACE_LIST.BTNS',
                  pro: true,
                },
            ...
            : customizeBtnGroup(
                {
                  code: 'SIEC.SUPPIER_WORKSPACE_LIST.ALL.BTNS',
                  pro: true,
                },
    ...
    return (
      <Fragment>
        <Header
          title={intl
            .get('siec.pcnmanageWorkbench.view.title.supplierPcnUpdateWorkbench')
            .d('销售方变更工作台')}
        >
            ...
            >
              <div style={{ height: 'calc(100vh - 245px)' }}>
                {statusConfigId &&
                  customizeTable(
                    {
                      code: 'SIEC.SUPPIER_WORKSPACE_LIST.LIST',
                    },
                    <SearchBarTable
                      boxSizing="wrapper"
                      style={{ maxHeight: `calc(100% - 22px)` }}
                      searchCode="SIEC.SUPPIER_WORKSPACE_LIST.WAIT_QUERY"
                      dataSet={tableDs}
                      columns={columns}
                      cacheState
                      searchBarConfig={{
            ...
            >
              <div style={{ height: 'calc(100vh - 245px)' }}>
                {statusConfigId &&
                  customizeTable(
                    {
                      code: 'SIEC.SUPPIER_WORKSPACE_LIST.ALL_LIST',
                    },
                    <SearchBarTable
                      boxSizing="wrapper"
                      style={{ maxHeight: `calc(100% - 22px)` }}
                      searchCode="SIEC.SUPPIER_WORKSPACE_LIST.ALL_QUERY"
                      dataSet={tableAllDs}
                      columns={allColumns}
                      cacheState
                      searchBarConfig={{
