...
const pcnmanageWorkbenchDS = ({ queryFlag = '0', id } = {}) => ({
  ...
  transport: {
    read: ({ data }) => {
      ...
      if (statusConfigId) {
        return {
          url: `${SRM_SIEC}/v1/${organizationId}/supplier/pcn-headers/list`,
          method: 'GET',
          data: {
            ...data,
            queryFlag,
            customizeUnitCode:
              queryFlag === '0'
                ? 'SIEC.SUPPIER_WORKSPACE_LIST.WAIT_QUERY,SIEC.SUPPIER_WORKSPACE_LIST.LIST'
                : 'SIEC.SUPPIER_WORKSPACE_LIST.ALL_QUERY,SIEC.SUPPIER_WORKSPACE_LIST.ALL_LIST',
          },
        };
      }
    },
