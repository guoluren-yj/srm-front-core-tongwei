...
@withCustomize({
  unitCode: [
    'SIEC.SUPPIER_PCN_MANAGEWORK_BENCH_DETAI.HEADER',
    'SIEC.SUPPIER_PCN_MANAGEWORK_BENCH_DETAI.LINE',
    'SIEC.SUPPIER_PCN_MANAGEWORK_BENCH_DETAI.ATTACHMENT',
    'SIEC.SUPPIER_PCN_MANAGEWORK_BENCH_DETAI.BTNS',
  ],
})
@cuxRemote(
  {
  ...
  changePage(relationPageValue) {
    ...
    if (relationPageValue) {
      ...
      if (relationPageValue.includes('/search')) {
        this.setState({
          pageFlags: { searchFlag: true, approveFlag: false, sqeApproveFlag: false },
        });
      } else if (relationPageValue.includes('/approve')) {
        this.setState({
          pageFlags: { searchFlag: false, approveFlag: true, sqeApproveFlag: false },
        });
      } else if (relationPageValue.includes('/sqe-approve')) {
        this.setState({
          pageFlags: { searchFlag: false, approveFlag: false, sqeApproveFlag: true },
        });
      } else {
        this.setState({
          pageFlags: { searchFlag: false, approveFlag: false, sqeApproveFlag: false },
          operationCode: null,
        });
      }
    ...
    const HeaderButtons = () => {
      return (
        <>
          {customizeBtnGroup(
            {
              code: 'SIEC.SUPPIER_PCN_MANAGEWORK_BENCH_DETAI.BTNS',
              pro: true,
            },
            <DynamicButtons buttons={btns} />
          )}
          {!path.includes('/pub') && <Fragment>{this.renderButton(newPageOperationList)}</Fragment>}
        </>
      );
    };
      ...
      <Fragment>
        <Spin dataSet={this.formDs}>
          <Header
            ...
            backPath="/siec/supplier-pcnmanage-workbench/list"
            onBack={() => this.onBack()}
          >
            <HeaderButtons />
          </Header>
