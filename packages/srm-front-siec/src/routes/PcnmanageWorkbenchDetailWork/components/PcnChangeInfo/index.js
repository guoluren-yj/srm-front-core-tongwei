...
export default class PcnChangeInfo extends Component {
  ...
  @Bind()
  getColumns() {
    const { pageFlags } = this.props;
    const { searchFlag, approveFlag, sqeApproveFlag } = pageFlags;

    const currentEditableFlag = false;
    const columns = {
      base: [
        {
          name: 'lineNum',
  ...
  buttons() {
    ...
    const { searchFlag } = pageFlags;
    const creatFlag = !(searchFlag || false); // 新建页面标识
  ...
  render() {
    ...
    return (
      <Fragment>
        {customizeTable(
          {
            code: 'SIEC.PCN_MANAGEWORK_BENCH_DETAI.WORKS.LINE',
            dataSet: tableDs,
            custLoading: false,
