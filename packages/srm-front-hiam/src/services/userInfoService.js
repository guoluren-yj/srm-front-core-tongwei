...
import { HZERO_FILE, HZERO_IAM, HZERO_PLATFORM, API_HOST } from 'utils/config';
import { getCurrentOrganizationId, isTenantRoleLevel, getUserOrganizationId } from 'utils/utils';
...
export async function userInfoUpdatePasswordPre({ password, originalPassword, businessScope }) {
  return request(`${HZERO_IAM}/hzero/v1/users/password/pre`, {
    method: 'PUT',
    body: {
      originalPassword,
      password,
 ...
 * @param {String} params.captchaKey 当前验证码的 key
 */
export async function userEmailInfoValidatePre(params) {
  return request(`${HZERO_IAM}/hzero/v1/users/self/captcha/pre-validate`, {
    method: 'GET',
    query: {
      ...params,
      businessScope: 'self',
 ...
 * 向旧邮箱发送验证码
 */
export async function userInfoPostOldEmailCaptchaNew(params) {
...
/**
 * 绑定新邮箱/手机号后，调用切换租户的接口并刷新，以保证拿到用户最新的手机号和邮箱
 * @param {String} tenantId 当前租户Id
 */
...
export async function queryLanguageData() {
  return request(`${HZERO_PLATFORM}/v1/languages/list/filter`, {
    method: 'GET',
  });
}
 ...
 * 管理员申请提交
 * @async
 * @function submitApplicantManager
 * @returns fetch Promise
 */
export async function submitApplicantManager(params) {
  const organizationId = getUserOrganizationId();
  return request(`${SRM_PLATFORM}/v1/${getCurrentOrganizationId()}/enterprise-role-applys/submit`, {
    method: 'POST',
    body: { ...params, tenantId: organizationId },
  });
}
 ...
 * 管理员申请提交状态
 * @async
 * @function submitApplicantManagerStatus
 * @returns fetch Promise
 */
export async function submitApplicantManagerStatus(params) {
  return request(
    `${SRM_PLATFORM}/v1/${getCurrentOrganizationId()}/enterprise-role-applys/find-one`,
    {
      method: 'GET',
      query: params,
    }
...
export async function uploadAvatar(params) {
  ...
  formData.append('attachmentUUID', params.uuid);
  return request(
    `${HZERO_FILE}/v1/${
      isTenantRoleLevel() ? `${params.organizationId}/` : ''
...
export async function saveAvatar(params) {
  return request(`${HZERO_IAM}/hzero/v1/users/avatar-with-token`, {
    method: 'PUT',
    query: params,
  });
}
...
export async function getCheckCode(params) {
  const {
    userAuthInfo: { userId, authMethod },
    userAuthInfo,
  } = params;
  return request(
    authMethod === 'bankCard'
      ? `${HZERO_IAM}/v1/${getCurrentOrganizationId()}/user-auth-info/get-auth-code/${userId}`
      : `${HZERO_IAM}/v1/${getCurrentOrganizationId()}/user-auth-info/phone-verify`,
    {
      method: 'POST',
      body: { ...userAuthInfo, authType: 'ESIGN' },
    }
  );
}

// 校验E签宝验证码
export async function checkPhoneCode(userAuthInfo) {
  const { authCode, authMethod } = userAuthInfo;
  return request(
    authMethod === 'bankCard'
      ? `${HZERO_IAM}/v1/${getCurrentOrganizationId()}/user-auth-info/check-auth-code/${authCode}`
      : `${HZERO_IAM}/v1/${getCurrentOrganizationId()}/user-auth-info/check-sms-verification-code`,
    {
      method: 'POST',
      body: userAuthInfo,
    }
...
// 注销认证
export async function dateleAuth(userAuthInfo) {
  const { userAuthId } = userAuthInfo;
  return request(
    `${HZERO_IAM}/v1/${getCurrentOrganizationId()}/user-auth-info/cancel-auth/${userAuthId}`,
    {
      method: 'POST',
      query: userAuthInfo,
    }
...
// 获取认证详细信息
export async function getCurrentInfo(userAuthInfo) {
  const { userId } = userAuthInfo;
  return request(
    `${HZERO_IAM}/v1/${getCurrentOrganizationId()}/user-auth-info/auth-detail/${userId}`,
    {
      method: 'GET',
      query: userAuthInfo,
    }
...
// 获取法大大实名调整路由
export async function getFddAuth(userAuthInfo) {
  // const { userId } = userAuthInfo;
  return request(`${HZERO_IAM}/v1/${getCurrentOrganizationId()}/user-auth-info/person-verify-url`, {
    method: 'POST',
    body: { ...userAuthInfo },
    responseType: 'text',
  });
...
// 获取法大大修改手机号url
export async function getFddPhone(userAuthInfo) {
  // const { userId } = userAuthInfo;
  return request(`${HZERO_IAM}/v1/${getCurrentOrganizationId()}/user-auth-info/modify-phone-url`, {
    method: 'POST',
    body: { ...userAuthInfo },
    responseType: 'text',
  });
...
// 契约锁实名认证
export async function getQYSAuth(userAuthInfo) {
  return request(
    `${HZERO_IAM}/v1/${getCurrentOrganizationId()}/user-auth-info/person-authenticate-url`,
    {
      method: 'POST',
      body: { ...userAuthInfo },
      responseType: 'text',
...
// 获取默认值信息
export async function getDefaultValue(params) {
  return request(`${HZERO_IAM}/v1/${getCurrentOrganizationId()}/user-defaults`, {
    method: 'GET',
    query: params,
  });
}
...
// 新建默认值信息
export async function createDefaultValue(body) {
  return request(`${HZERO_IAM}/v1/${getCurrentOrganizationId()}/user-defaults`, {
    method: 'POST',
    body,
  });
...
// 更新默认值信息
export async function updateDefaultValue(body) {
  return request(`${HZERO_IAM}/v1/${getCurrentOrganizationId()}/user-defaults`, {
    method: 'PUT',
    body,
  });
...
export async function fetchCancelAuth(params) {
  return request(
    `${HZERO_IAM}/v1/${getCurrentOrganizationId()}/user-auth-info/integration-authenticate-cancel`,
    {
      method: 'POST',
      body: params,
    }
 ...
 * @returns
 */
export async function fetchUnbindAuth(params) {
  return request(
    `${HZERO_IAM}/v1/${getCurrentOrganizationId()}/user-auth-info/integration-authenticate-unbind`,
    {
      method: 'POST',
      body: params,
    }
 ...
 * @returns
 */
export async function fetchAuthUrl(params) {
  return request(
    `${HZERO_IAM}/v1/${getCurrentOrganizationId()}/user-auth-info/integration-authenticate-url`,
    {
      method: 'POST',
      body: params,
      responseType: 'text',
...
export async function fetchAuthStatus(params) {
  return request(
    `${HZERO_IAM}/v1/${getCurrentOrganizationId()}/user-auth-info/integration-authenticate-info`,
    {
      method: 'POST',
      body: params,
    }
...
export async function updatePswSendCaptcha(params) {
  const origin = window.location.origin.includes('localhost') ? API_HOST : window.location.origin;
  return request(`${origin}/oauth/v2/password/send-captcha`, {
    method: 'GET',
    query: params,
  });
}

export async function updatePswCheckCaptcha(params) {
  const origin = window.location.origin.includes('localhost') ? API_HOST : window.location.origin;
...
export async function updatePswWithCaptcha(params) {
  const origin = window.location.origin.includes('localhost') ? API_HOST : window.location.origin;
