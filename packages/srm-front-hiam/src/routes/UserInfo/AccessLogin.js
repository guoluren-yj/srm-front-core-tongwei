...
export default class AccessLogin extends Component {
  ...
  toggleMethod = e => {
    if (e.target.dataset.key) {
      this.setState({ method: e.target.dataset.key });
    }
  }
  ...
  getVerifyCode = () => {
    return request(`${getEnvConfig().HZERO_PLATFORM}/v1/desensitize/captcha?type=${this.state.method}`, {
      method: 'GET',
    }).then((res) => {
      const result = getResponse(res);
      if (result) {
        sessionStorage.setItem("access-login-verify-code-timer", moment().format("YYYY-MM-DD HH:mm:ss"));
        this.setState({
  ...
  render() {
    ...
    const {
      loading,
      status, method, verifyCode, cacheVerifyMessage, interval, verifyResponse,
      verifyStatus,
      verifyMethodInfo,
      verifyMethodInfo: {
        supportTypes = [],
    ...
    else if (method === "email") placeholder = placeholder.replace(/^([a-zA-Z0-9-_]+).?[^@]+(@[a-zA-Z0-9-_]+.[a-zA-Z]+(.[a-zA-Z]+)?)$/, "$1******$2");
    switch (status) {
      case "loading": content = (<Spin spinning={loading}><div style={{ width: "100%", height: "247px" }} /></Spin>); break;
      case "selectMethod": content = (
        <>
          <div className={styles["access-method"]} onClick={this.toggleMethod}>
            {supportTypes.includes("phone") && (
              <div data-key="phone" className={method === "phone" ? "active" : ""}>
                {intl.get("hiam.userInfo.accessLogin.title.verifyByPhone").d("手机验证")}
              </div>
            )}
            {supportTypes.includes("email") && (
              <div data-key="email" className={method === "email" ? "active" : ""}>
                {intl.get("hiam.userInfo.accessLogin.title.verifyByEmail").d("邮箱验证")}
              </div>
            )}
            {supportTypes.includes("password") && (
              <div data-key="password" className={method === "password" ? "active" : ""}>
                {intl.get("hiam.userInfo.accessLogin.title.verifyByPwd").d("密码验证")}
              </div>
            )}
          </div>
          {
            method === "password" ? (
              <>
                <div style={{ color: "#9a9a9a", marginBottom: "20px", fontSize: "14px" }}>
                  {intl.get("hiam.userInfo.accessLogin.tip.verifyByPwd").d("为确保您本人操作，请先通过密码完成身份验证")}
                </div>
                <Password dataSet={this.dataSet} name="password" style={{ width: "100%", display: "block", marginBottom: "26px" }} />
                <div style={{ color: "red", marginTop: "-23px", marginBottom: "3px", lineHeight: "20px", height: "20px" }}>
                  {verifyStatus === "failed" ? intl.get("hiam.userInfo.accessLogin.tip.failedVerify").d("验证失败") : ""}
                </div>
                <Button color="primary" style={{ width: "100%", maxWidth: "unset" }} onClick={this.executeVerify} loading={verifyStatus === "validating"}>
                  {intl.get("hzero.common.button.confirm").d("确认")}
                </Button>
                <div style={{ color: "#9a9a9a", fontSize: "12px", marginTop: "12px" }}>
                  {intl.get("hiam.userInfo.accessLogin.tip.passwordHelp").d("若您忘记密码，请联系公司系统管理员，或拨打热线电话400-116-0808获取帮助")}
                </div>
              </>
            ) : (
              <>
                ...
                <div style={{ color: "#9a9a9a", marginBottom: "20px", fontSize: "14px" }}>
                  {intl.get("hiam.userInfo.accessLogin.tip.verifyByPhoneOrEmail").d("为确保您本人操作，请先通过以下方式获取验证码完成验证")}
                </div>
                <TextField disabled value={placeholder} style={{ width: "100%", display: "block", marginBottom: "24px" }} />
                <Button disabled={interval >= 0} color={interval < 0 ? "primary" : "default"} style={{ width: "100%", maxWidth: "unset" }} onClick={this.getVerifyCode}>
                  {interval < 0 ? intl.get("hzero.common.components.login.captcha").d("获取验证码")
                    : intl.get("hiam.userInfo.accessLogin.tip.retryTime", {
                      interval,
                    }).d("{interval}s后可重新获取验证码")}
                </Button>
                <div style={{ color: "#9a9a9a", fontSize: "12px", marginTop: "12px" }}>
                  {intl.get("hiam.userInfo.accessLogin.tip.phoneOrEmailHelp").d("若您的手机和邮箱均无法接收验证码，请联系公司系统管理员，或拨打热线电话400-116-0808获取帮助")}
                </div>
              </>
            )
          }
        ...
        break;
      case "verify": content = (
        <div style={{ paddingLeft: "32px", paddingRight: "32px", position: "relative", height: "247px" }}>
          <Button icon="arrow_back" style={{ position: "absolute", top: "3px", left: 0 }} size="small" onClick={() => this.setState({ status: "selectMethod", cacheVerifyMessage: {} })} />
          <div style={{ marginBottom: "12px", fontSize: "20px", fontWeight: 600 }}>
            {intl.get("hiam.userInfo.accessLogin.title.inputCaptcha").d("输入验证码")}
          </div>
          <div style={{ color: "#9a9a9a", marginBottom: "18px", fontSize: "14px" }}>{cacheVerifyMessage.message}</div>
          <div
            ...
            key={verifyCode.join("")}
          >
            {[0, 1, 2, 3, 4, 5].map(v => <input key={v} maxLength={1} data-index={v} value={verifyCode[v] === 0 && "0" || verifyCode[v] || ""} />)}
          </div>
          <div style={{ color: "red", marginTop: "-23px", marginBottom: "3px", lineHeight: "20px", height: "20px" }}>
            {verifyStatus === "failed" ? intl.get("hiam.userInfo.accessLogin.tip.failedVerify").d("验证失败") : ""}
          </div>
          <div style={{ marginBottom: "12px" }}>
            <Button disabled={interval >= 0} style={{ width: "100%", maxWidth: "unset" }} onClick={this.getVerifyCode}>
              {interval < 0 ? intl.get("hiam.userInfo.accessLogin.button.reFetch").d("重新获取")
                : intl.get("hiam.userInfo.accessLogin.tip.retryTime", {
                  interval,
                }).d("{interval}s后可重新获取验证码")}
            </Button>
        ...
        break;
      case "verifySuccess": content = successInfo || (
        <div style={{ height: "247px" }}>
          <div style={{ marginBottom: "28px" }}>
            {intl.get("hiam.userInfo.accessLogin.tip.verifySuccess").d("一次性登录链接已生成，链接10分钟内有效，请复制链接分享使用。")}
          </div>
          <div style={{ marginBottom: "28px", wordBreak: "break-all" }}>{(verifyResponse || {}).result}</div>
          <div style={{ textAlign: "right", padding: "0 16px", position: "absolute", width: "100%", left: 0, bottom: "16px" }}>
            <Button
              onClick={() => {
