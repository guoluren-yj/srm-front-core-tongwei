...
export default class AvatarUploadModal extends React.PureComponent {
  ...
  @Bind()
  handleCancel() {
    const { onCancel, modal } = this.props;
    onCancel();
   ...
   */
  @Bind()
  async handleCutAvatar() {
    const { fileList } = this.state;
    const {
      dispatch,
      userInfo: { imgFormData, uploadImgName },
      organizationId,
      onCancel,
      modal,
    } = this.props;
    ...
    let uuid;
    const response = await queryUUID({ tenantId: organizationId });
  ...
  render() {
    ...
    return (
      <div className={styles['avatar-modal-content']}>
        <div className={styles['avatar-modal-main']}>
          {fileList.length === 0 ? (
            <Dragger
            ...
            >
              <p className={styles['avatar-upload-drag-icon']}>
                <Icon type="inbox" />
              </p>
              <p className={styles['avatar-upload-text']}>
                {intl
                  .get('hiam.userInfo.view.message.uploadTips')
                  .d('点击或将图片拖到此区域上传图片')}
              </p>
              {displayFlag && (
                <p className={styles['avatar-upload-hint']}>
                  {typeFlag &&
                    intl
        ...
        <div className={styles['avatar-modal-footer']}>
          ...
          <Button key="back" funcType="raised" onClick={this.handleCancel}>
