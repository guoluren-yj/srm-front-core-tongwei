    ...
    return (
      <>
        <Form>
          ...
          {['menu', 'link', 'inner-link'].includes(getFieldValue('type')) && (
            <>
              <FormItem
                label={intl.get(`${modelPrompt}.shareToSupplier`).d('授权给供应商')}
                {...MODAL_FORM_ITEM_LAYOUT}
              >
                {getFieldDecorator('shareToSupplier', {
                  initialValue: shareToSupplier || 0,
                })(<Switch checkedValue={1} unCheckedValue={0} disabled={editable && !isEdit} />)}
              </FormItem>
              <FormItem
                label={intl.get(`${modelPrompt}.shareToThirdParty`).d('授权给第三方')}
                {...MODAL_FORM_ITEM_LAYOUT}
              >
                {getFieldDecorator('shareToThirdParty', {
                  initialValue: shareToThirdParty || 0,
                })(<Switch checkedValue={1} unCheckedValue={0} disabled={editable && !isEdit} />)}
              </FormItem>
              <FormItem
                label={intl.get(`${modelPrompt}.menuUser`).d('菜单使用方')}
                {...MODAL_FORM_ITEM_LAYOUT}
              >
                  ...
                  rules: [
                    {
                      required: !isEdit,
                      message: intl.get(`${commonPrompt}.validation.notNull`, {
                        name: intl.get(`${modelPrompt}.menuUser`).d('菜单使用方'),
                      }),
                    },
                  ],
                })(
                  <Select disabled={isEdit}>
                    {labelCodeList && labelCodeList.length > 0
                      ? labelCodeList.map((label) => (
