...
export default class AuthorityManagement extends React.Component {
  ...
  componentDidMount() {
    ...
    dispatch({
      ...
      if (!lodash.isEmpty(res)) {
        ...
        this.setState({ defaultList: arr, activeKey: res[0]?.valueSource });
        this.loadDimensions(res);
      }
    });
    // 获取角色维度是否显示
    dispatch({
      type: 'authorityManagement/fetchRoleDimension',
    }).then((res) => {
      this.setState({ roleDimensionShow: res === true });
    });
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const { authRoleId: preAuthRoleId, activeKey: preActiveKey } = prevState;
    const { authRoleId, activeKey } = this.state;
    if (preAuthRoleId !== authRoleId || preActiveKey !== activeKey) {
      this.renderDefaultTabs();
      this.loadDimensions(authorList);
    }
  }
  ...
  loadDimensions(layouts = []) {
    const { userId, authRoleId, activeKey } = this.state;
  ...
  renderDefaultTabs() {
    ...
    const { userId, organizationId, authRoleId, activeKey } = this.state;
    const tabList = [
      {
        code: 'COMPANY',
        component: (
  ...
  openHistory(curUserId) {
    C7NModal.open({
      ...
      children: <History userId={curUserId} actionType='authority' />,
      ...
      <>
        ...
        >
          <>
            ...
            {roleDimensionShow && (
              <FormItem
                label={intl.get(`hiam.authority.model.authority.role`).d('角色维度')}
                {...SEARCH_FORM_ITEM_LAYOUT}
                style={{ marginTop: 14 }}
              >
                {this.props.form.getFieldDecorator('authRoleId', {
                  initialValue: '',
                })(
                  <Lov
                    code="HIAM.USER_ASSIGN_ROLE"
                    queryParams={{ userId }}
            ...
            <ButtonPermission
              icon="clock-circle-o"
              onClick={() => this.openHistory(userId)}
              permissionList={[
                {
        ...
        <Content>
          <div style={{ margin: '0 0 24px 0px', lineHeight: '20px' }}>
            ...
            <span style={{ color: '#666' }}>
              {intl.get('hiam.authority.model.authorityManagement.userName').d('名称')}:
            </span>
