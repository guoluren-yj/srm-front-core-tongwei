...
@cuxRemote(
  {
    code: 'HIAM_SUBACCOUNT_ORGNEW', // 对应二开模块暴露的Expose的编码， 命名规范：模块编码+功能编码
    name: 'remote', // 默认 'remote'， 如有属性冲突可以改此属性
  },
  {
    process: {
      handleRenderCuxButton: undefined,
    },
  }
)
...
@formatterCollections({
  code: ['hiam.subAccount', 'hiam.securityGroup', 'hiam.roleManagement', 'hiam.userInfo'],
})
...
export default class SubAccountOrg extends React.Component {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'subAccountOrg/initLanguageDataSource',
    });
    dispatch({
  ...
  @Bind()
  onFetchMemberPermission(params) {
    const { dispatch } = this.props;
    return dispatch({
      type: 'subAccountOrg/queryAssignPermissionOfMember',
      payload: {
        ...params,
        memberType: 'user',
      },
   ...
   */
  @Bind()
  handleShieldMemberPermission(payload) {
    const { dispatch } = this.props;
    const { shieldFlag, ...params } = payload;
    const type = `subAccountOrg/${shieldFlag ? 'cancelShieldMemberPermission' : 'shieldMemberPermission'
      }`;
    return dispatch({
      type,
      payload: params,
  ...
  openHistory(userId) {
    Modal.open({
      key: 'tenant-history-modal',
      title: intl.get('hzero.common.button.operationRecords').d('操作记录'),
      destroyOnClose: true,
      style: {
        width: 700,
      },
      children: <History userId={userId} actionType="user" />,
      okText: intl.get('hzero.common.btn.close').d('关闭'),
      cancelButton: false,
    });
  }

  openEditModal(editFormProps) {
    const {
      subAccountOrg: {
        passwordTipMsg = {},
        createSubRoles = [],
    ...
    const modalProps = {
      ...
      footer: (okBtn, cancelBtn) => (
        <div>
          {okBtn}
          {cancelBtn}
          {editFormProps.memberId && (
            <ButtonPermission
              onClick={() => {
                this.openHistory(editFormProps?.memberId);
              }}
              permissionList={[
                {
                  code: `hzero.sys.account.button.action-history`,
                  type: 'button',
                  meaning: '权限维护-操作记录',
                },
              ]}
    ...
    const editProps = {
      ...
      customizeForm,
      labelList,
      fetchPermission: this.onFetchMemberPermission,
      onShield: this.handleShieldMemberPermission,
    };
    this.setState({
      editFormProps,
    });
  ...
  handleRecordResetPassword(record) {
    ...
    Modal.confirm({
      title: (
        <span>
          {intl
            .getHTML('hiam.userInfo.view.confirmResetPassword3', {
              name: `${record.realName}(${record.loginName})`,
            })
            .d(
              <div>
                是否确认重置
   ...
   */
  @Bind()
  async handleEditModalOkBtnClick() {
    const {
    ...
    if (!isEmpty(data)) {
      ...
      const res = await updateOrCreatePromise;
      if (res) {
        notification.success();
  ...
  render() {
    ...
    const { handleRenderCuxButton } = remote.props?.process || {};
    const listProps = {
      onRef: (ref) => {
        this.tableRef = ref;
      },
    ...
    return (
      <>
        <Header title={intl.get('hiam.subAccount.view.message.title').d('子账户管理')}>
          ...
          <ExcelExportPro
            exportAsync
            templateCode="IAM_USER_EXPORT" // 导出模板编码
            requestUrl={`${HZERO_IAM}/hzero/v1/${organizationId}/users/export`}
            queryParams={this.getExportQueryParams}
            queryFormItem={
              <ExportExtraItem dimensionList={dimensionList} handleSelect={this.handleSelect} />
            }
            buttonText={intl.get('hiam.subAccount.view.button.newExport').d('(新)导出')}
            otherButtonProps={{
              className: 'label-btn',
              permissionList: [
                {
            ...
            tableRef={this.tableRef} // 表格ref，可用于同步表格列和导出列的顺序
          />
          <Dropdown
            overlay={
              <Menu>
                <Menu.Item key="HIAM.ACCOUNT_CREATE">
                  <ButtonPermission
                    type="c7n"
                    ...
                    permissionList={[
                      {
                        code: `${path}.button.new.batchimport`,
                        type: 'button',
                        meaning: '子账户管理-账户导入',
                      },
                    ]}
                    onClick={() => this.handleImportNew('HIAM.ACCOUNT_CREATE')}
                  >
                    {intl.get('hiam.subAccount.view.button.batchImport').d('账户导入')}
                    <span className={styles['button-tag']}>NEW</span>
                  </ButtonPermission>
                </Menu.Item>
                <Menu.Item key="HIAM.ROLE_CREATE">
                  <ButtonPermission
                    type="c7n"
                    className={styles['import-button']}
                    funcType="flat"
                    permissionList={[
                      {
                        code: `${path}.button.new.roleimport`,
                        type: 'button',
                        meaning: '子账户管理-角色导入',
                      },
                    ]}
                    onClick={() => this.handleImportNew('HIAM.ROLE_CREATE')}
                  >
                    {intl.get('hiam.subAccount.view.button.roleImport').d('角色导入')}
                    <span className={styles['button-tag']}>NEW</span>
                  </ButtonPermission>
                </Menu.Item>
                <Menu.Item key="HIAM.AUTH_CREATE">
                  <ButtonPermission
                    type="c7n"
                    className={styles['import-button']}
                    funcType="flat"
                    permissionList={[
                      {
                        code: `${path}.button.new.permissionimport`,
                        type: 'button',
                        meaning: '子账户管理-权限导入',
                      },
                    ]}
                    onClick={() => this.handleImportNew('HIAM.AUTH_CREATE')}
                  >
          ...
          >
            <ButtonPermission
              icon="archive"
              className="srm-common-import-button"
              type="c7n-pro"
              permissionList={[
                {
                  code: `${path}.button.new.import`,
                  type: 'button',
                  meaning: '子账户管理-(新)导入',
                },
              ]}
              apmName="(新)导入"
            >
              {intl.get('hiam.subAccount.button.importNew').d('(新)导入')}
              <Icon type="down" />
              <span className={styles['button-tag']}>NEW</span>
            </ButtonPermission>
          </Dropdown>
          ...
          {typeof handleRenderCuxButton === 'function' && handleRenderCuxButton()}
        </Header>
        <Content>
          <Search {...filterProps} />
          <List {...listProps} />
...
const ExportExtraItem = (props) => {
  const { dimensionList = [], handleSelect } = props;
  const [checkedKeys, setCheckedKeys] = React.useState([]);
  const handleCheck = React.useCallback((keys) => {
    setCheckedKeys(keys);
    handleSelect(keys);
  }, []);
