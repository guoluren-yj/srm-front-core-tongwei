...
@formatterCollections({ code: ['hiam.subAccount', 'hiam.userInfo'] })
export default class SubAccountSite extends React.Component {
  ...
  handleFetchList(pagination) {
    ...
    const fieldsValueObj = filterNullValueObject(fieldsValue);
    if (isEmpty(fieldsValueObj)) {
              ...
              ele:
                admin || record.id === currentUserId ? null : (
                  <ButtonPermission
                    ...
                    permissionList={[
                      {
                        code: `hzero.site.sys.account.button.reset`,
                        type: 'button',
                        meaning: '子账户管理-重置密码',
                      },
                    ]}
  ...
  @Bind()
  onFetchMemberPermission(roleId) {
    const { dispatch } = this.props;
    const {
      editModalProps: { detailRecord },
    } = this.state;
    return dispatch({
      type: 'subAccount/queryAssignPermissionOfMember',
      payload: {
