...
export default class GroupModal extends React.Component {
  ...
  render() {
    const { visible, dataSource, pagination, form, fetchEmployeeLoading, userInfo } = this.props;
    const columns = [
      {
    ...
    const { loginName, realName } = userInfo || {};
    return (
      <Modal
        visible={visible}
        ...
        width={720}
        title={intl.get('hiam.subAccount.view.message.title.employee', { who: `${realName}(${loginName})` }).d('查看员工-{who}')}
        footer={null}
      >
