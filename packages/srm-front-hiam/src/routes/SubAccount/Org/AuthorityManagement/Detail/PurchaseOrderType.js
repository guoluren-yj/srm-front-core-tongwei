...
@connect(({ authorityPoType, loading }) => ({
  authorityPoType,
  addLoading: loading.effects['authorityPoType/addAuthorityPoType'],
  fetchLoading: loading.effects['authorityPoType/fetchAuthorityPoType'],
  fetchModalLoading: loading.effects['authorityPoType/fetchModalData'],
}))
@Form.create({ fieldNameProp: null })
export default class PurchaseOrderType extends PureComponent {
  purchaseOrderTypeRef;
  ...
  componentDidMount() {
    ...
    if (!isNil(userId) && this.preAuthRoleId !== authRoleId && activeKey === 'POTYPE') {
  ...
  fetchData(pageData = {}) {
    ...
    const staticData = {
      userId,
      authorityTypeCode: 'PURCHASE_ORDER_TYPE',
    };
    form.validateFields((err, fieldsValue) => {
      if (!err) {
        dispatch({
          type: 'authorityPoType/fetchAuthorityPoType',
          payload: {
            authRoleId,
            ...fieldsValue,
            ...staticData,
  ...
  addPrType(addRows) {
    const {
      dispatch,
      authorityPoType: { head = {} },
      queryParams: { userId },
      authRoleId,
    } = this.props;
    dispatch({
      type: 'authorityPoType/addAuthorityPoType',
      payload: {
        authorityTypeCode: 'PURCHASE_ORDER_TYPE',
        userId,
        authRoleId,
      ...
      if (response) {
        ...
        this.purchaseOrderTypeRef.state.addRows = [];
        this.refresh();
      }
    });
  }
  ...
  remove() {
    ...
    const onOk = () => {
      dispatch({
        type: 'authorityPoType/deleteAuthorityPoType',
        payload: {
          userId,
  ...
  fetchModalData(queryData = {}) {
    ...
    dispatch({
      type: 'authorityPoType/fetchModalData',
      payload: {
        userId,
        authRoleId,
        ...queryData,
  ...
  renderForm() {
    ...
    return (
      <Form layout="inline">
        <FormItem
          label={intl
            .get('hiam.authorityManagement.model.authorityPoType.dataCode')
            .d('订单类型编码')}
        >
        ...
        <FormItem
          label={intl
            .get('hiam.authorityManagement.model.authorityPoType.dataName')
            .d('订单类型名称')}
        >
  ...
  render() {
    ...
    const addModalOptions = {
      ...
      title: intl.get('hiam.authority.model.title.selectAuthorityPoType').d('选择采购订单类型'),
      rowKey: 'dataId',
      queryCode: 'dataCode',
      queryName: 'dataName',
      queryCodeDesc: intl
      ...
      <>
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          {!head?.includeAllFlag && (
            <React.Fragment>
              <Button style={{ margin: '0 8px 16px 0' }} onClick={() => this.onShowAddModal()}>
                {intl
                  .get('hiam.authorityManagement.view.button.table.create.authorityPoType')
                  .d('新建订单类型权限')}
              </Button>
              <Button
              ...
              >
                {intl
                  .get('hiam.authorityManagement.view.button.table.delete.authorityPoType')
                  .d('删除订单类型权限')}
              </Button>
            </React.Fragment>
          )}
          <div style={{ display: 'inline-block', margin: '0 8px 16px 0' }}>
            ...
            <Tooltip
              title={intl
                .get('hiam.authorityManagement.view.message.title.tooltip.authorityPoType')
                .d('“加入全部”即将所有订单类型权限自动添加至当前账户，无需再手工添加。')}
              placement="right"
            >
              <Switch
