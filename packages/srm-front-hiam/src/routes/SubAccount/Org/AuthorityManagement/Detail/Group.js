 ...
 * @reactProps {Object} authorityGroup - 数据源
...
@connect(({ authorityGroup, loading }) => ({
  authorityGroup,
  addLoading: loading.effects['authorityGroup/addAuthorityGroup'],
  fetchLoading: loading.effects['authorityGroup/fetchAuthorityGroup'],
  fetchModalLoading: loading.effects['authorityGroup/fetchModalData'],
}))
...
export default class Group extends PureComponent {
  ...
  componentDidMount() {
    ...
    if (this.preAuthRoleId !== authRoleId && activeKey === 'GROUP' && !isNil(userId)) {
  ...
  addCompany(addRows) {
    ...
    dispatch({
      type: 'authorityGroup/addAuthorityGroup',
      payload: {
        authorityTypeCode: 'GROUP',
        userId,
        userAuthority: head,
  ...
  remove() {
    ...
    const onOk = () => {
      dispatch({
        type: 'authorityGroup/deleteAuthorityGroup',
        payload: {
          userId,
          authRoleId,
  ...
  fetchModalData(queryData = {}) {
    ...
    dispatch({
      type: 'authorityGroup/fetchModalData',
      payload: {
        userId,
        authRoleId,
        ...queryData,
  ...
  renderForm() {
    ...
    return (
      <Form layout="inline">
        <FormItem label={intl.get('hiam.authorityManagement.model.authorityGroup.name').d('名称')}>
          {getFieldDecorator('dataName')(<Input />)}
        </FormItem>
        <FormItem label={intl.get('hiam.authorityManagement.model.authorityGroup.code').d('编码')}>
  ...
  render() {
    ...
    const columns = [
      {
        title: intl
          .get('hiam.authorityManagement.model.authorityGroup.groupsName')
          .d('公司/业务单元/库存组织'),
        dataIndex: 'dataName',
      },
      {
        title: intl.get('hiam.authorityManagement.model.authorityGroup.code').d('编码'),
        dataIndex: 'dataCode',
        width: 300,
      },
    ];
    ...
    const addModalOptions = {
      ...
      title: intl.get('hiam.authorityManagement.view.title.modal.group').d('选择组织架构'),
      rowKey: 'dataId',
      queryCode: 'dataCode',
      queryName: 'dataName',
      queryCodeDesc: intl.get('hiam.authorityManagement.model.authorityGroup.code').d('编码'),
      queryNameDesc: intl.get('hiam.authorityManagement.model.authorityGroup.name').d('名称'),
      ...
      <div>
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          {!head.includeAllFlag && (
            <React.Fragment>
              <Button style={{ margin: '0 8px 16px 0' }} onClick={() => this.onShowAddModal()}>
                {intl
                  .get('hiam.authorityManagement.view.button.table.create.group')
                  .d('新建组织架构权限')}
              </Button>
              <Button
              ...
              >
                {intl
                  .get('hiam.authorityManagement.view.button.table.delete.group')
                  .d('删除组织架构权限')}
              </Button>
            </React.Fragment>
          )}
          <div style={{ display: 'inline-block', margin: '0 8px 16px 0' }}>
            ...
            <Tooltip
              title={intl
                .get('hiam.authorityManagement.view.message.title.tooltip.group')
                .d('“加入全部”即将所有组织架构权限自动添加至当前账户，无需再手工添加。')}
              placement="right"
            >
              <Switch
