...
@connect(({ authorityUnitCreate, loading }) => ({
  authorityUnitCreate,
  updateLoading: loading.effects['authorityUnitCreate/updateAuthorityUnit'],
  fetchLoading: loading.effects['authorityUnitCreate/fetchAuthorityUnitAndExpand'],
  fetchHeaderLoading: loading.effects['authorityUnitCreate/fetchHeader'],
  updateFlagLoading: loading.effects['authorityUnitCreate/addAuthorityUnit'],
  refreshLoading: loading.effects['authorityUnitCreate/fetchAuthorityUnit'],
}))
...
export default class CustomerUnit extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.preAuthRoleId !== authRoleId && activeKey === 'CREATED_UNIT_ID' && !isNil(userId)) {
  ...
  refreshValue() {
    ...
    const staticData = {
      userId,
      authorityTypeCode: 'CREATED_UNIT_ID',
      variant: 'CREATED_UNIT_ID',
    };
    ...
    dispatch({
      type: 'authorityUnitCreate/fetchAuthorityUnit',
      payload: {
        authRoleId,
        ...queryParams,
        ...staticData,
  ...
  handleSave() {
    const {
      dispatch,
      authorityUnitCreate: { initCheckList = [], checkList = [], originList = [] },
      queryParams: { userId },
      authRoleId,
    } = this.props;
    const { queryParams } = this.state;
    ...
    dispatch({
      type: 'authorityUnitCreate/updateAuthorityUnit',
      payload: {
  ...
  queryValue() {
    ...
    form.validateFields((err, fieldsValue) => {
      if (!err) {
        ...
        dispatch({
          type: 'authorityUnitCreate/fetchHeader',
          payload: {
            authorityTypeCode: 'CREATED_UNIT_ID',
            userId,
          },
        ...
        dispatch({
          type: 'authorityUnitCreate/fetchAuthorityUnitAndExpand',
          payload: {
            ...fieldsValue,
            userId,
            authRoleId,
  ...
  setSelectRows(rows) {
    ...
    dispatch({
      type: 'authorityUnitCreate/updateCheckList',
  ...
  onExpand(expanded, record = {}) {
    const {
      dispatch,
      authorityUnitCreate: { expandedRowKeys = [] },
    } = this.props;
    dispatch({
      type: 'authorityUnitCreate/updateExpanded',
      payload: expanded
  ...
  handleExpand() {
    const {
      dispatch,
      authorityUnitCreate: { originList = [] },
    } = this.props;
  ...
  includeAllFlag(checked) {
    const {
      dispatch,
      queryParams: { userId },
      authorityUnitCreate: { header = {} },
    } = this.props;
    this.setState({
      switchLoading: true,
    });
    dispatch({
      type: 'authorityUnitCreate/addAuthorityUnit',
      payload: {
        authorityTypeCode: 'CREATED_UNIT_ID',
        userId,
        userAuthority: {
  ...
  renderForm() {
    ...
    const { header = {} } = authorityUnitCreate || {};
    const { expanded, checkSubsetFlag } = this.state;
    return (
      <Form layout="inline">
        <FormItem
          label={intl.get('hiam.authorityManagement.model.authorityUnit.unitName').d('部门名称')}
  ...
  render() {
    ...
    const {
      ...
      authorityUnitCreate: { data = [], checkList = [], expandedRowKeys = [] },
    } = this.props;
    const columns = [
      {
        title: intl.get('hiam.authorityManagement.model.authorityUnit.unitName').d('部门名称'),
    ...
    const { viewModalVisible, checkSubsetFlag } = this.state;
    const rowSelection = {
