...
import { Form, Input, Button, Modal, Tooltip, Switch, Checkbox, Icon } from 'hzero-ui';
 ...
 * @reactProps {Object} authoritySupplierSrm - 数据源
...
@connect(({ authoritySupplierSrm, loading }) => ({
  authoritySupplierSrm,
  addLoading: loading.effects['authoritySupplierSrm/addAuthoritySupplier'],
  fetchLoading: loading.effects['authoritySupplierSrm/fetchAuthoritySupplier'],
  fetchModalLoading: loading.effects['authoritySupplierSrm/fetchModalData'],
}))
@Form.create({ fieldNameProp: null })
@formatterCollections({ code: ['hiam.authorityManagement', 'hiam.authority', 'sslm.common'] })
export default class Supplier extends PureComponent {
  ...
  componentDidMount() {
    ...
    if (this.preAuthRoleId !== authRoleId && activeKey === 'SUPPLIER' && !isNil(userId)) {
  ...
  fetchData(pageData = {}) {
    ...
    form.validateFields((err, fieldsValue) => {
      if (!err) {
        const { categoryDescription, ...othersValue } = fieldsValue;
        dispatch({
          type: 'authoritySupplierSrm/fetchAuthoritySuppliers',
          payload: {
            authRoleId,
            ...othersValue,
            ...staticData,
  ...
  addSupplier(addRows) {
    const {
      dispatch,
      authoritySupplierSrm: { head = {} },
      queryParams: { userId },
      authRoleId,
    } = this.props;
    dispatch({
      type: 'authoritySupplierSrm/addAuthoritySupplier',
      payload: {
        authorityTypeCode: 'SUPPLIER',
        userId,
        userAuthority: head,
  ...
  remove() {
    ...
    const onOk = () => {
      dispatch({
        type: 'authoritySupplierSrm/deleteAuthoritySupplier',
        payload: {
          userId,
          authRoleId,
  ...
  fetchModalData(queryData = {}) {
    ...
    const { categoryDescription, ...othersValue } = queryData;
    dispatch({
      type: 'authoritySupplierSrm/fetchSupplierModalData',
      payload: {
        userId,
        authRoleId,
        ...othersValue,
  ...
  renderForm() {
    ...
    return (
      <Form layout="inline">
        ...
        <FormItem label={intl.get('sslm.common.view.supplier.class').d('供应商分类')}>
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: intl.get('sslm.common.view.supplier.class').d('供应商分类'),
