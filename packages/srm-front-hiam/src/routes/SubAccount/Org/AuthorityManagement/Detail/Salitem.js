 ...
 * @reactProps {Object} authoritySalitem - 数据源
...
@connect(({ authoritySalitem, loading }) => ({
  authoritySalitem,
  loading: loading.models.authoritySalitem,
}))
...
export default class Customer extends PureComponent {
  ...
  componentDidMount() {
    ...
    if (this.preAuthRoleId !== authRoleId && activeKey === 'SUPPLIER_ITEM' && !isNil(userId)) {
  ...
  addData() {
    ...
    dispatch({
      type: 'authoritySalitem/addNewData',
      payload: data,
    });
  }
  ...
  cancel(record) {
    ...
    if (record.isCreate) {
      ...
      dispatch({
        type: 'authoritySalitem/removeNewAdd',
        payload: {},
      });
    } else {
  ...
  render() {
    ...
    const {
      authoritySalitem: { data = {}, head = {} },
      loading,
    } = this.props;
    const { selectedRows, newCreateRows = [] } = this.state;
    const { getFieldDecorator } = this.props.form;
    const dataSource = (data.list || []).concat(newCreateRows);
    const columns = [
      {
        title: intl
          .get('hiam.authorityManagement.model.authorityCustomer.dataName')
