...
import { Checkbox, Col, Form, Row, Modal, Input, Button } from 'hzero-ui';
...
export default class UserGroupModal extends React.Component {
  ...
  renderForm() {
    const { form, userRecord = {} } = this.props;
    const {
      selectedRowKeys = [],
      dataSource = [],
      pagination = false,
        ...
        <Row style={{ textAlign: 'right' }}>
          <Col span={23}>
            <Form.Item>
              ...
              <Button type="primary" onClick={this.handleGroupCreate}>
      ...
      fetchCurrentUserGroups({ page, sort, userId: userRecord.id })
        .then(groupContent => {
          ...
          if (groupContent) {
            this.setState(
              {
                dataSource: map(groupContent.content, r => {
