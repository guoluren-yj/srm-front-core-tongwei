...
import { filter, forEach, isEmpty, isNil, isNumber, isUndefined, join, map, omit } from 'lodash';
import {
  Button,
  Checkbox,
  Col,
...
export default class EditForm extends React.Component {
      ...
      if (!err) {
        ...
        const cancelDefaultParams = [...this.cancelDefaultParams.values()].filter(tId => {
          ...
          return !memberRoleList.some(r => {
            return (
              (r.assignLevel === 'organization' || r.assignLevel === 'org') &&
  ...
  getRoleCurrentRecordLevelOptions(record) {
    ...
    if (!isEmpty(currentRole)) {
      const { parentRoleAssignLevel = '' } = currentRole;
      // String 是 编辑后的, Number 是用户之前拥有的
      forEach(level, l => {
        if (l.value !== 'site' && +l.tag >= +levelMap[parentRoleAssignLevel].tag) {
          $Options.push(
            <Option value={l.value} key={l.value}>
              {l.meaning}
            </Option>
  ...
  handleRoleDefaultChange(record) {
    ...
    this.setState({
      dataSource: dataSource.map(item => {
        ...
        } else if (item.tenantId === record.tenantId) {
        ...
        <Row type="flex">
          ...
          <Col {...FORM_COL_2_LAYOUT}>
            <FormItem
              required
              {...MODAL_FORM_ITEM_LAYOUT}
              label={intl.get('hiam.subAccount.model.user.confirmPassword').d('确认密码')}
            >
  ...
  renderEditForm() {
    ...
    return (
      <Row>
        ...
        <Col key="gender" {...FORM_COL_2_LAYOUT}>
          <Form.Item {...MODAL_FORM_ITEM_LAYOUT} label={intl.get('hzero.common.gender').d('性别')}>
            {form.getFieldDecorator('gender', {
