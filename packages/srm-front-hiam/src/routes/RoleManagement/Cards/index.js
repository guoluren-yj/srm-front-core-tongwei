...
import { Modal, InputNumber, Button, Form } from 'hzero-ui';
...
import CommonUserTree from './CommonUserTree';
...
export default class RoleAssignCardsEditModal extends React.Component {
    ...
    return (
      ...
      >
        ...
        <div className="table-list-operator" style={{ textAlign: 'right' }}>
          ...
          <Button type="primary" key="add" onClick={this.handleAddBtnClick}>
        ...
        <CommonUserTree
          ...
          visible={commonUserTreeVisible}
          hideModal={this.hideCommonUserModal}
          treeData={commonUserTreeData}
        />
      </Modal>
    );
  }
  ...
  queryFunctionExUser() {
    return this.props
      .dispatch({
        type: 'commonUser/queryFunctionExUser',
        payload: this.props.role.id,
      })
      .then((res) => {
        if (res) {
          this.setState({
            commonUserTreeData: res,
          });
        }
      });
  }
  ...
  @Bind()
  handleLineSetting() {
    this.queryFunctionExUser();
    this.setState({
      commonUserTreeVisible: true,
      commonUserTreeData: {},
    });
  }
          ...
          return (
            <Form.Item>
              {form.getFieldDecorator('cardId', {
                ...
                <Lov
                  code={isTenantRoleLevel() ? 'HPFM.ROLE_ASSIGN_CARD.ORG' : 'HPFM.ROLE_ASSIGN_CARD'}
                  onChange={onCardChange}
                  textValue={code}
                  queryParams={{ roleId: this.props.role.id }}
                />
              )}
            </Form.Item>
          );
              ...
              },
              {
                validator: (rule, value, callback) => {
                  if (value > 200 || value < 0) {
                    callback(
                      intl
                        .get('hiam.roleManagement.view.validate.valueNotValid')
                        .d(`不能小于0或大于200`)
                    );
                    return;
                  }
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        if (record._status === 'create') {
          ...
          if (record.code === 'SRM_CommonlyUsed' && isTenant) {
            actionButtons.push(
              <Text>
                <a key="action-setting" onClick={this.handleLineSetting.bind(undefined, record)}>
