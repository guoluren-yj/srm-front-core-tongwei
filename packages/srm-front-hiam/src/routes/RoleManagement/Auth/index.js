...
import { cloneDeep, isArray, isEmpty, debounce, isNil } from 'lodash';
...
export default class AuthDrawer extends React.Component {
  ...
  onRow(record) {
    ...
    // });
    let authUserDefaultValue = [];
    let authDefaultValue = [];
    // item.dimensionType === 'USER' && item.enabledFlag === 1;
    const { roleAuthorityLines } = record || {};
    if (roleAuthorityLines && roleAuthorityLines.length > 0) {
      authUserDefaultValue = roleAuthorityLines
        .filter((item) => item.dimensionType === 'USER' && item.enabledFlag === 1)
        .map((item) => item.authDimId);
      authDefaultValue = roleAuthorityLines
        .filter((item) => item.dimensionType === 'BIZ' && item.enabledFlag === 1)
        .map((item) => item.authDimId);
    }
    this.setState({
      selectedRow: record,
  ...
  scopeOnChange(e) {
    ...
    if (index !== -1) {
      if (
        e &&
        e.target &&
        !isNil(e.target.checked) &&
        sourceRow.docEnabledFlag !== e.target.checked
      ) {
      ...
      this.setState({
        scopeChangeList: [
          ...
          {
            ...sourceRow,
            docEnabledFlag: !isNil(e.target.checked) ? e.target.checked : sourceRow.docEnabledFlag,
          },
        ].map((item) => {
          delete item.msgFlag;
          delete item.roleAuthorityLines;
    ...
    this.setState({
      ...
      selectedRow: {
        ...selectedRow,
        docEnabledFlag: !isNil(e.target.checked) ? e.target.checked : selectedRow.docEnabledFlag,
      },
    });
    // 未选中的时候， 不显示权限值
    // if(!e.target.checked) {
  ...
  renderRightList() {
    ...
    const { authScopeCode, roleAuthorityLines } = selectedRow;
    if (authScopeCode === 'USER') {
      const lines = roleAuthorityLines
        ? roleAuthorityLines.filter((item) => item.dimensionType === 'USER')
        : [];
      return (
        <CheckboxGroup
          style={{ width: '100%' }}
      ...
      const lines = roleAuthorityLines
        ? roleAuthorityLines.filter((item) => item.dimensionType === 'BIZ')
        : [];
      return (
        <CheckboxGroup
          style={{ width: '100%' }}
  ...
  render() {
      ...
      },
      {
        ...
        render: (_, record) => {
          ...
          return (
            ...
            <>{roleAuthScopeCode.map((item) => item.meaning)}</>
            // </CheckboxGroup>
          );
        },
      },
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (_, record) => {
          ...
          return roleAuthScopeCode.map((item) => (
            <span
              key={item.value}
              style={{
                marginRight: 0,
                padding: '0 10px',
                display: item.value === record.authScopeCode ? '' : 'none',
              }}
            >
              {item.meaning}
            </span>
      ...
      {
        title: intl.get('hiam.roleManagement.model.role.controlDimension').d('控制维度'),
        dataIndex: 'controlDimension',
        width: 200,
        render: (_, record) => {
          const { authScopeCode, roleAuthorityLines } = record;
          if (authScopeCode === 'USER') {
            const data = roleAuthorityLines
              ? roleAuthorityLines.filter(
                  (item) => item.dimensionType === 'USER' && item.enabledFlag === 1
                )
              : [];
            return data.map((item) => item.authTypeMeaning).join('、');
          } else {
            const data = roleAuthorityLines
              ? roleAuthorityLines.filter(
                  (item) => item.dimensionType === 'BIZ' && item.enabledFlag === 1
                )
              : [];
            return data.map((item) => item.authTypeMeaning).join('、');
          }
    ...
    return (
      <Drawer
        // title={intl.get('hiam.roleManagement.view.title.editPermissionDession').d('维护权限维度')}
        destroyOnClose
        width="1000px"
        closable={false}
        wrapClassName={styles['permssion-modal']}
      ...
      >
        <div className={styles['content-container']}>
          <div className={styles['content-container-left']}>
            <div className={styles['content-container-left-title']}>
              {intl.get('hiam.roleManagement.view.title.editPermissionDession', {name: roleName}).d('“{name}”开启权限维度')}
            </div>
            <Input
              placeholder={intl
                .get('hiam.roleManagement.view.title.inputDocType')
                .d('请输入单据查询')}
              prefix={<Icon type="search" style={{ color: 'rgba(0,0,0,.25)' }} />}
              value={searchText}
              onInput={this.onChangeSearchText}
            />
            <Table
              className={styles['content-container-left-table']}
              loading={initLoading}
            ...
            />
            <div className={styles['content-container-left-footer']}>
          ...
          <div className={styles['content-container-right']}>
            <div className={styles['content-container-right-title']}>
              <Tooltip title={intl.get('hiam.roleManagement.view.title.selectLeftData').d('请在左侧选中行后维护控制维度')}>
                <div>
                  {intl.get('hiam.roleManagement.view.title.selectLeftData').d('请在左侧选中行后维护控制维度')}
                </div>
              </Tooltip>
              <Icon type="close" onClick={this.handleCancel} />
            </div>
            <div className={styles['content-container-right-list']}>{this.renderRightList()}</div>
          </div>
        </div>
      </Drawer>
    );
