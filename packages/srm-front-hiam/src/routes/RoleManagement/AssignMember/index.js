...
@connect(({ loading = {}, roleManagement }) => ({
  userTypeList: roleManagement.userTypeList || [],
  memberModalList: roleManagement.memberModalList || [],
  memberModalPagination: roleManagement.memberModalPagination || {},
  resourceLevel: roleManagement.code['HIAM.RESOURCE_LEVEL'],
  processing: {
    save: loading.effects['roleManagement/saveMembers'],
    delete: loading.effects['roleManagement/deleteMembers'],
    query: loading.effects['roleManagement/queryMemberRolesUsers'],
  },
}))
export default class AssignMember extends React.Component {
  ...
  componentDidMount() {
    const { roleDatasource = {}, modal } = this.props;
    if (modal) {
      modal.handleOk(this.handleOk);
      modal.handleCancel(this.handleClose);
    }
  ...
  render() {
    ...
    if (modal) {
      modal.update({
        cancelProps: { disabled: processing.save || processing.delete },
        okProps: {
          disabled:
    ...
    return (
      <>
        ...
        <EditDataTable
          ...
          // 分配权限
          onFetchPermission={fetchPermission}
          onShield={onShield}
        />
      </>
    );
