...
import { Form, Icon, Spin, Tooltip } from 'hzero-ui';
...
import CoreEnterpriseDrawer from './CoreEnterprise';
...
export default class RoleManagement extends React.Component {
  ...
  @Bind()
  onFetchMemberPermission(record) {
    const { dispatch } = this.props;
    const { currentRowData } = this.state;
    return dispatch({
      type: 'roleManagement/queryAssignPermissionOfMember',
      payload: {
        memberId: record.id,
        memberType: 'user',
  ...
  handleShieldMemberPermission(payload) {
    ...
    const type = `roleManagement/${
      shieldFlag ? 'cancelShieldMemberPermission' : 'shieldMemberPermission'
    }`;
    return dispatch({
      type,
      payload: params,
  ...
  openMembersDrawer(role) {
    ...
    Modal.open({
      title: intl
        .get('hiam.roleManagement.view.title.content.members', { name: role.name })
        .d(`给"${role.name}"分配用户`),
      drawer: true,
      style: { width: 900 },
      children: <AssignMember {...membersDrawerProps} />,
    });
  }
  ...
  toggleCoreEnterprisVisible = (record) => {
    const { coreEnterpriseDrawerVisible } = this.state;
    this.setState({
      coreEnterpriseDrawerVisible: !coreEnterpriseDrawerVisible,
      currentRowData: coreEnterpriseDrawerVisible ? {} : record,
    });
  };
  ...
  handleEnabled(record) {
    ...
    if (record.enabled) {
      Modal.confirm({
        title: intl.get('hiam.roleManagement.view.title.confirmDisabled').d('确定禁用该角色吗？'),
        onOk: _func,
      });
    } else {
      _func();
  ...
  handleAction(action, record) {
        ...
        this.setState({
          ...
          currentRowData:
            ...
            actionType === 'create' || actionType === 'copy' || actionType === 'inherit'
              ? {
                  ...record,
                  parentLabelCode: parentRecord.labelCode,
    ...
    const defaultAction = {
      ...
      assignCoreEnterprise: () => {
        this.toggleCoreEnterprisVisible(record);
      },
    };
  ...
  render() {
    ...
    const { name } = currentRowData || {};
    const clientsDrawerProps = {
      path,
      tenantRoleLevel,
      visible: clientsDrawerVisible,
      title: intl
        .get(`hiam.roleManagement.view.title.clients.header`, { name })
        .d('“{name}”分配客户端'),
    ...
    const detailDrawerProps = {
      ...
      currentRowData,
      tenantRoleLevel,
      labelCodeList: code.HIAM_MENU_ROLE_LABEL || [],
    };
    ...
    const coreEnterprisDrawerProps = {
      tenantRoleLevel,
      visible: coreEnterpriseDrawerVisible,
      close: this.toggleCoreEnterprisVisible,
      roleName: currentRowData.name,
      roleId: currentRowData.id,
    };
    // 工作台配置的属性
    ...
    return (
      <div className={styles['hiam-role-list']}>
        <Header title={intl.get(`hiam.roleManagement.view.title.listHeader`).d('角色管理')}>
          ...
          {tenantRoleLevel && (
            <ExcelExportPro
              ...
              requestUrl={`${HZERO_IAM}/hzero/v2/${getCurrentOrganizationId()}/roles/function/menu/export`}
              ...
              otherButtonProps={{
                ...
                permissionList: [
                  {
                    code: 'hzero.sys.role.button.list.export',
                    type: 'button',
                    meaning: '角色管理-导出',
                  },
                ],
        ...
        <CoreEnterpriseDrawer {...coreEnterprisDrawerProps} />
