...
import { Button, Col, Form, Input, Row, Spin, Select, Tooltip, Icon } from 'hzero-ui';
...
export default class Detail extends PureComponent {
  ...
  handleSave() {
    ...
    validateFields((error, values) => {
      ...
      const { description, name, tenantId, _tls, roleLabels, tplRoleName, labelCode } = values;
      const data = {
        ...currentRowData,
        description,
        name,
  ...
  openHistoryModal() {
    ...
    Modal.open({
      ...
      zIndex: 1000,
      children: <History roleId={roleId} />,
  ...
  render() {
    ...
    const drawerProps = {
      ...
      footer: (
        <>
          {roleId && <Button onClick={this.openHistoryModal.bind(this)}>
    ...
    const labelCodeEditable =
      actionType === 'edit' &&
      !tenantRoleLevel &&
      loginName === 'admin' &&
      tenantId !== 0 &&
      (isNil(labelCode) || ['MANAGEMENT', 'PURCHASER'].includes(labelCode));
    const editLabelCodeFLag =
      actionType === 'create' ? labelCode === 'MANAGEMENT' : labelCodeEditable;
    let labelCodeOptions = labelCodeList;
    if (editLabelCodeFLag) {
      labelCodeOptions = labelCodeOptions.filter((option) =>
        labelCodeEditable
          ? ['MANAGEMENT', 'PURCHASER'].includes(option.value)
          : !['THIRDPARTY'].includes(option.value)
      );
    }
    // 父级角色为供应商阵营的时候，角色适用方不允许选择管理方
    if (labelCode === 'SUPPLIER' || !labelCode) {
      labelCodeOptions = labelCodeOptions.filter(
        (option) => !['MANAGEMENT'].includes(option.value)
      );
    }
    const initLabelCode = actionType === 'create' ? undefined : labelCode;
    return (
      <Drawer {...drawerProps}>
        <Spin spinning={processing.query || processing.queryParentRole || false}>
          <Form className={styles['hiam-role-detail-form']}>
            ...
            <Row>
              ...
              {!tenantRoleLevel && (actionType !== 'create' ? true : !VERSION_IS_OP) && (
                <Col {...FORM_COL_2_LAYOUT}>
                  <FormItem
                    label={intl
                      .get('hiam.roleManagement.model.roleManagement.childreRoleName')
                      .d('子级角色默认名称')}
                    {...MODAL_FORM_ITEM_LAYOUT}
                  >
                    {getFieldDecorator('tplRoleName', {
                      initialValue: actionType === 'edit' ? tplRoleName : undefined,
                      rules: [
                        {
                          max: 64,
                          message: intl.get(`hzero.common.validation.max`, { max: 64 }),
                      ...
                      <TLEditor
                        ...
                        // eslint-disable-next-line
                        token={
                          actionType === 'create' || actionType === 'copy' ? undefined : _token
                        }
                      />
                    )}
                  </FormItem>
              ...
              <Col span={12}>
                  ...
                  label={
                    <span>
                      {intl
                        ...
                        .d('角色使用方')}
                      {actionType !== 'create' && !initLabelCode && (
                        <Tooltip
                          title={intl
                            .get('hiam.roleManagement.model.roleManagement.roleUserHelper')
                            .d(
                              '当选择的角色使用方与被复制/继承角色已有权限阵营不一致时，则不复制/继承角色使用方外的权限。'
                            )}
                        >
                ...
                >
                  {getFieldDecorator('labelCode', {
                    initialValue: initLabelCode,
                    rules: [
                      {
                        required: editLabelCodeFLag || !initLabelCode,
                        message: intl.get('hzero.common.validation.notNull', {
                          name: intl
                            .get('hiam.roleManagement.model.roleManagement.roleUser')
                            .d('角色使用方'),
                    ...
                    <Select disabled={initLabelCode && !editLabelCodeFLag}>
                      {labelCodeOptions && labelCodeOptions.length > 0
                        ? labelCodeOptions.map((label) => (
                          <Select.Option value={label.value}>{label.meaning}</Select.Option>
                        ))
                        : null}
                    </Select>
                  )}
