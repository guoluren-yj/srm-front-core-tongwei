...
export default class RoleManagement extends React.Component {
  ...
  handleListRequest() {
    ...
    if (
      ...
      // 无查询条件则只展示树形列表根节点
      this.fetchRoleRoot({ parentRoleId: null, enabled: formValues.enabled });
    } else {
      // 有查询条件则全量查询无分页
      dispatch({
        type: 'roleManagement/queryRoleAll',
  ...
  openMembersDrawer(role) {
    const {
      match: { path },
      tenantRoleLevel,
      roleManagement: { userTypeList = [], memberModalList = [], memberModalPagination = {}, code },
    } = this.props;
    ...
    Modal.open({
      title: intl.get('hiam.roleManagement.view.title.content.members', { name: role.name }).d(`给"${role.name}"分配用户`),
      drawer: true,
      style: { width: 900 },
   ...
   */
  @Bind()
  handleShieldMemberPermission(payload, callBack = e => e) {
    const { dispatch } = this.props;
  ...
  render() {
    ...
    const clientsDrawerProps = {
      ...
      title: intl.get(`hiam.roleManagement.view.title.clients.header`, { name }).d('“{name}”分配客户端'),
