...
export default class AuthForm extends React.Component {
  ...
  render() {
    ...
    const { matchVisible, matchValue, editForm } = this.state;
    const businessColumns = [
                ...
                <Select
                  ...
                  onChange={newType => {
                    record.$form.setFieldsValue({
                      subselectType: newType === 'CUSTOM' ? 'CUSTOM' : undefined,
                    });
                  }}
                >
      ...
      {
        title: intl.get('hiam.docType.model.docType.subselectType').d('子查询类型'),
        dataIndex: 'subselectType',
        width: 120,
        render: (value, record) =>
          record._status === 'create' || record._status === 'update' ? (
            <Form.Item>
              {record.$form.getFieldDecorator(`subselectType`, {
                rules: [
                  {
                    max: 30,
                    message: intl.get('hzero.common.validation.max', {
                  ...
                  },
                  {
                    required: true,
                    message: intl.get('hzero.common.validation.notNull', {
                      name: intl.get('hiam.docType.model.docType.subselectType').d('子查询类型'),
                    }),
                  },
                ],
                initialValue:
                  record.$form.getFieldValue('ruleType') === 'CUSTOM'
                    ? 'CUSTOM'
                    : value || 'EXISTS',
              })(
                record.$form.getFieldValue('ruleType') === 'CUSTOM' ? (
                  <Select disabled>
                    <Select.Option value="CUSTOM">SQL</Select.Option>
                  </Select>
                ) : (
                  <Select
                    style={{ width: '100%' }}
                    disabled={record.$form.getFieldValue('ruleType') === 'CUSTOM'}
                  >
                    {Object.keys(SUB_SELECT_TYPE).map(item => (
                      <Select.Option value={item}>{SUB_SELECT_TYPE[item]}</Select.Option>
                    ))}
                  </Select>
                )
              )}
            ...
            <span>{value === 'CUSTOM' ? 'SQL' : SUB_SELECT_TYPE[value]}</span>
          ),
      },
      {
          ...
          return record._status === 'create' || record._status === 'update' ? (
            <Form.Item>
              {record.$form.getFieldDecorator('sourceMatchField', {
                ...
                ['CUSTOM', 'SUB_SELECT'].includes(record.$form.getFieldValue('ruleType')) ? (
                  <Button
                    onClick={() =>
                      this.editMatchField(
                        'biz',
