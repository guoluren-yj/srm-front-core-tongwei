...
export default class PermissionSet extends React.Component {
    ...
    validateFields((err, values) => {
      if (isEmpty(err)) {
        ...
        if (record._status === 'create') {
          newRecord.code = `${defaultCode}.${values.permissionType}.${values.code}`;
          newRecord.permissionType = values.permissionType === 'ps' ? 'api' : values.permissionType;
  ...
  @Bind()
  getColumns() {
    const { controllerType = [], menuDataSource } = this.props;
    const { code: menuCode = '' } = menuDataSource || {};
    const defaultColumns = [
      {
        ...
        render: (value = '', record) => {
          ...
          return record._status === 'create' ? (
            <Form.Item>
              {record.$form.getFieldDecorator('permissionType', {
                initialValue: 'ps',
              })(
                <Select allowClear={false} style={{ width: '100%' }}>
                  <Select.Option value="ps">{texts.api}</Select.Option>
                  <Select.Option value="button">{texts.button}</Select.Option>
                </Select>
              )}
            </Form.Item>
            ...
            return (
              <Form.Item>
                {getFieldDecorator('code', {
                  ...
                  <Input
                    trim
                    addonBefore={`${menuCode}.${record.$form.getFieldValue('permissionType')}.`}
                    // style={{ width: 200 }}
                    typeCase="lower"
                    inputChinese={false}
          ...
          if (record._status === 'update' || record._status === 'create') {
            ...
            return (
              <Form.Item>
                ...
                })(<InputNumber min={0} step={1} parser={toSafeInteger} />)}
              </Form.Item>
            );
          } else {
            return val;
