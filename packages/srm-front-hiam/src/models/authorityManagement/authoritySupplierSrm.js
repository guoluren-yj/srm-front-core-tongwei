...
export default {
  namespace: 'authoritySupplierSrm',
  ...
  effects: {
    ...
    *fetchAuthoritySuppliers({ payload }, { call, put }) {
      const response = yield call(querySupplierData, payload);
      const data = getResponse(response);
      if (data) {
        yield put({
          type: 'updateState',
    ...
    *fetchSupplierModalData({ payload }, { call, put }) {
      const response = yield call(fetchSupplierModalData, payload);
      const data = getResponse(response);
      if (data) {
        yield put({
          type: 'updateState',
