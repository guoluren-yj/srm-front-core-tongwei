...
import {
  ...
  createDefaultValue,
  updateDefaultValue,
  userEmailInfoValidatePre,
  submitApplicantManager,
  submitApplicantManagerStatus,
...
export default {
  ...
  state: {
    ...
    publicKey: '', // 密码公钥
    detailEnumMap: {}, // 值集集合
    defaultValueArr: [], // 默认值配置数组
    applicantManagerMap: {}, // 管理员申请提交结果
    applicantStatusMap: [], // 管理员申请提交状态
  },
  effects: {
    // 加载用户信息并重置所有信息到初始状态
    ...
    *initLanguageDataSource(_, { call, put }) {
      let result = yield call(fetchSupportLanguageList);
    ...
    *updateReminderFlag({ payload }, { call, put }) {
    ...
    // 更新打印弹窗提醒状态
    *updatePrintModalFlag({ payload }, { call, put }) {
      const { printOutputConfigFlag = -1, userInfo } = payload;
      let result = yield call(updateDefaultMenu, { printOutputConfigFlag });
      result = getResponse(result);
      if (result) {
        yield put({
          type: 'updateState',
    ...
    // 更新是否屏蔽消息弹窗
    *updateWebReminderFlag({ payload }, { call, put }) {
      const { webReminderFlag, userInfo } = payload;
      let result = yield call(updateDefaultMenu, { webReminderFlag });
      result = getResponse(result);
      if (result) {
        yield put({
          type: 'updateState',
    ...
    // 管理员申请提交
    *submitApplicantManager({ payload }, { call, put }) {
      let result = yield call(submitApplicantManager, payload);
      result = getResponse(result);
      if (result) {
        yield put({
          type: 'updateState',
          payload: {
            applicantManagerMap: result,
          },
        });
        notification.success();
        return result;
    ...
    // 管理员申请提交状态
    *submitApplicantManagerStatus({ payload }, { call, put }) {
      let result = yield call(submitApplicantManagerStatus, payload);
      result = getResponse(result);
      if (result) {
        yield put({
          type: 'updateState',
          payload: {
            applicantStatusMap: result,
          },
        });
        return result;
      }
    ...
    *validatePreValidate({ payload }, { call, put }) {
      ...
      const nextModalProps = { ...modalProps };
      // 邮箱
      if (captchaKey && type === 'oldEmail') {
        res = getResponse(
          yield call(userEmailInfoValidatePre, { captchaKey, captcha, businessScope: 'self' })
        );
        ...
        if (sessionCaptchaKey) {
          res = getResponse(
            yield call(userEmailInfoValidatePre, {
    ...
    *fetchauthentication({ payload }, { call }) {
      const res = yield call(fetchAuthentication, payload);
      return getResponse(res);
    },
    // 验证码接口
    *getCheckCode({ payload }, { call }) {
      const res = yield call(getCheckCode, payload);
      return getResponse(res);
    },
    // 校验验证码
    *checkPhoneCode({ payload }, { call }) {
      const res = yield call(checkPhoneCode, payload);
      return getResponse(res);
    },
    // 注销认证
    *dateleAuth({ payload }, { call }) {
      const res = yield call(dateleAuth, payload);
      return getResponse(res);
    },
    // 获取认证详细信息
    *getCurrentInfo({ payload }, { call }) {
      const res = yield call(getCurrentInfo, payload);
      return getResponse(res);
    },
    // 法大大实名认证
    *getFddAuth({ payload }, { call }) {
      const res = yield call(getFddAuth, payload);
      return getResponse(res);
    },
    // 法大大修改手机号
    *getFddPhone({ payload }, { call }) {
      const res = yield call(getFddPhone, payload);
      return getResponse(res);
    },
    // 契约锁实名认证
    *getQYSAuth({ payload }, { call }) {
      const res = yield call(getQYSAuth, payload);
      return getResponse(res);
    },
    // 获取值集
    *fetchDetailEnum(_, { call, put }) {
    ...
    // 获取默认值信息
    *getDefaultValue({ payload }, { call, put }) {
      const data = getResponse(yield call(getDefaultValue, payload));
      yield put({
        type: 'updateState',
        payload: {
          defaultValueArr: [],
      ...
      if (data && data.id) {
        ...
        yield put({
          type: 'updateState',
          payload: {
            defaultValueArr: [{ ...data, _status: 'update' }],
          },
        });
        return data;
      }
    ...
    // 新建默认值信息
    *createDefaultValue({ payload }, { call }) {
      const res = yield call(createDefaultValue, payload);
      return getResponse(res);
    },
    // 新建默认值信息
    *updateDefaultValue({ payload }, { call }) {
      const res = yield call(updateDefaultValue, payload);
      return getResponse(res);
    },
  },
  reducers: {
