...
import {
  ...
  fetchTreeRole,
  queryAssignPermissionOfMember,
  shieldMemberPermission,
  cancelShieldMemberPermission,
...
export default {
  ...
  effects: {
    ...
    *queryRoleAll({ payload }, { call, put }) {
    ...
    // 获取授权类型信息
    *init(_, { call, put }) {
      const { levelList, flagList, labelCodeList } = yield call(queryMapIdpValue, {
        levelList: 'HIAM.RESOURCE_LEVEL',
        flagList: 'HPFM.ENABLED_FLAG',
        labelCodeList: 'HIAM_MENU_ROLE_LABEL',
      });
      yield put({
        type: 'updateState',
        payload: {
      ...
      yield put({
        ...
        payload: {
          ...
          HIAM_MENU_ROLE_LABEL: labelCodeList,
        },
      });
    },
    ...
    *fetchTreeRoleChildren({ payload }, { call, put, select }) {
    ...
    // 查询角色已分配的指定安全组的访问权限
    *queryAssignPermissionOfMember({ payload }, { call }) {
      const res = yield call(queryAssignPermissionOfMember, payload);
      const response = getResponse(res);
      const defaultExpandedRowKeys = [];
      ...
      function assignListData(collections = []) {
        return collections.map((n) => {
          ...
          if (isEmpty(m.subMenus)) {
            ...
            defaultExpandedRowKeys.push(m.id);
            // 已屏蔽的数量
            const checkedCount = m.subMenus.filter((o) => o.shieldFlag === 1).length;
            // 未屏蔽的数量
            // const indeterminateCount = m.subMenus.filter((o) => o.shieldFlag === 0).length;
            // 逻辑：如果已屏蔽的数量 = 子菜单的长度，则显示取消屏蔽（1）；否则相反
            m.shieldFlag = checkedCount === m.subMenus.length ? 1 : 0;
          }
          return m;
        });
      }
    ...
    // 屏蔽安全组权限
    *shieldMemberPermission({ payload }, { call }) {
      const res = yield call(shieldMemberPermission, payload);
      return getResponse(res);
    },

    // 取消屏蔽安全组权限
    *cancelShieldMemberPermission({ payload }, { call }) {
      const res = yield call(cancelShieldMemberPermission, payload);
      return getResponse(res);
    },
  },
  reducers: {
