...
function AuthorityTab(props) {
  ...
  useEffect(() => {
    if (authRoleId !== currentAuthRoleId && activeKey === tabCode) {
      currentAuthRoleId = authRoleId;
      fetchData();
    }
  }, [authRoleId, activeKey]);
   ...
   */
  const includeNullFlag = debounce((e) => {
    ...
    <div>
      ...
      {!isSecGrp && (
        <div style={{ textAlign: 'right' }}>
          {['BUDGET_UNIT', 'PROJECT'].includes(authorityTypeCode) && (
            <div style={{ display: 'inline-block', margin: '0 24px 16px 0' }}>
              ...
              >
                ...
                <Checkbox onChange={includeNullFlag} checked={head.includeNullFlag || 0} />
              </Tooltip>
            </div>
          )}
          {!head.includeAllFlag && (
