...
class AuthDetail extends React.Component {
  ...
  componentDidMount() {
    const { authRoleId, activeKey, tabCode } = this.props;
    if (this.preAuthRoleId !== authRoleId && activeKey === tabCode) {
  ...
  @Bind()
  remove() {
    const { dispatch, roleId, userId, code, authRoleId } = this.props;
