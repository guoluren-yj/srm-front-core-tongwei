...
export function showRecordModal({
  ...
  return c7nModal({
    ...
    title,
    children: <Table dataSet={tableDs} columns={columns || commonCols} queryFieldsLimit={2} />,
  });
}
...
export function agmRecordModal({ url = '', params = {}, width = 700 }, type = 'PUR') {
  const purColumns = [
    ...
    { name: 'operatedRemark', tooltip: 'overflow' },
  ];
  const purFields = [
  ...
  const agmMap = {
    PUR: { columns: purColumns, fields: purFields, prefix: SRM_MALL },
    SALE: { columns: saleColumns, fields: saleFields, prefix: '/sagm' },
  };
  const { columns = [], fields = [], prefix = SRM_MALL } = agmMap[type] || {};
