...
import {
  ...
  getBarAssignCompany,
  saveBarAssignCompany,
  getBannerAssignCompany,
  saveBannerAssignCompany,
  grantPackageCompany,
  quickAddProduct,
  fetchTypeTree,
  fetchProduct,
  ...
  effects: {
    ...
    *fetchAssignCompany({ payload }, { call, put }) {
      ...
      const response = getResponse(
        yield call(type === 'banner' ? getBannerAssignCompany : getBarAssignCompany, params)
      );
      if (response) {
        yield put({
          type: 'updateState',
          payload: {
            assignCompanyList: response.content,
            assignCompanyPagination: createPagination(response),
          },
        });
      }
      return response;
    ...
    *saveAssignCompany({ payload }, { call }) {
      const { type, saveData } = payload;
      return getResponse(
        yield call(type === 'banner' ? saveBannerAssignCompany : saveBarAssignCompany, saveData)
      );
    },
    // 启用/禁用采购套餐
    *enablePackage({ payload }, { call }) {
    ...
    // 保存采购套餐授权公司列表
    *grantPackageCompany({ payload }, { call }) {
      return getResponse(yield call(grantPackageCompany, payload));
    },
    // 快速添加商品
