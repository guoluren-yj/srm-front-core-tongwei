...
import { quickAddProduct, fetchTypeTree } from '@/services/groupCustomBarService';

export default {
  namespace: 'smallCustomBar',
  state: {
    barType: '',
  ...
  effects: {
    ...
    *enableAction({ payload }, { call }) {
      const response = yield call(enableAction, payload);
      return getResponse(response);
    },
    ...
    *quickAddProduct({ payload }, { call }) {
      const response = yield call(quickAddProduct, payload);
      return getResponse(response);
    },
