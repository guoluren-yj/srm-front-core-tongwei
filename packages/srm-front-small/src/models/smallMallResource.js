...
export default {
  ...
  effects: {
    *fetchComanyInfo({ payload }, { call, put }) {
      const response = yield call(fetchComanyInfo, payload);
      const responsed = getResponse(response);
      const { content } = responsed;
      if (content) {
        yield put({
          type: 'updateState',
          payload: {
            companyList: content,
            pagination: createPagination(responsed),
