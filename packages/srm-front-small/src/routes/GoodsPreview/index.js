...
export default class GoodsPreview extends React.Component {
  constructor(props) {
    ...
    const { detailUrl = '' } = state;
    const { productImageList = [] } = state;
    const baseInfoList = qs.parse(state.baseInfoList);
    const htmlList = qs.parse(state.htmlList);
    const { sourceFrom = 'CATA' } = state;
    const { selectImg = '' } = state;
    const { is7ToReturn } = state;
    this.state = {
      sourceFrom,
      detailUrl,
      baseInfoList,
  ...
  @Bind()
  changeImgUrl(url, type) {
    this.setState({ selectImg: url, selectImgType: type });
  }
  ...
  selectAttr(attrId, item) {
    ...
    if (item.imagePath) {
      this.setState({ selectImg: item.imagePath, selectImgType: item.type });
    }
      ...
      productImageList.forEach((detailImg) => {
        arr.push(
          <div
            ...
            onFocus={() => {
              this.changeImgUrl(detailImg.imagePath, detailImg.type);
            }}
            onMouseOver={() => {
              this.changeImgUrl(detailImg.imagePath, detailImg.type);
            }}
          >
            {detailImg.type === 'VIDEO' ? (
              <div
                className={
                  selectImg === detailImg.imagePath ? 'small-img small-img-box' : 'small-img'
                }
              >
                <Player muted src={detailImg.imagePath} />
  ...
  render() {
    const { productImageList = [], detailUrl, is7ToReturn } = this.state;
    const {
      baseInfoList = {},
      selectImg,
      selectImgType,
    ...
    return (
      <React.Fragment>
        <Header
          title={intl.get('small.goodsPreview.title.previewGoods').d('商品预览')}
          backPath={detailUrl}
        />
        <Content>
                        ...
                        <p className="p-name" title={name}>
                          ...
                          {!isCATA && (
                            <span className="platform-jd">
                              {intl.get('small.goodsPreview.model.platformJd').d('京东供应')}
                            </span>
                          )}
                        </p>
                      </li>
                      ...
                      >
                        {!isCATA && (
                          <div className="jd-price-label">
                            {intl.get('small.goodsPreview.model.platformCompany').d('企业专享')}
                          </div>
                        )}
                      ...
                      <div id="selectAttrs-box">
                        ...
                        <li className="product-attr">
                          <div className="p-label p-label-required">
                            {intl.get('small.common.model.organization').d('库存组织')}
                          </div>
                              ...
                              {minPurchaseQuantity && (
                                <span className="product-ware">
                                  <span className="ware-label">
                                    {intl
                                      .get('small.goodsPreview.model.minPurchaseQuantity')
                                      .d('最小采购量')}{' '}
                                  </span>
                                  {minPurchaseQuantity}
                                </span>
