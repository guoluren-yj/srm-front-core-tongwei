...
@withRouter
@formatterCollections({ code: ['small.goodsPreview', 'small.common'] })
export default class PreGoodInfo extends React.Component {
  constructor(props) {
    super(props);
    const {
      history: {
        location: {
          state: { detail = {}, detailUrl = '' },
        },
      },
    } = this.props;
    this.state = {
  ...
  @Bind()
  onMouseMove(e) {
    const fixWidth = document.getElementsByClassName('ant-layout-sider-children')[0].offsetWidth;
    const fixHeight =
    ...
    const scrollTop =
      gbWrapper.scrollTop || document.body.scrollTop || document.documentElement.scrollTop;
    const scrollLeft =
      gbWrapper.scrollLeft || document.body.scrollLeft || document.documentElement.scrollLeft;
  ...
  getParams() {
    ...
    // const { parameters, parametersDisplay } = this.state.productInfo;
    // let lineNum = 3;
    ...
    //   })
    // );
    return { __html: detail.productDetail.specificationsParam };
  }
  ...
  changeTaxPrice(num) {
    if (num && num.toString().indexOf('.') > -1) {
      const flag = num.toString().split('.')[0];
      if (flag.length < 2) {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <Header
          title={intl.get('small.common.view.previewGoods').d('商品预览')}
          backPath={detailUrl}
        />
        <Content>
              ...
              <div className="product-detail">
                <div className="product-detail-info">
                  ...
                  <div className="info-left">
                    ...
                    >
                      <Image className="main-img" width="100%" value={selectImg || imageProPath} />
                      <span id="span" />
                    </div>
                  </div>
                  <div id="bigBox" className="big-img-Box">
                    <Image width={960} height={960} id="bigImg" value={selectImg || imageProPath} />
                  </div>
                  <div className="info-right" id="info-right">
                    <ul>
                      <li className="product-title">
                        <p className="p-name" title={goodsInfo.productName}>
                          {goodsInfo.productName}
                        </p>
                      </li>
                      <li className="product-meta">
                        <ul>
                          <li>
                            <span className="meta-label">
                              {intl.get('small.common.model.productNum').d('商品编码')} :
                            </span>
                            {goodsInfo.productNum}
                          </li>
                          <li>
                            <span className="meta-label">
                              {intl.get('small.common.model.brand').d('品牌')} :
                            </span>
                            <span title={goodsInfo.brand}>{goodsInfo.brand}</span>
                          </li>
                          <li>
                            <span className="meta-label">
                              {intl.get('small.common.model.supplier').d('供应商')} :
                            </span>
                            <span title={goodsInfo.supplierName}>{goodsInfo.supplierName}</span>
                          </li>
                        </ul>
                      </li>
                      <li className="product-attr product-price-attr">
                        <div className="p-content">
                          <span className="product-price">
                            {/* {`￥${this.changeTaxPrice(goodsInfo.taxPrice) ||
                              this.changeTaxPrice(goodsInfo.netPrice)}`} */}
                            ￥{numberRender(goodsInfo.taxPrice || goodsInfo.netPrice, 2, false)}
                          </span>
                        </div>
                      </li>
                      ...
                      <div id="selectAttrs-box" className="selectAttrs-box">
                        {/* {this.showAttrs()} */}
                        <li className="product-attr">
                          <div className="p-label p-label-required">
                            {intl.get('small.common.model.inventory.organization').d('库存组织')}
                          </div>
                          <div className="p-content">
                            <div className="h-lov" style={{ height: 'auto' }}>
                              <a className="lov-link">
                                {intl.get('small.common.button.update').d('修改')}
                              </a>
                            </div>
                          </div>
                        </li>
                        <li className="product-attr">
                          <div className="p-label p-label-required">
                            {intl.get('small.common.model.address').d('地址')}
                          </div>
                          <div className="p-content">
                            <div className="h-lov" style={{ height: 'auto' }}>
                              <a className="lov-link">
                        ...
                        <li className="product-attr">
                          <div className="p-label p-label-required">
                            {intl.get('small.common.model.purchaseOrganization').d('采购组织')}
                          </div>
                          <div className="p-content">
                            <div className="h-lov" style={{ height: 'auto' }}>
                              <a className="lov-link">
                        ...
                        <li className="product-attr">
                          <div className="p-label">
                            {intl.get('small.common.model.uom').d('单位')}：
                          </div>
                          <div className="p-content">{goodsInfo.primaryUomName}</div>
                        </li>
                        <li className="product-attr" style={{ marginTop: '16px' }}>
                          <div className="p-label">
                            {intl.get('small.common.view.quantity').d('数量')}
                          </div>
                          <div className="p-content">
                            <Count />
                          </div>
                        ...
                        <li className="stock">
                          ...
                          <div className="product-service">
                            {intl
                              .get('small.goodsPreview.view.goodsWarning')
                ...
                <div className="p-container">
                  <div className="p-detial">
                    <div id="detail-head" className="p-detail-head">
                      ...
                      >
                        {intl.get('small.common.view.goodsIntroduction').d('商品介绍')}
                      </div>
                      <div
                        id="t2"
                        className={`brand-title1${!activeDetailPanel ? ' d-active' : ''}`}
                      ...
                      >
                        {intl.get('small.common.view.specifications').d('规格参数')}
                      </div>
                    </div>
