 ...
 */
const prompt = 'small.ecClientSite';
@connect(({ smallEcClientSite, loading }) => ({
  smallEcClientSite,
  tenantId: getCurrentOrganizationId(),
  saveLoading: loading.effects['smallEcClientSite/saveECClientSite'],
  editLoading: loading.effects['smallEcClientSite/editECClientSite'],
  fetchLoading: loading.effects['smallEcClientSite/fetchECClientSite'],
  paymentTypeLoading: loading.effects['smallEcClientSite/fetchPaymentType'],
  fetchInitStatusLoading: loading.effects['smallEcClientSite/fetchInitDataStatus'],
  initSyncDataLoading: loading.effects['smallEcClientSite/initSyncData'],
}))
@formatterCollections({
  code: ['small.ecClientSite', 'small.companyBanner', 'small.common', 'small.ecClient'],
})
export default class ECClientSite extends React.Component {
  ...
  fetchECClientSite() {
    ...
    dispatch({
      type: 'smallEcClientSite/fetchECClientSite',
      payload: { tenantId },
    });
  }
    ...
    dispatch({
      type:
        editRowData.ecClientId === undefined
          ? 'smallEcClientSite/saveECClientSite'
          : 'smallEcClientSite/editECClientSite',
      payload: [
        {
          tenantId,
          ...editRowData,
  ...
  onActivate(record) {
    const { dispatch, tenantId } = this.props;
    dispatch({
      type: 'smallEcClientSite/activateAccount',
      payload: {
        tenantId,
  ...
  fetchInitDataStatus(flag, record = {}) {
    ...
    dispatch({
      type: 'smallEcClientSite/fetchInitDataStatus',
      payload: {
        ecClientId: record.ecClientId,
      },
    }).then(res => {
  ...
  onInitSyncData() {
    ...
    const { editRowData } = this.state;
    dispatch({
      type: 'smallEcClientSite/initSyncData',
      payload: {
  ...
  onInitSingleData(step) {
    ...
    const { editRowData } = this.state;
    dispatch({
      type: 'smallEcClientSite/singleInit',
      payload: {
        step,
        ecClientId: editRowData.ecClientId,
      },
  ...
  onSaveNewPwd(value = {}) {
    ...
    dispatch({
      type: 'smallEcClientSite/changePwd',
      payload: {
        tenantId,
        ...value,
  ...
  onHandlePaymentVisible(flag, record = {}) {
    ...
    if (flag) {
      dispatch({
        type: 'smallEcClientSite/fetchPaymentType',
        payload: {
          tenantId,
          ecClientId: record.ecClientId,
        },
      ...
      dispatch({
        type: 'smallEcClientSite/updateState',
        payload: {
          paymentType: [],
        },
      });
  ...
  render() {
    const {
      ...
      saveLoading,
      editLoading,
      smallEcClientSite: { data = [], paymentType = [] },
    } = this.props;
    const { editRowData, modalVisible, paymentTypeVisible, changePwdModalVisible } = this.state;
    const columns = [
      {
        title: intl.get(`${prompt}.model.ecClientSite.ecplatformCode`).d('电商平台编码'),
        width: 120,
