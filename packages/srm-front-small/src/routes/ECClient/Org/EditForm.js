...
@CacheComponent({ cacheKey: '/small/ec-client' })
    ...
    form.validateFields((err, fieldsValue) => {
      if (!ecClientId) {
        if (!err) {
          const params = {
            ...fieldsValue,
            userName: trim(fieldsValue.userName),
            clientType: 'TENANT',
          };
  ...
  render() {
    ...
    return (
      <Modal
        ...
        title={intl.get('small.ecClient.view.message.title.accountMaintain').d('账户维护')}
      ...
      >
        <React.Fragment>
          <FormItem
            label={intl.get('small.ecClient.model.ecClient.customerCode').d('客户代码')}
            {...formLayout}
          >
          ...
          <FormItem
            label={intl.get('small.ecClient.model.ecClient.ecPlatform').d('电商平台')}
            {...formLayout}
          >
              ...
              rules: [
                {
                  ...
                  message: intl.get('hzero.common.validation.notNull', {
                    name: intl.get('small.ecClient.model.ecClient.ecPlatform').d('电商平台'),
                  }),
                },
              ],
            })(
              <Lov textValue={ecPlatformName} onChange={this.setTenantId} code="SMAL.EC_PLATFORM" />
            )}
          </FormItem>
              ...
              rules: [
                {
                  required: true,
                  message: intl.get('hzero.common.validation.notNull', {
                    name: intl.get('small.common.model.companyName').d('公司名称'),
                  }),
                },
              ],
            })(
          ...
          {ecClientId === undefined && (
            <FormItem
              label={intl.get('small.ecClient.model.ecClient.userPassword').d('密码')}
              {...formLayout}
            >
                ...
                rules: [
                  {
                    required: true,
                    message: intl.get('hzero.common.validation.notNull', {
                      name: intl.get('small.ecClient.model.ecClient.userPassword').d('密码'),
                    }),
                  },
                  {
                    max: 120,
