...
export default class MultipleSelectionLov extends React.Component {
  ...
  handleOk() {
    ...
    const {
      dispatch,
      barId,
      mallHomePlate: { barProductList = [], barProductPage = {} },
    } = this.props;
    let repeatFlag = false;
    barProductList.forEach((n) => {
    ...
    dispatch({
      ...
      payload: {
        barProductList: [...barProductList, ...newData].map((n, index) => ({
          ...n,
          orderSeq: index + 1,
        })),
        barProductPage: addItemsToPagination(newData.length, barProductList.length, barProductPage),
      },
    });
    this.setState(
      {
