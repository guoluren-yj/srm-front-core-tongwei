...
@CacheComponent({ cacheKey: '/small/mix-configure' })
    ...
    return (
      <Form>
        <Row gutter={12}>
          <Col span={6}>
            <FormItem
              label={intl.get(`small.maxConfigure.model.tenantNum`).d('租户编号')}
              {...formLayout}
            >
          ...
          <Col span={6}>
            <FormItem
              label={intl.get(`small.maxConfigure.model.tenantName`).d('租户名称')}
              {...formLayout}
            >
