...
export default class CommonModal extends React.Component {
  ...
  handleDefaultChange(e, index) {
    ...
    const newCommonData = commonData.map((data, dataIndex) => {
      ...
      if (index === dataIndex) {
        newData.$form.resetFields(['enabledFlag']);
        newData.$form.setFieldsValue({
          enabledFlag: 1,
          defaultValue: e.target.value ? 1 : 0,
        });
      } else {
        newData.$form.setFieldsValue({
          defaultValue: 0,
          ...
          ['update', 'create'].includes(record._status) ? (
            <Form.Item>
              {record.$form.getFieldDecorator('orderSeq', {
                initialValue: val,
              })(<span>{record.$form.getFieldValue('orderSeq')}</span>)}
            </Form.Item>
          ) : (
            val
          ),
          ...
          ['update', 'create'].includes(record._status) ? (
            <Form.Item>
              {record.$form.getFieldDecorator('enabledFlag', {
                initialValue: val,
              })(<Checkbox disabled={!!record.$form.getFieldValue('defaultValue')} />)}
            </Form.Item>
          ) : (
            val
          ),
