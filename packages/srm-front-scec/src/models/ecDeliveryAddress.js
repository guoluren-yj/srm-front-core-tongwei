...
const addressType = { addressType: 'RECEIVER', ownerType: 'PERSON' };
const addType = { addressType: 'RECEIVER' };

export default {
  namespace: 'ecDeliveryAddress',
  state: {
    list: {},
    pagination: {},
    companyList: [],
  ...
  effects: {
    ...
    *fetchAllDeliveryAddress({ payload }, { call, put }) {
      const response = yield call(fetchEcAddress, { ...addType, ...payload });
      const list = getResponse(response);
      if (list) {
        yield put({
          type: 'updateState',
