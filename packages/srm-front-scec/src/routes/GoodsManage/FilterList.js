        ...
        <Form layout="inline" className="more-fields-form">
          ...
          <Row gutter={12} style={{ display: display ? 'none' : 'block' }}>
                  ...
                  {getFieldDecorator('productStatus')(
                    <Select allowClear>
                      <Option key="APPROVED" value="APPROVED">
                        {intl.get('scec.goodsManage.model.goodsManage.approved').d('审批通过')}
                      </Option>
                      <Option key="UNSHELVE" value="UNSHELVE">
                        {intl.get('scec.goodsManage.model.goodsManage.unSheleve').d('已下架')}
                      </Option>
                    </Select>
                  )}
                </Form.Item>
            ...
            {tabStatus === 'b' && (
              <Col span={6}>
                <Form.Item
                  label={intl.get('scec.common.model.mappingState').d('映射状态')}
                  {...formlayout}
                >
                  {getFieldDecorator('refFlag')(
                    <Select allowClear>
                      <Option key={1}>
                        {intl.get('scec.goodsManage.model.goodsManage.mapped').d('已映射')}
                      </Option>
                      <Option key={0}>
                        {intl.get('scec.goodsManage.model.goodsManage.mapping').d('未映射')}
                      </Option>
                    </Select>
                  )}
                </Form.Item>
