...
const viewPrompt = 'scec.ecCategoryCatalog.view.ecCategoryCatalog';
...
@connect(({ loading, ecCategoryCatalog }) => ({
  ecCategoryCatalog,
  loading: loading.effects['ecCategoryCatalog/fetchEcCategoryCatalog'],
  saveLoading: loading.effects['ecCategoryCatalog/setEcCategoryMap'],
  enableLoading: loading.effects['ecCategoryCatalog/setPermissionSetEnable'],
}))
export default class EcCategoryCatalog extends Component {
  form;
  ...
  fetchEcData(params = {}) {
    const {
      dispatch,
      ecCategoryCatalog: { modalPagination = {} },
    } = this.props;
    ...
    dispatch({
      type: 'ecCategoryCatalog/fetchEcCategoryCatalog',
      payload: {
  ...
  handleImport() {
    openTab({
      key: `/hiam/sub-account-org/data-import/CTGY_CATA_MAP`,
  ...
  handleSetMap(params) {
    const { dispatch } = this.props;
    dispatch({
      type: 'ecCategoryCatalog/setEcCategoryMap',
      payload: params,
    }).then(res => {
      if (res) {
        notification.info({
   ...
   */
  @Bind()
  setCatalogList(_, lovRecord, record) {
    const { catalogList = [] } = this.state;
  ...
  handleDisable(record) {
    const { dispatch } = this.props;
    dispatch({
      type: 'ecCategoryCatalog/setPermissionSetEnable',
      payload: {
        ...record,
  ...
  render() {
    ...
    const { dataSource = [], selectedRows = [], visible, fromType } = this.state;
    const columns = [
      {
              ...
              <FormItem style={{ margin: 0 }}>
                {getFieldDecorator(`catalogId#${record.ecCategoryId}`, {
                  ...
                  <Lov
                    ...
                    code="SCEC.TENANT.CATALOG"
                    lovOptions={{ displayField: 'catalogCode', valueField: 'catalogId' }}
                    onChange={(_, params) => this.setCatalogList(_, params, record)}
                  />
                )}
              </FormItem>
            </Form>
    ...
    return (
      <React.Fragment>
        <Header title={intl.get(`${viewPrompt}.title`).d('集团目录映射')}>
          ...
          {/* <ExcelExport
            requestUrl={`${SRM_SCEC}/v1/${organizationId}/category-catalog-maps/export`}
