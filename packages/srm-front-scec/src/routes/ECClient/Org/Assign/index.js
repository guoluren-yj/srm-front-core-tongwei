...
@formatterCollections({ code: ['scec.assign, scec.common'] })
export default class Assign extends React.Component {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <Header
          title={intl.get('scec.assign.view.ecClient.assignmentSet').d('分配设置')}
          backPath="/scec/ec-client/list"
        >
        ...
        <Content>
          <div className="table-list-search">
            <Row gutter={24}>
              <Col span={6}>
                {intl.get('scec.common.model.ecPlatformName').d('电商名称')}：
                {ecClientData.ecPlatformName}
              </Col>
              <Col span={6}>
                {intl.get('scec.common.model.ecClientName').d('电商公司名称')}：
          ...
          <EditModal
            ...
            title={
              editData.ecClientId === undefined
                ? intl.get('scec.assign.view.message.title.modal.create').d('新建公司分配设置')
                : intl.get('scec.assign.view.message.title.modal.edit').d('维护公司分配设置')
            }
