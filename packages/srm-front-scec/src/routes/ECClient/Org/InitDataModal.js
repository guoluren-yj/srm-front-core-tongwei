...
export default class InitDataModal extends React.Component {
  ...
  render() {
      ...
      },
      {
        ...
        render: (index, record) => {
          if (record.syncStatus !== '1') {
            return (
              <a onClick={() => this.singleInitData(record.interfaceType)}>
                {intl.get('scec.ecClient.model.ecClient.view.ecClient.sync').d('同步')}
              </a>
            );
          } else {
            return (
    ...
    const productPool = initStatus.find(n => n.interfaceType === 'PRODUCT_POOL') || {};
    const price = initStatus.find(n => n.interfaceType === 'PRICE') || {};
    const paymentType = initStatus.find(n => n.interfaceType === 'PAYMENT_MODE') || {};
    const stepStatus = [
      {
