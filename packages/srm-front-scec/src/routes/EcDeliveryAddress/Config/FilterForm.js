...
export default class FilterForm extends PureComponent {
  ...
  render() {
    ...
    const flags = [
      {
        value: 1,
        meaning: intl.get('scec.ecAcquirerAddress.model.enable').d('已设置'),
      },
      {
        value: 0,
        meaning: intl.get('scec.ecAcquirerAddress.model.disabled').d('未设置'),
      },
    ];
    return (
      <Form layout="inline" className="more-fields-form">
        <Row gutter={12}>
          <Col span={6}>
            <FormItem label={intl.get('scec.common.model.company').d('公司')} {...formLayout}>
              {getFieldDecorator('companyName')(<Input />)}
            </FormItem>
          </Col>
          <Col span={6}>
            <FormItem
              label={intl.get('scec.ecDeliveryAddress.model.ecDeliveryAddress.status').d('状态')}
              {...formLayout}
            >
