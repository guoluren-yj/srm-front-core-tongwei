...
@connect(({ loading, ecAcquirerAddress, ecDeliveryAddress }) => ({
  ...
  saveLoading: loading.effects['ecDeliveryAddress/updateEcDeliveryAddress'],
...
export default class EcDeliveryAddress extends Component {
  ...
  fetchEcData(params) {
    const {
      dispatch,
      ecDeliveryAddress: { pagination = {} },
    } = this.props;
    const filterValues = isUndefined(this.form)
      ? {}
      : filterNullValueObject(this.form.getFieldsValue());
    dispatch({
      type: 'ecDeliveryAddress/fetchEcDeliveryAddress',
      payload: {
  ...
  handleSaveData(data = {}) {
    ...
    if (objectVersionNumber) {
      ...
      dispatch({
        type: 'ecDeliveryAddress/addEcDeliveryAddress',
        payload: { ...data, defaultFlag: 0 },
      }).then(res => {
        if (res) {
          notification.success();
  ...
  render() {
    const {
      ecDeliveryAddress: { list = {}, pagination = {} },
      loading,
      saveLoading,
      addLoading,
    } = this.props;
    ...
    return (
      <React.Fragment>
        <Header title={intl.get(`${viewPrompt}.title`).d('收货地址')}>
          <Button type="primary" icon="plus" onClick={this.handleCreateData}>
            {intl.get('hzero.common.button.create').d('新建')}
          </Button>
          <Button onClick={() => this.props.history.push('/scec/ec-delivery-address/config')}>
            默认收货地址配置
          </Button>
        </Header>
        <Content>
