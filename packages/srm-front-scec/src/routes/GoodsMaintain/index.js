...
import { Table, Button, Popconfirm, Badge } from 'hzero-ui';
...
@connect(({ goodsMaintain, loading }) => ({
  goodsMaintain,
  loading: loading.effects['goodsMaintain/fetchGoodsList'],
  ...
  importSoucing: loading.effects['goodsMaintain/importSourcingList'],
}))
@formatterCollections({
  code: ['scec.goodsMaintain', 'scec.common', 'ssrc.inquiryHall', 'scec.goodsMaintainPur'],
})
...
export default class index extends Component {
  ...
  queryGoodsList(params = {}) {
    ...
    dispatch({
      type: 'goodsMaintain/fetchGoodsList',
      payload: {
  ...
  handleToDetail(record = {}) {
    const router = record.productNum
      ? `/scec/goods-maintain/detail?productId=${record.productId}`
      : `/scec/goods-maintain/detail`;
  ...
  handleUpdateGoodsCatelog(params = []) {
    ...
    const productIds = selectedRows.map(item => item.productId);
    dispatch({
      type: 'goodsMaintain/updateCateLog',
      payload: {
        productIds,
        catalogId,
      },
  ...
  importSourcing(params = []) {
    ...
    dispatch({
      type: 'goodsMaintain/importSourcingList',
      payload: {
        resultIds,
        createdParty: 'SUPPLIER',
      },
    }).then(res => {
      if (res && res.length > 0) {
        ...
        if (productId) {
          this.props.history.push(`/scec/goods-maintain/detail?productId=${productId}`);
        }
      }
    });
  }
  ...
  handleImport() {
    openTab({
      key: `/hiam/sub-account-org/data-import/PRODUCT_REF_SUP`,
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <Header title={intl.get('scec.goodsMaintain.view.goodsMaintain.title').d('商品维护')}>
