...
@connect(({ loading, customBar }) => ({
  loading: loading.effects['customBar/fetchCustomBarHistory'],
  customBar,
}))
export default class HistoryModal extends Component {
  ...
  handleSearch(params = {}) {
    const {
      dispatch,
      customBar: { historyPagination = {} },
      barId,
    } = this.props;
    if (params) {
      dispatch({
        type: 'customBar/fetchCustomBarHistory',
        payload: {
  ...
  render() {
    const {
      customBar: { historyPagination = {}, historyList = {} },
      visible,
      loading,
      onCancel,
    } = this.props;
