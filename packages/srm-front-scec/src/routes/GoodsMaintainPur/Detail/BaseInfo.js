...
export default class BaseInfo extends Component {
  ...
  // componentDidMount() {
  //   const { detail: { catalogList = [] } } = this.props;
  //   const list = catalogList.map(item => item.catalogId);
  ...
  /**
   * 采购商创建时需要传supplierTenantId
   */
  @Bind()
  getSupplierId(val, record = {}) {
    const { onGetSupplierId } = this.props;
    onGetSupplierId(record.supplierTenantId, record.supplierName);
  }
  ...
  render() {
    ...
    const { taxRate, tenantId } = this.state;
        ...
        <Form>
          ...
          <Row gutter={48} className="inclusion-row">
            <Col span={8}>
              <Form.Item
                label={intl
                  .get('scec.goodsMaintainPur.model.goodsMaintainPur.companyName')
                  .d('创建公司')}
                {...formlayout}
              >
              ...
              <Form.Item label={intl.get('scec.common.model.supplier').d('供应商')} {...formlayout}>
                {getFieldDecorator('supplierId', {
                  ...
                  <Lov
                    ...
                    onChange={(val, record) => this.getSupplierId(val, record)}
                  />
                )}
              </Form.Item>
            </Col>
              ...
              >
                {getFieldDecorator('sourceFromType', {
                  initialValue: detail.sourceFromType
                    ...
                    : intl.get('scec.common.model.manually.create').d('手工创建'),
                })(
                  <Select allowClear disabled>
                    {sourceType.map((item) => {
                      return (
              ...
              >
                {getFieldDecorator('createdParty', {
                  initialValue: detail.createdParty
                    ...
                    : intl.get('scec.common.model.purchaser').d('采购方'),
                })(<Input disabled />)}
              </Form.Item>
            </Col>
          </Row>
              ...
              <Form.Item label={intl.get('scec.common.model.taxCode').d('税种')} {...formlayout}>
                {getFieldDecorator('taxId', {
                  ...
                  <Lov
                    ...
                    disabled={
                      (detail.taxIncloudedFlag === 1 && detail.sourceFromType === 'SOURCING') ||
                      detail.sourceFromType === 'SHARE'
                    }
                  />
                )}
          ...
          <Row gutter={48} className="inclusion-row">
            ...
            <Col span={8}>
              ...
              >
                ...
                })(<Input disabled={detail.sourceFromType === 'SHARE'} />)}
              </Form.Item>
            </Col>
            <Col span={8}>
              <Form.Item
