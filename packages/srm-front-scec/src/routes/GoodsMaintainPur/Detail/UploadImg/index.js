...
@formatterCollections({ code: ['scec.goodsMaintain', 'scec.common'] })
export default class index extends Component {
    ...
    if (file.status === 'error') {
      notification.error({
        message:
          ...
          intl.get(`scec.common.warning.uploadFail`).d('上传失败'),
      });
      this.setState({
        loading: false,
      });
  ...
  render() {
    ...
    const uploadButton = (
      <div>
        <Icon type="plus" />
        <div>{intl.get(`scec.common.model.uploadImage`).d('上传图片')}</div>
      </div>
    );
    return (
      <React.Fragment>
                ...
                return (
                  <li className={style['upload-li']}>
                    <div className={currentIndex === indexImg ? style.selected : style.noSelect}>
                      <span>{intl.get(`scec.common.view.mainPicture`).d('主图')}</span>
