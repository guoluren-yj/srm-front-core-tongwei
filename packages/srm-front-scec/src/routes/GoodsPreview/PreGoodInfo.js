...
export default class PreGoodInfo extends React.Component {
  constructor(props) {
    ...
    const detailUrl = search.substr(1);
    if (detailUrl && isEmpty(detail)) {
      this.props.history.push(detailUrl);
    }
    this.state = {
      detailUrl,
      goodsInfo: !isEmpty(detail) && detail,
      quantity: 1,
      // productInfo: {},
      selectImg: '',
      attrNotice: false,
  ...
  getProductDetail() {
    ...
    return {
      __html: goodsInfo ? goodsInfo.productDetail && goodsInfo.productDetail.introduction : '',
    };
  }
  ...
  getParams() {
    ...
    const detail = state.detail || {};
    return { __html: detail.productDetail && detail.productDetail.specificationsParam };
  }
  ...
  render() {
    const { goodsInfo = {} } = this.state;
    const {
      goodsInfo: { productImageList = [], attributeDetails = [] },
      detailUrl,
      selectImg,
      activeDetailPanel,
    } = this.state;
    ...
    return (
      <React.Fragment>
        <Header
          title={intl.get('scec.common.view.previewGoods').d('商品预览')}
          backPath={detailUrl}
        />
        <Content>
                        ...
                        <ul>
                          ...
                          {goodsInfo.frameAgreementNum && (
                            <li>
                              ...
                              <span title={goodsInfo.frameAgreementNum}>
                                {goodsInfo.frameAgreementNum}
                              </span>
                            </li>
                          )}
                      ...
                      <div id="selectAttrs-box" className="selectAttrs-box">
                        ...
                        <li className="product-attr">
                          <div className="p-label">
                            {intl.get('scec.goodsPreview.model.goodsPreview.uomName').d('单位')}：
                          </div>
                    ...
                    <div id="content1" className="p-detail-img">
                      {activeDetailPanel ? (
                        ...
                        <div className="attr-body">
                          {attributeDetails.map(item => this.renderAttr(item))}
                        </div>
                      )}
                    </div>
                  </div>
