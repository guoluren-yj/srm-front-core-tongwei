...
import { Button, Form, Popconfirm, Table, Menu, Dropdown, Icon } from 'hzero-ui';
...
@connect(({ loading, groupCustomBar }) => ({
  fetchCurrentCompanyLoading: loading.effects['groupCustomBar/fetchCurrentCompanyValue'],
  loading: loading.effects['groupCustomBar/fetchCustomBarList'],
  groupCustomBar,
}))
@Form.create({ fieldNameProp: null })
@formatterCollections({ code: ['scec.customBar, scec.common', 'scec.shopBasket'] })
@cacheComponent({ cacheKey: '/scec/group-custom-bar/list' })
export default class CustomBar extends Component {
  ...
  handleCreateData() {
    ...
    history.push(`/scec/group-custom-bar/create/0`);
  }
  ...
  handleEditData(record) {
    const { history } = this.props;
    history.push(`/scec/group-custom-bar/detail/0/${record.barId}`);
  }
  ...
  handleCheckData(record) {
    const { history } = this.props;
    history.push(`/scec/group-custom-bar/check-detail/0/${record.barId}`);
  }
  ...
  fetchBarList(page = {}) {
    ...
    dispatch({
      type: 'groupCustomBar/fetchCustomBarList',
      payload,
    });
  }
  ...
  handleShelfAction(record) {
    ...
    dispatch({
      type: 'groupCustomBar/shelfAction',
      payload: {
  ...
  render() {
    ...
    const { historyVisible, barId } = this.state;
