...
export default class Drawer extends PureComponent {
  ...
  render() {
    ...
    return (
      <Modal
        destroyOnClose
        title={intl.get(`${viewPrompt}.title`).d('收货地址')}
        width={520}
        onCancel={onCancel}
        onOk={this.onOk}
        visible={visible}
      ...
      >
        <FormItem
          label={intl.get('scec.companyDeliveryAddress.view.type').d('类型')}
          {...formLayout}
        >
          {getFieldDecorator('ownerType', { initialValue: 'COMPANY' })}
          {getFieldDecorator('ownerTypeMeaning', { initialValue: '公司' })(<Input disabled />)}
        </FormItem>
        <FormItem
          label={intl.get('scec.companyDeliveryAddress.view.companyName').d('公司名称')}
          {...formLayout}
        >
          {getFieldDecorator('companyId', {
            rules: [
              {
                ...
                message: intl.get('hzero.common.validation.notNull', {
                  name: intl.get(`scec.companyDeliveryAddress.view.companyName`).d('公司名称'),
                }),
              },
            ],
            ...
            <Lov
              allowClear={false}
              textValue={tableRecord.companyName}
              code="SPFM.USER_AUTHORITY_COMPANY"
              onChange={companyId => this.props.form.setFieldsValue({ companyId })}
            />
          )}
        </FormItem>
        <FormItem
          label={intl.get('scec.ecAcquirerAddress.entity.roles.contacts').d('联系人')}
          {...formLayout}
        >
          {getFieldDecorator('contactName', {
            rules: [
              ...
              },
              {
                required: true,
                message: intl.get('hzero.common.validation.notNull', {
                  name: intl.get('scec.ecAcquirerAddress.entity.roles.contacts').d('联系人'),
                }),
              },
            ],
          ...
          {getFieldDecorator('mobile', {
            rules: [
              ...
              },
              {
                required: true,
                message: intl.get('hzero.common.validation.notNull', {
                  name: intl.get('hzero.common.phone').d('手机'),
                }),
              },
            ],
