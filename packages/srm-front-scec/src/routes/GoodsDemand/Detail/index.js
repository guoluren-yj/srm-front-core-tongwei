...
import { Form, Table, Button, Collapse, Icon, Spin } from 'hzero-ui';
...
@connect(({ goodsDemand, loading }) => ({
  goodsDemand,
  loading: loading.effects['goodsDemand/fetchProductDetail'],
}))
export default class Detail extends Component {
  ...
  fetchProductDetail() {
    ...
    dispatch({
      type: 'goodsDemand/fetchProductDetail',
      payload: {
        productId,
      },
    });
  ...
  preview() {
    ...
    const router = {
      pathname: `/scec/goods-demand/goods-preview-only`,
      state: {
        detail,
      },
      search: `?${detailUrl}`,
  ...
  viewLadderPriceModal(productId = undefined) {
    const { dispatch } = this.props;
    dispatch({
      type: 'goodsDemand/fetchLadderPriceTable',
      payload: { productId },
    });
    this.setState({
  ...
  render() {
    const {
      goodsDemand: { detail = {}, ladderPriceData = [] },
      loading,
    } = this.props;
    ...
    return (
      <React.Fragment>
        <Header
          title={intl.get('scec.goodsDemand.view.goodsDemand.title').d('商品查询')}
          backPath="/scec/goods-demand/list"
        >
