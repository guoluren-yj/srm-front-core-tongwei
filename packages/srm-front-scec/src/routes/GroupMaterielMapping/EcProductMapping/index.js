...
@connect(({ loading, groupMaterielMapping }) => ({
  ...
  loading: loading.effects['groupMaterielMapping/fetchEcProductMappingList'],
  saveLoading: loading.effects['groupMaterielMapping/setEcProductMap'],
  deleteLoading: loading.effects['groupMaterielMapping/delEcProductMap'],
}))
export default class CategoryMapping extends PureComponent {
  ...
  handleImport() {
    openTab({
      key: `/hiam/sub-account-org/data-import/SCEC.GROUP_EC_PRODUCT_ITEM_REF`,
  ...
  fetchList(page = {}) {
    ...
    const filterValues = this.filterForm
      ...
      : {};
    dispatch({
      type: 'groupMaterielMapping/fetchEcProductMappingList',
      payload: {
        page,
        ...filterValues,
      },
  ...
  singleSetMap(record = {}, lovRecord = {}, type = true) {
    const {
      dispatch,
      groupMaterielMapping: { ecProductMapList = [] },
    } = this.props;
    const { ecProductId } = record;
    ...
    this.setState({ editRow, isSave: true }, () => {
      const dataSource = ecProductMapList.map(item => {
        return item.ecProductId === ecProductId ? editRow : item;
      });
      dispatch({
        type: 'groupMaterielMapping/updateState',
        payload: { ecProductMapList: dataSource },
      });
    });
  }
  ...
  setMaps(params = []) {
    ...
    const ecProductMapList = params.length ? params : [editRow];
    if (params.length) this.setState({ visible: false });
    if (ecProductMapList.length) {
      const {
        dispatch,
        groupMaterielMapping: { ecProductMapPagination = {} },
      } = this.props;
      dispatch({
        type: 'groupMaterielMapping/setEcProductMap',
        payload: getEditTableData(ecProductMapList),
      }).then(res => {
        if (res) {
          notification.info({
            message: intl
          ...
          this.setState(
            ...
            () => this.fetchList(ecProductMapPagination)
          );
        }
      });
    }
  ...
  delMaps() {
    ...
    dispatch({
      type: 'groupMaterielMapping/delEcProductMap',
  ...
  getColumns() {
    return [
      ...
      {
        title: intl.get('scec.groupMaterielMapping.model.ecProductNum').d('商品编码'),
        width: 100,
        dataIndex: 'ecProductNum',
      },
      {
        title: intl.get('scec.groupMaterielMapping.model.ecProductName').d('商品名称'),
        dataIndex: 'ecProductName',
      },
      {
        title: intl.get('scec.groupMaterielMapping.model.ecCategoryName').d('商品分类'),
        width: 100,
  ...
  render() {
    ...
    const { ecProductMapList = [], ecProductMapPagination = {} } = groupMaterielMapping;
    ...
    const tableProps = {
      ...
      dataSource: ecProductMapList,
      pagination: ecProductMapPagination,
      rowSelection: {
    ...
    return (
      <div className="matermap-tab-body">
        ...
        <div className="table-operation">
          ...
          <ExcelExport
            requestUrl={`${SRM_SCEC}/v1/${organizationId}/group-ec-product-item-refs-export`}
