...
import { Table, Modal, Row, Col, Form, Input, Badge } from 'hzero-ui';
...
@connect(({ goodsApprove, loading }) => ({
  goodsApprove,
  loading: loading.effects['goodsApprove/fetchGoodsListHwfp'],
...
export default class GoodsApprove extends Component {
  ...
  constructor(props) {
    super(props);
    const { productIds } = props.match.params;
    this.state = {
  ...
  fetchGoodsList(params = {}) {
    ...
    dispatch({
      type: 'goodsApprove/fetchGoodsListHwfp',
      payload: {
  ...
  render() {
    ...
    const { visible, productIds } = this.state;
    // const filterList = {
    //   status,
    //   sourceType,
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (val, record) => {
          return (
            <Link
              to={`/pub/scec/goods-approve-hwfp/detail?productId=${record.productId}&productIds=${productIds}`}
            >
              {val}
            </Link>
          );
