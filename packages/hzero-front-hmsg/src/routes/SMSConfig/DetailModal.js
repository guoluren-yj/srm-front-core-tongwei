...
import { Modal, Form, Input, Select, Icon, Tooltip } from 'hzero-ui';
      ...
      >
        <Form>
          ...
          <FormItem
            ...
            label={
              <span>
                {intl.get('hmsg.email.model.email.filterStrategy').d('安全策略')}
                <Tooltip
                  title={
                    <>
                      {intl
                        .get('hmsg.email.model.email.blackListTips')
                        .d('黑名单：默认所有人都发送，维护在黑名单中的账户不发送；')}
                      <br />
                      {intl
                        .get('hmsg.email.model.email.whiteListTips')
                        .d(
                          '白名单：开启后，维护黑名单中的账户则会发送，测试、验证环境建议开启白名单'
                        )}
                    </>
                  }
                >
              ...
              rules: [
                {
                  required: true,
                  message: intl.get('hzero.common.validation.notNull', {
                    name: intl.get('hmsg.email.model.email.filterStrategy').d('安全策略'),
                  }),
                },
              ],
            })(
