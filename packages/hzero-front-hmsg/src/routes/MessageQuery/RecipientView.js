...
import { Modal, Table, Badge } from 'hzero-ui';
...
export default class RecipientView extends React.PureComponent {
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: intl.get('hmsg.messageQuery.model.messageQuery.sendFlag').d('发送状态'),
        width: 120,
        dataIndex: 'sendFlag',
        render: (value) => {
          return (
            <Badge
              ...
              text={
                value === 1
                  ? intl.get('hmsg.messageQuery.model.messageQuery.send').d('已发送')
                  : intl.get('hmsg.messageQuery.model.messageQuery.noSend').d('未发送')
              }
            />
          );
        }
