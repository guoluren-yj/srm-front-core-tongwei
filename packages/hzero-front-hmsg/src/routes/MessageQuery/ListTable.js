...
export default class ListTable extends PureComponent {
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: intl.get('hmsg.messageQuery.model.messageQuery.templateCode').d('消息模板编码'),
        dataIndex: 'templateCode',
        width: 150,
      },
      {
        title: intl.get('hmsg.messageQuery.model.messageQuery.templateServerCode').d('消息发送配置编码'),
        dataIndex: 'templateServerCode',
        width: 150,
      },
      {
      ...
      },
      {
        ...
        width: 100,
        render: (val, record) => <span>{statusRender(statusAlias[val] || val, record.trxStatusMeaning)}</span>,
      },
      {
            ...
            operators.push({
              ...
              ele: (
                <ButtonPermission
                  ...
                  disabled={record.trxStatusCode === 'R'}
                >
                  {['I', 'R'].includes(record.trxStatusCode) ? intl.get('hmsg.messageQuery.view.button.resend2').d('继续发送')
                  : intl.get('hmsg.messageQuery.view.button.resend').d('重试')}
                </ButtonPermission>
              ),
              len: 2,
              title: ['I', 'R'].includes(record.trxStatusCode) ? intl.get('hmsg.messageQuery.view.button.resend2').d('继续发送')
              : intl.get('hmsg.messageQuery.view.button.resend').d('重试'),
            });
          }
          return operatorRender(operators);
