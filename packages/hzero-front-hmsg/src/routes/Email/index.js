...
export default class Email extends PureComponent {
  ...
  fetchEmail(params = {}, query) {
    ...
    const formValues =
      this.filterFormRef.current.props.form
        ? filterNullValueObject(this.filterFormRef.current.props.form.getFieldsValue())
        : {};    
    dispatch({
      type: 'email/fetchEmail',
      payload: { page: pagination, ...formValues, ...params, ...query },
    });
  }
