...
@connect(({ receiverType, loading }) => ({
  ...
  queryNoAssignPositionListLoading: loading.effects['receiverType/queryNoAssignPositionList'],
  queryNoAssignRoleListLoading: loading.effects['receiverType/queryNoAssignRoleList'],
}))
...
export default class ReceiverType extends Component {
  ...
  handleSearchFormSearch() {
    this.handleSearch({}, this.props.form.getFieldsValue());
  }
   ...
   */
  @Bind()
  handleDrawerOk(values, onlySaveHeader = false, callback) {
    const {
      dispatch,
    ...
    if (typeModeCode === 'EXT_USER') {
      if (values.receiverTypeId) {
        if (!onlySaveHeader && ((Array.isArray(data) && data.length > 0) || !validData)) {
  ...
  @Bind()
  queryNoAssignPositionList(payload) {
    const { dispatch } = this.props;
    return dispatch({
      type: 'receiverType/queryNoAssignPositionList',
      payload,
    });
  }

  @Bind()
  queryNoAssignRoleList(payload) {
    const { dispatch } = this.props;
    return dispatch({
      type: 'receiverType/queryNoAssignRoleList',
      payload,
    });
  }
  ...
  render() {
    ...
    const drawerProps = {
      ...
      queryNoAssignUserGroupListLoading,
      queryNoAssignPositionList: this.queryNoAssignPositionList,
      queryNoAssignPositionListLoading,
      queryNoAssignRoleList: this.queryNoAssignRoleList,
      queryNoAssignRoleListLoading,
      extTypeList,
      dispatch,
      assignDataSource,
