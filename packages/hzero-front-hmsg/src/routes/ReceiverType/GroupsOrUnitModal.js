...
export default class GroupsModal extends Component {
  ...
  handleSearch(pagination) {
    ...
    if (typeModeCode === 'POSITION') {
      const { queryNoAssignPositionList } = this.props;
      queryFunc = queryNoAssignPositionList;
    }
    if (typeModeCode === 'ROLE') {
      const { queryNoAssignRoleList } = this.props;
      queryFunc = queryNoAssignRoleList;
    }
    if (queryFunc) {
      queryFunc({
        id: receiverTypeId,
  ...
  getColumns() {
    ...
    } else if (typeModeCode === 'POSITION') {
      return [
        {
          title: intl.get('hmsg.receiverType.model.receiverType.positionCode').d('岗位编码'),
          dataIndex: 'positionCode',
          width: 200,
        },
        {
          title: intl.get('hmsg.receiverType.model.receiverType.positionName').d('岗位名称'),
          dataIndex: 'positionName',
        },
      ];
    } else if (typeModeCode === 'ROLE') {
      return [
        {
          title: intl.get('hmsg.receiverType.model.receiverType.roleCode').d('角色编码'),
          dataIndex: 'roleCode',
          width: 200,
        },
        {
          title: intl.get('hmsg.receiverType.model.receiverType.roleName').d('角色名称'),
          dataIndex: 'roleName',
        },
      ];
    }
  ...
  handleRowSelectionChange(_, selectedRows = []) {
    ...
    if (typeModeCode === 'USER_GROUP') {
      ...
      this.setState({
        selectedRows,
        selectedRowKeys: selectedRows.map((record) => record.positionId),
      });
    } else if (typeModeCode === 'ROLE') {
      this.setState({
        selectedRows,
        selectedRowKeys: selectedRows.map((record) => record.roleId),
      });
    } else {
      this.setState({
        selectedRows: [],
  ...
  render() {
    const {
      ...
      queryNoAssignPositionListLoading = false,
      queryNoAssignRoleListLoading = false,
      typeModeCode,
      form,
    } = this.props;
    const {
    ...
    const isPosition = typeModeCode === 'POSITION';
    const isRole = typeModeCode === 'ROLE';
    const title = isUserGroup
      ...
      : isPosition
      ? intl.get('hmsg.receiverType.view.title.positionAdd').d('新增岗位')
      : isRole
      ? intl.get('hmsg.receiverType.view.title.roleAdd').d('新增角色')
      : '';
    return (
      <Modal
        destroyOnClose
              ...
              <Form.Item
                ...
                label={
                  isUserGroup
                    ...
                    : isPosition
                    ? intl.get('hmsg.receiverType.model.receiverType.positionCode').d('岗位编码')
                    : isRole
                    ? intl.get('hmsg.receiverType.model.receiverType.roleCode').d('角色编码')
                    : ''
                }
              >
              ...
              <Form.Item
                ...
                label={
                  isUserGroup
                    ...
                    : isPosition
                    ? intl.get('hmsg.receiverType.model.receiverType.positionName').d('岗位名称')
                    : isRole
                    ? intl.get('hmsg.receiverType.model.receiverType.roleName').d('角色名称')
                    : ''
                }
              >
                {form.getFieldDecorator(
