...
export async function queryNoAssignPositionList(receiverTypeId, query) {
  const isTenant = isTenantRoleLevel();
  const reqUrl = isTenant
    ? `${HZERO_MSG}/v1/${getCurrentOrganizationId()}/receiver-types/lines/position/${receiverTypeId}`
    : `${HZERO_MSG}/v1/receiver-types/lines/position/${receiverTypeId}`;
  return request(reqUrl, {
    method: 'GET',
    query: parseParameters(query),
  });
...
export async function queryNoAssignRoleList(receiverTypeId, query) {
  const isTenant = isTenantRoleLevel();
  const reqUrl = isTenant
    ? `${HZERO_MSG}/v1/${getCurrentOrganizationId()}/receiver-types/lines/roles/${receiverTypeId}`
    : `${HZERO_MSG}/v1/receiver-types/lines/roles/${receiverTypeId}`;
  return request(reqUrl, {
    method: 'GET',
    query: parseParameters(query),
  });
