...
export async function queryMessageList(parmas) {
  const { onlyCountFlag, oldTotalElements } = parmas;
  return request(
      ...
      : `${HZERO_MSG}/v1/messages`,
    {
      ...
      query:
        onlyCountFlag === 'Y'
          ? { onlyCountFlag, oldTotalElements, asyncCountFlag: 'Y', ...parmas }
          : { oldTotalElements, asyncCountFlag: 'Y', ...parmas },
    }
  );
}
...
export async function batchRetry(params) {
  return request(`${HZERO_MSG}/v1/messages/resend-batch`, {
    method: 'POST',
    body: params,
  });
}
...
export async function sendCustomMessage(params) {
  return request(`${HZERO_MSG}/v1/${getCurrentOrganizationId()}/messages/custom-send`, {
    method: 'POST',
    body: params,
  });
}