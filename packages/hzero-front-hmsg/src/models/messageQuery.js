...
export default {
  ...
  effects: {
    ...
    *queryMessageList({ payload }, { call, put }) {
    ...
    // 获取消息数据
    *queryMessageListPage({ payload }, { select, call, put }) {
      if (payload.needCountFlag === 'Y') {
        const { page = 0, size = 10, ...params } = payload;
        const { messageData } = yield select((state) => state.messageQuery);
        const res = yield call(queryMessageList, { page, size, ...params, onlyCountFlag: 'Y' });
        const list = getResponse(res);
        if (list) {
          yield put({
            type: 'updateState',
    ...
    *batchRetry({ payload }, { call }) {
      const res = yield call(batchRetry, payload);
      return getResponse(res);
    },
  },
  reducers: {
