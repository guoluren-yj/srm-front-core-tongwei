...
import { FormDS, ResponseFailRankingDS } from '@/stores/customize/traceLogsDS';
...
class ResponseFailRankingCard extends React.Component {
  constructor(props) {
    super(props);
    this.ResponseFailRankingDS = new DataSet(
      ResponseFailRankingDS({
  ...
  handleFieldUpdate({ name, value }) {
    if (name === 'timeRange') {
      ...
      this.ResponseFailRankingDS.setQueryParameter(
        'startDate',
      ...
      this.ResponseFailRankingDS.setQueryParameter('tenantId', tenantId);
    }
    this.ResponseFailRankingDS.query();
  }
  ...
  handleReload() {
    ...
    this.ResponseFailRankingDS.setQueryParameter('tenantId', tenantId);
    this.ResponseFailRankingDS.setQueryParameter('startDate', dateTimeRender(startDate));
    this.ResponseFailRankingDS.setQueryParameter('endDate', dateTimeRender(endDate));
    this.ResponseFailRankingDS.query();
  }
  ...
  getOption() {
    ...
    return {
      tooltip: {
        ...
        label: {
          show: true,
          formatter: `${getLang('TRACE_LOGS_RESPONSE_FAIL_COUNT')}:{value}`,
        },
      },
      grid: {
        left: '1%',
      ...
      series: [
        {
          name: getLang('TRACE_LOGS_RESPONSE_FAIL_COUNT'),
          type: 'bar',
          label: {
            show: true,
            position: 'right',
  ...
  render() {
    ...
    return (
      <div id="responseFailRankingCard">
        <Card
          key="responseFailRankingCard"
          title={<h3>{name}</h3>}
          bordered={false}
          extra={
        ...
        >
          <Spin dataSet={this.ResponseFailRankingDS}>
...
export default ResponseFailRankingCard;
