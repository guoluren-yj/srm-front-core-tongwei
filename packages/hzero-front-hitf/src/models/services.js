...
import { createPagination, getResponse } from 'hzero-front/lib/utils/utils';
...
import {
  ...
  batchDeleteDocumentParam,
  batchDeleteTestCaseParam,
} from '../services/servicesService';
import { queryDetail } from '../services/interfacesService';
...
export default {
  ...
  effects: {
    *queryIdpValue(params, { call, put }) {
      const enumMap = getResponse(
        yield call(queryMapIdpValue, {
          ...
          operatorList: ASSERTION_CONDITION,
          assertionSubjects: ASSERTION_SUBJECT,
          dsRequestMethods: DS_REQUEST_METHOD,
          invokeUrlVersions: INVOKE_URL_VERSION,
        })
      );
      yield put({
        type: 'updateState',
    ...
    *queryReqDemo({ payload }, { call, put }) {
      ...
      if (response) {
        let { code = '' } = response;
        const { pathname } = new URL(API_HOST);
        code = code.replace('/v1/soap/invoke', `${pathname}${HZERO_HITF}/v1/soap/invoke`);
        code = code.replace('/v1/rest/invoke', `${pathname}${HZERO_HITF}/v1/rest/invoke`);
        yield put({
          type: 'updateState',
          payload: { reqDemo: code },
        });
      }
    },
    ...
    *queryViewCode({ payload }, { call, put }) {
      ...
      if (response) {
        ...
        if (type === 'HTTP') {
          ...
          code = code.replace('/v1/soap/invoke', `${API_HOST}${HZERO_HITF}/v1/soap/invoke`);
          code = code.replace('/v1/rest/invoke', `${API_HOST}${HZERO_HITF}/v1/rest/invoke`);
        }
        yield put({
          type: 'updateState',
          payload: {
    ...
    *queryViewData({ payload }, { call, put }) {
      ...
      const response = getResponse(res);
      if (response) {
        if (response.interfacesDetail) {
          const { publishUrl = '' } = response.interfacesDetail;
          set(response, 'interfacesDetail.publishUrl', `${API_HOST}${HZERO_HITF}${publishUrl}`);
        }
        if (response.requestDemoString) {
          const { pathname } = new URL(API_HOST);
          let { requestDemoString = '' } = response;
          requestDemoString = requestDemoString.replace(
            '/v1/soap/invoke',
            `${pathname}${HZERO_HITF}/v1/soap/invoke`
          );
          requestDemoString = requestDemoString.replace(
            '/v1/rest/invoke',
            `${pathname}${HZERO_HITF}/v1/rest/invoke`
          );
          set(response, 'requestDemoString', requestDemoString);
        }
        yield put({
          type: 'updateState',
          payload: {
    ...
    *getTemplateServerDetail(params, { call }) {
    ...
    // 批量删除接口文档参数
    *batchDeleteDocumentParam({ payload }, { call }) {
      const res = yield call(batchDeleteDocumentParam, payload);
      return getResponse(res);
    },
    // 批量测试用例参数
    *batchDeleteTestCaseParam({ payload }, { call }) {
      const res = yield call(batchDeleteTestCaseParam, payload);
      return getResponse(res);
    },
  },
  reducers: {
