  ...
  componentDidMount() {
    ...
    document.documentElement.onmouseup = (event) => {
      ...
      if (
        ...
        this.props.getRelationScript(relation, this.props.sourceData, this.props.targetData);
        sourceDom = null;
      }
      this.props.changeIconStatus();
      this.setState({ ...defaultState });
        ...
        <svg width="100%" height="100%" version="1.1" xmlns="http://www.w3.org/2000/svg">
          ...
          <g>
            {relation
              .filter((item) => {
                let showLine = false;
                if (searchingSource === true && searchingTarget === false) {
                  if (item.source.displayTree === true) {
                    showLine = true;
                  }
                } else if (searchingSource === false && searchingTarget === true) {
                  if (item.target.displayTree === true) {
                    showLine = true;
                  }
                } else if (searchingSource === true && searchingTarget === true) {
                  if (item.target.displayTree === true && item.source.displayTree === true) {
                    showLine = true;
                  }
                }
                return (
                  item.source.key &&
                  item.target.key &&
                  (!(searchingSource || searchingTarget) || showLine)
                );
              })
              .map((item) => {
                return (
