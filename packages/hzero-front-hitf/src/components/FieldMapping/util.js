...
export const calCoord = (data = [], FieldMapping) => {
  ...
  return data.map((item) => {
    ...
    const sourceNode = temp.source;
    const targetNode = temp.target;
    sourceNum = findNodeIndex(sourceData, sourceNode);
    targetNum = findNodeIndex(targetData, targetNode);
    const sourcePoint =
    ...
    temp.source.x = 0;
    if (getOffset(sourcePoint).top !== 0) {
    ...
    if (getOffset(targetPoint).top !== 0) {
...
export const getFieldType = (data) => {
  return isNumber(data)
    ? 'Number'
    : /^\[object\s(.*)\]$/.exec(Object.prototype.toString.call(data))[1];
};
...
export const transformLine = (script, sourceData = [], targetData = []) => {
  const splitList = _.split(script, '---');
  ...
  // 去除\n,\t,用于字符串匹配
  let temp = _.replace(splitList[1], /\s+/g, '');
  temp = _.replace(temp, /[\r\n]/g, '');
  // TODO: 命名空间提取，暂时用不到
  // let namespaces = {};
  // const headers = _.split(splitList[0], 'ns ');
  // headers.forEach((item, index) => {
  //   if (index > 0) {
  //     const nsList = _.words(item, /[^ ]+/g);
  //     const key = nsList[0];
  //     let value = nsList[1];
  //     value = _.replace(_.replace(value, /\s+/g, ''), /[\r\n]/g, '');
  //     namespaces = { ...namespaces, [key]: value };
  //   }
  // });
  const destructList = destructStript(temp);
  let relations = [];
...
/**
 * 将名空间变量替换为.
 * @param {String} value key或者value
 */
const replaceNsWithDot = (value) => {
  let temp = _.replace(value, /\..*?#/g, '.');
...
const findNodeByPath = (path = '', treeList = []) => {
  // 将array[0]这类的结构替换为array
  const paths = path.split('.').map((item) => _.replace(item, /\[[1-9][0-9]*\]$/, ''));
  const { length } = paths;
  let currentNode = {};
...
const destructStript = (script) => {
  ...
  return temps.map((item) => replaceNsWithDot(item));
};
...
export const getSameLineRel = (sourceData = [], targetData = []) => {
  ...
  const recursion = (sources, targets) => {
    sources.forEach((source, sourceIndex) => {
      ...
      if (target && !isObject(source) && !isObject(target)) {
 ...
 * 连线关系转换为脚本
 */
export const getRelationScript = (relations = [], sourceData = [], targetData = []) => {
  const assembleList = [];
  relations.forEach((relation) => {
    ...
    const sourcePath = [sourceParentPath, sourceName].includes(undefined)
      ? null
      : `${sourceParentPath}.${sourceName}`;
    const sourceTypePath = [sourceParentTypePath, sourceType].includes(undefined)
      ? null
  ...
  return translateObject(relationScript, sourceData, targetData);
};
 ...
 */
const translateObject = (relationScript, sourceData, targetData) => {
  ...
  const recursion = (name, script, level = 0, preIndex = 0, path = '', mapTimes = 0) => {
    ...
    _.keys(formatData).forEach((key, index) => {
      ...
      const { nsKey, namespaces: tempNamespaces1 } = setNsPrefix(
        targetPath,
        targetData,
        namespaces
      );
      namespaces = tempNamespaces1;
      const { renderType, mapFiled, prefixField = '', prefixFields = '' } = getRenderType(
        targetPath,
        relationScript
      );
      if (renderType === 'mapFunc') {
        const { nsKeyPath, namespaces: tempNamespaces2 } = setNsPrefixForValue(
          prefixFields,
          sourceData,
          namespaces
        );
        namespaces = tempNamespaces2;
        // map函数中如果出现其它map函数，则prefixField需加上01
        let prefix = mapTimes > 0 ? `${prefixField}01` : nsKeyPath;
        prefix = _.isEmpty(prefixField) ? '' : `${prefix}.`;
        // 同一层级且都为数组，生成map函数
        const mapStr = `${newLine}${createTab(
          level
        )}${nsKey}: ${prefix}${mapFiled} map (${mapFiled}01, ${_.camelCase(
          `indexOf ${mapFiled}01`
        ...
        // 生成数组对象结构
        const arrayStr = `${newLine}${createTab(level)}${nsKey}: [{REPLACE_FLAG_${
          level + 1
        ...
        // 生成map对象结构
        const str = `${newLine}${createTab(level)}${nsKey}: {REPLACE_FLAG_${
          level + 1
        }_${index}\n${createTab(level)}}${dot}REPLACE_FLAG_${level}_${preIndex}`;
        ...
        const { nsKeyPath, namespaces: tempNamespaces3 } = setNsPrefixForValue(
          replacedPath,
          sourceData,
          namespaces
        );
        namespaces = tempNamespaces3;
        const str = `${newLine}${createTab(
          level
        )}${nsKey}: ${nsKeyPath}${dot}REPLACE_FLAG_${level}_${preIndex}`;
        funcContent = _.replace(funcContent, `REPLACE_FLAG_${level}_${preIndex}`, str);
      }
    });
  };
  ...
  let nsContent = '';
  const isXml = isXmlFormat(targetData);
  nsContent = nsPrefixScript(namespaces);
  funcContent = deletePlaceHolder(funcContent, relationScript);
  funcContent = replaceEmpty(funcContent);
  funcContent = insertScriptHeader(funcContent, isXml, nsContent);
  return funcContent;
};
 ...
 * 判断是否为xml脚本
 */
const isXmlFormat = (targetData) => {
  const { dataType } = targetData[0] || {};
  return dataType === 'xml';
};

const nsPrefixScript = (namespaces) => {
  let ns = '';
  namespaces.forEach((item, index) => {
    ns = `${ns}ns ns${index} ${item} \t\n`;
  });
  return ns;
};
 ...
 * 设置key命名空间
 */
const setNsPrefix = (path, data, namespaces) => {
  let temps = namespaces;
  const currentNode = findNodeByPath(path, data) || {};
  const { name, namespace } = currentNode;
  if (!_.isEmpty(namespace)) {
    temps.push(namespace);
  }
  temps = _.uniq(temps);
  const nsIndex = _.indexOf(temps, namespace);
  const nsKey = nsIndex === -1 ? name : `ns${nsIndex}#${name}`;
  return { nsKey, namespaces: temps };
};

/**
 * 设置value命名空间
 */
const setNsPrefixForValue = (path, data, namespaces = []) => {
  let temps = namespaces;
  let newPath = '';
  let nsKeyPath = '';
  path.split('.').forEach((key) => {
    newPath = mergePath(newPath, key);
    const currentNode = findNodeByPath(newPath, data) || {};
    const { name, namespace } = currentNode;
    if (!_.isEmpty(namespace)) {
      temps.push(namespace);
    }
    temps = _.uniq(temps);
    const nsIndex = _.indexOf(temps, namespace);
    const nsKey = nsIndex === -1 ? key : `ns${nsIndex}#${name}`;
    nsKeyPath = mergePath(nsKeyPath, nsKey);
  });
  return { nsKeyPath, namespaces: temps };
};
...
const deletePlaceHolder = (replacedStr, relationScript) => {
  ...
  // 去除REPLACE_FLAG的副作用，残留的数字
  str = _.replace(str, /,[0-9]+/g, ',');
  // 去除空行
 ...
 * ---
 */
export const insertScriptHeader = (script, isXml = false, namespace = '') => {
...
/**
 * 根据sourceData和targetData过滤掉不存在relations中的数据
 * @param {Array} sourceData 来源数据
 * @param {Array} targetData 目标数据
 * @param {Array} relations 原关系数据
 */
export const screenRelation = (sourceData, targetData, relations) => {
  const newRelations = relations.filter((relation) => {
    const {
      source: { parentPath: sourceParentPath, name: sourceName },
      target: { parentPath: targetParentPath, name: targetName },
    } = relation;
    const sourcePath = mergePath(sourceParentPath, sourceName);
    const targetPath = mergePath(targetParentPath, targetName);
    const sourceNode = findNodeByPath(sourcePath, sourceData);
    const targetNode = findNodeByPath(targetPath, targetData);
    return !_.isEmpty(sourceNode) && !_.isEmpty(targetNode);
  });
  return newRelations;
};

const mergePath = (parentPath, currentName) => {
  return _.isEmpty(parentPath) ? currentName : `${parentPath}.${currentName}`;
};

export const xmlParse = (inputData) => {
  let index = 0;
  const recursion = (
    data,
    iterate = [],
    ...
    const dot = _.isEmpty(parentPath) ? '' : '.';
    const formatData = mergeArray(data);
    const keys = _.keys(formatData);
    temps = keys
      .filter((key) => key !== '$' && key !== '$ns' && key !== '_')
      .map((key) => {
        const item = formatData[key];
        // 命名空间
        const {
          $ns = {},
        ...
        const { uri: namespace = '' } = $ns;
        //  TODO: 暂时不解释标签属性
        // 额外的标签属性
        // _.keys(attrs).filter(attrKey => attrs[attrKey].prefix !== 'xmlns').forEach(attrKey => {
        //   const attr = attrs[attrKey];
        //   _.set(item, `@${attr.local}`, attr.value);
        // });
        const fieldType = _.isEmpty(others) ? getFieldType(value) : getFieldType(item);
        const name = _.isEmpty(namespace) ? key : key.split(':')[1] || key;
        const temp = {
          name,
          level,
          parentName,
