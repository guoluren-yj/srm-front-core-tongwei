...
import { TagRender, enableRender, operatorRender } from 'hzero-front/lib/utils/renderer';
...
@formatterCollections({
  code: ['hzero.common', 'hitf.structureField', 'hitf.chargeSet', 'view.validation'],
})
@withProps(
  () => {
    // 结构字段头 数据源
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        align: 'center',
        width: 100,
        renderer: ({ value, text }) => TagRender(value, STRUCTURE_CATEGORY_TAGS, text),
      },
      {
        name: 'composition',
        align: 'center',
        width: 100,
        renderer: ({ value, text }) => TagRender(value, STRUCTURE_COMPOSITION_TAGS, text),
      },
      { name: 'structureDesc' },
      {
        name: 'bizUsage',
        align: 'center',
        width: 130,
        renderer: ({ value, text }) => TagRender(value, STRUCTURE_BIZ_USAGE_TAGS, text),
      },
      {
        name: 'enabledFlag',
        align: 'center',
        ...
        renderer: ({ record }) => {
          const actions = [
            ...
            },
            {
              ...
              key: 'disable',
              len: 2,
              title: intl.get('hzero.common.disable').d('禁用'),
            },
            {
              ele: (
                <ButtonPermission
