...
import { saveDataConversionDetail } from '@/services/appManageInterfaceService';
...
const DataConversion: React.FC<DataConversionProps> = ({
    ...
    if (!validate) {
      ...
      if (type === 'MODULE') {
        ...
        if (!conversionValidate) {
          ...
          saveDataConversionDetail({ ...formValue, sourceConvert: tableValue, interfaceId: id, tenantId, orderSeq }).then(res => {
            const result = getResponse(res);
            if (result) {
              notification.success({});
              tableDs.query();
  ...
  }, [tableDs, dataFormDs, dataConversionDs, dataScriptDs, dataInputDs, dataOutputDs, tenantId]);
  ...
  }, [dataFormDs, dataConversionDs, dataScriptDs, dataInputDs, dataOutputDs, tenantId]);
  ...
  return (
    <SearchBarTable
      searchCode="HITF.INTERFACE_CONFIGURATION_WORKBENCH.APP.DATACONVERSION"
      dataSet={tableDs}
      columns={columns}
      rowNumber
