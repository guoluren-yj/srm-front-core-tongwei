...
import { queryInputDs, queryOutputDs, queryEditInputDs, queryEditOutputDs, queryConditionDs, getDataConversionDetail, getDataConversionSourceTable } from '@/services/appManageInterfaceService';
...
const DataConversionModal: React.FC<DataConversionModalProps> = ({ dataFormDs, dataConversionDs, dataScriptDs, dataInputDs, dataOutputDs, data, editFlag }) => {
  ...
  useEffect(() => {
    const {
      convertType,
      openInterfaceConvertTnId,
    } = data;
    // 编辑的情况下，显示表格数据
    if (convertType && openInterfaceConvertTnId) {
      setState(preState => ({
        ...preState,
        spinFlag: true,
      }));
      getDataConversionDetail(convertType, openInterfaceConvertTnId).then(res => {
        const result = getResponse(res);
        if (result) {
          if (dataFormDs.current) {
            dataFormDs.current.set({ ...result, openInterfaceConvertTnId });
          }
          if (convertType === 'MODULE') {
            getModuleTable(openInterfaceConvertTnId);
          } else if (convertType === 'SOURCE') {
            getDataConversionSourceTable(openInterfaceConvertTnId).then(sourceTableRes => {
              const sourceTableResult = getResponse(sourceTableRes);
            ...
            getConditionTable(openInterfaceConvertTnId);
          }
        }
      }).finally(() => {
        setState(preState => ({
  ...
  const handleSourceCategory = useCallback(() => {
    if (dataConversionDs.current) {
      dataConversionDs.current.set({ sourceValueObj: '', sourceValue: '', lineParamName: '', structure: '' });
    }
  }, [dataConversionDs]);
  ...
  const getModuleTable = useCallback((moduleHeaderId) => {
   ...
   // 编辑详情页时获取条件表格值
   const getConditionTable = useCallback((moduleHeaderId) => {
    queryConditionDs(moduleHeaderId).then(res => {
      const result = getResponse(res);
      if (result) {
        dataScriptDs.loadData(result);
      }
    }).finally(() => {
      setState(preState => ({
        ...preState,
      ...
      <Form dataSet={dataFormDs} labelLayout={LabelLayout.float}>
        ...
        {state.convertTypeValue && state.convertTypeValue !== 'MODULE' && (
          <Row gutter={16}>
            ...
            <Col span={12} className={styles['form-col']}>
              <TextField name="structure" disabled />
            </Col>
          </Row>
        )}
  ...
  const handleAdd = useCallback(() => {
    ...
    dataScriptDs.create({ conditionCode: `applicationLineCode${uuid}` }, 0);
  }, [dataScriptDs]);
