...
const SreRightTable: React.FC<any> = ({ tableDs, defaultSelectId, handleSave, typeFlag }) => {
    ...
    (): ColumnProps[] => [
      {
        name: 'paramName',
        width: 200,
        editor: (record) => (!record.get('id') || record.get('interfaceParamLineId')) && !typeFlag ?
      ...
      {
        name: 'paramDescribe',
        editor: (record) => (!record.get('id') || record.get('interfaceParamLineId')) && !typeFlag ? <TextField /> : false,
      },
      {
        name: 'paramType',
        width: typeFlag ? 120 : 80,
        editor: (record) => (!record.get('id') || record.get('interfaceParamLineId')) && !typeFlag ? <Select /> : false,
      },
      {
        name: 'paramLength',
        width: typeFlag ? 120 : 80,
        editor: (record) => (!record.get('id') || record.get('interfaceParamLineId')) && !typeFlag ? <NumberField /> : false,
      },
      {
        name: 'notNull',
      ...
      {
        name: 'remark',
        editor: (record) => (!record.get('id') || record.get('interfaceParamLineId')) && !typeFlag ? <TextArea /> : false,
      },
    ],
    [tableDs, defaultSelectId]
  );
  ...
  return (
    <div className={styles['params-table']}>
      <SearchBarTable
        mode={TableMode.tree}
        selectionMode={typeFlag ? SelectionMode.none : SelectionMode.treebox}
