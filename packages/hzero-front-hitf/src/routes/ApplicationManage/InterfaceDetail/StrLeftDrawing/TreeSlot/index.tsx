...
const Index = ({ propsItem = {}, openModal, handleDelete, typeFlag }: IIndex) => {
  const { id } = propsItem;
  const getTitle = type => {
    let title = '';
    let thisClass = '';
  ...
  const menu = (
    <Menu>
      ...
      <Menu.Item
        key="delete"
        onClick={({ domEvent }) => handleDelete(domEvent, propsItem)}
      >
        <span>
  ...
  return (
    <div className={styles['model-source-tree-slot']}>
      ...
      <div className={styles['model-source-tree-slot-menu']} style={{ display: typeFlag ? 'none' : '' }}>
