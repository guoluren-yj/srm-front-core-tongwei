...
import { createTreeData, renameParamTable, deleteParamTable, getTreeData, saveParamsTable } from '@/services/interfaceDefinitionService';
...
const Params: React.FC<any> = ({ id, paramsCreateDs, tableDs, childRef, tabKey }) => {
  ...
  const handleCreate = useCallback(async (selectId) => {
    ...
    if (!validate) {
      ...
      createTreeData({ ...formValue, interfaceId: id }, typeFlag).then(res => {
  ...
  const handleRename = useCallback(async (selectId) => {
    ...
    if (!validate) {
      ...
      const formValue = paramsCreateDs.current.toJSONData();
      renameParamTable({ ...formValue }, typeFlag).then(res => {
        const result = getResponse(res);
        if (result) {
          notification.success({});
          queryTree(selectId);
  ...
  }, [paramsCreateDs, state.defaultSelectId]);

  // 删除关系表
  const handleDeleteTable = useCallback((event, idValue) => {
    ...
    Modal.confirm({
      ...
      onOk: () => handleConfirmDelete(idValue),
    });
  ...
  const handleConfirmDelete = useCallback((id) => {
    ...
    deleteParamTable(id, typeFlag).then(res => {
      const result = getResponse(res);
      if (result) {
        notification.success({});
  ...
  }, [state.dataSource, state.defaultSelectId, tableDs]);
  ...
  const renderNoData = useMemo(() => {
    ...
    return (
      <div className={styles['params-nodata']}>
        <div className={styles['none-content-svg']}><NoneSvg /></div>
        ...
        <div className={styles['params-nodata-tip']}>
          ...
          <span
            ...
            style={{ cursor: id ? 'pointer' : 'not-allowed', color: !id ? '#000' : '#36c2cf' }}
