...
import { MODAL_FORM_ITEM_LAYOUT } from 'hzero-front/lib/utils/constants';
...
export default class ParamsDrawer extends PureComponent {
  ...
  @Bind()
  renderTimeZoneSelect(data) {
    return (
      <Select allowClear>
        {data.length &&
          data.map(({ value, meaning }) => (
            <Option key={value} value={value}>
              {meaning}
            </Option>
          ))}
  ...
  @Bind
  renderFormatSelect(data) {
    const { form } = this.props;
    const paramValueType = form.getFieldValue('paramValueType');
    return (
      <Select allowClear>
        {data.length &&
          data
            .filter((item) => item.parentValue === paramValueType)
            .map(({ value, meaning }) => (
              <Option key={value} value={value}>
                {meaning}
              </Option>
    ...
    return (
      ...
      >
        <Form>
          ...
          {/* <FormItem
            label={intl.get('hitf.services.model.services.sourceTimeZone').d('来源时区')}
            {...MODAL_FORM_ITEM_LAYOUT}
          >
            {getFieldDecorator('sourceTimeZone', {
              initialValue: isEdit ? currentParamData.sourceTimeZone : undefined,
            })(this.renderTimeZoneSelect(timeZones))}
          </FormItem>
          <FormItem
            label={intl.get('hitf.services.model.services.targetTimeZone').d('目标时区')}
            {...MODAL_FORM_ITEM_LAYOUT}
          >
            {getFieldDecorator('targetTimeZone', {
              initialValue: isEdit ? currentParamData.targetTimeZone : undefined,
            })(this.renderTimeZoneSelect(timeZones))}
          </FormItem>
          <FormItem
            label={intl.get('hitf.services.model.services.dateTimeFormat').d('时间格式')}
            {...MODAL_FORM_ITEM_LAYOUT}
          >
            {getFieldDecorator('dateTimeFormat', {
              initialValue: isEdit ? currentParamData.dateTimeFormat : undefined,
            })(this.renderFormatSelect(dateFormats))}
          </FormItem> */}
          <FormItem
          ...
          <FormItem label={intl.get('hzero.common.explain').d('说明')} {...MODAL_FORM_ITEM_LAYOUT}>
