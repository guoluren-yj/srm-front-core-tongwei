...
import { Button, Drawer, Radio, Spin, Tabs, Dropdown, Menu } from 'hzero-ui';
...
import { reqParamParse, resParamParse } from '@/services/servicesService';
...
export default class Document extends Component {
  ...
  defaultReqMimeType = 'raw';

  defaultRespMimeType = 'raw';
   ...
   */
  @Bind()
  openPreview(invokeVersion) {
    const {
      services: { currentInterface },
      namespace,
      serverCode,
    } = this.props;
    const { interfaceCode, tenantId } = currentInterface;
    window.open(
      `${
        window.$$env.BASE_PATH || '/'
      }pub/hitf/document-view?namespace=${namespace}&serverCode=${serverCode}&interfaceCode=${interfaceCode}&invokeVersion=${invokeVersion}&tenantId=${tenantId}`
    );
  }
   ...
   */
  @Bind()
  handleSaveConfig(dataStr, actionType, cancelFun) {
    const {
  ...
  handleSaveParams({ values, flag, cb = (e) => e }) {
    const {
      dispatch,
      services: { documentData, currentInterface },
    } = this.props;
    dispatch({
   ...
   */
  @Bind()
  handleBatchDeleteParams(data) {
    const {
      dispatch,
      services: { documentData },
    } = this.props;
    return dispatch({
      type: 'services/batchDeleteDocumentParam',
      payload: {
        data,
        documentId: documentData.documentId,
      },
  ...
  handleRecognizeParams() {
    const {
      dispatch,
      tenantId,
      services: { currentInterface, documentData },
    } = this.props;
    const { interfaceId } = currentInterface;
    dispatch({
  ...
  @Bind()
  handleDownloadFile(invokeVersion) {
    // 下载文件
    const {
      services: { currentInterface },
    } = this.props;
    const api = `${HZERO_HITF}/v1/${
      isTenantRoleLevel()
        ? `${getCurrentOrganizationId()}/interfaces/${
            currentInterface && currentInterface.interfaceId
          }/export`
        : `interfaces/${currentInterface && currentInterface.interfaceId}/export`
    }`;
    downloadFileByAxios({
      requestUrl: api,
      method: 'POST',
      queryParams: [{ name: 'invokeVersion', value: invokeVersion }],
    });
  }

  @Bind()
  invokeVersionMenu(invokeFunc = () => {}) {
    const {
      services: { enumMap = {} },
    } = this.props;
    const { invokeUrlVersions = [] } = enumMap;
    return (
      <Menu>
        {invokeUrlVersions.map((item) => (
          <Menu.Item key={item.value}>
            <a onClick={() => invokeFunc(item.value)}>{item.meaning}</a>
          </Menu.Item>
        ))}
      </Menu>
    );
  ...
  render() {
    ...
    const respMimeTypes = (mimeTypes && mimeTypes.slice(0, 3)) || [];
    ...
    const commonParamsProps = {
      ...
      onBatchDelete: this.handleBatchDeleteParams,
      mainColumns: [
        {
    ...
    return (
      <Drawer {...drawerProps}>
            ...
            <TabPane tab={intl.get('hitf.document.view.title.demo').d('示例')} key="demo">
              ...
              >
                ...
                <TabPane
                  tab={
                    <QuestionPopover
                      text={intl.get('hitf.document.view.title.successDemo').d('成功示例')}
                      message={intl
                        .get('hitf.document.view.title.successDemoTip')
                        .d(
                          '透传目标接口处理成功的响应，依次配置HTTP响应码（200/404/500……）、响应内容类型Content-Type（application/json或者application/xml）、示例内容（响应报文样例）'
                        )}
                    />
                  }
                  key="successDemo"
                ...
                <TabPane
                  tab={
                    <QuestionPopover
                      text={intl.get('hitf.document.view.title.failedDemo').d('失败示例')}
                      message={intl
                        .get('hitf.document.view.title.failedDemoTip')
                        .d(
                          '透传目标接口处理失败的响应，依次配置HTTP响应码（200/404/500……）、响应内容类型Content-Type（application/json或者application/xml）、示例内容（响应报文样例）'
                        )}
                    />
                  }
                  key="failedDemo"
            ...
            >
              ...
              >
                <TabPane
                  tab={
                    <QuestionPopover
                      text={intl.get('hitf.document.view.title.detailDes').d('详细说明')}
                      message={intl
                        .get('hitf.document.view.title.detailDesTip')
                        .d('对整个接口功能说明')}
                    />
                  }
                  key="detailedDes"
                ...
                <TabPane
                  tab={
                    <QuestionPopover
                      text={intl.get('hitf.document.view.title.requestDes').d('请求说明')}
                      message={intl
                        .get('hitf.document.view.title.requestDesTip')
                        .d('对外部接口请求参数进行说明')}
                    />
                  }
                  key="requestDes"
                ...
                <TabPane
                  tab={
                    <QuestionPopover
                      text={intl.get('hitf.document.view.title.responseDes').d('响应说明')}
                      message={intl
                        .get('hitf.document.view.title.responseDesTip')
                        .d('对外部接口响应进行说明')}
                    />
                  }
                  key="responseDes"
        ...
        <div className={styles['hiam-interface-model-btns']}>
          ...
          <Dropdown overlay={this.invokeVersionMenu(this.handleDownloadFile)}>
            <Button style={{ marginRight: 8 }} onClick={() => this.handleDownloadFile('v1')}>
              {intl.get('hitf.document.view.button.export').d('导出接口文档')}
            </Button>
          </Dropdown>
          <Dropdown overlay={this.invokeVersionMenu(this.openPreview)}>
            <Button style={{ marginRight: 8 }} onClick={() => this.openPreview('v1')}>
