...
class AssertionCard extends React.Component {
  ...
  componentDidMount() {
    ...
    this.assertionDS.loadData(data);
  }

  async _validate() {
    const validate = await this.assertionDS.validate();
    return validate;
  }

  _toData() {
    return this.assertionDS.toData();
  }
  ...
  renderFormItem(items, record) {
    const { operatorList, assertionSubjects, filterOption } = this.props;
    ...
    formItems.push(
      ...
      >
        {assertionSubjects
          .filter((item) => (filterOption ? item.tag === filterOption : true))
          .map((item) => (
