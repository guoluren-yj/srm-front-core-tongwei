...
import { getEditTableData, tableScrollWidth } from 'hzero-front/lib/utils/utils';
...
export default class TestCase extends Component {
  ...
  @Bind()
  async handleBatchDeleteParams(params = []) {
    const filterParams = params.filter((item) => item._status !== 'create');
    return this.props
      .dispatch({
        type: 'services/batchDeleteTestCaseParam',
   ...
   */
  @Bind()
  viewCode(type = 'JAVA', requestMethod = 'GET') {
    const { dispatch, interfaceId } = this.props;
    const { currentUsecaseId } = this.state;
    dispatch({
      type: 'services/queryViewCode',
      payload: { interfaceId, usecaseId: currentUsecaseId, type, requestMethod },
    });
  }
  ...
  render() {
    ...
    const {
      ...
      services: {
        ...
        testCaseDetail = {},
        paramsWithValues = {},
        enumMap: { codeTypes, mimeTypes, usecaseTypes, dsRequestMethods },
      },
      interfaceId,
      listLoading,
      createLoading,
