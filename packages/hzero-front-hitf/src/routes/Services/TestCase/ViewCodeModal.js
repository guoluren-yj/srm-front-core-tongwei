...
export default class ViewCodeModal extends Component {
  ...
  changeCodeType(value) {
    const { requestMethod } = this.state;
    this.setState(
      {
        type: value,
      },
      () => {
        ...
        onChangeType(value, requestMethod);
      }
    );
  }
  ...
  render() {
    ...
    const { type, requestMethod } = this.state;
    return (
      <Modal
        width={700}
        visible={visible}
        ...
        footer={[
          ...
          <Button type="primary" key="copy" onClick={() => this.handleCopy()}>
      ...
      >
        <Spin spinning={loading}>
          ...
          {!hiddenRequestMethodOption && (
            <Select
              className={classnames(styles['type-select'])}
              onChange={this.changeRequestMethod}
              value={requestMethod}
            >
              {dsRequestMethods.length &&
                dsRequestMethods.map(({ value, meaning }) => (
                  <Option key={value} value={value}>
                    {meaning}
                  </Option>
                ))}
