...
import { DataSet, Form, Button, TextField, Select, Table, Modal, Lov } from 'choerodon-ui/pro';
...
import { CAST_TYPE_TAGS } from '@/constants/constants';
...
export default class Detail extends React.Component {
  constructor(props) {
    ...
    const { editFlag = true } = location.state || {};
    this.state = {
  ...
  async handleFetchDetail(id) {
    ...
    this.headerFormDS.setQueryParameter('queryType', 'SJYS');
  ...
  handleGotoDetail(id) {
    ...
    dispatch(
      routerRedux.push({
        pathname: `/hitf/data-mapping/detail/${id}`,
      })
    );
  }
  ...
  handleOpenCastLineDrawer(isNew, record) {
    ...
    const castLineDrawerProps = {
      ...
      tenantId: this.headerFormDS.current.get('tenantId'),
  ...
  handleOpenFormulaDrawer(record) {
    ...
    const { readOnly, historyFlag } = this.state;
  ...
  render() {
    ...
    return (
      <>
        <Header title={getLang('DETAIL')} backPath="/hitf/data-mapping/list">
          {!readOnly && (
            <ButtonPermission
              permissionList={[
                {
