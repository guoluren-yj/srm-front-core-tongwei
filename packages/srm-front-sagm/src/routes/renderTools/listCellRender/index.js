...
const getNameValue = ({ label, color, labelMinWidth, showLabel, content, render }) => {
  ...
  return (
    <p className={styles['c7n-list-cell-row']}>
      {showLabel && (
        <span className={labelClass} title={label} style={{ minWidth: labelMinWidth + 1 }}>
          {label}
        </span>
      )}
      {render ? (
        ...
        <span className={contentClass} title={content} style={{ color }}>
          {text}
        </span>
      )}
    </p>
