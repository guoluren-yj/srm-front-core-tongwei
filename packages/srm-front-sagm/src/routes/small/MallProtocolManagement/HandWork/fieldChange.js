...
// 判断为有效数字||大数字
export function isCalcNumber(num) {
  return typeof num === 'number' || math.isBigNumber(num);
}

// 未税单价可修改获得含税
export function getTaxPrice({ tax, unitPrice, precision }) {
  const _value = new BigNumber(unitPrice) || 0;
  const _tax = math.plus(math.multipliedBy(new BigNumber(tax) || 0, 0.01), 1);
  const taxPrice = math.multipliedBy(_value, _tax);
  return getPrecisionVal(taxPrice, precision);
}

// 含税单价可修改获得未税
export function getUnitPrice({ tax, taxPrice, precision }) {
  const _value = new BigNumber(taxPrice) || 0;
  const _tax = math.plus(math.multipliedBy(new BigNumber(tax) || 0, 0.01), 1);
  const unitPrice = math.div(_value, _tax);
...
export function taxChange(record, lovInfo, changeField) {
  ...
  const priceChangeFn = changeField === 'unitPrice' ? unitPriceChange : taxPriceChange;
  if (isCalcNumber(price)) {
    priceChangeFn(price, record, precision, item.taxRate);
  }
  const agreementLadders = record.get('agreementLadders');
  if (agreementLadders && agreementLadders.length > 0 && item.taxRate !== undefined) {
    const newList = agreementLadders.map((m) => {
      const prices = {};
    ...
    record.set('agreementLadders', newList);
...
export function currencyChange(record, lovInfo, changeField) {
  ...
  if (agreementLadders && agreementLadders.length > 0 && item.currencyId) {
