...
// key = backPath
export function setWorkFlowControl(key) {
  workFlowControl[key] = true;
}

// 商品详情
export function openSkuDetail({ record, recordData, type = 'pur', backPath, tabState = {} }) {
  const { spuId, skuId, agreementBusinessType } =
    recordData || record.get(['spuId', 'skuId', 'agreementBusinessType']);
  const isWorkFlow = workFlowControl[backPath];
  ...
  const searchPara = qs.stringify(
    filterNullValueObject({
      ...
      skuId,
      backPath,
      menuId: isWorkFlow ? getMenuId() : null,
