...
import { DataSet, Table, Modal, Button, Form, TextArea } from 'choerodon-ui/pro';
...
export function showRecordModal({
  ...
  const commonFields = [
    { name: 'operateByName', label: intl.get('sagm.common.view.operateName').d('操作人') },
    {
      name: 'operateDate',
      type: 'dateTime',
      label: intl.get('sagm.common.view.operateTime').d('操作时间'),
    },
...
export function agmRecordModal({ url = '', params = {}, width = 700 }, type = 'PUR') {
  ...
  const purFields = [
    { name: 'realName', label: intl.get('small.common.view.operateName').d('操作人') },
    {
      name: 'operatedTime',
      type: 'dateTime',
      label: intl.get('small.common.view.operateTime').d('操作时间'),
    },
...
export function confirmBefore({
  ...
  const { reasonLabel = '', reasonName = '', required = true } = field;
  const waningDs = new DataSet({
    autoCreate: true,
    fields: [
      {
        name: reasonName,
        label: reasonLabel || intl.get('sagm.common.view.confirmReason').d('原因'),
        required,
        maxLength: 100,
      },
    ],
      ...
      <>
        <Alert
          className={classnames({
            [styles['warning-help']]: true,
            [styles['warning-color']]: type === 'warning',
            [styles['info-color']]: type === 'info',
          })}
          message={<OverflowTip overflow2>{message}</OverflowTip>}
          type={type}
          showIcon
          iconType={type === 'info' ? 'info' : 'error'}
          closable
        />
        {customizeCode ? (
          customizeForm(
            ...
            <Form dataSet={waningDs} columns={1} labelLayout="float" styles={{ width: '100%' }}>
              <TextArea name={reasonName} showLengthInfo />
            </Form>
          )
        ) : (
