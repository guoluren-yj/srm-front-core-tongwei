...
/**
 * 查询常用菜单
 * {SRM_PLATFORM}/v1/{tenantId}/dbd-user-menu-sets/queryFunction
 */
 export async function queryCommonMenus() {
  const tenantId = getCurrentOrganizationId();
  return request(`/spfm/v1/${tenantId}/dbd-user-menu-sets/queryFunction`, {
    method: 'GET',
  });
}
...
/**
 * 创建常用菜单
 * {SRM_PLATFORM}/v1/{tenantId}/dbd-user-menu-sets/createMenu
 */
 export async function createCommonMenus(body = []) {
  const tenantId = getCurrentOrganizationId();
  return request(`/spfm/v1/${tenantId}/dbd-user-menu-sets/createMenu`, {
    method: 'POST',
    body,
  });
}
