...
import type { BaseEnvConfig } from 'utils/iocUtils';
import { getEnvConfig } from 'utils/iocUtils';
import { getCurrentOrganizationId, isTenantRoleLevel } from 'utils/utils';
import { queryIdpValue } from './lov';
...
export async function fetchSupportLanguageList() {
  const { HZERO_PLATFORM } = getEnvConfig<BaseEnvConfig>();
  return request(
    `${HZERO_PLATFORM}/v1/${
      isTenantRoleLevel() ? `${getCurrentOrganizationId()}/` : ''
    }tenant-languages/list`,
    {
      method: 'GET',
    }
  );
...
export async function queryTenantLanguagesConfig({ tenantId, isTenant }) {
  const { HZERO_PLATFORM } = getEnvConfig<BaseEnvConfig>();
  return request(`${HZERO_PLATFORM}/v1/${isTenant ? `${tenantId}/` : ''}/tenant-languages/list`, {
    method: 'GET',
  });
}
...
export async function queryTimeZoneList(dayLightFlag) {
  if (dayLightFlag) {
    const { HZERO_PLATFORM } = getEnvConfig<BaseEnvConfig>();
    return request(`${HZERO_PLATFORM}/v1/time-zone`, {
      method: 'GET',
      query: {
        page: 0,
        size: 1000,
    ...
    return queryIdpValue('HIAM.TIME_ZONE');
  }
}