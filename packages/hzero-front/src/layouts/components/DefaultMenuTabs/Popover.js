...
class DefaultMenuTabs extends React.Component {
  ...
  handleMenuTabCloseOthers(pane, e) {
    ...
    removeOtherMenuTab({ tab: pane }).then((nextActiveTabKey) => {
      ...
      removeTabs.forEach((t) => {
        cleanCache(t.key);
        menuTabEventManager.emit('close', { tabKey: t.key, tabPath: t.path });
      });
    });
  }
  ...
  handleMenuTabRefresh(pane, e) {
    ...
    menuTabEventManager.emit('refresh', { tabKey: pane.key, tabPath: pane.path });
  }
  ...
  render() {
    ...
    return (
      <Dropdown
        ...
        overlay={
          <Menu>
            ...
            <Menu.Item
              ...
              className="default-menu-tabs-context-menu-item default-menu-tabs-context-menu-item-group"
            >
        ...
        trigger="contextMenu"
        hidden={pane.key !== contextMenuVisibleKey || !contextMenuVisible}
        onHiddenChange={(hidden) => {
          this.handleContextMenuTrigger(pane, !hidden);
        }}
      >
        {pane.path === '/workplace' ? (
          <span>
            <Icon type="home-o" key="icon" />
