...
import { Link, Redirect, Route, Switch, withRouter } from 'dva/router';
        ...
        >
          {map(tabs, (pane, index) => (
            <TabPane
              ...
              key={pane.key}
              className="hzero-common-tab-pane"
              data-tab-path={pane.path}
            >
              <Content className="page-container" key={refreshMenuTabsKeyMap.get(pane.key)}>
                {(!menuLoad || layoutLoading) && activeTabKey === pane.key ? (
