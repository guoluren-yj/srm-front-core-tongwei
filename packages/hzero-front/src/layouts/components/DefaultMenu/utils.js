 ...
 */
export function renderIcon(menu, classPrefix = 'main-menu-item', style, className = [] /* , isHover */) {
  const classNames = [...className, styles[`${classPrefix}-icon`]];
  const { name, icon, path } = menu;
  classNames.push(styles[`${classPrefix}-icon-img`]);
  if (!isUndefined(icon)) {
    return name.includes('choerodon') && path.includes('hlcd') ? (
      <Icon type={icon} style={{ fontSize: '12px', ...style }} className={classNames.join(' ')} />
    ) : (
      <Icons style={{ width: '12px', ...style }} type={icon} className={classNames.join(' ')} />
    );
  } else {
    return (
      <Icons
