...
class PublicLayout extends React.Component {
  componentDidMount() {
    ...
    const showPublicPage = () => {
      ...
      const { language: paramLanguage, organizationId } = routerParam;
      const lang = paramLanguage || cookie.get('language') || 'zh_CN';
      dispatch({
    ...
    dispatch({
      type: 'global/initPublicInfo',
      payload: {
        host: window.location.host,
      }
    }).finally(showPublicPage);
  }
  ...
  componentWillUnmount() {
    this.props.dispatch({
      ...
      payload: {
        // 此处置为false, 以解决从public路由跳转到普通路由时不查询菜单数据的问题
        menuLoad: false,
        tabsIsInit: false,
      },
    });
