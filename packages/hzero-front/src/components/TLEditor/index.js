...
import { queryTL, fetchSupportLanguageList } from '../../services/api';
...
export default class TLEditor extends React.Component {
  ...
  openModal() {
    ...
    const { list, text, language, resLangList = [], currentSystemLang = [] } = this.state;
    if (Array.isArray(resLangList) && resLangList.length === 0) {
      // 仅调用一次接口，后续更新都使用缓存的语言数据
      queryTL({ fieldName, _token }).then((response) => {
        if (getResponse(response)) {
          const res = currentSystemLang.map((item, index) => {
            const value = response.find(i => i.code === item.code);
            return value || {
              code: item.code,
              name: item.name,
              value: '',
  ...
  render() {
    ...
    // 若没传inputSize且设置max rule,则以max为准
    if (isEmpty(inputSize) && otherProps['data-__meta'] && otherProps['data-__meta'].rules) {
      const maxRule = otherProps['data-__meta'].rules.find((item) => 'max' in item);
      if (maxRule) {
        inputSize.zh = maxRule.max;
        inputSize.en = maxRule.max;
      }
    }
