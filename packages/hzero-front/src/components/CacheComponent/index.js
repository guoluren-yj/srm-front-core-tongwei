      ...
      loadCache(component, callback) {
        if (cacheKey) {
          ...
          if (form && cache && cache.form) {
            // 在加载lov的text缓存时，如果查询表单不是在页面组件的挂载时渲染，必定出现instance为空的情况
            // 通过检查form中已注册字段是不是都存在instance来判断是否可以加载缓存
            let mayNotMountField = true;
            if (isObject(cache.form)) {
              Object.keys(cache.form).forEach((item) => {
              ...
              if (cache.__LOV_TEXT_FIELD__) {
                Object.keys(cache.__LOV_TEXT_FIELD__).forEach((k) => {
                  ...
                  if (
                    ...
                    instance.setState({ text: cache.__LOV_TEXT_FIELD__[k] });
                  }
                });
              }
            };
            form.setFieldsValue(cache.form, cb);
          }
