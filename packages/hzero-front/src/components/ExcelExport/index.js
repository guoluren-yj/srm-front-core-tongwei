...
import C7Notification from 'utils/notification';
import intl from 'utils/intl';
import {
  getResponse,
  listenDownloadError,
  filterNullValueObject,
  checkGetRequestUrlLength,
} from 'utils/utils';
...
export default class ExcelExport extends React.PureComponent {
  constructor(props) {
    ...
    this.state = {
      ...
      showAsync: true,
      defaultRequestMode: props.requestMode,
    };
    if (props.onRef) {
      props.onRef(this);
    }
  ...
  queryColumnData(config) {
    const { requestUrl, method = 'GET', defaultSelectAll = false, form, requestMode } = this.props;
    ...
    queryColumn({ requestUrl, method })
      .then((res) => {
        ...
        nextState.defaultRequestMode = requestMode || response?.defaultRequestMode; // 不为真 就为假
  ...
  handleExport(config = {}) {
    ...
    if (!checkedKeys || (Array.isArray(checkedKeys) && checkedKeys.length === 0)) {
      ...
      if (newQueryParams.async === 'true') {
        initiateAsyncExport({ requestUrl, queryParams: params, method, queryData, beforeRequest: checkGetRequestUrlLength }).then((res) => {
          if (res) {
