...
export default class CKEditor extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    window.addEventListener('popstate', () => {
      // to do
      if (editor && editor.commands && editor.commands.maximize && editor.commands.maximize.state === 1) {
        editor.destroy();
  ...
  getContent() {
    ...
    const content = this.editor ? this.editor.getData() : '';
    let str = content || '';
    if (privateBucket) {
        ...
        try {
          ...
          if (this.element) {
            this.editor = CKEDITOR.replace(this.element, {
              ...
              extraPlugins: 'function_placeholder,variable_placeholder',
              ...config,
            });
          }
...
const getEditorUrl = () => {
  ...
  if (!window.CKEDITOR_BASEPATH) {
    let publicUrl = getEnvConfig().PUBLIC_URL || '/';
