...
import { findIndex, forEach, isArray, isEmpty, isObject, isString, slice, split, values } from 'lodash';
import EventEmitter from 'event-emitter';
import qs from 'query-string';
import uuid from 'uuid/v4';
import { getDvaApp, getEnvConfig } from './iocUtils/helpers';

import { getCurrentUser, getCurrentOrganizationId, getCurrentRole, getUserOrganizationId, isTenantRoleLevel } from './utils/user';
import { getAccessToken } from './utils/token'; 
import { checkThridEmbeddedBrowser } from './utils/common'; 

import { getSession, setSession } from './utils/storage';

import { checkModuleLoaded } from './utils/module';
...
/**
 * @type {{string: Map<string, Map<string, [key, Function][]>>}}
 * 各模块使用menuTabEventManager时，无法在页面跳转时销毁历史监听句柄，统一在这里存储一份，在下次初始化时自行销毁
 */
export const menuTabEventCache = {
  /**
   * withProps使用
   */
  withProps: new Map(),
   ...
   * 模块自定义事件使用
   */
  moduleCustomize: new Map(),
};
Object.defineProperties(menuTabEventCache, {
  withProps: {
    configurable: false,
    writable: false,
  },
...
const clearEventListener = ({ tabKey, tabPath }) => {
  if (tabPath) {
    Object.keys(menuTabEventCache).forEach(cacheType => {
      const handlersCache = menuTabEventCache[cacheType].get(tabPath);
      if (handlersCache) {
        handlersCache.forEach((handlersByEventType, eventType) => {
          handlersByEventType.forEach((handler) => {
            menuTabEventManager.off(eventType, handler);
          });
          handlersCache.set(eventType, []);
        });
      }
    });
  }
};
menuTabEventManager.on('close', clearEventListener);
menuTabEventManager.on('refresh', clearEventListener);
 ...
 */
export function listenAfterFreeHandler(cacheType, eventKey, callback) {
  const { activeTabKey, tabs } = window.dvaApp._store.getState().global || {};
  const activeTab = tabs.find(i => i.key === activeTabKey) || {};

  if (activeTab.path) {
    const oldHandlers = menuTabEventCache[cacheType].get(activeTab.path);
    if (oldHandlers) {
      let currentHandlers = oldHandlers.get(eventKey);
      if (!currentHandlers) {
        currentHandlers = [];
        oldHandlers.set(eventKey, currentHandlers);
      }
      currentHandlers.push(callback);
    } else {
      const currentHandlers = [callback];
      const newEventCacheByPath = new Map();
      newEventCacheByPath.set(eventKey, currentHandlers);
      menuTabEventCache[cacheType].set(activeTab.path, newEventCacheByPath);
    }
    menuTabEventManager.on(eventKey, callback);
  }
}
...
// 角色工作台 tab
export const roleWorkplaceTab = {
  title: 'hzero.common.title.workspace.role',
  closable: false,
  key: '/swbh/role-workbench',
...
function getIsPubLayout() {
  const state = getDvaApp()._store.getState();
  const { global: { isPubLayout = false } = {} } = state;
  return isPubLayout;
}

function getUnauthorizedRouteList() {
  const state = getDvaApp()._store.getState();
  const { global: { unauthorizedRouteList = [] } = {} } = state;
  return unauthorizedRouteList;
}
...
// 根据子路由查找父路由
function getParentRoute(childRoute) {
  const routerData = getRouterData();
  if (!childRoute || !childRoute.path || !routerData) {
    return undefined;
  }
  return values(routerData).find(
    (route) =>
      route &&
      route.components &&
      route.components.length &&
      route.components.some((r) => r.path === childRoute.path),
  );
}
...
function getTabFromKey(key) {
  ...
  let findTab = getDefaultWorkbenchTab();
  forEach(tabData, (tab) => {
    if (tab.key === key) {
      findTab = tab;
      return false;
 ...
 * @returns {*}
 */
function getInitialActiveTabKey(initialTabs) {
  const roleId = getCurrentRole().id;
  const prevTabs = getSession(`${menuTabSessionKey}-${roleId}`);
  let key = isEmpty(prevTabs.activeTabKey) ? getDefaultWorkbenchTab().path : prevTabs.activeTabKey;
  if (key && initialTabs && initialTabs.length) {
    const targetTab = initialTabs.find(t => getRealPathName() === t.path);
    if (targetTab && targetTab.key !== key) {
      key = targetTab.key;
    }
  }
  return key;
...
export function parsePath(path) {
  let pathname = path && isString(path) ? path : '/';
  let search = '';
  let hash = '';
...
function searchFormatString(search) {
  if (isObject(search)) {
 ...
 */
function openTab(newTab, options) {
  const {
    key,
    path = key,
    title,
  ...
  const {
    closeCurrent, // 打开新tab之前关闭当前tab
  } = options || {};
  const activeTabKey = getActiveTabKey();
  if (closeCurrent) {
    closeTab(activeTabKey, key);
  }
  const keyParseData = parsePath(key);
  const oldTab = findTabFromKey(keyParseData.pathname);
  let pathParseData;
  let searchData;
  const defaultWorkbenchTab = getDefaultWorkbenchTab();
  if (newTab.key === defaultWorkbenchTab.path || newTab.key === '/') {
    activeTabAndPush(defaultWorkbenchTab);
  } else if (oldTab === null) {
    pathParseData = parsePath(path);
    searchData =
      !pathParseData.search ? searchFormatString(search) : 
        qs.stringify(Object.assign(searchFormat(search), qs.parse(pathParseData.search)));
    addTabAndPush({
 ...
 * @param {!String} newKey - 指定新 tab 的key
 * @param {!boolean} openNew - 是否打开新tab
 */
function closeTab(key, newKey, openNew = true) {
  if (isString(key)) {
    // getDvaApp()._store
    //   .dispatch({
    ...
    //     });
    //   });
    const oldTabPath = (findTabFromKey(key) || {}).path;
    getDvaApp()
      ._store.dispatch({
      ...
      .then((nextKey) => {
        menuTabEventManager.emit('close', { tabKey: key, tabPath: oldTabPath });
        if (openNew) {
          const changeTabKey = isString(newKey) ? newKey : nextKey;
          /**
           * 修复：先从tab列表中找已存在的tab，找不到再用旧逻辑
           */
          openTab(findTabFromKey(changeTabKey) || { key: changeTabKey });
        }
      });
  }
}
...
function tabListen(pathname = '') {
  const isPubLayout = getIsPubLayout();
  // isPubLayout为true时，所有页面url上都自动拼上isPub=true,便于刷新页面时仍保持pubLayout
  if (
    isPubLayout &&
    (!window.location.search || window.location.search.indexOf('isPub=true') === -1)
  ) {
    if (window.location.search) {
      window.history.replaceState(null, null, window.location.href + '&isPub=true');
    } else {
      window.history.replaceState(
        null,
        null,
        window.location.href + (window.location.href.indexOf('?') === -1 ? '?' : '') + 'isPub=true',
      );
    }
  }
  if (checkThridEmbeddedBrowser()) {
    const currentUser = getCurrentUser();
    // 判断是否已登录，已登录的才拼token
    if (!isEmpty(currentUser)) {
      const access_token = getAccessToken();
      if (access_token && (!window.location.pathname.includes("/public") && !window.location.hash || window.location.hash.indexOf('access_token=') === -1)) {
        // 构建新的 URL，保留 path 和 search，设置 hash
        window.history.replaceState(null, null, `${window.location.pathname}${window.location.search}${window.location.hash ? `${window.location.hash}&` : '#'}access_token=${access_token}`);
      }
    }
  }
  ...
  const { _history = {} } = getDvaApp();
  const { location: { search, state: tabState } = {} } = _history;
  ...
  if (isValidTabPathname(realPathname)) {
    ...
    const state = getDvaApp()._store.getState();
    const { global: { tabs = [], activeTabKey, menuHidden = false } = {} } = state;
    let listenTab;
    forEach(tabs, (tab) => {
    ...
    if (isEmpty(listenTab)) {
      ...
      if (isEmpty(route)) {
        ...
        // 打开工作台(监听到不存在的tab)
        const defaultWorkbenchTab = getDefaultWorkbenchTab();
        replaceTabPath(defaultWorkbenchTab, defaultWorkbenchTab.path, '');
        // 供应商访问页面404时 若路由在配置表中维护了，自动跳到工作台页面
        if (
          getUserOrganizationId() !== getCurrentOrganizationId() &&
          isTenantRoleLevel() &&
          checkModuleLoaded(realPathname)
        ) {
          const unauthorizedRouteList = getUnauthorizedRouteList();
          if (unauthorizedRouteList && unauthorizedRouteList.length > 0) {
            const flag = unauthorizedRouteList.some((r) => realPathname && r.url === realPathname);
            if (flag) {
              // 有路由但无菜单权限,且路由在配置表内维护了，自动跳转到工作台页面
              const defaultWorkbenchTab = getDefaultWorkbenchTab();
              _history.replace(defaultWorkbenchTab.path);
            }
          }
        }
        ...
        // 对于 isPub 页面，需要获取父路由的path作为tab key
        if (isPubLayout && route) {
          if (menuData && menuData.length) {
            const menuByRoute = getMenuInfoByRoute(route, menuData);
            if (menuByRoute && menuByRoute.path) {
              newRouteTab.key = menuByRoute.path;
            }
          } else {
            const parentRoute = getParentRoute(route);
            if (parentRoute) {
              newRouteTab.key = parentRoute.path;
            }
          }
        }
        if (routeTab) {
          replaceTabPath(routeTab, realPathname, realSearch);
        } else if (!menuHidden || (newRouteTab.path && newRouteTab.path.indexOf('/pub') === 0)) {
          // 隐藏菜单时，不创建tab
          // pub/public 路由不拦截
        ...
        let menu = null;
        let hasUpdate = false;
        // 若父路由本身带路径参数，则按照实际路径匹配菜单，否则会匹配到错误的菜单
        if (route.exactRoute || (route.components && route.path && route.path.includes('/:'))) {
          if (route.components) {
            menu = getMenuLeafData().find((menu) => menu.path === realPathname);
          } else {
            const realPathnamePart = realPathname.split('/');
            const realPathnameParent = realPathnamePart.slice(0, realPathnamePart.length -1).join('/');
            menu = getMenuLeafData().find((menu) => menu.path === realPathnameParent);
          }
        } else {
          menu = getMenuInfoByRoute(route, menuData);
        }
        ...
        } else if (checkModuleLoaded(realPathname)) {
          const unauthorizedRouteList = getUnauthorizedRouteList();
          if (unauthorizedRouteList && unauthorizedRouteList.length > 0) {
            const flag = unauthorizedRouteList.some(
              (r) => route && route.path && r.url === route.path,
            );
            if (flag) {
              // 有路由但无菜单权限,且路由在配置表内维护了，自动跳转到工作台页面
              const defaultWorkbenchTab = getDefaultWorkbenchTab();
    ...
    } else if (listenTab.key !== activeTabKey || listenTab.search !== realSearch) {
...
function persistMenuTabs() {
  const { tabs = [], activeTabKey = getDefaultWorkbenchTab().path } =
 ...
 */
function cleanMenuTabs(logout) {
  const roleId = getCurrentRole().id;
  setSession(`${menuTabSessionKey}-${roleId}`, {});
  if (!logout) {
    openTab(getDefaultWorkbenchTab()); // 先打开工作台 再清空数据 注意顺序
  }
  getDvaApp()._store.dispatch({
...
function refreshTab(key, e) {
  ...
  const tab = getTabFromKey(tabKey) || {};
  const refreshKeyMap = refreshMenuTabsKeyMap;
  refreshKeyMap.set(tabKey, uuid());
  if (e) {
    e.stopPropagation();
  }
  menuTabEventManager.emit('refresh', { tabKey, tabPath: tab.path });
  getDvaApp()._store.dispatch({
...
function findMenuPath(menus, pathname, paths = []) {
  const found = menus.find((menu) => {
    ...
    if (children && children.length) {
      paths.push(menu);
      const ret = findMenuPath(children, pathname, paths);
      if (ret.length) {
        return true;
      } else {
        paths.pop();
...
function getDefaultWorkbenchTab() {
  const { global: { workbenchPermission } = {} } = getDvaApp()._store.getState();
  if (workbenchPermission.workbenchFlag) {
    return roleWorkplaceTab;
  }
  return workplaceTab;
}
