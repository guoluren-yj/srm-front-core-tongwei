...
export function createPagination(data, list) {
  if (data) {
    return {
      ...
      current: (isNumber(Number(data.number)) ? Number(data.number) : Number(data.start)) + 1,
      pageSize: Number(data.size), // 每页大小
      total: isNumber(Number(data.totalElements)) ? Number(data.totalElements) : Number(data.total),
