...
import { listenAfterFreeHandler } from './menuTab';
...
export function throttleTimeSingle(func: Function, options: WithPropsOption): ThrottleTimeSingled {
  ...
  function invokeFunc(listenFlag = true) {
    const args = lastArgs;
    ...
    // 监听 tab 刷新
    const { activeTabKey } = window.dvaApp._store.getState().global || {};
    // 根据参数判断是否加监听器
    if (listenFlag && activeTabKey) {
      listenAfterFreeHandler('withProps', 'refresh', ({ tabKey }) => {
        if (tabKey === activeTabKey) {
          // refresh触发的时间在第二次页面加载前。要根据timerId的状态决定是否监听事件
          // 刷新时需要清除状态，重新初始化
          clean();
          refreshFlag = true;
          invokeFunc(timerId === undefined || timerId === null);
        }
      });
      if (cleanWhenClose) {
        listenAfterFreeHandler('withProps', 'close', ({ tabKey }) => {
          if (tabKey === activeTabKey) {
            clean();
          }
        });
  ...
  function throttleTimeSingled(this: any, ...rest) {
    ...
    lastThis = this;
    if (timerId === undefined) {
      // 如果cacheState为false，timerId一定为空
      return invokeFunc();
    } else {
      clearTimeout(timerId);
      ...
      if (lastResult) {
        Object.keys(lastResult).forEach((dataSetName) => {
          ...
          if (cacheDs.queryDataSet) {
            const cacheFormData = cacheDs.queryDataSet.records.length && cacheDs.queryDataSet.records[0].toData();
  ...
  throttleTimeSingled.invokeFunc = invokeFunc;
...
export default function withProps(
  ...
  return (Components) => {
    class WithPropsComponent extends React.Component {
      constructor(props) {
        super(props);
        let dataSets = throttleTimeSingleFun(props);
        // 未知原因会导致dataSets为空，此处为兜底处理
        if (isEmpty(dataSets)) {
          dataSets = throttleTimeSingleFun.invokeFunc();
        }
        this.state = dataSets;
      }
      ...
      static getDerivedStateFromProps(nextProps, prevState) {
        if (prevState && nextProps && prevState.language && nextProps.language !== prevState.language) {
          throttleTimeSingleFun.clean();
