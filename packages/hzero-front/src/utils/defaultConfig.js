...
const {
  $$env = {
    ...
    YQCLOUD_PASS_URL: process.env.YQCLOUD_PASS_URL,
    HELPROBOT_WECHAT_ICON: process.env.HELPROBOT_WECHAT_ICON,
    CHUNK_UPLOAD_ENABLE: process.env.CHUNK_UPLOAD_ENABLE,
    NODE_ENV: process.env.NODE_ENV,
    PUBLIC_URL: process.env.PUBLIC_URL,
    BASE_PATH: process.env.BASE_PATH,
    PACKAGE_PUBLIC_URL: process.env.PACKAGE_PUBLIC_URL,
    APP_NAME: process.env.APP_NAME,
    PACKAGE_NAME: process.env.PACKAGE_NAME,
    AUTH_HOST: process.env.AUTH_HOST,
    YQCLOUD_LOGIN_URL: process.env.YQCLOUD_LOGIN_URL,
    PUBLIC_BUCKET: process.env.PUBLIC_BUCKET,
    SRM_MALL_HOST: process.env.SRM_MALL_HOST,
    PRIVATE_BUCKET: process.env.PRIVATE_BUCKET,
    SKIP_NO_CHANGE_MODULE: process.env.SKIP_NO_CHANGE_MODULE,
    ENV_FLAG_BACKGROUND: process.env.ENV_FLAG_BACKGROUND,
    PROMPT_CHECK: process.env.PROMPT_CHECK,
    YQCLOUD_BOTPRESS_URL: process.env.YQCLOUD_BOTPRESS_URL,
    HZERO_PLATFORM_CUSZ_DATA_MIGRATE_ENABLE: process.env.HZERO_PLATFORM_CUSZ_DATA_MIGRATE_ENABLE,
    ENV_FLAG: process.env.ENV_FLAG,
    HELP_ROBOTR_URL: process.env.HELP_ROBOTR_URL,
    YQCLOUD_API_HOST: process.env.YQCLOUD_API_HOST,
    CUST_HEADER: process.env.CUST_HEADER,
    DATA_RANGERS_APP_ID: process.env.DATA_RANGERS_APP_ID,
    HSCF_HOST: process.env.HSCF_HOST,
    HELP_ROBOTR_URL_ONLINE: process.env.HELP_ROBOTR_URL_ONLINE,
    ENV_NAME: process.env.ENV_NAME,
    YQCLOUD_HOST: process.env.YQCLOUD_HOST,
    SRC_WEBSOCKET_HOST: process.env.SRC_WEBSOCKET_HOST,
    SRM_AMKT_HOST: process.env.SRM_AMKT_HOST,
    PROMPT_COLLECT_FLAG: process.env.PROMPT_COLLECT_FLAG,
    IM_WEBSOCKET_HOST: process.env.IM_WEBSOCKET_HOST,
    REPET_FIELD_WARNING: process.env.REPET_FIELD_WARNING,
    GJSC_HOST: process.env.GJSC_HOST,
    // 兼容旧变量方案
    CLIENT_ID: process.env.CLIENT_ID,
    API_HOST: process.env.API_HOST,
    BPM_HOST: process.env.BPM_HOST,
    WFP_EDITOR: process.env.WFP_EDITOR,
    WEBSOCKET_HOST: process.env.WEBSOCKET_HOST,
    IM_ENABLE: process.env.IM_ENABLE,
    TRACE_LOG_ENABLE: process.env.TRACE_LOG_ENABLE,
    MULTIPLE_SKIN_ENABLE: process.env.MULTIPLE_SKIN_ENABLE,
    TOP_MENU_LABELS: process.env.TOP_MENU_LABELS,
    TOP_MENU_UNION_LABEL: process.env.TOP_MENU_UNION_LABEL,
    ENV_SIGN: process.env.ENV_SIGN,
    INVALID_TIME: process.env.INVALID_TIME,
    CUSTOMIZE_ICON_NAME: process.env.CUSTOMIZE_ICON_NAME,
    MULTIPLE_LANGUAGE_ENABLE: process.env.MULTIPLE_LANGUAGE_ENABLE,
    BAIDU_MAP_AK: process.env.BAIDU_MAP_AK,
    LOGIN_URL: process.env.LOGIN_URL,
    LOGOUT_URL: process.env.LOGOUT_URL,
    PLATFORM_VERSION: process.env.PLATFORM_VERSION,
    BKT_PUBLIC: process.env.BKT_PUBLIC,
    BKT_PLATFORM: process.env.BKT_PLATFORM,
    BKT_MSG: process.env.BKT_MSG,
    BKT_SDR: process.env.BKT_SDR,
    BKT_RPT: process.env.BKT_RPT,
    BKT_INVOICE: process.env.BKT_INVOICE,
    BKT_OCR: process.env.BKT_OCR,
    BKT_ADM: process.env.BKT_ADM,
    BKT_HWFP: process.env.BKT_HWFP,
    BKT_HIMS: process.env.BKT_HIMS,
    BKT_HITF: process.env.BKT_HITF,
    BKT_HIMP: process.env.BKT_HIMP,
    BKT_HFILE: process.env.BKT_HFILE,
    BKT_HIOT: process.env.BKT_HIOT,
    BKT_HORC: process.env.BKT_HORC,
    BKT_HFNT: process.env.BKT_HFNT,
    BKT_HWKF: process.env.BKT_HWKF,
    AUTH_SELF_URL: process.env.AUTH_SELF_URL,
    AUTO_API_HOST: process.env.AUTO_API_HOST,
  },
} = window;
// #region initConfig
const CLIENT_ID = `${$$env.CLIENT_ID}`;
const API_HOST = changeAPI_HOST(`${$$env.API_HOST}`);
let BPM_HOST = `${$$env.BPM_HOST}`;
let WFP_EDITOR = `${$$env.WFP_EDITOR}`;
let WEBSOCKET_URL = `${$$env.WEBSOCKET_HOST}`;
let IM_ENABLE = `${$$env.IM_ENABLE}`;
let TRACE_LOG_ENABLE = `${$$env.TRACE_LOG_ENABLE}`;
let MULTIPLE_SKIN_ENABLE = `${$$env.MULTIPLE_SKIN_ENABLE}`;
let TOP_MENU_LABELS = `${$$env.TOP_MENU_LABELS}`;
let TOP_MENU_UNION_LABEL = `${$$env.TOP_MENU_UNION_LABEL}`;
let ENV_SIGN = `${$$env.ENV_SIGN}`;
let INVALID_TIME = `${$$env.INVALID_TIME}`;
let CUSTOMIZE_ICON_NAME = `${$$env.CUSTOMIZE_ICON_NAME}`;
let MULTIPLE_LANGUAGE_ENABLE = `${$$env.MULTIPLE_LANGUAGE_ENABLE}`;
let BAIDU_MAP_AK = `${$$env.BAIDU_MAP_AK}`;
...
let LOGIN_URL = `${
  $$env.LOGIN_URL || `${AUTH_HOST}/oauth/authorize?response_type=token&client_id=${CLIENT_ID}`
}`;
let LOGOUT_URL = `${$$env.LOGOUT_URL || `${AUTH_HOST}/logout`}`;
let AUTH_SELF_URL = changeAUTH_SELF_URL(API_HOST, HZERO_IAM);
let VERSION_IS_OP = `${$$env.PLATFORM_VERSION}` === 'OP';
let BKT_PUBLIC = `${$$env.BKT_PUBLIC || 'public'}`;
let BKT_PLATFORM = `${$$env.BKT_PLATFORM || 'hpfm'}`;
let BKT_MSG = `${$$env.BKT_MSG || 'hmsg'}`;
let BKT_SDR = `${$$env.BKT_SDR || 'hsdr'}`;
let BKT_RPT = `${$$env.BKT_RPT || 'hrpt'}`;
let BKT_INVOICE = `${$$env.BKT_INVOICE || 'hivc'}`;
let BKT_OCR = `${$$env.BKT_OCR || 'hocr'}`;
let BKT_ADM = `${$$env.BKT_ADM || 'hadm'}`;
let BKT_HWFP = `${$$env.BKT_HWFP || 'hwfp'}`;
let BKT_HIMS = `${$$env.BKT_HIMS || 'hims'}`;
let BKT_HITF = `${$$env.BKT_HITF || 'hitf'}`;
let BKT_HIMP = `${$$env.BKT_HIMP || 'himp'}`;
let BKT_HFILE = `${$$env.BKT_HFILE || 'hfle'}`;
let BKT_HIOT = `${$$env.BKT_HIOT || 'hiot'}`;
let BKT_HORC = `${$$env.BKT_HORC || 'horc'}`;
let BKT_HFNT = `${$$env.BKT_HFNT || 'hfnt'}`;
let BKT_HWKF = `${$$env.BKT_HWKF || 'hwkf'}`;
let AUTO_API_HOST = $$env.AUTO_API_HOST;
// #endregion

function changeAPI_HOST(API_HOST) {
  const { protocol, host, hostname } = window.location;
  const ipv4Reg = /^((25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(25[0-5]|2[0-4]\d|[01]?\d\d?)$/;
  const ipv6Reg = /^([\da-fA-F]{1,4}:){7}[\da-fA-F]{1,4}$/;
  if (
    !$$env.AUTO_API_HOST ||
    $$env.AUTO_API_HOST == 'false' ||
    hostname.includes('going-link.com') ||
    hostname == 'localhost' ||
    ipv4Reg.test(hostname) ||
    ipv6Reg.test(hostname)
  ) {
    return API_HOST;
  }
  return `${protocol}//gateway-${host}`;
}
...
function changeAUTH_SELF_URL(API_HOST, HZERO_IAM) {
  return `${$$env.AUTH_SELF_URL || `${API_HOST}${HZERO_IAM}/hzero/v1/users/self`}`;
}
...
const {
  ...
  ENV_FLAG_BACKGROUND,
  PROMPT_CHECK,
  YQCLOUD_BOTPRESS_URL,
  HZERO_PLATFORM_CUSZ_DATA_MIGRATE_ENABLE,
  ENV_FLAG,
  HELP_ROBOTR_URL,
  YQCLOUD_API_HOST,
  CUST_HEADER,
