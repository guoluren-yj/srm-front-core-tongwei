...
import { Tag as C7NTag  } from 'choerodon-ui';
...
export function enableTagRender(v) {
  return (
    <C7NTag
      color={v === 1 ? 'green' : 'red'}
      border={false}
...
export function yesOrNoRenderWithNull(v) {
...
export function dateRender(dateStr) {
  ...
  return dateStr && moment(dateStr, DEFAULT_DATE_FORMAT).locale('en').format(dateTemplate);
}
...
export function dateTimeRender(dateTimeStr) {
  ...
  return dateTimeStr && moment(dateTimeStr, DEFAULT_DATETIME_FORMAT).locale('en').format(dateTimeTemplate);
}
 ...
 */
export function TagRender(status, statusList = [], text = '', fontColor) {
  ...
  const currentStatus =
    ...
    {};
  return (
    <Tag style={{ color: fontColor }} color={currentStatus.color || ''}>
