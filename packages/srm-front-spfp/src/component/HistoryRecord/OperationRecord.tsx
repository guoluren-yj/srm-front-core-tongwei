...
const getDefaultFieldsConfig = (): FieldsConfig<OutputProps> => {
  return {
    ...
    typeName: {
      ...
      renderer: ({ text }) => {
        return text;
        // return intl.get('spfp.common.view.message.alreadyOperated', { operationName: text }).d('{operationName}了');
      },
    },
    remark: {
      ...
      return remark && (
        <div className={styles[`${itemPreCls}-extra`]}>
          ...
          <span className={styles[`${itemPreCls}-action`]}>
            {intl.get('spfp.common.view.message.alreadyOperated', {
              operationName: intl.get('spfp.common.view.message.add').d('添加'),
            }).d('{operationName}了')}
          </span>
