...
const ApprovalRecord = (props: ApprovalRecordProps) => {
  ...
  const handleInit = useCallback(async () => {
    try {
      const [nodeData, approvalDataMap] = (await Promise.all([
        queryUnifyIdpValue('SSTA.PROCESS_DEFINITION'),
        // 如果父组件已查询过数据源无需重复查询
  ...
  // 获取折扣审批记录数据
  const getApprovalRecords = useCallback(async () => {
    const result = dataSource || getResponse(await fetchApprovalData({ primaryId: documentId, documentType }));
    if (result) {
      // 按需对接口返回的审批记录数据进行处理
      const approveRecordDataResult = result.map((item) => {
        return {
          value: 'RULE_PUBLISH_APPROVAL',
          meaning: intl.get('spfp.ruleMaintenance.view.records.discountApproval').d('折扣规则审批记录'),
          historicTaskExtList: item?.historicTaskExtList || [],
        };
      });
      setApprovalData(approveRecordDataResult);
    }
    setLoading(false);
  }, [documentId, documentType, dataSource]);
  ...
  }, [handleInit, getApprovalRecords, documentType]);
