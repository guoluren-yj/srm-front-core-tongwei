...
const DetailContent = observer(() => {
  ...
  const paneList = useMemo(() => [
    {
      key: 'sceneInfo',
      header: intl.get('spfp.ruleMaintenance.detail.card.title.discountScenario').d('折扣场景'),
      content: <SceneInfo />,
    },
    !triggerIsHideAll && {
    ...
    !applyIsHideAll && {
      ...
      content: <MaintainView dataSet={applyRangeDs} dimensionType={DimensionType.apply} setBubblePrompt={setBubblePrompt} />,
    } as any,
    {
      key: 'calcRules',
  ...
  ].filter(Boolean), [applyIsHideAll, triggerIsHideAll, applyRangeDs, bubblePrompt]);


  return (
    <Fragment>
      <div className={modal ? styles['spfp-rule-maintain-content-view-modal'] : null}>
        <Spin spinning={modal ? false : loading} wrapperClassName="full-height-spinning">
          <Collapse
            ghost
            trigger="icon"
