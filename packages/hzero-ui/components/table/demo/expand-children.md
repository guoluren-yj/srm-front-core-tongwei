...
class Demo extends React.Component {
  ...
  exTreeKey = (list, result)=> {
      if (list) {
        for (let i = 0; i < list.length; i++) {
          result.push(list[i].key);
          if (list[i].children) {
            this.exTreeKey(list[i].children, result);
          }
        }
      }
    }
  ...
  onClick = ()=> {
    this.setState({
      data,
      expandedRowKeys:data.map(d=>d.key)
    })
  }

  onExpandAll = () => {
    const all = [];
    this.exTreeKey(data, all);
    this.setState({
      expandedRowKeys:all
    })
  }
  ...
  render(){
    ...
    return(
      <div>
        ...
        <button onClick={this.onExpandAll}>expandALl</button>
        <button onClick={this.onRefresh}>onRefresh</button>
        <Table columns={columns} expandedRowKeys={expandedRowKeys} uncontrolled={true}  dataSource={data} />
      </div>
    )
  }
} 
