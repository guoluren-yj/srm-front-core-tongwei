...
| showUploadList | 是否展示 uploadList, 可设为一个对象，用于单独设定 showPreviewIcon 和 showRemoveIcon | Boolean or { showPreviewIcon?: boolean, showRemoveIcon?: boolean, showDownloadIcon?: boolean, removePopConfirmTitle: string } | true |
...
|removePopConfirmTitle|删除按钮的popConfirm消息|string &#124; ReactNode | 无 |
|showReUploadIcon| 是否显示重新上传按钮| Boolean &#124; (file: UploadFile) => boolean | false |
|reUploadText| 是否显示重新上传按钮的title| string | 无 |
|reUploadPopConfirmTitle| 是否显示重新上传按钮的确认消息| (file: UploadFile) => string | 无 |
