...
import {
  ...
  regionImport,
  regionUpdate,
} from '../services/regionalMappingService';
...
export default {
  ...
  effects: {
    ...
    *regionUpdate({ payload }, { call }) {
      const res = yield call(regionUpdate, payload);
      return getResponse(res);
    },
    // 引入云级数据
    *regionImport({ payload }, { call }) {
      const res = yield call(regionImport, payload);
      return getResponse(res);
    },
    // 获取映射状态
    *searchEsStatus(_, { call }) {
      const res = yield call(queryIdpValue, 'SCEC.REGION_ASSOCIATION_STATE');
      return getResponse(res);
    },
  },
