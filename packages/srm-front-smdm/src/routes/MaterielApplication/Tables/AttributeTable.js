...
export default class AttributeTable extends PureComponent {
  ...
  onSelectChange(selectedRowKeys, selectedRows) {
    ...
    selectedRows.forEach((item) => {
      if (!item.isLocal) {
        idList.push(item.itemAttributeReqId);
      }
    });
  ...
  saveFormData() {
    ...
    form.validateFields((err, fieldsValues) => {
      if (!err) {
        ...
        const newFieldsValues = recordSource.itemAttributeReqId
          ? {
              ...recordSource,
              ...fieldsValues,
              attributeValue:
        ...
        if (newFieldsValues.isCreat) {
          ...
          const { itemAttributeReqId } = newFieldsValues;
          newDataSource = dataSource.map((item) => {
            if (item.itemAttributeReqId === itemAttributeReqId) {
  ...
  handleDelete() {
    ...
    const newSelectedRows = selectedRows.map((item) => {
      return item.itemAttributeReqId;
    });
    const newDataSource = dataSource.filter((item) => {
      return newSelectedRows.indexOf(item.itemAttributeReqId) > -1 === false;
    });
    this.setState({ selectedRows: [] });
  ...
  renderForm() {
    ...
    return customizeForm(
      {
        code: 'SMDM_MATERIELAPPLICATION_ATTRIBUTETABL.EDITFORM',
        form,
  ...
  renderLineBtns() {
    ...
    const normalBtns = [
      ...
      <Button data-name="create" type="primary" onClick={() => this.onOpen()}>
        {intl.get(`smdm.materiel.view.message.toolTip.attribute.create`).d('新建属性')}
      </Button>,
    ];
    return customizeBtnGroup({ code: 'SMDM_MATERIELAPPLICATION_EDIT.ATTRIBUTE_BTNS' }, normalBtns);
  }
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        {customizeTable(
          {
            code: 'SMDM_MATERIELAPPLICATION_ATTRIBUTETABL.TABLE',
          },
          <Table
            rowKey="itemAttributeReqId"
        ...
        {drawerVisible && (
          <Drawer
            title={
              recordSource.itemAttributeReqId
