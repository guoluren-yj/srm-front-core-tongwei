...
export default class ComponentTable extends PureComponent {
  ...
  onSelectChange(selectedRowKeys, selectedRows) {
    ...
    selectedRows.forEach((item) => {
      if (!item.isLocal) {
        idList.push(item.componentReqId);
      }
    });
  ...
  saveFormData() {
    ...
    form.validateFields((err, fieldsValues) => {
      if (!err) {
        const newFieldsValues = recordSource.componentReqId
          ? {
              ...recordSource,
              ...fieldsValues,
            }
  ...
  handleDelete() {
    ...
    const newSelectedRows = selectedRows.map((item) => {
      return item.componentReqId;
    });
    const newDataSource = content.filter((item) => {
      return newSelectedRows.indexOf(item.componentReqId) > -1 === false;
    });
    this.setState({ selectedRows: [] });
  ...
  @Bind()
  getQueryParams() {
    const { remote, form, affliatedData } = this.props;
    return remote
      ? remote.process(
          'SMDM_ITEM_APPLICATION_DETAIL.COMPTABLE.QUERYPARAMS',
          { tenantId },
          {
            form,
            affliatedData,
  ...
  renderForm() {
    ...
    return customizeForm(
      {
        code: 'SMDM_MATERIELAPPLICATION_COMPONENTTABLE.EDITFORM',
        form,
  ...
  render() {
    ...
    const { dataSource, editAble, customizeTable, remote } = this.props;
    const { content = [] } = dataSource;
    const { disableCompentPrecision } = remote?.props?.process || {};
    const columns = [
      {
    ...
    return (
      <React.Fragment>
        ...
        {customizeTable(
          {
            code: 'SMDM_MATERIELAPPLICATION_COMPONENTTABLE.TABLE',
          },
          <Table
            rowKey="componentReqId"
            dataSource={content}
            columns={columns}
            bordered
