...
export default class PartnerTable extends PureComponent {
  ...
  onSelectChange(selectedRowKeys, selectedRows) {
    ...
    selectedRows.forEach((item) => {
      if (!item.isLocal) {
        idList.push(item.partnerRelationReqId);
      }
    });
  ...
  saveFormData() {
    ...
    form.validateFields((err, fieldsValues) => {
      if (!err) {
        const newFieldsValues = recordSource.partnerRelationReqId
          ? {
              ...recordSource,
              ...fieldsValues,
            }
  ...
  handleDelete() {
    ...
    const newSelectedRows = selectedRows.map((item) => {
      return item.partnerRelationReqId;
    });
    const newDataSource = content.filter((item) => {
      return newSelectedRows.indexOf(item.partnerRelationReqId) > -1 === false;
    });
    this.setState({ selectedRows: [] });
