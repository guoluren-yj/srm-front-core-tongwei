...
export default class UomType extends PureComponent {
  ...
  handleAdd(fields) {
    ...
    const item = {
      ...uom,
      ...otherFields,
      baseUomName: baseUomName1 || baseUomName,
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <Header title={intl.get(`smdm.uomType.view.message.title`).d('计量单位类型定义')}>
        ...
        <UomForm
          ...
          title={
            uom.uomTypeId
              ? intl.get(`smdm.uomType.view.message.title.edit`).d('编辑单位类型')
              : intl.get(`smdm.uomType.view.message.title.create`).d('新建单位类型')
          }
          onRef={(node) => {
            this.uomForm = node;
          }}
