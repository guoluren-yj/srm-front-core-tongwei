...
const Index = function Index({
  ...
  const renderWholeTable = () => {
    ...
    return customizeTable(
      {
        code: 'SMDM_ITEM_FEEDBACK.LIST',
      },
      <SearchBarTable
        style={{ maxHeight: 'calc(100% - 22px)' }}
        searchCode="SMDM_ITEM_FEEDBACK.SEARCH"
        dataSet={wholeListDs}
        columns={columns}
        data={[]}
        key="wholeFeedback"
        ...
        searchBarConfig={{
          right: {
            render: () => (
              <ViewFilter updateType={updateType} type={type} sourceType="feedBackType" />
            ),
          },
          fieldProps: {
  ...
  const renderLineTable = () => {
    ...
    return customizeTable(
      {
        code: 'SMDM_ITEM_FEEDBACK.LINE_LIST',
      },
      <SearchBarTable
        style={{ maxHeight: 'calc(100% - 22px)' }}
        searchCode="SMDM_ITEM_FEEDBACK.LINE_SEARCH"
        dataSet={lineListDs}
        columns={lineColumns}
        data={[]}
        key="lineFeedback"
