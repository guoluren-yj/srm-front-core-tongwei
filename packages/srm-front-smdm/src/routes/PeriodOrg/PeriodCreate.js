 ...
 * @reactProps {Function} addPeriod - 添加期间操作
 * @reactProps {Function} cleanLine - 清除新增行操作
 * @reactProps {Function} changeEditFlag - 变更行编辑状态
 * @reactProps {Function} save - 保存操作
 * @reactProps {Function} onCancel - modal关闭
 * @reactProps {Array} dataSource - 表格数据源
 * @reactProps {Object} periodItem - 维护期间的会计期间对象
 * @return React.element
 */
export default class PeriodCreate extends PureComponent {
  /**
   * 保存
   */
  @Bind()
  ...
  render() {
    ...
    const columns = [
                ...
                rules: [
                  {
                    ...
                    message: intl.get('hzero.common.validation.notNull', {
                      name: intl.get('smdm.period.model.period.periodYear').d('年'),
                    }),
                  },
                ],
              ...
              {record.$form.getFieldDecorator(`periodName`, {
                ...
                rules: [
                  ...
                  },
                  {
                    required: true,
                    message: intl.get('hzero.common.validation.notNull', {
                      name: intl.get('smdm.period.model.period.periodName').d('期间'),
                    }),
                  },
                ],
      ...
      {
        title: intl.get('smdm.period.view.message.range').d('期间范围'),
        dataIndex: 'dateRange',
        align: 'center',
        width: 250,
        render: (val, record) =>
                ...
                rules: [
                  {
                    required: true,
                    message: intl.get('hzero.common.validation.notNull', {
                      name: intl.get('smdm.period.view.message.range').d('期间范围'),
                    }),
                  },
                ],
                ...
                rules: [
                  {
                    required: true,
                    message: intl.get('hzero.common.validation.notNull', {
                      name: intl.get('smdm.period.model.period.periodQuarter').d('季度'),
                    }),
                  },
                ],
    ...
    return (
      ...
      >
        <Header title={intl.get('smdm.period.view.message.maintain').d('期间维护')} />
        <Content>
