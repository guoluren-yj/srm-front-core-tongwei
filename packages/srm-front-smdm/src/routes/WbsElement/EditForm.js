  ...
  render() {
    ...
    return (
      ...
      >
        {customizeForm(
          {
            code: 'SMDM.WBSELE.CREATE_FORM',
            form, // 无论个性化单元是否只读，均必传
            dataSource: initData, // 必传，从后端接口获取到的数据
          },
          <Form>
            <Row>
              <Col span={24}>
                ...
                >
                  {getFieldDecorator('wbsCode', {
                    initialValue: wbsCode,
                    rules: [
                      {
                        required: true,
                        message: intl.get('hzero.common.validation.notNull', {
                          name: intl.get(`smdm.common.model.wbs.code`).d('WBS编码'),
                        }),
                      },
                      {
                        max: 24,
            ...
            <Row>
              <Col span={24}>
                <FormItem
                  {...formLayout}
                  label={intl.get('smdm.common.model.wbs.name').d('WBS元素描述')}
                >
                  {getFieldDecorator('wbsName', {
                    initialValue: wbsName,
                    rules: [
                      {
                        required: true,
                        message: intl.get('hzero.common.validation.notNull', {
                          name: intl.get(`smdm.common.model.wbs.name`).d('WBS元素描述'),
                        }),
                      },
                      {
                        max: 120,
