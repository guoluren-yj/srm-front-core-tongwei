...
const Index = function Index({
      ...
      },
      {
        ...
        renderer: ({ value, record }) => (
          <div className="row-agent-column">
            <a
              onClick={() => handleJumpDetail(record.toData(), 'testResultEntry')}
  ...
  const renderWholeTable = () => {
    ...
    return customizeTable(
      {
        code: 'SMDM.ITEM_TEST_RESULT_ENTRY.LIST',
      },
      <SearchBarTable
        style={{ maxHeight: 'calc(100% - 22px)' }}
        searchCode="SMDM.ITEM_TEST_RESULT_ENTRY.SEARCH"
        dataSet={wholeListDs}
        columns={wholeColumns}
        data={[]}
        key="testResultEntryWhole"
        searchBarConfig={{
          editorProps: {
            authReqStatusCode: {
              optionsFilter: (options) =>
                ['TEST_RESULTS_TO_BE_ENTERED'].includes(options.get('value')),
            },
          },
          fieldProps: {
          ...
          right: {
            render: () => (
              <ViewFilter updateType={updateType} type={type} sourceType="testResultEntryType" />
            ),
          },
          left: {
            render: () => (
  ...
  const renderLineTable = () => {
    ...
    return customizeTable(
      {
        code: 'SMDM.ITEM_TEST_RESULT_ENTRY.LINE_LIST',
      },
      <SearchBarTable
        style={{ maxHeight: 'calc(100% - 22px)' }}
        searchCode="SMDM.ITEM_TEST_RESULT_ENTRY.LINE_SEARCH"
        dataSet={linelListDs}
        columns={lineColumns}
        key="testResultEntryLine"
        data={[]}
        searchBarConfig={{
          editorProps: {
            authReqStatusCode: {
