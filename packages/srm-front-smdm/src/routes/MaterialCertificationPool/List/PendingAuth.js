...
import { revokeWorkFlow, getBatchOperationFlag } from '../util.js';
...
const Index = function Index({
  ...
  const lineColumns = useMemo(() => {
    ...
    const newColumns = remote
      ? remote.process('SMDM_MATERIALCERTIFICATIONPOOL_LIST_PENDINGAUTH_COLUMN', cols, {
        viewDetail,
        linelListDs,
      ...
      : cols;
    return newColumns;
  }, [handleJumpDetail, linelListDs, remote]);
  ...
  const renderWholeTable = () => {
    ...
    return customizeTable(
      {
        code: 'SMDM.ITEM_PENDING_AUTH.LIST',
      },
      <SearchBarTable
        style={{ maxHeight: 'calc(100% - 22px)' }}
        searchCode="SMDM.ITEM_PENDING_AUTH.SEARCH"
        dataSet={wholeListDs}
        ...
        searchBarConfig={{
          ...
          right: {
            render: () => (
              <ViewFilter updateType={updateType} type={type} sourceType="pendingType" />
            ),
          },
          left: {
            render: () => (
  ...
  const renderLineTable = () => {
    ...
    return customizeTable(
      {
        code: 'SMDM.ITEM_PENDING_AUTH.LINE_LIST',
      },
      <SearchBarTable
        style={{ maxHeight: 'calc(100% - 22px)' }}
        searchCode="SMDM.ITEM_PENDING_AUTH.LINE_SEARCH"
        dataSet={linelListDs}
        columns={lineColumns}
        key="pendingLine"
        data={[]}
