...
export const notifyValidErrors: (dataSet: DataSet) => void = (dataSet) => {
  ...
  const errorTextList = allErrors.map(dsError => {
    ...
    const errorTextPrefix = intl.get(`smdm.payTermsCtrl.view.message.inValidationTitle`, { validationTitle }).d(`【{validationTitle}】中，`);
    const errorTextContent = errorInfoList.map(errorInfo => {
      const { num, labelList } = errorInfo;
      const numPrefix = validationCode === 'header' ? '' : intl.get(`smdm.payTermsCtrl.view.message.rowNum`, { num }).d(`第{num}行，`);
  ...
  notification.error({
    message: intl.get('smdm.payTermsCtrl.view.validation.fieldEmptyOrBadFormat').d('操作失败，失败原因是存在必填字段未维护，或格式维护不正确，请将鼠标悬浮于问题字段，查看维护提示，问题字段如下：'),
