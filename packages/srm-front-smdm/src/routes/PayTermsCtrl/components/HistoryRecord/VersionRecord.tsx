...
// 版本号 | 发布人 | 发布时间 | 登录名
export type FieldName = 'versionNumber' | 'userName' | 'time' | 'loginName';
...
const getDefaultFieldsConfig = (): FieldsConfig<OutputProps> => {
  return {
    versionNumber: {
      ...
      renderer: ({ text }) => intl.get('smdm.payTermsCtrl.view.message.versionVNumber', { versionNumber: text }).d('版本v{versionNumber}'),
    },
    userName: {
...
const VersionRecord = observer((props: VersionRecordProps) => {
  ...
  const getContent = useCallback((record: DSRecord) => {
    const defaultRender = (record: DSRecord) => {
      return (
    ...
    <div className={styles['version-wrapper']}>
      {isEmpty(dataSet) ? (
        <div className={styles[`empty-version-wrapper`]}>
          {intl.get('smdm.payTermsCtrl.view.message.noHistoricalVersionInfo').d('暂无历史版本信息')}
        </div>
      ) : (
        dataSet.map((record) => (
