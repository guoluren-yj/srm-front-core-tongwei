...
const PayDateValidRule = () => {
  ...
  const { viewFlag, payDateValidRuleDs, customizeTable } = useContext<StoreValueType>(Store);
  ...
  const columns: ColumnProps[] = useMemo(() => {
    return [
      ...
      },
      {
        ...
        width: 130,
        editor: !viewFlag,
        help: intl.get('smdm.payTermsCtrl.view.help.payDateValidRuleLevel').d('警告：在预付款、付款页面报错形式为中心弹窗提示，用户可选是否继续提交单据；禁止，在预付款、付款页面报错形式为右下角报错弹窗，禁止继续提交单据'),
      },
      {
        name: 'validContext',
        width: 180,
        editor: !viewFlag,
        help: intl.get('smdm.payTermsCtrl.view.help.payDateValidRuleContext').d('对于付款申请，结算策略启用「是否预计期望付款日期」后，系统将在发票确认、付款提交等多个时点更新基于发票申请创建付款申请、基于结算事务创建付款申请页面的「预计期望付款日期」字段，若对应付款计划阶段配置启用「校验内容=早于预计付款日期（不包含等于）」，则当系统当前日期早于（不含等于）预计期望付款日期时报错'),
      },
      {
        name: 'notCalcPreExpPayDateProcessMode',
        width: 220,
        editor: !viewFlag,
        help: intl.get('smdm.payTermsCtrl.view.help.payDateValidRuleNotCalcPreExpPayDateProcessMode').d('预计付款日期可能由于基准日期为空，或系统程序错误导致无法计算，针对该场景可选择报错提示禁止操作，或直接跳过阶段付款日期校验规则允许用户正常创建提交单据'),
      },
      {
        name: 'validPosition',
        width: 130,
        editor: !viewFlag,
        help: intl.get('smdm.payTermsCtrl.view.help.payDateValidRulePosition').d('创建对应引用订单/协议/协议阶段创建预付款申请时点、引用发票/结算事务创建付款申请时点，包含行新增，提交对应预付款申请、付款申请提交'),
      },
      {
        name: 'stageLineNums',
        width: 150,
