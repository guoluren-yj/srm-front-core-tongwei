...
export default class ItemOrgUomTable extends PureComponent {
  ...
  saveFormData() {
    ...
    //    const { dispatch, organizationId, itemId } = this.props;
    ...
    form.validateFields((err, fieldsValues) => {
      if (!err) {
        ...
        // }
        //  else {
        const newFieldsValues = recordSource.itemOrgUomId
          ? {
            ...recordSource,
            ...fieldsValues,
          }
        ...
        if (newFieldsValues.isCreat) {
          ...
          const { itemOrgUomId } = newFieldsValues;
          newDataSource = content.map((item) => {
            if (item.itemOrgUomId === itemOrgUomId) {
        ...
        if (isEdit) {
          ...
          //   itemOrgUomId: recordSource.itemOrgUomId ? recordSource.itemOrgUomId : null,
          //   primaryUomId: fieldsValues.primaryUomId,
          //   optionalAmount: fieldsValues.optionalAmount,
          //   primaryAmount: fieldsValues.primaryAmount,
          // };
          // dispatch({
          //   type: 'materiel/uomValid',
          //   payload: { organizationId, itemId, value: valuelist },
          // }).then(res => {
          ...
          const flag = dataList.every((item) => {
            if (item.itemOrgUomId === newFieldsValues.itemOrgUomId) {
              return true;
            }
  ...
  renderForm() {
    const { form = {}, primaryUomId, primaryUomName, doubleControlFlag } = this.props;
            ...
            >
              {getFieldDecorator('primaryAmount', {
                ...
                <InputNumber
                  min={0}
                  precision={0}
                  disabled={doubleControlFlag && !recordSource?.isLocal && recordSource?.primaryAmount}
                />
              )}
            </FormItem>
          </Col>
            ...
            >
              {getFieldDecorator('optionalAmount', {
                ...
                <InputNumber
                  min={0}
                  precision={0}
                  disabled={doubleControlFlag && !recordSource?.isLocal && recordSource?.optionalAmount}
                />
              )}
            </FormItem>
          </Col>
            ...
            >
              {getFieldDecorator('uomId', {
                ...
                <Lov
                  ...
                  disabled={doubleControlFlag && !recordSource?.isLocal && recordSource?.uomId}
                />
              )}
            </FormItem>
          </Col>
  ...
  render() {
    ...
    const { dataSource, isEdit = false, customizeTable, customizeForm, form } = this.props;
    const { content = [] } = dataSource;
    const columns = [
      // {
    ...
    return (
      <React.Fragment>
        <div className="table-list-search" style={{ textAlign: 'right' }}>
          ...
          >
            {intl.get(`smdm.materiel.view.message.toolTip.ItemOrgUom.create`).d('新建单位转换关系')}
          </Button>
        </div>
        {
          customizeTable ? customizeTable(
          {
            code: 'SMDM_MATERIEL_EDIT.UOM_LIST',
          },
            <Table
              bordered
        ...
        >
          {
            customizeForm ? customizeForm(
              {
                code: 'SMDM_MATERIEL_EDIT.UOM_FORM',
                form,
