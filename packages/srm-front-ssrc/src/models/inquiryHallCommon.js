...
import { getResponseParse, getDownloadFileUrl } from '@/utils/utils';
import { queryFileListOrg, queryMapIdpValue, queryUUID, removeFileOrg } from 'services/api';

function dealDataState(data = [], otherData = {}) {
  // 处理行 处理字段为update
  let config = [];
...
const getModel = (modelName = 'inquiryHall') => ({
  namespace: modelName,
  state: {
    cacheMap: {}, // 缓存
    list: [], // 询价大厅数据列表
    ...
    sectionSubmit: {},
    checkPriceHeader: {}, // 询价单明细_核价节点_头数据源
  },
  effects: {
    // 物料行批量维护
    ...
    *fetchRFScoreHeader({ payload }, { call, put }) {
      const result = getResponse(yield call(fetchRFScoreHeader, payload));
      if (result) {
        yield put({
          type: 'updateState',
          payload: {
    ...
    *fetchLadderLevelTable({ payload }, { call, put }) {
      ...
      result = getResponse(result);
      if (result) {
        const { suggestedFlag } = payload || {};
        yield put({
          type: 'updateState',
          payload: {
            quotaLadderLevelData: dealDataState(result.content, { suggestedFlag }),
          },
        });
        return result;
      }
    ...
    *fetchAloneSupplierItemLine({ payload }, { call, put }) {
      const { supplierCompanyId, rfxLineSupplierId } = payload;
    ...
    *fetchApplyToInquiryPage({ payload }, { call, put }) {
      let result = yield call(fetchApplyToInquiry, { ...payload, onlyCountFlag: 'Y' });
      result = getResponse(result);
      if (result) {
        yield put({
          type: 'updateState',
    ...
    *checkApplyToInquiry({ payload }, { call }) {
    ...
    // 申请转询价创建前校验API - 新
    *newBatchValidatePurchase({ payload }, { call }) {
      const result = getResponse(yield call(newBatchValidatePurchase, payload));
      return result;
    },
    // 风险监控
    ...
    *fetchAllRoundQuotationList({ payload }, { call, put }) {
      const { returnCompleteFlag = 0 } = payload || {};
    ...
    *fetchItemRoundQuotationDetail({ payload }, { call, put }) {
      ...
      if (result) {
        ...
        if (returnCompleteFlag === 1) {
          // 返回完整数据结构
          completeData = {
            roundQuotationItemList: result.content,
            roundQuotationItemListPagination: createPagination(result),
          };
          return completeData;
        }
      }
    ...
    *fetchSupplierRoundQuotationDetail({ payload }, { call, put }) {
      ...
      if (result) {
        ...
        if (returnCompleteFlag === 1) {
          // 返回完整数据结构
          completeData = {
            roundQuotationSupplierList: result.content,
            roundQuotationSupplierListPagination: createPagination(result),
          };
          return completeData;
        }
      }
    ...
    *exportCheckPriceData({ payload }, { call }) {
      // fix: 对于 responseType 为 `text`, getResponse 无法直接判断, 仍需处理
      const url = getDownloadFileUrl(yield call(exportCheckPriceData, payload));
      return url;
    },
    ...
    *fetchQueryPriceInfo({ payload }, { call, put, select }) {
      ...
      if (result) {
        const { quoteLine = [] } = yield select((state) => state.inquiryHall);
        yield put({
          type: 'updateState',
          payload: {
    ...
    *getClearLogic({ payload }, { call }) {
    ...
    // 退回至汇总
    *returnToSummary({ payload }, { call }) {
      const result = getResponse(yield call(returnToSummary, payload));
      return result;
    },
  },
  reducers: {
    ...
    updateSupplierItemList(state, { payload }) {
      ...
      const { rfxLineSupplierId, supplierCompanyId, list, pagination } = payload;
      return {
        ...state,
        aloneSupplierItemLine: {
          ...aloneSupplierItemLine,
          [`${rfxLineSupplierId || supplierCompanyId}`]: {
            list,
            pagination,
          },
        },
    ...
    updateItemQuoteLineData(state, { payload }) {
      let FilterItemLineLine = state.itemQuoteLine;
      const itemLineList = state.itemQuoteLine.map((item) => item.rfxLineItemId);
      payload.itemQuoteLine.forEach((item) => {
        if (itemLineList.some((list) => list === item.rfxLineItemId)) {
          FilterItemLineLine = state.itemQuoteLine.filter((items) => {
            return items.rfxLineItemId !== item.rfxLineItemId;
          });
        }
      });
      return {
        ...state,
        itemQuoteLine: [...FilterItemLineLine, ...payload.itemQuoteLine],
    ...
    updateSupplierRoundQuotaion(state, { payload }) {
      ...
      const quotationSupplierList = state.quotationSupplierList.map((itemLine) => {
        // if (itemLine.quotationHeaderId === supplierRoundQuotationDetail[0].quotationHeaderId) {
    ...
    updateEliItemData(state, { payload }) {
      const newKeys = payload.eliItemLine.map((item) => item.quotationLineId);
      const oldData = (state.eliItemLine || [])
        .map((item) => {
          if (!newKeys.includes(item.quotationLineId)) {
            return item;
          }
          return null;
        })
      ...
      return {
        ...state,
        eliItemLine: [...(oldData || []), ...(payload.eliItemLine || [])],
        eliItemLinePagination: {
