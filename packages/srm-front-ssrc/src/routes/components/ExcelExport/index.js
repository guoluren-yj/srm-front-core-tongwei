/**
 * 这个组件仅适用于execl导出默认不能勾选全导出情况
 */

import React from 'react';
import { Dropdown, Form, Icon, Menu, Modal, notification, Tree, Popover } from 'hzero-ui';
...
export default class ExcelExport extends React.PureComponent {
  ...
  handleExport(config = {}) {
    ...
    const { exportList } = this.state;
    const checkedKeys = this.traversalTreeNodesID(exportList);
    ...
    if (!checkedKeys || (Array.isArray(checkedKeys) && checkedKeys.length === 0)) {
      ...
      if (newQueryParams.async === 'true') {
        ...
        //   }
        // });
        downloadFileByAxios({ requestUrl, queryParams: params, method })
          .catch((err) => {
  ...
  /**
   * @function traversalTreeNodesID - 遍历树的Id
   * @param {object} data - 列数据
   */
  @Bind()
  traversalTreeNodesID(data = [], arr = []) {
    const ids = arr;
    data.map((item) => {
      const temp = item;
      ids.push(temp.id);
      if (temp.children) {
        this.traversalTreeNodesID(temp.children, ids);
      }
      return temp;
    });
    return ids;
      ...
      exportAsync && isEmpty(defaultConfig) ? (
        ...
        <Popover content={content} placement={placement}>
          {uiType === 'h0' ? (
            ...
            <C7NButton {...buttonProps} onClick={this.showSyncExportModal} loading={exportPending}>
              {buttonText}
            </C7NButton>
          )}
        </Popover>
