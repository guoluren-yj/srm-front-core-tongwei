...
export default class PrecisionInputNumber extends React.Component {
  ...
  @Bind()
  formatterValue(value, precision) {
    const { omitZeroFlag, readOnly } = this.props;
    // return isValidateValue(value) && Number(value).toFixed(precision);
    if (!isValidateValue(value) || math.isNaN(value)) return readOnly ? '-' : '';
    return numberSeparatorRender(value, precision, { omitZeroFlag });
  }
   ...
   */
  query(context) {
    const { uom, currency, financial, queryPrecisionParams } = this.props;
    ...
    if (
      ...
      let queryParams = queryPrecisionParams;
      if (isFunction(queryPrecisionParams)) {
        queryParams = queryPrecisionParams();
      }
      context.precisionCtx.query(
        { code, type, queryPrecisionParams: queryParams },
    ...
    this.setState(
      ...
      () => {
        ...
        if ((isUndefined(type) || type === 'c7n-pro') && isObject(record)) {
          ...
          record.setState(precisionVariableName, precision);
          /**
           * 此处代码注释掉，主要是因为存在一个问题，例如：给单价填入16.1061949611，一个10位的数据，
           * 再去选一个精度为4位的币种，页面上显示的是16.1062，但是传给后端的值仍然是16.1061949611。
           */
          // // 修复当前字段必输 原始无值 但精度组件触发set 触发必输校验
          // // 原始无值 无需record.set 触发必填校验
          // const pristineValue = record.getPristineValue(name);
          // if (!pristineValue) return;

          // 修复当前字段必输但无值 但精度组件触发set 会触发必输校验，无值则无需record.set 触发必填校验
  ...
  handleFormatter(value, precision, focusValue = null) {
    ...
    return this.formatterValue(value, precision);
  }
  ...
  renderComponent() {
    const { onRef, type, value: inputValue, currency, readOnly, ...otherProps } = this.props;
    const { precision } = this.state;
    ...
    const expandProps = Object.assign(
      ...
      (isUndefined(type) || type === 'c7n-pro') && {
        renderer: ({ value }) => this.formatterValue(value, precision),
    ...
    if (readOnly) {
      return <span> {this.formatterValue(inputValue, precision)} </span>;
    } else {
      switch (type) {
        case 'hzero':
        ...
        case 'c7n-pro':
          return (
            <C7nProInputNumber
              ref={(vnode) => isFunction(onRef) && onRef(vnode)}
              {...expandProps}
            />
          );
        default:
