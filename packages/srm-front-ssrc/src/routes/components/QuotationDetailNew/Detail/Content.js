...
function Content(props = {}) {
  ...
  const init = () => {
    ...
    const params = remote
      ? remote.process('SSRC_QUOTATION_DETAIL_VIEW_MODAL_PROCESS_QUERY_PARAMS', queryParams, {
          props,
        })
      : queryParams;
            ...
            moduleList.forEach((i) => {
              tableDsRef.current = {
                ...
                [i.templateId]: new DataSet(
                  remote
                    ? remote.process(
                        'SSRC_QUOTATION_DETAIL_VIEW_PROCESS_TABLE_DS',
                        tableDS({
                          rowKeyId,
                          queryParams: params,
                          handleDataSource,
                        ...
                        }),
                        {
                          ...
                          rowData,
                          uiType,
                          moduleRule: headerRef.current?.moduleRule,
                          module: i,
                        }
                      )
                    : tableDS({
  ...
  const getCustomizeUnitCode = useCallback(
    (type = null) => {
      ...
      const ProjectCodeMap = new Map([
        ['baseInfoForm', 'SSRC.PROJECT_SETUP_QUOTATION_DETAIL.BASE_FORM_DETAIL'], // 基础信息
      ]);

      const RfxCodeMap = new Map([
        ['baseInfoForm', 'SSRC.INQUIRY_HALL_QUOTATION_DETAIL.BASE_FORM_DETAIL'], // 基础信息
      ]);

      const BidCodeMap = new Map([
        ['baseInfoForm', 'SSRC.BID_HALL_QUATION_DETAIL.BASE_FORM_DETAIL'], // 基础信息
      ]);
  ...
  const renderFieldType = (field = {}, options = {}) => {
    ...
    const fieldConfigObject = remote
      ? remote.process('SSRC_QUOTATION_DETAIL_VIEW_PROCESS_FIELD_TYPE_CONFIG', fieldConfig, {
          field,
          pageProps: props,
          module,
        })
  ...
  const getColumns = useCallback(
    (templateId, item) => {
      ...
      return remote
        ? remote.process('SSRC_QUOTATION_DETAIL_VIEW_PROCESS_COLUMNS', preColumns, {
            pageFrom,
            rowData,
            uiType,
            moduleRule: headerRef.current?.moduleRule,
    ...
    [dynamicColumns, headerRef.current, uiType, pageFrom, rowData]
  );
  ...
  const handleExport = () => {
    exportQuotationDetail({
      ...
      sourceFrom,
      quotationHeaderId,
      recordId: quotationHistoryFlag ? recordId : null,
      ...(exportParas || {}),
    }).then((response) => {
      try {
...
export default withCustomize({
  unitCode: [
    'SSRC.INQUIRY_HALL_QUOTATION_DETAIL.BASE_FORM_DETAIL',
    'SSRC.BID_HALL_QUATION_DETAIL.BASE_FORM_DETAIL',
    'SSRC.PROJECT_SETUP_QUOTATION_DETAIL.BASE_FORM_DETAIL',
  ],
})(observer(Content));
