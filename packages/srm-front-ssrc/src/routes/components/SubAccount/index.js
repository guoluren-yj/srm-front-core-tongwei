...
@withCustomize({
  unitCode: [
    'SSRC.INQUIRY_HALL.NEW_LIST.COMPONENT_SUB_ACCOUNT',
    'SSRC.INQUIRY_HALL.NEW_LIST.SUB_ACCOUNT_FILTER',
    'SSRC.BID_HALL.NEW_LIST.COMPONENT_SUB_ACCOUNT',
    'SSRC.BID_HALL.NEW_LIST.SUB_ACCOUNT_FILTER',
  ],
})
...
export default class SubAccountModal extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      selectRows: {}, // 勾选行
  ...
  @Bind()
  handleSearch(page = {}) {
    const { dispatch, form, organizationId, remote, remoteCode, bidFlag } = this.props;
    ...
    let params = {
      ...
      customizeUnitCode: !bidFlag
        ? 'SSRC.INQUIRY_HALL.NEW_LIST.COMPONENT_SUB_ACCOUNT,SSRC.INQUIRY_HALL.NEW_LIST.SUB_ACCOUNT_FILTER'
        : 'SSRC.BID_HALL.NEW_LIST.COMPONENT_SUB_ACCOUNT,SSRC.BID_HALL.NEW_LIST.SUB_ACCOUNT_FILTER',
    };

    params =
      remote && remoteCode
        ? remote.process(`${remoteCode}_PROCESS_HANDLE_SEARCH_PARAMS`, params, {
            that: this,
          })
        : params;
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: intl.get(`ssrc.common.model.common.phone`).d('电话'),
        dataIndex: 'phone',
        width: 120,
      },
      {
        title: intl.get(`ssrc.common.model.common.email`).d('邮箱'),
        dataIndex: 'email',
        width: 220,
      },
    ];
