...
export default class UploadExcel extends React.Component {
  ...
  beforeUpload(file) {
    ...
    if (quotationHeaderCurrentId || sourceHeaderId) {
      ...
      const headerId = quotationHeaderCurrentId || sourceHeaderId;
      const url = projectLineSectionId
        ? `${API_HOST}${SRM_SSRC}/v2/${organizationId}/rfx/sup-dtl/import-upload?templateCode=${templateCode}&&${headerName}=${headerId}&&sourceFrom=${sourceFrom}&&projectLineSectionId=${projectLineSectionId}&&operationType=${operationType}`
        : `${API_HOST}${SRM_SSRC}/v2/${organizationId}/rfx/sup-dtl/import-upload?templateCode=${templateCode}&&${headerName}=${headerId}&&sourceFrom=${sourceFrom}&&operationType=${operationType}`;
      this.setState({
        uploadLoading: true,
      });
      request(url, {
