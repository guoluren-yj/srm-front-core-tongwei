      ...
      <div>
        <div className={`${stylePrefix}-field-selector-header`}>
          ...
          <TextField
            ...
            placeholder={intl.get('ssrc.filterBar.view.title.filterCondition').d('筛选过滤条件')}
          />
        </div>
        <div className={`${stylePrefix}-field-selector-summary`}>
          <span>
            {intl
              .get('ssrc.filterBar.view.message.selectedField', { size: noLockFields.length })
          ...
          >
            {intl.get('ssrc.filterBar.view.title.allSelect').d('全选')}
          </span>
          {noLockFields.length > 0 && (
            ...
            >
              {intl.get('ssrc.filterBar.view.title.clearSelect').d('清除已选')}
            </span>
          )}
        </div>
