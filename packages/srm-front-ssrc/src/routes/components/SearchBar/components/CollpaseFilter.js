...
const CollpaseFilter = ({ expand, handleExpand }) => {
  return (
    <div onClick={handleExpand} className={`${stylePrefix}-collpase-filter`}>
      ...
      <span className={`${stylePrefix}-filter-text`}>
        {expand
          ? intl.get('ssrc.filterBar.view.message.collapseFilter').d('收起筛选')
          : intl.get('ssrc.filterBar.view.message.expandFilter').d('展开筛选')}
      </span>
      <span>
