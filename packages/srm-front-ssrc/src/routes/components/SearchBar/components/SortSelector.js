...
import { DataSet, Dropdown, Tooltip } from 'choerodon-ui/pro';
import { Icon, Menu } from 'choerodon-ui';
import { Action } from 'choerodon-ui/pro/lib/trigger/enum';
import { Bind } from 'lodash-decorators';

import intl from 'utils/intl';
import { stylePrefix, SortFieldName } from '../util';
      ...
      >
        {fields.map((item) => (
          <Menu.Item key={item.name} className={`${stylePrefix}-sort-menu-item`}>
            {intl.get('ssrc.filterBar.view.label.orderBy').d('按')}
            {item.label}
          </Menu.Item>
        ))}
      </Menu>
      ...
      <span className={`${stylePrefix}-sort`}>
        <Dropdown overlay={overlayMenu} trigger={[Action.click]}>
          <span className={`${stylePrefix}-sort-control`}>
            {sortField.label
              ? `${intl.get('ssrc.filterBar.view.label.orderBy').d('按')}${sortField.label}`
              : ''}
        ...
        <Tooltip
          title={
            sortFlag === 'asc'
              ? intl.get('ssrc.filterBar.view.tooltip.asc').d('升序')
              : intl.get('ssrc.filterBar.view.tooltip.desc').d('降序')
          }
        >
          <img
            alt="orderBy"
