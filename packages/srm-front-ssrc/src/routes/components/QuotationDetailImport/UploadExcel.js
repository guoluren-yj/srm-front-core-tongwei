  ...
  beforeUpload(file) {
    ...
    if (quotationHeaderId || sourceHeaderId) {
      ...
      const url = projectLineSectionId
        ? `${API_HOST}${SRM_SSRC}/v1/${organizationId}/share/sup-dtl/import-upload?templateCode=${templateCode}&&${headerName}=${headerId}&&sourceFrom=${sourceFrom}&&projectLineSectionId=${projectLineSectionId}&&operationType=${operationType}`
        : `${API_HOST}${SRM_SSRC}/v1/${organizationId}/share/sup-dtl/import-upload?templateCode=${templateCode}&&${headerName}=${headerId}&&sourceFrom=${sourceFrom}&&operationType=${operationType}`;
      this.setState({
        uploadLoading: true,
      });
      request(url, {
