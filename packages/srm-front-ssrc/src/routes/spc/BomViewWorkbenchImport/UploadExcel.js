...
export default class UploadExcel extends React.Component {
  ...
  @Bind()
  beforeUpload(file) {
    const { bomTemplateCode, param = {}, success = (e) => e, tenantId } = this.props;
    const formData = new FormData();
    formData.append('excel', file, file.name);
    if (bomTemplateCode) {
      const url = `${API_HOST}${SRM_SPC}/v1/${tenantId}/price-bom-workbenches/data-upload?bomTemplateCode=${bomTemplateCode}&&tenantId=${tenantId}`;
      this.setState({
        uploadLoading: true,
      });
      request(url, {
