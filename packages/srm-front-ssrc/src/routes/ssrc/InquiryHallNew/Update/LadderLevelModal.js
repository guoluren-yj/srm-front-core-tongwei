...
class LadderLevelModal extends React.Component {
  ...
  // 改变数量
  handleLadderQuantity = debounce((val, record = {}, type = 'ladderFrom', idx) => {
    const { remote } = this.props || {};
    if (remote?.event) {
      remote.event.fireEvent('remoteHandleLadderQuantity', { val, record, type, idx, that: this });
    }
  }, 350);

  // 改变基本数量
  handleLadder = debounce((val, record = {}, type = 'secondaryLadderFrom', idx) => {
    const { doubleUnitFlag, LadderLevelHeaderData = {}, remote } = this.props || {};
    const { itemId = '', secondaryUomId = '', uomId = '' } = LadderLevelHeaderData;
    const {
      $form: { setFieldsValue, getFieldValue },
    } = record;
    const handleLadderOrigin = () => {
      if (val) {
    ...
    if (remote?.event) {
      remote.event.fireEvent('remoteHandleLadder', {
        val,
        record,
        type,
        idx,
  ...
  feedLadderLevelyTable() {
                ...
                <Form.Item>
                  {record.$form.getFieldDecorator('secondaryLadderFrom', {
                    ...
                    <PrecisionInputNumber
                      ...
                      style={{ width: '100%' }}
                      onChange={(value) =>
                        this.handleLadder(value, record, 'secondaryLadderFrom', idx)
                      }
                    />
                  )}
                </Form.Item>
            ...
            <Form.Item>
              {record.$form.getFieldDecorator('ladderFrom', {
                ...
                <PrecisionInputNumber
                  ...
                  max="99999999999999999999"
                  style={{ width: '100%' }}
                  onChange={(value) => this.handleLadderQuantity(value, record, 'ladderFrom', idx)}
                />
              )}
            </Form.Item>
          ) : (
    ...
    return customizeTable(
      {
        code: `SSRC.${sourceKey}_HALL.NEW_EDIT.ITEMLINE_LADDER_LEVEL`,
      },
      <EditTable
        bordered
...
const hocLadderLevel = (Com, { currentPageSymbol = 'INQUIRY_HALL' } = {}) => {
  return withCustomize({
    unitCode: [`SSRC.${currentPageSymbol}.NEW_EDIT.ITEMLINE_LADDER_LEVEL`],
  })(Com);
};

const LadderLevelModalNew = hocLadderLevel(LadderLevelModal);
export default LadderLevelModalNew;
