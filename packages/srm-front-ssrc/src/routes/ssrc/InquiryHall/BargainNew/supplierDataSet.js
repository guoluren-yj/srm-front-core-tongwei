...
import { Prefix, getQuotationName, BID } from '@/utils/globalVariable';
import {
  getUomName,
  getQtyName,
  getAvailableQtyName,
...
// 单据头基本信息
const supplierListDataSet = (options) => {
  const { organizationId, sourceKey } = options || {};
  ...
  return {
    ...
    transport: {
      read: ({ data, params }) => {
        ...
        const { rfxHeaderId, quotationHeaderId } = commonProps || {};

        if (sourceKey === BID) {
          return {
            url: `/marmot/v1/${organizationId}/marmot-api/21VFib8pTuCtIB51x4HLW7Po6Att7icSbyRxuUbOepIFHXw3I0rYqYTektxbWHLN6F`,
            params: {
              ...params,
              ...others,
              rfxHeaderId,
        ...
        return {
          url: `${Prefix}/${organizationId}/rfx/${rfxHeaderId}/bargain/suppliers`,
          method: 'GET',
          data: {
            ...params,
            ...others,
...
// supplier table
const supplierTableDataSet = (options) => {
  const {
    doubleUnitFlag,
    // sourceKey,
    bidFlag,
  ...
  return {
    ...
    pageSize: 10,
    selection: bargainFlag ? 'multiple' : false,
    autoQueryAfterSubmit: false,
    dataToJSON: 'all',
    cacheModified: true,
    fields: [
      ...
      {
        label: intl.get(`ssrc.inquiryHall.model.inquiryHall.lineStatus`).d('行状态'),
...
export { supplierListDataSet, supplierTableDataSet };
