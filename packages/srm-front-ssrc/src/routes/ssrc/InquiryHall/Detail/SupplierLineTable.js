...
export default class SupplierLineTable extends Component {
  render() {
    ...
    const newDataSource = supplierQuoteLine.filter(
      (supplier) => supplier.rfxLineSupplierId === rfxLineSupplierId
    );
    ...
    const preTableProps = {
      ...
      columns: remote
        ? remote.process('SSRC_DETAIL_CHECK_PRICE_TABS_SUPPLIER_COLUMNS', columns, {
            doubleUnitFlag,
            header,
            rfx,
          })
      ...
      onChange: (page, filters, sorter) => onChange(page, rfxLineSupplierId, filters, sorter),
    };

    const tableProps = remote
      ? remote.process('SSRC_DETAIL_CHECK_PRICE_TABS_SUPPLIER_TABLE_PROPS', preTableProps, {
          header,
          rfx,
        })
      : preTableProps;
    ...
    return customizeTable(
      {
        code: `SSRC.${unitCodeSymbol}_DETAIL.SUPPLIER_DETAIL`, // 单元编码，必传
        readOnly: true,
      },
      <Table {...tableProps} />
    );
