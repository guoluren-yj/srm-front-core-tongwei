...
import { Attachment, Button } from 'choerodon-ui/pro';
...
const FullQuotation = (props = {}) => {
    ...
    () =>
      [
        ...
        },
        {
          ...
          renderer: ({ record }) => (
            <React.Fragment>
              {<QuotationDetail rowData={record.toData()} sourceFrom="RFX" allowBuyerViewFlag bidFlag={bidFlag} />}
            </React.Fragment>
          ),
        },
        {
  ...
  const getButttons = () => {
    const btns = [
      <Button onClick={batchEditorTable} icon="auto_complete" funcType="flat">
...
export default observer(FullQuotation);
