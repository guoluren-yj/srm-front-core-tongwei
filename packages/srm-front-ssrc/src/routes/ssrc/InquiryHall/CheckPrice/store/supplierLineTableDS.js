...
const supplierLineTableDS = ({
  sourceKey,
  rfxLineSupplierId: panelLineSupplierId,
  doubleUnitFlag,
}) => ({
  ...
  fields: [
    {
      ...
      dynamicProps: {
        disabled: ({ record, dataSet }) => {
          const onlyAllowAllWinBids = dataSet.getState('onlyAllowAllWinBids');
          const {
            invalidFlag,
            summaryReviewResult,
    ...
    read: ({ dataSet: { queryParameter } }) => {
      ...
      return {
        ...
        transformResponse: (data) => {
          ...
          const content =
            normalData?.content?.map((item) => ({
              ...item,
              rfxLineSupplierId: panelLineSupplierId, // 解决主键加密不同步bug
            })) || [];
          return {
            ...normalData,
            content,
  ...
  events: {
    load({ dataSet }) {
      dataSet.forEach((record) => {
        const onlyAllowAllWinBids = dataSet.getState('onlyAllowAllWinBids');
        const { priceBatchQuantity, batchPrice, eliminateRoundNumber, supplierStatus } = record.get(
          [
            'priceBatchQuantity',
            'batchPrice',
            'eliminateRoundNumber',
        ...
        const selectable =
          !onlyAllowAllWinBids && !eliminateRoundNumber && supplierStatus !== 'PROHIBIT_QUOTATION';
