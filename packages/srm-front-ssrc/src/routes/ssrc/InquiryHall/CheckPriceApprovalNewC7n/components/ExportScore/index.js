...
// import { Tabs, Tooltip } from 'hzero-ui';
...
const ExpertScore = () => {
  ...
  const { currentUserIsOnlyTechnologyExpertFlag, preEvaluationFlag, } = headerCurrent
  ...
  const [bidSectionList, setBidSectionList] = useState({});
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      expertScoreDS.loadData(newTable || []);
      setBidSectionList(data);
      setSupplierList(newTable);
    } catch (e) {
      throw e;
    }
  };
  ...
  const generateDataSource = () => {
    ...
    const supplierListCurrentData = toJS(current?.get('supplierList'));
    if (!supplierListCurrentData) {
      return {};
    }
    // 打平分数细项 到供应商行上
    const NewSupplierList = supplierListCurrentData.map((item = {}) => {
  ...
  const addDataStateForBidSectionList = (data = {}) => {
    const mapData = data.evaluateSummaryMap || {};

    const { supplierList: supplierListCurrentData = [] } = generateDataSource() || {};
    let newTableData = null;

    Object.keys(mapData).forEach((item) => {
      const values = mapData[item] || [];
      ...
      const subConfig = values.map((child) => {
        // 每一个供应商 映射取值外层 `supplierList`
        let childMappingSupplier;
        if (child.supplierCompanyId) {
          childMappingSupplier = supplierListCurrentData.find(
    ...
    return {
      ...
      supplierList: supplierListCurrentData,
    };
  };
  ...
  const generateDynamicColumns = (evaluateExpertsColumnsData = []) => {
    ...
    const RelativeColumns = evaluateExpertsColumnsData.map((item) => {
      ...
      return {
        ...
        align: 'right',
        renderer: ({ record }) => {
          const scoreExpertStatusName = `${team}${evaluateExpertId}Score`;
          const scoreExpertValueName = `${team}${evaluateExpertId}`;
          const { [scoreExpertStatusName]: scoreExpertStatus, [scoreExpertValueName]: scoreExpertValue } = record.get([scoreExpertStatusName, scoreExpertValueName]);

          const onlyShowValue =
            !['ALL_PASS', 'PART_PASS', 'UN_PASS'].includes(scoreExpertStatus) && isNaN(Number(scoreExpertValue));

          const render = onlyShowValue ? scoreExpertValue : renderScoringDetail({
            scoringDetailProps,
            record,
            scoreExpertValue,
            scoreExpertStatus,
  ...
  const renderScoringDetail = (data) => {
    const {
      scoringDetailProps = {},
      record,
      scoreExpertValue,
    ...
    return (
      <ScoringDetail
        {...scoringDetailProps}
        recordData={record}
        text={scoreExpertValue}
        redFlag={scoreExpertStatus === 'UN_PASS'}
        useCustomize={false}
      />
    );
  };
  ...
  const getTableColumns = () => {
    let columns = [
      ...
      {
        title: supplierList[0]?.sumPassStatus
        ...
        renderer: ({ value, record }) => {
          ...
          return <a onClick={() => openScoreDetailModal(record)}>{sumPassStatus || value}</a>;
        },
      },
