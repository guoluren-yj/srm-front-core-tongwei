...
const quoteLineDS = ({ rfxHeaderId, sourceKey = INQUIRY }) => {
  return {
    ...
    fields: [
      ...
      },
      {
        ...
        dynamicProps: {
          defaultValue: ({ record }) =>
            record.get('priceBatchQuantity') || record.get('priceBatchQuantity') === 0
              ? record.get('priceBatchQuantity')
              : record.get('batchPrice') === null
              ? 1
              : record.get('batchPrice'),
      ...
      },
      {
        ...
        dynamicProps: {
          required: ({ record, dataSet }) =>
            dataSet.getState('checkWay') === 'quantity' && record.get('suggestedFlag'),
          disabled: ({ record }) => !record.get('suggestedFlag'),
        },
      },
      {
    ...
    events: {
      load: ({ dataSet }) => {
        dataSet.forEach((record) => {
          ...
          // 如果选择策略是最低价且选用，选用理由赋值为最低价
          if (dataSet.getState('selectStrategy') === 'MINIMUM' && record.get('suggestedFlag')) {
            record.set(
              'suggestedRemark',
              intl.get('ssrc.inquiryHall.model.inquiryHall.minPrice').d('最低价')
            );
          }
          // 如果选择策略是最高价且选用，选用理由赋值为最高价
          if (dataSet.getState('selectStrategy') === 'MAXIMUM' && record.get('suggestedFlag')) {
            record.set(
              'suggestedRemark',
      ...
      update: ({ record, name, value }) => {
        if (name === 'suggestedFlag') {
          if (value) {
            record.set('allottedQuantity', record.get('rfxQuantity'));
            ...
            checkPriceDTOLineList: [
              {
                quotationLineList: data.map((item) => {
                  ...
                  return { ...item, allottedQuantity: null };
                }),
                type: 'DETAIL',
              },
            ],
