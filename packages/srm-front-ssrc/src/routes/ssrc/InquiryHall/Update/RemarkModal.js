...
import { Modal, Form, Select, Col, Row, InputNumber, Checkbox, Popover } from 'hzero-ui';
import withCustomize from 'srm-front-cuz/lib/h0Customize';
import { Text } from 'choerodon-ui';

import { SEARCH_FORM_CLASSNAME } from 'utils/constants';
import { Bind } from 'lodash-decorators';
import { isNumber, compose } from 'lodash';
...
class RemarkModal extends PureComponent {
  ...
  handleSaveRemark() {
    ...
    form.validateFieldsAndScroll({ force: true }, (err, values) => {
      const { benchmarkPriceFactor, benchmarkPriceMethod } = values || {};
      let resetFields = {};
  ...
  renderFormulaField(form, formula) {
    const { remote, sourceKey = INQUIRY } = this.props;
    const that = this;
    const selection = (
    ...
    const values = remote
      ? remote.process('SSRC_INQUIRYHALLNEW_UPDATE_PROCESS_FORM_FIELD_FORMULA', selection, {
          that,
          form,
          formula,
          sourceKey,
  ...
  cuxFormulaField() {
    ...
    const { evaluateIndicDetail = {} } = record;
    return remote
      ? remote.process('SSRC_INQUIRYHALLNEW_UPDATE_PROCESS_REMARK_SCORE_FORMULA', null, {
          form,
          evaluateIndicDetail,
          that: this,
        })
  ...
  renderContent() {
    ...
    return customizeForm(
      {
        code: `SSRC.${sourceKey}_HALL.NEW_EDIT.SCORING_RUBRIC`,
            ...
            <Col span={16}>
              <Form.Item
                label={
                  remote
                    ? remote.process(
                        'SSRC_INQUIRYHALLNEW_UPDATE_PROCESS_FORM_FIELD_LABEL_BENCHMARK_PRICE_FACTOR',
              ...
              >
                {getFieldDecorator('benchmarkPriceFactor', {
                  ...
                  remote
                    ? remote.process(
                        'SSRC_INQUIRYHALLNEW_UPDATE_PROCESS_FORM_FIELD_BENCHMARK_PRICE_FACTOR',
                ...
                {getFieldDecorator('aboveFactor', {
                  ...
                  <InputNumber
                    ...
                    min={
                      remote
                        ? remote.process(
                            'SSRC_INQUIRYHALLNEW_UPDATE_RENDER_INPUT_NUMBER_ABOVE_FACTOR',
                            0,
                            {
                              benchmarkPriceMethod: form.getFieldValue('benchmarkPriceMethod'),
                            }
            ...
            <Col span={6}>
              <Form.Item
                label={
                  remote
                    ? remote.render(
                        'SSRC_INQUIRYHALLNEW_UPDATE_RENDER_SCORE_REMARK_BELOW_FACTOR_LABEL',
                        intl
                ...
                {getFieldDecorator('belowFactor', {
                  ...
                  <InputNumber
                    ...
                    min={
                      remote
                        ? remote.process(
                            'SSRC_INQUIRYHALLNEW_UPDATE_RENDER_INPUT_NUMBER',
          ...
          <Row style={{ marginTop: '15px' }}>
            <Col span={24} style={{ width: '100%' }}>
              <Form.Item label="" colon={false} labelCol={{ span: 12 }} wrapperCol={{ span: 12 }}>
                {getFieldDecorator('linearMappingNotice', {
                  initialValue: `${intl
...
const HOCComponent = (Comp, type = INQUIRY) => {
  return compose(
    withCustomize({
      unitCode: [
        `SSRC.${type}_HALL.NEW_EDIT.SCORING_RUBRIC`, // 寻源过程控制(新)-询价要求-评分要素-评分细则
      ],
    }),
...
const BidRemarkModal = HOCComponent(RemarkModal, BID);

export default HOCComponent(RemarkModal, INQUIRY);

export { HOCComponent, RemarkModal, BidRemarkModal };
