...
export default class Drawer extends React.Component {
  ...
  renderComputeChild() {
    ...
    const addButtons = [
      ...
      ['delete', { icon: 'delete_sweep', children: intl.get(`hzero.common.button.batchdelete`).d('批量删除') }],
    ];
    return (
      <React.Fragment>
        <Form
        ...
        >
          <ConstructForm formType="TextField" isEdit={enabledEdit} disabled={editor} name="dimensionCode" />,
          <ConstructForm formType="IntlField" isEdit={enabledEdit} name="dimensionName" />,
          <ConstructForm formType="Select" isEdit={enabledEdit} disabled={editor} name="dimensionCategory" />,
          <ConstructForm formType="Select" isEdit={enabledEdit} optionsFilter={this.triggerTypeOptionsFilter} name="triggerType" />,
          <ConstructForm formType="TextField" isEdit={enabledEdit} name="computeFunction" />,
          ...
          <ConstructForm formType="IntlField" isEdit={enabledEdit} type="multipleLine" name="computeLogic" colSpan={1.5} newLine resize="vertical" rows={6} />,
        </Form>
  ...
  render() {
    ...
    return (
      <Sidebar
        ...
        visible={visible}
        onOk={enabledEdit ? onOk : onCancel}
        okCancel={enabledEdit}
        okText={!enabledEdit ? intl.get('hzero.common.button.close').d('关闭') : intl.get('hzero.common.button.ok').d('确定')}
        onCancel={onCancel}
        confirmLoading={saveLoading}
        maskStyle={{ zIndex: 997 }}
        wrapClassName={classNames(style['c7n-modal-price-warp'], !enabledEdit && style['view-only-form'])}
      >
