...
export default class ConditionConfig extends PureComponent {
  ...
  async handleDeleteCondition(record) {
    ...
    if (record.data.ruleLineId) {
      const res = getResponse(await deleteConditionOrg([record.toData()]));
      if (res && !res.failed) {
        notification.success();
        fetchConditionData(conditionDs, conditionParams);
      }
  ...
  renderAppointValue(record) {
    if (
      ...
      return <Select record={record} />;
    } else {
      return <TextField record={record} />;
    }
  }

  get columns() {
    const { enabledEdit } = this.props;
    ...
    return [
      ...
      },
      {
        ...
        editor: (record) =>
          enabledEdit &&
          !['IS_NULL', 'NOT_NULL'].includes(record.get('ruleExpression')) && (
            <Select
              clearButton={false}
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <this.CardTitle
          title={intl.get('ssrc.priceLibDimension.view.message.analyzingConditions').d('判断条件')}
        />
        <Table
          customizable
          buttons={
            enabledEdit && [
              [
                ...
                {
                  children: intl
                    .get('ssrc.priceLibDimension.view.message.addCondition')
                    .d('添加条件'),
          ...
          customizedCode="SSRC.PRICE_LIB_DIMENSION.CONDITION_CONFIG"
          dataSet={conditionDs}
          columns={this.columns}
          style={{ maxHeight: 200 }}
        />
        <this.CardTitle
          title={intl.get('ssrc.priceLibDimension.view.message.filterLogic').d('筛选逻辑')}
        />
        <Form
          columns={2}
          dataSet={filterDs}
        ...
        >
          <ConstructForm name="combExpression" formType="TextField" isEdit={enabledEdit} />
        </Form>
