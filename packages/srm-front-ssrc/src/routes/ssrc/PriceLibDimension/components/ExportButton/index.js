...
import { Button, Modal, Dropdown, Menu, Icon } from 'choerodon-ui/pro';
...
import { exportPrice } from '@/services/priceLibDimensionService';
...
const ExportButton = () => {
  ...
  const handleExport = async (templateIds) => {
    closeModal();
    setLoading(true);
    const res = await exportPrice(templateIds);
    if (res && isString(res)) {
  ...
  return (
    <Dropdown
      ...
      trigger={['click', 'hover']}
    >
      <Button funcType="flat" icon="unarchive" loading={loading} style={{ height: '32px' }}>
