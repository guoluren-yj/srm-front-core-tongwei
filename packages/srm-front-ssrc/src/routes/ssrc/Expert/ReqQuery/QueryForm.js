...
@CacheComponent({ cacheKey: '/ssrc/reqQuery' })
export default class QueryForm extends PureComponent {
   ...
   */
  render() {
    const { form, expertTypeList, expertCategoryList, documentsState } = this.props;
          ...
          <Col span={18}>
            <Row>
              <Col span={8}>
                <FormItem
                  {...formItemProps}
                  label={intl.get(`${promptCode}.expertReqNum`).d('申请单号')}
                >
            ...
            <Row style={{ display: expandForm ? 'block' : 'none' }}>
              ...
              <Col span={8}>
                <FormItem
                  {...formItemProps}
                  label={intl.get(`${promptCode}.expertReqStatus`).d('单据状态')}
                >
                  {getFieldDecorator('expertReqStatus')(
                    <Select allowClear style={{ width: '100%' }}>
                      {documentsState.map((item) => (
                        <Option value={item.value} key={item.value}>
                          {item.meaning}
                        </Option>
                      ))}
              ...
              <Col span={8}>
                <Form.Item
                  label={intl.get(`${promptCode}.itemCategoryName`).d('品类')}
                  {...formItemProps}
                >
                  {getFieldDecorator('itemCategories')(
                    <LovMulti code="SSRC.CATEGORY_MIN_LEVEL_PTH" />
                  )}
                </Form.Item>
              </Col>
            </Row>
