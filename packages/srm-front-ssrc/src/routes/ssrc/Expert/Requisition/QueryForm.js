...
@CacheComponent({ cacheKey: '/ssrc/requisition' })
export default class QueryForm extends PureComponent {
   ...
   */
  render() {
    const { form, expertTypeList, expertCategoryList, expertReqList } = this.props;
          ...
          <Col span={18}>
            <Row>
              ...
              <Col span={8}>
                <FormItem
                  {...formItemProps}
                  label={intl.get(`${promptCode}.model.expert.expertName`).d('专家姓名')}
                >
            ...
            <Row style={{ display: expandForm ? 'block' : 'none' }}>
              ...
              <Col span={8}>
                <FormItem
                  {...formItemProps}
                  label={intl.get(`${promptCode}.model.expert.expertReqStatus`).d('单据状态')}
                >
                  {getFieldDecorator('expertReqStatus')(
                    <Select allowClear style={{ width: '100%' }}>
                      {expertReqList.map(item => (
                        <Option value={item.value} key={item.value}>
                          {item.meaning}
                        </Option>
                      ))}
