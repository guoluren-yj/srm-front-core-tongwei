...
import { basicFormDS, scopeTableDS, ladderQuotationDS, relevantQueryFormDS } from './lineDS';
import { relevantPriceDS } from '../lineDS';
...
class Detail extends React.Component {
  ...
  async componentDidMount() {
    ...
    // 请求配置头
    await this.fetchPriceLibHeaderConfig();
          ...
          if (
            ...
            columnList.push({
              ...
              render: ({ rowData }) => {
                ...
                return remote
                  ? remote.render('SSRC_PRICELIB_WORK_FLOW_RENDER_COLUMNS', content, {
                      item,
                      rowData,
                    })
                  : content;
  ...
  @Bind()
  renderExportButton() {
    const { templateCode, requestId } = this;
    const ResponsiveComponent = ({ ...other }) => {
      return (
        <ExportDynamicExcel
    ...
    return <DynamicButtons buttons={buttons} trigger="hover" />;
  }
  ...
  render() {
    ...
    const { customizeForm, customizeVTable = noop, remote } = this.props;
    ...
    const { tableData = [], pagination = {}, queryLoading = false } = this.state;
    const tableHeight = tableData.length > 10 ? 374 : tableData.length * 38 + 46;
    return (
      <React.Fragment>
        {remote
          ? remote.render('SSRC_PRICELIB_WORK_FLOW_RENDER_HEADER', this.renderHeader(), {
              templateCode: this.templateCode,
              tableData,
              basicFormData,
              requestId: this.requestId,
              current: this,
              fetchPriceLibData: this.fetchPriceLibData,
            })
          : this.renderHeader()}
        <Content>
          {customizeForm(
            {
              code: 'SSRC.PRICE_LIB_NEW.REQ_APPRO',
              dataSet: this.basicFormDs,
            },
            <Form dataSet={this.basicFormDs} columns={3}>
              ...
              <Upload
                ...
                bucketDirectory="price-library-inform-workflow"
                tenantId={organizationId}
                viewOnly
              />
            </Form>
...
const HOCWorkFlow = (Comp) =>
  compose(
    WithCustomizeC7N({
      unitCode: [
        'SSRC.PRICE_LIB_NEW.REQ_APPRO', // 工作流发布申请页面
        'SSRC.PRICE_LIBRARY_NEW.REQ_MAIN_LIST', // 审批结果列表
        'SSRC.PRICE_LIBRARY_NEW.LADDER_QUOTATION_LIST', // 阶梯报价
      ],
    }),
    formatterCollections({ code: ['ssrc.priceLibraryNew', 'ssrc.common'] }),
    hocRemote({
      code: 'SSRC_PRICELIB_WORK_FLOW', // 对应二开模块暴露的Expose的编码， 命名规范：模块编码+功能编码
      name: 'remote', // 默认 'remote'， 如有属性冲突可以改此属性
    })
  )(Comp);

export default HOCWorkFlow(Detail);

export { HOCWorkFlow, Detail };
