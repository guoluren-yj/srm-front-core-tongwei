  ...
  return {
    ...
    transport: {
      read: ({ dataSet, params, data }) => {
        ...
        const { organizationId, userId, companyId, adjustRecordId } = commonProps || {};
        if (!companyId || !userId) {
          return;
        }

        const allData = data;
        const { itemCategoryIds, queryItemIds, ...restSearchParams } = allData;
        const itemCategoryIdString = JSON.stringify(itemCategoryIds);
        const queryItemIdString = JSON.stringify(queryItemIds);

        delete allData.commonProps;

        return {
          url: `${Prefix}/${organizationId}/rfx/suppliers/adjust/lovForSupplier`,
          method: 'GET',
          data: {
            ...params,
  ...
  return {
    ...
    transport: {
      read: ({ dataSet }) => {
        ...
        return {
          url: `${Prefix}/${organizationId}/rfx/item-sup-assign/adjust/details`,
          method: 'GET',
          data: { ...otherProps },
        };
      },
    ...
    events: {
      ...
      load: ({ dataSet }) => {
        ...
        const isControllerEditFlag = dataSet.getState('showItemAssignFlag') === 1;
        dataSet.forEach((record) => {
          if(isControllerEditFlag) {
...
//           data: {
//             ...commonProps,
//             customizeUnitCode: 'SSRC.INQUIRY_HALL.NEW_EDIT.SECTION.LINE',
//           },
//         };
//       },
//     },
