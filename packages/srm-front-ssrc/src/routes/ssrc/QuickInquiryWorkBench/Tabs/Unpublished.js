...
export default observer(function Unpublished(props = {}) {
  const {
    dataSet,
    customizeTable = noop,
  ...
  const [aggregation, setAggregation] = useState(
    tableDisplayMap?.unpublished?.isAggregation || false
  );
  ...
  const changeTableDisplay = (aggregationType = false) => {
    ...
    onChangeAggregation('unpublished', aggregationType);
  };
          ...
          },
          {
            ...
            renderer: ({ record }) => {
              return record.get('ladderInquiryFlag') ? (
                <a onClick={() => onShowLadderLevelModal(record)}>
  ...
  return customizeTable(
    {
      code: 'SSRC.QUICK_INQUIRY.LIST.UN_PUBLISHED',
    },
    <SearchBarTable
      virtual
      virtualCell
      cacheState
      searchCode="SSRC.QUICK_INQUIRY.LIST.UN_PUBLISHED_FILTER"
      dataSet={dataSet}
      columns={columns}
