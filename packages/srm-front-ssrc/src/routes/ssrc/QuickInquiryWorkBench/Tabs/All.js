...
export default observer(function All(props = {}) {
  ...
  // 是否聚合
  const [aggregation, setAggregation] = useState(tableDisplayMap?.all?.isAggregation || false);
  ...
  const changeTableDisplay = (aggregationType = false) => {
    ...
    onChangeAggregation('all', aggregationType);
  };
  ...
  return customizeTable(
    {
      code: 'SSRC.QUICK_INQUIRY.LIST.ALL',
    },
    <SearchBarTable
      virtual
      virtualCell
      cacheState
      searchCode="SSRC.QUICK_INQUIRY.LIST.ALL_FILTER"
      dataSet={dataSet}
      columns={columns}
