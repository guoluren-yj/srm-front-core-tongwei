...
import { statusRender, quotationRoundsRender, priceStatusRender } from '../utils';

import styles from '../index.less';

export default observer(function SelectApproval(props = {}) {
  const {
    dataSet,
    customizeTable = noop,
  ...
  const [aggregation, setAggregation] = useState(
    tableDisplayMap?.selectApproval?.isAggregation || false
  );
  ...
  const changeTableDisplay = (aggregationType = false) => {
    ...
    onChangeAggregation('selectApproval', aggregationType);
  };
        ...
        children: [
          ...
          },
          {
            ...
            renderer: ({ record, value, name }) => {
              return value ? (
                <span>{priceStatusRender({ value, name, record })}</span>
              ) : (
                <span>-</span>
              );
            },
  ...
  return customizeTable(
    {
      code: 'SSRC.QUICK_INQUIRY.LIST.SELECT_APPROVAL',
    },
    <SearchBarTable
      virtual
      virtualCell
      cacheState
      searchCode="SSRC.QUICK_INQUIRY.LIST.SELECT_APPROVAL_FILTER"
      dataSet={dataSet}
      columns={columns}
