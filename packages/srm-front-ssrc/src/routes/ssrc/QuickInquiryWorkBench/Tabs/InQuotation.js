...
export default observer(function InQuotation(props = {}) {
  const {
    dataSet,
    customizeTable = noop,
  ...
  const [aggregation, setAggregation] = useState(
    tableDisplayMap?.inQuotation?.isAggregation || false
  );
  ...
  const changeTableDisplay = (aggregationType = false) => {
    ...
    onChangeAggregation('inQuotation', aggregationType);
  };
  ...
  return customizeTable(
    {
      code: 'SSRC.QUICK_INQUIRY.LIST.IN_QUOTATION',
    },
    <SearchBarTable
      virtual
      virtualCell
      cacheState
      searchCode="SSRC.QUICK_INQUIRY.LIST.IN_QUOTATION_FILTER"
      dataSet={dataSet}
      columns={columns}
