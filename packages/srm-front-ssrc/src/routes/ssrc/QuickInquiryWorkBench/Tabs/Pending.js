...
export default observer(function Pending(props = {}) {
  const {
    remote,
    dataSet,
    customizeTable = noop,
    ...
    renderOperate = noop,
    handleChooseFromList = noop,
    onShowQuoLadderLevelModal = noop,
  } = props || {};

  // 是否聚合
  const [aggregation, setAggregation] = useState(tableDisplayMap?.pending?.isAggregation || false);
  ...
  const changeTableDisplay = (aggregationType = false) => {
    ...
    onChangeAggregation('pending', aggregationType);
  };
  ...
  const columns = useMemo(() => {
    ...
    return remote?.process?.('SSRC_QUICK_INQUIRY_WORKBENCH_LIST.PENDING_COLUMNS', _columns, {
      dataSet,
      aggregation,
  ...
  return customizeTable(
    {
      code: 'SSRC.QUICK_INQUIRY.LIST.PENDING',
    },
    <SearchBarTable
      virtual
      virtualCell
      cacheState
      searchCode="SSRC.QUICK_INQUIRY.LIST.PENDING_FILTER"
      dataSet={dataSet}
      columns={columns}
