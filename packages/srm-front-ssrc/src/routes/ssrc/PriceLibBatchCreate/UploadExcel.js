...
export default class UploadExcel extends React.Component {
  ...
  @Bind()
  beforeUpload(file) {
    const { templateCode, param = {}, success = (e) => e, tenantId } = this.props;
    const formData = new FormData();
    formData.append('excel', file, file.name);
    if (templateCode) {
      const url = `${API_HOST}${SRM_SPC}/v1/${tenantId}/lib-mains-import/data-upload?templateCode=${templateCode}&&tenantId=${tenantId}`;
      this.setState({
        uploadLoading: true,
      });
      request(url, {
