...
export default class UndistributedTable extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  handleSelectChange(selectedRowKeys) {
    ...
    father.setState({
      disabledRight: !selectedRowKeys.length > 0,
    });
  }
  ...
  render() {
    ...
    const columns = [
      {
        title: intl.get(`${promptCode}.modal.material.code`).d('物料编码'),
        dataIndex: 'itemCategoryCode',
        width: 200,
      },
      {
        title: intl.get(`${promptCode}.modal.material.name`).d('物料名称'),
    ...
    return (
      <Fragment>
          ...
          <Row gutter={24} style={{ marginBottom: 16 }}>
            <Col span={8}>
              <Row style={{ display: 'flex', alignItems: 'center' }}>
                <Col span={9}>{intl.get(`${promptCode}.modal.material.code`).d('物料编码')}:</Col>
                <Col span={15}>
            ...
            <Col span={8}>
              <Row style={{ display: 'flex', alignItems: 'center' }}>
                <Col span={9}>{intl.get(`${promptCode}.modal.material.name`).d('物料名称')}:</Col>
        ...
        <Table
          ...
          dataSource={undistributedMaterialList}
          pagination={undistributedMaterialPagination}
        />
      </Fragment>
    );
  }
