...
class AttrsDrawer extends React.Component {
        ...
        render: (v, record) => {
          ...
          if (componentType === 'Upload') {
            if (record.attributeName === 'bucketName') {
              this.uploadData.bucketNameFormField = `attributeValue#${record.quotationColumnCmptId}`;
              // 附件的桶是必输的
              formOptions.rules = [
                {
                  required: true,
                  message: intl
                    ...
                    .d(`${record.attributeName}不能为空`),
                },
              ];
            }
            if (record.attributeName === 'bucketDirectory') {
              this.uploadData.bucketDirectoryFormField = `attributeValue#${record.quotationColumnCmptId}`;
            }
          ...
          return (
            <FormItem style={{ marginBottom: '0px' }}>
              {form.getFieldDecorator(`attributeValue#${record.quotationColumnCmptId}`, {
                initialValue,
                ...formOptions,
              })(inputComponent)}
            </FormItem>
  ...
  queryData() {
    ...
    queryInvestigateConfigComponents({
      quotationColumnId: this.props.record.quotationColumnId,
    })
      .then((data) => {
        this.setState({
          list: data,
    ...
    return new Promise((resolve, reject) => {
      this.props.form.validateFields((err, formData) => {
        ...
        this.state.list.forEach((item) => {
          if (!item.quotationColumnCmptId) {
            return;
          }
          const newValue = formData[`attributeValue#${item.quotationColumnCmptId}`];
      ...
      >
          ...
          <Col span={12}>
            <FormItem
              ...
              label={intl
                ...
                .d('字段描述')}
            >
              {getFieldDecorator('columnName', { initialValue: record.columnName })(
                <Input disabled />
              )}
            </FormItem>
          </Col>
        ...
        >
          ...
          <Button
            ...
            type="primary"
            disabled={templateStatus === 'RELEASED'}
