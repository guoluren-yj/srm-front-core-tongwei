...
export default class TableList extends PureComponent {
  ...
  jumpToDetail(record) {
    ...
    if (clarifyNotifyType === 'PRICE') {
      ...
      history.push({
        pathname: isPubPage(path, `${this.activeTabKey}/price-clarification-replay-detail`),
        search: searchString,
      });
      return;
    }
    ...
    history.push({
      pathname: isPubPage(path, `${this.activeTabKey}/review-clarification-detail`),
      search: searchString,
    });
  }
  ...
  jumpToReplayDetail(record) {
    const { history, pathname, search, sourceFrom, activeKey, path } = this.props;
    ...
    history.push({
      pathname: isPubPage(path, `${this.activeTabKey}/review-clarification-replay-detail`),
      search: searchString,
    });
  }
  ...
  jumpToReplay(record) {
    ...
    history.push({
      pathname: isPubPage(path, `${this.activeTabKey}/review-clarification-pending-reply`),
      search: searchString,
    });
  }
  ...
  directionPriceReplyUpdate(record) {
    ...
    history.push({
      pathname: isPubPage(path, `${this.activeTabKey}/price-clarification-replay-update`),
      search: searchString,
    });
  }
  ...
  renderColumns() {
    ...
    let columns = [
      {
        ...
        render: (val, record) => {
          return (
            <>
              <a onClick={() => this.jumpToDetail(record)}>{val}</a>
              <Badge count={record.clarifyNotifySize} className={styles['badge-item']} />
            </>
          );
        },
      },
    ...
    columns = remote
      ? remote.process(
          'SSRC_SUPPLIERQUOTATION_LIST_PROCESS_QUESTIONREPLY_TABLE_COLUMNS',
          columns,
          {
            that: this,
          }
