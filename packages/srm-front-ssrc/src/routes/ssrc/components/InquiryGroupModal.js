...
@formatterCollections({ code: ['ssrc.inquiry'] })
...
export default class InquiryGroup extends Component {
  ...
  @Bind()
  fetchInquiryGroup() {
    const { dispatch, rfxHeaderId, organizationId, readOnly, header = {} } = this.props;
    if (!rfxHeaderId) {
      return;
    }
          ...
          record.$form.getFieldValue('rfxRole') === 'OPENED_BY' ? (
            <FormItem>
              {record.$form.getFieldDecorator('passwordFlag', {
                initialValue: header.passwordFlag || record.passwordFlag,
