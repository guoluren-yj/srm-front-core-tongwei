  ...
  return (
    <div className={style['rules-definition-editor']}>
      ...
      >
        ...
        >
          <Option value="TRUE">
            {intl.get('ssrc.rulesDefinition.view.select.true').d('无条件限制')}
          </Option>
          {!isEmptyParam && (
            <>
              <Option value="OR">
                {intl.get('ssrc.rulesDefinition.view.select.or').d('满足任一条件')}
              </Option>
              <Option value="AND">
                {intl.get('ssrc.rulesDefinition.view.select.and').d('满足所有条件')}
              </Option>
              <Option value="CUSTOMIZE">
                {intl.get('ssrc.rulesDefinition.view.select.customize').d('自定义组合规则')}
              </Option>
            </>
          )}
        </SelectBox>
            ...
            <Form dataSet={customizeConditionCombinationDs} labelLayout="float" columns={20}>
              ...
              {policyConfigDataDs.current &&
                policyConfigDataDs.current.get('conditionType') !== 'TRUE' && (
                  <Tooltip
                    title={intl.get('ssrc.rulesDefinition.view.card.button.add').d('新建条件规则')}
                    colSpan={18}
                  >
                    <a
              ...
              {conditionJsonDs && conditionJsonDs.records && conditionJsonDs.records.length > 0 && (
                <>
                  ...
                  <TextField
                    ...
                    colSpan={18}
                    help={intl
                      .get('ssrc.rulesDefinition.view.help.customizeConditionCombination')
