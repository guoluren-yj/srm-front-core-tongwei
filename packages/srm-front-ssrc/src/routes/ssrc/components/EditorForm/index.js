...
const EditorForm = observer((props) => {
  const {
    customizeForm,
    customizeOptions,
    editorFlag = false,
  ...
  const outputRender = (item) => {
    if (!item) {
      ...
      const { visible = true, ...otherProps } = item;
      const { name: key } = otherProps;
