...
const headerDS = (payload) => {
  const { sourceProjectId, customizeUnitCode, changeType } = payload;
  return {
    fields: [
      ...headerInfoFields(),
      ...getBaseInfoDSFields(),
    ...
    transport: {
      read({ dataSet }) {
        ...
        if (changeType === 'onlyChange') {
          return {
            url: `${prefix}/${getCurrentOrganizationId()}/source-projects/only/change/${sourceProjectId}`,
            method: 'GET',
            data: {
              changeFieldsPrompt: 1,
              ...(templateInfo || {}),
            },
          };
        }
        return {
          url: `${prefix}/${getCurrentOrganizationId()}/source-projects/change/${sourceProjectId}`,
          method: 'GET',
          data: {
            customizeUnitCode,
            changeFieldsPrompt: 1,
...
const itemLineDS = (payload) => {
  const { sourceProjectId, customizeUnitCode, changeType, pageType } = payload;
  return {
    ...
    fields: [
      {
        label: intl.get(`ssrc.projectSetup.model.spChange.changeType`).d('变更类型'),
    ...
    transport: {
      read: ({ data, dataSet }) => {
        ...
        if (pageType === 'viewDetail') {
          // 标段查看物料弹框，不需要区分变更全量还是仅变更
          return {
        ...
        if (changeType === 'onlyChange') {
          return {
            url: `${prefix}/${getCurrentOrganizationId()}/project-line-items/${sourceProjectId}/only/changed/items`,
            method: 'GET',
            data: {
              projectLineSectionId,
              changeFieldsPrompt: 1,
        ...
        return {
          url: `${prefix}/${getCurrentOrganizationId()}/project-line-items/${sourceProjectId}/changed/items`,
          method: 'GET',
          data: {
            projectLineSectionId,
            customizeUnitCode,
  ...
  return {
    ...
    transport: {
      read: ({ dataSet }) => {
        ...
        if (changeType === 'onlyChange') {
          return {
            url: `${prefix}/${getCurrentOrganizationId()}/project-line-sections/${sourceProjectId}/only/changed`,
            method: 'GET',
            data: {
              changeFieldsPrompt: 1,
              ...(templateInfo || {}),
        ...
        return {
          url: `${prefix}/${getCurrentOrganizationId()}/project-line-sections/${sourceProjectId}/changed`,
          method: 'GET',
          data: {
            customizeUnitCode,
            changeFieldsPrompt: 1,
  ...
  return {
    ...
    transport: {
      read: ({ dataSet }) => {
        ...
        if (changeType === 'onlyChange') {
          return {
            url: `${prefix}/${getCurrentOrganizationId()}/project-line-suppliers/${sourceProjectId}/only/changed/suppliers`,
            method: 'GET',
            data: {
              changeFieldsPrompt: 1,
              ...(templateInfo || {}),
        ...
        return {
          url: `${prefix}/${getCurrentOrganizationId()}/project-line-suppliers/${sourceProjectId}/changed/suppliers`,
          method: 'GET',
          data: {
            customizeUnitCode,
            changeFieldsPrompt: 1,
  ...
  return {
    ...
    transport: {
      read({ dataSet }) {
        ...
        if (changeType === 'onlyChange') {
          return {
            url: `${prefix}/${getCurrentOrganizationId()}/project-line-plans/${sourceProjectId}/only/changed/plans`,
            method: 'GET',
            data: {
              changeFieldsPrompt: 1,
              ...(templateInfo || {}),
        ...
        return {
          url: `${prefix}/${getCurrentOrganizationId()}/project-line-plans/${sourceProjectId}/changed/plans`,
          method: 'GET',
          data: {
            customizeUnitCode,
            changeFieldsPrompt: 1,
