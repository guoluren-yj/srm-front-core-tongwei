...
import { Table, Lov, Button, useModal } from 'choerodon-ui/pro';
...
const ItemLine = observer((props) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 80,
      renderer: ({ record }) => {
        if (record?.get('projectLineItemId')) {
          return (
            <DocFlow
    ...
    },
    {
      name: 'ouId',
      editor: (record) => {
        return <Lov record={record} name="ouId" onChange={(value) => changeOuId(value, record)} />;
      },
    },
    {
    ...
    },
    {
      ...
      renderer: ({ record }) => {
        ...
        return (itemCategoryId?.categoryId || itemId?.itemId || quotationTemplateId?.templateId) &&
          projectLineItemId ? (
            <>
              <QuotationDetailModal
                ...
                modalProps={{
                onClose: () => fetchHeader({ refreshSectionFieldsFlag: true }),
              }}
              />
  ...
  const handleCopy = () => {
    ...
    selectedRecords.forEach((r) => {
      ...
      const remoteCopyItemLine = remote
        ? remote.process('SSRC_PROJECTSETUP_SP_CHANGE_PROCESS_COPY_ITEM', copyItemLine, {
            sourceRecord: r,
            sourceJSONRecord,
          })
        : copyItemLine;
  ...
  const handleQuoteApply = async () => {
    const search = querystring.stringify({
      routeFrom: 'projectSetupChange',
      sourceProjectId,
      backPath: `/ssrc/new-project-setup/sp-change/${sourceProjectId}`,
    });
    let data = null;
    try {
      data = await fetchConfigSheet({
  ...
  const buttons = useMemo(
    () => {
      ...
      return remote
      ? remote.process(
          'SSRC_PROJECTSETUP_SP_CHANGE_PROCESS_ITEM_LINE_TABLE_BUTTONS',
          sourceButtons,
          { history, headerDs }
        )
      : sourceButtons;
