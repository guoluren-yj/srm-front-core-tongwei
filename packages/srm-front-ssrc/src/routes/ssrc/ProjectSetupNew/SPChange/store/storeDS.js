...
const getPurAndOrgFields = (payload) => {
  ...
  return [
    ...
    },
    {
      ...
      transformResponse: (value, data) => {
        return value ? { id: value, realName: data?.purAgent } : null;
      },
    },
    {
      name: 'purAgent',
...
const headerDS = (payload) => {
  const { sourceProjectId, customizeUnitCode, remote } = payload || {};
  const fields = [
    ...getBaseInfoDSFields(),
  ...
  const remoteHeaderFields = remote
    ? remote.process(
      'SSRC_PROJECTSETUP_SP_CHANGE_PROCESS_SET_HEADER_CUX_FIELD',
      fields
    )
    : fields;
  return {
...
const itemLineDS = (payload) => {
  const { remote, sourceProjectId, customizeUnitCode, headerDs } = payload || {};
  const dsProps = {
    ...
    transport: {
      ...
      destroy: ({ data }) => {
        return {
          url: `${prefix}/${getCurrentOrganizationId()}/project-line-items/change`,
          method: 'DELETE',
          data,
        };
      },
  ...
  return remote
    ? remote.process('SSRC_PROJECTSETUP_SP_CHANGE_ITEMLINE_DS', dsProps, { payload })
    : dsProps;
};
...
const sectionOrPacketInfoDS = (payload) => {
  ...
  return {
    ...
    transport: {
      ...
      destroy: ({ data }) => {
        return {
          url: `${prefix}/${getCurrentOrganizationId()}/project-line-sections/change`,
          method: 'DELETE',
          data,
        };
      },
    ...
    transport: {
      ...
      destroy: ({ data }) => {
        if (type === 'allot') {
          // 分配物料弹框
          return {
            url: `${prefix}/${getCurrentOrganizationId()}/project-line-sections/change/allot/delete`,
            method: 'DELETE',
            data,
          };
        }
      ...
      submit: ({ data, dataSet }) => {
        ...
        if (type === 'allot') {
          // 分配物料弹框
          return {
            url: `${prefix}/${getCurrentOrganizationId()}/project-line-sections/change/${projectLineSectionId}/allot`,
            method: 'POST',
            data,
          };
        }
...
const supplierLineTableDS = (payload) => {
  ...
  const dsProps = {
    ...
    transport: {
      ...
      destroy: ({ data }) => {
        return {
          url: `${prefix}/${getCurrentOrganizationId()}/project-line-suppliers/change`,
          method: 'DELETE',
          data,
        };
      },
  ...
  return remote
    ? remote.process('SSRC_PROJECTSETUP_SP_CHANGE_SUPPLIER_LINE_TABLE_DS', dsProps, { payload })
    : dsProps;
};
...
const planLineTableDS = (payload) => {
  ...
  return {
    ...
    transport: {
      ...
      destroy: ({ data }) => {
        return {
          url: `${prefix}/${getCurrentOrganizationId()}/project-line-plans/change`,
          method: 'DELETE',
          data,
        };
      },
