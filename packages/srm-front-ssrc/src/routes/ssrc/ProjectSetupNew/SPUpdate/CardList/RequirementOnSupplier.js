...
import { Table, Form, Select, Lov, Button, Tooltip } from 'choerodon-ui/pro';
...
import PackSupplierLov from '../../Components/PackSupplierLov';
import SupplierRelatedGraph from '../../Components/SupplierRelatedGraph';

import Style from '../index.less';

const warnIconSvg = require('@/assets/warn-icon.svg');

// 标段/包信息
const supplierLineTable = observer((props) => {
  const { handleSetOperateLoading = noop, fetchSupplierInfo = noop, fetchHeader = noop } = props;
  ...
  const getSupplierLovProps = useCallback(
    (options = {}) => {
      ...
      const supplierLovFieldProps = {
        ...
        disabled: createFlag,
        buttonTooltip: intl
          .get('ssrc.projectSetup.view.tooltip.supplierLovTooltip')
          .d('请保存后添加'),
        buttonTooltipShowFlag: createFlag, // 只有新建情况下显示气泡
        placeholder: intl
  ...
  const supplierLovOk = () => {
    ...
    return supplierLovDs.submit().then(async (res) => {
      if (res && !res.failed) {
        supplierLovDs.loadData([]);
          ...
          {record.get('qualificationExpiredFlag') === 1 && (
            <Tooltip
              title={intl
                ...
                .d('资质到期')}
            >
              <img src={warnIconSvg} alt="" style={{ marginRight: 8, marginBottom: 2 }} />
            </Tooltip>
          )}
          <span>{value}</span>
        </>
  ...
  const buttons = useMemo(() => {
    const sourceButtons = [
      <PackSupplierLov {...getSupplierLovProps()}>
      ...
      <SupplierRelatedGraph
        name="relationMap"
        disabled={createFlag} // 后续如果修改这里也要修改对应的提示语
    ...
    return remote
      ? remote.process(
          'SSRC_PROJECTSETUP_SP_UPDATE_PROCESS_SUPPLIER_TABLE_BUTTONS',
          sourceButtons,
          { history }
        )
      : sourceButtons;
