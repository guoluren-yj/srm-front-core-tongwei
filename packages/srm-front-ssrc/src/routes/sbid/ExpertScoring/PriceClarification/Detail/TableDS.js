    ...
    events: {
      load: ({ dataSet }) => {
        ...
        dataSet.forEach((record = {}) => {
          const priceClarifyIssueLineId = record.get('priceClarifyIssueLineId');
    ...
    transport: {
      read: ({ data }) => {
        ...
        return {
          ...
          data: {
            clarifyNotifyId,
            customizeUnitCode: `SSRC.${sourceKey}_HALL.CLARIFICATION.SUPPLIERQUOTATION_CREATE_DETAIL`,
          },
        };
      },
    },
...
const LadderLevelModalDS = () => {
  return {
    ...
    transport: {
      read: ({ dataSet }) => {
        const { organizationId, quotationLineId } = dataSet.queryParameter.commonProps || {};
        if (!quotationLineId) {
          return;
        }
