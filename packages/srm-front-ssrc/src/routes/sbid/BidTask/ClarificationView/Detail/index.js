...
@connect(({ bidTask, loading }) => ({
  bidTask,
  detailsLoading: loading.effects['bidTask/fetchClarifyDetail'],
  tableLoading: loading.effects['bidTask/fetchClarifyReferIssue'],
...
export default class Detail extends Component {
  ...
  componentDidMount() {
    const {
      dispatch,
      bidTask: { clarificationQuestionPagination = {} },
    } = this.props;
    const lovCodes = {
  ...
  fetchDetail() {
    ...
    dispatch({
      type: 'bidTask/fetchClarifyDetail',
      payload: {
        organizationId,
  ...
  handleClarificationQuestion(page = {}) {
    ...
    dispatch({
      type: 'bidTask/fetchClarifyReferIssue',
      payload: {
        page,
        organizationId,
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <Header
          backPath={`/ssrc/bid-task/clarification-view/${params.sourceId}/${params.bidNum}/${params.bidTitle}/${params.companyId}/2`}
          title={intl.get(`ssrc.bidTask.view.message.title.detail.clarifyDetail`).d('澄清函详情')}
        />
        <Content>
          <Spin
              ...
              <Panel
                ...
                header={
                  <Fragment>
                    <h3>
                      {intl
                        .get(`ssrc.bidTask.view.message.title.detail.clarifyHeader`)
                        .d('澄清函基本信息')}
                    </h3>
                    <a>
              ...
              <Panel
                ...
                header={
                  <Fragment>
                    <h3>
                      {intl
                        .get(`ssrc.bidTask.view.message.title.detail.clarifyContent`)
                        .d('澄清函正文')}
                    </h3>
                    <a>
              ...
              <Panel
                ...
                header={
                  <Fragment>
                    <h3>
                      {intl
                        .get(`ssrc.bidTask.view.message.title.detail.lineQuestion`)
                        .d('关联问题')}
                    </h3>
                    <a>
