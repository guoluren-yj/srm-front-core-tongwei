...
export default class DetailsForm extends React.Component {
  render() {
    const { issueStatus, sourceNum, questionInformationHeader } = this.props;
    return (
      <div className={styles['information-container']}>
        <Row className={styles['information-item']}>
          <Col span={8}>
            <Row>
              <Col span={9}>
                {intl.get(`${promptCode}.model.supplierBid.questionNum`).d('问题单号')}:
              </Col>
          ...
          <Col span={8}>
            <Row>
              <Col span={9}>
                {intl.get(`${promptCode}.model.supplierBid.questionSupplier`).d('供应商')}:
              </Col>
          ...
          <Col span={8}>
            <Row>
              <Col span={9}>
                {intl.get(`${promptCode}.model.supplierBid.issueStatus`).d('状态')}:
              </Col>
              <Col span={15}>
        ...
        <Row className={styles['information-item']}>
          <Col span={8}>
            <Row>
              <Col span={9}>
                {intl.get(`${promptCode}.model.supplierBid.sourceNo`).d('寻源单号')}:
              </Col>
          ...
          <Col span={8}>
            <Row>
              <Col span={9}>
                {intl.get(`${promptCode}.model.supplierBid.questionSubmitter`).d('提交人')}:
              </Col>
          ...
          <Col span={8}>
            <Row>
              <Col span={9}>
                {intl.get(`${promptCode}.model.supplierBid.questionSubmitDate`).d('提交时间')}:
              </Col>
        ...
        <Row className={styles['information-item']}>
          <Col span={24}>
            <Row>
              <Col span={3}>
                {intl.get(`${promptCode}.model.supplierBid.questionHistory`).d('相关历史问题')}:
              </Col>
        ...
        <Row className={styles['information-item']}>
          <Col span={24}>
            <Row>
              <Col span={3}>
                {intl.get(`${promptCode}.model.supplierBid.questionRemark`).d('备注')}:
              </Col>
