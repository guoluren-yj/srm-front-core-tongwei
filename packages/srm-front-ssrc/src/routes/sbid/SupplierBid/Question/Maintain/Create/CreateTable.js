...
@connect(({ supplierBid, loading }) => ({
  ...
  loading: loading.effects['supplierBid/fetchQuestionRows'],
  questionRowsList: supplierBid.questionRowsList,
  questionRowsPagination: supplierBid.questionRowsPagination,
  deleteRowsLoading: loading.effects['supplierBid/deleteQuestionRows'],
}))
export default class CreateTable extends React.Component {
  ...
  handleRowDelete() {
    ...
    const onOk = () => {
      ...
      if (!isEmpty(arrRow)) {
        dispatch({
          type: 'supplierBid/deleteQuestionRows',
          payload: {
                ...
                rules: [
                  {
                    ...
                    message: intl.get('hzero.common.validation.notNull', {
                      name: intl
                        .get(`${promptCode}.model.supplierBid.clarificationType`)
                        .d('澄清类型'),
                    }),
                  },
                ],
      ...
      },
      {
        ...
        render: (val, record) =>
          ['create', 'update'].includes(record._status) ? (
                ...
                rules: [
                  {
                    required: true,
                    message: intl.get('hzero.common.validation.notNull', {
                      name: intl.get(`${promptCode}.model.supplierBid.description`).d('描述'),
                    }),
                  },
                  {
                    max: 480,
            ...
            <Popover content={record.description}>{record.description}</Popover>
          ),
      },
      {
