...
@connect(({ supplierBid, loading }) => ({
  ...
  questionNoticeHeader: supplierBid.questionNoticeHeader,
  noticeRowsList: supplierBid.noticeRowsList,
  noticeRowsPagination: supplierBid.noticeRowsPagination,
  fetchNoticeRowsLoading: loading.effects['supplierBid/fetchNoticeRows'],
  loadingSubmit: loading.effects['supplierBid/submitNoticeQuestion'],
  loadingSave: loading.effects['supplierBid/saveNoticeQuestion'],
}))
export default class Details extends Component {
  ...
  handleBidIssueHeader() {
    ...
    const routerParam = queryString.parse(location.search.substr(1));
    dispatch({
      type: 'supplierBid/fetchNoticeHeader',
      payload: {
  ...
  fetchMyQuestion(page = {}) {
    ...
    const routerParam = queryString.parse(location.search.substr(1));
    dispatch({
      type: 'supplierBid/fetchNoticeRows',
      payload: {
        page,
      ...
      <div className={styles.itemList}>
        <div className={styles.itemListHeaderInfo}>
          ...
          <div className={styles.itemListHeader} style={{ width: '102%' }}>
            ...
            <span>
              <Tooltip
                title={intl.get(`${promptCode}.view.message.button.questionFile`).d('问题附件')}
                placement="topLeft"
              >
                {''}
                {item.attachmentUuid && (
            ...
            <span style={{ position: 'absolute', right: '100px' }}>
              {item.currentAnswer ? (
                <a style={{ marginRight: '32px' }} onClick={() => this.goAnswerQuestion(item)}>
                  {intl.get(`${promptCode}.view.message.button.editAnsower`).d('修改答复')}
                </a>
              ) : (
                <a style={{ marginRight: '32px' }} onClick={() => this.goAnswerQuestion(item)}>
                  {intl.get(`${promptCode}.view.message.button.ansowerQuestion`).d('回答问题')}
                </a>
              )}
            </span>
  ...
  showAllottedModel() {
    ...
    mean = (
      <Modal
        ...
        title={intl.get(`${promptCode}.model.supplierBid.answerQuestion`).d('回答问题')}
      >
        <Form>
          <Row gutter={48}>
            <Col span={24}>
                  ...
                  rules: [
                    {
                      ...
                      message: intl.get('hzero.common.validation.max', {
                        name: intl
                          .get(`${promptCode}.model.supplierBid.questionBigThanNumLimit`)
                          .d('问题描述超过字数限制'),
                      }),
                    },
                  ],
  ...
  saveAnswerQuestion(item) {
    ...
    form.validateFields((err) => {
      if (isEmpty(err)) {
        ...
        dispatch({
          type: 'supplierBid/saveAnswerQuestion',
          payload: {
            issueFrom: 'SUPPLIER',
            lineNoticeSaveDTOS,
            sourceHeaderId: routerParam.bidHeaderId,
  ...
  handleSave(params) {
    ...
    const routerParam = queryString.parse(location.search.substr(1));
    dispatch({
      type: 'supplierBid/saveNoticeQuestion',
      payload: params,
    }).then((res) => {
      if (res) {
        dispatch({
  ...
  handleNoticeSubmit(params) {
    ...
    const onOk = () => {
      dispatch({
        type: 'supplierBid/submitNoticeQuestion',
        payload: params,
      }).then((res) => {
        if (res === 1) {
          notification.success();
          ...
          Modal.confirm({
            title: intl
              .get(`${promptCode}.model.supplierBid.orConfirmSubmit`)
              .d('有问题还没有回复，是否确认提交?'),
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <Header
          title={intl.get(`${promptCode}.view.message.title.queReply`).d('评审问题回复')}
          backPath={backPath}
        >
          <Button
            type="primary"
          ...
          <Spin spinning={false} wrapperClassName={classnames('ued-detail-wrapper')}>
            ...
            >
              <Panel
                ...
                header={
                  <Fragment>
                    <h3>
                      {intl
                        .get(`${promptCode}.view.message.panel.clarifyInfo`)
                        .d('澄清通知基本信息')}
                    </h3>
                    <a>
              ...
              <Panel
                ...
                header={
                  <Fragment>
                    <h3>{intl.get(`${promptCode}.view.message.clarifyCon`).d('澄清通知正文')}</h3>
                    <a>
