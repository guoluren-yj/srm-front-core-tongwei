...
export default class QuateAttachment extends Component {
  ...
  dataProcess() {
    if (this.props.businessUuid) {
      ...
      this.fetchbusinessAttachment(this.state.businessAttachmentUuid);
    }
    if (this.props.techUuid) {
      ...
      this.fetchTechAttachment(this.state.techAttachmentUuid);
    }
  }
  ...
  render() {
    ...
    const { viewOnly = false, bucketName, bucketDirectory, businessUuid, techUuid } = this.props;
    const {
      businessAttachments,
      techAttachments,
      previewVisible,
    ...
    return (
      <React.Fragment>
        <Row>
          <Col span={11}>
            <p>{intl.get(`ssrc.supplierQuotation.model.supQuo.busiAttach`).d('商务附件')}：</p>
            <UploadButton
              filePreview
              viewOnly={viewOnly}
              multiple
              ...
              attachmentUUID={businessUuid || businessAttachmentUuid}
              fileSize={FIlESIZE}
          ...
          <Col span={11}>
            <p>{intl.get(`ssrc.supplierQuotation.model.supQuo.techAttach`).d('技术附件')}：</p>
            <UploadButton
              filePreview
              viewOnly={viewOnly}
              multiple
              ...
              onPreview={this.handlePreview}
              attachmentUUID={techUuid || techAttachmentUuid}
              bucketName={bucketName}
              fileSize={FIlESIZE}
              bucketDirectory={bucketDirectory}
