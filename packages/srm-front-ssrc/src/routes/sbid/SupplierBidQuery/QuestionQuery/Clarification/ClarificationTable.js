...
export default class ClarificationTable extends React.Component {
  render() {
    ...
    const columns = [
      {
        title: intl.get(`${promptCode}.model.supplierBidQuery.clarificationNo`).d('澄清单号'),
        dataIndex: 'clarifyNum',
        width: 100,
        render: (val, record) => <a onClick={() => onClick(record)}>{val}</a>,
      },
      {
        title: intl.get(`${promptCode}.model.supplierBidQuery.Titleing`).d('标题'),
        dataIndex: 'title',
        width: 200,
      ...
      {
        title: intl.get(`${promptCode}.model.supplierBidQuery.Publisherss`).d('发布人'),
        dataIndex: 'submittedByUserName',
        width: 100,
      },
      {
        title: intl.get(`${promptCode}.model.supplierBidQuery.PublishDatetime`).d('发布时间'),
