...
@connect(({ bidHall, loading }) => ({
  bidHall,
  code: bidHall.code,
  loading: loading.effects['bidHall/fetchQuestionRows'],
  questionRowsList: bidHall.questionRowsList,
  questionRowsPagination: bidHall.questionRowsPagination,
  deleteRowsLoading: loading.effects['bidHall/deleteQuestionRows'],
  saveQuestRowLineLoading: loading.effects['bidHall/saveQuestRowLine'],
}))
export default class CreateTable extends React.Component {
  ...
  handleRowDelete(record = {}) {
    const { dispatch, deleteRowsLoading, questionRowsList, questionRowsPagination } = this.props;
    const onOk = () => {
      // 存储新建行
      const arrCreateRow = [];
      // 存储已有行
      ...
      if (!isEmpty(arrRow)) {
        dispatch({
          type: 'bidHall/deleteQuestionRows',
          payload: {
  ...
  @Bind()
  handleRowCreate() {
    const { dispatch, questionRowsList, questionRowsPagination } = this.props;
    dispatch({
      type: 'bidHall/updateState',
      payload: {
        questionRowsList: [
  ...
  handleRowSave() {
    const {
      ...
      dispatch,
      fetchMyQuestion,
      bidHall: { questionRowsList = [], questionRowsPagination = {} },
    } = this.props;
  ...
  render() {
    ...
    const columns = [
      {
        title: intl.get(`ssrc.question.view.question.isQuotation`).d('是否引用'),
      ...
      {
        title: intl.get(`ssrc.question.view.question.description`).d('问题描述'),
                ...
                rules: [
                  {
                    ...
                    message: intl.get('hzero.common.validation.notNull', {
                      name: intl.get(`ssrc.question.view.question.description`).d('问题描述'),
                    }),
                  },
                  {
                    max: 480,
      ...
      {
        title: intl.get(`ssrc.question.view.question.questionSubmitter`).d('提交人'),
        dataIndex: 'askedByName',
        render: (val) => (val ? <Popover content={val}>{val}</Popover> : { val }),
      },
      {
        title: intl.get(`ssrc.question.view.question.operations`).d('操作'),
