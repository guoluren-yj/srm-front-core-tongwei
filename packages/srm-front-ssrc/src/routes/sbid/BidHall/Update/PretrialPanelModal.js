...
@connect(({ bidHall, loading }) => ({
  bidHall,
  loading: loading.effects['bidHall/fetchPretrialPanel'],
  save: loading.effects['bidHall/savePretrialPanel'],
  deleting: loading.effects['bidHall/deletePretrialPanel'],
}))
export default class PretrialPanelModal extends PureComponent {
  ...
  @Bind()
  handleCreateRows() {
    const { dispatch, bidHall = {} } = this.props;
  ...
  handleDelete() {
    ...
    // 过滤出勾选数据
    const newParameters = filter(bidHall[dataListName], (item) => {
      return selectedRowKeys.indexOf(item.prequalMemberId) >= 0;
    });
    // 过滤出勾选数据的剩下数据
    const newDataList = filter(bidHall[dataListName], (item) => {
      return selectedRowKeys.indexOf(item.prequalMemberId) < 0;
    });
    Modal.confirm({
      ...
      onOk: () => {
        ...
        if (isEmpty(remoteDelete)) {
          ...
          dispatch({
            type: 'bidHall/deletePretrialPanel',
  ...
  @Bind()
  handleSaveRows() {
    const { dispatch, bidHall, sourceHeaderId } = this.props;
    const { rowKey, dataListName } = this.state;
    const dataList = getEditTableData(bidHall[dataListName], [rowKey]);
    ...
    dispatch({
      type: 'bidHall/savePretrialPanel',
      payload: {
        sourceHeaderId,
        organizationId,
        newDataList,
              ...
              <React.Fragment>
                <Form.Item>
                  {getFieldDecorator('phone', {
                    ...
                    <div>
                      {phoneRender(
                        getFieldValue('internationalTelCodeMeaning'),
                        getFieldValue('phone')
                      )}
                    </div>
                  )}
                ...
                <Form.Item style={{ display: 'none' }}>
                  {getFieldDecorator('internationalTelCodeMeaning', {
      ...
      <React.Fragment>
        ...
        >
          ...
          <EditTable
            ...
            dataSource={bidHall[dataListName]}
            columns={columns}
            pagination={false}
          />
        </Modal>
