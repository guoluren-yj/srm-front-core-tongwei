...
export default class CommentImport extends PureComponent {
  ...
  getTemplate() {
    ...
    loadTemplate({ code, prefixPatch, tenantId, requestParams })
      .then((res) => {
  ...
  fetchStatus(params = {}, options = { showMessage: true }) {
    const { prefixPatch, sync, tenantId, batch } = this.state;
    const { afterImport } = this.props;
        ...
        if (statusRes) {
          ...
          if (statusRes.status) {
            ...
            if (statusRes.status === 'IMPORTED') {
              if (afterImport) {
                afterImport({ batch, result: statusRes });
              }
            }
          }
        }
  ...
  handleDownloadTemplateExcel() {
    ...
    const queryParams = [{ name: 'type', value: 'bpmn20' }];
    if (requestParams) {
      // eslint-disable-next-line guard-for-in
      for (const name in requestParams) {
        queryParams.push({ name, value: requestParams[name] });
      }
    }
  ...
  render() {
    ...
    const { requestParams, disabled } = this.props;
    ...
    return (
      <>
        ...
        >
          <Dropdown overlay={templateDownloadMenu} disabled={disabled}>
            ...
            <React.Fragment key="noAutoSync">
              ...
              {dataImportButton === 'true' && (
                <Button
                  ...
                  disabled={uploadLoading || !batch || disabled}
