...
export default class OperationRecord extends PureComponent {
  ...
  handleSearch(fields = {}) {
    ...
    if (
      ...
      dispatch({
        type: 'sendOrder/fetchOperationRecordList',
        payload: {
          page: fields,
          ...filterValues,
  ...
  handleSearchApprove() {
    ...
    const { params } = match;
    dispatch({
      type: 'sendOrder/fetchApproveRecordList',
      payload: {
        poHeaderId: params.id,
      },
    }).then((res) => {
      if (res) {
        const approveData = res
          .reduce(
            (total, currentValue) => [...total, ...(currentValue.historicTaskExtList || [])],
            []
          )
          .filter((item) => item.id);
        this.setState({
  ...
  render() {
    ...
    const tableProps = {
      ...
      bordered: true,
      className: styles['operate-table-container'],
