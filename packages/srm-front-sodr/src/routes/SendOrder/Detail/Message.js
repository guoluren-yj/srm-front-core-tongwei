...
import { recallMessage } from '@/services/sendOrderService';
// TextArea组件初始化
...
export default class Message extends PureComponent {
  ...
  handleFetchMessageByScroll(e) {
    if (e && e.target && e.target.id === 'wrapperMessageBoxList' && e.target.scrollTop === 0) {
  ...
  componentWillUnmount() {
    document.removeEventListener('scroll', this.handleFetchMessageByScroll, true);
  }
        ...
        if (res) {
          ...
          this.setState(
            ...
            () => {
              ...
              if (isScrollBottom && wrapperMessageBoxList) {
                setWrapperMessageBoxListHeight();
    ...
    if (file.size > 100 * 1024 * 1024) {
      ...
      const res = {
        message: intl
          ...
          .d(`上传文件大小不能超过: ${(100 * 1024 * 1024) / (1024 * 1024)} MB`),
      };
   ...
   */
  setFileList(file) {
    const { attchmentAendMessage = (e) => e } = this.props;
    const { listPagination } = this.state;
