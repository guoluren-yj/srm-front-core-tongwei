...
import { clearPoItemBOM, rfxToLine, addDelete } from '@/services/orderWorkspaceService';
...
import { sourcingResults } from '../../../ReferenceDocument/store/referenceDocumentDs';
...
const DetailInfo = (props) => {
  ...
  const { fixedAssetsFlag, returnOrderFlag, displayPoNum } =
    basicCurrent?.get(['fixedAssetsFlag', 'returnOrderFlag', 'displayPoNum']) || {};
  ...
  const handleCreateLine = async () => {
    ...
    if (validateRes.findIndex((i) => !i) === -1) {
      ...
      const sourceCreateRes = await remote.process('sourceCreate', {
        sourcingResultsDs,
        data,
        selected,
      });
      if (!sourceCreateRes) return false;
    ...
    Modal.open({
      ...
      children: (
        <SourcingResults
          ...
          cacheKey="sourcingResults_detail"
        />
      ),
    ...
    Modal.open({
      ...
      children: (
        <Fragment>
          ...
          />
          {customizeForm(
            { code: 'SODR.WORKSPACE_SOURCINGRESULTS_DETAIL.BATCHEDIT', lovIgnore: false },
  ...
  const handleDelete = async () => {
    ...
    if (deleteFlag) {
      const validateFlag = remote
        ? remote.process(
            'SODR.WORKSPACE_MAINTENANCE_SOURCINGRESULTS_PROCESS_VALIDATE',
  ...
  const getButtons = () => {
    const Buttons = observer(({ dataSet }) => {
      ...
      const buttons = [
        {
          ...
          btnProps: {
            ...
            btnProps: {
              ...
              permissionList: [
                {
                  code: 'srm.po-admin.po.order-workspace.ps.button.sourceresultsline.batchedit',
                  type: 'c7n-pro',
                  meaning: '订单工作台-寻源结果明细行-批量编辑',
                },
              ],
            },
          },
        },
        {
          ...
          btnProps: {
            businessObjectTemplateCode: 'SPUC.SOURCE_PO_LINE_IMPORT',
        ...
        },
        {
          ...
          btnProps: {
            ...
            permissionList: [
              {
                code: 'srm.po-admin.po.order-workspace.ps.button.sourceresultsline.delete',
                type: 'c7n-pro',
                meaning: '订单工作台-寻源结果明细行-删除',
              },
            ],
          },
        },
        {
          ...
          btnProps: {
            ...
            permissionList: [
              {
                code: 'srm.po-admin.po.order-workspace.button.sourcing_results.batchSplit',
                meaning: '订单工作台-寻源结果明细行-批量拆分',
              },
            ],
          },
        },
      ...
      return customizeBtnGroup(
        { code: 'SODR.WORKSPACE_SOURCINGRESULTS_DETAIL.LINE_BUTTONS', pro: true },
      ...
      Modal.open({
        ...
        children: (
          <Bom
            ...
            code="SODR.WORKSPACE_SOURCINGRESULTS_DETAIL.BOM"
          />
        ),
      });
    },
              ...
              <Button
                ...
                onClick={() =>
                  viewCostInformation({
                    record,
                    displayPoNum,
                    lineCode: 'SODR.WORKSPACE_SOURCINGRESULTS_DETAIL.COSTINFORMATION',
                  })
                }
              >
  ...
  return (
    <Fragment>
      <ChangeOuAlert dataSet={ds.getState('organizationInfoDs')} />
      {remote.process('linePriceTip', { basicInfoDs, detailInfoDs: ds })}
      {customizeTable(
        {
          code: 'SODR.WORKSPACE_SOURCINGRESULTS_DETAIL.DETAILINFO',
          __force_record_to_update__: true,
          lovIgnore: false,
        },
        <SearchBarTable
          searchCode="SODR.WORKSPACE_SOURCINGRESULTS_DETAIL.DETAILINFO_FILTER"
