...
const BasicInfo = (props) => {
  const { ds, isCreate, customizeForm, remote, organizationInfoDs, oldTermHideFlag } = props;
  const { companyId, supplierId, supplierCompanyId } = organizationInfoDs.current.get([
    'companyId',
    'supplierId',
    'supplierCompanyId',
  ]);
  return customizeForm(
    {
      code: 'SODR.WORKSPACE_CREATEDMANUALLY.BASICINFO',
    ...
    <Form dataSet={ds} columns={3} labelLayout="float" useWidthPercent>
      ...
      <Lov name="poTypeId" />
      <NumberField name="amount" disabled renderer={useAmountRender(ds?.current)} />
      <NumberField name="taxIncludeAmount" disabled renderer={useAmountRender(ds?.current)} />
      <NumberField name="quantityTotal" disabled />
      <Lov name="currencyCode" disabled={!isCreate} />
      ...
      <Lov
        name="pcHeaderIdLov"
        disabled={!companyId?.companyId || !(supplierId || supplierCompanyId)}
      />
