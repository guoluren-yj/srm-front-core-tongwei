...
const OrganizationInfo = (props) => {
  const { ds, isCreate, customizeForm, supplierLovFlag, remote } = props;
  ...
  const supplierLovProps = remote.process(
    'getSupplierLovCompProp',
    {
  ...
  return customizeForm(
    {
      code: 'SODR.WORKSPACE_CREATEDMANUALLY.ORGANIZATIONINFO',
    ...
    <Form dataSet={ds} columns={3} labelLayout="float" useWidthPercent>
      <Lov
        name="companyId"
        {...remote.process('companyEditorProps', { disabled: !isCreate }, { ds })}
      />
      {supplierLovFlag ? (
      ...
      <Lov name="ouId" {...remote.process('ouEditorProps', { disabled: !isCreate }, { ds })} />
      <Lov
        name="purchaseOrgId"
        {...remote.process('purchaseOrgEditorProps', { disabled: !isCreate }, { ds })}
      />
      <Lov name="agentId" />
      {/* 默认隐藏字段 */}
