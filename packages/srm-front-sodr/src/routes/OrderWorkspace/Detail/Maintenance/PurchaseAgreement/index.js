...
import {
  ...
  detailInfo,
  batchMaintenance,
} from './store/PurchaseAgreementDs';
...
const PurchaseAgreement = (props) => {
  ...
  const detailInfoDs = useMemo(
    () =>
      ...
      new DataSet({
        ...(remote
          ? remote.process(
              'SODR.WORKSPACE_MAINTENANCE_PURCHASEAGREEMENT_PROCESS_DETAIL_DS',
              detailInfo()
            )
          : detailInfo()),
        transport: {
  ...
  const giftInfoDs = useMemo(
    () =>
      ...
      new DataSet(
        giftInfoDsConfig({
          poHeaderId: id,
          params: { customizeUnitCode: 'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.GIFTINFO' },
        })
      ),
    [id]
  );
  ...
  const createPrefix = 'srm.po-admin.po.order-workspace.ps.button.purchaseagreement';
  ...
  const fetchDoubleUom = async () => {
    ...
    const res = await queryCommonDoubleUomConfig({ moduleCode: 'SPCM' });
  ...
  useEffect(() => {
    ...
    resetSearchBarCache('SODR.WORKSPACE_PURCHASEAGREEMENT.SEARCH', 'purchaseAgreement_detail');
    if (cacheKey) {
  ...
  const handleSaveOrSubmit = async (type) => {
    ...
    const customizeUnitCode = String([
      ...
      'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.DETAILINFO',
      ...
      'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.COSTINFORMATION',
    ]);
    ...
    if (status.findIndex((i) => !i) === -1) {
      if (type === 'save') {
        ...
        if (getResponse(ras)) {
          ...
          if (ras.value) {
            const confirmModalProps = remote.process('getConfirmModalProps', { data: ras });
            const modalRes = await Modal.confirm({
  ...
  const getHeaderButtons = () => {
    ...
    return (
      <Fragment>
        {customizeBtnGroup({ code: 'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.BUTTONS' }, [
          <Button
            wait={THROTTLE_TIME}
            data-name="submit"
            color="primary"
            ...
            permissionList={[
              {
                code: 'srm.po-admin.po.order-workspace.ps.button.purchaseagreement.submit',
                type: 'c7n-pro',
                meaning: '订单工作台-采购协议明细-提交',
              },
            ]}
          >
          ...
          <Button
            ...
            permissionList={[
              {
                code: 'srm.po-admin.po.order-workspace.ps.button.purchaseagreement.save',
                type: 'c7n-pro',
                meaning: '订单工作台-采购协议明细-保存',
              },
            ]}
          >
          ...
          statusCode && statusCode !== 'REJECTED' && (
            <Button
              ...
              permissionList={[
                {
                  code: 'srm.po-admin.po.order-workspace.ps.button.purchaseagreement.delete',
                  type: 'c7n-pro',
                  meaning: '订单工作台-采购协议明细-删除',
                },
              ]}
            >
  ...
  const getValues = useCallback(() => {
    ...
    if (fieldMap) {
      values.fieldMap = {
        ...fieldMap,
        unitCode: 'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.BATCHEDIT',
      };
    }
    return values;
  ...
  const renderContent = () => (
    <Fragment>
      {customizeCollapse(
        {
          code: 'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.COLLAPSE',
        },
        <Collapse
          trigger="text-icon"
          ghost
        ...
        >
          ...
          >
            <GiftInfo
              ...
              sourceMaintenance
              customizeTable={customizeTable}
              code="SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.GIFTINFO"
            />
          </Panel>
          <Panel
          ...
          >
            <PaymentTermInfo
              ...
              source="maintenance"
              customizeForm={customizeForm}
              customizeCode="SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.PAYMENTTERMINFO"
              getValues={getValues}
              fetchDetailHeader={fetchDetailHeader}
            />
          </Panel>
  ...
  const onTabChange = (newActiveKey) => {
    ...
    history.push({
      pathname: `/sodr/order-workspace/detail/purchase-agreement/${newActiveKey}`,
...
export default compose(
  withCustomize({
    unitCode: [
      ...
      'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.ORGANIZATIONINFO',
      'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.DETAILINFO',
      'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.BOM',
      'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.ATTACHMENTINFO',
      'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.ATTACHMENTINFO_EXTERNAL',
      'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.BATCHEDIT',
      'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.BUTTONS',
      'SODR.WORKSPACE_PURCHASEAGREEMENT.LIST',
      'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.LINE_BUTTONS',
      'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.COLLAPSE',
      'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.GIFTINFO',
      'SODR.WORKSPACE_PURCHASEAGREEMENT_DETAIL.PAYMENTTERMINFO',
    ],
  }),
