...
const DetailInfo = (props) => {
  ...
  const basicInfoDs = ds.getState('basicInfoDs');
  const collByLineFlag = basicInfoDs.current.get('collByLineFlag');
    ...
    Modal.open({
      ...
      children: (
        <Bom
          ...
          code="SODR.WORKSPACE_FEEDBACK_DETAIL.BOM"
        />
      ),
    });
  };
  ...
  return customizeTabPane(
    {
      code: 'SODR.WORKSPACE_FEEDBACK_DETAIL.TABS',
    },
    <Tabs activeKey={activeKey} onChange={setActiveKey} animated={false}>
      ...
      <TabPane tab={intl.get('sodr.workspace.view.panel.otherInfo').d('其他信息')} key="otherInfo">
        {customizeTable(
          {
            code: 'SODR.WORKSPACE_FEEDBACK_DETAIL.OTHERINFO',
          },
      ...
      <TabPane tab={intl.get('sodr.workspace.view.panel.partner').d('合作方')} key="partner">
        {customizeTable(
          {
            code: 'SODR.WORKSPACE_FEEDBACK_DETAIL.PARTNER',
          },
