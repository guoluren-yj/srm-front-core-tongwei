...
const detailInfo = () => ({
  ...
  queryParameter: {
    ...
    lineDisplay: 0,
    customizeUnitCode:
      'SODR.WORKSPACE_FEEDBACK_DETAIL.DETAILINFO,SODR.WORKSPACE_FEEDBACK_DETAIL.DETAILINFO_FILTER',
  },
  events: {
    load: ({ dataSet }) => {
      for (const i of dataSet.records) {
        ...
        const {
          ...
          displayStatusCode,
        } = i.get(['domesticDefaultPrecision', 'domesticFinancialPrecision', 'displayStatusCode']);
        i.init({
        ...
        if (i.selectable) {
          i.selectable = displayStatusCode === 'DELIVERY_DATE_REVIEW';
        }
      }
    },
  },
