...
import { Table, Modal, Form, Button, InputNumber, DatePicker, Col, Row, Tooltip } from 'hzero-ui';
...
export default class OperationRecord extends PureComponent {
      ...
      notification.warning({
        message: intl
          .get('hzero.common.validation.date.after', {
            startDate: intl.get(`sodr.common.model.common.operationDateStart`).d('操作日期从'),
            endDate: intl.get(`sodr.common.model.common.operationDateEnd`).d('操作日期至'),
          })
          .d('操作日期从不能晚于操作日期至'),
      });
    } else {
      dispatch({
        type: 'receivedOrder/fetchOperationRecordList',
        payload: {
          page: fields,
          ...filterValues,
      ...
      <Form layout="inline" className="more-fields-form">
        <Row>
          ...
          <Col span={6}>
            <FormItem
              {...formItemLayout}
              label={intl.get(`sodr.common.model.common.operationDateStart`).d('操作日期从')}
            >
          ...
          <Col span={6}>
            <FormItem
              {...formItemLayout}
              label={intl.get(`sodr.common.model.common.operationDateEnd`).d('操作日期至')}
            >
  ...
  render() {
    ...
    const columns = [
      {
        title: intl.get(`sodr.common.model.common.statusChangeRecord`).d('状态变更记录'),
        children: [
          {
          ...
          {
            title: intl.get(`sodr.common.model.common.operationDate`).d('操作日期'),
            dataIndex: 'processedDate',
            width: 150,
            render: (value, record, index) => this.handleMergeRows(value, record, index, true),
          },
          {
            title: intl.get(`sodr.common.model.common.operationCode`).d('动作'),
              ...
              return (
                record.processRemark && (
                  ...
                  >
                    ...
                    >
                      {this.handleMergeRows(value, record, index).children}
                    </div>
                  </Tooltip>
                )
              );
      ...
      {
        title: intl.get(`sodr.common.model.common.dataChangeRecord`).d('数据变更记录'),
        children: [
          {
            title: intl.get(`sodr.common.model.common.operationCode`).d('动作'),
          ...
          {
            title: intl.get(`sodr.common.model.common.changeContent`).d('修改内容'),
            dataIndex: 'changeFieldNameMeaning',
          },
          {
            title: intl.get(`sodr.common.model.common.oldValue`).d('修改前'),
            dataIndex: 'oldValue',
            width: 80,
          },
          {
            title: intl.get(`sodr.common.model.common.newValue`).d('修改后'),
            dataIndex: 'newValue',
            width: 80,
          },
        ],
    ...
    const modalProps = {
      ...
      title: intl.get(`sodr.receivedOrder.view.message.title.actionHistory`).d('操作记录'),
    };
    const tableProps = {
      loading,
