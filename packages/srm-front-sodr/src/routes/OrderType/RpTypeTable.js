...
import { Table, Badge, Tooltip } from 'hzero-ui';
...
export default class DemangTable extends Component {
  ...
  showEditModal(record) {
    if (this.props.showEditModal) {
      this.props.onRecordCategory(record);
    }
  }
  ...
  render() {
    ...
    const columns = [
      {
        title: intl.get(`entity.order.type.requisitionPlanTypeCode`).d('需求计划类型编码'),
        dataIndex: 'rpTypeCode',
        width: 150,
      },
      {
        title: intl.get(`entity.order.type.requisitionPlanTypeName`).d('需求计划类型'),
      ...
      {
        title: intl.get(`sodr.orderType.model.orderType.visibleRoles`).d('可见角色'),
