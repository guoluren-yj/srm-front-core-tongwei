...
export default class DemangTable extends Component {
  ...
  render() {
    const { loading, dataSource, pagination, onSearch, customizeTable } = this.props;
    const columns = [
      {
        title: intl.get(`entity.order.type.typeCode`).d('项目类型编码'),
        dataIndex: 'typeCode',
        width: 150,
      },
      {
        title: intl.get(`entity.order.type.typeDescription`).d('项目类型'),
