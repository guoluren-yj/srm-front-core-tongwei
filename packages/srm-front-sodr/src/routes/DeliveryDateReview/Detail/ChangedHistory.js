...
export default class ChangedHistory extends PureComponent {
  ...
  handleSearch(fields) {
    ...
    const { params } = match;
    dispatch({
      type: 'deliveryDateReview/fetchChangedHistoryList',
  ...
  render() {
    ...
    const columns = [
      {
        title: intl.get(`${modelPrompt}.revisionNum`).d('版本号'),
        dataIndex: 'revisionNum',
        width: 80,
      },
      {
        title: intl.get(`${modelPrompt}.poLineNum`).d('行号'),
        dataIndex: 'poLineNum',
        width: 80,
      },
      {
        title: intl.get(`${modelPrompt}.poShipNum`).d('发运号'),
        dataIndex: 'poShipNum',
        width: 90,
      },
      {
        title: intl.get(`${modelPrompt}.operationCodeMeaning`).d('动作'),
      ...
      {
        title: intl.get(`${modelPrompt}.oldValue`).d('修改前'),
        dataIndex: 'oldValue',
        width: 80,
      },
      {
        title: intl.get(`${modelPrompt}.newValue`).d('修改后'),
        dataIndex: 'newValue',
        width: 80,
      },
    ];
    const modalProps = {
      ...
      title: intl.get('sodr.deliveryDateReview.view.message.title.changeHistory').d('修改记录'),
    };
    const tableProps = {
      loading,
      columns,
