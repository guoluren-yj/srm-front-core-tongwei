...
@cacheComponent({ cacheKey: '/sodr/delivery-date-review/list' })
    ...
    return customizeFilterForm(
      {
        form,
        expand: expandForm,
        code: 'SODR.DELIVERY_DATE_REVIEW.FILTER',
      },
          ...
          <Col span={18}>
            <Row {...SEARCH_FORM_ROW_LAYOUT}>
              <Col span={8}>
                <FormItem
                  {...formItemLayout}
                  label={intl.get(`${commonModelPrompt}.displayPoNum`).d('订单号')}
                >
            ...
            <Row {...SEARCH_FORM_ROW_LAYOUT} style={{ display: expandForm ? 'block' : 'none' }}>
              ...
              <Col span={8}>
                <FormItem
                  {...formItemLayout}
                  label={intl.get(`${modelPrompt}.feedbackDateStart`).d('交期反馈日期从')}
                >
                  {getFieldDecorator('feedbackDateStart')(
                    <DatePicker
                      format={getDateFormat()}
                      style={{ width: '100%' }}
                      placeholder={null}
                      disabledDate={(currentDate) =>
                        getFieldValue('feedbackDateEnd') &&
                        moment(getFieldValue('feedbackDateEnd')).isBefore(currentDate, 'day')
                      }
                    />
                  )}
                </FormItem>
              ...
              <Col span={8}>
                <FormItem
                  {...formItemLayout}
                  label={intl.get(`${modelPrompt}.feedbackDateEnd`).d('交期反馈日期至')}
                >
                  {getFieldDecorator('feedbackDateEnd')(
                    <DatePicker
                      disabledDate={(currentDate) =>
                        getFieldValue('feedbackDateStart') &&
                        moment(getFieldValue('feedbackDateStart')).isAfter(currentDate, 'day')
                      }
                      format={getDateFormat()}
                      style={{ width: '100%' }}
                      placeholder={null}
              ...
              <Col span={8}>
                <FormItem
                  {...formItemLayout}
                  label={intl.get(`${commonModelPrompt}.urgentOrder`).d('是否加急')}
                >
