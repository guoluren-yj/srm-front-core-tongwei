...
const headerInfo = () => ({
  ...
  fields: [
    ...
    },
    {
      ...
      dynamicProps: {
        disabled: ({ record, dataSet }) =>
          record.get('unSaveEnable') === 2 && !dataSet?.getState('companyIdChange'),
    ...
    },
    {
      ...
      dynamicProps: {
        disabled: ({ record, dataSet }) => {
          const { agentName } = dataSet.getState('response') || {};
          return record.get('unSaveEnable') === 0 && agentName;
        },
        lovPara: ({ record }) => {
          return {
            organizationId,
    ...
    },
    {
      ...
      //     dataSet.current.getPristineValue('currencyCode')?.currencyCode,
      // },
      required: true,
      transformResponse: (value, object) => ({ currencyCode: object?.currencyCode || 'CNY' }),
    ...
    },
    {
      ...
      dynamicProps: {
        disabled: ({ dataSet }) => {
          const { unSaveEnable, termsName } = dataSet.getState('response') || {};
          return unSaveEnable === 0 && termsName;
        },
      },
    },
    {
  ...
  events: {
    load: ({ dataSet }) => {
      ...
      dataSet.forEach((i) => {
        i.init({
          // currencyCode: i.get('currencyCode') || 'CNY',
    ...
    update: async ({ record, name, value, dataSet }) => {
      ...
      if (name === 'ouId') {
        getAutoBind({ record, name, value, dataSet, lineDs: lineInfoDs, type: 'request' });
        record.set({ originalPoHeaderId: null, originalPoNum: null, supplierSiteId: null });
      }
      if (name === 'companyId') {
        const { currencyCode, defaultEnabledFlag } = value || {};
        dataSet.setState({ companyIdChange: 1 });
        const defaultCurrencyCode = defaultEnabledFlag ? 'CNY' : '';
        const currency =
          currencyCode === 'CNY' ? defaultCurrencyCode : currencyCode || defaultCurrencyCode;
        record.set({
          //  ouId: null,
          tempKey: null,
