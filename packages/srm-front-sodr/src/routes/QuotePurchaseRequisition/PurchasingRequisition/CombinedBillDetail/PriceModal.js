...
export default class PriceModle extends Component {
  ...
  onTableChange(page = {}) {
    ...
    dispatch({
      ...
      payload: {
        ...
        sourceBillTypeCode,
        page,
        orderTypeCode: orderHeaderFormDataSource.orderTypeCode,
        localSupplierCompanyId: orderHeaderFormDataSource.supplierId,
      },
    }).then((res) => {
      if (res) {
        const index =
  ...
  render() {
    ...
    const tableProps = {
      ...
      rowSelection:
        [1, -1].includes(orderHeaderFormDataSource.modifyablePriceFlag) &&
        newPriceLibFlag &&
        !orderHeaderFormDataSource.returnOrderFlag
          ? {
              type: 'radio',
              selectedRowKeys,
