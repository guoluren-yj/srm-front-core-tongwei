...
const viewPrompt = 'sodr.orderApproval.view.message';
...
export default class OperationRecord extends PureComponent {
  ...
  handleSearch(fields = {}) {
    ...
    if (
      ...
      dispatch({
        type: 'orderApproval/fetchOperationRecordList',
        payload: {
          page: fields,
          ...filterValues,
  ...
  handleRecordListRows(dataList) {
    ...
    dispatch({
      type: 'orderApproval/updateState',
      payload: {
  ...
  render() {
    ...
    const modalProps = {
      ...
      title: intl.get(`${viewPrompt}.actionHistory`).d('操作记录'),
    };
    const tableProps = {
      loading,
