...
const yesOrNoRender = (text) =>
  intl
    .get(`${commonPrompt}${Number(text) === 1 ? '.status.yes' : '.status.no'}`)
    .d(text === 1 ? '是' : '否');
/**
 * 业务组件 - 我发送的订单
...
export default class List extends Component {
   ...
   */
  setRowBackground(record, radioGroupValue) {
    if (radioGroupValue === 'basic') {
      const map = record.changeMap || {};
      if ('insertFlag' in map) {
        return { backgroundColor: 'rgba(242,85,53,0.10)' };
      }
    }
  ...
  changeTip = (record, dom, fieldName) => {
    ...
    if (fieldName in changeMap) {
      const fieldValue = changeMap[fieldName];
      let tipContent = fieldValue || '【】';
      if (BOOL_FIELDS.includes(fieldName) && [1, 0, '0', '1'].includes(fieldValue)) {
        tipContent = yesOrNoRender(fieldValue);
      }
      const tipTitle = `${intl
        .get('sodr.common.model.common.beforeUpdate')
        .d('变更前')}: ${tipContent}`;
      const _dom = math.isBigNumber(dom) ? (
        dom.toString()
      ) : (
        <span style={{ color: 'red' }}>{dom}</span>
      );
      if (shieldFlag) return _dom;
      return (
        <Tooltip
          title={tipTitle}
          autoAdjustOverflow
          placement="topLeft"
          overlayClassName={styles['change-tip-tooltip']}
        >
          {_dom}
        </Tooltip>
      );
  ...
  getColumns(actionKey) {
    ...
    const defaultColumns = [
      {
        ...
        render: (text, record) => {
          const dom = text;
          return this.changeTip(record, dom, 'displayStatusMeaning');
        },
        onCell: (record) => onCell(record, 'displayStatusMeaning'),
      },
      {
      ...
      },
      {
        ...
        render: (text, record) => {
          const dom = text;
          return this.changeTip(record, dom, 'projectCategoryMeaning');
        },
        onCell: (record) => onCell(record, 'projectCategoryMeaning'),
      },
      {
        title: intl.get(`${modelPrompt}.itemCode`).d('物料编码'),
        dataIndex: 'itemCode',
        width: 90,
        onCell: (record) => onCell(record, 'itemCode'),
        render: (text, record) => {
      ...
      },
      {
        ...
        dataIndex: 'itemName',
        width: 150,
        onCell: (record) => onCell(record, 'itemName'),
      ...
      [
        ...
        [
          ...
          },
          {
            ...
            onCell: (record) => onCell(record, 'quantity'),
          },
          {
            title: getDynamicLabel(doubleUnitEnabled, 'uom'),
            width: 100,
            dataIndex: 'uomName',
            onCell: (record) => onCell(record, 'uomName'),
            render: (_, record) => {
          ...
          },
          {
            ...
            onCell: (record) => onCell(record, 'promiseDeliveryDate'),
            render: (text, record) => {
              const dom = text ? moment(text).format(DEFAULT_DATE_FORMAT) : null;
              const formatDom = dateRender(dom) || null;
              return this.changeTip(record, formatDom, 'promiseDeliveryDate');
            },
          },
          {
            ...
            onCell: (record) => onCell(record, 'lastPurchasePrice'),
            render: (text, record) => {
          ...
          },
          {
            ...
            dataIndex: 'unitPrice',
            align: 'right',
            onCell: (record) => onCell(record, 'unitPrice'),
            render: (text, record) => {
              const dom =
          ...
          },
          {
            ...
            align: 'right',
            onCell: (record) => onCell(record, 'enteredTaxIncludedPrice'),
            render: (text, record) => {
              const dom =
                record.priceShieldFlag === 1 ? (
                  '******'
          ...
          },
          {
            ...
            onCell: (record) => onCell(record, 'unitPriceBatch'),
            render: (val, record) => {
          ...
          },
          {
            ...
            align: 'right',
            onCell: (record) => onCell(record, 'lineAmount'),
            render: (text, record) => {
          ...
          },
          {
            ...
            align: 'right',
            onCell: (record) => onCell(record, 'taxIncludedLineAmount'),
            render: (text, record) => {
              const dom = amountFinancialPrecision(
                record.priceShieldFlag,
                text,
          ...
          },
          {
            ...
            width: 80,
            dataIndex: 'taxRate',
            onCell: (record) => onCell(record, 'taxRate'),
            render: (text, record) => {
              const dom =
          ...
          },
          {
            ...
            onCell: (record) => onCell(record, 'currencyCode'),
            render: (text, record) => {
              const dom = text;
          ...
          },
          {
            ...
            width: 130,
            onCell: (record) => onCell(record, 'departmentName'),
            render: (text, record) => {
              const dom = text;
          ...
          // },
          {
            ...
            onCell: (record) => onCell(record, 'invOrganizationName'),
            render: (text, record) => {
              const dom = text;
          ...
          },
          {
            ...
            onCell: (record) => onCell(record, 'inventoryName'),
            render: (text, record) => {
              const dom = text;
          ...
          },
          {
            ...
            onCell: (record) => onCell(record, 'locationName'),
            render: (text, record) => {
              const dom = text;
          ...
          },
          {
            ...
            dataIndex: 'costName',
            width: 180,
            onCell: (record) => onCell(record, 'costName'),
            render: (text, record) => {
              const dom = text;
          ...
          },
          {
            ...
            width: 180,
            onCell: (record) => onCell(record, 'accountSubjectName'),
            render: (text, record) => {
              const dom = text;
          ...
          },
          {
            ...
            dataIndex: 'wbs',
            width: 180,
            onCell: (record) => onCell(record, 'wbs'),
            render: (text, record) => {
              const dom = text;
          ...
          },
          {
            ...
            width: 100,
            onCell: (record) => onCell(record, 'specifications'),
          ...
          },
          {
            ...
            width: 100,
            dataIndex: 'model',
            onCell: (record) => onCell(record, 'model'),
            render: (text, record) => {
              const dom = text;
          ...
          },
          {
            ...
            width: 120,
            onCell: (record) => onCell(record, 'productBrand'),
            render: (text, record) => {
              const dom = text;
          ...
          },
          {
            ...
            width: 120,
            onCell: (record) => onCell(record, 'productModel'),
            render: (text, record) => {
              const dom = text;
          ...
          },
          {
            ...
            width: 120,
            onCell: (record) => onCell(record, 'packingList'),
            render: (text, record) => {
              const dom = text;
          ...
          // },
          {
            ...
            onCell: (record) => onCell(record, 'brand'),
          },
          {
          ...
          },
          {
            ...
            dataIndex: 'feedback',
            width: 150,
            onCell: (record) => onCell(record, 'feedback'),
            render: (text, record) => {
              const dom = text;
          ...
          },
          {
            ...
            onCell: (record) => onCell(record, 'domesticTaxIncludedPrice'),
            render: (val, record) => {
              const dom =
                record.priceShieldFlag === 1
                  ? '******'
          ...
          },
          {
            ...
            onCell: (record) => onCell(record, 'domesticUnitPrice'),
            render: (val, record) => {
              const dom =
                record.priceShieldFlag === 1
                  ? '******'
          ...
          },
          {
            title: intl
              ...
              .d('本币含税金额'),
            width: 120,
            onCell: (record) => onCell(record, 'domesticTaxIncludedLineAmount'),
          ...
          },
          {
            ...
            onCell: (record) => onCell(record, 'domesticLineAmount'),
            render: (val, record) => {
              const dom = amountFinancialPrecision(
                record.priceShieldFlag,
                val,
          ...
          },
          {
            ...
            onCell: (record) => onCell(record, 'budgetAccountId'),
            render: (_, record) => {
          ...
          },
          {
            ...
            onCell: (record) => onCell(record, 'receiveToleranceQuantityType'),
            render: (val, record) => {
              const dom = val;
      ...
      [
        ...
        [
          ...
          // },
          {
            ...
            onCell: (record) => onCell(record, 'categoryName'),
            render: (val, record) => {
              const dom = (
              ...
              return this.changeTip(record, dom, 'categoryName');
            },
          },
          {
            ...
            onCell: (record) => onCell(record, 'exchangeRate'),
            render: (val, record) => {
              const dom = val;
              return this.changeTip(record, dom, 'exchangeRate');
            },
          },
          {
            ...
            onCell: (record) => onCell(record, 'consignedFlag'),
            render: (text, record) => {
              const dom = intl
                .get(`${commonPrompt}${text === 1 ? '.status.yes' : '.status.no'}`)
                .d(text === 1 ? '是' : '否');
              return this.changeTip(record, dom, 'consignedFlag');
            },
          },
          {
            ...
            onCell: (record) => onCell(record, 'returnedFlag'),
            render: (text, record) => {
              const dom = intl
                .get(`${commonPrompt}${text === 1 ? '.status.yes' : '.status.no'}`)
                .d(text === 1 ? '是' : '否');
              return this.changeTip(record, dom, 'returnedFlag');
            },
          },
          {
            ...
            width: 90,
            dataIndex: 'freeFlag',
            onCell: (record) => onCell(record, 'freeFlag'),
            render: (text, record) => {
              const dom = intl
                .get(`${commonPrompt}${text === 1 ? '.status.yes' : '.status.no'}`)
                .d(text === 1 ? '是' : '否');
              return this.changeTip(record, dom, 'freeFlag');
            },
          },
          {
            ...
            onCell: (record) => onCell(record, 'immedShippedFlag'),
            render: (text, record) => {
              const dom = intl
                .get(`${commonPrompt}${text === 1 ? '.status.yes' : '.status.no'}`)
                .d(text === 1 ? '是' : '否');
              return this.changeTip(record, dom, 'immedShippedFlag');
            },
          },
          // TODO 后端没字段
          {
          ...
          },
          {
            ...
            onCell: (record) => onCell(record, 'shipToThirdPartyName'),
            render: (text, record) => {
              const dom = text;
              return this.changeTip(record, dom, 'shipToThirdPartyName');
            },
          },
          {
          ...
          },
          {
            ...
            onCell: (record) => onCell(record, 'shipToThirdPartyContact'),
            render: (val, record) => {
              const dom = val;
              return this.changeTip(record, dom, 'shipToThirdPartyContact');
            },
          },
          {
          ...
          },
          {
            ...
            onCell: (record) => onCell(record, 'priceUomName'),
            render: (_, record) => {
              const { priceUomCodeName } = record;
              const dom = priceUomCodeName;
              return this.changeTip(record, dom, 'priceUomName');
            },
          },
          {
    ...
    function onCell(record, fieldName) {
      const { changeMap = {}, priceShieldFlag } = record || {};
      const fieldMeaning = fieldName in changeMap;
      const shieldFlag = priceShieldFlag === 1 && SHIED_FIELDS.includes(fieldName);
      const colorStyle = fieldMeaning && !shieldFlag ? { color: 'red' } : {};
      return {
        style: {
          overflow: 'hidden',
          maxWidth: 180,
    ...
    return {
      style:
        radioGroupValue !== 'invoice'
          ? this.setRowBackground(record, radioGroupValue)
          : {
