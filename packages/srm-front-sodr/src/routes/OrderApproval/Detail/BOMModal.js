...
import { Modal, Table, Form, Input, Tooltip } from 'hzero-ui';
...
export default class BOMModal extends PureComponent {
  ...
  handleSearch(params = {}) {
    ...
    fetchBOM({ ...params, creatFlag: 1 }, (res) => {
      this.setState({
        ...res,
      });
    });
  ...
  onCell(record, fieldName) {
    const { changeMap = {} } = record || {};
    const fieldMeaning = changeMap[fieldName];
    const fieldValue = record[fieldName];
    const colorStyle = fieldValue && fieldMeaning ? { color: 'red' } : {};
    return {
      style: {
        ...colorStyle,
      },
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        align: 'center',
        dataIndex: 'itemCode',
        onCell: (e) => this.onCell(e, 'itemCode'),
        render: (value, record) => this.changeTip(record, value, 'itemCode'),
      },
      {
        ...
        width: 120,
        dataIndex: 'itemName',
        onCell: (e) => this.onCell(e, 'itemName'),
        render: (value, record) => this.changeTip(record, value, 'itemName'),
      },
      // {
      ...
      // },
      {
        ...
        onCell: (e) => this.onCell(e, 'categoryName'),
        render: (value, record) => this.changeTip(record, value, 'categoryName'),
      },
      {
        ...
        width: 100,
        dataIndex: 'quantity',
        onCell: (e) => this.onCell(e, 'quantity'),
        render: (value, record) => this.changeTip(record, formatAumont(value), 'quantity'),
      },
      {
        ...
        width: 120,
        dataIndex: 'uomName',
        onCell: (e) => this.onCell(e, 'uomName'),
        render: (_, record) => this.changeTip(record, record.uomCodeAndName, 'uomName'),
      },
      {
        ...
        onCell: (e) => this.onCell(e, 'invOrganizationName'),
        render: (value, record) => this.changeTip(record, value, 'invOrganizationName'),
      },
      {
        ...
        onCell: (e) => this.onCell(e, 'needByDate'),
        render: (text, record) => {
