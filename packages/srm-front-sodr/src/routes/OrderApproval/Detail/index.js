...
@connect(({ loading, orderApproval }) => ({
  ...
  queryPoItemBOMLoading: loading.effects['orderApproval/newQueryPoItemBOM'],
...
export default class Detail extends Component {
   ...
   */
  componentDidMount() {
    const { match = {}, onLoad, onFormLoaded, remote } = this.props;
    const { params } = match;
    this.fetchEnum();
      ...
      onLoad({
        submit: (approveResult) =>
          remote.process(
            ...
            'approvalPromise',
            {
              orderHeaderForm: this.orderHeaderForm,
            },
            approveResult
          ),
      });
  ...
  fetchDetailList(queryParams = {}) {
    ...
    const { params = {} } = match;
    const editFlag =
      this.list && this.list.props && this.list.props.form
        ? this.list.props.form.getFieldValue('changeEditFlag')
        : null;
    const changeParams = !isNil(editFlag) ? { changeEditFlag: editFlag ? 1 : 0 } : {};
    dispatch({
  ...
  fetchBOM(params, success = (e) => e) {
    ...
    dispatch({
      type: 'orderApproval/newQueryPoItemBOM',
      params: {
  ...
  reject() {
    ...
    const viewOnly = openFrom === 'searchModal';
  ...
  getHeaderAttachmentUuid(attachmentUuid) {
    ...
    if (!poHeaderId || !attachmentUuid) return;
    dispatch({
  ...
  getHeaderAttachmentUuidLoad(getHeaderAttachmentUuidLoad) {
    ...
    if (!getHeaderAttachmentUuidLoad || !poHeaderId) return;
    dispatch({
      type: 'orderApproval/saveAttachmentUUID',
  ...
  fetchUUID() {
    ...
    dispatch({
      ...
      if (res && params.poHeaderId && res.content) {
        dispatch({
  ...
  headerBtnsRender() {
    ...
    return remote.process('processHeaderBtn', btns, { orderHeaderFormDataSource });
  }
      ...
      <div className={styles['sodr-order-approval-detail']}>
        {!match.path.includes('/pub/sodr/order-approval/detail') ? (
          ...
          <Header title={intl.get(`${viewMessagePrompt}.titleDetail`).d('订单明细')}>
            ...
            {remote.render('renderOutUuidUpload', <UploadModal {...uploadModalProps} />, {
              orderHeaderFormDataSource,
            })}
            ...
            {remote ? (
              remote.render('SODR.ORDERAPPROVAL_DETAIL_RENDER_APPROVE_HEADER_BTN', <></>, {
                context: this,
              })
            ) : (
              <></>
