...
export default class Attachment extends PureComponent {
    ...
    if (isFunction(onFetchPurchaserAttachmentList)) {
      onFetchPurchaserAttachmentList({
        ...
        if (res) {
          ...
          //   };
          // });
          this.setState({ fileList: this.changeFileList(res, PURCHASER_EXTERNAL_DIRECTORY) }); // 将从数据库获得的文件列表放在fileList里 uuid对应的是 attachmentUUID
        }
      });
    }
  }
  ...
  querySupplierAttachmentList() {
    ...
    if (isFunction(onFetchSupplierAttachmentList)) {
      onFetchSupplierAttachmentList({
        ...
        if (res) {
          this.setState({ receivedFileList: this.changeFileList(res, SUPPLIER_DIRECTORY) });
        }
      });
    }
  }
  ...
  uploadData(file) {
    ...
    return {
      bucketName: BUCKET_NAME,
      directory: PURCHASER_EXTERNAL_DIRECTORY,
      fileName: file.name,
      attachmentUUID,
    };
  }
  ...
  render() {
    const { loading, hideAttachment = (e) => e, viewOnly = false } = this.props;
    const {
      receivedFileList,
      fileList,
      tenantId,
