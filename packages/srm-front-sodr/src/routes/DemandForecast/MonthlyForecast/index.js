...
import { Form, Input, DatePicker, Popover } from 'hzero-ui';
...
export default class extends React.Component {
   ...
   */
  @Bind()
  onRowSelectChange(monthlyForecastSelectedRowKeys, selectedRows) {
    const { handleRowSelectChange } = this.props;
    const monthlyForecastSelectedRows = selectedRows.map((o) => {
  ...
  @Bind()
  highlightLine(month, record) {
    const { forecastStatus, demandForecastDetails = [] } = record;
    const curMonth = `month${month}`;
    const flag = demandForecastDetails.some(
      (item) => item.businessFlag === curMonth && (item.enoughFlag === 0 || !item.enoughFlag)
    );
    if (forecastStatus === 'FEEDBACK' && flag) {
      return { color: 'red' };
    }
  }
  ...
  render() {
    ...
    const columns = [
      {
        ...
        //             message: `${intl.get(`${promptCode}`).d('行号')}不能为空`,
        //           },
        //         ],
        //         initialValue: record.rowKey,
        //       })(<InputNumber min={0} style={{ width: '100%' }} onChange={() => clearSelected()} />)}
        //     </Form.Item>
        //   );
          ...
          const returnComponent = !(
            ...
            <Form.Item>
              {record.$form.getFieldDecorator('forecastYear', {
                ...
                initialValue: record.forecastYear && moment(record.forecastYear, 'YYYY-MM'),
              })(
                <MonthPicker
                  style={{ width: '100%' }}
            ...
            <Form.Item>
              {record.$form.getFieldDecorator('itemCode', {
                ...
                <Lov
                  ...
                  onChange={(_, lovRecord) =>
                    handleItemChange(record, lovRecord, 'monthlyForecast')
                  }
                />
              )}
            </Form.Item>
      ...
      //     .d('月度预测数量'),
      //   dataIndex: 'monthlyForecastQuantity',
      //   key: 'monthlyForecastQuantity',
      //   width: 1200,
      //   children: [
      {
        ...
        render: (text, record) => {
          const returnComponent =
            <Form.Item>
              {record.$form.getFieldDecorator('month1', {
                initialValue: record.month1,
              })(
                <Popover placement="top" content={this.getMonthPopover(1, record.forecastYear)}>
                  <a
                    style={this.highlightLine(1, record)}
                    onClick={() => this.openDetailModal(1, record)}
                  >
                    {text || 0}
                  </a>
                </Popover>)}
      ...
      },
      {
        ...
        render: (text, record) => {
          const returnComponent =
            <Form.Item>
              {record.$form.getFieldDecorator('month2', {
                initialValue: record.month2,
              })(
                <Popover placement="top" content={this.getMonthPopover(2, record.forecastYear)}>
                  <a
                    style={this.highlightLine(2, record)}
                    onClick={() => this.openDetailModal(2, record)}
                  >
                    {text || 0}
                  </a>
                </Popover>)}
      ...
      },
      {
        ...
        render: (text, record) => {
          const returnComponent = (
            <Form.Item>
              {record.$form.getFieldDecorator('month3', {
                initialValue: record.month3,
              })(
                <Popover placement="top" content={this.getMonthPopover(3, record.forecastYear)}>
                  <a
                    style={this.highlightLine(3, record)}
                    onClick={() => this.openDetailModal(3, record)}
                  >
                    {text || 0}
                  </a>
                </Popover>)}
      ...
      },
      {
        ...
        render: (text, record) => {
          const returnComponent = (
            <Form.Item>
              {record.$form.getFieldDecorator('month4', {
                initialValue: record.month4,
              })(
                <Popover placement="top" content={this.getMonthPopover(4, record.forecastYear)}>
                  <a
                    style={this.highlightLine(4, record)}
                    onClick={() => this.openDetailModal(4, record)}
                  >
                    {text || 0}
                  </a>
                </Popover>)}
      ...
      },
      {
        ...
        render: (text, record) => {
          const returnComponent = (
            <Form.Item>
              {record.$form.getFieldDecorator('month5', {
                initialValue: record.month5,
              })(
                <Popover placement="top" content={this.getMonthPopover(5, record.forecastYear)}>
                  <a
                    style={this.highlightLine(5, record)}
                    onClick={() => this.openDetailModal(5, record)}
                  >
                    {text || 0}
                  </a>
                </Popover>)}
      ...
      },
      {
        ...
        render: (text, record) => {
          const returnComponent = (
            <Form.Item>
              {record.$form.getFieldDecorator('month6', {
                initialValue: record.month6,
              })(
                <Popover placement="top" content={this.getMonthPopover(6, record.forecastYear)}>
                  <a
                    style={this.highlightLine(6, record)}
                    onClick={() => this.openDetailModal(6, record)}
                  >
                    {text || 0}
                  </a>
                </Popover>)}
      ...
      },
      {
        ...
        render: (text, record) => {
          const returnComponent = (
            <Form.Item>
              {record.$form.getFieldDecorator('month7', {
                initialValue: record.month7,
              })(
                <Popover placement="top" content={this.getMonthPopover(7, record.forecastYear)}>
                  <a
                    style={this.highlightLine(7, record)}
                    onClick={() => this.openDetailModal(7, record)}
                  >
                    {text || 0}
                  </a>
                </Popover>)}
      ...
      },
      {
        ...
        render: (text, record) => {
          const returnComponent = (
            <Form.Item>
              {record.$form.getFieldDecorator('month8', {
                initialValue: record.month8,
              })(
                <Popover placement="top" content={this.getMonthPopover(8, record.forecastYear)}>
                  <a
                    style={this.highlightLine(8, record)}
                    onClick={() => this.openDetailModal(8, record)}
                  >
                    {text || 0}
                  </a>
                </Popover>)}
      ...
      },
      {
        ...
        render: (text, record) => {
          const returnComponent = (
            <Form.Item>
              {record.$form.getFieldDecorator('month9', {
                initialValue: record.month9,
              })(
                <Popover placement="top" content={this.getMonthPopover(9, record.forecastYear)}>
                  <a
                    style={this.highlightLine(9, record)}
                    onClick={() => this.openDetailModal(9, record)}
                  >
                    {text || 0}
                  </a>
                </Popover>)}
      ...
      },
      {
        ...
        render: (text, record) => {
          const returnComponent = (
            <Form.Item>
              {record.$form.getFieldDecorator('month10', {
                initialValue: record.month10,
              })(
                <Popover placement="top" content={this.getMonthPopover(10, record.forecastYear)}>
                  <a
                    style={this.highlightLine(10, record)}
                    onClick={() => this.openDetailModal(10, record)}
                  >
                    {text || 0}
                  </a>
                </Popover>)}
      ...
      },
      {
        ...
        render: (text, record) => {
          const returnComponent = (
            <Form.Item>
              {record.$form.getFieldDecorator('month11', {
                initialValue: record.month11,
              })(
                <Popover placement="top" content={this.getMonthPopover(11, record.forecastYear)}>
                  <a
                    style={this.highlightLine(11, record)}
                    onClick={() => this.openDetailModal(11, record)}
                  >
                    {text || 0}
                  </a>
                </Popover>)}
      ...
      },
      {
        ...
        render: (text, record) => {
          const returnComponent = (
            <Form.Item>
              {record.$form.getFieldDecorator('month12', {
                initialValue: record.month12,
              })(
                <Popover placement="top" content={this.getMonthPopover(12, record.forecastYear)}>
                  <a
                    style={this.highlightLine(12, record)}
                    onClick={() => this.openDetailModal(12, record)}
                  >
                    {text || 0}
                  </a>
                </Popover>)}
    ...
    return (
      <React.Fragment>
        ...
        {customizeTable(
          {
            code: 'SPRM.PREDICTION_ORDER_CREATION.MONTH_LIST',
          },
