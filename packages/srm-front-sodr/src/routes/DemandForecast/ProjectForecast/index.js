...
export default class extends React.Component {
  ...
  @Bind()
  handleSearch() {
    const { fetchList, projectForecast = {} } = this.props;
    const { list = [] } = projectForecast;
   ...
   */
  @Bind()
  onRowSelectChange(projectForecastSelectedRowKeys, selectedRows) {
    const { handleRowSelectChange } = this.props;
    const projectForecastSelectedRows = selectedRows.map((o) => {
      return { ...o, ...o.$form.getFieldsValue() };
    });
    handleRowSelectChange({ projectForecastSelectedRowKeys, projectForecastSelectedRows });
  }
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: intl
          .get(`${promptCode}.view.message.model.demandForecast.forecastProject`)
          .d('项目'),
        dataIndex: 'forecastProject',
        key: 'forecastProject',
        width: 100,
        render: (text, record) => {
          const returnComponent = !(
            ...
            <Form.Item>
              {record.$form.getFieldDecorator('forecastProject', {
                rules: [
                  {
                    required: activeKey === 'projectForecast',
                    message: `${intl
                ...
                initialValue: record.forecastProject,
      ...
      {
        title: intl.get(`${promptCode}.view.message.model.demandForecast.forecastArea`).d('区域'),
        dataIndex: 'forecastArea',
        key: 'forecastArea',
        width: 150,
        render: (text, record) => {
          const returnComponent = !(
            ...
            <Form.Item>
              {record.$form.getFieldDecorator('forecastArea', {
                rules: [
                  {
                    required: activeKey === 'projectForecast',
                    message: `${intl
                      .get(`${commonPrompt}.validation.notNull`, {
                        name: intl
                          .get(`${promptCode}.view.message.model.demandForecast.forecastArea`)
                          .d('区域'),
                      })
                      .d('区域不能为空')}`,
                  },
                ...
                initialValue: record.forecastArea,
              })(<Input onChange={() => clearSelected()} />)}
            </Form.Item>
          );
          return returnComponent;
            ...
            <Form.Item>
              {record.$form.getFieldDecorator('itemCode', {
                ...
                <Lov
                  ...
                  onChange={(_, lovRecord) => {
                    handleItemChange(record, lovRecord, 'projectForecast');
                    clearSelected();
                  }}
                />
              )}
      ...
      {
        title: intl
          .get(`${promptCode}.view.message.model.demandForecast.demandQuantity`)
          .d('需求总量'),
        ...
        render: (text, record) => {
          const returnComponent = !(
            ...
            <Form.Item>
              {record.$form.getFieldDecorator('demandQuantity', {
                rules: [
                  {
                    required: activeKey === 'projectForecast',
                    message: `${intl
                ...
                initialValue: record.demandQuantity,
              })(
      ...
      {
        title: intl
          .get(`${promptCode}.view.message.model.demandForecast.estimatedDeliveryDate`)
          .d('预计提货时间'),
        ...
        render: (text, record) => {
          const returnComponent = !(
            ...
            <Form.Item>
              {record.$form.getFieldDecorator('estimatedDeliveryDate', {
                rules: [
                  {
                    required: activeKey === 'projectForecast',
                    message: `${intl
                      .get(`${commonPrompt}.validation.notNull`, {
                        name: intl
                          .get(
                            `${promptCode}.view.message.model.demandForecast.estimatedDeliveryDate`
                          )
                          .d('预计提货时间'),
                      })
                      .d('预计提货时间不能为空')}`,
                ...
                initialValue: record.estimatedDeliveryDate
                  ? moment(record.estimatedDeliveryDate)
                  : null,
              })(<DatePicker onChange={() => clearSelected()} />)}
            </Form.Item>
          );
          return returnComponent;
        },
      ...
      },
      {
        ...
        //             required: activeKey === 'projectForecast',
        //             message: `${intl
        //               .get(`${promptCode}.feedbackQuantity`)
        //               .d('供应商反馈库存数量')}不能为空`,
        //           },
        //         ],
      ...
      {
        title: intl.get(`hzero.common.date.supplierRemark`).d('供应商备注'),
    ...
    return (
      <React.Fragment>
        ...
        {customizeTable(
          {
            code: 'SPRM.PREDICTION_ORDER_CREATION.PROJECT_LIST',
          },
