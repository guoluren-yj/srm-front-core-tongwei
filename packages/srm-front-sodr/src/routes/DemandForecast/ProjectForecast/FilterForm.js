...
export default class FilterForm extends PureComponent {
  state = {
    display: false,
    forecastPickFrom: null,
    forecastPickTo: null,
  };

  componentDidMount() {
    const { bindForm, form } = this.props;
    bindForm(form, 'projectForecast');
  }
  ...
  render() {
    ...
    const { display, forecastPickFrom, forecastPickTo } = this.state;
          ...
          <Col span={18}>
            ...
            <Row gutter={12} style={{ display: display ? 'block' : 'none' }}>
              <Col span={8}>
                <Form.Item
                  label={intl
                    .get(`${promptCode}.view.message.model.demandForecast.forecastPickFrom`)
                    .d('预计提货时间从')}
                  {...formLayout}
                >
                  {getFieldDecorator('forecastPickFrom')(
                    <DatePicker
                      disabledDate={currentDate => {
                        if (isEmpty(forecastPickTo)) {
                          return false;
                        }
                        return (
                          moment(currentDate).format('YYYYMMDD') >
                          moment(forecastPickTo).format('YYYYMMDD')
                        );
                      }}
                      onChange={date => {
                        this.setState({ forecastPickFrom: date });
                      }}
                    />
                  )}
                </Form.Item>
              ...
              <Col span={8}>
                <Form.Item
                  label={intl
                    .get(`${promptCode}.view.message.model.demandForecast.forecastPickTo`)
                    .d('预计提货时间至')}
                  {...formLayout}
                >
                  {getFieldDecorator('forecastPickTo')(
                    <DatePicker
                      disabledDate={currentDate => {
                        if (isEmpty(forecastPickFrom)) {
                          return false;
                        }
                        return (
                          moment(currentDate).format('YYYYMMDD') <
                          moment(forecastPickFrom).format('YYYYMMDD')
                        );
                      }}
                      onChange={date => {
                        this.setState({ forecastPickTo: date });
                      }}
                    />
                  )}
                </Form.Item>
              ...
              <Col span={8}>
                <Form.Item
                  ...
                  label={intl
                    ...
                    .d('项目')}
                >
                  {getFieldDecorator('forecastProject')(<Input />)}
                </Form.Item>
              </Col>
            </Row>
            <Row gutter={12} style={{ display: display ? 'block' : 'none' }}>
              <Col span={8}>
                <Form.Item
                  ...
                  label={intl
                    ...
                    .d('区域')}
                >
                  {getFieldDecorator('forecastArea')(<Input />)}
                </Form.Item>
              </Col>
              <Col span={8}>
                <Form.Item
