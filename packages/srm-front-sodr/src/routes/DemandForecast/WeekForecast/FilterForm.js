          ...
          <Col span={18}>
            ...
            <Row gutter={12} style={{ display: display ? 'block' : 'none' }}>
              <Col span={8}>
                <Form.Item
                  label={intl
                    .get(`${promptCode}.view.message.model.demandForecast.weekForecastDateFrom`)
                    .d('预测起始日期从')}
                  {...formLayout}
                >
                  {getFieldDecorator('weekForecastDateFrom')(
                    <DatePicker
                      ...
                      disabledDate={currentDate =>
                        getFieldValue('weekForecastDateTo') &&
                        moment(getFieldValue('weekForecastDateTo')).isBefore(currentDate, 'day')
                      }
                    />
                  )}
                </Form.Item>
              ...
              <Col span={8}>
                <Form.Item
                  label={intl
                    .get(`${promptCode}.view.message.model.demandForecast.weekForecastDateTo`)
                    .d('预测起始日期至')}
                  {...formLayout}
                >
                  {getFieldDecorator('weekForecastDateTo')(
                    <DatePicker
                      format={getDateFormat()}
                      disabledDate={currentDate =>
                        getFieldValue('weekForecastDateFrom') &&
                        moment(getFieldValue('weekForecastDateFrom')).isAfter(currentDate, 'day')
                      }
                    />
                  )}
                </Form.Item>
