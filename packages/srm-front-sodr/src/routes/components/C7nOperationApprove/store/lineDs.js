...
const line = (poHeaderId) => ({
  ...
  autoQuery: false,
  pageSize: 50,
  queryParameter: { queryType: 'tree' },
  queryFields: [
    {
      name: 'processTypeCode',
      display: true,
      noCache: true,
      lookupCode: 'SPUC.PO_OPERATED_ACTION',
      lovPara: { tenantId: organizationId, poHeaderId },
      label: intl.get('sodr.common.operate.processTypeCode').d('操作节点'),
    },
    {
      ...
      range: true,
      display: true,
      label: intl.get('sodr.common.operate.processedDateRange').d('操作时间'),
    },
  ],
  fields: [
    {
...
const approval = () => ({
  ...
  transport: {
    read: () => {
      return {
        url: `${SRM_SPUC}/v1/${organizationId}/po-change-records/list-history-approval/new`,
        method: 'GET',
      };
    },
  },
