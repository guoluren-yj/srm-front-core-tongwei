      ...
      <div className={Styles['approve-list']}>
        ...
        >
          {(approveData || []).map((item) => {
            return (
              <Panel
                header={intl.get('sodr.workspace.model.common.orderApproval').d('订单审批')}
                key={item.id}
                style={{
                  border: 0,
                }}
                      ...
                      <Item color={approveNameRenderColor(ite.action).actionColor || '#E5E5E5'}>
                        {ite.action ? (
                          ...
                          <span className="under-approval">
                            {intl.get('sodr.workspace.model.common.approvaling').d('审批中')}
                          </span>
                        )}
                        {ite.actType === 'startEvent' && (
                          <p className="line">
                            <span className="desc">
                              {intl.get('sodr.workspace.model.common.process.id').d('流程标识')}
                            </span>
                        ...
                        {ite.actType !== 'endEvent' &&
                          (ite.actType === 'startEvent' ? (
                            <p className="line">
                              <span className="desc">
                                {intl.get('sodr.workspace.model.common.initiator').d('发起人')}
                              </span>
                            ...
                            <p className="line">
                              <span className="desc">
                                {intl.get('sodr.workspace.model.common.approvaUser').d('审批人')}
                              </span>
                              <span className="text-name">{ite.assigneeName}</span>
                              {ite.action !== 'Rejected' && (
                        ...
                        {ite.comment && (
                          <div className="approval-comment">
                            <span style={{ width: 60 }} className="approval-title">
                              {intl.get('sodr.workspace.model.common.approveComment').d('审批意见')}
                            </span>
                        ...
                        {ite.attachmentUuid && (
                          <div className="enclosure">
                            <span className="approval-upload">
                              {intl.get('sodr.workspace.model.common.enclosure').d('附件')}
                            </span>
                            <Upload
                              filePreview
                              viewOnly
                        ...
                        {ite.actType !== 'endEvent' && (
                          <p className="line date-line">
                            <span className="desc">
                              {intl.get('sodr.workspace.model.common.dateTime').d('日期')}
                            </span>
