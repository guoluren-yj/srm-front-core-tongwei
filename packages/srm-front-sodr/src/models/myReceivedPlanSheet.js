...
export default {
  ...
  effects: {
    ...
    *batchCode(params, { call, put }) {
    ...
    // 我收到的计划单列表
    *queryMyReceivedPlanUpdateList({ payload }, { call, put }) {
      const result = getResponse(yield call(queryMyReceivedPlanUpdateList, payload));
      if (result) {
        yield put({
          type: 'updateState',
          payload: {
