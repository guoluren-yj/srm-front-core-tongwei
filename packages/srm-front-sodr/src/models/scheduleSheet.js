...
export default {
  ...
  state: {
    ...
    selectedCreateRowKeys: [], // 计划单创建-Tab选中行Key
    selectedUpdateRowKeys: [], // 计划单创建-更新Tab选中行Key
    planPubLish: [], // 计划排程审批表单
  },
  ...
  effects: {
    ...
    *init({ payload }, { call, put }) {
    ...
    // 计划排程审批表单
    *queryPlanPublish({ payload }, { call, put }) {
      const result = getResponse(yield call(queryPlanPublish, payload));
      if (result) {
        yield put({
          type: 'updateState',
          payload: {
    ...
    *createQuery({ payload }, { call, put }) {
      ...
      if (result) {
        yield put({
          type: 'updateState',
          payload: {
            createQueryList: result.content,
    ...
    *queryPlanUpdateList({ payload }, { call, put }) {
    ...
    // 查询计划维护列表(count)
    *queryPlanUpdateListPage({ payload }, { call, put }) {
      const result = getResponse(
        yield call(queryPlanUpdateList, { ...payload, onlyCountFlag: 'Y' })
      );
      if (result) {
        yield put({
          type: 'updateState',
    ...
    *releasePlan({ payload }, { call }) {
    ...
    // 反馈同意计划单
    *feedbackAgreePlan({ payload }, { call }) {
      const res = yield call(feedbackAgreePlan, payload);
      return getResponse(res);
    },
    ...
    *deletePlan({ payload }, { call }) {
    ...
    // 删除前校验计划单
    *deleteWarnPlan({ payload }, { call }) {
      const res = yield call(deleteWarnPlan, payload);
      return getResponse(res);
    },
    // 操作记录
