/*
 * deliveryDateReviewService - 交期审核
...
export async function queryList(params) {
  return request(`${SRM_SPUC}/v1/${organizationId}/po-header/delivery-query`, {
    method: 'GET',
 ...
 */
export async function agree(payload) {
  const { data, customizeUnitCode } = payload;
...
export async function queryDetail(params) {
  const query = { ...filterNullValueObject(parseParameters(params)), camp: 1, sortType: 3 };
  return request(`${SRM_SPUC}/v1/${organizationId}/po-line/detailList`, {
    query,
  });
}
 ...
 * @returns {object} fetch Promise
 */
export async function lineReject(payload) {
  const { data, customizeUnitCode } = payload;
 ...
 * @returns {object} fetch Promise
 */
export async function lineAgree(payload) {
  const { data, customizeUnitCode } = payload;
