 ...
 */
import { SRM_SPUC, SRM_PLATFORM, SRM_HPFM } from '_utils/config';
...
export async function queryPlanCreateList(params) {
  // const customizeUnitCode = 'SODR.PLAN_SHEET_CREATE.LIST_NEW';
...
export async function createQuery(params) {
  // const customizeUnitCode = 'SODR.PLAN_SHEET.CREATE_LIST';
...
export async function releasePlan(data) {
  const customizeUnitCode = 'SODR.PLAN_SHEET_CREATE.LIST_NEW,SODR.PLAN_SHEET_UPDATE.LIST';
  return request(
    `${SRM_SPUC}/v1/${organizationId}/plans/batch/publish?customizeUnitCode=${customizeUnitCode}`,
    {
      method: 'PUT',
      body: data,
    }
 ...
 *  批量反馈确认计划单
 * @async
 * @function feedbackAgreePlan
 * @returns {object} fetch Promise
 */
export async function feedbackAgreePlan(data) {
  const customizeUnitCode = 'SODR.PLAN_SHEET_CREATE.LIST_NEW,SODR.PLAN_SHEET_UPDATE.LIST';
  return request(
    `${SRM_SPUC}/v1/${organizationId}/plans/batch/feedback-approve?customizeUnitCode=${customizeUnitCode}`,
    {
      method: 'PUT',
      body: data,
    }
...
export async function savePlan(data) {
  const customizeUnitCode = 'SODR.PLAN_SHEET_CREATE.LIST_NEW,SODR.PLAN_SHEET_UPDATE.LIST';
  return request(
    `${SRM_SPUC}/v1/${organizationId}/plans/batch?customizeUnitCode=${customizeUnitCode}`,
    {
      method: 'PUT',
      body: data,
    }
 ...
 * @returns {object} fetch Promise
 */
export async function deleteWarnPlan(data) {
  return request(`${SRM_SPUC}/v1/${organizationId}/plans/batch/delete-warn`, {
    method: 'POST',
    body: data,
  });
}
...
export async function queryPlanReleaseList(params) {
  return request(`${SRM_SPUC}/v1/${organizationId}/plans/query/confirm`, {
    method: 'GET',
    query: parseParameters(params),
  });
}
 ...
 * @returns {object} fetch Promise
 */
export async function feedBackSurePlan({ data, customizeUnitCode }) {
  return request(`${SRM_SPUC}/v1/${organizationId}/plans/batch/confirm-feedback`, {
    method: 'POST',
    body: data,
 ...
 * @returns
 */
export async function queryPlanPublish(params) {
  const { planId } = params;
  return request(`${SRM_SPUC}/v1/${organizationId}/plans/queryPlanPublish/${planId}`, {
    method: 'GET',
    query: parseParameters(params),
  });
}
 ...
 * 查询个性化信息
 * @param {Object} params - 查询参数
 * @param {Array} params.code - 个性化编码
 * @returns {object} fetch Promise
 */
export async function cuzQuery(params) {
