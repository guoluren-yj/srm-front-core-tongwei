...
export async function queryList(params) {
  return request(`${SRM_SPUC}/v1/${organizationId}/po-evaluations/query`, {
    method: 'GET',
 ...
 * @param {String} params.page - 页码
 * @param {String} params.size - 页数
 * @param {String} params.poHeaderId - 订单评价头ID
 ...
 * @returns {object} fetch Promise
 */
export async function queryDetailList({ poHeaderId, ...others }) {
  const query = { ...filterNullValueObject(parseParameters(others)), camp: 1, sortType: 1 };
...
export async function queryPartners(poHeaderId, params) {
  const query = filterNullValueObject(parseParameters(params));
  query.customizeUnitCode = 'SODR.ORDER_EVALUATE_DETAIL.PARTNERS';
...
export async function submitEvaluation(params) {
  return request(`${SRM_SPUC}/v1/${organizationId}/po-evaluations/submit`, {
    method: 'PUT',
    body: params,
  });
}
