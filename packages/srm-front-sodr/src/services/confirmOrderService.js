...
import { SRM_SPUC, SRM_PLATFORM, SRM_FINANCE } from '_utils/config';
...
export async function queryList(params) {
  const { supplierTenantId, tenantId, ...otherParams } = params;
  return request(`${SRM_SPUC}/v1/${tenantId}/po-header/confirm-list`, {
    method: 'GET',
    query: parseParameters({ ...otherParams, supplierTenantId }),
  });
}
...
export async function newQueryPoItemBOM(params) {
  const { poHeaderId, poLineId, splQuantity, ...param } = params;
  const query = filterNullValueObject(parseParameters(param));
  return request(`${SRM_SPUC}/v1/${organizationId}/po-item-boms/change`, {
    method: 'POST',
    body: { poHeaderId, poLineId, splQuantity },
    query,
  });
}
...
export async function queryDetailList(poHeaderId, params) {
  const query = {
    ...filterNullValueObject(parseParameters(params)),
    camp: 2,
    sortType: 4,
    poEntryPoint: 'PURCHASE_PO_CONFIRM',
  };
 ...
 * @param {object} data.poLineLocationId - 订单发运行id
 * @returns {object} fetch Promise
 */
...
export async function fetchBillLines(params) {
  return request(`/ssta/v1/${organizationId}/bill-lines/supplier`, {
    method: 'GET',
...
export async function getFeedbackVerificationList(params) {
  ...
  return request(`${SRM_SPUC}/v1/${organizationId}/po-workbench/po-header/feedback/verify`, {
    method: 'POST',
    body: poHeaderList,
    responseType: 'text',
  });
 ...
 * @param {Object} body
 */
export async function getFeedbackVerificationDetail(params) {
  const { poHeader } = params;
  return request(`${SRM_SPUC}/v1/${organizationId}/po-workbench/po-header/feedback/verify/detail`, {
    method: 'POST',
    body: poHeader,
    responseType: 'text',
  });
