...
export default {
  ...
  effects: {
    ...
    *init(params, { call, put }) {
    ...
    // 查询送货单审批列表
    *queryDeliveryApprovedList({ payload }, { call, put }) {
      const result = getResponse(yield call(queryDeliveryApprovedList, payload));
      if (result) {
        yield put({
          type: 'updateState',
          payload: {
