 ...
 *  查询送货单审批列表的数据
 * @async
 * @function queryDeliveryReviewList
 * @param {Object} params - 查询参数
 * @param {String} params.page - 页码
 * @param {String} params.size - 页数
 */
export async function queryDeliveryReviewList(params) {
  const query = filterNullValueObject(parseParameters(params));
  return request(`${SRM_SPUC}/v1/${organizationId}/asn-header/review`, {
    method: 'GET',
    query,
  });
}
...
export async function approveDeliveryOrder(params) {
  ...
  return request(
    `${SRM_SPUC}/v1/${organizationId}/asn-header/review/pass?customizeUnitCode=${customizeUnitCode}`,
    {
      method: 'POST',
...
export async function rejectDeliveryOrder(params) {
  const { customizeUnitCode = '', ...data } = params[0];
  return request(
    `${SRM_SPUC}/v1/${organizationId}/asn-header/review/reject?customizeUnitCode=${customizeUnitCode}`,
    {
      method: 'POST',
      body: customizeUnitCode ? [data] : params,
    }
