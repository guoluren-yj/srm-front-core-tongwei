...
const Index = (props) => {
  const { match = {}, customizeForm, customizeTable, customizeBtnGroup, history } = props;
  const { params } = match;
  const asnHeaderId = useMemo(() => params.id, [match]);

  const headerFormDs = useMemo(() => new DataSet(headerFormDataSet()), [asnHeaderId]);

  const shipmentFormDs = useMemo(() => new DataSet(shipmentFormDataSet()), [asnHeaderId]);

  const logisticsFormDs = useMemo(() => new DataSet(logisticsFormDataSet()), [asnHeaderId]);

  const lineListDs = useMemo(() => new DataSet(lineListDataSet()), [asnHeaderId]);

  const addListDs = useMemo(() => new DataSet(addListDataSet()), [asnHeaderId]);

  const attachmentDs = useMemo(() => new DataSet(attachmentDataSet(headerFormDs)), [asnHeaderId]);
  ...
  /**
   * submitErrorResponse - 处理保存和提交时返回值错误信息
   * @param {object} [res={}] - 返回值
   */
  const submitErrorResponse = (res = {}) => {
    if (Object.prototype.toString.call(res) === '[object Array]') {
      const { message } = res || {};
      const { asnLineList = [] } = res[0];
      const description = (
        <ul style={{ margin: 0, padding: 0 }}>
          {asnLineList
            .filter((o) => o.quantityInvalidFlag === 1)
  ...
  const handleDeleteList = async () => {
    ...
    const { _token, objectVersionNumber } = headerInfo;
    const data =
      state.asnStatus === 'NEW' ? { ...headerInfo } : { asnHeaderId, _token, objectVersionNumber };
    Modal.confirm({
      title: intl.get(`sinv.common.model.common.confirmDelete`).d('是否删除送货单'),
      okText: intl.get('hzero.common.button.sure').d('确定'),
      cancelText: intl.get('hzero.common.button.cancel').d('取消'),
      onOk: async () => {
        try {
  ...
  const saveList = async () => {
    ...
    try {
      const res = await addDetailLines(params.id, data);
      if (getResponse(res)) {
        notification.success();
        headerFormDs.query();
  ...
  const lineDelete = async () => {
    ...
    if (deleteFlag) {
      try {
        const res = await deleteDetailLines(params.id, data);
        if (getResponse(res)) {
          notification.success();
          headerFormDs.query();
          shipmentFormDs.query();
  ...
  const onAttachmentsChangeList = async (record, value) => {
    if (record.get('asnLineId') && !value) {
      const id = uuid();
      const res = await getLineAttachmentUuid({
  ...
  const headerBtns = () => {
    const btns = [
      ...
      },
      {
        ...
        child: (
          <ExcelExportPro
            ...
            requestUrl={`${SRM_SPUC}/v1/${organizationId}/asn-header/${params.id}/lines/export-new`}
      ...
      },
      {
        ...
        child: (
          <CommonImport
            businessObjectTemplateCode="SRM_C_SRM_SINV_ASN_LINE_DETAIL"
            prefixPatch={SRM_SPUC}
            refreshButton
            buttonText={intl.get(`sinv.deliveryCreation.view.button.newImport`).d('新版导入')}
            args={{
              tenantId,
              templateCode: 'SRM_C_SRM_SINV_ASN_LINE_DETAIL',
            }}
            buttonProps={{
              icon: 'archive',
              type: 'c7n-pro',
  ...
  const listProps = {
    ...
    onUploadSuccessList: () => onUploadSuccessList(),
    onAttachmentsChangeList: (record, value, uid) => onAttachmentsChangeList(record, value, uid),
  };
