...
import { Button, Tabs, Modal, Input, Form, InputNumber, DatePicker } from 'hzero-ui';
...
export default class List extends Component {
  ...
  @Bind()
  showUomText(record) {
    const { weightUomCode, weightUomName, unitCodeIsShow } = record;
    let text = weightUomCode && weightUomName ? `${weightUomCode}/${weightUomName}` : weightUomName;
    if (!isNil(unitCodeIsShow)) {
      text =
        unitCodeIsShow === '1' && weightUomCode && weightUomName
          ? `${weightUomCode}/${weightUomName}`
          : weightUomName;
    }
    return text;
  }
          ...
          },
          {
            ...
            render: (_, record) => (
              <FormItem>
                ...
                })(<InputNumber allowThousandth min={0} />)}
              </FormItem>
            ),
          },
          {
            ...
            render: (val, record) =>
              ['create', 'update'].includes(record._status) ? (
                <FormItem>
                  ...
                  })(<Lov code="SMDM.ITEM.UOM.ORG" textValue={this.showUomText(record)} />)}
                </FormItem>
              ) : (
                val
              ),
            ...
            render: (val, record) => (
              <FormItem>
                {record.$form.getFieldDecorator(`productionDate`, {
                  ...
                  //     required: true,
                  //     message: intl.get('hzero.common.validation.notNull', {
                  //       name: intl.get(`sinv.common.model.common.productionDate`).d('生产日期'),
                  //     }),
                  //   },
                  // ],
  ...
  render() {
    ...
    const tableProps = {
      ...
      scroll: { x: scrollX >= 1200 ? scrollX : false, y: 'calc(100vh - 400px)' },
      rowSelection: {
    ...
    return (
      <Fragment>
        <Tabs defaultActiveKey="itemInfo" onChange={this.onRadioGroupChange}>
          <TabPane
