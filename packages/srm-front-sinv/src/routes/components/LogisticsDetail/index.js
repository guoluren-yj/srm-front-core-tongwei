...
import { Row, Col, Spin, Form, Icon } from 'hzero-ui';
...
@withCustomize({
  unitCode: ['SINV.SUPPLIER_DELIVERY.DETAIL.LOGISTICS'],
})
...
export default class LogisticsDetail extends PureComponent {
  ...
  fetchLogistics(flag = false) {
    ...
    if (!isNil(asnHeaderId)) {
      dispatch({
        ...
        if (res && res?.length > 0) {
          const newSymbol =
            (res[0].state || '').substr(0, 1) === '3' || (res[0].state || '').substr(0, 1) === '4'; // 物流签收状态编码为3、4开头的不刷
          this.setState({
  ...
  handleRefreshLogistics() {
    const { dispatch, headerInfo, fetchDetailHeader = (e) => e } = this.props;
