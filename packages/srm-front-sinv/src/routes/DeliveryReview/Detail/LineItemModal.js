...
export default class Attachment extends PureComponent {
  ...
  @Bind()
  uploadData(file) {
    const { reviewAttachmentUuid } = this.props;
    const { bucketDirectory = 'sinv-delivery' } = this.props;
    return {
      bucketName: 'private-bucket',
      directory: bucketDirectory,
      fileName: file.name,
      attachmentUUID: reviewAttachmentUuid,
    };
  }
  ...
  onUploadChange({ file, fileList }) {
    const { tenantId, reviewFileList } = this.state;
    const { bucketName = 'private-bucket', bucketDirectory = 'sinv-delivery' } = this.props;
    ...
    if (status === 'done') {
      if (response.type === 'error') {
        ...
        const fileLists = reviewFileList.filter(
          (f) => f.status === 'done' && f.response.type !== 'error'
        );
        this.setState({
          reviewFileList: fileLists,
        });
        return;
      } else {
      ...
      const fileLists = reviewFileList.filter((f) => f.status === 'done');
      this.setState({
        reviewFileList: fileLists,
      });
      return;
  ...
  @Bind()
  onUploadRemove(file) {
    const { reviewFileList } = this.state;
    const { onRemoveAttachment, reviewAttachmentUuid, onGetPicNums } = this.props;
    ...
    if (isFunction(onRemoveAttachment) && file.url) {
      return onRemoveAttachment({
        ...
        if (res) {
          this.setState({
            reviewFileList: reviewFileList.filter((o) => o.uid !== file.uid),
          });
          notification.success();
        }
      });
          ...
          <Row>
            ...
            <Row>
              <Col>
                ...
                <UploadButton {...uploadProps} fileList={reviewFileList}>
                  <div>
            ...
            <Row>
              <Col>
                ...
                <UploadButton {...downloadProps} fileList={otherFileList} viewOnly>
                  {/* {otherFileList >= 0 ? null : (
                    <div>
                      <Icon style={{ fontSize: '32px', color: '#999' }} type="plus" />
                    </div>
                  )} */}
