...
export default class Search extends PureComponent {
  ...
  render() {
    ...
    const { expandForm, organizationId, tenantNum } = this.state;
    const { flag = [], type = [], status = [], importStatus = [], cancelStatus = [] } = enumMap;
    return customizeFilterForm(
      {
        form,
        expand: expandForm,
        code: 'SINV.PURCHASER_DELIVERY_LIST.FILTER',
      },
            ...
            <Row {...SEARCH_FORM_ROW_LAYOUT}>
              ...
              <Col span={8}>
                <FormItem
                  {...formItemLayout}
                  label={intl
                    .get(`sinv.purchaserDelivery.model.purchaserDelivery.asnStatus`)
                    .d('送货单状态')}
                >
            ...
            <Row {...SEARCH_FORM_ROW_LAYOUT} style={{ display: expandForm ? 'block' : 'none' }}>
                ...
                >
                  {getFieldDecorator('expectedArriveDateFrom')(
                    <DatePicker
                      ...
                      placeholder={null}
                      disabledDate={(currentDate) =>
                        getFieldValue('expectedArriveTo') &&
              ...
              <Col span={8}>
                <FormItem
                  {...formItemLayout}
                  label={intl
                    .get(`sinv.purchaserDelivery.model.purchaserDelivery.printFlag`)
                    .d('是否可打印')}
                >
              ...
              <Col span={8}>
                <FormItem {...formItemLayout} label={intl.get(`entity.item.code`).d('物料编码')}>
                  {getFieldDecorator('itemId')(
                    <Lov queryParams={{ tenantId: organizationId }} code="LOV_ASN_ITEM_CODE_VIEW" />
                  )}
                </FormItem>
              </Col>
              <Col span={8}>
