...
const waitConfirmTableDS = () => ({
  ...
  transport: {
    read: ({ data }) => {
      ...
      return {
        url: `${SRM_SPUC}/v1/${organizationId}/sinv/rcv/trx/workbench/supplier/confirm/query`,
...
const waitConfirmAsnTableDS = () => ({
  ...
  transport: {
    read: ({ data }) => {
      ...
      const queryData = filterNullValueObject({ ...params, ...other });
      return {
        url: `${SRM_SPUC}/v1/${organizationId}/sinv/rcv/trx/workbench/supplier/confirm/line-query`,
        method: 'GET',
        data: queryData,
      };
    },
