...
const tableDS = (renderValidateField) => ({
  dataToJSON: 'all',
  ...
  fields: [
    ...
    },
    {
      ...
      dynamicProps: {
        ...
        //     return 1;
        //   }
        //   const uomPrecision = !isNil(record.get('secondaryUomPrecision')) ? record.get('secondaryUomPrecision') : 10;
    ...
    },
    {
      ...
      dynamicProps: {
        ...
        max: ({ record }) => {
          if (record.get('parentLimitlessReceiptFlag') === 1 && record.get('subjectType') === "QUANTITY") {
            return false;
          }
          if (
    ...
    },
    {
      ...
      dynamicProps: {
        ...
        disabled: ({ dataSet }) => {
          ...
          const invFlag =
            new Set(invData || []).size === 0 ||
            (new Set(invData || []).size === 1);// 判断采购组织是否相等 相等true 不等 false
