...
export default class Attachment extends PureComponent {
  ...
  @Bind()
  onUploadRemove(file) {
    const { supplierOtherFileList } = this.state;
    const { onRemoveAttachment, supplierAttaUuid } = this.props;
    if (isFunction(onRemoveAttachment) && file.url && supplierAttaUuid) {
      return onRemoveAttachment({
        bucketName: 'private-bucket',
        bucketDirectory: 'sinv-delivery',
        urls: (isString(file.url) && [file.url.substr(file.url.lastIndexOf('=') + 1)]) || '',
        attachmentUUID: supplierAttaUuid,
      }).then((res) => {
        if (res) {
          this.setState({
            supplierOtherFileList: supplierOtherFileList.filter((o) => o.uid !== file.uid),
          });
          notification.success();
        }
      });
  ...
  render() {
    const { bucketName, bucketDirectory, hideAttachment = (e) => e, supplierAttaUuid } = this.props;
    const {
      approvedFileList,
      reviewFileList,
      otherFileList,
        ...
        <Col span={12}>
          ...
          <Row>
            <Col>
              ...
              <UploadButton {...downinOtherProps} fileList={supplierOtherFileList} />
            </Col>
          </Row>
        </Col>
      </Row>
