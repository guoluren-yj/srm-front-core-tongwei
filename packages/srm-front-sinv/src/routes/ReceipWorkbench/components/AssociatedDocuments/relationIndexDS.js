...
const tableDS = (workFlag, doubleUnitEnabled) => ({
  ...
  fields: [
    ...
    },
    {
      ...
      type: 'number',
      label: intl.get('sinv.receiptWorkbench.model.receipt.pcQuantity').d('标的数量'),
    },
    {
      name: 'pcQuantity',
      type: 'number',
      label: doubleUnitEnabled
        ? intl.get('sinv.receiptWorkbench.model.receipt.basePcQuantity').d('标的基本数量')
        : intl.get('sinv.receiptWorkbench.model.receipt.pcQuantity').d('标的数量'),
    },
    {
      name: 'stageCode',
      type: 'string',
