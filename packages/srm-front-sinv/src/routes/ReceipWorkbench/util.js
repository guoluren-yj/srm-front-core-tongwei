...
import { Popover, Checkbox, Icon, Tag } from 'choerodon-ui';
...
// 来源单据状态、
export const ColorRender = ({ value, setShow, record }) => {
  const { data } = record;
    ...
    return (
      <div>
        <Tag color="red" style={{ border: 'none' }}>
          <span onClick={() => setShow(true, record)}>{record.get('sourceStatusMeaning')}</span>
          <C7nPopover record={data} />
        </Tag>
      </div>
    );
...
// 状态
export const statusShow = (record) => {
  const value = record.get('rcvStatusCode');
  if (['20_SUBMITTED', '35_PUBLISH'].includes(value)) {
    // 绿色
    return (
      <div>
...
export const useLineTag = (record) => {
  const value = record.get('importStatus');
...
// 操作记录导出
const CuxExcelExportPro = ({ dataSet, rcvTrxHeaderId }) => {
  // const params = dataSet?.queryDataSet?.toData()[0];
  const params = dataSet?.queryDataSet?.current?.toData();
  return (
    <ExcelExportPro
      buttonText={intl.get('hzero.common.button.export').d('导出')}
      templateCode="SRM_C_SRM_SINV_RCV_TRX_HEADER_RECORD_EXPORT" // 导出模板编码
      // exportAsync // 是否异步
      otherButtonProps={{
        type: 'c7n-pro',
        icon: 'none',
      }}
      requestUrl={`${SRM_SPUC}/v1/${getCurrentOrganizationId()}/sinv/rcv/trx/rcv-action-record/export/new-module`}
      queryParams={{ ...params, rcvTrxHeaderId }}
      allBody
      method="POST"
    />
  );
