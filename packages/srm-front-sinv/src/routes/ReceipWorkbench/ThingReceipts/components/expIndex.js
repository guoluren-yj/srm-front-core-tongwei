...
class Process extends Component {
  ...
  async componentDidMount() {
    ...
    const { sourceHeaderNum, sourceLineNum, rcvTrxLineId, sourceOrderType} = dataGather;
    // 拿到permissionList
  ...
  render() {
    ...
    const _quantityPercent = isNaN((quantityPercent * 100).toFixed(0))
      ? 0
      : (quantityPercent * 100).toFixed(0);
    const _quantity = isNaN(quantity) ? 0 : quantity;
    return (
...
export default class Index extends Component {
  constructor(props) {
    ...
    this.tableDs = new DataSet(tableDS(this.props.doubleUnitEnabled));
  }
  ...
  async componentDidMount() {
    ...
    // 取首个node判断是否有数据
    const nodeConfigId = Array.isArray(res) && res.length && res[0]?.nodeConfigId;
    this.tableDs.setQueryParameter('params', {
      size: 3,
      nodeConfigId,
      sourceHeaderNum,
      ...
      <Fragment>
        ...
        <Spin spinning={nodeListLoading}>
          <div className="exp-list">
            ...
            {doubleUnitEnabled ? (
              <div className="exp-item secondaryAmount">
                {intl.get(`sinv.receiptWorkbench.model.receipt.amount`).d('数量')}
              </div>
            ) : null}

            <div className="exp-item amount">
              {doubleUnitEnabled
