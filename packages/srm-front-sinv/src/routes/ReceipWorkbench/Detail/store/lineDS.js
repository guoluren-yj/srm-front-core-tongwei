...
const formDS = (doubleUnitEnabled, cuxUpdate) => ({
  ...
  fields: [
    ...
    },
    {
      ...
      label: doubleUnitEnabled
        ...
        : intl.get('sinv.receiptExecution.model.receipt.invoicesQuantity').d('汇总数量'),
    },
    {
  ...
  events: {
    ...
    update: ({ record, name, value, dataSet }) => {
      // 二开埋点编辑
      try {
        if (typeof cuxUpdate === 'function') {
          cuxUpdate({ record, name, value, dataSet });
        }
      } catch (e) {
        // console.log(e);
      }
...
const tableDS = (cuxListField, renderValidateField, handleLineChange) => ({
  dataToJSON: 'all',
  ...
  events: {
    ...
    update: ({ record, name, value, dataSet }) => {
      ...
      if (isFunction(handleLineChange)) {
        handleLineChange({ name, record, value, dataSet });
      }
    },
  },
  transport: {
