...
export default class ReceiptExecution extends Component {
  ...
  goDetail = (labelHeaderId) => {
    ...
    this.props.history.push({
      pathname: `/sinv/purchase-box-label-creation/detail/${labelHeaderId}`,
  ...
  getButtons = (fieldStatus) => {
    const { createAndPrintLoading = false } = this.props;
  ...
  render() {
    ...
    return (
      <Fragment>
        <Header
          title={intl
            .get('sinv.boxLabelCreation.view.title.purchaseBoxLabelCreation')
            .d('采购方标签创建/查询')}
        >
          {buttons}
        </Header>
