...
const tableDS = (formDs, cuxLineUpdate, cuxListField, doubleUnitEnabled) => ({
  dataToJSON: 'all',
  ...
  fields: [
    ...
    },
    {
      ...
      // min: 0,
      label: intl
        .get('sinv.receiptExecution.model.receipt.secondaryExecuteReverseQuantity')
        .d('退货辅助数量'),
      dynamicProps: {
        step: ({ record }) => {
  ...
  events: {
    update: ({ record, name, value, dataSet }) => {
      ...
      try {
        if (typeof cuxUpdate === 'function') {
          cuxLineUpdate({ record, name, value, dataSet });
        }
      } catch (e) {
        // console.log(e);
      }
