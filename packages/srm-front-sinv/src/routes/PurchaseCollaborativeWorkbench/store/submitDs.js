...
const submitDataSet = (sureSupplier) => ({
  autoQuery: false,
  dataToJSON: 'all',
  cacheSelection: true,
  cacheModified: true,
  ...
  queryParameter: {
    customizeUnitCode: sureSupplier
      ? 'SINV.SUPPLIER.COLLABORATIVE.WORKBENCH.WAIT.SEARCH,SINV.SUPPLIER.COLLABORATIVE.WORKBENCH.WAIT.LIST'
      : 'SINV.PURCHASER.COLLABORATIVE.WORKBENCH.WAIT.SEARCH,SINV.PURCHASER.COLLABORATIVE.WORKBENCH.WAIT.LIST',
  },
  fields: [
    {
      name: 'invStatus',
  ...
  transport: {
    read: () => {
      return {
        url: sureSupplier
          ? `${SRM_SPUC}/v1/${organizationId}/stockout/inv/header/supplier/waiting`
          : `${SRM_SPUC}/v1/${organizationId}/stockout/inv/header/waiting`,
        method: 'GET',
      };
    },
  },
