...
export default function SubmitList(props) {
  const { affirmDs, history, activeKey, customizeTable, sureSupplier, location } = props;
  const columns = [
    {
      name: 'invStatus',
      align: 'left',
    ...
    <div style={{ height: 'calc(100vh - 252px)' }}>
      {customizeTable(
        ...
        <SearchBarTable
          ...
          searchBarConfig={{
            onQuery: (params) => handleQuery(params, affirmDs, location, 'displayInvNums', history),
            onClear: () => handleReset(affirmDs),
            onReset: () => handleReset(affirmDs),
            left: {
              render: () => (
                <MultiTextFilter
                  name="displayInvNums"
