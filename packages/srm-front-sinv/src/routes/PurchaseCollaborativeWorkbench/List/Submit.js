...
export default function SubmitList(props) {
  const { submitDs, history, activeKey, customizeTable, sureSupplier, location } = props;
    ...
    <div style={{ height: 'calc(100vh - 252px)' }}>
      {customizeTable(
        ...
        <SearchBarTable
          ...
          searchBarConfig={{
            onQuery: (params) => handleQuery(params, submitDs, location, 'displayInvNums', history),
            onClear: () => handleReset(submitDs),
            onReset: () => handleReset(submitDs),
            left: {
              render: () => (
                <MultiTextFilter
                  name="displayInvNums"
