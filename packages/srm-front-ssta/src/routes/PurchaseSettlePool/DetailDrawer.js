...
const FilterDrawer = (props) => {
  ...
  const ecInvoiceFormDs = React.useMemo(() => new DataSet(ecInvoiceFormDS()), []);

  const ecInvoiceLineDs = React.useMemo(() => new DataSet(ecInvoiceLineDS()), []);

  const [DetailData, setDetailData] = React.useState(null);
  const [activeKey, setActiveKey] = React.useState('main');
  const [ecInvoiceViewFlag, setEcInvoiceViewFlag] = React.useState(false); // 电商发票信息字段是否可查看
  ...
  React.useEffect(() => {
    ...
    ecInvoiceLineDs.setQueryParameter('settleId', record.get('settleId'));
    ecInvoiceFormDs.setQueryParameter('settleId', record.get('settleId'));
    detailDS.query().then((res) => {
      if (res) {
        const { invoiceMethod } = res;
        detailDS.loadData([{ ...record.get(['errorSettleNum']), ...res }]);
        // 开票方式为随货开票 或 集中开票才查询电商信息接口
        if (['1', '2'].includes(invoiceMethod)) {
          ecInvoiceFormDs.query();
          ecInvoiceLineDs.query().then((ecLineRes) => {
            if (ecLineRes?.content && !isEmpty(ecLineRes.content)) {
              setEcInvoiceViewFlag(true);
            } else {
              setEcInvoiceViewFlag(false);
            }
          });
        } else {
          setEcInvoiceViewFlag(false);
    ...
    reconDs.setQueryParameter('customizeUnitCode', 'SSTA.PURCHASE_POOL_RECORD.BILL_GRID');
    reconDs.query();
    ...
    invoiceDs.setQueryParameter('customizeUnitCode', 'SSTA.PURCHASE_POOL_RECORD.INVOICE_GRID');
    invoiceDs.query();
    ...
    payDs.setQueryParameter('customizeUnitCode', 'SSTA.PURCHASE_POOL_RECORD.PAYMENT_GRID');
    payDs.query();
  }, []);
  ...
  /**
   * 结算事务行-抽屉-结算主信息-交易事务信息-电商发票信息 跳转
   */
  const handleInvoiceInfo = () => {
    Modal.open({
      ...
      closable: true,
      style: { width: 1000 },
      title: intl.get(`${prefix}.view.title.invoiceInformation`).d('发票查看'),
      children: (
        <InvoiceInfoModal
          ecInvoiceFormDs={ecInvoiceFormDs}
          ecInvoiceLineDs={ecInvoiceLineDs}
          history={history}
          detailDS={detailDS}
        />
      ),
      ...
      <Tabs activeKey={activeKey} animated onChange={handleTabChange}>
        <TabPane tab={intl.get(`${prefix}.view.title.main`).d('结算主信息')} key="main">
          ...
          >
            {customizeForm(
              {
                code: 'SSTA.PURCHASE_POOL_DETAIL.TRADINGPARTY',
              },
          ...
          >
            {customizeForm(
              {
                code: 'SSTA.PURCHASE_POOL_DETAIL.TRANSACTIONAMOUNT',
              },
              <Form dataSet={detailDS} columns={4} useColon={false} labelLayout="vertical">
          ...
          >
            {customizeForm(
              {
                code: 'SSTA.PURCHASE_POOL_DETAIL.TRANSACTIONMATTER',
              },
              <Form dataSet={detailDS} columns={4} useColon={false} labelLayout="vertical">
                ...
                <Output
                  ...
                  renderer={() => {
                    return (
                      <a onClick={handleInvoiceInfo}>
                        {ecInvoiceViewFlag ? intl.get('hzero.common.button.view').d('查看') : '-'}
                      </a>
                    );
                  }}
                />
              ...
              <div className={Styles['ssta-detailDrawer-content']}>
                ...
                >
                  <TabPane key={Number(infoIsTable) === 1 ? 'mainFinal' : 'mainRecord'}>
                    {customizeTable(
                      { code: 'SSTA.PURCHASE_POOL_RECORD.BILL_GRID' },
              ...
              <div className={Styles['ssta-detailDrawer-content']}>
                ...
                >
                  <TabPane key={Number(statusIsTable) === 1 ? 'statusFinal' : 'statusRecord'}>
                    {customizeTable(
                      { code: 'SSTA.PURCHASE_POOL_RECORD.INVOICE_GRID' },
              ...
              <div className={Styles['ssta-detailDrawer-content']}>
                ...
                >
                  <TabPane key={Number(paymentIsTable) === 1 ? 'paymentFinal' : 'paymentRecord'}>
                    {customizeTable(
                      { code: 'SSTA.PURCHASE_POOL_RECORD.PAYMENT_GRID' },
        ...
        <TabPane tab={intl.get(`${prefix}.view.title.strategy`).d('结算策略信息')} key="strategy">
          ...
          >
            {customizeForm(
              { code: 'SSTA.PURCHASE_POOL_DETAIL.DATARULES' },
          ...
          >
            {customizeForm(
              { code: 'SSTA.PURCHASE_POOL_DETAIL.BILLRULES' },
              <Form dataSet={configDS} columns={4} useColon={false} labelLayout="vertical">
          ...
          >
            {customizeForm(
              { code: 'SSTA.PURCHASE_POOL_DETAIL.SETTLERULES' },
              <Form dataSet={configDS} columns={4} useColon={false} labelLayout="vertical">
...
export default compose(
  ...
  withCustomize({
    unitCode: [
      ...
      'SSTA.PURCHASE_POOL_RECORD.BILL_GRID',
      'SSTA.PURCHASE_POOL_RECORD.INVOICE_GRID',
      'SSTA.PURCHASE_POOL_RECORD.PAYMENT_GRID',
      'SSTA.PURCHASE_POOL_RECORD.PEYPAYMENT_BOX',
    ],
  })
)(FilterDrawer);
