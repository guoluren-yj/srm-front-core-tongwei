...
import { DataSet, Table, Modal, Attachment, Button } from 'choerodon-ui/pro';
...
const customizeUnitCodes = {
  basic: 'SSTA.SUPPLIER_INVOICE_POOL_DETAIL.BASIC',
  purchase: 'SSTA.SUPPLIER_INVOICE_POOL_DETAIL.PURCHASE_INFO',
  supply: 'SSTA.SUPPLIER_INVOICE_POOL_DETAIL.SUPPLY_INFO',
  other: 'SSTA.SUPPLIER_INVOICE_POOL_DETAIL.OTHER_INFO',
  file: 'SSTA.SUPPLIER_INVOICE_POOL_DETAIL.FILE_INFO',
};

const otherCustomizeUnitCodes = {
  line: 'SSTA.SUPPLIER_INVOICE_POOL_DETAIL.LINE_LIST',
  btn: 'SSTA.SUPPLIER_INVOICE_POOL_DETAIL.HEADER_BTNS',
};
...
const Headers = observer(({ handleRecord, props }) => {
  ...
  let url = '/ssta/supply-invoice-pool/list';
  ...
  return (
    <Header title={intl.get('ssta.invoiceSheet.invoiceDetail').d('发票详情')} backPath={url}>
      {customizeBtnGroup(
...
class index extends Component {
  ...
  constructor(props) {
    ...
    this.operationDs = new DataSet(
      operationDS({
        url: `/ssta/v1/${getCurrentOrganizationId()}/invoice-action/${invoiceHeaderId}?size=999`,
        pk: 'invoiceHeaderId',
