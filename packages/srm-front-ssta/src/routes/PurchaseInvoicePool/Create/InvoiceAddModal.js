...
export default observer((props) => {
  ...
  const handleNext = useCallback(async () => {
    ...
    if (res) {
      ...
      // invoiceLineAddDS.setState('headInfo', datas[0]);
      // 头信息保存成功,下一步
      headerAddDS.setState('stepCurrent', stepCurrent + 1);
    }
  ...
  const handleHeadInfo = useCallback(async () => {
    ...
    const data = { ...(headerAddDS.current?.toData() || {}), camp: 'PURCHASER' };
  ...
  const handleSave = useCallback(
    async (source) => {
      if (source === 'head') {
        ...
        invoiceLineAddDS.dataToJSON = 'all';
        const res = await invoiceLineAddDS
          .setState('headInfo', { ...(headerAddDS.current?.toData() || {}), camp: 'PURCHASER' })
          .submit();
        invoiceLineAddDS.dataToJSON = 'dirty';
