...
import { Tabs, Radio, Card, Tag } from 'choerodon-ui';
...
class ExeResult extends Component {
  ...
  viewDetail(record, documentType) {
    ...
    const billHeaderId = record.get('documentId');
    if (documentType === 'BILL') {
      ...
      history.push({
        ...
        search: stringify({
          editFlag: 0,
          billList: JSON.stringify([{ billHeaderId, billNum: documentNum }]),
          from: 'costsheet',
        }),
      });
    } else {
      getSettleHeaderId(documentNum).then((res) => {
        if (res) {
          if (modal) modal.close();
      ...
      BILL: [
        {
          ...
          tooltip: 'overflow',
          renderer: (records) => {
            return this.formatStatusTag(records.value, records.record.get('recordStatus'));
          },
        },
        {
        ...
        },
        {
          ...
          width: 120,
          renderer: ({ record }) =>
            record.get('autoGenerateFlag') === 0 &&
            !['LOCK'].includes(record.get('recordStatus')) && (
              <a onClick={() => this.viewDetail(record, 'BILL')}>
