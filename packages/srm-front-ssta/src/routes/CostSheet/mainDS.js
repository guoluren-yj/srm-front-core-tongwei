  ...
  return {
    ...
    fields: [
      ...
      },
      {
        ...
        type: 'string',
        label: intl.get('ssta.costSheet.model.costSheet.syncStatusMeaning').d('同步ERP状态'),
      },
      {
        name: 'syncResponseMsg',
        type: 'string',
        label: intl.get('ssta.costSheet.model.costSheet.syncResponseMsg').d('反馈信息'),
      },
      {
        name: 'erpChargeNum',
        type: 'string',
        label: intl.get('ssta.costSheet.model.costSheet.erpChargeNum').d('同步ERP单据编号'),
      },
      {
        name: 'operation',
        type: 'string',
    ...
    transport: {
      read: ({ data }) => {
        ...
        data.chargeType = data.chargeType === 'APPROVAL' ? 'PURCHASER_APPROVAL' : data.chargeType;
        return {
          url: `/ssta/v1/${organizationId}/charge-headers?customizeUnitCode=SSTA.COST_SHEET_LIST.GRID,SSTA.COST_SHEET_LIST.SEARCH_BAR`,
          method: 'GET',
          data: {
            ...data,
...
const detailTableDs = () => ({
  ...
  transport: {
    read: ({ data }) => {
      return {
        url: `/ssta/v1/${organizationId}/charge-lines/purchase/detail-line-list?customizeUnitCode=SSTA.COST_SHEET_LIST.GRID_DETAIL,SSTA.COST_SHEET_LIST.SEARCH_BAR_DETAIL`,
        method: 'GET',
        data: {
          ...data,
          ...transformQselectDate(data, { creationDateRange: 'creationDate' }),
