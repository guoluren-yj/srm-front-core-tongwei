...
const MessageRule = () => {
  ...
  const columns: ColumnProps[] = useMemo(() => {
    return [
      ...
      },
      {
        name: 'messagePoint',
        width: 150,
        editor: editorFlag && <Select dropdownMatchSelectWidth={false} popupContent={(popupProps) => getNumberSelectContent({ min: 0, max: 23, widthLength: 6, ...popupProps })} />,
      },
      {
        name: 'frequency',
        width: 150,
        ...
        help: intl.get('ssta.paymentPlan.view.help.msgRemindFrequency').d('请维护整数，提醒频率单位为「天」，多次提醒用英文逗号分割（,），如维护-5，-1，0，2，4则表示在付款计划日期前5天、1天、当前、后2天、4天发送消息提醒'),
      },
      {
        name: 'stageLineNums',
        width: 150,
        editor: (record) => editorFlag && stageLineNumsEditor(record),
        help: intl.get('ssta.paymentPlan.view.help.msgRemindStageLineNums').d('系统将在配置的「消息发送时间」发送消息提醒'),
      },
    ];
  }, [editorFlag]);
  ...
  return (
    <Fragment>
      {customizeTable(
        { code: DetailCustomizeCode.StageMessageCode, readOnly: !editorFlag },
        <Table
          columns={columns}
