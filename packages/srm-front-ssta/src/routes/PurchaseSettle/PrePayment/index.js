...
const permPrefix = `srm.settle-account.jsd.purchase.ps`;

const unitCodes = [
  'SSTA.PURCHASE_SETTLE_DETAIL.PRE_BASIC',
  'SSTA.PURCHASE_SETTLE_DETAIL.PRE_PAYMENT_INFO',
  'SSTA.PURCHASE_SETTLE_DETAIL.PRE_TRADINGPARTY',
  'SSTA.PURCHASE_SETTLE_DETAIL.PRE_TRANSACTIONDETAIL',
  'SSTA.PURCHASE_SETTLE_DETAIL.OTHER_INFO',
...
const Prepayment = props => {
  ...
  const { source, settleHeaderId, type = 'ALL', editablePub } = queryString.parse(
    search.substring(1)
  );
  ...
  const [updateBtn, approveBtn, cancelBtn, syncBtn] = React.useMemo(
    () =>
  ...
  const getSaveSendData = async () => {
    ...
    if (headerValidateFlag && linesValidateFlag) {
      const headerData = headerDS.current.toData() ? headerDS.current.toData() : {};
    ...
    if (sendData) {
      ...
      if (res) {
        if (source === 'create') {
          ...
          lineDS.query(undefined, undefined, false).then(() => {
            setAllLoading(false);
          });
          setAllLoading(false);
        }
  ...
  const requestNewUrl = () => {
    ...
    return `/ssta/v1/${organizationId}/pre-payment-lines/purchaser/export/new?customizeUnitCode=${customizeUnitCode}`;
  };
  ...
  const readOnlyButtons = [
    createPermsMap.get(`${permPrefix}.detail.pay.newexport`) && (
      <ExcelExportPro
        ...
        templateCode="SSTA_SETTLE_HEADER_PREMENT_LINE_PURCHASE_EXPORT"
        otherButtonProps={{
          type: 'c7n-pro',
          funcType: 'flat',
          color: 'primary',
    ...
    return addLines({
      ...
      if (getResponse(res)) {
        ...
        if (source === 'create') {
          ...
          headerDS.setQueryParameter('settleHeaderId', settleHeaderId);
          lineDS.setQueryParameter('settleHeaderId', settleHeaderId);
          getPreHeader(settleHeaderId, unitCodes).then(res1 => {
            if (res1) {
              headerDS.current.set({
                objectVersionNumber: res1.objectVersionNumber,
                ...Object.fromEntries(
  ...
  const detailTabPaneRender = () => {
    ...
    const showAlertFlag = remoteProps
      ? remoteProps.process(
          'SSTA_PURCHASESETTLE_DETAIL_PREPAYMENT_CUX_SHOW_ALERT_FLAG',
          !!tipsText,
          {
            headerDS,
          }
    ...
    return (
      <Spin spinning={allLoading}>
        <Content>
          ...
          {customizeForm(
            {
              code: 'SSTA.PURCHASE_SETTLE_DETAIL.PRE_BASIC',
        ...
        <Content>
          ...
          {customizeForm(
            {
              code: 'SSTA.PURCHASE_SETTLE_DETAIL.PRE_TRADINGPARTY',
              readOnly: readOnlyFlag,
            },
            <Form
              dataSet={headerDS}
            ...
            {customizeTable(
              ...
              <SearchBarTable
                ...
                buttons={
                  remoteProps
                    ? remoteProps?.process(
                        'SSTA_PURCHASESETTLE_DETAIL_PREPAYMENT_CUX_LINE_BTN',
                        tableButtons,
                        {
                          headerDS,
                          updateFlag,
        ...
        <Content>
          ...
          {customizeForm(
            {
              code: 'SSTA.PURCHASE_SETTLE_DETAIL.OTHER_INFO',
              readOnly: readOnlyFlag && !editablePub,
            },
            <Form
              dataSet={headerDS}
              columns={3}
            ...
            >
              ...
              {!['NEW', 'SUBMITED', 'SUBMITED_APPROVING'].includes(settleStatus) &&
                (!['CANCELING', 'CANCEL_APPROVING'].includes(settleStatus) || readOnlyFlag) && (
                  <FormItem
                    name="canceledRemark"
                    newLine
                    colSpan={2}
  ...
  const headerBtns = () => {
    ...
    const btns = remoteProps
      ? remoteProps.process('SSTA_PURCHASESETTLE_DETAIL_PREPAYMENT_OLD_BTNS', allBtns, {
          notPub,
          lineDS,
          headerDS,
        })
...
export default compose(
  ...
  remote({
    code: 'SSTA_PURCHASESETTLE_DETAIL_PREPAYMENT_CUX',
    name: 'remote',
  })
)(Prepayment);
