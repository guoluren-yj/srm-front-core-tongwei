...
import '../PurchaseSettlePool/index.less';

const tenantId = getCurrentOrganizationId();
// 采购方

const prefix = 'ssta.purchaseSettlementKanban';
const uxFlag = findMenuName('srm.settle-account.reconciliation-workbench.ux-purchaser');
...
const SettlementKanban = (props) => {
  ...
  const handleDetail = async (record, keys) => {
    ...
    switch (keys) {
      ...
      default:
        if (uxSettleFlag) {
          history.push({
            pathname: `/ssta/new-purchase-settle/${documentType.toLowerCase()}/${headerId}`,
            search: queryString.stringify({
              source: 'purchaseKanban',
              type: 'view',
            }),
          });
        } else {
  ...
  return (
    <React.Fragment>
      <Header
        title={intl
          .get(`${prefix}.view.message.title.purchaseSettlementKanban`)
          .d('采购方结算看板')}
      >
        <ExcelExport
          otherButtonProps={{ icon: 'export' }}
          requestUrl={`/ssta/v1/${getCurrentOrganizationId()}/settle-report/purchaser/report-page/export`}
      ...
      <Content>
        {customizeTable(
          {
            code: 'SSTA.PURCHASE_SETTLEMENT_KANBAN.LIST_GRID',
          },
          <SearchBarTable
            cacheState
            searchBarRef={(ref) => {
            ...
            searchCode="SSTA.PURCHASE_SETTLEMENT_KANBAN.SEARCH_BAR"
            columns={columns}
            dataSet={tableDS}
            searchBarConfig={{
              fieldProps: {
          ...
          {show && (
            <div>
              ...
              <Tabs activeKey={currentKey} animated onChange={handleTabChange}>
                <TabPane
                  tab={
                    <span title={recordCount?.billRecordCount}>
                ...
                >
                  <div style={{ height: '100%' }}>
                    <div style={{ height: '100%' }}>
                      {customizeTable(
                        {
                          code: 'SSTA.PURCHASE_KANBAN_RECORD.BILL_GRID',
                        },
                ...
                >
                  <div style={{ height: '100%' }}>
                    <div style={{ height: '100%' }}>
                      {customizeTable(
                        {
                          code: 'SSTA.PURCHASE_KANBAN_RECORD.INVOICE_GRID',
                        },
                ...
                >
                  <div style={{ height: '100%' }}>
                    <div style={{ height: '100%' }}>
                      {customizeTable(
                        {
                          code: 'SSTA.PURCHASE_KANBAN_RECORD.PAYMENT_GRID',
                        },
                ...
                >
                  <div style={{ height: '100%' }}>
                    <div style={{ height: '100%' }}>
                      {customizeTable(
                        {
                          code: 'SSTA.PURCHASE_KANBAN_RECORD.PREPAYMENT_GRID',
                        },
...
export default compose(
  formatterCollections({
    code: ['ssta.purchaseSettlementKanban', 'hzero.common', 'hzero.c7nProUI', 'hzero.c7nProU'],
  }),
  withCustomize({
    unitCode: [
      'SSTA.PURCHASE_SETTLEMENT_KANBAN.SEARCH_BAR',
      'SSTA.PURCHASE_SETTLEMENT_KANBAN.LIST_GRID',
      'SSTA.PURCHASE_KANBAN_RECORD.BILL_GRID',
      'SSTA.PURCHASE_KANBAN_RECORD.INVOICE_GRID',
      'SSTA.PURCHASE_KANBAN_RECORD.PAYMENT_GRID',
      'SSTA.PURCHASE_KANBAN_RECORD.PREPAYMENT_GRID',
    ],
  })
)(SettlementKanban);
