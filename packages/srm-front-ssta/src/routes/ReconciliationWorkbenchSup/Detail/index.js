...
import { Spin, Tabs, Card, Popover } from 'choerodon-ui';
...
const permPrefix = 'srm.settle-account.reconciliation-workbench.supplier.ps';
...
export default class Detail extends PureComponent {
  ...
  handleCancelLines = async () => {
    ...
    if (data) {
      this.setState({ loading: true });
      const res = getResponse(await cancelSupplierLines(data));
      this.setState({ loading: false });
      if (res) {
  ...
  handleRoleImport = () => {
    ...
    history.push({
      pathname: '/ssta/reconciliation-workbench-supplier/data-import/SSTA.BILL_LINE_BATCH_UPDATE',
      title: intl
        .get(`ssta.reconciliationWorkbenchSup.view.message.billBatchEdit`)
        .d('对账单批量编辑'),
      search: queryString.stringify({
        action: intl.get('ssta.common.button.batchUpdate').d('批量编辑'),
        backPath: `/ssta/reconciliation-workbench-supplier/detail${location.search}`,
        historyButton: false,
        args: JSON.stringify({
  ...
  handleCancelLoading = async (reqFun, operation) => {
    const sendData = this.formDs.current.toData();
    this.setState({ loading: true });
    const res = getResponse(await reqFun(sendData));
    this.setState({ loading: false });
    if (res) {
  ...
  handleSave = async () => {
    ...
    if (sendData) {
      this.setState({ loading: true });
      const res = getResponse(await saveSupplier(sendData));
      this.setState({ loading: false });
      if (res) {
        notification.success();
        this.handleFetch();
  ...
  handleSubmit = async () => {
    ...
    const validateOk = async () => {
      this.setState({ loading: true });
      const res = getResponse(await submitSupplier(sendData));
      this.setState({ loading: false });
      if (res) {
  ...
  afterSplitAction = (needSplit) => {
    ...
    if (needSplit && isArray(billList) && billList.length > 1) {
      ...
      history.replace({
        pathname: '/ssta/reconciliation-workbench-supplier/detail',
        search: queryString.stringify({
          action,
          editFlag: Number(editFlag),
          billList: JSON.stringify(filterList),
      ...
      // history.push('/ssta/reconciliation-workbench-supplier/list');
      history.push({
        pathname: '/ssta/reconciliation-workbench-supplier/list',
        state: { _back: 1 },
      });
    }
  };
  ...
  titleRender = () => {
    ...
    //   case 'UPDATE':
    //     return intl
    //       .get('ssta.reconciliationWorkbenchSup.view.title.billDetailUpdate')
    //       .d('对账单维护');
    //   case 'APPROVE':
    //     return this.formatHeaderTitle('APPROVE', billStatus, billNum, headerData);
    //   case 'CANCEL':
    //     return intl
    //       .get('ssta.reconciliationWorkbenchSup.view.title.billDetailCancel')
    //       .d('对账单取消');
    //   default:
  ...
  linkListRender = () => {
    return [
      {
        key: 'ReconciliationWorkbenchSup-header',
        title: intl
          .get(`ssta.reconciliationWorkbenchSup.view.message.panel.baseInfos`)
          .d('基本信息'),
      },
      {
        key: 'ReconciliationWorkbenchSup-tradingPartyInformation',
        title: intl
          .get(`ssta.reconciliationWorkbenchSup.view.message.panel.tradingPartyInformation`)
          .d('交易方信息'),
      },
      {
        key: 'ReconciliationWorkbenchSup-transactionAmountInformation',
        title: intl
          .get(`ssta.reconciliationWorkbenchSup.view.message.panel.transactionAmountInformation`)
          .d('交易金额信息'),
      },
      {
        key: 'ReconciliationWorkbenchSup-transactionDetailInformation',
        title: intl
          .get(`ssta.reconciliationWorkbenchSup.view.message.panel.transactionDetails`)
          .d('交易明细信息'),
      },
      {
        key: 'ReconciliationWorkbenchSup-settleConfigInfo',
        title: intl
          .get(`ssta.reconciliationWorkbenchSup.view.message.panel.settleConfigInfo`)
          .d('主策略信息'),
      },
      {
        key: 'ReconciliationWorkbenchSup-othersInf',
        title: intl
          .get(`ssta.reconciliationWorkbenchSup.view.message.panel.othersInf`)
          .d('其他信息'),
      },
      {
        key: 'ReconciliationWorkbenchSup-attachment',
  ...
  detailTabPaneRender = (billHeaderId) => {
    ...
    return (
      <Spin spinning={loading}>
        <Content>
          <h3 className="ssta-form-title" id="ReconciliationWorkbenchSup-header">
            {intl.get(`ssta.reconciliationWorkbenchSup.view.message.panel.baseInfos`).d('基本信息')}
          </h3>
          {customizeForm(
            { code: 'SSTA.SUPPLIER_BILL_DETAIL.BASIC', readOnly: !editFlag },
            <Form
              columns={3}
              useColon={false}
              dataSet={this.formDs}
        ...
        <Content>
          <h3 className="ssta-form-title">
            {intl
              .get(`ssta.reconciliationWorkbenchSup.view.message.panel.tradingInformation`)
              .d('交易信息')}
          </h3>
          <Card
            id="ReconciliationWorkbenchSup-tradingPartyInformation"
            bordered={false}
          ...
          >
            {customizeForm(
              { code: 'SSTA.SUPPLIER_BILL_DETAIL.TRADING_PARTY', readOnly: !editFlag },
              <Form
                dataSet={this.formDs}
                columns={3}
                useColon={false}
          ...
          <Card
            id="ReconciliationWorkbenchSup-transactionAmountInformation"
            bordered={false}
            className={DETAIL_CARD_CLASSNAME}
            title={intl
              .get(
                `ssta.reconciliationWorkbenchSup.view.message.panel.transactionAmountInformation`
              )
              .d('交易金额信息')}
          >
            {customizeForm(
              { code: 'SSTA.SUPPLIER_BILL_DETAIL.TRANSACTION_AMOUNT', readOnly: !editFlag },
              <Form
                columns={3}
                useColon={false}
                dataSet={this.formDs}
          ...
          <Card
            bordered={false}
            id="ReconciliationWorkbenchSup-transactionDetailInformation"
            className={DETAIL_CARD_CLASSNAME}
            title={intl
              .get(`ssta.reconciliationWorkbenchSup.view.message.panel.transactionDetails`)
              .d('交易明细信息')}
        ...
        <Content>
          <h3 className="ssta-form-title" id="ReconciliationWorkbenchSup-settleConfigInfo">
            {intl
              .get(`ssta.reconciliationWorkbenchSup.view.message.panel.settleConfigInfo`)
              .d('主策略信息')}
          </h3>
          {customizeForm(
            { code: 'SSTA.SUPPLIER_BILL_DETAIL.SETTLE_CONFIG', readOnly: !editFlag },
            <Form
              columns={3}
              useColon={false}
              dataSet={this.formDs}
            ...
            >
              ...
              {!editFlag ||
                (['CANCELING', 'CANCEL_APPROVING', 'CONFIRM', 'WAIT_SUPPLIER_CANCEL'].includes(
                  billStatus
                ) && (
        ...
        <Content>
          <h3 className="ssta-form-title" id="ReconciliationWorkbenchSup-othersInf">
            {intl.get(`ssta.reconciliationWorkbenchSup.view.message.panel.othersInf`).d('其他信息')}
          </h3>
          {customizeForm(
            { code: 'SSTA.SUPPLIER_BILL_DETAIL.OTHERS', readOnly: !editFlag },
            <Form
              columns={3}
              useColon={false}
              dataSet={this.formDs}
        ...
        <Content wrapperClassName="ssta-last-page-content-wrapper">
          <h3 className="ssta-form-title" id="ReconciliationWorkbenchSup-attachment">
            {intl.get(`ssta.reconciliationWorkbenchSup.view.message.panel.attachment`).d('附件')}
          </h3>
          {customizeForm(
            {
      ...
      type &&
        ...
        headerData.camp === 'SUPPLIER' && {
          ...
          btnProps: {
            ...
            permissionList: [
              {
                code: 'srm.settle-account.reconciliation-workbench.supplier.ps.radio.button.update',
                type: 'button',
              },
            ],
          },
      ...
      type &&
        ((billStatus === 'SUBMITED' &&
          ...
          btnProps: {
            ...
            permissionList: [
              {
                code: 'srm.settle-account.reconciliation-workbench.supplier.ps.radio.button.audit',
                type: 'button',
              },
            ],
          },
      ...
      type &&
        ...
        !(billCancelType === 'ERP' && syncStatus === 'SYNC_SUCCESS') && {
          ...
          btnProps: {
            ...
            permissionList: [
              {
                code: 'srm.settle-account.reconciliation-workbench.supplier.ps.radio.button.cancel',
                type: 'button',
              },
            ],
          },
    ...
    return (
      <Fragment>
        <Header
          ...
          backPath={
            notPub ? state?.backPath || '/ssta/reconciliation-workbench-supplier/list' : null
          }
          onBack={() => {
        ...
        >
          <HeaderButtons>
            <PermissionBtns>
              {customizeBtnGroup(
                { code: 'SSTA.SUPPLIER_BILL_DETAIL.HEADER_BTNS', pro: true },
        ...
        <div
          ...
          id="ssta-detail-content-ReconciliationWorkbenchSup"
        >
          {!listFlag ? (
          ...
          <FixedAnchor
            ...
            className="ssta-detail-content-ReconciliationWorkbenchSup"
          />
        </div>
      </Fragment>
    );
