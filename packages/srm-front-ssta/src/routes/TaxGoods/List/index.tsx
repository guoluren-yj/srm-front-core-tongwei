...
const ListImportTempCode: Record<ActiveKey, string> = {
  [ActiveKey.Info]: `SDIM_COMMODITY_SUP_IMPORT`,
  [ActiveKey.Mapping]: `SDIM_COMMODITY_MAPPING_SUP_IMPORT`,
};
// 列表页导出组件模板编码
const ListExportTempCode: Record<ActiveKey, string> = {
  [ActiveKey.Info]: `SDIM_COMMODITY_SUP_EXPORT`,
  [ActiveKey.Mapping]: `SDIM_COMMODITY_MAPPING_SUP_EXPORT`,
};
// 列表页导出组件requestUrl
const ListExportUrl: Record<ActiveKey, string> = {
  [ActiveKey.Info]: `${SRM_SSTA}/v1/${tenantId}/direct-commoditys/excel-export`,
  [ActiveKey.Mapping]: `${SRM_SSTA}/v1/${tenantId}/direct-commodity-mappings/excel-export`,
};
...
const List = observer(() => {
  ...
  const allBtns: any = useMemo(() => {
    ...
    return btns;
  }, [loading, handleInitGoodsInfo, activeKey, onOk, tenantId, handleCreateGoodsMapping, selected, exportParams, handleCreateGoodsInfo]);

  return (
    <Fragment>
      <Header title={intl.get('ssta.goodsInfo.view.title.taxGoodsInfo').d('税收商品信息')}>
        {customizeBtnGroup(
          {
      ...
      <Content>
        {customizeTabPane(
          ...
          <Tabs keyboard={false} activeKey={activeKey} onChange={handleTabChange}>
            <TabPane
              key={ActiveKey.Info}
              tab={intl.get('ssta.goodsInfo.view.title.baseTaxGoodsInfo').d('基础税收商品信息')}
