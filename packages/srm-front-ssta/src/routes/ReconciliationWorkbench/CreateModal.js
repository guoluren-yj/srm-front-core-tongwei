...
import { tableDS } from '@/stores/PurchaseSettlePoolDS';
import MultiTextFilter from '../Components/MultiTextFilter';
import {
  createPurchaseBill,
  allCreate,
  getPriceAndcreatePurchaseBill,
} from '../../services/settlePoolServices';

const prefix = 'ssta.supplySettlePool';
const customizeUnitCode =
  'SSTA.PURCHASE_POOL_LIST.BILL_GRID,SSTA.PURCHASE_POOL_LIST.SEARCH_BAR_BILL';
...
const AddModal = (props) => {
  ...
  const isPage = pathname === '/ssta/new-reconciliation-workbench/create-steps';
  ...
  const fetchPermissions = async () => {
    const res = getResponse(
      await getPermissions([
        ...
        `srm.settle-account.reconciliation-workbench.ux-purchaser.ps.allcreate`,
        `srm.settle-account.reconciliation-workbench.ux-purchaser.api.priceAndcreate`,
      ])
    );
    if (res) {
      setPermsMap(res);
  ...
  const handleAdd = async () => {
    if (tableDs.selected.length > 0) {
      ...
      const handleContinueAddAfterValidate = async () => {
        ...
        const res = getResponse(await createPurchaseBill(selectData));
  ...
  const getPriceAndCreate = async () => {
    if (remote?.event) {
      const beforePriceAndCreateBillRes = await remote.event.fireEvent('beforePriceAndCreateBill', {
        tableDs,
      });
      if (beforePriceAndCreateBillRes === false) return false;
    }
    // 前置埋点会被勾选数据做处理
    let selectData = [];
    const queryData = getQueryData() || {};
    ...
    tableDs.status = 'loading';
    const res = getResponse(
      await getPriceAndcreatePurchaseBill(selectData, {
        ...queryData,
        ...transformSupplierData(supplierCompanyId),
      })
    );
  ...
  const columns = React.useMemo(() => {
    const normalColumns = [
      ...
      },
      {
        width: 160,
        name: 'companyName',
        title: intl.get(`${prefix}.model.purchaseSettlePool.reconciliationCompany`).d('对账公司'),
      },
      {
        width: 160,
      ...
      },
      {
        ...
        title: intl.get(`${prefix}.model.purchaseSettlePool.accountSupplier`).d('对账供应商'),
      },
      {
        width: 80,
        name: 'currencyCode',
      ...
      },
      {
        width: 120,
        name: 'quantity',
        title: intl.get(`${prefix}.model.purchaseSettlePool.reconcilableQuantity`).d('可对账数量'),
      },
      {
        width: 150,
        name: 'netPrice',
    ...
    return remote
      ? remote.process('SSTA.PURCHASER_BILL_LIST_CUX.CREATE_COLUMNS', normalColumns, {
          cuxProps,
        })
      : normalColumns;
  }, [cuxProps, remote]);
  ...
  const searchBarConfig = useMemo(() => {
    ...
    return remote
      ? remote.process('SSTA.PURCHASER_BILL_LIST_CUX.CREATE_SEARCH_BAR_CONFIG', normalConfig, {
          cuxProps,
        })
      : normalConfig;
  ...
  const trxTableRender = () => {
    return customizeTable(
      {
        code: 'SSTA.PURCHASE_POOL_LIST.BILL_GRID',
      },
      <SearchBarTable
        searchCode="SSTA.PURCHASE_POOL_LIST.SEARCH_BAR_BILL"
        columns={columns}
        dataSet={tableDs}
        searchBarRef={(ref) => {
  ...
  const handleRoleImport = () => {
    ...
    const url = isNew
      ? '/ssta/new-reconciliation-workbench/data-import-create/'
      : '/ssta/reconciliation-workbench/data-import-create/';
    history.push({
      pathname: `${url}${perCode}`,
      search: queryString.stringify({
        backPath: isNew
          ? `/ssta/new-reconciliation-workbench/list`
          : `/ssta/reconciliation-workbench/list`,
  ...
  const btns = () => {
    return [
      ...
      permsMap.get(`srm.settle-account.reconciliation-workbench.ux-purchaser.ps.allcreate`) && {
        name: 'allCreate',
  ...
  const createContent = (
    <Fragment>
      ...
      <div className="ssta-body-footer">
        {customizeBtnGroup(
          { code: 'SSTA.PURCHASER_BILL_LIST.CREATE.HEADER_BTNS', pro: true },
...
export default compose(
  ...
  withCustomize({
    unitCode: [
      ...
      'SSTA.PURCHASER_BILL_LIST.CREATE.HEADER_BTNS',
    ],
  }),
  observer
)(AddModal);
