...
const prefix = 'ssta.supplySettlementKanban.model.supplySettlementKanban';
  ...
  return {
    ...
    fields: [
      ...
      },
      {
        name: 'companyName',
        type: 'string',
        label: intl.get(`${prefix}.companyName`).d('客户公司'),
      },
      {
        name: 'displaySupplierName',
        type: 'string',
        label: intl.get(`${prefix}.displaySupplierName`).d('供应商公司'),
      },
      {
        name: 'currencyCode',
        type: 'string',
    ...
    transport: {
      read: ({ data, dataSet: { reParams } }) => {
        ...
        return {
          url: `/ssta/v1/${organizationId}/settle-report/supplier/report-page?customizeUnitCode=SSTA.SUPPLY_SETTLEMENT_KANBAN.SEARCH_BAR,SSTA.SUPPLY_SETTLEMENT_KANBAN.GRID`,
          method: 'GET',
...
export const reconDs = () => {
  return {
    ...
    fields: [
      {
        ...
        type: 'string',
        label: intl.get(`${prefix}.documentAndLineNums`).d('对账单编号|行号'),
      },
      {
        name: 'settleNum',
        type: 'string',
    ...
    transport: {
      read: ({ data }) => {
        return {
          url: `/ssta/v1/${organizationId}/settle-report/supplier/bill/report-page`,
          method: 'GET',
          data: { ...data.val },
        };
      },
...
export const invoiceDs = () => {
  return {
    ...
    transport: {
      read: ({ data }) => {
        return {
          url: `/ssta/v1/${organizationId}/settle-report/supplier/invoice/report-page`,
          method: 'GET',
          data: { ...data.val },
        };
      },
  ...
  return {
    ...
    fields: [
      ...
      },
      {
        ...
        type: 'string',
        label: intl.get(`${prefix}.paymentTypes`).d('付款类型'),
      },
      {
        name: 'paymentAmount',
        type: 'string',
        label: intl.get(`${prefix}.paymentAmounts`).d('付款金额'),
      },
      {
        name: 'recordStatusMeaning',
        type: 'string',
        label: intl.get(`${prefix}.recordStatuss`).d('付款状态'),
      },
      {
        name: 'recordDate',
        type: 'string',
        label: intl.get(`${prefix}.recordDates`).d('付款日期'),
      },
      {
        name: 'recordSource',
        type: 'string',
        label: intl.get(`${prefix}.recordSources`).d('付款来源'),
      },
      {
        name: 'companyName',
        type: 'string',
    ...
    transport: {
      read: ({ data }) => {
        return {
          url: `/ssta/v1/${organizationId}/settle-report/supplier/payment/report-page`,
          method: 'GET',
          data: { ...data.val },
        };
      },
...
export const prepaymentDs = () => {
  return {
    ...
    fields: [
      {
        name: 'documentAndLineNum',
        type: 'string',
        label: intl.get(`${prefix}.prepaymentDocumentAndLineNums`).d('预付款结算单编号|行号'),
      },
      {
    ...
    transport: {
      read: ({ data }) => {
        return {
          url: `/ssta/v1/${organizationId}/settle-report/supplier/prepayment/report-page`,
          method: 'GET',
          data: { ...data.val },
        };
      },
