...
import { prePaymentHeaderDS as headerDs } from '../../../stores/SupplySettleDS';
...
const FilledInfoModal = (props) => {
  ...
  return customizeForm(
    ...
    <Form dataSet={filledInfoDS} useColon={false} columns={1} labelLayout="float">
      ...
      {approvalFlag && settleStatus === 'SUBMITED' && <TextArea name="approvedRemark" />}
      {approvalFlag && settleStatus === 'CANCELING' && <TextArea name="canceledRemark" />}
      {approvalFlag && settleStatus === 'WAIT_SUPPLIER_CONFIRM' && (
        <TextArea name="supplierApprovedRemark" />
      )}
      {approvalFlag && settleStatus === 'WAIT_SUPPLIER_CANCEL' && (
        <TextArea name="supplierCanceledRemark" />
      )}
    </Form>
  );
};
