...
import { taxDS as taxDs } from '../../../stores/SupplySettleDS';
import TaxDetailModal from './TaxDetailModal';
import InvoicePoolDetailModal from './InvoicePoolDetailModal';
import { choseInvoicePoolDs } from '../../SupplyInvoicePool/mainDS';
...
const TaxModal = (props) => {
  ...
  const permPrefix = 'srm.settle-account.jsd.supply.ps.deatil.taxline.button';
  const {
    updateFlag,
    approveFlag,
    onCancel,
  ...
  const fetchPermissions = async () => {
    const res = getResponse(
      await getPermissions([
        ...
        `${permPrefix}.ocrread`,
        `${permPrefix}.excel`,
        'srm.settle-account.jsd.supply.ps.newexcel',
        `${permPrefix}.chosepool`,
        `${permPrefix}.ofd`,
      ])
    );
    if (res) {
  ...
  const checkSave = (record) => {
    checkList({ ...record.toData() }).then((res) => {
      if (res) {
        if (res.failed) {
          notification.error({
            message: res.message,
  ...
  const updateDetail = (record) => {
    ...
    const editModel = Modal.open({
      ...
      children: customizeForm(
        {
          code: 'SSTA.SUPPLY_SETTLE_DETAIL.TAX_INVOICE_EDIT_OLD',
        },
        <Form dataSet={newDS} labelLayout="float">
          ...
          <NumberField name="netAmount" renderer={amountLocalRender} />
          <NumberField name="taxAmount" renderer={amountLocalRender} />
      ...
      onOk: async () => {
        ...
        if (validateFlag) {
          ...
          const res = await newTax(settleHeaderId, data, { source: 'sup' });
          if (!res) return;
          if (res?.failed) {
            notification.error({
              message: res.message,
  ...
  const poolColumns = [
    ...
    },
    {
      ...
      renderer: ({ record }) => {
        const uploadProps = {
          btnText: intl.get(`entity.attachment.uploads`).d('附件上传'),
          showFilesNumber: false,
          attachmentUUID: record.get('attachmentUuid') || uuidv4(),
        ...
        const oprCom = {
          update: {
            opr: (
              <a style={{ marginRight: '5px' }} onClick={() => updateDetail(record)}>
                {intl.get('ssta.invoiceSheet.view.button.editInfo').d('修改信息')}
              </a>
            ),
            more: (
              <Menu.Item key="updateDetail">
                {intl.get('ssta.invoiceSheet.view.button.editInfo').d('修改信息')}
              </Menu.Item>
            ),
          },
          ...
          approve: {
            opr: (
              <a onClick={() => checkSave(record)} style={{ marginRight: '5px' }}>
                {intl.get('ssta.invoiceSheet.view.button.checkoutInfo').d('查验补全')}
              </a>
            ),
            more: (
              <Menu.Item key="approveRecord">
                <Upload {...uploadProps} />
              </Menu.Item>
            ),
          },
  ...
  const handleAdd = () => {
    ...
    const newModel = Modal.open({
      ...
      children: customizeForm(
        { code: 'SSTA.SUPPLY_SETTLE_DETAIL.TAX_INVOICE_ADD_OLD' },
        <Form dataSet={newDS} labelLayout="float">
          <Select name="invoiceSpecies" />
          <TextField name="invoiceCode" />
          <TextField name="invoiceNumber" />
  ...
  const handleExcle = () => {
    onCancel();
    history.push({
      pathname: `/ssta/supply-settle/data-import/SSTA.TAX_INVOICE_HEADER`,
      search: querystring.stringify({
        backPath: `/ssta/supply-settle/detail${location.search}`,
        action: intl.get('ssta.invoiceSheet.view.button.excelIns').d('Excel导入'),
        historyButton: false,
        args: JSON.stringify({
      ...
      <Dropdown
        overlay={
          <Menu>
            ...
            {permsMap.get(`${permPrefix}.ocrread`) && (
              <Menu.Item onClick={handleModalVisible} name="ocr">
                {intl.get('hzero.common.button.addocr').d('OCR识别')}
              </Menu.Item>
            )}
            {permsMap.get(`${permPrefix}.ofd`) && (
            ...
            {(permsMap.get(`srm.settle-account.jsd.supply.ps.newexcel`) || !notPub) && (
              <CommonImport
            ...
            {permsMap.get(`${permPrefix}.chosepool`) && (
  ...
  const handleUpload = async () => {
    ...
    if (picturesWallRef && picturesWallRef.uploadChild) {
      ...
      if (isEmpty(fileList)) {
        ...
        // OCRCheckSettle(settleHeaderId, list).then(res => {
        if (getResponse(res)) {
          const { errorMessageMap } = res;
          // if (JSON.stringify(res.errorMessageMap) === '{}') {
  ...
  const attachmentModalProps = {
    ...
    title: [
      <>
        <span>
          ...
          {btnType === 'OCR'
            ? intl
                .get('ssta.invoiceSheet.verify.newSstaOcrUpload')
    ...
    footer: [
      ...
      >
        {btnType === 'OCR'
          ? intl.get(`ssta.invoiceSheet.view.button.invoiceBill.ocrDistinguish`).d('OCR识别')
          : intl.get(`ssta.common.view.button.ofdAnalysis`).d('OFD解析')}
      </Button>,
    ],
  };
  ...
  return (
    <Fragment>
      <Spin spinning={checkLoading}>
        ...
        />
        {customizeTable(
          { code: 'SSTA.SUPPLY_SETTLE_DETAIL.TAXINVOICE' },
          <Table
            columns={columns}
            dataSet={taxDS}
            buttons={buttons}
            selectionMode={
              (invoiceMatch === 'OFFLINE_INVOICE' && updateFlag) || approveFlag ? 'rowbox' : 'none'
            }
          />
        )}
        {invoiceMatch === 'OFFLINE_INVOICE' && updateFlag && (
          <div className="ssta-body-footer">
            <Button
              ...
              color="primary"
              // disabled={isEmpty(taxDS.records)}
            >
              {intl.get('hzero.common.button.confirm').d('确认')}
            </Button>
...
export default compose(
  withCustomize({
    unitCode: [
      'SSTA.SUPPLY_SETTLE_DETAIL.TAXINVOICE',
      'SSTA.SUPPLY_SETTLE_DETAIL.TAX_INVOICE_ADD_OLD',
      'SSTA.SUPPLY_SETTLE_DETAIL.TAX_INVOICE_EDIT_OLD',
    ],
  }),
  observer
)(TaxModal);
