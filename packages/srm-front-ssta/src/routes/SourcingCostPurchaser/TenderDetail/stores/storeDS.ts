  ...
  return {
    ...
    fields: [
      ...
      },
      {
        ...
        label: intl.get('ssta.sourcingCost.model.sourcingCost.tenderFileDownloadableFlag').d('招标文件可下载标识'),
        lookupCode: 'HPFM.FLAG',
      },
      {
        name: 'supplierParticipationFlag',
        label: intl.get('ssta.sourcingCost.model.sourcingCost.supplierCanParticipateFlag').d('供应商可参与标识'),
        lookupCode: 'HPFM.FLAG',
      },
    ],
...
export const payRecordDS = (workflowBatch): DataSetProps => {
  return {
    pageSize: 20,
    autoQuery: false,
    selection: false,
    ...
    queryParameter: { workflowBatch, customizeUnitCode: TenderDetailGridUnitCode.PAY },
    transport: {
      read: ({ data }) => {
