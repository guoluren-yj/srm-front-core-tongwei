...
export const syncRecordDS = (feeRecord: DSRecord | null | undefined, docType: DocType): DataSetProps => {
  ...
  return {
    ...
    transport: {
      ...
      submit: ({ data, dataSet }) => {
        const bindCurrent = dataSet?.parent?.current;
        const reSyncUrlMap = {
          deposit: `${apiPrefix}/deposits/deposit-return-supplier-export`,
          service: `${apiPrefix}/server-feess/server-fees-export`,
        };
        const lineNameMap = {
          deposit: 'depositSyncRecordList',
          service: 'serverSyncRecordList',
        };
        return {
          url: reSyncUrlMap[docType],
          method: 'POST',
          data: {
            ...(bindCurrent?.toData() || feeRecordData),
            [lineNameMap[docType]]: data,
          },
        };
      },
    },
