...
export const depositHeaderDS = (depositId: string, customizeUnitCode: string): DataSetProps => {
  return {
    paging: false,
    autoQuery: false,
    forceValidate: true,
    ...
    fields: [
      ...
      },
      {
        ...
        type: FieldType.string,
        label: intl.get('ssta.sourcingCost.model.sourcingCost.sourceDocumentInfo').d('关联寻源单信息'),
      },
      {
        name: 'creationDate',
        type: FieldType.date,
    ...
    queryParameter: { depositId, customizeUnitCode },
    transport: {
      read: () => {
        return {
          url: `${apiPrefix}/deposits/workflow/detail`,
          method: 'GET',
        };
      },
    },
...
// 缴费记录
export const payRecordDS = (customizeUnitCode: string): DataSetProps => {
  return {
    pageSize: 20,
    autoQuery: false,
    selection: false,
...
// 转出记录
export const transferOutDS = (customizeUnitCode: string): DataSetProps => {
  return {
    pageSize: 20,
    autoQuery: false,
    selection: false,
