...
import { pcPending, poPending } from '@/services/settlePoolServices';
// import DetailDrawer from './DetailDrawer';
import { tableDs2, tableDs3, tableDs4, tableDs5 } from './stores/addDS';

const buttonPermPrefix = `srm.settle-account.jsd.ux-supply.button`;

const barCodes = {
  ORDER: 'SSTA.SUPPLY_SETTLE_DETAIL.SEARCH_ADD_PRE_ORDER',
  CONTRACT: 'SSTA.SUPPLY_SETTLE_DETAIL.SEARCH_ADD_PRE_CONTRACT',
  PO_LINE: 'SSTA.SUPPLY_SETTLE_DETAIL.SEARCH_ADD_PRE_POLINE',
  CONTRACT_STAGE: 'SSTA.SUPPLY_SETTLE_DETAIL.SEARCH_PRE_CONSTAGE',
  CONTRACT_SUBJECT: 'SSTA.SUPPLY_SETTLE_DETAIL.SEARCH_PRE_CONSUBJECT',
};

const listCodes = {
  PO_LINE: 'SSTA.SUPPLY_SETTLE_DETAIL.PRE_ADD_ORDER_LINE',
  ORDER: 'SSTA.SUPPLY_SETTLE_DETAIL.PRE_ADD_ORDER',
  CONTRACT: 'SSTA.SUPPLY_SETTLE_DETAIL.PRE_ADD_CONTRACT',
  CONTRACT_STAGE: 'SSTA.SUPPLY_SETTLE_DETAIL.PRE_ADD_CONTRACT_STAGE',
  CONTRACT_SUBJECT: 'SSTA.SUPPLY_SETTLE_DETAIL.PRE_ADD_CONTRACT_SUBJECT',
};

const AddModal = (props) => {
  const { addLine, headerDs, customizeTable, modal, createPermsMap } = props;
  ...
  const tableDS = React.useMemo(() => new DataSet(tbds[prepaymentType]()), [prepaymentType]);
  // 暂挂查询条件
  const pendingFlag = tableDS?.queryDataSet?.current?.get('pendingFlag');
  const pcTypeFlag = React.useMemo(() => {
  ...
  const handleHodle = React.useCallback(async () => {
    ...
    const requestFunc = pcTypeFlag ? pcPending : poPending;
  ...
  }, [pendingFlag, prepaymentType, tableDS, pcTypeFlag]);
  ...
  return (
    <>
      ...
      <div className="ssta-body-footer">
        ...
        {createPermsMap?.get(`${buttonPermPrefix}.pre-source-hold`) && pendingFlag === '0' && (
          <Button disabled={isEmpty(tableDS.selected)} onClick={handleHodle}>
            {intl.get('ssta.prePayment.view.button.suspend').d('暂挂')}
          </Button>
        )}
        {createPermsMap?.get(`${buttonPermPrefix}.pre-source-unhold`) && pendingFlag === '1' && (
          <Button disabled={isEmpty(tableDS.selected)} onClick={handleHodle}>
...
export default compose(
  withCustomize({
    unitCode: [
      'SSTA.SUPPLY_SETTLE_DETAIL.PRE_ADD_ORDER_LINE',
      'SSTA.SUPPLY_SETTLE_DETAIL.PRE_ADD_ORDER',
      'SSTA.SUPPLY_SETTLE_DETAIL.SEARCH_ADD_PRE_ORDER',
      'SSTA.SUPPLY_SETTLE_DETAIL.SEARCH_ADD_PRE_CONTRACT',
      'SSTA.SUPPLY_SETTLE_DETAIL.SEARCH_ADD_PRE_POLINE',
      'SSTA.SUPPLY_SETTLE_DETAIL.SEARCH_PRE_CONSTAGE',
      'SSTA.SUPPLY_SETTLE_DETAIL.SEARCH_PRE_CONSUBJECT',
      'SSTA.SUPPLY_SETTLE_DETAIL.PRE_ADD_CONTRACT',
      'SSTA.SUPPLY_SETTLE_DETAIL.PRE_ADD_CONTRACT_STAGE',
      'SSTA.SUPPLY_SETTLE_DETAIL.PRE_ADD_CONTRACT_SUBJECT',
    ],
  }),
  observer
)(AddModal);
