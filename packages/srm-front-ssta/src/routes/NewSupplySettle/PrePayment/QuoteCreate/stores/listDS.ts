  ...
  return {
    ...
    fields: [
      ...
      },
      {
        ...
        label: intl.get('ssta.prePayment.model.prePayment.preCollectionInitiatedAmount').d('预收款已发起金额'),
        computedProps: { formatterOptions: amountFormatterOptions },
      },
      {
        name: 'prepaymentOccupiedAmount',
        type: FieldType.number,
        label: intl.get('ssta.prePayment.model.prePayment.preCollectionOccupiedAmount').d('预收款已占用金额'),
        computedProps: { formatterOptions: amountFormatterOptions },
      },
      {
        name: 'launchPrepaymentCompleteAmount',
        type: FieldType.number,
        label: intl.get('ssta.prePayment.model.prePayment.preCollectionCompletedAmount').d('预收款已完成金额'),
        computedProps: { formatterOptions: amountFormatterOptions },
      },
      {
    ...
    transport: {
      ...
      submit: ({ dataSet }): any => {
        ...
        if (submitType === 'hodle') {
          ...
          return {
            url: `${apiPrefix}/pre-pay-headers/supplier/validate/create/new`,
            method: 'PUT',
            params: queryParameter,
          };
        } else if (submitType === 'create') {
          return {
            url: `${apiPrefix}/pre-pay-headers/supplier/create/new`,
            method: 'PUT',
            params: queryParameter,
          };
        }
