...
const flowBasicCardCodes = {
  INVOICE: 'SSTA.SUPPLY_SETTLE_DETAIL.INV_FLOW_BASIC_CARD',
  PAYMENT: 'SSTA.SUPPLY_SETTLE_DETAIL.PAY_FLOW_BASIC_CARD',
};

const flowExtraCardCodes = {
  INVOICE: 'SSTA.SUPPLY_SETTLE_DETAIL.INV_FLOW_EXTRA_CARD',
  PAYMENT: 'SSTA.SUPPLY_SETTLE_DETAIL.PAY_FLOW_EXTRA_CARD',
};
...
const WorkflowCard = (props) => {
  ...
  const contentRemainRender = useCallback(() => {
    const label = settleType === 'INVOICE'
      ...
      : intl.get('ssta.common.model.settle.thisColTotalAmountInTax').d('本次收款含税总金额');
