...
import { taxLineRecordDS } from '@/stores/NewSupplySettleDS';
          ...
          {operateData?.length > 0 &&
            operateData.map((t) => {
              if (t) {
                return (
                  <Item color="#E5E5E5">
                    <Icon type={iconMap[t.processStatus] || ''} className="small-icon" />
                    <div className="process-user">
                      ...
                      <span className="result">
                        ...
                        {['CREATE_LINE', 'DELETE_LINE', 'UPDATE_LINE'].includes(t.processStatus)
                          ? intl.get(`ssta.supplySettle.view.title.taxInvoiceLine`).d('税务发票行')
                          : intl.get(`ssta.supplySettle.view.title.taxInvoice`).d('税务发票')}
                        】
                      </span>
                    </div>
                    {t.processRemark && (
