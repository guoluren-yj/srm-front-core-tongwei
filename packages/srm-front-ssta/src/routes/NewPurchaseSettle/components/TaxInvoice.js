...
import BatchModifyInvoiceModal from './BatchModifyInvoiceModal';

const customizeUnitCodeMap = {
  list: 'SSTA.PURCHASE_SETTLE_DETAIL.TAXINVOICE',
  btn: 'SSTA.PURCHASE_SETTLE_DETAIL.TAXINVOICE_BTNS',
};
...
export default observer(({ source }) => {
  ...
  const handleOcr = useCallback(() => {
    ...
    const ocrChildrenPropsNew = remoteProps
      ? remoteProps.process(
          'SSTA_PURCHASESETTLE_DETAIL.TAXINVOICE_OCR_CHILDREN',
          ocrChildrenProps,
          { size }
        )
      : ocrChildrenProps;
  ...
  const handleOfd = useCallback(() => {
    Modal.open({
      ...
      footer: (okBtn, cancelBtn) => {
        ...
        // 星巴克二开埋点pur-17501
        return remoteProps
          ? remoteProps.process('SSTA_PURCHASESETTLE_DETAIL_TAXINVOICE_OFD_BTN', footerBtns)
          : footerBtns;
      },
    });
  ...
  const handleGain = useCallback(async () => {
    ...
    taxInvoiceDs.query();
    handleLoadHeader();
  }, [settleHeaderId, handleLoadHeader, taxInvoiceDs]);

  const handleCancelInvoice = useCallback(async () => {
    const res = getResponse(await onlyCancelDirectInvoice(settleHeaderId));
    if (!res) return;
    notification.success();
    taxInvoiceDs.query();
    handleLoadHeader();
  ...
  const handleViewLogistics = useCallback(() => {
    modalOpen({
      ...
      editFlag: false,
      destroyOnClose: true,
      children: <LogisticModal settleHeaderId={settleHeaderId} />,
          ...
          return (
            <PermissionDropdown
              ...
              dataSource={
                remoteProps
                  ? remoteProps.process(
                      'SSTA_PURCHASESETTLE_DETAIL_TAXINVOICE_CUX.COLUMN_BTNS',
                      dataSource,
                      {
                        record,
                      }
  ...
  // const handleToDirInvApply = useCallback(async(value) => {
  //   settleHeaderDs.status = 'loading';
  ...
  const handleBatchModify = useCallback(() => {
    modalOpen({
      ...
      children: <BatchModifyInvoiceModal closeCallback={handleAfterCloseExcel} />,
    });
  ...
  const buttons = useMemo(() => {
        ...
        ? [
          <Button
            ...
            disabled={
                remoteProps
                  ? remoteProps.process(
                      'SSTA_PURCHASESETTLE_DETAIL_TAXINVOICE_CUX.CHECK_INVOICE_DISABLE',
                      isEmpty(selected),
                      {
                        selected,
                        settleHeaderDs,
    ...
    const EditBtns = updateFlag
      ? [
          ...
          directInvoicingType === 'INVOICE_PLATFORM' &&
            ['INVOICE_SUCCESS', 'CANCELED_PART_SUCCESS', 'CANCEL_FAILED'].includes(
              sdimApplyStatus
            ) && (
              <Button name="cancelInvoice" onClick={handleCancelInvoice} icon="cancel">
                {intl.get('ssta.common.view.button.cancelInvoice').d('发票作废/红冲')}
              </Button>
            ),
        ]
          .concat(CheckBtn)
    ...
    const btns = [
      ...
      permissionMap.get('taxInvNewExportLineBtn') && (
        <ExcelExportPro
          name="newLineExport"
          templateCode="SSTA_TAX_INVOICE_HEADER_PURCHASE_EXPORT"
          method="POST"
          allBody
          requestUrl={`${prefix}/tax-invoice-headers/export/${settleHeaderId}?customizeUnitCode=${customizeUnitCodeMap.list}`}
      ...
      permissionMap.get('taxInvLogisticsView') &&
        ['DIRECT_INVOICING'].includes(invoiceMatchRuleCode) &&
        ['EC'].includes(directInvoicingType) &&
        !['NEW', 'INVOICE_FAILED', 'CANCEL', 'INVOICE_REFUNDED'].includes(settleStatus) && (
          <Button name="logisticsView" onClick={handleViewLogistics} icon="visibility-o">
            {intl.get('ssta.common.button.invoiceLogisticsView').d('发票物流查看')}
          </Button>
        ),
      (!notPub || isEditPub) && (
        <Button
    ...
    return remoteProps
      ? remoteProps.process('SSTA_PURCHASESETTLE_DETAIL_TAXINVOICE_CUX.BTNS', btns, {
          settleHeaderDs,
          taxInvoiceDs,
          handleOcr,
          updateFlag,
