...
import { choseInvoicePoolDS } from '@/stores/NewPurchaseSettleDS';

export default observer((props) => {
  const { modal, onSuccess } = props;
  const c7nModal = useModal();
  const modalOpen = useModalOpen(c7nModal);
  const { settleHeaderDs, settleHeaderId, customizeTable, remoteProps, taxInvoiceDs } = useContext(
    Store
  );
  ...
  const supplierCompanyIdParams = useMemo(() => {
    return remoteProps
      ? remoteProps.process(
          'SSTA_PURCHASESETTLE_DETAIL_CHOSINVPOOL_SUPPLIERCOMPANYID',
          {},
          { settleHeaderDs }
        )
      : {};
  ...
  useEffect(() => {
    modal.update({
      ...
      footer: (okBtn, cancelBtn) => {
        return remoteProps
          ? remoteProps.process(
              'SSTA_PURCHASESETTLE_DETAIL_CHOSINVPOOL_FOOTER_BTNS',
              [okBtn, cancelBtn],
              {
                settleHeaderDs,
                okBtn,
  ...
  }, [modal, selected, remoteProps, settleHeaderDs, tableDs, taxInvoiceDs]);
  useEffect(() => {
  ...
  return customizeTable(
    {
      code: 'SSTA.PURCHASE_SETTLE_DETAIL.INV_TAX_POOL_GRID',
    },
    <SearchBarTable
      dataSet={tableDs}
      columns={columns}
