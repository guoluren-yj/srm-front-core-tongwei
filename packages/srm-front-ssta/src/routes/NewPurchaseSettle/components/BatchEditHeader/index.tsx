...
export const payBatchEditCode: BatchEditCode = {
  SELECT: 'SSTA.PAY_SETTLE_PUR.BATCH_EDIT_SELECT',
  EDIT: 'SSTA.PAY_SETTLE_PUR.BATCH_EDIT_EDIT',
};

export const preBatchEditCode: BatchEditCode = {
  SELECT: 'SSTA.PRE_SETTLE_PUR.BATCH_EDIT_SELECT',
  EDIT: 'SSTA.PRE_SETTLE_PUR.BATCH_EDIT_EDIT',
};
...
const BatchEditHeader = flow(
  ...
  return (
    <div className={styles['settle-header-batch-edit']}>
      <DynamicAlert
        placement='modal-top'
        message={intl.get('ssta.purchaseSettle.view.message.batchEditDocument').d('可通过该功能批量更新单据信息，仅当维护字段值时更新对应单据字段，维护空值不覆盖原值')}
      />
      <div className={styles['batch-edit-steps-bar']}>
        <Steps size='small' current={stepCurrent}>
          <Step title={intl.get('ssta.purchaseSettle.view.title.selectSettleDoc').d('选择结算单')} />
          <Step title={intl.get('ssta.purchaseSettle.view.title.batchEdit').d('批量编辑')} />
        </Steps>
      </div>
