...
const AddModal = (props) => {
  ...
  const handleAdd = async() => {
    if (tableDS.selected) {
      if (remoteProps && remoteProps.event) {
        // 埋点处理
        const beforeValidateRes = await remoteProps.event.fireEvent('onLoadAddLineValidateCux', { tableDS, prepaymentType });
  ...
  const processBarProps = remoteProps
    ? remoteProps.process('SSTA_PURCHASESETTLE_DETAIL_PRE.LINE_ADD_BAR_PROPS', {}, {})
    : {};
        ...
        {customizeTable(
          ...
          <SearchBarTable
            ...
            searchBarConfig={{
              ...
              onLoad: (queryDs) => {
                if (selfRemote) {
                  selfRemote.process('SSTA_PREPAYMENT_ADD_CUX_REF', queryDs, {
                    headerData,
                  });
                }
              },
