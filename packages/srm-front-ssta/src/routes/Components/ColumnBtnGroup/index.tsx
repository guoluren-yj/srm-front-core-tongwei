...
import { Button, Dropdown, Menu, Icon, Tooltip } from 'choerodon-ui/pro';
...
interface ColumnBtnProps
{
  ...
  text?: ReactNode,
  wait?: number,
  tooltip?: string | ReactNode | any,
  group?: boolean,
  disabled?: boolean, // 按钮置灰逻辑
...
const ColumnBtnGroup = memo((props: ColumnBtnGroupProps) =>
{
  ...
  return (
    <div className={styles['ssta-columnBtnGroup-wrapper']}>
      {btnList.map(item =>
      {
        const { name, text, wait, onClick, btnComp, disabled, tooltip } = item;
        return btnComp ? cloneElement(btnComp, { key: name }) : (
          tooltip ? <Tooltip title={tooltip} key={name}>
          <Button key={name} funcType={FuncType.link} color={ButtonColor.primary} wait={wait} onClick={onClick} disabled={disabled}>
            {text}
          </Button>
          </Tooltip> : <Button key={name} funcType={FuncType.link} color={ButtonColor.primary} wait={wait} onClick={onClick} disabled={disabled}>
            {text}
          </Button>
        );
      })}
      {!isEmpty(menuItemList) && (
        <Dropdown
          overlay={() => (
            <Menu className={styles['ssta-columnBtnGroup-menu']}>
              {menuItemList.map(item =>
              {
                const { name, text, onClick, btnComp, disabled, tooltip } = item;
                return (
        ...
        >
          <Button funcType={FuncType.link} color={ButtonColor.primary}>
            ...
            <Icon type="expand_more" className={styles['ssta-columnBtnGroup-more-icon']} />
          </Button>
        </Dropdown>
      )}
    </div>
