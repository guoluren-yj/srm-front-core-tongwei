...
import { previewPdf, getResponse } from '../../../utils/utils';
...
const DynamicAlert = flow(
  ...
  const handlePreviewFile = useCallback(() => {
    previewPdf(fileUrl, 'public-bucket');
  }, [fileUrl]);
    ...
    if (fileUrl) {
      arr.push(
        <Tooltip
          ...
          placement="top"
          title={intl.get('ssta.common.view.button.DownloadAttachments').d('下载附件')}
        >
          <Icon
            type="attach_file"
            className={styles['ssta-alert-attach-file']}
  ...
  return text ? (
    <div ref={wrapperRef} className={styles[`ssta-alert-wrapper`]}>
      <Alert
        closable
        showIcon
        message={message || dynamicText}
        className={classNames(styles[`ssta-alert-${type}`], styles[`ssta-alert-${placement}`])}
