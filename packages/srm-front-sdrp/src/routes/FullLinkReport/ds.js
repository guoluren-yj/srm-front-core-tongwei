...
export default function ReportDs({ customizeUnitCodes }) {
  return {
    ...
    fields: [
      ...
      {
        label: intl.get('sdrp.fullLink.model.approvedDate').d('采购申请审批时间'),
        name: 'approvedDate',
      },
      {
      ...
      {
        label: intl.get('sdrp.fullLink.model.invoiceCurrencyCode').d('币种'),
      ...
      {
        label: intl
          .get('sdrp.fullLink.model.lastExternalConfirmProcessDate')
          .d('付款终次外部系统审批确认时间'),
        name: 'lastExternalConfirmProcessDate',
      },
    ],
    transport: {
      read: ({ params }) => {
