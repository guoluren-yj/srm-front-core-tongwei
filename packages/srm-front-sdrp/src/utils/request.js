...
export default function request(url, options, customOptions = {}) {
  ...
  const accessToken = getAccessToken() ? getAccessToken() : localStorage.getItem('access_token');
    ...
    .then((response) => {
      ...
      if (responseIntercept) {
        if (typeof responseIntercept === 'function') {
          responseIntercept(newUrl, response);
        }
      }
  ...
  fetchChain = fetchChain.catch(catchNormalError).catch((e) => {
    ...
    const responseUrl = e.response?.url || '';
    ...
    // if(/audit-documents/page-route)
    if (data && data.detailsMessage && !responseUrl.includes('/audit-documents/page-route')) {
      notification.error({
        icon: <></>,
        message: (
          <>
            ...
            <div className="ant-notification-notice-message-content">
              {intl.get(`hzero.common.requestNotification.${status}`) || data.detailsMessage}
            </div>
          </>
        ),
        className: 'request error',
      });
    } else if (!responseUrl.includes('/audit-documents/page-route')) {
      notification.error({
        icon: <></>,
        message: (
          <>
