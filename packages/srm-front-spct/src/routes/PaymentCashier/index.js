...
function PaymentCashier(props) {
  ...
  const { paymentOrderNum, payBusinessType, cashierConfigSource, companyCode } = qs.parse(
  ...
  function fetchPaymentMethod() {
    if (payBusinessType === 'SUPPLIER_PAY') {
      ...
      fetchPaymentData(filterNullValueObject({ enabledFlag: 1, cashierFlag: 1, companyCode })).then(
        (res) => {
          if (res && !res.failed) {
            const result = getResponse(res);
            setPaymentList(result?.content);
    ...
    <div className={styles['cashier-content']}>
      <div className="cashier-header">
        <div className="cashier-title">
          <img
            src={configData?.fileDTOS?.[0]?.fileUrl}
