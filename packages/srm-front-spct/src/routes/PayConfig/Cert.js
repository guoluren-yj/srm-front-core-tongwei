...
const uploadApi = `${API_HOST}${SRM_SPCT}/v1/${getCurrentOrganizationId()}/configs/upload-cert`;
    ...
    switch (file.status) {
      case 'error':
        notification.warning({
          message: intl.get('spct.payConfig.view.message.uploadError').d('上传失败'),
        });
        break;
      case 'done':
              ...
              <Row>
                  ...
                  {pcName ? (
                    <>
                      <span style={{ width: 80, display: 'inline-block' }}>
                        {intl.get('spct.payConfig.model.payConfig.content').d('证书内容')}：
                      </span>
                    ...
                    <FormItem
                      ...
                      label={intl.get('spct.payConfig.model.payConfig.content').d('证书内容')}
                    >
                ...
                <Col>
                  {pcName && (
                    ...
                    >
                      {intl.get('spct.payConfig.button.clearCert').d('清除证书')}
                    </Button>
                  )}
                </Col>
              </Row>
      ...
      case 'unionpay': {
        ...
        itemDom = (
          <>
                  ...
                  {pcName ? (
                    <>
                      <span style={{ width: 87, display: 'inline-block', textAlign: 'right' }}>
                        {intl.get('spct.payConfig.model.payConfig.privateCert').d('私钥证书')}：
                      </span>
                      <Icons type="cert" size="20" />
                      <span style={{ marginLeft: 4 }}>{pcName}</span>
                    </>
                    ...
                    <FormItem
                      {...MODAL_FORM_ITEM_LAYOUT}
                      style={{ flexGrow: 2 }}
                      label={intl.get('spct.payConfig.model.payConfig.privateCert').d('私钥证书')}
                    >
                      {getFieldDecorator('privateCert', {
                        initialValue: privateCert,
                      })(
            ...
            {!['wysepay', 'wyseqrpay'].includes(code) && (
              <Col span={24}>
                <Row>
                  <Col span={18}>
                    {mcName ? (
                      <>
                        <span style={{ width: 87, display: 'inline-block', textAlign: 'right' }}>
                          {intl.get('spct.payConfig.model.payConfig.middleCert').d('中级证书')}：
                        </span>
                        <Icons type="cert" size="20" />
                      ...
                      <FormItem
                        {...MODAL_FORM_ITEM_LAYOUT}
                        label={intl.get('spct.payConfig.model.payConfig.middleCert').d('中级证书')}
                      >
              ...
              <Row>
                <Col span={18}>
                  {rcName ? (
                    <>
                      <span style={{ width: 87, display: 'inline-block', textAlign: 'right' }}>
                        {intl.get('spct.payConfig.model.payConfig.rootCert').d('根证书')}：
                      </span>
                      <Icons type="cert" size="20" />
                    ...
                    <FormItem
                      {...MODAL_FORM_ITEM_LAYOUT}
                      label={intl.get('spct.payConfig.model.payConfig.rootCert').d('根证书')}
                    >
