...
@connect(({ loading, refundOrderNewly }) => ({
  refundOrderNewly,
  fetchOrderListLoading: loading.effects['refundOrderNewly/fetchOrderList'],
  getOrderDetailLoading: loading.effects['refundOrderNewly/getOrderDetail'],
  refuseOrderloading: loading.effects['refundOrderNewly/refuseOrder'],
}))
@formatterCollections({ code: 'spct.refundOrder' })
export default class RefundOrder extends React.Component {
  ...
  componentDidMount() {
    ...
    dispatch({ type: 'refundOrderNewly/init' });
    this.fetchOrderList();
  }
  ...
  fetchOrderList(params = {}) {
    const { dispatch } = this.props;
    dispatch({
      type: 'refundOrderNewly/fetchOrderList',
      payload: { ...params },
    });
  }
  ...
  getOrderDetail(record) {
    const { dispatch } = this.props;
    dispatch({
      type: 'refundOrderNewly/updateState',
      payload: {
        orderDetail: {},
      },
    });
    dispatch({
      type: 'refundOrderNewly/getOrderDetail',
      payload: record,
    });
  }
  ...
  handleConfirm(record) {
    const { dispatch } = this.props;
    dispatch({
      type: 'refundOrderNewly/confirmOrder',
      payload: record,
    }).then((res) => {
      if (res) {
  ...
  handleRefuse(record) {
    ...
    // dispatch({
    //   type: 'refundOrderNewly/getRefusedOrderDetail',
    //   payload: record,
    // });
  }
  ...
  handleConfirmRefuse(params) {
    const { dispatch } = this.props;
    dispatch({
      type: 'refundOrderNewly/refuseOrder',
      payload: params,
    }).then((res) => {
      if (res) {
  ...
  render() {
    ...
    const orderColumns = [
      {
        title: intl.get('spct.refundOrder.model.refundOrder.refundOrderNum').d('退款订单号'),
        width: 180,
        dataIndex: 'refundOrderNum',
      },
      {
        title: intl.get('spct.refundOrder.model.refundOrder.paymentOrderNum').d('支付订单号'),
        width: 180,
        dataIndex: 'paymentOrderNum',
      },
      {
        title: intl.get('spct.refundOrder.model.refundOrder.merchantOrderNum').d('商户订单号'),
        width: 100,
        dataIndex: 'merchantOrderNum',
      },
      {
        title: intl.get('spct.refundOrder.model.refundOrder.channelTradeNo').d('支付流水号'),
        width: 220,
        dataIndex: 'channelTradeNo',
      },
      {
        title: intl.get('spct.refundOrder.model.refundOrder.channelCode').d('支付渠道'),
        width: 90,
        dataIndex: 'channelMeaning',
      },
      {
        title: intl.get('spct.refundOrder.model.refundOrder.refundAmount').d('退款金额'),
        dataIndex: 'refundAmount',
      },
      {
        title: intl.get('spct.refundOrder.model.refundOrder.refundReason').d('退款原因'),
        dataIndex: 'refundReason',
      },
      {
        title: intl.get('spct.refundOrder.model.refundOrder.status').d('退款状态'),
        width: 120,
        dataIndex: 'status',
        render: (val) => {
          const statusLists = [
            {
              status: 'UNREF',
              text: intl.get('spct.refundOrder.model.refundOrder.unref').d('待退款'),
            },
            {
              status: 'REFUNDING',
              text: intl.get('spct.refundOrder.model.refundOrder.refunding').d('退款中'),
            },
            {
              status: 'REFUNDED',
              color: 'green',
              text: intl.get('spct.refundOrder.model.refundOrder.refunded').d('已退款'),
            },
            {
              status: 'FAILED',
              color: 'red',
              text: intl.get('spct.refundOrder.model.refundOrder.failed').d('退款失败'),
            },
            {
              status: 'REFUSED',
              color: 'red',
              text: intl.get('spct.refundOrder.model.refundOrder.refused').d('退款拒绝'),
            },
          ];
      ...
      {
        title: intl.get('spct.refundOrder.model.refundOrder.refundDatetime').d('退款成功时间'),
        width: 150,
    ...
    return (
      <>
        <Header title={intl.get('spct.refundOrder.view.message.title').d('退款订单')} />
