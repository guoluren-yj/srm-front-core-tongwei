...
import { DataSet, Button, Tooltip } from 'choerodon-ui/pro';
...
export default class PaymentConfig extends React.Component {
  ...
  async handleSave() {
    ...
    const param = this.configForm?.formDs?.current?.toJSONData();
    if (flag) {
  ...
  render() {
    ...
    const columns = [
      ...
      { name: 'configName', width: 130 },
      {
        name: 'companyName',
        renderer: ({ text }) => text ? <Tooltip title={text} placement="topRight">{text}</Tooltip> : '-',
      },
