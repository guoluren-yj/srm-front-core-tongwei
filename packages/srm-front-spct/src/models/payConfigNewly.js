...
export default {
  ...
  effects: {
    *init(_, { put, call }) {
      const channelCodeList = getResponse(yield call(queryIdpValue, 'SPCT.PAYMENT_CHANNEL'));
      const wysepayPayTypeList = getResponse(yield call(queryIdpValue, 'SPCT.WYSEPAY_PAY_TYPE'));
      yield put({
        type: 'updateState',
        payload: {
          channelCodeList,
