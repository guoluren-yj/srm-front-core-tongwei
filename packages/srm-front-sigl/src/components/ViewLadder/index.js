...
export default class TableList extends Component {
  ...
  renderLadderTable() {
    ...
    const defaultColumns = [
      {
        title: intl.get('smodr.common.model.lineNumber').d('行号'),
        dataIndex: 'ladderLineNum',
        width: 60,
        render: (val, record) => val || record.lineNum,
      },
      {
        title: intl.get('smodr.common.model.numberFrom').d('数量从(>=)'),
        dataIndex: 'ladderFrom',
        width: 100,
      },
      {
        title: intl.get('smodr.common.model.numberTo').d('数量至(<)'),
        dataIndex: 'ladderTo',
        width: 100,
      },
      {
        title: intl.get('smodr.common.model.noTaxPrice').d('未税单价'),
        dataIndex: 'unitPrice',
        width: 120,
        align: 'right',
      ...
      {
        title: intl.get('smodr.common.model.taxPrice').d('含税单价'),
        dataIndex: 'taxPrice',
        width: 120,
        align: 'right',
    ...
    return (
      <Table
        bordered
        loading={loading}
        className="smodr-table-all-space"
  ...
  render() {
    const {
      link = intl.get('smodr.common.model.ladderPrice').d('阶梯价格'),
      disabled = false,
      onClick = (e) => e,
      popOverProps = {},
    } = this.props;
