...
import { drill, getDrillInfo } from '@/services/drillService';
  ...
  const renderer = (param: RenderProps, options?: DataSet | undefined | string) => {
    ...
    return String(curEdit) === name ? (
      ...
      <div className={styles['drill-show']} onClick={() => handleChangeEdit(Number(name), val)}>
        {getText(val, text) || intl.get('hzero.c7nProUI.Lov.choose').d('请选择')}
      </div>
    );
  };
        ...
        <Row gutter={24}>
          ...
          <Col span={16}>
            <Card bordered={false} style={{ height: 160, marginTop: 20 }}>
              <p>{intl.get('hmde.bo.view.label.sourceObject').d('来源对象')}：{result?.businessObjectName}</p>
              <p>{intl.get('hmde.bo.field.name').d('字段名称')}：{result?.businessObjectFieldName}</p>
              <p>
                {intl.get('hmde.bo.field.componentType').d('字段类型')}：
