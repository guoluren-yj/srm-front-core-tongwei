...
import { getDrillInfo, getNewDrillInfo } from '@/services/drillService';
  ...
  const openDrillModal = () => {
    if (!readOnly && curId && !disabled) {
      Modal.open({
        ...
        closable: true,
        title: title || intl.get('hzero.c7nProUI.Lov.choose').d('请选择'),
        children: (
          <DrillContent
            ref={drillRef}
...
export default formatterCollections({ code: [ "hzero.c7nProUI"] })(
  DrillComponent
);
