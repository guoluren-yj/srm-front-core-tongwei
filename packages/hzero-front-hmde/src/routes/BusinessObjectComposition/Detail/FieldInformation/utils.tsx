...
export const flatTree = data => {
  ...
  result.push({
    ...data,
    _id_: data.businessObjectRelationId,
  });
  const _func = (node) => {
    if (node.businessObjectRelationFieldList && node.businessObjectRelationFieldList.length) {
      node.businessObjectRelationFieldList.forEach(field => {
        result.push({
          ...field,
          _id_: field.businessObjectRelationFieldId,
          _parentId_: node.businessObjectRelationId,
        });
      });
    }
    if (node.businessObjectRelationList && node.businessObjectRelationList.length) {
      node.businessObjectRelationList.forEach(obj => {
        result.push({
          ...obj,
          _id_: obj.businessObjectRelationId,
          _parentId_: node.businessObjectRelationId,
        });
        _func(obj);
      });
...
export const findRelatedNode = (data: any[], key: string, value: any) => {
  if (!data) return [];
  const relatedNodes: any[] = [];
  const targetNode = data.find(item => item[key] === value);
  if (targetNode) {
    relatedNodes.push(targetNode);
    const _findChildren = (parentId) => {
      const children = data.filter(item => item._parentId_ === parentId);
      if (children.length > 0) {
        children.forEach(child => {
          relatedNodes.push(child);
          _findChildren(child._id_);
        });
      }
    };
    _findChildren(value);
