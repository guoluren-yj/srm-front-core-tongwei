...
import { treeFindPath, treeFindNode } from '../utils';
...
const TreeShow = props => {
  ...
  // 文字内容
  type IRenderContent = (props: { element: boModel.combine.IBusinessObject }) => void;
  ...
  const renderActiveStyle: IRenderActiveStyle = element => {
  ...
  // 设置边框样式
  type ISetCellStyle = (props: { element: boModel.combine.IBusinessObject }) => CSSProperties;
  ...
  // 横线
  const renderHorizontalLine = (element = {} as any) => {
    return (
      <div
  ...
  // 横线宽度
  const calHorizontalLineWidth = (element = {} as any) => {
  ...
  // 横线左边距离
  const calHorizontalLeft = (element = {} as any) => {
    const _cellMarginLeft = calCellMarginLeft(element);
    if (element.parentId && !cellWidth) {
  ...
  // 距离左边
  const calCellMarginLeft = (element = {} as any) => {
    if (element.parentId) {
  ...
  const handleSelectSource: IHandleSelectSource = async ({ element = {}, init, findElement }) => {
    setLoading(true);
    ...
    if (findElement) {
      record = treeFindNode(
        dataSource,
        item => item.businessObjectRelationId && item.businessObjectRelationId === element?.businessObjectRelationId
      );
    }
    store.setItem('record', record); // 设置选中对象
    if (!init) {
      // eslint-disable-next-line no-unused-expressions
      rightFieldsRef.current?.filterRightFields(record);
    }
    setTimeout(() => {
      setLoading(false);
    }, 0);
  ...
  const getElement = _dataSource => {
    ...
    _dataSource.forEach((element, i) => {
      arr[i] = (
        ...
        >
          {/* 关联关系名称 */}
          {['SLAVE_MASTER', 'LINK'].includes(element?.relateType) &&
          element.relBusinessObjectFieldName ? (
            <Tooltip
              placement="top"
              title={`${intl.get('hmde.boComposition.fieldInfo.relation.prefix').d('关系名：')}${
                element?.relBusinessObjectFieldName
              }`}
            >
          ...
          <div
            className={classnames(styles['content-wrapper'], {
              [styles['content-wrapper-active']]: element && activeCell === `${element?.businessObjectRelationId}`,
            })}
          ...
          {/* 递归遍历子节点 */}
          {element?.businessObjectRelationList?.length > 0
            ? getElement(element?.businessObjectRelationList)
            : null}
        </div>
      );
    });
