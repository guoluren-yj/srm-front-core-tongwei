...
function getTargetPortLabel(relateBusinessObjectFieldCode?: string, relateConditions?: boModel.combine.IRelateConditions[]): Markup.JSONMarkup | undefined {
  const markups: Markup.JSONMarkup[] = [];
  if (relateBusinessObjectFieldCode) {
    markups.push({
      tagName: 'text',
      textContent: relateBusinessObjectFieldCode,
      attrs: {
        fill: EDGE_COLOR,
    ...
    if (parentEntity) {
      ports.push({
        ...
        label: relateBusinessObjectFieldCode || relateConditions ? {
          markup: getTargetPortLabel(relateBusinessObjectFieldCode, relateConditions),
          position: {
            args: {
              x: -5,
              y: -distance / 2,
            ...
            parentNode.setPortProp(`port-${index}`, 'label', {
              markup: {
                ...
                children: [
                  ...
                  {
                    tagName: 'text',
                    textContent: parentBusinessObjectFieldCode,
                    attrs: {
                      fill: EDGE_COLOR,
                      fontSize: NODE_FONT_SIZE,
                      textAnchor: 'left',
