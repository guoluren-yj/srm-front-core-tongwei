...
import { saveModelBatchAuthorization } from '@/services/modelListService';
...
const Index = () => {
  const {
    ...
    storeData: {
      ...
      modelType,
      modelDetail,
      modelDetail: { code, assignPattern },
      // apiDetail: { code: apiCode },
  ...
  // const addTenantIdsRef: any = useRef([]);
  ...
  // 白名单table ds实例
  const allowTableDs: DataSet = useMemo(() => new DataSet(tableDsObj), [
    code,
    modelDetailRefreshSignal,
  ]);
  // 白名单table ds实例
  const blockTableDs: DataSet = useMemo(() => new DataSet(tableDsObj), [
    code,
    modelDetailRefreshSignal,
  ]);
  ...
  const formDs = useMemo(
    () =>
      new DataSet({
        ...
        // FIXME: 感觉用ds.addEventListener好点
        events: {
            ...
            dynamicProps: {
              lovQueryAxiosConfig: function lovQueryAxiosConfig() {
                return {
                  url: `${lowcodeOrganizationURL({
                    route: HZERO_HMDE,
                  })}/model-assigns/${code}/tenants/assignable`,
                  method: 'GET',
                };
              },
  ...
  const handleSaveData = async () => {
    ...
    if (flag) {
      ...
      const res = await saveModelBatchAuthorization(params);
      if (res && res.failed) {
        notification.error({ message: '错误', description: res.message });
        return false;
      }
      setDataStore('modelDetail', { ...modelDetail, assignPattern: params.assignPattern });
      notification.success({ message: '提示', description: '保存成功' });
      ds.reset();
      tableDs.query();
      unTableDs.loadData([]);
      removeTenantIdsRef.current = [];
      listViewRef.current.handleMenuQueryList();
      setEditor(false);
    }
    return false;
  };
  ...
  return (
    <div className={styles['source-authorization']}>
        ...
        <span>
          {editor ? (
            <span>
              ...
              <Button color={ButtonColor.blue} onClick={handleSaveData}>
                保存
              </Button>
            </span>
          ) : (
            <Button
              ...
              onClick={() => {
                setSourceData('sourceDetailType', '');
                setEditor(true);
              }}
            >
              编辑
      ...
      <div className={styles['source-title']}>模型授权租户信息</div>
      ...
      {editor && (
        <article className={styles['article-text']}>
          {formDs?.current?.get('assignPattern') === 'ALLOW_LIST' && (
            <p>白名单模式允许选择的租户查看指定模型；</p>
          )}
          {formDs?.current?.get('assignPattern') === 'BLOCK_LIST' && (
            <p>黑名单模式限制选择的租户不可查看指定模型；</p>
          )}
        </article>
      )}
      {/* {editor && <p className={styles['table-title']}>授权租户</p>} */}
      <Table
        ...
        className={`${globalStyles['table-style']} ${styles['model-authority-table']}`}
        columns={columns}
