...
export function needDraftFlag() {
  ...
  if (window.$$env.BASE_PATH.length > 1) {
    pathname = pathname.replace(window.$$env.BASE_PATH, '/');
  }
...
export function getAppId() {
  ...
  if (!appId || appId !== appId) {
    // window.location = `${window.location.protocol}//${window.location.host}${window.$$env.BASE_PATH}pub/authority/authority-manage`;
