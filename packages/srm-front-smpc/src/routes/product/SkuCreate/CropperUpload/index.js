...
// 图片名超过{length}截取
function splitFileUrl(url = '', length = 20) {
  const arr = url.split('.') || [];
  const suffix = `.${arr[arr.length - 1]}`;
  const prefix = url.split(suffix)[0];
  if (prefix.length <= length) {
    return url;
  }
  return prefix.substring(0, length) + suffix;
}
...
function CropperComp(props) {
  ...
  async function handleSave() {
    ...
    // 将裁减后的blob转成File
    const newFile = new RewriteFile([fileBlob], splitFileUrl(fileName));
  ...
  return (
    <div className={styles['cropper-box']}>
      ...
      <div>
        ...
        >
          <Button funcType="flat" icon="file_upload">
