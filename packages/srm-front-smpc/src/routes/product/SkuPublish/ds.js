...
const tableDs = () => ({
  ...
  transport: {
    read: ({ data }) => {
      const { queryParams = {}, isSup, ...other } = data;
      return {
        url: `/smpc/v1/${organizationId}/${isSup ? 'sup' : 'pur'}-skus/new`,
        method: 'GET',
        data: { ...other, ...queryParams },
      };
    },
  },
});
  ...
  return {
    ...
    fields: [
      ...
      },
      {
        ...
        dynamicProps: {
          lookupCode: ({ record }) =>
            record.get('skuType') !== 'EC' ? 'SMPC.PUR_SKU_STATUS' : 'SMPC.EC_SKU_STATUS',
        },
      },
      {
        name: 'thirdSkuCode',
