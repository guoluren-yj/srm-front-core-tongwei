...
export default class GoodsDetailsModal extends Component {
  ...
  render() {
    ...
    const columns = [
      {
        title: intl.get('smodr.common.model.productNum').d('商品编码'),
        dataIndex: 'productCode',
        width: 140,
      },
      {
        title: intl.get('smodr.common.model.productName').d('商品名称'),
    ...
    return (
      <Modal
        destroyOnClose
        title={`${intl
          .get('smodr.common.model.currentClassify')
          .d('当前分类：')}${code}-${name}${intl
          .get('smodr.common.model.currentTotal')
          .d(' 下共有 ')}${totalCount}${intl.get('smodr.common.model.productPieces').d(' 件商品')}`}
        visible={visible}
        onCancel={() => onCancel()}
        footer={null}
        width={705}
      >
        ...
        <Table className="smodr-table-all-space" {...tableProps} />
      </Modal>
    );
  }
}
