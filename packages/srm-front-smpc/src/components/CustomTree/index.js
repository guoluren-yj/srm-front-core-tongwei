...
export default function CustomTree({
  ...
  useEffect(() => {
    ...
    if (checkValueConfig && treeData.length > 0) {
      fetchCheckNodes();
    }
  }, []);

  useEffect(() => {
    if (typeof checkValueConfig?.queryService !== 'function') {
  ...
  async function fetchCheckNodes(treeList) {
    const { params, getValues = (e) => e, queryService = () => [] } = checkValueConfig;
    setCheckLoading(true);
    const res = getResponse(await queryService(params));
    if (res) {
      const checkNodes = getValues(res);
      onChange(checkNodes);
      const initialKeys = getCheckedAllKeys(treeList || treeData, checkNodes, idField);
      setCheckKeys((initialKeys || []).map((m) => String(m)));
    }
  ...
  async function fetchTree() {
    ...
    if (checkValueConfig) fetchCheckNodes(res);
    setLoading(false);
  }
  ...
  return (
    <Spin spinning={loading || checkLoading} wrapperClassName={styles['custom-tree-container']}>
      ...
      {quickCheck && (
        <div className="custom-tree-check-all">
          ...
          >
            {intl.get('smpc.product.view.checkedQuery').d('选中所有查询结果')}
          </CheckBox>
        </div>
      )}
      <Tree
