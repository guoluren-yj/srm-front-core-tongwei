...
import { Lov, Select, TextField } from 'choerodon-ui/pro';
import { isNumber, isEmpty } from 'lodash';
import intl from 'utils/intl';
import { getCurrentOrganizationId } from 'utils/utils';
import { openList, openSelectList } from './c7nModal';
      ...
      renderer: ({ text, value, record }) => {
        if (readOnly) {
          ...
          if (componentType === 'LOV') {
            ...
            // 组件类型不同构建数据方式不同
            const { tableFields = [] } = lovViewDTO || {};
            return (
              <a
                onClick={() =>
                  openList({ columns: tableFields, title: dimensionName, data: value })
                }
              >
