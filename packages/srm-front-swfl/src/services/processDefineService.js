...
export async function fetchProcessList(params) {
  const { page, size, ...other } = parseParameters(params);
  if (isSiteFlag) {
  ...
  return request(`${prefix}/process/models/page`, {
    method: 'POST',
    query: {
      customizeUnitCode: 'HWFP.PROCESS_DEFINITION.GRID,HWFP.PROCESS_DEFINITION.QUERY',
      page,
      size,
    },
    body: other,
...
export async function addProcess(params) {
  return request(`${prefix}/process/models`, {
    method: 'POST',
    body: { ...params.process },
    query: { customizeUnitCode: 'HWFP.PROCESS_DEFINITION.CREATE' },
  });
}
 ...
 * @returns {object} fetch Promise
 */
export async function queryRemind() {
  return request(`${HZERO_HWFP}/v1/${tenantId}/notification`, {
    method: 'POST',
  });
}
...
export async function addRemind(params) {
  return request(`${HZERO_HWFP}/v1/${tenantId}/notification/save`, {
    method: 'POST',
    body: params,
  });
}
...
export async function releaseProcess(params) {
  const { modelId, ...other } = filterNullValueObject(params);
  return request(`${prefix}/process/models/${modelId}/deploy`, {
    method: 'POST',
    body: other,
  });
}
...
export async function importProcess(params) {
  return request(`${prefix}/process/models/upload`, {
    method: 'POST',
    body: params,
    query: { customizeUnitCode: 'HWFP.PROCESS_DEFINITION.IMPORT' },
  });
}
...
export async function saveProcessSetting({ modelId, ...other }) {
  return request(`${prefix}/process/models/${modelId}`, {
    method: 'POST',
    body: other,
    query: { customizeUnitCode: 'HWFP.PROCESS_DEFINITION.SETTING' },
  });
}
...
export async function queryProcessTagConfs() {
  return request(`${prefix}/process-label-confs`, {
    method: 'GET',
    query: { tenantId },
  });
}

export async function saveProcessTagConfs(data) {
  return request(`${prefix}/process-label-confs`, {
    method: 'POST',
    body: data,
  });
...
export async function deleteProcessTagConfs(params, data) {
  return request(`${prefix}/process-label-confs`, {
    method: 'DELETE',
    body: data,
    query: params,
