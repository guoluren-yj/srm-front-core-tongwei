...
// 查询流程单据-分类树
export async function fetchInvolvedTaskCategoryList() {
  return request(
    `${prefix}/${getCurrentOrganizationId()}/activiti/task/document-list?type=involved`
  );
}

// 审批工作台查询流程单据-分类树
export async function fetchNewInvolvedTaskCategoryList() {
  return request(
    `${prefix}/${getCurrentOrganizationId()}/activiti/task/document-list-new?type=involved`
  );
}

export async function setCarbonCopy(params) {
  return request(`${prefix}/${getCurrentOrganizationId()}/process/instance/new-carbon-copy`, {
    method: 'POST',
    responseType: 'text',
    body: params,
  });
