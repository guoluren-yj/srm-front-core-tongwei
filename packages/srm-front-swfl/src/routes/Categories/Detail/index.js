...
const sourceModalRowKey = 'sourceParentId';
/**
 * 流程分类详情
...
export default class Detail extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      routeFrom: props.location.state && props.location.state.routeFrom,
      canUpdateCode: false, // 控制字段编码输入框编辑状态
    };
  }
      ...
      if (!err) {
        if (isUndefined(id)) {
          dispatch({
            ...
            payload: {
              ...values,
              documentIdSet: documentList.map((n) =>
                n[sourceModalRowKey] ? n[sourceModalRowKey] : n[documentModalRowKey]
              ),
            },
          }).then((res) => {
            if (res) {
  ...
  handleEditVariable(record) {
    this.setState({ drawerVisible: true, record, canUpdateCode: true });
  }
  ...
  @Bind()
  onBackBtnClick() {
    const { routeFrom, id } = this.state;
    if (routeFrom) {
      closeTab(`/hwfp/setting/categories/detail/${id}`);
    }
  }
  ...
  render() {
    ...
    const drawerProps = {
      ...
      enumMap,
      tenantId,
      headerTenantId: headerInfo.tenantId,
      dataType,
      operator,
      anchor: 'right',
    ...
    return (
      <>
        <Header
          title={headerTitle}
          backPath="/hwfp/setting/categories/list"
          onBack={this.onBackBtnClick}
        >
          <Button
            ...
            <Form className={classnames(EDIT_FORM_CLASSNAME, styles['detail-form'])}>
              ...
              <Row className={ROW_READ_ONLY_CLASSNAME}>
                ...
                >
                  {!predefined && (
                    <Button
                      disabled={predefined || documentList.length >= 1}
                  ...
                  <div className={styles['form-item-control-wrapper']}>
                    {documentList.map(
                      (item) =>
                        ...
                        (!predefined && !item.sourceParentId ? (
                          <Popconfirm
                            placement="topRight"
                            title={intl
                          ...
                          <Tag
                            color="blue"
                            closable={!predefined && !item.sourceParentId}
