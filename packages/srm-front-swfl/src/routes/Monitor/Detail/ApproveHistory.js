...
import { Form, Table, Tooltip, Tag, Popover } from 'hzero-ui';
...
export default class TaskDetail extends PureComponent {
   ...
   */
  render() {
    const { detail = {}, loading, processDefineConfig } = this.props;
    const showModelStandardTime =
      processDefineConfig && processDefineConfig.modelStandardTimeFlag === 1;
    const columns = [
      {
        ...
        render: (value, record, index) => {
          const { actType, modelStandardTime } = record;
          let val = value;
          ...
          const obj = {
            children: (
              <>
                {val}
                {showModelStandardTime && modelStandardTime ? (
                  <div>
                    <span
                      style={{
                        marginTop: '2px',
                    ...
                    >
                      {intl
                        .get('hwfp.common.view.message.current.preStage.modelStandardTime')
                        .d('标准用时')}
                      : {modelStandardTime}
                    </span>
                  </div>
      ...
      },
      {
        ...
        render: (action, record) =>
          action ? (
            <>
              ...
              {record.rejectJumpType &&
                ...
                [-1, 0, 1, 2, -2, -3, -4].indexOf(record.rejectJumpFlag) > -1 && (
          ...
          ) : record.actType === 'CommentCarbonCopy' ? (
            <Tag>{intl.get('hzero.common.text.comment').d('评论')}</Tag>
          ) : (
            <Tag color="geekblue">
    ...
    return (
      <>
        <Table
          ...
          loading={loading}
          pagination={false}
          dataSource={this.transformData(detail.historicTaskExtList)}
          columns={columns}
