...
import { Table, Popconfirm, Tooltip } from 'hzero-ui';
...
export default class FormList extends Component {
  ...
  render() {
    const { loading, predefined, dataSource = [], isSiteFlag } = this.props;
    const columns = [
      {
      ...
      {
        title: intl.get('hwfp.common.model.common.cuszStageCode').d('单据样式阶段'),
        dataIndex: 'cuszStageName',
      },
      {
        title: (
          <Tooltip
      ...
      isSiteFlag && {
        ...
        render: (val, record) => {
          ...
          if (!predefined && !record.copyFlag) {
            // 租户级不能编辑单据样式配置
            if (record.cuszStageCode && !isSiteFlag) {
              return;
            }
