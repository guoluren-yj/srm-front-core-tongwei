...
@formatterCollections({
  code: ['hwfp.documents', 'hwfp.common', 'hpfm.permission', 'entity.tenant'],
})
export default class Document extends Component {
   ...
   */
  @Bind()
  copySiteRecord(record = {}) {
    const { dispatch } = this.props;
    dispatch({
      type: 'documents/copySiteRecord',
      payload: record,
    }).then((res) => {
      if (res) {
        notification.success();
        this.handleToDetail(res.documentId);
      }
    });
  }
  ...
  render() {
    const {
      isSiteFlag,
      currentTenantId,
      documents: { dataSource = [], pagination = {}, enumMap = {} },
    ...
    const formProps = {
      ...
      ref: this.searchRef,
      documentSourceTypes: enumMap.documentSourceTypes || [],
    };
    const tableProps = {
      isSiteFlag,
      pagination,
      ...
      onCopySiteRecord: this.copySiteRecord,
    };
    return (
      <>
