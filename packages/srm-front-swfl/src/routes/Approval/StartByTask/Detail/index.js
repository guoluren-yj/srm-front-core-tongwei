...
import { observerDrawerResize, START_TASK_TAB_DRAWER_ID, computeDrawerHeight } from '../../utils';
...
@remote({
  code: 'SWFL_APPROVAL_WORKBENCH_START_DETAIL',
  name: 'processRemote',
})
...
export default class StartByTaskDetail extends Component {
  ...
  // 监听弹窗尺寸变化
  observerSize = () => {
    this.drawerResizeObserver = observerDrawerResize(START_TASK_TAB_DRAWER_ID);
  };

  computeDrawerHeight = () => {
    computeDrawerHeight(START_TASK_TAB_DRAWER_ID);
  };
  ...
  componentWillUnmount = () => {
    ...
    if (this.drawerResizeObserver) {
      this.drawerResizeObserver.unobserve(document.getElementById(START_TASK_TAB_DRAWER_ID));
    }
  ...
  renderApproveRecord = () => {
    ...
    if (processRemote) {
      return processRemote.render(
        'SWFL_APPROVAL_WORKBENCH_START_DETAIL_APPROVAL_RECORD',
      ...
      <>
        <div
          className={classNames(
            ...
            'swfl-approval-workbench-start-task-detail'
          )}
