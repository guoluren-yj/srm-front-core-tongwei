...
export default class FilterForm extends Component {
  ...
  onChangeSort = () => {
    ...
    localStorage.setItem(
      `APPROVAL_LIST_CC_TASK_FILTER_SORT.${getCurrentOrganizationId()}.${getCurrentUserId()}`,
  ...
  render() {
    ...
    // 状态下拉框
    const statusList = [
      { title: intl.get('hzero.common.status.read').d('已读'), value: 1, key: 1 },
      { title: intl.get('hzero.common.status.unread').d('未读'), value: 0, key: 0 },
    ];
    const statusData = {
      title: intl.get('hwfp.carbonCopyTask.view.message.priority').d('状态'),
      filterName: 'readFlag',
