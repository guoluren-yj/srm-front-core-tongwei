...
export default class ExpressionParamter extends React.Component {
  ...
  renderColumnsOperator = (value) => {
    const { serviceOperatorList = [] } = this.props;
    const filterArr = serviceOperatorList.filter((item) => item.value === value);
    const meaning = filterArr.length > 0 && filterArr[0].meaning ? filterArr[0].meaning : value;
    return <span>{meaning}</span>;
  };

  @Bind()
  getColumns() {
    const { editControl = false } = this.props;
    const optionsColumn = editControl
      ? []
      : [
          {
    ...
    this.columns = [
      ...
      {
        title: intl.get('hwfp.serviceDefinition.model.param.parameterDescription').d('左参数名称'),
        dataIndex: 'parameterDescription',
        width: 200,
      },
      {
        title: intl.get('hwfp.serviceDefinition.model.service.leftParameterSource').d('左参数来源'),
      ...
      },
      {
        ...
        dataIndex: 'operator',
        render: this.renderColumnsOperator,
        width: 150,
      },
      {
      ...
      },
      {
        title: intl
          ...
          .d('右参数名称'),
        dataIndex: 'rightParameterDescription',
        width: 200,
      },
      {
        title: intl
