...
export default function DatePickerFilter(props) {
  ...
  useEffect(() => {
    if (dataSet.current) {
      ...
      if (!isEmpty(value)) {
        if (value[datePickerAfter]) {
          setStartTimeValue(moment(value[datePickerAfter]).format('YYYY-MM-DD'));
        }
        if (value[datePickerBefore]) {
          setEndTimeValue(moment(value[datePickerBefore]).format('YYYY-MM-DD'));
        }
      }
    }
  }, [defaultValue]);
        ...
        {(startTimeValue || endTimeValue) && (
          <>
            ...
            >
              ...
              <span>{intl.get('hzero.common.view.to').d('至')}</span>
              <div
                // eslint-disable-next-line
                tabIndex="0"
                onFocus={() => setDatePickerStatus(true)}
