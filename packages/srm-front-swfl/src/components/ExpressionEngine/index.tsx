...
import { DataSet, Form, SelectBox, TextField, Button, Tooltip, Lov, Select, NumberField, DatePicker, DateTimePicker, Switch } from 'choerodon-ui/pro';
...
import { getOperatorList, getC7nComponentType, isJSON } from './utils.ts';
import { getExpressionEngine, saveExpressionEngine, deleteExpressionEngine } from './expressionEngineService.ts';
    ...
    <div className='expression-engine-multiple-rule'>
      {
        controlFlag && dataSet.records && dataSet.records.length > 0 && dataSet.records.map(expression => {
          if (expression.status !== 'delete') {
            return (
              <Form record={expression} labelLayout="float" columns={20} className='expression-engine-multiple-rule-row'>
...
function ExpressionEngine(props) {
  const { code, dataSource, currentTenantId = currentOrganizationId, childRef, leftValueLovQueryPara, disabled = false, booleanTransformFlag = false, secondLevelTitleFlag = false, showActionButton = true, afterQuery } = props;
      ...
      fields: [
        {
          ...
          required: true,
          disabled,
          lovCode: isTenant ? 'HWFP.ENGINE.PROCESS_VARIABLE_LOV_VIEW' : 'HWFP.ENGINE.PROCESS_VARIABLE_LOV_VIEW.SITE',
        ...
        },
        {
          ...
          transformRequest: (value, record) => {
            ...
            if (componentType === 'number' && isNumber(value) && value < 0) {
          ...
          transformResponse: (value, object) => {
            ...
            const { type } = object.leftValue || {};
            const componentType = getC7nComponentType(type);
            if (componentType === 'number' && isString(value) && value.includes('NEGATIVE')) {
              return 0 - parseFloat(value.replace(/NEGATIVE\(|\)/g, ''));
            }
          ...
          dynamicProps: {
            ...
            lovCode: ({ record }) => {
              ...
              if (rightValueType === 'variable') {
                return isTenant ? 'HWFP.ENGINE.PROCESS_VARIABLE_LOV_VIEW' : 'HWFP.ENGINE.PROCESS_VARIABLE_LOV_VIEW.SITE'
              } else {
                return lovCode
              }
            },
      ...
      events: {
        update: ({ record, name }) => {
          if (name === 'leftValue') {
            ...
            const { type, ...otherProps } = record.get('leftValue') || {};
            if (type === 'RADIO') {
              record.set('leftValue', { type: 'SINGLE_SELECT', ...otherProps });
            }
          }
  ...
  const getExpressionEngineJson = async () => {
    if (!customizeConditionCombinationDs.current) {
      return false;
    }
  ...
  const AddExpressionRuleBtn = observer((propsParam) => {
    ...
    return dataSet.current && dataSet.current.get('conditionType') !== 'TRUE' && (
      <Tooltip
        title={intl.get('component.ExpressionEngine.view.button.create.ruleBtn').d('新建条件规则')}
      >
        <a
          // className={`expression-engine-multiple-rule-add-btn ${disabled ? 'rule-add-btn-disabled' : 'rule-add-btn-enabled'}`}
          className={classNames(`expression-engine-multiple-rule-add-btn`, {
            'rule-add-btn-disabled': disabled
          })}
  ...
  return (
    <Spin spinning={code ? expressionEngineLoading : false}>
      <div className={styles['expression-engine1']}>
        {code && (
          <div className={classNames("expression-engine-title", {
            "expression-engine-title-secondLevel": secondLevelTitleFlag
          })}>
            <span>{intl.get('component.ExpressionEngine.view.title.expressionEngine').d('条件引擎')}</span>
            {
              !disabled && showActionButton && (
                <div className="expression-engine-buttons">
                  ...
                  <Button color='primary' onClick={onSaveExpressionEngine}>{intl.get('hzero.commom.button.save').d('保存')}</Button>
                </div>
              )
            }
          </div>
        )}
        <Form columns={2} labelLayout="float" dataSet={conditionTypeDs}>
          <SelectBox
            label={intl.get('component.ExpressionEngine.model.expressionEngine.conditionType').d('策略逻辑')}
            name="conditionType"
            colSpan={2}
