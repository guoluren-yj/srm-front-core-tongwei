...
import { queryImportDataHistory } from '@/services/processAppointServices';
...
function RecordList({ currentRecord, setCurrentRecord }) {
        ...
        <div style={{ color: '#000' }}>
          ...
          <span>
            {isImporting
              ...
              : intl
                  .get('srm.common.view.message.importJson.summaryTitle', { name: totalCount })
                  .d(`共导入 ${totalCount} 个JSON文件`)}
          </span>
        </div>
      );
    },
  ...
  const listItemSummaryDetailRender = useCallback(
    (listItem) => {
      ...
      const successCount =
        statusCount && statusCount[ImportStatus.SUCCESS] ? statusCount[ImportStatus.SUCCESS] : 0;
      const failCount =
        statusCount && statusCount[ImportStatus.ERROR] ? statusCount[ImportStatus.ERROR] : 0;
      const warnCount =
        statusCount && statusCount[ImportStatus.WARN] ? statusCount[ImportStatus.WARN] : 0;
      const noPassCount =
        statusCount && statusCount[ImportStatus.NOT_PROCESS]
          ? statusCount[ImportStatus.NOT_PROCESS]
          : 0;
      ...
      if (total === successCount) {
        ...
        return (
          <div>
            ...
            {warnCount > 0 && (
              <>
                <span style={{ color: StatusColor.WARN, margin: '0 4px', fontWeight: 500 }}>
                  {warnCount}
                </span>
                <span>
                  {intl.get('srm.common.view.message.importReport.importWarnCount').d('个异常')}
                </span>
              </>
            )}
            {noPassCount > 0 && (
              <>
                <span style={{ color: StatusColor.NOT_PROCESS, margin: '0 4px', fontWeight: 500 }}>
                  {noPassCount}
                </span>
                <span>
                  {intl.get('srm.common.view.message.importReport.importNoPassCount').d('个未处理')}
                </span>
              </>
            )}
            {message && (
