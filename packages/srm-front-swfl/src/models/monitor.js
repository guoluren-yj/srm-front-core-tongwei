...
import {
  ...
  fetchProcessVariable,
  fetchApprovalHistoryList,
  batchSuspend,
  batchRestore,
  continueProcess,
...
export default {
  ...
  effects: {
    *queryProcessStatus(_, { call, put }) {
      const { processStatus = [], variableTypes = [], sourceTypes = [] } = getResponse(
        yield call(queryMapIdpValue, {
          processStatus: 'HWFP.PROCESS_APPROVE_STATUS',
          variableTypes: 'HWFP.PROCESS.VARIABLE_TYPE',
          sourceTypes: 'HWFP.VARIABLE_SOURCE_CODE',
        })
      );
      yield put({
        type: 'updateState',
        payload: { processStatus, variableTypes, sourceTypes },
      });
    },
    ...
    *fetchMonitorListPage({ payload }, { call, put }) {
      if (payload.needCountFlag === 'Y') {
        const resForCount = yield call(fetchMonitorList, { ...payload, onlyCountFlag: 'Y' });
    ...
    *fetchProcessException({ payload }, { call, put }) {
    ...
    // 流程变量
    *fetchProcessVariable({ payload }, { call, put }) {
      const res = yield call(fetchProcessVariable, payload.record);
      const pageCount = getResponse(res);
      if (pageCount) {
        yield put({
          type: 'updateState',
          payload: {
            processVariableList: pageCount.content,
            processVariablePagination: createPagination(pageCount),
          },
        });
      }
      return pageCount;
    ...
    *fetchApprovalHistoryList({ payload }, { call }) {
      const res = yield call(fetchApprovalHistoryList, payload);
      return getResponse(res);
    },
    *batchSuspend({ payload }, { call }) {
      const res = yield call(batchSuspend, payload);
      return getResponse(res);
    },
    *batchRestore({ payload }, { call }) {
      const res = yield call(batchRestore, payload);
      return getResponse(res);
    },
    *continueProcess({ payload }, { call }) {
      const res = yield call(continueProcess, payload);
      return getResponse(res);
    },
    *fetchProcessDefineConfig({ payload }, { call }) {
      const res = yield call(getProcessDefineConfig, payload);
      return getResponse(res);
    },
  },
  reducers: {
