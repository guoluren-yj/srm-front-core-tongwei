...
export const listFormDS = () => ({
  ...
  fields: [
    ...
    {
      name: 'assigneeUnitList',
      label: intl.get('hwfp.common.current.department').d('当前处理部门'),
      type: 'object',
...
export const listTableDS = () => ({
  ...
  transport: {
    read: ({ data, params }) => {
      const { startedBy = true, page, size } = params;
      const { queryParams = {} } = data;
      return {
        url: `${prefix}/process/instance/query/new-page?${stringify({
          startedBy,
        })}&needPreviousComment=true&customizeUnitCode=HWFP.APPROVAL_TABLE_UNIT_GROUP.STARTEDBY,HWFP.APPROVAL_WORKBENCH_LIST.STARTEDBY.FILTER`,
        method: 'POST',
        data: {
          ...queryParams,
          page,
  ...
  events: {
    update: ({ record, name, value }) => {
      if (name === 'addCc') {
        ...
        request(`${prefix}/process/instance/carbon-copy?processInstanceId=${id}&type=startedBy`, {
          method: 'POST',
          body: employeeNum,
          responseType: 'text',
        }).then((res) => {
