...
const Index = memo((props: ProcessProps) => {
  ...
  const RenderShowStep = useCallback(() => {
    let info: any = stepList[0];
    const notStartLastIndex = findIndex(stepList, (v: any) => v.status === 'NOT_STARTED');
    const endLastIndex = findLastIndex(stepList, (v: any) => v.status === 'CLOSED');
    const progressLastIndex = findLastIndex(stepList, (v: any) => v.status !== 'CLOSED' && v.status !== 'NOT_STARTED');
