...
import { ActionType, ProjectListCode, ProjectSearchCode, DocumentListCode, DocumentSearchCode, StageListCode, StageSearchCode, ActiveKey, campCode } from '../../utils/type';
  ...
  return {
    ...
    fields: [
      ...
      },
      {
        name: 'alterStatus',
        type: FieldType.string,
        lookupCode: 'SQAM.PPAP_PROJECT_ALTER_STATUS',
        label: intl.get(`sqam.ppap.model.project.alterStatus`).d('变更状态'),
      },
    ],
    queryParameter: {
...
export const documentTableDS = (activeKey: ActiveKey, remoteProps: any): DataSetProps => {
  const dsProps = {
    autoQuery: false,
    pageSize: 20,
    selection: false,
    ...
    transport: {
      read: ({ data }) => {
        return {
          url: `${SRM_SQAM}/v1/${organizationId}/access-documents/page`,
          method: 'GET',
          data: {
            ...data,
            ...transformQselectDate(data, { creationDateRange: 'creationDate' }),
  ...
  return remoteProps?.process ? remoteProps.process('SQAM_PPAPWORKBENCH_LIST_CUX_DOCUMENT_TABLE_DS_PROPS', dsProps, { activeKey }) : dsProps;
};
...
export const stageTableDS = (activeKey: ActiveKey): DataSetProps => {
  return {
    ...
    transport: {
      read: ({ data }) => {
        return {
          url: `${SRM_SQAM}/v1/${organizationId}/access-stages/page`,
          method: 'GET',
          data: {
            ...data,
            ...transformQselectDate(data, { creationDateRange: 'creationDate' }),
