...
const DocumentInfo = (props) => {
  ...
  const {
    documentInfoDs, typeFlag, customizeCollapse, customizeForm, remoteProps, headerDs, pubEditDocFlag,
  } = useContext<StoreValueType>(Store);
  const { documentStatus, documentId, camp, documentUploadPoint, stageStatus } = documentInfoDs?.current?.get(['documentStatus', 'documentId', 'camp', 'documentUploadPoint', 'stageStatus']) || {};
    ...
    return [
      {
        ...
        renderer: ({ value, text, record }) => {
          ...
          return remoteProps.process ? remoteProps.process('SQAM_PPAPWORKBENCH_DETAIL_CUX_DOCUMENT_INFO_STATUS_RENDERER', statusTag, { text, value, record, TagColor, StatusTag }) : statusTag;
        },
      },
      'documentNum',
      'documentName',
  ...
  const attachmentColumns = useMemo(() => {
    ...
    const columns = [
      { name: 'purchaseAttachmentUuid', editor: Attachment, useEditor: true, showHistory: true, readOnly: (typeFlag || !editFlag) || !attachmentUploadFlag },
      camp === 'SUPPLIER' && { name: 'documentAttachmentUuid', editor: Attachment, useEditor: true, showHistory: true, readOnly: true },
    ...
    return remoteProps.process ? remoteProps.process('SQAM_PPAPWORKBENCH_DETAIL_CUX_DOCUMENTDETAIL', columns, {
      headerDs, editFlag, documentInfoDs,
    }) : columns;
  }, [typeFlag, editFlag, headerDs, remoteProps, documentInfoDs, camp, documentUploadPoint, documentStatus, projectStatus, stageStatus]);
  ...
  }, [columnsNum, documentInfoDs, editorColumns, attachmentColumns, readOnly, editFlag, customizeForm, pubEditDocFlag]);
