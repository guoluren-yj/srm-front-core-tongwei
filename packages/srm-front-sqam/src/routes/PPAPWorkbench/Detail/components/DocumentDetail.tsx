...
const DocumentDetail = observer(() => {
  ...
  const getTabHeaderRander = useCallback((
    ...
    const statusTagRender = remoteProps.process ? remoteProps.process('SQAM_PPAPWORKBENCH_DETAIL_CUX_DOCUMENT_DETAIL_TAB_HEADER_STATUS_RENDERER', statusTag, { record, text: documentStatusMeaning, stageStatus, documentStatus, TagColor, StatusTag }) : statusTag;
    return (
    ...
    <div className={styles['document-detail']}>
      <Tabs tabPosition={TabsPosition.left} activeKey={activeKey} tabBarExtraContent={getRenderHeaDer()} onChange={handleChangeTabs}>
        {documentListDs.map((item) => {
          ...
          // 如果是交付物可编辑表单，则不显示其他交付物
          if (pubEditDocFlag && documentNum !== activeDocumentNum) return null;
          return (
