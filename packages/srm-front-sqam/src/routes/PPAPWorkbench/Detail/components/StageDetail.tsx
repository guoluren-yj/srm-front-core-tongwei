...
const DocumentDetail = observer(() => {
  ...
  }, [stageListDs, handleLoadInfo, activeStageNum, headerDs]);
    ...
    return [
      {
        ...
        renderer: ({ record, text, value }) => {
          ...
          return remoteProps.process ? remoteProps.process('SQAM_PPAPWORKBENCH_DETAIL_CUX_STAGE_DETAIL_STATUS_RENDERER', statusTag, { record, text, value, TagColor, StatusTag }) : statusTag;
        },
      },
      'stageNum',
      'stageName',
  ...
  const getTabHeaderRander = useCallback((stageName: string | null, stageNum: string | null, stageStatus: string, stageStatusMeaning: string | null, record: any) => {
    const statusTag = <StatusTag value={stageStatusMeaning} flag color={TagColor[stageStatus] || 'success'} />;
    const statusTagRender = remoteProps.process ? remoteProps.process('SQAM_PPAPWORKBENCH_DETAIL_CUX_STAGE_DETAIL_TAB_HEADER_STATUS_RENDERER', statusTag, { record, text: stageStatusMeaning, stageStatus, TagColor, StatusTag }) : statusTag;
    return (
    ...
    <div className={styles['document-detail']}>
      <Tabs tabPosition={TabsPosition.left} activeKey={activeKey} tabBarExtraContent={getRenderHeaDer()} onChange={handleChangeTabs}>
        {stageListDs.map((item) => {
          const { stageName, stageNum, stageStatus, stageStatusMeaning, stageId } = item?.get(['stageName', 'stageNum', 'stageStatus', 'stageStatusMeaning', 'stageId']) || {};
          // 如果是阶段可编辑表单，则不显示其他交付物
          if (pubEditStageFlag && stageNum !== activeStageNum) return null;
          return (
            <TabPane key={stageNum} tab={getTabHeaderRander(stageName, stageNum, stageStatus, stageStatusMeaning, item)}>
              <div>
                <div id={`ppap-stage-detail-content-${stageId}`} className={styles['sqam-detail-content-ppapWorkbench']}>
                  {
                    customizeCollapse(
                      {
                      ...
                      >
                        <Panel key='basic' forceRender id={`ppap-stage-basic-${stageId}`} header={`${stageName || ''}-${intl.get(`hzero.common.view.baseInfo`).d('基本信息')}`} showArrow={false}>
                          <EditorForm
                            columns={3}
                            useColon={false}
                            dataSet={stageInfoDs}
                            editorFlag={pubEditStageFlag}
                            editorColumns={editorColumns}
                            customizeForm={customizeForm}
                            customizeOptions={{ code: DetailStageFormCode }}
                          />
                        </Panel>
                        <Panel id={`ppap-stage-list-${stageId}`} forceRender header={intl.get(`sqam.ppap.model.template.deliverableDetail`).d('交付物清单')} key='list' showArrow={false}>
                          <StageDocumentList />
                        </Panel>
                      </Collapse>
                    )
