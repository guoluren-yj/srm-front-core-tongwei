...
const DynamicAlert = flow(
    ...
    if (fileUrl) {
      arr.push(
        <Tooltip
          ...
          placement="top"
          title={intl.get('sqam.common.view.button.DownloadAttachments').d('下载附件')}
        >
          <Icon
            type="attach_file"
            className={styles['sqam-alert-attach-file']}
  ...
  return text ? (
    <div ref={wrapperRef} className={styles[`sqam-alert-wrapper`]}>
      <Alert
        closable
        showIcon
        message={message || dynamicText}
        className={classNames(styles[`sqam-alert-${type}`], styles[`sqam-alert-${placement}`])}
