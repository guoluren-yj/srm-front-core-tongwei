...
const DocumentTable = (props: DocumentTableProps) => {
  ...
  const handleClickNum = useCallback((record) => {
    ...
    if (activeKey === ActiveKey.DocumentPending) {
      handleToDetail(projectHeaderId, 'edit', 'document-edit', documentNum, projectType);
    ...
    return [
      {
        ...
        renderer: ({ text, value, record }) => {
          ...
          return remoteProps.process ? remoteProps.process('SQAM_PPAPWORKBENCH_SUP_LIST_CUX_DOCUMENT_STATUS_RENDERER', statusTag, { text, value, record, TagColor, StatusTag }) : statusTag;
        },
      },
      activeKey === ActiveKey.DocumentAll && {
        ...
        renderer: ({ record }) => [
          ...
          ['UNUPLOADED'].includes(record?.get('documentStatus')) && !['CLOSED'].includes(record?.get('projectStatus')) && ['SUPPLIER'].includes(record?.get('camp')) && (
            <PermissionButton
              type="c7n-pro"
          ...
          ['COMPLETED', 'SUBMITTED'].includes(record?.get('documentStatus')) && !['CLOSED'].includes(record?.get('projectStatus')) && ['IN_PROGRESS'].includes(record?.get('stageStatus')) && ['SUPPLIER'].includes(record?.get('camp')) && (
            <PermissionButton
              type="c7n-pro"
              funcType={FuncType.link}
              color={ButtonColor.primary}
              ...
              permissionList={[
                {
                  code: `srm.sqam.ppap.supworkbench.button.documentChange`,
                  type: 'button',
                },
              ]}
            >
