    ...
    return customizeForm(
      ...
      <Form>
          ...
          <Col {...FORM_COL_3_LAYOUT}>
            <Form.Item label={intl.get(`${prefix}.model.originNum`).d('来源单号')}>
              {getFieldDecorator('dataSourceNum')(
                <DataSourceNum detail={detail} camp="PURCHASE" history={history} remoteProps={remoteProps} />
              )}
            </Form.Item>
          </Col>
        </Row>
        <Row {...EDIT_FORM_ROW_LAYOUT} className="read-row">
          <Col {...FORM_COL_3_LAYOUT}>
            {/* <DisplayFormItem
              label={intl.get(`${prefix}.model.common.claimAmount`).d('索赔金额')}
              value={numberRender(totalAmount, 2)}
            /> */}
          ...
          <Col {...FORM_COL_3_LAYOUT}>
            ...
            <Form.Item label={intl.get(`sqam.common.model.expenseProcessType`).d('费用处理方式')}>
              {getFieldDecorator('expenseProcessType')(<span>{expenseProcessTypeMeaning}</span>)}
            </Form.Item>
          </Col>
        </Row>
        <Row {...EDIT_FORM_ROW_LAYOUT} className="read-row">
