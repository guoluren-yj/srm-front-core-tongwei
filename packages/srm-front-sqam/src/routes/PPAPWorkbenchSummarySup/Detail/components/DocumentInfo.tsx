...
const DocumentInfo = (props) => {
  ...
  return (
    <div className={classNames(styles['sqam-detail-content-ppapWorkbench'], { [styles['sqam-detail-content-ppapWorkbench-panel']]: readOnly })}>
      ...
      >
        <Panel key='basic' header={intl.get(`sqam.ppap.view.decument.info`).d('交付物基本信息')} showArrow={false} forceRender>
          <EditorForm
        ...
        <Panel className={classNames({ [styles['sqam-detail-content-ppapWorkbench-info']]: !readOnly }, styles['sqam-detail-content-ppapWorkbench-attachment'])} header={intl.get(`sqam.ppap.view.title.document`).d('交付物')} key='attachment' showArrow={false} forceRender>
          <EditorForm
            columns={1}
            useColon={false}
            dataSet={documentInfoDs}
