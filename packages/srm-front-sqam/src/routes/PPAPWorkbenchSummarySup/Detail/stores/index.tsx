...
import { basicInfoDS, partLineDS, documentLineDS, stageLineDS, documentInfoDS, partInventoryDS } from './indexDS';
...
const StoreProvider = flow(withCustomize({
  ...
  const handleToList = useCallback(() => {
    history.push({
      pathname: '/sqam/PPAPWorkbenchSummarySup/list',
      state: { _back: 1 },
    });
  }, [history]);
  ...
  const handleToDetail = useCallback((id: string | number, operateType: Operate) => {
    if (!id) return;
    history.push({
      pathname: `/sqam/PPAPWorkbenchSummarySup/detail/${id}`,
  ...
  }, [createFlag, checkFlag, handleToDetail, handleToList, history, customizeForm, customizeBtnGroup, customizeTable, modal, viewFlag, editFlag, operate, partLineDs, documentLineDs, stageLineDs, headerDs, pendingFlag, location, documentInfoDs, partInventoryDs, customizeCollapse, custConfig]);
