...
@remote({
  code: 'SQAM_CREATE_CLAIM_DETAIL_CUX',
  name: 'remote',
})
...
export default class Detail extends Component {
  ...
  async componentDidMount() {
    const { match, dispatch, remote: remoteProps } = this.props;
  ...
  addLine() {
    ...
    const { remote: remoteProps, form } = this.props;
    const { claimTypeId, typeNum, claimTypeName } = form.getFieldsValue(['claimTypeId', 'typeNum', 'claimTypeName']);
    const defaultField = {
      rowKey: uuid(),
      formLineId: uuid(),
      _status: 'create',
      disabledTax: false,
      disabledNoTax: false,
      claimItemId: claimTypeId,
      claimItemNum: typeNum,
      claimItemDesc: claimTypeName,
    };
    const addField = remoteProps
      ? remoteProps.process('SQAM_CREATE_CLAIM_DETAIL_CUX_LINE_ADD_VALUE', defaultField, {
        lineDataSource,
        state: this.state,
      })
      : defaultField;
    ...
    form.validateFieldsAndScroll((err, values) => {
      ...
      if (!err && ((lineDataSource.length > 0 && !isEmpty(lineData)) || isEmpty(lineDataSource))) {
        dispatch({
          ...
          if (res) {
            notification.success();
            const url = '/sqam/createClaim/detail';
  ...
  handleClaimTypeChange(value) {
    const { lineDataSource } = this.state;
    const { claimTypeId, typeNum, typeDesc } = value;
    const newlineDateSource = lineDataSource.map((item) => ({
      ...item,
      claimItemId: claimTypeId,
      claimItemNum: typeNum,
      claimItemDesc: typeDesc,
    }))
    ;
    this.setState({
      lineDataSource: newlineDateSource,
  ...
  render() {
    ...
    const basicInfoProps = {
      ...
      customizeForm,
      remoteProps,
      onChangeClaimType: this.handleClaimTypeChange,
    };
