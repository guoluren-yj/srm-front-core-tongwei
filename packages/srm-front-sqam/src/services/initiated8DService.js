...
export async function save8D(params) {
  ...
  return request(
    `${prefix}/${params.tenantId}/problem-headers/save?customizeUnitCode=${customizeUnitCode}`,
    {
      method: 'PUT',
      body: others,
    }
...
export async function fetchAssociation(params) {
  ...
  return request(
    `${SRM_SQAM}/v1/${organizationId}/header-associates/${params.problemHeaderId}/assocaite/Deliver`,
    {
      method: 'GET',
    }
  );
...
export async function fetchListPrint(params) {
  const organizationId = getCurrentOrganizationId();
  return request(`${prefix}/${organizationId}/problem-headers/batch/print`, {
    method: 'POST',
    responseType: 'blob',
    body: params,
  });
 ...
 * @param {object} params
 */
export async function copyQualityRectification(body) {
  const organizationId = getCurrentOrganizationId();
  return request(`${prefix}/${organizationId}/problem-headers/copy`, {
    method: 'PUT',
    body,
  });
}
export async function syncExternalSystem(param) {
  const organizationId = getCurrentOrganizationId();
  return request(`${prefix}/${organizationId}/problem-headers/sync-external-system`, {
    method: 'POST',
    body: param,
  });
}
...
// 工作流校验
export async function validate8D(params) {
  const { customizeUnitCode, ...others } = params;
  return request(
    `${prefix}/${params.tenantId}/problem-headers/wlf/validate?customizeUnitCode=${customizeUnitCode}`,
    {
      method: 'PUT',
      body: others,
    }
...
// 撤回
export async function recall(param) {
  const { basicInfo, customizeUnitCode } = param;
  const organizationId = getCurrentOrganizationId();
  return request(`${prefix}/${organizationId}/problem-headers/undo?customizeUnitCode=${customizeUnitCode}`, {
    method: 'PUT',
    body: basicInfo,
  });
}
