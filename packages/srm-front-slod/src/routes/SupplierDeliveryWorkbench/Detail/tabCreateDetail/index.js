import React, { Fragment, useState, useMemo, useEffect, useLayoutEffect, useCallback } from 'react';
...
const TabCreate = (props) => {
  ...
  const formDs = useMemo(
    () =>
      new DataSet(
        remote
          ? remote.process(
              'SLOD_PRDETAIL_REMOTE_PROCESS_FORM_DS',
              headerInfoDataSet({
                ..._object,
                setAlertFlag,
                unitCode:
  ...
  const getlist = async () => {
    try {
      const res = await combineTabQuery({ nodeTemplateCode, nodeConfigId, cacheKey, campKey: 's' });
      if (getResponse(res)) {
        if (res && res?.length > 0) {
  ...
  // 销毁tab缓存数据
  const destroyEvent = () => {
    destroyChange({ nodeTemplateCode, nodeConfigId, cacheKey, campKey: 's' });
  };
  ...
  const operationChangeComposite = useCallback(
    async (_keys = {}, componentType) => {
      ...
      if (_keys?.key === 'instantly') {
        ...
        const paramsDel = { headerInfo, nodeTemplateCode, nodeConfigId, campKey };
        const params = onHandleWrapperAcquisitionData();
        if (componentType === 'delete') {
  ...
  const tabProps = {
    ...
    receiptsCod: ['UNIQUE_LABEL'].includes(nodeTemplateCode) ? 'LABEL' : nodeTemplateCode,
    queryChangeBack,
  };
    ...
    <Fragment>
      ...
      >
        ...
        {typeof cuxHanscncImportandExportBtns === 'function' &&
          nodeTemplateCode === 'ASN' &&
          cuxHanscncImportandExportBtns({ formDs, lineDs, lineItemDs, campKey: 's' })}
      </Header>
      <div
              ...
              <div id="detcontent" className={styles['det-content']}>
                {customizeCollapse(
                  ...
                  >
                    ...
                    <Panel
                      forceRender
                      hidden={nodeTemplateCode !== 'ASN' && waitCustomize}
