...
const lineListDataSet = ({ id, unitLineCode, nodeTemplateType, doubleUnitEnabled }) => ({
  ...
  fields: [
    ...
    // {
    //   name: 'unit_price', // 待创建-待提交 只有标签才有这个字段
    //   type: 'number',
    ...
    },
    {
      ...
      type: 'number',
      required: !['UNIQUE_LABEL'].includes(nodeTemplateType), // 送货-计划-要求必输
      label:
        nodeTemplateType === 'PLAN'
          ? doubleUnitEnabled
            ? intl
    ...
    },
    {
      ...
      type: 'number',
      required: ['UNIQUE_LABEL', 'LABEL'].includes(nodeTemplateType),
  ...
  events: {
    load: ({ dataSet }) => {
      ...
      const MarkerType = dataSet.getState('MarkerType') || 'left';
      // batchFlag 判断是否为勾选
      const fieldMapValues = dataSet.getState('fieldMapValues');
      if (fieldMapValues && MarkerType === 'left') {
        dataSet.forEach((i) => {
    ...
    read: ({ data, params: _p }) => {
      ...
      if (unitLineCode) {
        param = {
          ...
          cuszTplStageCode,
          cuszTplPageCode,
          operationType: 'waitSubmitDetail',
        };
      }
