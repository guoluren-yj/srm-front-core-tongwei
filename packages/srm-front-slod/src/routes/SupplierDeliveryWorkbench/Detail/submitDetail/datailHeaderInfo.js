...
const HeaderInfo = (props) => {
  const { formDs, headerId, nodeConfigId, nodeTemplateCode, customizeForm, remote = noop } = props;
  ...
  const getFields = () => {
    ...
    return remote
      ? remote.process('SLOD_DELIVERY_WORKBENCH_SUPPLIER_SUBMIT_DETAIL_PROCESS_PLAN_FORM', fields, {
          dataSet: formDs,
          nodeTemplateCode,
        })
      : fields;
...
const AsnHeaderShipmentsInfo = (props) => {
  ...
  const getFields = () => {
    ...
    return remote
      ? remote.process(
          'SLOD_DELIVERY_WORKBENCH_SUPPLIER_SUBMIT_DETAIL_PROCESS_SHIPMENT_FIELDS',
          fields,
          { formDs }
        )
      : fields;
