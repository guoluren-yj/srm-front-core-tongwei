...
const headerInfoDataSet = ({ id, unitCode, nodeTemplateType, handleInfoDataUpdate }) => ({
  ...
  fields: [
    ...
    },
    {
      ...
      dynamicProps: {
        // 根据状态（采购方已发布	PURCHASER_PUBLISHED、采购方已拒绝PURCHASER_REJECTED、采购方已反馈PURCHASER_FEEDBACK）可编辑，其他只读
        disabled: ({ record }) =>
  ...
  events: {
    load: ({ dataSet }) => {
      dataSet.forEach((record) => {
        record.set({ camp: 'SUPPLIER', pageProperty: '0', currentTabs: 'confirmed' });
      });
    },
...
const attachmentDataSet = () => ({
  ...
  events: {
    update: ({ dataSet, record, name }) => {
      ...
      if (['supplierAttachmentUuid', 'uuidNum'].includes(name)) {
        if (record.get('asnHeaderId')) {
          const data = {
            ...
            supplierAttachmentUuid: record?.get('supplierAttachmentUuid'),
          };
