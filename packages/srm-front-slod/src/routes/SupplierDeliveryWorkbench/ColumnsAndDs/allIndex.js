...
import RenderExportComp from '../../SupplierDeliveryWorkbench/components/ExportStatusComp';
import {
  colorRender,
...
const allColumns = (_object, props) => {
  ...
  const labelDanColumns = [
    ...
    },
    {
      ...
      width: 120,
      renderer: ({ value, record }) => (
        <RenderExportComp isByOrder value={value} record={record} summarization={summarization} remote={remote} />
      ),
    },
    {
      name: 'operating',
  ...
  const labelHangColumns = [
    ...
    },
    {
      ...
      width: 120,
      renderer: ({ value, record }) => (
        <RenderExportComp value={value} record={record} summarization={summarization} remote={remote} />
      ),
    },
    {
      name: 'projectTaskId',
