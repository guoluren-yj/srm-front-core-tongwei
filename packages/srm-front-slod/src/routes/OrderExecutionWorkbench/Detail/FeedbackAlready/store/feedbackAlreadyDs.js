...
const detailInfo = () => ({
    ...
    },
    {
      ...
      dynamicProps: {
        ...
        required: ({ record, dataSet }) =>
          dataSet.getState('doubleUnitEnabled') ? false : record.get('quantityEnableFlagConfirm'),
        disabled: ({ dataSet }) => {
          const basicInfoDs = dataSet.getState('basicInfoDs');
          const transactionMode = basicInfoDs?.current.get('transactionMode');
          return transactionMode === 'TRIPARTITE';
  ...
  queryParameter: {
    camp: 2,
    sortType: 4,
    poEntryPoint: 'ORDER_FEEDBACK_AGAIN',
  ...
  events: {
    load: ({ dataSet }) => {
      ...
      dataSet.forEach((i) => {
        ...
        Object.assign(i, {
          // status: 'update',
          selectable: ['CONFIRMED'].includes(i.get('displayStatusCode')),
        });
      });
    },
