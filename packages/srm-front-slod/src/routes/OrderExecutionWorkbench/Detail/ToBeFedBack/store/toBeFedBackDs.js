...
const basicInfo = () => ({
  ...
  fields: [
    ...
    {
      name: 'creationDate',
      label: intl.get('slod.orderExecution.model.common.creationDateTime').d('创建时间'),
      type: 'dateTime',
    },
    {
    ...
    },
    {
      ...
      label: intl.get('sodr.orderExecution.model.common.electricSignOrder').d('签署顺序'),
    },
    {
      name: 'electricSignStage',
      label: intl.get('sodr.orderExecution.model.common.electricSignStage').d('签署阶段'),
    },
    {
  ...
  queryParameter: {
    camp: 2,
    poEntryPoint: 'PURCHASE_PO_CONFIRM',
    customizeUnitCode: [
...
const detailInfo = () => ({
  ...
  fields: [
    ...
    },
    {
      ...
      dynamicProps: {
        ...
        // disabled: ({ record }) => !record.get('quantityEditFlag'),
        ...
        disabled: ({ dataSet }) => {
          ...
          return transactionMode === 'TRIPARTITE';
        },
      },
    },
    {
    ...
    },
    {
      ...
      dynamicProps: {
        disabled: ({ dataSet }) => {
          const basicInfoDs = dataSet.getState('basicInfoDs');
          const transactionMode = basicInfoDs?.current.get('transactionMode');
          return dataSet.getState('doubleUnitEnabled') || transactionMode === 'TRIPARTITE';
        },
        label: ({ dataSet }) =>
  ...
  events: {
    load: ({ dataSet }) => {
      ...
      dataSet.forEach((i) => {
        if (!i.getState('batchFlag') && fieldMap) {
          const valueList = Object.keys(fieldMap).map((n) => ({ key: n, value: fieldMap[n] }));
          valueList.forEach((item) => {
            const field = i.getField(item.key);
            const editor = editorRule(i, item.key);
            if (!field.disabled && editor && !field.get('isCustomizeText')) {
              i.set({ [item.key]: item.value });
            }
          });
        }
        Object.assign(i, {
          // 仅已发布，反馈审核拒绝可勾选
          status: 'update',
          selectable: ['DELIVERY_DATE_REJECT', 'PUBLISHED'].includes(i.get('displayStatusCode')),
        });
      });
    },
    update: ({ name, record, dataSet, value }) => {
      record.setState('edited', true);
...
const batchMaintenance = () => ({
  ...
  events: {
    // load: ({ dataSet }) => {
    //   dataSet.setState({ allBatchMap: null });
    // },
    // update: ({ dataSet, value, name }) => {
    //   const detailInfoDs = dataSet.getState('detailInfoDs');
    //   // 当前页不存在勾选时，清空之前的保存 批量变更信息
    //   const preBatchMap = dataSet.getState('allBatchMap') || {};
    //   detailInfoDs.setState({ allBatchMap: null });
    //   const allBatchMap = {
    //     ...preBatchMap,
    //     [name]: value,
    //   };
    //   if (isEmpty(detailInfoDs.selected)) {
    //     detailInfoDs.setState({ allBatchMap });
    //   } else {
    //     detailInfoDs.setState({ allBatchMap: null });
    //   }
    // },
...
const filterLine = ({ detailInfoDs }) => ({
  fields: [
    {
      name: 'lineDisplay',
      lookupCode: 'SPUC.SUPPLIER_CONFIRM.FILTERLINE',
      defaultValue: '0',
    },
  ],
  events: {
