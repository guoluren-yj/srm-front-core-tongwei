...
import { Modal, TextArea, Attachment, message } from 'choerodon-ui/pro';
  ...
  const empty = useMemo(
    () => (
      <div className={styles['modal-content-empty']}>
        {<EmptySvg />}
        <p>{intl.get('slod.orderExecution.view,message.noMessage').d('暂无留言')}</p>
      </div>
    ),
    []
  );
      ...
      if (input) {
        ...
        if (!isEmpty(trim(inputMessage))) {
          ...
          notification.warning({
            description: intl.get(`slod.orderExecution.view.message.needMessage`).d('消息不能为空'),
          });
        }
        return false;
      }
      ...
      >
        ...
        <div className={styles['wrapper-button-tab']}>
          ...
          <TextArea
            ...
            ref={input}
            placeholder={intl
              .get('slod.orderExecution.view.message.pleaseEnterMessage')
              .d('请输入留言')}
          />
        </div>
      </div>
...
const C7nMessage = (props) => {
  ...
  const handleOpen = () => {
    Modal.open({
      ...
      style: { width: 742 },
      title: intl.get('slod.orderExecution.view.button.messageBoard').d('留言板'),
