...
const combineTab = (res = {}, composite = {}, history) => {
  ...
  if (mergeFlag === 1) {
    history.push({
      pathname: `/slod/delivery-workbench/detail/tab-create`,
    ...
    history.push({
      pathname: `/slod/delivery-workbench/detail/create`,
 ...
 * @object _object
 * */
const handleDeleteList = (_object = {}) => {
...
const handleOffList = async (_object = {}) => {
  ...
  const { handleOffCloseCux = undefined } = remote.props?.process || {};
  const params = conditionList(_object, campKey);
  if (isFunction(handleOffCloseCux)) {
    const flag = await handleOffCloseCux(params);
    if (!flag) {
      return;
    }
  }
...
const handleExportList = async (_object = {}) => {
  ...
  const fileURL = URL.createObjectURL(file);
  // window.open(fileURL, '_self'); // 文件名后端传入乱码 自定义 不用open方法
...
const allDetailEntrance = (headerId = null, params = {}, props) => {
  ...
  params.history.push({
    pathname: `/slod/delivery-workbench/detail/${params.tabKey}`,
...
const unitCodeMapList = (summarization, tabKey, hdKey, customizeCode) => {
  const unitCodeMap = {
    'PLAN-create': `${customizeCode}.SEARCH.CREATE,${customizeCode}.CREATE`,
    'PLAN-submit': `${customizeCode}.SEARCH.SUBMIT,${customizeCode}.SUBMIT`,
    'PLAN-submit-right': `${customizeCode}.SEARCH.SUBMIT_H,${customizeCode}.SUBMIT_H`,
    'PLAN-affirm-left': `${customizeCode}.SEARCH.AFFIRM_D,${customizeCode}.AFFIRM_D`,
    'PLAN-affirm-right': `${customizeCode}.SEARCH.AFFIRM_H,${customizeCode}.AFFIRM_H`,
    'PLAN-all-left': `${customizeCode}.SEARCH.ALL_D,${customizeCode}.ALL_D`,
    'PLAN-all-right': `${customizeCode}.SEARCH.ALL_H,${customizeCode}.ALL_H`,
    'PLAN-all-date': `${customizeCode}.SEARCH.ALL_R,${customizeCode}.ALL_R`,
    'LABEL-create': `${customizeCode}.SEARCH.CREATE,${customizeCode}.CREATE`,
    'LABEL-submit': `${customizeCode}.SEARCH.SUBMIT,${customizeCode}.SUBMIT`,
    'LABEL-affirm-left': `${customizeCode}.SEARCH.AFFIRM_D,${customizeCode}.AFFIRM_D`,
    'LABEL-affirm-right': `${customizeCode}.SEARCH.AFFIRM_H,${customizeCode}.AFFIRM_H`,
    'LABEL-all-left': `${customizeCode}.SEARCH.ALL_D,${customizeCode}.ALL_D`,
    'LABEL-all-right': `${customizeCode}.SEARCH.ALL_H,${customizeCode}.ALL_H`,
    'ASN-create': `${customizeCode}.SEARCH.CREATE,${customizeCode}.CREATE`,
    'ASN-submit': `${customizeCode}.SEARCH.SUBMIT,${customizeCode}.SUBMIT`,
    'ASN-affirm-left': `${customizeCode}.SEARCH.AFFIRM_D,${customizeCode}.AFFIRM_D`,
    'ASN-affirm-right': `${customizeCode}.SEARCH.AFFIRM_H,${customizeCode}.AFFIRM_H`,
    'ASN-all-left': `${customizeCode}.SEARCH.ALL_D,${customizeCode}.ALL_D`,
    'ASN-all-right': `${customizeCode}.SEARCH.ALL_H,${customizeCode}.ALL_H`,
    'UNIQUE_LABEL-create': `${customizeCode}.SEARCH.CREATE,${customizeCode}.CREATE`,
    'UNIQUE_LABEL-submit': `${customizeCode}.SEARCH.SUBMIT,${customizeCode}.SUBMIT`,
    'UNIQUE_LABEL-affirm-left': `${customizeCode}.SEARCH.AFFIRM_D,${customizeCode}.AFFIRM_D`,
    'UNIQUE_LABEL-affirm-right': `${customizeCode}.SEARCH.AFFIRM_H,${customizeCode}.AFFIRM_H`,
    'UNIQUE_LABEL-all-left': `${customizeCode}.SEARCH.ALL_D,${customizeCode}.ALL_D`,
    'UNIQUE_LABEL-all-right': `${customizeCode}.SEARCH.ALL_H,${customizeCode}.ALL_H`,
  };
...
const getCustomize = () => {
  ...
  nodeTemplateCode.forEach((item) => {
    unitCodeMap.push(
      ...
      `SLOD.DELIVERY__WORKBENCH_${item}.SEARCH.CREATE`,
      `SLOD.DELIVERY__WORKBENCH_${item}.SEARCH.SUBMIT`,
      `SLOD.DELIVERY__WORKBENCH_${item}.SEARCH.AFFIRM_D`,
      `SLOD.DELIVERY__WORKBENCH_${item}.SEARCH.AFFIRM_H`,
      `SLOD.DELIVERY__WORKBENCH_${item}.SEARCH.AFFIRM_R`,
      `SLOD.DELIVERY__WORKBENCH_${item}.SEARCH.ALL_D`,
      `SLOD.DELIVERY__WORKBENCH_${item}.SEARCH.ALL_H`,
...
const tabCode = (tabKey, nodeTemplateCode, hdKey, customizeCode) => {
  ...
  switch (tabKey) {
    case 'create':
      tabSearchCode = `${customizeCode}.SEARCH.CREATE`;
      break;
    case 'submit':
      if (nodeTemplateCode === 'PLAN' && hdKey === 'right') {
        tabSearchCode = `${customizeCode}.SEARCH.SUBMIT_H`;
      } else {
        tabSearchCode = `${customizeCode}.SEARCH.SUBMIT`;
      }
      break;
    case 'affirm':
      tabSearchCode = `${customizeCode}.SEARCH.AFFIRM_${code}`;
      break;
    case 'all':
      tabSearchCode = `${customizeCode}.SEARCH.ALL_${code}`;
      break;
    default:
      tabSearchCode = `${customizeCode}.SEARCH.CREATE`;
      break;
