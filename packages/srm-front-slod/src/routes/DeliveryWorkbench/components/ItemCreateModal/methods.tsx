...
function lineColumns(): any{
  const lineCms = [
    ...
    },
    {
      ...
      editor: true,
      dynamicProps: {
        disabled: ({ record }) => record?.get('custLineId') || record?.get('leftCanCreateQuantity') === 0,
    ...
    dataSet.forEach((item, index, array) => {
      ...
      if (item.get('itemCode') === record.get('itemCode') && item.get('custLineId')) {
        ...
        if (array.length > 2) {
          ...
          console.log(`第${index+1}轮开始`)
          console.log("calculationResultsQuantity", calculationResultsQuantity);
          console.log(`第${index + 1}轮结束`);
