...
const headerInfoDataSet = ({
  ...
  fields: [
    ...
    },
    {
      ...
      dynamicProps: {
        disabled: ({ dataSet }) =>
          dataSet?.getState(`change`) ? isNil(dataSet?.getState(`purchaseRemark`)) : true,
      },
    },
    {
      name: 'supplierRemark',
  ...
  events: {
    load: async ({ dataSet }) => {
      ...
      dataSet.forEach((record) => {
        record.set({ camp: 'PURCHASER', currentTabs: 'all' });
        if (change) {
  ...
  transport: {
    read: ({ data }) => {
      ...
      return {
        url: `${SRM_SLOD}/v1/${organizationId}/delivery/${nodeTemplateCode}/${nodeConfigId}/detail/header/${headerId}?campKey=p`,
        method: 'GET',
        params,
        data: other,
      };
...
const logisticsDataSet = () => ({
  ...
  fields: [
    ...
    },
    {
      ...
      lookupCode: 'HPFM.IDD',
      dynamicProps: {
        disabled: ({ record }) => record.getField('logisticsPhoneNum').disabled,
