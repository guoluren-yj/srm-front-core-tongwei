...
// 基本信息
const headerInfoDataSet = ({ id, unitCode, nodeTemplateType, setAlertFlag }) => ({
  dataToJSON: 'all',
  primaryKey: id,
  paging: false,
  forceValidate: true,
  ...
  events: {
    load: ({ dataSet }) => {
      dataSet.forEach((record) => {
        ...
        record.set({ camp: 'PURCHASER', pageProperty: '0', currentTabs: 'created' });
      });
    },
    update: ({ name, record }) => {
      if (
