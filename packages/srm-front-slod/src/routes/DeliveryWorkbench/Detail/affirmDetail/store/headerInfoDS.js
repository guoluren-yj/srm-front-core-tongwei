...
import { getHeaderAttachmentUuid } from '@/services/DeliveryWorkbenchServices';
import { SLOD_DIRECTORY } from '@/utils/constant';

const organizationId = getCurrentOrganizationId();
// 基本信息
const headerInfoDataSet = ({ id, unitCode, nodeTemplateType, handleInfoDataUpdate }) => ({
  dataToJSON: 'all',
  primaryKey: id,
  paging: false,
  forceValidate: true,
  fields: [
    ...
    },
    {
      ...
      dynamicProps: {
        // 根据状态（供应商已发布SUPPLIER_PUBLISHED、供应商已拒绝SUPPLIER_REJECTED、供应商已反馈SUPPLIER_FEEDBACK）可编辑，其他只读
        disabled: ({ record }) =>
  ...
  events: {
    load: ({ dataSet }) => {
      dataSet.forEach((record) => {
        record.set({ camp: 'PURCHASER', pageProperty: '0', currentTabs: 'confirmed' });
      });
    },
  ...
  transport: {
    read: ({ data }) => {
      ...
      return {
        url: `${SRM_SLOD}/v1/${organizationId}/delivery/${nodeTemplateCode}/${nodeConfigId}/detail/header/${headerId}`,
        method: 'GET',
        params,
        data: other,
      };
...
const attachmentDataSet = () => ({
  ...
  events: {
    update: ({ dataSet, record, name }) => {
      const formRecord = dataSet?.getState('formDs')?.current;
      if (['purchaseAttachmentUuid', 'uuidNum'].includes(name)) {
        if (record.get('asnHeaderId')) {
          const data = {
            campKey: 'p',
            asnHeaderId: formRecord?.get('asnHeaderId'),
            objectVersionNumber: formRecord?.get('objectVersionNumber'),
            _token: formRecord?.get('_token'),
            statusCode: formRecord?.get('statusCode'),
            purchaseAttachmentUuid: record?.get('purchaseAttachmentUuid'),
          };
          getHeaderAttachmentUuid(data).then((res) => {
            if (getResponse(res)) {
              // eslint-disable-next-line no-unused-expressions
              formRecord?.init({
                purchaseAttachmentUuid: res.purchaseAttachmentUuid,
