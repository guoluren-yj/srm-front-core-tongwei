...
import { getActiveTabKey, closeTab, openTab } from 'utils/menuTab';
...
const IndexMenu = (props) => {
  ...
  const {
    nodeConfigId: configId = null,
    nodeTemplateCode: templateNodeCode = null,
    customerUnitCode: customCode = null,
  ...
  useMemo(() => {
    ...
    const code = ((templateNodeCode || props?.tableConfigRef?.nodeTemplateCode || summarization) ===
    'UNIQUE_LABEL'
      ? 'LABEL'
      : templateNodeCode || props?.tableConfigRef?.nodeTemplateCode || summarization
    )?.toLowerCase();
    ...
    if (!props?.tableConfigRef?.cache) {
      ...
      props.tableConfigRef.dataSet = {
        ...props?.tableConfigRef?.dataSet,
        ...(remote
          ? remote?.process('SLOD_DELIVERY_WORKBENCH_LIST_PROCESS_LIST_DATASET', null, {
              composite,
              props,
            })
          : {}),
  ...
  }, [composite.menuMarkId, composite.summarization]);

  useEffect(() => {
    menuChangeQuery();
    try {
      const divMenu = document?.getElementById('deliveryWorkbenchMenuId');
  ...
  const menuChangeQuery = async () => {
    ...
    try {
      const res = await menuChange({ campKey: 'p' });
        ...
        if (Array.isArray(res) && res[0]?.noPermissionTabFlag === 1) {
          return Modal.confirm({
            ...
            footer: () => {
              return (
                ...
                >
                  {intl.get('slod.deliveryWorkbench.view.title.offWeb').d('关闭页面')}
                </Button>
              );
            },
          });
          ...
          return Modal.confirm({
            ...
            children: (
              <span style={{ fontSize: 14 }}>
                {intl
                  .get('slod.deliveryWorkbench.view.title.tips')
                  .d(
                    '未配置发货节点，暂无可查询节点及数据,请前往<发货管理配置>菜单下维护节点数据,或者点击下方【引用推荐配置】按钮，快速创建节点操作；'
                  )}
              </span>
            ),
            okText: intl.get('hzero.common.button.sure').d('确定'),
            cancelText: intl.get('hzero.common.button.recommend').d('引用推荐配置'),
            cancelProps: {
            ...
            onOk: async () => {
              ...
              setTimeout(() => {
                openTab({
                  key: '/slod/shipments-configuration/list',
                  title: intl
                    .get('slod.deliveryManage.view.title.deliveryManage')
                    .d('发货管理配置'),
                  search: {},
          ...
          const nodeTemplateCode = templateNodeCode || props.tableConfigRef.nodeTemplateCode || key;
          const hdKey = ['PLAN'].includes(nodeTemplateCode)
            ? props.tableConfigRef.hdKey || 'right'
            : props.tableConfigRef.hdKey || 'left';
          const tabKey = defaultTabIndexFlag
            ? defaultTabIndex
            : props.tableConfigRef.tabKey || res[0].permissionTabList[0];
          props.tableConfigRef.query = {
            nodeTemplateCode,
            nodeConfigId: configId || props.tableConfigRef.nodeConfigId || id,
            hdKey,
            campKey: 'p', // 采购方标识 后端用来识别页面
          };
          props.tableConfigRef.hdKey = hdKey;
          props.tableConfigRef.menuTabList = res;
          props.tableConfigRef.customizeCode =
            customCode || props.tableConfigRef.customizeCode || customizeCode;
          props.tableConfigRef.allowQuery = true;
          const defaultTabIndexFlag = res[0].permissionTabList.includes(defaultTabIndex);
          useMenuTab({
            ...menuTab,
            // menuList: res,
            menuList: remote
              ? remote.process('SLOD_DELIVERY_WORKBENCH_LIST_PROCESS_MENU_LIST', res, {})
              : res,
          });
          useComposite({
            ...composite,
            ...
            menuMarkId: configId || defaultNodeConfigId || props.tableConfigRef.nodeConfigId || id,
            summarization: nodeTemplateCode,
            tabKey: remote
              ? remote.process('SLOD_DELIVERY_WORKBENCH_LIST_PROCESS_TAB_KEY', tabKey, {
                  menuTab,
                  composite,
                  props,
                })
              : tabKey,
            customizeCode: customCode || props.tableConfigRef.customizeCode || customizeCode,
          });
          loadingFlag(false);
          queryTabCountList({
            nodeConfigId: configId || props.tableConfigRef.nodeConfigId || id,
          ...
          queryCountList({ campKey: 'p' }).then((e) => {
            if (getResponse(e)) {
              setCountMenu(e);
            }
          });
  ...
  useEffect(() => {
    ...
    const code = templateNodeCode || props.tableConfigRef.nodeTemplateCode || summarization;
    const custCuzCode = customCode || props.tableConfigRef.customizeCode || customizeCode;
    const unitCuzCode = unitCodeMapList(code, tabKey, hdKey, custCuzCode);
  ...
  const menuClick = (_object) => {
    ...
    const tabCut = remote
      ? remote.process('SLOD_DELIVERY_WORKBENCH_LIST_PROCESS_TAB_CUT', permissionTabList[0], {
          menuTab,
          composite,
          props,
          _object,
    ...
    setTimeout(
      () =>
        [
          `${composite.customizeCode}.SEARCH.CREATE`,
          `${composite.customizeCode}.SEARCH.SUBMIT`,
          `${composite.customizeCode}.SEARCH.AFFIRM_D`,
          `${composite.customizeCode}.SEARCH.AFFIRM_H`,
          `${composite.customizeCode}.SEARCH.ALL_D`,
          `${composite.customizeCode}.SEARCH.ALL_H`,
          `${composite.customizeCode}.SEARCH.ALL_R`,
        ].forEach((item) => {
    ...
    setTimeout(() => setWaitCustomize(false), 100);
  };
  ...
  const queryTabCountList = async (_object) => {
    ...
    const code = nodeTemplateCode || props.tableConfigRef.nodeTemplateCode;
    const id = nodeConfigId || props.tableConfigRef.nodeConfigId;
  ...
  const HeaderBtns = observer(({ dataSet }) => {
    ...
    const btnArr = [
      {
        name: 'instantly',
        child: (name) =>
          name || intl.get('slod.deliveryWorkbench.view.title.selectCreate').d('勾选新建'),
        btnProps: {
      ...
      {
        name: 'all',
        child: (name) => name || intl.get('hzero.common.model.allCreate').d('全选新建'),
        btnProps: {
      ...
      create: [
        ...
        },
        {
          ...
          btnProps: {
            allBody: true,
            method: 'POST',
            templateCode: `SLOD_${code}_WAIT_CREATE_PURCHASER_EXPORT`,
            queryParams,
            requestUrl: `${SRM_SLOD}/v1/${tenantId}/delivery/${templateNode}/${menuMarkId}/wait-create/line/export?campKey=p`,
      ...
      submit: [
        ...
        },
        {
          ...
          btnProps: {
            allBody: true,
            method: 'POST',
            templateCode: 'SLOD_PLAN_WAIT_SUBMIT_LINE_PURCHASER_EXPORT',
            queryParams,
            requestUrl: `${SRM_SLOD}/v1/${tenantId}/delivery/${templateNode}/${menuMarkId}/wait-submit/${headerOrLine}/export?campKey=p`,
            buttonText: isEmpty(dataSet?.selected)
              ? intl.get(`slod.deliveryWorkbench.model.common.newExport`).d('导出')
              : intl.get(`slod.deliveryWorkbench.model.common.newCheckExport`).d('勾选导出'),
            otherButtonProps: {
      ...
      affirm: [
        ...
        },
        {
          ...
          btnProps: {
            ...
            templateCode:
              hdKey === 'left'
                ? 'SLOD_PLAN_WAIT_CONFIRM_HEADER_PURCHASER_EXPORT'
                : 'SLOD_PLAN_WAIT_CONFIRM_LINE_PURCHASER_EXPORT',
            queryParams,
            requestUrl: `${SRM_SLOD}/v1/${tenantId}/delivery/${templateNode}/${menuMarkId}/wait-confirm/${headerOrLine}/export?campKey=p`,
            buttonText: isEmpty(dataSet?.selected)
              ? intl.get(`slod.deliveryWorkbench.model.common.newExport`).d('导出')
              : intl.get(`slod.deliveryWorkbench.model.common.newCheckExport`).d('勾选导出'),
            otherButtonProps: {
      ...
      all: [
        ...
        },
        {
          ...
          hidden:
            ...
            (['ASN'].includes(composite?.summarization) && composite.hdKey !== 'left'),
          ...
          btnProps: {
            ...
            requestUrl: `${SRM_SLOD}/v1/${getCurrentOrganizationId()}/delivery/${templateNode}/batch-print-token?campKey=p`,
            method: 'POST',
        ...
        },
        {
          ...
          btnProps: {
            ...
            templateCode:
              headerOrLine === 'header'
                ? `SLOD_${code}_ALL_HEADER_PURCHASER_EXPORT`
                : `SLOD_${code}_ALL_LINE_PURCHASER_EXPORT`,
            queryParams,
            requestUrl: `${SRM_SLOD}/v1/${tenantId}/delivery/${templateNode}/${menuMarkId}/all/${headerOrLine}/export?campKey=p`,
            buttonText: isEmpty(dataSet?.selected)
              ? intl.get(`slod.deliveryWorkbench.model.common.newExport`).d('导出')
              : intl.get(`slod.deliveryWorkbench.model.common.newCheckExport`).d('勾选导出'),
            otherButtonProps: {
        ...
        },
        {
          name: 'printNew',
          child: (name) => name || intl.get('slod.deliveryWorkbench.model.view.print').d('打印'),
          hidden: !(composite.summarization === 'PLAN' && composite.hdKey === 'left'),
          childFor: 'buttonText',
          btnComp: PrintProButton,
          btnProps: {
            buttonProps: {
    ...
    const buttons = remote
      ? remote.process('SLOD_DELIVERY_WORKBENCH_LIST_HEADER_BTNS', button, {
          composite,
          dataSet,
        })
      : button;
    ...
    if (composite.tabKey === 'submit') {
      return customizeBtnGroup(
        ...
        <DynamicButtons
          buttons={buttons?.submit?.filter((i) => !i.hidden)}
          maxNum={5}
          defaultBtnType="c7n-pro"
        />
      );
    ...
    if (composite.tabKey === 'affirm') {
      return customizeBtnGroup(
        ...
        <DynamicButtons
          ...
          buttons={buttons?.affirm?.filter((i) => !i.hidden)}
          maxNum={5}
          defaultBtnType="c7n-pro"
          permissions={[
            {
              code:
                ['ASN'].includes(composite?.summarization) && ['left'].includes(composite.hdKey)
                  ? 'srm.logistics.delivery.work.bench.button.asn.cancelchange'
                  : ['PLAN'].includes(composite?.summarization) &&
                    ['left'].includes(composite.hdKey)
                  ? 'srm.logistics.delivery.work.bench.button.plan.cancelchange'
                  : '',
              name: 'cancelChange',
            },
          ]}
    ...
    if (composite.tabKey === 'all') {
      return customizeBtnGroup(
        ...
        <DynamicButtons
          key={composite.summarization || composite.tabKey || composite.hdKey}
          buttons={buttons?.all?.filter((i) => !i.hidden)}
          maxNum={5}
          defaultBtnType="c7n-pro"
          permissions={[
            {
              code: 'srm.logistics.delivery.work.bench.button.replenishprint',
              name: 'replenishPrint',
            },
            {
              code:
                ['ASN'].includes(composite?.summarization) && ['left'].includes(composite.hdKey)
                  ? 'srm.logistics.delivery.work.bench.button.printnew'
                  : ['LABEL'].includes(composite?.summarization)
                  ? 'srm.logistics.delivery.work.bench.button.label.printnew'
                  : ['PLAN'].includes(composite?.summarization)
                  ? `srm.logistics.delivery.work.bench.button.list.printnew`
                  : 'srm.logistics.delivery.work.bench.button.unique.label.printnew',
              name: 'printNew',
            },
            {
              code: ['ASN'].includes(composite?.summarization)
                ? 'srm.logistics.delivery.work.bench.button.print'
                : ['LABEL'].includes(composite?.summarization)
                ? 'srm.logistics.delivery.work.bench.button.label.print'
                : 'srm.logistics.delivery.work.bench.button.unique.label.print',
              name: 'print',
            },
          ]}
        />
    ...
    return remote
      ? remote.render('SLOD_DELIVERY_WORKBENCH_LIST_RENDER_OTHER_TAB_BTN', null, {
          menuTab,
          composite,
          props,
          loadingFlag,
  ...
  const count = (key) => {
    ...
    return remote
      ? remote.process('SLOD_DELIVERY_WORKBENCH_LIST_PROCESS_TABLE_NUM', num, { key })
      : num;
  };
  ...
  const tabList = remote
    ? remote.process(
        'SLOD_DELIVERY_WORKBENCH_LIST_PROCESS_TAB_LIST',
        tableViewList(
          props?.tableConfigRef?.permissionTabList || composite?.permissionTabList,
          props
        ),
        {
          composite,
  ...
  const listProps = {
    ...
    searchCode: !waitCustomize && searchCodeList,
  ...
  return (
    <Fragment>
      ...
      <div
        className={classNames('page-content')} // TODO
        id="deliveryWorkbenchMenuId" // TODO
        style={{
      ...
      >
        <Spin spinning={menuLoading}>
          {menuTab.menuList.length > 1 ? (
            <Layout>
              <div
                style={{ height: heightMenu > 400 ? heightMenu : '615px' }}
                                  ...
                                  >
                                    ...
                                    <div className={styles['text-style-text']}>
                                      ...
                                      >
                                        {remote ? (
                                          remote.render(
                                            'SLOD_DELIVERY_WORKBENCH_LIST_RENDER_COUNT',
                                            <CountMenu
                                              _obj={item}
                                              campKeys="p"
                                              countMenu={countMenu}
              ...
              <div
                style={{ height: heightMenu > 400 ? heightMenu : '615px' }} // TODO
                // id="deliveryWorkbenchMenuId" // TODO
                className={
                  styles[!composite.collapsed ? 'workbench-page-open' : 'workbench-page-off']
                }
              >
                <a
...
export default compose(
  ...
  withProps(
    ...
    { cacheState: true },
    { cacheKey: '/slod/delivery-workbench/list' }
  ),
  ...
  remoteFun(
    {
      code: 'SLOD_DELIVERY_WORKBENCH_LIST',
      name: 'remote',
    },
    {
      events: {
