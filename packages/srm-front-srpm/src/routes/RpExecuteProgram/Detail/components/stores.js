...
const lineDs = ({
  ...
  return {
    ...
    fields: [
      ...
      // },
      {
        label: intl.get(`${commonPrompt}.sourceNum`).d('来源单号'),
        name: 'rpNum',
        type: 'string',
      },
      {
        label: intl.get(`${commonPrompt}.splitLine`).d('是否拆分行'),
        name: 'vtSplitFlagMeaning',
      },
      {
        label: intl.get(`${commonPrompt}.splitLine`).d('是否拆分行'),
      ...
      },
      {
        ...
        dynamicProps: {
          disabled() {
            ...
            // const invOrganizationId = record.get('invOrganizationId')?.invOrganizationId;
          ...
          lovPara({ record }) {
            ...
            // // 库存组织
            // if (itemLimitRule.find(rule => rule === 'invOrganizationId')) {
            //     params.invOrganizationId = record.get('invOrganizationId')?.invOrganizationId;
            // }
            return params;
          },
        },
      ...
      {
        label: intl.get(`srpm.common.model.common.planQuantity`).d('平衡后数量'),
        name: 'quantity',
        type: 'number',
        validator(value) {
          if (value <= 0) {
      ...
      {
        name: 'vtHeaderRemark',
        label: intl.get(`${commonPrompt}.vtHeaderRemark`).d('头备注'),
      },
    ],
    transport: {
      read: ({ data = {} }) => {
        ...
        return {
          url: `${SRM_SRPM}/v1/${organizationId}/request-plan-balance/lines/${rpHeaderId ||
            currentId}`,
          data: {
            ...data,
            customizeUnitCode,
