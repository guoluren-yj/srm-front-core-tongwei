import React, { Fragment, useEffect, useState, useRef, memo, useMemo } from 'react'; // useEffect
...
import { queryDetail, cancel } from '@/services/RequisitionPlanServices';
...
const Index = ({
  ...
  const pubPathFlag = path.includes('/pub/srpm/requisition-plan/erp-only-read/');
  ...
  const [rpHeaderId] = useState(params ? params.id : modalRpHeaderId);
  const [headerInfo, setHeaderData] = useState({});
  ...
  useEffect(() => {
    ...
    if (isFunction(onLoad)) {
      onLoad({
        submit: handleWorkFlowCheck ? handleSubmit : undefined,
      });
    }
    return () => {
      if (isFunction(onLoad)) {
  ...
  const commonUpdate = (currpHeaderId) => {
    ...
    Promise.all([
      queryDetail({
        rpHeaderId,
        customizeUnitCode:
          'SRPM.RP_PLATFORM_ERP_READONLY.BASEINFO,SRPM.RP_PLATFORM_ERP_READONLY.PURCHASEORGINFO,SRPM.RP_PLATFORM_ERP_READONLY.ATTACHMENT,SRPM.RP_PLATFORM_ERP_READONLY.EXTERNALFILE',
  ...
  const toEditPage = () => {
    history.push({
      pathname: `/srpm/requisition-plan/erp-edit/${rpHeaderId}`,
      search: 'back=read',
    });
  };
  ...
  const headerBtn = () => {
    ...
    if (headerInfo?.rpStatus === 'APPROVED' && headerInfo?.cancelStatusCode !== 'CANCELLED') {
    ...
    return (
      <>
        {customizeBtnGroup(
          {
            code: 'SRPM.RP_PLATFORM_ERP_READONLY.BTNS',
            pro: true,
          },
          <DynamicButtons
            buttons={!pubPathFlag ? headerButtons : pubHeaderBtns}
            maxNum={5}
            ...
            <div className={classnames(maintainStyles['rfx-detail-list-card'])}>
              <Content className={maintainStyles['custom-page-content']}>
                ...
                <Base
                  ...
                  code="SRPM.RP_PLATFORM_ERP_READONLY.BASEINFO"
                />
              </Content>
            </div>
            <div className={classnames(maintainStyles['rfx-detail-list-card'])}>
              <Content className={maintainStyles['custom-page-content']}>
                ...
                <PurchaseOrgInfo
                  ...
                  handleDetailField={handleDetailField}
                  customizeForm={customizeForm}
                  code="SRPM.RP_PLATFORM_ERP_READONLY.PURCHASEORGINFO"
                />
              </Content>
            </div>
            <div className={classnames(maintainStyles['rfx-detail-list-card'])}>
              <Content className={maintainStyles['custom-page-content']}>
                ...
                <DemandLine
                  ...
                  code="SRPM.RP_PLATFORM_ERP_READONLY.LINEINFO"
              ...
              >
                <div>
                  ...
                  <div className={classnames(maintainStyles['sprm-workspace-attachment'])}>
                    {customizeForm(
                      {
                        code: 'SRPM.RP_PLATFORM_ERP_READONLY.ATTACHMENT', // 必传，和unitCode一一对应
                        dataSet: attachDs,
                ...
                <div>
                  ...
                  <div className={classnames(maintainStyles['sprm-workspace-attachment'])}>
                    {customizeForm(
                      {
                        code: 'SRPM.RP_PLATFORM_ERP_READONLY.EXTERNALFILE', // 必传，和unitCode一一对应
                        dataSet: attachDs,
                        custLoading: headerLoading,
                      },
                      <Form columns={1} labelLayout="float" dataSet={attachDs}>
...
export default compose(
  ...
  WithCustomizeC7N({
    unitCode: [
      'SRPM.RP_PLATFORM_ERP_READONLY.BASEINFO',
      'SRPM.RP_PLATFORM_ERP_READONLY.LINEINFO',
      'SRPM.RP_PLATFORM_ERP_READONLY.PURCHASEORGINFO',
      'SRPM.RP_PLATFORM_ERP_READONLY.ATTACHMENT',
      'SRPM.RP_PLATFORM_ERP_READONLY.EXTERNALFILE',
      'SRPM.RP_PLATFORM_ERP_READONLY.BTNS',
    ],
  }),
  cuxRemote(
    {
