  ...
  return (
    <Fragment>
      <div style={{ height: 'calc(100vh - 253px)' }}>
        {customizeTable(
          {
            code: 'SRPM.RP_PLATFORM.APPROVED.LIST',
          },
          <SearchBarTable
            style={{ maxHeight: `calc(100% - 22px)` }}
            searchCode="SRPM.RP_PLATFORM.APPROVED_SEARCHBAR"
            dataSet={lineDs}
            columns={lineColumns}
            data={[]}
            cacheState
            searchBarConfig={{
              editorProps: {
                rpStatus: {
                  optionsFilter: options => ['APPROVED'].includes(options.get('value')),
                },
              },
              fieldProps: {
...
export default memo(
  withCustomize({
    unitCode: ['SRPM.RP_PLATFORM.APPROVED.LIST'],
  })(Index)
);
