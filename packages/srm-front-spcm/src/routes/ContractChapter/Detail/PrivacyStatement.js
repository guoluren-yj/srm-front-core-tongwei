  ...
  return (
    <>
      ...
      <div className="protocol-content">
        <p>
          {intl
            .get(`hzero.common.pushMessage.content`, {
              authTypeMeaning,
            })
            .d(
              `我们通过第三方（${authTypeMeaning}）进行实名认证，根据第三方实名认证的要求，您需要提供（姓名，证件号，联系方式等）。`
            ) +
            intl
              .get('hzero.common.pushMessage.content1')
              .d(
                '我们会在收集阶段就加密您所提交的信息并发送给第三方。整个过程中我们不会存储和使用您提交用于实名认证的信息。在提交您的信息给第三方之后，我们就会销毁您提交的数据。您如果有任何疑问，请通过项目经理或运维经理联系我们。'
              ) +
            intl
