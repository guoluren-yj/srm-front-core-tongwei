...
export default class UserModal extends PureComponent {
  ...
  render() {
    ...
    const columns = [
      {
        title: intl.get('entity.user.code').d('账号'),
        dataIndex: 'loginName',
        width: 150,
      },
      {
        title: intl.get('entity.user.name').d('名称'),
        dataIndex: 'realName',
      },
      // {
      //   title: intl.get('entity.user.loginName').d('用户账号'),
      //   dataIndex: 'loginName',
      //   width: 150,
      // },
    ];
    return (
      ...
      >
        <Form layout="inline" style={{ marginBottom: 16 }}>
          <Form.Item label={intl.get('entity.user.code').d('账号')}>
            {getFieldDecorator('loginName', {})(<Input trim inputChinese={false} />)}
          </Form.Item>
          <Form.Item label={intl.get('entity.user.name').d('名称')}>
            {getFieldDecorator('realName', {})(<Input />)}
          </Form.Item>
          {/* <Form.Item label={intl.get('entity.user.loginName').d('用户账号')}>
