...
export default class PromptForm extends React.Component {
  ...
  constructor(props) {
    super(props);
    const { languageRequired } = window.dvaApp._store.getState().global || {};
    this.languageRequired = [...(languageRequired || {}).lang] || [];
  }
                  ...
                  <Col {...FORM_COL_2_LAYOUT}>
                    ...
                    >
                      {getFieldDecorator(`promptConfigs[${item.code}]`, {
                        ...
                        rules: [
                          {
                            required: this.languageRequired.includes(item.code),
                            message: intl.get('hzero.common.validation.notNull', {
