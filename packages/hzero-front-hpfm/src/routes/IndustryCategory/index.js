...
import { Button, Col, Form, Input, Modal, Row, Table, Tabs } from 'hzero-ui';
...
export default class IndustryCategory extends Component {
  ...
  saveData(fieldsValue, form, level) {
    ...
    const { topIndustry, secondIndustry, domesticFlag } = this.state;
    if (level === 1) {
      dispatch({
  ...
  updateData(fieldsValue, form, level) {
    const { dispatch, tenantId } = this.props;
    const { editRowData, topIndustry, secondIndustry, domesticFlag } = this.state;
    if (level === 1) {
      dispatch({
  ...
  @Bind()
  handleChange(domesticFlag) {
    this.setState({ domesticFlag }, () => this.queryTopIndustry());
  }
      ...
      <React.Fragment>
        ...
        <Content>
          ...
          >
            <TabPane
              tab={intl
                .get('hpfm.industryCategory.view.message.title.nationalStandard')
                .d('国标品类')}
              key={1}
            >
              <Row gutter={24}>
            ...
            <TabPane
              tab={intl
                .get('hpfm.industryCategory.view.message.title.internationalStandard')
                .d('国际品类')}
              key={0}
            >
              <Row gutter={24}>
