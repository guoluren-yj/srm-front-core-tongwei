...
export default class LovSetting extends React.Component {
  ...
  getExportQueryParams() {
    const form = this.searchForm && this.searchForm.props && this.searchForm.props.form;
    const params = isUndefined(form) ? {} : form.getFieldsValue();
    const filterValues = filterNullValueObject(params);
    return filterValues;
  }
  ...
  renderHeader() {
    ...
    const { match, isSiteFlag, currentTenantId } = this.props;
    return (
      <Header title={intl.get('hpfm.lov.view.message.title.lovSetting').d('值集视图配置')}>
        ...
        {!isSiteFlag && (
          <ExcelExportPro
            ...
            requestUrl={`${HZERO_PLATFORM}/v1/${currentTenantId}/lov-view/view-export`}
