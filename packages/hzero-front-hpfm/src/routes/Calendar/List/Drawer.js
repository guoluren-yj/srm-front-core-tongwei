...
import { Form, Input, Modal, Select, Icon, Tooltip } from 'hzero-ui';
      ...
      >
        <Form>
          ...
          >
            ...
            })(<TLEditor field="calendarName" token={targetItem._token}/>)}
          </Form.Item>
          <Form.Item
          ...
          >
            {getFieldDecorator('companyId', {
              ...
              <Lov
                ...
                disabled={!isUndefined(targetItem.calendarId)}
                textValue={targetItem.companyName}
                textField="companyName"
                onChange={(value) => {
                  if (value) {
                    setFieldsValue({ defaultFlag: '0' });
                  }
                }}
              />
            )}
          ...
          <Form.Item
            label={
              <span>
                {intl.get('smdm.taxRateOrg.model.taxRate.isDefault').d('是否默认')}&nbsp;
                <Tooltip
                  title={intl
                    .get('hpfm.calendar.model.calendar.matchingRules')
                    .d('租户级启用日历定义后若匹配不到单据上公司对应的日历则读取默认日历')}
                >
          ...
          >
            {getFieldDecorator('defaultFlag', {
              initialValue:
                targetItem.defaultFlag || targetItem.defaultFlag === 0
                  ? String(targetItem.defaultFlag)
                  : undefined,
              rules: [
                {
                  required: true,
                  message: intl.get('hzero.common.validation.notNull', {
                    name: intl.get('smdm.taxRateOrg.model.taxRate.isDefault').d('是否默认'),
                  }),
                },
              ],
            })(
          ...
          >
            ...
            })(<Switch disabled={[1, '1'].includes(getFieldValue('defaultFlag'))} />)}
          </Form.Item>
        </Form>
      </Modal>
    );
