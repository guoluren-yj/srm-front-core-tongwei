...
export default class ConfigDetail extends Component {
            ...
            >
              ...
              {!isSite && (
                <Tabs.TabPane
                  tab={
                    <React.Fragment>
                      <div className={styles['config-content-tab-theme']}>
                        <Icon type="auto_fix_high" />
                        <span>{intl.get('hpfm.config.view.title.theme').d('主题配置')}</span>
                      </div>
                    </React.Fragment>
                  }
                  key="theme"
                >
                  <Theme config={config} wrappedComponentRef={this.themeRef} />
                </Tabs.TabPane>
              )}
  ...
  handleSave() {
    ...
    if (this.themeRef.current && activeKey === 'theme') {
      this.themeRef.current.saveThemeConfig();
    }
      ...
      dispatch({
        ...
        if (res) {
          ...
          if (this.themeRef.current) {
            this.themeRef.current.props.form.resetFields();
          }
          ...
          if (res.length > 0 && activeKey === 'system') {
            ...
            const timeZone = { ...res.filter((item) => item.configCode === 'TIMEZONE')[0] };
            const popoutReminderFlag = {
              ...res.filter((item) => item.configCode === 'POPOUT_REMINDER_FLAG')[0],
            };
            // const loginAgreement = {
            //   ...res.filter((item) => item.configCode === 'LOGIN_AGREEMENT')[0],
            // };
            const pageAsyncFlag = {
              ...res.filter((item) => item.configCode === 'PAGE_ASYNC_FLAG')[0],
            };
            // const iaeTimeZoneConvert = {
            //   ...res.filter((item) => item.configCode === 'IMPORT_EXPORT_ZONE_CONVERT')[0],
            // };
            dispatch({
              ...
              payload: {
                ...
                // timeZone: timeZone.configValue,
                popoutReminderFlag: popoutReminderFlag.configValue,
                // loginAgreement: loginAgreement.configValue,
                pageAsyncFlag: pageAsyncFlag.configValue,
                // iaeTimeZoneConvert: iaeTimeZoneConvert.configValue,
              },
            });
          }
        }
      ...
      Object.keys(obj).forEach((item) => {
        switch (item) {
          ...
          case 'logo': {
            ...
            if (obj.logoFileToken) {
              logoItem._fileToken = obj.logoFileToken;
            }
            values.push(logoItem);
            break;
          }
          case 'favicon': {
            const faviconItem = {
            ...
            if (obj.faviconFileToken) {
              faviconItem._fileToken = obj.faviconFileToken;
            }
            values.push(faviconItem);
            break;
          }
          case 'menuLayout':
            values.push({
          ...
          case 'loginLogo': {
            ...
            if (obj.loginLogoFileToken) {
              loginLogoItem._fileToken = obj.loginLogoFileToken;
            }
            values.push(loginLogoItem);
            break;
          }
          case 'languageFlag':
            values.push({
          ...
          case 'timeZone':
            values.push({
              ...
              configCode: 'TIMEZONE',
              tenantId: organizationId,
              ...this.findConfig('TIMEZONE', data),
              configValue: obj.timeZone,
            });
            break;
          case 'popoutReminderFlag':
            values.push({
              category: 'system',
              configCode: 'POPOUT_REMINDER_FLAG',
              tenantId: organizationId,
              ...this.findConfig('POPOUT_REMINDER_FLAG', data),
              configValue: obj.popoutReminderFlag,
            });
            break;
          // case 'loginAgreement':
          //   values.push({
          //     category: 'system',
          //     configCode: 'LOGIN_AGREEMENT',
          //     tenantId: organizationId,
          //     ...this.findConfig('LOGIN_AGREEMENT', data),
          //     configValue: obj.loginAgreement,
          //   });
          //   break;
          case 'pageAsyncFlag':
            values.push({
              category: 'system',
              configCode: 'PAGE_ASYNC_FLAG',
              tenantId: organizationId,
              ...this.findConfig('PAGE_ASYNC_FLAG', data),
              configValue: obj.pageAsyncFlag,
            });
            break;
          case 'iaeTimeZoneConvert':
            values.push({
              category: 'system',
              configCode: 'IMPORT_EXPORT_ZONE_CONVERT',
              tenantId: organizationId,
              ...this.findConfig('IMPORT_EXPORT_ZONE_CONVERT', data),
              configValue: obj.iaeTimeZoneConvert,
            });
            break;
          default:
            break;
