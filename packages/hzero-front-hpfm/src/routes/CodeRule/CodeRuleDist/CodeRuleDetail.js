  ...
  return (
    ...
    >
      <React.Fragment>
        {!isNil(ruleDetailId) && (
          <Form.Item>
            {form.getFieldDecorator('ruleDetailId', {
              initialValue: ruleDetailId,
            })}
          </Form.Item>
        )}
        <FormItem
        ...
        >
          ...
          })(<InputNumber disabled={editRecordData.usedFlag} min={1} style={{ width: '300px' }} />)}
        </FormItem>
        <FormItem
          {...formLayout}
        ...
        {handleVisible.resetFrequency && (
          ...
          >
            {form.getFieldDecorator('resetFrequency', {
              ...
              <Select style={{ width: '300px' }} disabled={editRecordData.usedFlag}>
            ...
            >
              {form.getFieldDecorator('startValue', {
                ...
                <InputNumber
                  style={{ width: '300px' }}
                  min={1}
                  disabled={editRecordData.usedFlag}
                />
              )}
            </FormItem>
            <FormItem
              {...formLayout}
              label={intl.get('hpfm.codeRule.model.codeRule.currentValue').d('当前值')}
            >
              {form.getFieldDecorator('currentValue', {
                initialValue: currentValue,
                rules: [
                  {
                    required: handleVisible.currentValue,
                    message: intl.get('hzero.common.validation.notNull', {
                      name: intl.get('hpfm.codeRule.model.codeRule.currentValue').d('当前值'),
                    }),
                  },
                ],
              })(
                <InputNumber
                  style={{ width: '300px' }}
                  min={form.getFieldValue('startValue') || 1}
                />
              )}
            </FormItem>
          </>
        ...
        {(handleVisible.encryptedFlag || form.getFieldValue('fieldType') === 'SEQUENCE') && (
          ...
          >
            ...
            })(<Checkbox checkedValue disabled={editRecordData.usedFlag} />)}
          </FormItem>
        )}
      </React.Fragment>
    </Modal>
...
export default class CodeRuleDetail extends PureComponent {
   ...
   */
  componentDidMount() {
    const { codeRule: { code = {} } = {} } = this.props;
    this.setState({ fieldTypeList: code.FieldType || [] });
  }
        ...
        render: (_, record) => {
          ...
          if (usedFlag) {
            operators.push({
              ...
              ele: (
                <a
                  style={editFlag ? {} : { color: '#ccc', cursor: 'default' }}
                  onClick={() => {
                    if (editFlag) {
                      this.edit(record);
                    }
  ...
  handleAddCodeDetail(fieldsValue, form) {
    const {
      dispatch,
      editRecordData,
      codeRule: { organizationId, keyValue, detail },
    } = this.props;
    const { editDetailData } = this.state;
    const { _token, objectVersionNumber, ruleDetailId } = editDetailData;
    if (editRecordData.usedFlag === 1) {
      let newContent =
        detail && detail.data.content && detail.data.content.length ? detail.data.content : [];
      newContent = newContent.map((item) => {
        if (item.ruleDetailId && item.ruleDetailId === fieldsValue.ruleDetailId) {
          return {
            ...item,
            ...fieldsValue,
          };
      ...
      dispatch({
        type: 'codeRule/updateDetailData',
        payload: {
          content: newContent,
        },
      });
    ...
    if (record) {
      ...
      this.setState({
        ...
        handleVisible: {
          ...
          currentValue: !!record.startValue || record.startValue === 0,
        },
      });
    } else {
      this.setState({
  ...
  @Bind()
  handleCancelEditUsedData() {
    this.props.dispatch({
      type: 'codeRule/restoreDetailData',
    });
    this.setState({
      editFlag: false,
    });
  ...
  handleSaveUsedData() {
    ...
    const data = {
      ...
      ruleCode: head.ruleCode,
      level: head.ruleLevel,
      levelCode: editRecordData.levelCode,
      levelValue: editRecordData.levelValue,
      ruleDetailList: detail.data.content,
    };
    this.props
      .dispatch({
        type: 'codeRule/saveCodeRuleDetail',
        payload: data,
      })
      .then((res) => {
        if (res) {
  ...
  @Bind()
  renderTableOperator() {
    const { editFlag, selectedRows } = this.state;
    const {
      match,
      editRecordData,
      codeRule: {
    ...
    if (!usedFlag) {
      ...
      return (
        <>
          <Button onClick={this.handleCancelEditUsedData}>
            {intl.get('hzero.common.button.cancel').d('取消')}
          </Button>
          <Button type="primary" onClick={this.handleSaveUsedData}>
            {intl.get('hzero.common.button.save').d('保存')}
          </Button>
        </>
      );
    } else if (!isTenantRoleLevel() && getCurrentOrganizationId() !== head.tenantId) {
      // 对于已使用的编码规则， 平台级且规则是租户的数据才能编辑
      return (
        <Tooltip
          title={intl
            .get('hpfm.codeRule.view.tooltip.editToolitip')
            .d('该操作会更改租户编码规则，请与项目确认后更改！')}
          placement="topLeft"
        >
          <ButtonPermission
            type="primary"
            permissionList={[
              {
                code: `hzero.site.sys.code-rule.button.detailuseddata`,
                type: 'button',
                meaning: '编码规则详情层-编辑已使用编码',
              },
            ]}
            onClick={this.handleEditUsedData}
          >
            {intl.get('hzero.common.button.edit').d('编辑')}
          </ButtonPermission>
        </Tooltip>
      ...
      >
        <React.Fragment>
          ...
          <Content>
            ...
            <div className="table-operator">{this.renderTableOperator()}</div>
            <Table
              bordered
              pagination={false}
              rowKey="ruleDetailId"
