...
import { DataSet, Form, Icon, TextArea, TextField, Tooltip, Switch, Lov, Button, Modal } from 'choerodon-ui/pro';
...
import { checkMQHealth } from '@/services/configService';
...
const RefreshPermission = ({ match }) => {
  ...
  const refreshTimeZoneDs = React.useMemo(() => new DataSet(refreshTimeZoneDS()), []);
  ...
  const handleFreshTimeZoneConfig = React.useCallback(async () => {
    setLoading({
      ...loadingProxy.loading,
      [buildRefreshPermissionLoadingName('hiam', 'switch-timeZone-config')]: true,
    });
    const valid = await refreshTimeZoneDs.validate();
    if (valid) {
      try {
        await refreshTimeZoneDs.submit();
      } catch (e) {
        // do nothing, ds noticed
      }
    }
    setLoading({
      ...loadingProxy.loading,
      [buildRefreshPermissionLoadingName('hiam', 'switch-timeZone-config')]: false,
    });
  }, [refreshTimeZoneDs]);
  ...
  const handleCheckMQ = async() => {
    const res = await checkMQHealth();
    if (getResponse(res) && res.message) {
      Modal.info({
        autoCenter: true,
        children: <div dangerouslySetInnerHTML={{ __html: res.message }} />,
      });
    }
  };
    ...
    <>
      ...
      <Card
        ...
        title={
          <h3>
            {intl
              .get('hpfm.sysTools.view.message.title.switchTimeZoneConfig')
              .d('租户切换新时区配置权限')}
          </h3>
        }
      >
        <Popconfirm
          title={intl
            .get('hpfm.sysTools.view.message.switchTimeZoneConfig.confirm')
            .d('确认刷新租户切换新时区配置权限吗')}
          onConfirm={() => {
            handleFreshTimeZoneConfig();
          }}
        >
          <ButtonPermission
            icon="sync"
            permissionList={[
              {
                code: `${match.path}.button.switchTimeZoneConfig`,
                type: 'button',
                meaning: '系统工具-刷新权限-租户切换新时区配置权限',
              },
            ]}
            loading={loading[buildRefreshPermissionLoadingName('hiam', 'switch-timeZone-config')]}
          >
            {intl.get('hzero.common.button.refresh').d('刷新')}
          </ButtonPermission>
        </Popconfirm>
        <Form columns={3} dataSet={refreshTimeZoneDs} labelWidth={140}>
          <Lov colSpan={2} name="tenantNum" />
        </Form>
      </Card>
      {isAdmin && (
        <Card
          ...
          title={
            <h3>
              {intl
                .get('hpfm.sysTools.view.message.title.thirdComponentCheck')
                .d('三方组件检查')}
            </h3>
          }
        >
          <Button onClick={handleCheckMQ}>
            {intl.get('hpfm.sysTools.view.button.MQ').d('MQ组件')}
          </Button>
        </Card>
      )}
      {/* <Card
