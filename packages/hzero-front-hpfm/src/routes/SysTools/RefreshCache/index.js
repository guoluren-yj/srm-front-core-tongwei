  ...
  const refreshCacheProxyCall = React.useCallback((module, code) => {
    ...
    if (refreshCacheAxionsConfig[module] && refreshCacheAxionsConfig[module][code]) {
      axios(refreshCacheAxionsConfig[module][code]())
        .then((res) => {
          if (isUndefined(res) || res === true || res === "") {
            setLoading({
              ...loadingProxy.loading,
        ...
        <Row {...EDIT_FORM_ROW_LAYOUT}>
          <Col {...FORM_COL_3_LAYOUT}>
            <Popconfirm
              title={intl
                .get('hpfm.sysTools.view.message.refreshCache', {
                  action: intl.get('hpfm.sysTools.view.button.refreshCache.hpfm.lov').d('刷新值集'),
                })
                .d(
                  `是否${intl
                    .get('hpfm.sysTools.view.button.refreshCache.hpfm.lov')
                    .d('刷新值集')}缓存`
                )}
              onConfirm={() => {
                refreshCacheProxyCall('hpfm', 'lov');
              }}
            >
              <ButtonPermission
                style={btnStyle}
                ...
                permissionList={[
                  {
                    code: `${match.path}.button.refreshCache.hpfm.lov`,
                    type: 'button',
                    meaning: '系统工具-刷新缓存-刷新值集',
                  },
                ]}
                loading={loading[buildRefreshCacheLoadingName('hpfm', 'lov')]}
              >
                {intl.get('hpfm.sysTools.view.button.refreshCache.hpfm.lov').d('刷新值集')}
              </ButtonPermission>
            </Popconfirm>
          </Col>
          <Col {...FORM_COL_3_LAYOUT}>
            <Popconfirm
              title={intl
                .get('hpfm.sysTools.view.message.refreshCache', {
                  action: intl.get('hpfm.sysTools.view.button.refreshCache.hpfm.msgService').d('刷新消息服务'),
                })
                .d(
                  `是否${intl
                    .get('hpfm.sysTools.view.button.refreshCache.hpfm.msgService')
                    .d('刷新消息服务')}缓存`
                )}
              onConfirm={() => {
                // 借用hpfm的逻辑，实际服务是hmsg
                refreshCacheProxyCall('hpfm', 'msg-service');
              }}
            >
              <ButtonPermission
                style={btnStyle}
                ...
                permissionList={[
                  {
                    code: `${match.path}.button.refreshCache.hpfm.msgService`,
                    type: 'button',
                    meaning: '系统工具-刷新缓存-刷新消息服务',
                  },
                ]}
                loading={loading[buildRefreshCacheLoadingName('hpfm', 'msg-service')]}
              >
                {intl.get('hpfm.sysTools.view.button.refreshCache.hpfm.msgService').d('刷新消息服务')}
              </ButtonPermission>
            </Popconfirm>
          </Col>
          <Col {...FORM_COL_3_LAYOUT}>
            <Popconfirm
              title={intl
                .get('hpfm.sysTools.view.message.refreshCache', {
                  action: intl
                    .get('hpfm.sysTools.view.button.refreshCache.hmde.model')
                    .d('刷新模型'),
                })
                .d(
                  `是否${intl
              ...
              onConfirm={() => {
                refreshCacheProxyCall('hmde', 'model');
              }}
            >
              <ButtonPermission
                style={btnStyle}
                ...
                permissionList={[
                  {
                    code: `${match.path}.button.refreshCache.hmde.model`,
                    type: 'button',
                    meaning: '系统工具-刷新缓存-刷新模型缓存',
                  },
                ]}
                loading={loading[buildRefreshCacheLoadingName('hmde', 'model')]}
              >
                {intl.get('hpfm.sysTools.view.button.refreshCache.hmde.model').d('刷新模型')}
              </ButtonPermission>
            </Popconfirm>
          </Col>
        </Row>
          ...
          <Col {...FORM_COL_3_LAYOUT}>
            <Popconfirm
              title={intl
                .get('hpfm.sysTools.view.message.refreshCache', {
                  action: intl
                    .get('hpfm.sysTools.view.button.refreshCache.hmsg.tenantQueue')
                    .d('同步租户消息队列'),
                })
                .d(
                  `是否${intl
              ...
              onConfirm={() => {
                refreshCacheProxyCall('hmsg', 'tenantQueue');
              }}
            >
              <ButtonPermission
                style={btnStyle}
                ...
                permissionList={[
                  {
                    code: `${match.path}.button.refreshCache.hmsg.tenantQueue`,
                    type: 'button',
                    meaning: '系统工具-刷新缓存-同步租户消息队列',
                  },
                ]}
                loading={loading[buildRefreshCacheLoadingName('hmsg', 'tenantQueue')]}
              >
                {intl.get('hpfm.sysTools.view.button.refreshCache.hmsg.tenantQueue').d('同步租户消息队列')}
              </ButtonPermission>
            </Popconfirm>
          </Col>
        </Row>
