...
export default class ListTable extends Component {
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: intl.get(`${promptCode}.model.dashboard.menuCode`).d('条目跳转路由'),
        width: 100,
        dataIndex: 'menuCode',
      },
      {
        title: intl.get(`${promptCode}.model.dashboard.route`).d('条目后端接口'),
        width: 100,
        dataIndex: 'route',
      },
      {
      ...
      },
      {
        ...
        render: (_, record) => {
          ...
          if (!isTenantRoleLevel() && record.dataTenantLevel === 'TENANT') {
            actions.push(
              <Tooltip title={intl.get(`${promptCode}.view.message.title.tenantAssign`).d('分配租户')}>
