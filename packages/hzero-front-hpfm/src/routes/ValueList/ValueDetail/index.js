...
export default class EditForm extends Component {
  ...
  @Bind()
  handleChangeTranslationMethod(event) {
    const {
      form: { setFieldsValue },
    } = this.props;
    const method = event && event.target && event.target.value;
    if (method === 'SQL') {
      setFieldsValue({ translationUrl: undefined });
    } else if (method === 'URL') {
      setFieldsValue({ translationSql: undefined });
    }
  }
  ...
  render() {
    ...
    const rowSelection = {
      selectedRowKeys: selectedRows.map((n) => `${n.lovValueId}-${n.parentValue}`),
            ...
            <Form>
              <Row {...EDIT_FORM_ROW_LAYOUT}>
                ...
                {!isTenant && tenantId === currentTenantId && (
                  <Col {...FORM_COL_3_LAYOUT}>
                    <Form.Item
                      {...EDIT_FORM_ITEM_LAYOUT}
                      label={intl
                        .get('hpfm.valueList.model.header.limitCustomFlag')
                        .d('是否标准值集')}
                    >
                      {getFieldDecorator('limitCustomFlag', {
                        initialValue: valueListHeader.limitCustomFlag,
                      })(<Switch />)}
                    </Form.Item>
                  </Col>
                )}
                ...
                <Col {...FORM_COL_3_LAYOUT}>
                  <Form.Item
                    {...EDIT_FORM_ITEM_LAYOUT}
                    label={intl.get('hpfm.valueList.model.header.valueField').d('值字段')}
                  >
                    {isTenant && isNotCurrentTenant
                ...
                <Col {...FORM_COL_3_LAYOUT}>
                  <Form.Item
                    {...EDIT_FORM_ITEM_LAYOUT}
                    label={intl.get('hpfm.valueList.model.header.displayField').d('显示字段')}
                  >
                    {isTenant && isNotCurrentTenant
                ...
                {['URL', 'SQL'].includes(valueListHeader.lovTypeCode) && (
                  <Col {...FORM_COL_3_LAYOUT}>
                    <Form.Item
                      {...EDIT_FORM_ITEM_LAYOUT}
                      label={
                        ...
                        >
                          <Text>
                            {intl.get('hpfm.valueList.model.header.labelCode').d('值集使用方')}
                          </Text>
                          <Tooltip
                            title={intl
                    ...
                    >
                      {getFieldDecorator('labelCode', {
                        initialValue: valueListHeader.labelCode,
                      })(<span>{valueListHeader.labelCodeMeaning}</span>)}
                    </Form.Item>
                  </Col>
                )}
                ...
                <>
                  ...
                  >
                    <Col {...FORM_COL_2_LAYOUT}>
                      <Form.Item
                        {...EDIT_FORM_ITEM_LAYOUT_COL_2}
                        label={intl
                          .get('hpfm.valueList.model.header.translationMethod')
                          .d('翻译方式')}
                      >
                        {isTenant && isNotCurrentTenant
                          ? valueListHeader.translationMethod || ''
                          : getFieldDecorator('translationMethod', {
                              rules: [
                                {
                                  required: true,
                                  message: intl.get('hzero.common.validation.notNull', {
                              ...
                              initialValue: valueListHeader.translationMethod || 'SQL',
                            })(
                              <RadioGroup
                                className={FORM_FIELD_CLASSNAME}
                                onChange={this.handleChangeTranslationMethod}
                              >
                    ...
                    <Col {...FORM_COL_2_LAYOUT}>
                      <Form.Item
                        ...
                        style={{
                          display:
                            ['URL', 'SQL'].includes(valueListHeader.lovTypeCode) &&
                            getFieldValue('translationMethod') === 'SQL'
                              ? 'block'
                              : 'none',
                        }}
                      >
                  ...
                  >
                    <Col {...FORM_COL_2_LAYOUT}>
                      <Form.Item
                        {...EDIT_FORM_ITEM_LAYOUT_COL_2}
                        label={intl.get('hpfm.valueList.model.header.translationUrl').d('翻译 URL')}
                        style={{
                          display: getFieldValue('translationMethod') === 'URL' ? 'block' : 'none',
                        }}
                      >
                        {isTenant && isNotCurrentTenant
                          ? valueListHeader.translationUrl || ''
                          : getFieldDecorator('translationUrl', {
                              initialValue: valueListHeader.translationUrl || '',
                            })(<Input className={FORM_FIELD_CLASSNAME} />)}
                      </Form.Item>
                    </Col>
                  </Row>
          ...
          {valueListHeader.lovTypeCode === 'IDP' ? (
            ...
            >
              ...
              <Table
                ...
                loading={loading}
                // rowKey="lovValueId"
                rowKey={(record) => `${record.lovValueId}-${record.parentValue}`}
