...
import { HZERO_IAM, HZERO_PLATFORM, HZERO_MSG } from 'utils/config';
...
export const sysToolsCacheAxiosConfig = () => ({
  hpfm: {
    ...
    lov: () => ({
      url: `${HZERO_PLATFORM}/v1/lov-cache/clean`,
      method: 'DELETE',
      data: {},
    }),
    'msg-service': () => ({
      url: `${HZERO_MSG}/v1/message-template-cache/clean`,
      method: 'DELETE',
      data: {},
    }),
  },
  ...
  hmde: {
    model: () => ({
      url: `/hmde/v1/cache?useScan=true`,
      data: {},
      method: 'DELETE',
    }),
  },
  hmsg: {
    tenantQueue: () => ({
      url: `/hmsg/v1/message-tenant-queue/refresh`,
      method: 'GET',
    }),
  },
});
...
export const refreshTimeZoneDS = () => ({
  autoCreate: true,
  fields: [
    {
      label: intl.get('hpfm.sysTools.model.refreshPermission.tenant').d('租户'),
      name: 'tenantNum',
  ...
  transport: {
    submit: ({ data: [data] }) => {
      return {
        url: `${HZERO_IAM}/v1/tenants/time-zone-to-daylight?tenantNum=${data.tenantNum}`,
        method: 'POST',
        data: null,
      };
    },
