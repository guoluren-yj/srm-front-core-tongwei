...
export async function searchAll(params) {
  ...
  return request(`${prefix}/${params.tenantId}/units/department/tree`, {
    method: 'GET',
    query: filterNullValueObject(parsedParams),
  });
}
/**
 * 根据查询条件获取部门信息
 ...
 * @returns
 */
export async function queryCostCenterData(params) {
  const query = filterNullValueObject(parseParameters({ ...params }));
  return request(`/smdm/v1/lovs/sql/data?lovCode=SPRM.COST_CENTER`, {
    method: 'GET',
    query,
  });
}
 ...
 * @returns {object} fetch Promise
 */
export async function queryRelatedInformation({ unitId, customizeUnitCode }) {
  return request(
    `/spfm/v1/${organizationId}/hr/unit/line-attr/${unitId}?customizeUnitCode=${customizeUnitCode}`,
    {
      method: 'GET',
 ...
 * @returns {object} fetch Promise
 */
export async function deleteRelatedInformation(unitId, list, customizeUnitCode) {
  return request(
    `/spfm/v1/${organizationId}/hr/unit/line-attr/${unitId}?customizeUnitCode=${customizeUnitCode}`,
    {
      method: 'DELETE',
 ...
 * @returns {object} fetch Promise
 */
export async function updateRelatedInformation(unitId, list, customizeUnitCode) {
  return request(
    `/spfm/v1/${organizationId}/hr/unit/line-attr/${unitId}?customizeUnitCode=${customizeUnitCode}`,
    {
      method: 'PUT',
