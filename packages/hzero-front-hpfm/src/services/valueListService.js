...
export async function insertLovHeaders(params) {
  return request(
    `${HZERO_PLATFORM}/v1${isTenantRoleLevel() ? `/${tenantId}/` : '/'}lov-headers`,
    {
      method: 'POST',
      body: params,
    },
...
export async function exportLov(params, data) {
  return request(`${urlPrefix(params)}lov/idp/export`, {
    method: 'POST',
    responseType: 'text',
    body: data,
  });
...
export async function importLov(params) {
  return request(`${urlPrefix({ tenantId: getCurrentOrganizationId() })}lov/idp/import`, {
    method: 'POST',
    body: params,
    responseType: 'text',
  });
...
export async function queryImportLovLog() {
  return request(`${urlPrefix({ tenantId: getCurrentOrganizationId() })}lov/idp/log`, {
    method: 'POST',
  });
}
