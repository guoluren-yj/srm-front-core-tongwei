...
export default {
  ...
  effects: {
    ...
    *queryLovHeadersList({ payload }, { call, put, all }) {
      ...
      const { lovType, requestMethods, labelType } = getResponse(lovTypeResult);
      const lovTypeFilter = getResponse(lovTypeFilterResult);
      const pagination = createPagination(list);
      yield put({
        type: 'updateState',
        payload: { list, lovType, requestMethods, labelType, lovTypeFilter, pagination },
      });
    },
    // 根据 ID 查询值集头
