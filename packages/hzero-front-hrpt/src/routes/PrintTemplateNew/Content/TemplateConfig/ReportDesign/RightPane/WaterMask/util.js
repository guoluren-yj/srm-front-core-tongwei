...
export const createWatermarkItem = (options) => {
  ...
  const mask_div = document.createElement(isImg ? 'img' : 'div');
...
export const createWatermark = (container, options) => {
  const { enableFlag, type, content } = options;
  if (!container) { return; }
  // 先清空
  container.querySelectorAll(`.${MarkClassName}`).forEach(el => {
    el.remove();
  });
  if (enableFlag === false || !content) {
    return;
  }
  ...
  container.appendChild(mask_div);
  const { width: itemWidth, height: itemHeight } = mask_div.getBoundingClientRect();
