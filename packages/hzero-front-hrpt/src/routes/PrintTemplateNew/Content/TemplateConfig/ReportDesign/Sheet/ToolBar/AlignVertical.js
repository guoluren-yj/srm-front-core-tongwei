...
const clsPrefix = 'sheet-toolbar-align-vertical';

export default function AlignVertical({ cell, item, sheetRef, disabled }) {
  ...
  useEffect(() => {
    ...
    if (target) {
      ...
      const defaultOption = options.find((option) => Number(option.value) === 0);
      if (defaultOption) {
  ...
  const setCellStyle = useCallback(
    (newValue) => {
      sheetRef.current.updateFormat('vt', Number(!isNil(newValue) ? newValue : value));
    },
    [value]
  );
    ...
    () => (
      <Menu
        ...
        className={classnames(
          ...
          styles['sheet-toolbar-align-vertical-menu']
        )}
      >
        {options.map((option) => (
    ...
    <Tooltip title={title}>
      ...
      >
        <div onClick={() => setCellStyle()}>
          {icon && <img src={require(`@/assets/sheet/${icon}.svg`)} />}
        </div>
