...
export default class StorageDrawer extends React.PureComponent {
  ...
  render() {
    ...
    const { FILE_PHYSICS_DELETE_FLAG } = getEnvConfig();
      ...
      >
        <Spin spinning={initLoading}>
          <Form>
            ...
            <Form.Item
              label={intl.get('hfile.storage.model.storage.directoryStrategy').d('文件目录策略')}
              {...formItemLayout}
            >
              {getFieldDecorator('directoryStrategy', {
                initialValue: directoryStrategy,
              })(
                <Select allowClear>
                  {directoryStrategyList.map((item) => (
                    <Select.Option key={item.value} value={item.value}>
                      {item.meaning}
                    </Select.Option>
                  ))}
            ...
            {FILE_PHYSICS_DELETE_FLAG === 'true' && (
              <Form.Item
                label={intl.get('hfile.storage.model.storage.physicsDeleteFlag').d('物理删除')}
                {...formItemLayout}
              >
                {getFieldDecorator('physicsDeleteFlag', {
                  initialValue: isNil(physicsDeleteFlag) ? 0 : physicsDeleteFlag,
                })(<Switch />)}
              </Form.Item>
            )}
          </Form>
