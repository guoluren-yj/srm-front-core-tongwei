...
export default class TableList extends PureComponent {
  ...
  @Bind()
  reviewFile(record) {
    const fileExtMatch = record.fileUrl.match(/(.[^.]+)$/);
    const fileExt = fileExtMatch ? fileExtMatch[1] : '';
    window.open(
      `${HZERO_FILE}/v1/${record.tenantId}/${
        newPreviewList.includes(fileExt) ? 'file/preview' : 'file-preview/by-url'
      }?url=${encodeURIComponent(record.fileUrl)}&bucketName=${record.bucketName}${
