...
const drawerDS = () => ({
  ...
  fields: [
    ...
    },
    {
      ...
      dynamicProps: {
        required: ({ record }) =>
          record && ['TEXT', 'TILE_TEXT'].includes(record.get('watermarkType')),
      },
    },
    {
      name: 'detail',
      ...
      dynamicProps: {
        required: ({ record }) =>
          record && ['IMAGE', 'TILE_IMAGE'].includes(record.get('watermarkType')),
      },
    },
    {
      name: 'fontUrl',
  ...
  transport: {
    ...
    create: ({ data }) => {
      const { __id, _status, watermarkType, detail1, detail, ...other } = Array.isArray(data)
        ? data[0]
        : {};
      return {
        url: `${apiPrefix}/watermark-configs`,
        ...
        data: {
          ...other,
          watermarkType,
          detail: watermarkType === 'IMAGE' || watermarkType === 'TILE_IMAGE' ? detail : detail1,
        },
      };
    },
    update: ({ data }) => {
