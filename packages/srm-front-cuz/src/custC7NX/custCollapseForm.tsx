...
import { fieldNameFx, getFieldValueObject, renderCheckBox } from '../customizeTool';
import {
  assignOrderToField,
  assignRowColToField,
  parseNoneLayoutNode,
...
export default function custCollapseForm(
  this: Customize,
...
function makeNewEditor(
  ...
  if (!oldChild && visible !== -1) {
    ...
    if (labelLayout === 'vertical' && !noRenderer) {
      fieldProps.renderer = ({ name, record, text }) => {
        ...
        if (['INPUT_NUMBER', 'CURRENCY'].includes(fieldType!)) {
          return (Number(visualValue), numberPrecision, !!allowThousandth, true);
        }
    ...
    if (fieldType === 'CHECKBOX' || fieldType === 'SWITCH') {
      ...
      fieldProps.renderer = ({ value }) => numberRender(value, numberPrecision, !!allowThousandth, true);
    }
