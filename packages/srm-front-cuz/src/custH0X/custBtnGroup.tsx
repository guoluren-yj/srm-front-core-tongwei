...
import { useComputed } from '../custC7NX/hooks';
...
export default function custBtnGroup(
  ...
  if (this.manualQuery && this.state.loading) return btnGroup;
...
const ObserverWrapper: FunctionComponent<{
  ...
  fields.forEach((i) => {
    ...
    if (btn) {
      ...
      newChildren.push(
        <Button onClick={onClick} loading={loadingStatus[fieldCode]}>
          <span>
            {fieldName}
            {helpNode}
          </span>
...
const ObserverWrapperPro: FunctionComponent<{
  ...
  const memoColumns = useComputed(() => {
    const childrenMap = new Map<string, FieldPlainMap>();
    const newColumns: string[] = [];
    const groupColumns: string[][] = [];
    parseNestColumnns(childrenMap, buttons);
    const tools = { cache, code, ctxParams };
    fields.forEach((i) => {
      const {
        fieldName,
      ...
      if (btn) {
        ...
        const newColumn: ColumnProps = {
          ...
          text: fieldName,
          btnProps: { onClick, btnType, loading: loadingStatus[fieldCode] },
          btnType,
        };
    ...
    return toNestColumns(newColumns, childrenMap).reverse();
    /**
...
type FieldPlainMap = {
  column: ColumnProps;
  parent?: FieldPlainMap;
};

function parseNestColumnns(
  plainMap: Map<string, FieldPlainMap>,
  originColumns: Array<ColumnProps>,
  parent?: FieldPlainMap
) {
  originColumns.forEach((column) => {
    if (!column) return;
    const { name = '', group, children, ...others } = column;
    const currentField: FieldPlainMap = {
      column: {
        name,
        group,
        ...others,
...
function setColumn(plainMap: Map<string, FieldPlainMap>, column: ColumnProps) {
...
function setColumnParent(plainMap: Map<string, FieldPlainMap>, name: string, parent: string) {
...
function toNestColumns(columnsSeq: string[], plainMap: Map<string, FieldPlainMap>) {
  const finalColumns: ColumnProps[] = [];
  columnsSeq.forEach((field) => {
    const col = plainMap.get(field);
    if (col) {
      if (col.parent) {
        if (!col.parent.column.children) col.parent.column.children = [];
        col.parent.column.children.push(col.column);
      } else finalColumns.push(col.column);
    }
    plainMap.delete(field);
  });
  plainMap.forEach((col) => {
    if (col.parent) {
