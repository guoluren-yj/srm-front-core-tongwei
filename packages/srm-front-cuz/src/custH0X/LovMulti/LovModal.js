...
const __HAS_CHILDREN__ = '__HAS_CHILDREN__';
const DEFAULT_CHILDREN_FIELD_NAME = 'children';
...
export default class LovMultiModal extends React.Component {
  ...
  componentDidMount() {
    ...
    if (values.length > 0) {
      ...
      this.translate(values, this.setCurrentRightRecords);
    }
    this.mounted = true;
  }
  ...
  componentDidUpdate() {
    ...
    if (!this.firstLoadFinshed && !isEmpty(lov)) {
      ...
      if (tableFields.length > 0) {
        ...
        this.setState({ scrollX: scrollX > 580 ? scrollX : 580 });
      }
  ...
  @Bind()
  processQueryParams(pagination, prefix = 'left', options) {
    const { onlyFormField } = options || {};
    ...
    if (onlyFormField) {
      return { queryField, queryValue: filterNullValueObject(filter) };
    }
  ...
  queryLovDataByPost(url, params) {
    ...
    return getResponse(
      request(url, {
        method: 'POST',
        body: others,
        query: { page, size, asyncCountFlag: 'N' },
      })
    );
  }
  ...
  @Bind()
  setHasChildren = (data, childName) => {
    return childName && isArray(data) ? data.map((n) => {
      if (!isEmpty(n[childName])) {
        // eslint-disable-next-line no-undef
        n[__HAS_CHILDREN__] = true;
        this.setHasChildren(n[childName], childName);
      }
      return n;
    }) : data;
  }
  ...
  dataFilter(data) {
    ...
    const list = isTree ? this.setChildren(data, childrenFieldName) : this.setHasChildren(data.content, childrenFieldName || DEFAULT_CHILDREN_FIELD_NAME);
  ...
  async getSelectedData() {
    ...
    const meaningMap = {};
    // 只有再第一次全选时取filtersRow中的数据才有意义，此判断是为了少调用一次翻译接口而已
    if (filterRows.length > 0 && filterRows.length === selectedKeys.length) {
  ...
  selectToRight() {
    ...
    const addLeftRows = processTreeData(list).filter((i) => newLeftKeys.includes(this.parseField(i, valueField)));
    const newTranslateValues =
      translateValues.concat(
        addLeftRows.filter(i => {
          const valueFieldV = this.parseField(i, valueField);
          return isNil(valueFieldV) || !translateValues.find(j => this.parseField(j, valueField) === valueFieldV);
        })
      );
    if (newLeftKeys.length > 0) {
      ...
      if (disableTranslate) {
        ...
        this.setState({
          ...
          translateValues: newTranslateValues,
        });
      } else {
        this.setState({
          leftSelectedKeys: [],
  ...
  unSelectRight() {
    ...
    const leftKeys = processTreeData(list).map((i) => this.parseField(i, valueField));
    ...
    const newTranslateValues = translateValues.filter((i) => {
      const value = this.parseField(i, valueField);
      return !rightSelectedKeys.includes(value);
    });
    ...
    this.splitGroupSelect({
      resolve: (data) => {
        ...
        this.setState({
          ...
          translateValues: this.state.translateValues.concat(data),
          translateFlag: true,
        });
      },
      onError: () => {
      ...
      queryFun(url, queryParams)
        .then((res) => {
          if (getResponse(res)) {
            ...
            if (Number(res.number) < Number(res.totalPages)) {
              select({ ...queryParams, page: Number(res.number) + 1 });
  ...
  onSearchSelected() {
    ...
    const { queryField, queryValue } = this.processQueryParams({}, 'right', { onlyFormField: true });
    const { translateValues, queryFields } = this.state;
    if (!translateValues.length) {
      this.setState({ filterRows: [] });
      return;
    } else if (queryField && queryValue && !isNil(queryValue[queryField])){
      const queryFieldType = queryFields && queryFields[queryField] && queryFields[queryField].dataType;
      this.setState({
        filterRows:
          translateValues.filter(i => {
            if (isNil(i[queryField])) {
              return false;
            }
            return ['INT', 'DATE', 'DATETIME', 'SELECT'].includes(queryFieldType) ?
              i[queryField] === queryValue[queryField]
              : i[queryField].includes(queryValue[queryField]);
          }),
      });
    } else {
      this.setState({ filterRows: translateValues });
    }
  }
  ...
  async translate(values, onSuccess) {
    ...
    if (this.state.translateFlag) {
      const { translateValues } = this.state;
      if (translateValues && translateValues.length) {
        const resArr = values.map((i) => {
          const v = translateValues.find(j => this.parseField(j, valueField) === i) || {};
    ...
    return request(
      ...
      .then((res) => {
        if (getResponse(res) && res) {
          this.setState({ translateFlag: true, translateValues: Object.values(res) });
  ...
  render() {
    ...
    const rowSelection = {
      ...
      getCheckboxProps: (record) => {
        ...
        if (onlySelectLastFlag && record[__HAS_CHILDREN__]) {
          newProps.disabled = true;
        }
        return newProps;
      },
      ...
      <div className={styles['lovmulti-wrapper']}>
        <div className="transfer-wrapper">
          ...
          <div className="control">
            <Tooltip
              title={intl.get('hzero.common.button.selectAllByQuery').d('选中符合条件的所有数据（总数据量超过2000时不可用）')}
            >
              <Button onClick={this.selectAllByQuery} loading={selectAllLoading} disabled={Number.isNaN(pagination.total) || Number(pagination.total) > 2000}>
                <Icon type="double-right" />
              </Button>
            </Tooltip>
            <Button onClick={this.selectToRight} disabled={leftSelectedKeys.length === 0}>
              <Icon type="right" />
            </Button>
