...
export default class LovMulti extends React.Component<LovProps, any> {
  // 选中记录
  ...
  componentDidUpdate(prevProps) {
    const { displayData, value, viewOnly } = this.props;
    const { translateData } = this.props;

    if (
      value !== prevProps.value ||
      displayData !== prevProps.displayData ||
      viewOnly !== prevProps.viewOnly
    ) {
      this.initDisplayData(value, displayData, translateData);
    }
  }
  ...
  async selectAndClose() {
    ...
    typeof onChange === 'function' && onChange(value, meaningMap);
  ...
  onSearchBtnClick(): void {
    ...
    if ((!viewOnly && disabled) || this.loading) return; // 节流
    ...
    (queryLov({ viewCode, tenantId })
      .then((oriLov) => {
        const lov = { ...oriLov, queryUsePost: oriLov && oriLov.lovTypeCode === "IDP" ? false : queryUsePost };
        if (customValueField) {
