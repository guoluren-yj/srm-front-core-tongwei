...
export default class LovMultiModal extends React.Component {
  ...
  render() {
    ...
    const rightPagination = {
      ...
      total: disableTranslate ? filterRows.length : selectedKeys.length
    };
    if (!disableTranslate) {
      rightPagination.total = selectedKeys.length;
    }
    const rightScroll = { x: scrollX, y: 365 };
    ...
    return (
      <div className={styles['lovmulti-wrapper']}>
        {queryFields.length > 0 ? (
          <Form style={{ display: 'flex', marginBottom: '10px', alignItems: 'flex-start' }}>
