...
export default function custCollapse(this: Customize, options: Options, collapse: ReactElement) {
  ...
  if (this.manualQuery && this.state.loading) {
    return null;
  }
...
const ObserverWrapper: FunctionComponent<{
  ...
  fields.forEach((i) => {
    ...
    if (visible === 1 && !isStandardField && !targetPane) {
      ...
      targetPane = (
        // NOTICE: 正常使用h0-ui的这个class可以忽略，加的原因是一些自定义样式冲突，需要单独处理
        <Collapse.Panel className='cust-ext-collapse' key={fieldCode} header={extHeader} forceRender>
    ...
    if (targetPane.props) {
      ...
      if (typeof oldHeader === 'function') {
        ...
        paneProps.header = extPanelHeader ? extPanelHeader(fieldName) : <h3 style={{display: "inline-block"}}>{fieldName}</h3>;
      }
    }
