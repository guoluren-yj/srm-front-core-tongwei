...
class Compiler {
  ...
  importContext(name) {
    ...
    if (!has(contextMap, name) && ignore.indexOf(name) === -1) {
      if (has(internal, name)) {
        ...
        value = `${IMPORTS}.${name}`;
      } else if (this.options.useDataGets) {
        // 在使用dataGets方式获取数据的时候，采用生成Proxy的方式获取单元字段
        value = `getProxyObject(${DATA}, "${name}")`
      } else {
        value = `${DATA}.${name}`;
      }
  ...
  build() {
    ...
    stacks.push(`function(${DATA}, rowKey){`);
    stacks.push(`'use strict'`);
    stacks.push(`${DATA}=${DATA}||{}`);
    if (this.options.useDataGets) {
      stacks.push(`function getProxyObject(gets, name) { return new Proxy({}, {get(_, property){return gets[name](property, rowKey);}, set(){}}); }`);
    }
