...
import { getResponse } from '@/utils/utils'; // getUrlParam
...
function SupplierBlacklistManage(props = {}) {
  ...
  const [localPermissionList, setPermissionList] = useState(false);
  ...
  useEffect(() => {
    ...
    checkPermission([
      'risk-workbench-new.api.blackList-addBusiness',
      'risk-workbench-new.api.blackList-import',
    ]).then(res => {
      if (getResponse(res)) {
        setPermissionList(res.length ? res : []);
      }
    });
  ...
  // 校验权限集
  const checkIfPermission = (code = '') => {
    let checkFlag = false;
    if (localPermissionList.length) {
      localPermissionList.forEach(item => {
        if (item.code === code) {
          checkFlag = item.approve;
        }
      });
    }
  ...
  const buttons = [
    checkIfPermission('risk-workbench-new.api.blackList-import') && {
      name: 'import',
  ...
  const hasPermission = checkIfPermission('risk-workbench-new.api.blackList-addBusiness');

  const menu = () => {
    return disabledBtn || !hasPermission ? null : (
      <Menu>
        <Menu.Item>
    ...
    <>
      <Header
        title={intl
          ...
          .d('黑名单管理')}
        backPath="/sdat/new-risk-scan-workbench/list"
      >
        <Dropdown overlay={menu} disabled={disabledBtn || !hasPermission}>
          <Button disabled={disabledBtn || !hasPermission} icon="add" color="primary">
        ...
        <ReactExportButton
          ...
          permissionList={[{ code: 'risk-workbench-new.api.blackList-export' }]}
        />
        {customizeBtnGroup(
