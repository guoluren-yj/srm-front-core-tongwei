...
export default function CustomizedTable(props) {
  ...
  const viewBusinessMap = record => {
    if (record.get('dataId')) {
      history.push(
        `/sdat/risk-workbench-new/scheme-config/supplier-blacklist-manage/detail/${record.get(
          'dataId'
        )}`
      );
    }
      ...
      renderer: ({ record }) => (
        <>
          {!record?.getState('editing') && (
            <PermissionButton
              permissionList={[{ code: 'risk-workbench-new.api.blackList-remove' }]}
              type="text"
              color="primary"
          ...
          {!record?.getState('editing') ? (
            <PermissionButton
              ...
              permissionList={[{ code: 'risk-workbench-new.api.blackList-edit' }]}
              type="text"
              color="primary"
              style={{ marginRight: '16px' }}
            >
            ...
            <PermissionButton
              ...
              permissionList={[{ code: 'risk-workbench-new.api.blackList-save' }]}
              type="text"
              color="primary"
              style={{ marginRight: '16px' }}
            >
          ...
          {!record?.getState('editing') ? (
            <PermissionButton
              ...
              permissionList={[{ code: 'risk-workbench-new.api.viewMap' }]}
              type="text"
              style={{ marginRight: '16px' }}
            >
