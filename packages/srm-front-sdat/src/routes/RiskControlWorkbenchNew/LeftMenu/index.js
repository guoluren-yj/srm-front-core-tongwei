...
const LeftMenu = forwardRef((props, ref) => {
  ...
  useEffect(() => {
    if (statusList.length && levelList.length) {
      ...
      if (arr && arr.length) {
        defaultOpen = arr.map((item) => item.itemCode);
      }

      onChangeFilter({
        statusList: [...cacheStatusList],
        levelList: [...cacheLevelList],
        codeList: [...cacheCodeList],
      });
  ...
  const handleChangeEvent = (e, codeStr = '') => {
    ...
    const changeTree = (data = [], check) => {
      data.forEach((item) => {
        ...
        if (check && (!item.children || !item.children.length)) cacheCodes.push(item.itemCode);
        if (!check && (!item.children || !item.children.length)) {
          const index = cacheCodes.indexOf(item.itemCode);
          if (index >= 0) {
    ...
    const loopTree = (list, check, code) => {
      list.forEach((item) => {
        if (item.itemCode === code) {
          ...
          if (item.children && item.children.length) {
            changeTree(item.children, check);
          }
        } else if (item.children && item.children.length) {
          loopTree(item.children, check, code);
        }
      });
    };
      ...
      if (list && list.length) {
        list.forEach((item) => {
          if (item.themeId === parentId) {
            ...
            changeParent(item.children, parentId, check);
          }
        });
      }
    };
    ...
    const changeStatus = (list) => {
      list.forEach((item) => {
        // 判断当前节点的下一级 是否都选中
        if (item.children && item.children.length) {
          const checkedList = item.children.filter((item2) => item2.checked); // 选中的列表
    ...
    return (list || []).map((item) => {
      return (
        <Panel
          header={
            <span>
              <CheckBox
                ...
                onChange={(e) => handleChangeEvent(e, item.itemCode)}
              >
                {item.itemName}
              </CheckBox>
            </span>
          ...
          key={item.itemCode}
        >
          {item.children && item.children.length ? drawSubTree(item.children) : null}
        </Panel>
      );
    });
  };
  ...
  const drawSubTree = (list) => {
    return (list || []).map((item) => {
      if (item.children && item.children.length) {
        ...
        return (
          <div key={item.itemCode} style={{ padding: '8px 0' }}>
            {/* <Tooltip title={item.itemName}> */}
            <CheckBox onChange={(e) => handleChangeEvent(e, item.itemCode)} checked={item.checked}>
              <span className={styles['menu-item-panel']}>{item.itemName}</span>
            </CheckBox>
            {/* </Tooltip> */}
          </div>
        );
