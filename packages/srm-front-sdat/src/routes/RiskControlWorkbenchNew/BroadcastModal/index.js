...
import { fetchDynamicDetail } from '@/services/riskNewWorkPlaceService';
...
export default function BroadcastModal(props) {
  ...
  const openSelectPeople = async () => {
    ...
    const handleSubmit = async () => {
      ...
      if (broadcastDS && broadcastDS.current) {
        broadcastDS.current.set('broadcastReceivers', uniList);
      }
      modal.close();
    };
  ...
  const count = broadcastDS?.current?.toData()?.broadcastReceivers?.length ?? 0;
