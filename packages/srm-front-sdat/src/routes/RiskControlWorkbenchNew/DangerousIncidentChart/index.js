...
import { fetchBarChartData } from '@/services/riskNewWorkPlaceService';
...
export default function DangerousIncidentChart(props) {
  ...
  useEffect(() => {
    const dom = document.getElementById('risk-control-new-dangerous-incident-chart-container');
  ...
  const refreshChartResize = () => {
    if (incidentTypeChart) {
      const heig = document.getElementById('sdat-risk-new-control-incident-type').height;
      const wid = document.getElementById('sdat-risk-new-control-incident-type').width;
  ...
  const refreshChart = () => {
    incidentTypeNode = document.getElementById('sdat-risk-new-control-incident-type');
  ...
  return (
    <>
      <div
        id="risk-control-new-dangerous-incident-chart-container"
      ...
      <div
        id="sdat-risk-new-control-incident-type"
        style={{
          minHeight: '100px',
          minWidth: '200px',
          marginTop: '8px',
