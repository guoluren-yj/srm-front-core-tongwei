...
import {
  ...
  fetchApprovePeople,
  fetchDynamicDetail,
} from '@/services/riskNewWorkPlaceService'; // fetchIsSupplier
...
export default function RiskVoucherModal(props) {
  ...
  const { disposalDS, localRecord, typeMode, parentAttach, eventDetail } = props;
  ...
  const openSelectPeople = async () => {
    ...
    // 默认选中的数据
    const defaultArr = record?.broadcastReceivers ?? [];
    ...
    const handleSubmit = async () => {
      ...
      if (disposalDS && disposalDS.current) {
        disposalDS.current.set('broadcastReceivers', uniList);
      }
      modal.close();
    };
  ...
  const count = disposalDS?.current?.toData()?.broadcastReceivers?.length ?? 0;
  ...
  const handleChangeType = (value) => {
    ...
    if (value && value.length && value.indexOf('RISK_BROADCAST') !== -1) {
      ...
      setHidden(true);
      if (disposalDS && disposalDS.current) {
        disposalDS.current.set('broadcastReceivers', []);
      }
    }
  };
