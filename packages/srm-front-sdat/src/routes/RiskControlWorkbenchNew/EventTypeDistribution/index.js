...
import { fetchTypeChartData } from '@/services/riskNewWorkPlaceService';
...
export default function EventTypeDistribution(props) {
  ...
  useEffect(() => {
    const dom = document.getElementById(
      'risk-workplace-new-incident-type-distribution-chart-panel'
    );
    refreshChart();
  ...
  const refreshChartResize = () => {
    if (incidentDistributionChart) {
      const heig = document.getElementById('sdat-risk-new-incident-type-distribution').height;
      const wid = document.getElementById('sdat-risk-new-incident-type-distribution').width;
  ...
  const refreshChart = () => {
    incidentDistributionNode = document.getElementById('sdat-risk-new-incident-type-distribution');
  ...
  const queryRefreshChart = (params) => {
    fetchTypeChartData({
      ...
      if (getResponse(res) && Array.isArray(res) && res.length) {
        const map = {};
        res.forEach((item) => {
          map[item.status] = item.riskCount;
        });
        setData(map);
        setChartOption(map);
      } else {
  ...
  return (
    <>
      <div
        ...
        id="risk-workplace-new-incident-type-distribution-chart-panel"
      >
        <span
          style={{
            fontSize: '16px',
      ...
      <div
        id="sdat-risk-new-incident-type-distribution"
        style={{
          // minWidth: '160px',
          minHeight: '160px',
          flex: '1 1 auto',
