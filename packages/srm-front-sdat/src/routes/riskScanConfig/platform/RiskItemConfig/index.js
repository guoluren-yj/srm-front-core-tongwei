...
const RiskItemConfig = (props) => {
    ...
    return [
      ...
      // },
      {
        ...
        renderer: ({ record }) => {
          ...
          const { enabledFlag, endFlag, level, type } = obj || {};
          if (!record) {
            return null;
          }
                ...
                {type === 'CATEGORY' && (
                  <>
                    {endFlag === 1 ? (
                      ...
                      <a
                        style={{ marginRight: '10px' }}
                        onClick={() => handleCreate(1, obj, 'create')}
                      >
                        {intl
                          .get('sdat.riskItemConfig.view.button.createSubItem')
                          .d('新建下级分类')}
                      </a>
                    )}
                  </>
  ...
  const handleImport = () => {
    openTab({
      key: `/sdat/commentImport/SDAT.WB2_RISK_ITEM_IMPORT`,
  ...
  const queryParams = () => {
    const queryList = riskItemListDS?.queryDataSet?.toData() ?? [];
    const param = queryList.length ? queryList[0] : {};
    return { ...param };
  };

  return (
    <>
      <Header title={intl.get('sdat.riskItemConfig.view.header.riskHeaderConfig').d('风险项管理')}>
        <Button color="primary" icon="add" onClick={() => handleCreate(0, '', 'create')}>
          {intl.get('sdat.riskItemConfig.button.createSuperItem').d('新建顶级')}
        </Button>

        <ExcelExportPro
          buttonText={intl.get('hzero.common.button.confirm.export').d('导出')}
          requestUrl={`${SRM_DATA_SDAT}/v1/wb2-risk-items/risk-item-export`}
          otherButtonProps={{
            icon: 'export',
            type: 'c7n-pro',
            funcType: 'flat',
        ...
        <Button funcType="flat" onClick={handleImport}>
...
export default connect((state) => state)(
  formatterCollections({
    code: ['sdat.riskItemConfig', 'srm.filterBar'],
  })(
    withProps(
      () => {
        const riskItemListDS = new DataSet(RiskItemListDS());

        return { riskItemListDS };
      },
