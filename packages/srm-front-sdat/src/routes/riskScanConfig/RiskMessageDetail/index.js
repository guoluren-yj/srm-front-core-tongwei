...
import CommonDetail from '@/routes/RiskControlWorkbenchNew/CommonDetail';
      ...
      fetchDynamicDetail({ riskEventId }).then((res) => {
        ...
        if (getResponse(res)) {
          ...
          const {
            ...
            attachmentUuid,
            eventAttachmentUuid,
            broadcastAttachmentUuid,
          } = res;

          basicAttachDS.loadData([{ attachmentUuid: attachmentUuid || eventAttachmentUuid }]);
          childAttachDS.loadData([{ attachmentUuid: broadcastAttachmentUuid }]);
          broadcastDS.loadData([{ processReason }]);

          setEventData(res && res ? { ...res, ...obj } : {});
          setDetail({
          ...
          const socialCode = res?.socialCode ?? '';
          if (socialCode) {
            fetchMonitorMsg({
              socialCode,
              riskEventId,
    ...
    <Spin spinning={loading}>
      <div className={styles['risk-incident-detail-basic']}>
        ...
        <div className={styles['risk-incident-detail-modal-basic-info']}>
          ...
          <div style={{ flex: 1, padding: '20px', background: '#fff', marginTop: '8px' }}>
            <div className={styles['risk-incident-detail-modal-basic']}>
