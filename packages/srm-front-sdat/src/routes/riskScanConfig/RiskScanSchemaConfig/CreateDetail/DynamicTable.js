...
import { fetchRemoveObjectList } from '@/services/riskScanConfig/schemaConfigService';
...
export default function DynamicTable(props) {
  ...
  const handleCreateHandItem = () => {
    ...
    selectHandListDS.setQueryParameter('domesticForeignRelation', domesticForeignRelation);
    selectHandListDS.setQueryParameter('riskPlanId', localId);
  ...
  const buttons = () => {
    return [
      ...
      dynamicType === 'SUPPLIER_CATEGORY' && (
        <Button key="3" funcType="flat" icon="assignment_ind-o" onClick={handleViewSupplier}>
          {intl.get('sdat.riskScanConfig.button.viewSupplier').d('查看供应商')}
        </Button>
      ),

      dynamicType === 'MANUAL_SUPPLIER' && (
        <Button icon="add" key="4" funcType="flat" onClick={handleCreateHandItem}>
          {intl.get('hzero.common.button.add').d('新增')}
        </Button>
      ),
      dynamicType === 'MANUAL_SUPPLIER' && (
      ...
      dynamicType === 'PURCHASER' && (
        <Button icon="add" key="6" funcType="flat" onClick={handleCreateAccount}>
          {intl.get('hzero.common.button.add').d('新增')}
        </Button>
      ),
      dynamicType === 'PURCHASER' && (
      ...
      dynamicType === 'PURCHASER' && (
        <Button funcType="flat" key="8" onClick={handleViewAccount}>
          {intl.get('sdat.riskScanConfig.button.viewSupplier').d('查看供应商')}
        </Button>
      ),

      dynamicType === 'PLATFORM_OUTER' && (
        <Button icon="add" key="9" funcType="flat" onClick={handleAddOuterCompany}>
          {intl.get('hzero.common.button.add').d('新增')}
        </Button>
      ),
      dynamicType === 'PLATFORM_OUTER' && (
