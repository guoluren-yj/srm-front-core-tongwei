...
import { fetchUpdateConfig } from '@/services/riskScanConfig/schemaConfigService';
...
const RiskScanSchemaConfig = ({ defineListDS, history }) => {
  ...
  const handleEditItem = (record, pageType) => {
    ...
    if (riskPlanId) {
      history.push(`/sdat/risk-workbench-new/scheme-config/view-detail/${riskPlanId}/${pageType}`);
    }
  };
  ...
  const handleViewItem = (record) => {
    const riskPlanId = record?.get('riskPlanId') ?? '';
    if (riskPlanId) {
      history.push(`/sdat/risk-workbench-new/scheme-config/view/${riskPlanId}`);
    }
  };
  ...
  const columns = () => {
    return [
      ...
      { name: 'autoFlag' },
      { name: 'autoFrequency', width: 150 },
      { name: 'scanObjectType', width: 200 },
      { name: 'scanScopeType' },
      { name: 'planCompanyType' },
      { name: 'yearConsumption', width: 150 },
  ...
  const handleCreateItem = () => {
    history.push('/sdat/risk-workbench-new/scheme-config/detail/create');
  };

  return (
    <div className={styles['risk-scan-schema-config-basic']}>
      <Header
        title={intl.get('sdat.riskScanConfig.view.title.businessScanPlan').d('企业扫描方案配置')}
      ...
      <div className={styles['risk-scan-schema-config-content']}>
        <div className={styles['risk-def-search-basic-panel']}>
          <StaticSearchBar
            cacheState
            clearButton
            searchCode="SDAT.RISK_SCAN_SCHEMA_CONFIG_SEARCH_BAR"
            filters={getFilters()}
        ...
        <div>
          <div style={{ height: 'calc(100vh - 298px)' }}>
            <Table
              ...
              customizable
              customizedCode="SDAT.RISK_SCAN_SCHEMA_CONFIG_LIST"
            />
          </div>
        </div>
      </div>
