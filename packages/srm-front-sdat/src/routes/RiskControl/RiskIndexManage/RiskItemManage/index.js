...
export default function RiskItemManage(props) {
  ...
  const handleCreate = async (type, item, viewType = 'create') => {
    ...
    const handleOkCreate = async () => {
      const isValid = await commonDS.validate(true, true);
      ...
      if (isValid) {
        ...
        // 新增列表没有itemTenantId
        const newList = nowTenantList.length
          ? nowTenantList.filter((rcd) => !rcd.itemTenantId)
          : [];
    ...
    <>
      ...
      >
        <FilterBar
          ...
          cacheState
          cacheKey="SDAT.RISK_SCAN_ITEM_SEARCH_BAR_PLATFORM"
