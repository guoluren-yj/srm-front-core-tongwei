 ...
 * @Last Modified time: 2025-05-28 15:36:36
 */
...
const ScanItemListDS = () => ({
  transport: {
    read: ({ data, params }) => {
      ...
      return {
        url: `${SRM_DATA_SDAT}/v1/${getCurrentOrganizationId()}/wb2-risk-items/query-tree`,
        params: {
          ...data,
          ...params,
        },
...
const ScanMenuDetailDS = () => ({
  ...
  transport: {
    read: ({ data, params }) => {
      return {
        url: `${SRM_DATA_SDAT}/v1/${getCurrentOrganizationId()}/wb2-risk-items/${data.riskItemId}`,
        params: {
          ...data,
          ...params,
        },
    ...
    create: ({ data }) => {
      return {
        url: `${SRM_DATA_SDAT}/v1/${getCurrentOrganizationId()}/wb2-risk-items`,
        data,
        method: 'POST',
      };
    },
...
const ScanDetailDS = () => ({
  ...
  fields: [
    {
      label: intl.get(`sdat.riskItemConfig.model.scanItemCode`).d('企业信息补充项编码'),
      name: 'itemCode',
      type: 'string',
      pattern: '^[a-zA-Z][a-zA-Z0-9_]*$',
      maxLength: 30,
    ...
    {
      label: intl.get(`sdat.riskItemConfig.model.scanItemName`).d('企业信息补充项名称'),
      name: 'itemName',
      type: 'intl',
      required: true,
    },
    // {
    //   label: intl.get(`sdat.riskItemConfig.model.scanItemType`).d('企业信息补充项类型'),
    //   name: 'dsMatchType',
    //   type: 'string',
    //   required: true,
    //   lookupCode: 'SDAT.WB2_RISK_ITEM_MATCH_TYPE',
...
export { ScanItemListDS, ScanMenuDetailDS, ScanDetailDS };
