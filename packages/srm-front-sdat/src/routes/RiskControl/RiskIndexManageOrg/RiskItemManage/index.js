...
import { fetchUpdateItem } from '@/services/riskScanConfig/riskItemOrgService';

import RiskItemModal from './RiskItemModal';
...
export default function RiskItemManage(props) {
  const { riskItemListDS, riskMenuDetailDS, riskDetailDS } = props;

  useEffect(() => {
    riskItemListDS.addEventListener('load', initData);

    return () => {
      riskItemListDS.removeEventListener('load', initData);
      expandKeys = [];
    };
  }, []);
    ...
    return [
      ...
      },
      {
        ...
        renderer: ({ record }) => {
          ...
          const { enabledFlag, endFlag, level, type, tenantId } = obj || {};
          if (!record) {
            return null;
          }
                ...
                <>
                  {endFlag === 1 ? (
                    <a
                      ...
                      onClick={() => handleCreate(2, obj, 'create')}
                    >
                      {intl.get('sdat.riskItemConfig.view.button.createRiskItem').d('新建风险项')}
                    </a>
                  ) : null}
                </>
              )}
    ...
    if (obj && Object.keys(obj).length > 0) {
      Modal.confirm({
        ...
        if (button === 'ok' && canClick === 1) {
          ...
          // if (flagValue === 1) {
          //   // 启用， 查询所有父级
          //   const allData = riskItemListDS?.toData() ?? [];
          ...
          list.push({ ...obj, enabledFlag: flagValue });
  ...
  const handleCreate = async (type, item, viewType = 'create') => {
    ...
    const titleMap = {
      ...
      2: intl.get('sdat.riskItemConfig.view.title.createRiskItem').d('新建风险项'),
    };
    ...
    const editTitleMap = {
      ...
      2: intl.get('sdat.riskItemConfig.view.title.editRiskItem').d('编辑风险项'),
    };
    ...
    if (viewType === 'create') {
        ...
        commonDS.create(
          {
            ...
            parentCode: item.itemCode?.startsWith(`${tenantNum}_`)
              ? item.itemCode
              : `${tenantNum}_${item.itemCode}`,
      ...
      if (type === 1) {
        const { parentId } = item;
        const list = riskItemListDS.toData();
        ...
        if (commonDS && commonDS.current) {
          ...
          commonDS.current.set(
            'parentCode',
            result?.itemCode?.startsWith(`${tenantNum}_`)
              ? result?.itemCode
              : `${tenantNum}_${result?.itemCode}`
          );
        }
      }

      const originItemCode = commonDS?.current?.get('itemCode') ?? '';

      if (commonDS && commonDS.current) {
        commonDS.current.set(
          'itemCode',
          originItemCode?.startsWith(`${tenantNum}_`)
            ? originItemCode.slice(tenantNum.length + 1)
            : originItemCode
        );
      }
    ...
    const handleOkCreate = async () => {
      ...
      if (isValid) {
        ...
        const res = await fetchUpdateItem([
          {
            ...obj,
            tenantList: [...newList, ...diffTenantList],
            itemCode: obj.itemCode?.startsWith(`${tenantNum}_`)
              ? obj.itemCode
              : `${tenantNum}_${obj.itemCode}`,
          },
        ]);
        if (getResponse(res)) {
          handleCloseModal();
    ...
    modal = Modal.open({
      ...
      key: 'createItem',
      children: (
        <RiskItemModal dataSet={commonDS} type={type} viewType={viewType} dsType={dsType} />
      ),
      closable: true,
      drawer: true,
      mask: false,
  ...
  return (
    <>
      ...
      >
        <FilterBar
          dataSet={[riskItemListDS]}
          cacheState
          cacheKey="SDAT.RISK_SCAN_ITEM_SEARCH_BAR"
