    ...
    return (dataArr || []).map((item) => {
      ...
      return (
        <div
          ...
          style={{ backgroundColor: selectId === item.cockpitHeaderId ? '#F2F3F5' : '' }}
        >
          <div className={styles['template-card-panel']}>
            <div style={{ lineHeight: '18px', color: '#1D2129' }}>
              <span style={{ fontSize: '14px' }}>{item.name}</span>
              {/* {mouseTagMap[item.templateId] || selectId === item.cockpitHeaderId ? (
                <a
                  style={{ float: 'right' }}
                  onClick={(e) => handleSelectTemp(e, item.cockpitHeaderId, 'edit')}
                >
                  {intl.get('sdat.reportConfig.view').d('应用')}
                </a>
              ) : null} */}
            </div>
