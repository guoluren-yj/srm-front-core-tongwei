...
import { Row, Col, Icon, Popover, Collapse } from 'choerodon-ui';
...
export default function CardScrollComp(props) {
  const { setting, queryCardTextList = () => {}, basicGroupList = [], readCards = [] } = props;
  ...
  const renderCardList = (cardList = []) => {
    ...
    return (dataArr || []).map((item) => {
      return (
        <SourceBox key={item.cardId} forbidDrag={setting} record={item} aggregation={aggregation} />
      );
    });
  };
  ...
  return (
    <div className={styles['card-list-panel']}>
      <div className={styles['card-list-panel-title']}>
        <Row>
          <Col span={18}>{intl.get('sdat.reportConfig.view.title.selectCard').d('选择卡片')}</Col>
          <Col>
            <div className={styles.search}>
              <Popover content={intl.get('sdat.reportConfig.view.title.cardMode').d('卡片模式')}>
                ...
                >
                  <span
                    ...
                    style={{ color: aggregation ? colorCode : '#868D9C' }}
                  >
                    <IconCard />
                  </span>
                </div>
              ...
              <Popover
                content={intl.get('sdat.reportConfig.view.title.normalListMode').d('列表模式')}
              >
                <div
                  className={!aggregation ? styles.active : styles['change-table']}
                  onClick={changeAggregation}
