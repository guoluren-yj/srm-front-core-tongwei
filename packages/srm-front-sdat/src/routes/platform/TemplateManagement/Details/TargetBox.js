...
const TargetBox = memo(function TargetBox(props) {
  ...
  const [{ canDrop }, drop] = useDrop({
    ...
    drop(_item, monitor) {
      const scrollTop =
        document.getElementById('platform-template-manage-report-config')?.children[0]?.scrollTop ??
        0; // 获取画板滚动高度
  ...
  const contentHeight =
    document.getElementById('platform-template-manage-report-config')?.children[0]?.clientHeight ??
    0; // 获取画板高度
  const scrollTop =
    document.getElementById('platform-template-manage-report-config')?.children[0]?.scrollTop ?? 0; // 获取画板滚动高度
