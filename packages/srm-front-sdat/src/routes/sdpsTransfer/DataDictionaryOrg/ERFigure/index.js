...
const ERFigure = (props) => {
  ...
  const getBOIdUrl = `${SRM_DATA_SDAT}/v1/${getCurrentOrganizationId()}/meta-table/map-relation`;
    ...
    return getCombinationBOList({
      ...
      .then((res) => {
        if (getResponse(res) && res && res.length) {
          ...
          return fetchBatchColumns(tables)
            .then((columns) => {
  ...
  useEffect(() => {
    ...
    if (localRecord?.tableName && refreshKey === '4') {
      setLoading(true);
