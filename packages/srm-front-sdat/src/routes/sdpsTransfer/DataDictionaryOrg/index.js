...
import {
  ...
  fetchTenantMessage,
} from '@/services/sdpsTransfer/dataDictionaryService';
import NoneSvg from '@/assets/none.svg';

import LeftMenuClassify from './LeftMenuClassify';
import LeftMenuRemoveClassify from './LeftMenuRemoveClassify';
...
const DataDictionary = (props) => {
  ...
  const [coreUser, setCoreUser] = useState(0);

  useEffect(() => {
    // 监听列属性表的加载事件，查询赋值字段值集项
    listDS.addEventListener('load', handleListener);
    fetchTenantMessage().then((res) => {
      if (getResponse(res)) {
        setCoreUser(res?.coreEnterprise ?? 0);
      }
    });
    return () => {
  ...
  const handleListener = () => {
    listDS.forEach((record) => {
      // lovId是平台级的lovId
      // orgLovId是租户级的lovId
      const { orgLovId = undefined } = record?.get(['lovType', 'orgLovId']);
      if (!orgLovId) return;
      // LovCode存在则访问接口
      // 先检查是否已经缓存
   ...
   * 打开主题订阅弹窗
   */
  const handleOpenSubscriLov = async () => {
    ...
    <SourceManagerProvider>
      <Header title={intl.get('sdps.dataDictionary.view.title.dataDictionary').d('数据字典')}>
        <ExcelExportPro
          ...
          defaultSelectAll
          requestUrl={`${SRM_DATA_SDAT}/v1/${organizationId}/meta-table/table-export`}
          queryParams={queryParams}
          templateCode="SRM_C_SDAT_DATA_DICTIONARY_EXPORT"
          ...
          <Tabs animated={false} activeKey={activeKey} onChange={handleChangeTab}>
            ...
            >
              <LeftMenuRemoveClassify
                ...
                fetchUrl={`${SRM_DATA_SDAT}/v1/${organizationId}/data-table-manages/subscribed-topic-list`}
                fetchTabsUrl={`${SRM_DATA_SDAT}/v1/${organizationId}/meta-table/subscribe-tables`}
                onSelect={(e) => handleSelect(e, '1')}
                onAddScription={handleOpenSubscriLov}
            ...
            >
              <LeftMenuClassify
                ...
                placeholder={intl
                  ...
                  .d('请输入数据表编码、名称查询')}
                nodataMsg={intl.get('sdps.dataSheet.view.message.noData').d('当前主题下暂无数据')}
                fetchUrl={`${SRM_DATA_SDAT}/v1/${organizationId}/data-table-manages/pending-topic-list`}
                fetchTabsUrl={`${SRM_DATA_SDAT}/v1/${organizationId}/meta-table/pending-tables`}
            ...
            >
              <LeftMenuClassify
                ...
                placeholder={intl
                  ...
                  .d('请输入数据表编码、名称查询')}
                nodataMsg={intl.get('sdps.dataSheet.view.message.noData').d('当前主题下暂无数据')}
                fetchUrl={`${SRM_DATA_SDAT}/v1/${organizationId}/data-table-manages/rejected-topic-list`}
                fetchTabsUrl={`${SRM_DATA_SDAT}/v1/${organizationId}/meta-table/reject-tables`}
...
export default formatterCollections({
  code: ['sdps.dataDictionary', 'srm.filterBar', 'sdps.dataSheet', 'hmde.boComposition'],
})(
  withProps(
    () => {
