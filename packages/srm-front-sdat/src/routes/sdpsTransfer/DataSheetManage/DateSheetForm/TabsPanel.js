...
import { fetchUnBind, fetchPublicCloud } from '@/services/sdpsTransfer/dataSheetService';
...
// const { PRIVATE_CLOUD_TENANT } = getEnvConfig();
        ...
        <TabPane tab={intl.get('sdps.dataSheet.view.title.columnProp').d('列属性')} key="1">
          <div style={{ marginTop: '16px' }}>
            ...
            <Table
              ...
              customizable
              customizedCode="SDAT.DATASHEET_DATATABLE_COLUMNTABLE"
            />
          </div>
        </TabPane>
        {pubFlag && (
          <TabPane
            tab={intl.get('sdps.dataSheet.view.title.mappingRelations').d('映射关系')}
            key="2"
          >
        ...
        <TabPane tab={intl.get('sdps.dataSheet.view.title.subscripTenant').d('订阅租户')} key="3">
          ...
          >
            <Table
              ...
              customizable
              customizedCode="SDAT.DATASHEET_DATATABLE_TENTANTSUBSCRIP"
            />
          </div>
        </TabPane>
        <TabPane
