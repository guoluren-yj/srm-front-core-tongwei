...
import { TextField, Dropdown, Tooltip } from 'choerodon-ui/pro'; // tooltip
            ...
            <Tooltip title={`${item.name || ''} ${item.description || ''}`}>
              <span
                style={{
                  ...
                  width: item.level === 'org' ? '160px' : '200px',
                  overflow: 'hidden',
            ...
            {item.level === 'org' && (
              <span
                style={{
                  ...
                  padding: '0 3px',
                  color: '#F06200',
                  background: 'rgba(242,128,26,0.15)',
                  borderRadius: '2px',
                }}
              >
                {intl.get('sdps.common.view.title.orgLevel').d('租户层')}
              </span>
            )}
