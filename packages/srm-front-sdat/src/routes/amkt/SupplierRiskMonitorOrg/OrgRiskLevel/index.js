...
export default function OrgRiskLevel(props = {}) {
  ...
  useEffect(() => {
    eTable?.showLoading();
    getMsgByLovCode({ code: 'SDAT.RISK_LEVEL_TYPE' })
      .then((res) => {
        const mapObj = {};
        (res || []).forEach((item) => {
  ...
  const renderChart = () => {
    ...
    if (eTable) {
      ...
      const option = {
        ...
        series: [
          {
            name: intl.get('sdat.supplierRiskMonitor.view.title.orgRiskLevel').d('企业风险等级'),
            type: 'pie',
            radius: ['40%', '60%'],
            ...
            label: {
              normal: {
                ...
                formatter: `{total|${total}}\r{active|${intl
                  .get('sdat.supplierRiskMonitor.view.title.orgs')
                  .d('家')}}`,
                rich: {
                  total: {
                    fontSize: 35,
            ...
            color: ['#458c4e', '#3ab445', '#fab411', '#ff7800', '#f25535'].reverse(), // 低风险到高风险的五个颜色
            data: chartsData,
          },
          {
            name: intl.get('sdat.supplierRiskMonitor.view.title.orgRiskLevel').d('企业风险等级'),
  ...
  return (
    <div className={style['out-box']}>
      <div className={style['title-bar']}>
        {intl
          .get('sdat.supplierRiskMonitor.view.title.orgRiskLevelDistribution')
          .d('企业风险等级分布')}
      </div>
      {(resData?.length ?? 0) === 0 && (
        <Result
