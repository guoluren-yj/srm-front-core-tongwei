...
export default class Field extends Component {
  ...
  handleClickField() {
    ...
    if (['LOV', 'SELECT', 'DATE_PICKER', 'TREE_SELECT'].includes(fieldWidget)) {
  ...
  handlebBlurField() {
    ...
    if (['SELECT', 'DATE_PICKER', 'TREE_SELECT'].includes(fieldWidget) && !multipleFlag) {
      this.handlePopupFieldEditMode(FieldEditMode.OUTPUT);
    }
  }
  ...
  handleClearValue(event, record) {
    ...
    if (multipleFlag === 1) {
      ...
      if (fieldWidget === 'TREE_SELECT') {
        record.set(name, undefined);
        // eslint-disable-next-line no-unused-expressions
        record?.set(`${getMeaningFieldName(name)}`, undefined);
        // eslint-disable-next-line no-unused-expressions
        record?.set(`${name}-zpTemp`, undefined);
      }
    }
  }
        ...
        return (
          <Tooltip title={fieldMeaingText}>
            <Cascader
              ...
              onChange={(e) => {
                if (e) {
                  const dimName = e[e.length - 1]?.split('-')[0] ?? '';
                  const dimCode = e[e.length - 1]?.split('-')[1] ?? '';
                  // eslint-disable-next-line no-unused-expressions
                  record?.set(`${name}`, dimCode);
                  // eslint-disable-next-line no-unused-expressions
                  record?.set(`${getMeaningFieldName(name)}`, dimName);
                  // eslint-disable-next-line no-unused-expressions
                  record?.set(`${name}-zpTemp`, dimName);
                } else {
                  // eslint-disable-next-line no-unused-expressions
                  record?.set(`${name}`, '');
                  // eslint-disable-next-line no-unused-expressions
                  record?.set(`${getMeaningFieldName(name)}`, '');
                  // eslint-disable-next-line no-unused-expressions
                  record?.set(`${name}-zpTemp`, null);
                }
              }}
            />
          </Tooltip>
  ...
  renderValue({ field, record }) {
    ...
    if (['INPUT', 'SELECT', 'LOV', 'TREE-SELECT'].includes(fieldWidget)) {
