...
export default class JobLog extends React.Component {
  ...
  renderFilterForm() {
    ...
    const timeEnd = getFieldValue('timeEnd');
    const timeStart = getFieldValue('timeStart');
    const min = timeEnd
      ? moment(timeEnd).subtract(3, 'days')
      : undefined;
    const max = timeStart
      ? moment(timeStart).subtract(-3, 'days')
      : undefined;
    return (
            ...
            >
              {getFieldDecorator('taskId', {
                rules: [
                  {
                    pattern: /^[0-9]*$/,
                    message: intl.get('hsdr.jobLog.validation.digital').d('只能输入数字'),
                  },
                ],
          ...
          <Col {...FORM_COL_4_LAYOUT}>
            ...
            >
              {getFieldDecorator('timeStart', {
                initialValue: moment().startOf('days'),
                rules: [
                  {
                    required: true,
                    message: intl.get('hzero.common.validation.notNull', {
                      name: intl.get('hsdr.jobLog.model.jobLog.startTime').d('执行开始时间'),
                    }),
                  },
                ],
              })(
                <DatePicker
                  ...
                  disabledDate={currentDate =>
                    (timeEnd && moment(timeEnd).isBefore(currentDate, 'day')) ||
          ...
          <Col {...FORM_COL_4_LAYOUT}>
            ...
            >
              {getFieldDecorator('timeEnd', {
                initialValue: moment().endOf('days'),
                rules: [
                  {
                    required: true,
                    message: intl.get('hzero.common.validation.notNull', {
                      name: intl.get('hsdr.jobLog.model.jobLog.endTime').d('执行结束时间'),
                    }),
                  },
                ],
              })(
                <DatePicker
                  ...
                  placeholder=""
                  disabledDate={currentDate =>
                    (timeStart && moment(timeStart).isAfter(currentDate, 'day')) ||
