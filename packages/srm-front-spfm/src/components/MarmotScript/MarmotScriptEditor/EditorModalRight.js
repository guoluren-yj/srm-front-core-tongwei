...
export default function EditorModalRight(props = {}) {
  const { scriptOutputDs, jsxOptions, jsonOptions, scriptCodeDs, testParam } = props.queryParam;
  ...
  return (
    <div style={{ height: '100%' }}>
      <Tabs defaultActiveKey="outputJson">
        ...
        <TabPane
          tab={intl.get('spfm.adaptorPlayGround.view.title.objList').d('对象列表')}
          key="objList"
        >
          <div style={{ height: 'calc(100vh - 178px)' }}>
            <ObjectList testParam={testParam} />
          </div>
        </TabPane>
        <TabPane
          tab={intl.get('spfm.adaptorPlayGround.view.title.resourceReport').d('资源报告')}
          key="executionInfo"
        >
          <CodeArea
            dataSet={scriptOutputDs}
