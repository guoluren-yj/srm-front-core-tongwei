...
export async function approve(body) {
  // const { companyId } = body;
  return request(`${SRM_PLATFORM}/v1/${organizationId}/company-ca-auth-info/account_register`, {
    method: 'PUT',
    body,
  });
}
...
export async function companyVerify(params) {
  // const { userId } = params;
  return request(`${SRM_PLATFORM}/v1/${organizationId}/company-ca-auth-info/company-verify-url`, {
    method: 'POST',
    body: params,
    responseType: 'text',
  });
...
export async function commonCompanyVerify(params) {
  // const { userId } = params;
  return request(
    `${SRM_PLATFORM}/v1/${organizationId}/company-ca-auth-info/common/company-verify-url`,
    {
      method: 'POST',
      body: params,
      responseType: 'text',
...
export async function fetchElectronicSignatureUrl(params) {
  const query = filterNullValueObject(parseParameters(params));
  return request(
    `${SRM_PLATFORM}/v1/${organizationId}/company-ca-auth-info/fdd-company-openService`,
    {
      method: 'POST',
      body: query,
      responseType: 'text',
