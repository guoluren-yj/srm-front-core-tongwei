...
import { Icon, Button, Spin } from 'choerodon-ui';
...
export default function QrCodePay(props) {
  ...
  const { timeoutFlag, setTimeoutFlag, setLeftTime, timerRender } = useTimer(0, timerTrigger); // 计时器
  ...
  const fetchPayStatus = () => {
    queryOrderStatus(orderInfo.paymentOrderNum).then((res) => {
      // 1成功 0失败
      if (res) {
  ...
  const refreshQrCode = async () => {
    ...
    const result = await reGenerateOrder(orderInfo.paymentOrderNum);
    if (result) {
