...
import { updateLicenceUrl } from '@/services/simplifiedRegisterService';
...
export default class UploadModal extends Component {
  ...
  onUploadSuccess(response, file = {}) {
    ...
    this.handleUpdateLicenceUrl();
    const { name } = file;
    this.handleFileList({ licenceFilename: name, licenceUrl: response });
  }

  @Bind()
  handleFileList() {
    const { dataSet } = this.props;
    const { licenceUrl, licenceFilename } = dataSet.current.get(['licenceUrl', 'licenceFilename']);
    if (licenceUrl) {
      const newLicenceFilename = this.handleFileName(licenceUrl);
      const fileName = newLicenceFilename || licenceFilename;
      let url = getAttachmentUrl(licenceUrl, PRIVATE_BUCKET, tenantId, 'spfm-comp');
  ...
  handleRender() {
    const { dataSet } = this.props;
    const uploadFlag = dataSet?.current && dataSet.current.get('uploadFlag');
  ...
  render() {
    ...
    const { licenceUrl, uploadFlag } = dataSet?.current ? dataSet.current.get(['licenceUrl', 'uploadFlag']) : {};
