...
@formatterCollections({
  code: ['entity.company', 'hzero.common', 'hiam.userInfo', 'spfm.configServer'],
})
export default class ElectronicSignatureLov extends Component {
  ...
  fetchElectronicSignatureUrl() {
    ...
    const comp = companyDetail ? { ...companyDetail } : {};
    dispatch({
      type: 'certificateAuthoritySdat/fetchElectronicSignatureUrl',
      payload: {
        redirectUrl,
        authType,
        ...comp,
    ...
    return (
      <span>
        <Button
          ...
          disabled={disabled}
          onClick={disabled ? () => {} : this.fetchElectronicSignatureUrl}
        >
