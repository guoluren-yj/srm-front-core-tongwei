...
export default class RealNameAuth extends Component {
  ...
  handleCancelAuth = (userAuthId, authType) => {
    ...
    fetchCancelAuth({
      userAuthId,
      authType,
      orderTenantId: this.props.tenantId,
      sourceMenu: 'sel',
    }).then((res) => {
  ...
  handleOk = async () => {
    ...
    const { authType, onRefreshStatus = () => {}, companyDetail, tenantId } = this.props;
    ...
    if (authType === 'QYS') {
      getQYSAuth({
        ...
        if (res && (res.includes('http') || res.includes('success'))) {
          ...
          const parseStr = JSON.parse(res);
          if (parseStr && parseStr.code === 'hiam_error.amkt_not_to_authenticate') {
            this.handleAuthentication();
            this.setState({
              toAuthenticate: false,
              eSignVisible: false,
  ...
  handleGainValidCodeBtnClick = async () => {
    ...
    // this.setState({ postCaptchaLoading: true });
  ...
  handleRealAuth = async () => {
    const { authType, tenantId } = this.props;
          ...
          <div className={styles['real-name-auth-basic-row']}>
            <div className={styles['real-name-auth-block']}>
              ...
              <div className={styles['real-name-auth-second-level-msg']}>
                {intl
                  .get('spfm.buyerElectronicSign.view.message.selfAuthAlert')
                  .d('填写实名认证信息完成个人认证')}
              </div>
