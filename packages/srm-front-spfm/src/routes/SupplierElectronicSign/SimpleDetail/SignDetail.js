...
import { createAttachmentUUID, getParseUrlParam } from '@/utils/utils';
...
const SignDetail = (props) => {
  const { companyCode, basicFormDS, location, tenantNum } = props;

  const { companyId = '', authType = '', tenantId = '' } =
  ...
  const handleCreateSign = async (record) => {
    ...
    if (record) {
      ...
      if (signAttachDS && signAttachDS.current) {
        ...
        signAttachDS.current.set('tenantNum', tenantNum);
      }
  ...
  const deleteSign = () => {
    if (signListDS.selected.length) {
      ...
      signListDS.delete(signListDS.selected).then(() => {
        signListDS.query();
      });
    }
  };
  ...
  const signColumns = () => {
    return [
      ...
      { name: 'sealCode' },
      { name: 'sealName' },
      tenantNum === 'SRM-EPPEN' && { name: 'sealBizType' },
      ...
      >
        ...
        <div>
          <Form dataSet={basicFormDS} columns={3} labelLayout="float">
            ...
            {/* <div className={styles['auth-done-basic-form-switch']}>
              <Form.Item>
                <Switch size="small" name="enabledFlag" onChange={handleChangeEnabled} />
              </Form.Item>
            </div> */}
          </Form>
        </div>
