        ...
        <Form>
          {!isPub && (
            <>
              ...
              <FormItem
                label={intl
                  .get('spfm.certificationApproval.model.detailForm.tianyanchaMark')
                  .d('天眼查分数')}
              >
                {getFieldDecorator('tianyanchaMark', {
                  rules: [],
                })(
                  <Input
                    style={{ width: 300 }}
              ...
              <FormItem
                label={intl
                  .get('spfm.certificationApproval.model.detailForm.purchaseAgentId')
                  .d('采购员')}
              >
                {getFieldDecorator('purchaseAgentId', {
                  rules: [
                    {
                      validator: (rule, value, callback) => {
                        const { isReject } = this.props;
                        if (!isNumber(value) && !isReject) {
                          callback(
                            intl
                              .get('spfm.certificationApproval.view.validate.approve.purchaseAgent')
                              .d('请填写采购员')
                          );
                        } else {
                          callback();
                  ...
                  <Lov
                    ...
                    // textField="rePurchaseAgentName"
                    // textValue={rePurchaseAgentName}
