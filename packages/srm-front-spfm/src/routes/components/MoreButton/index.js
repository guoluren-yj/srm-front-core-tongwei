...
const Index = ({ buttons }) => {
  ...
  const renderMenus = () => {
    ...
    return (
      <Fragment>
        <Menu>
          {notMenuList.map(({ child, name, ...props }) => {
            return (
              <Menu.Item style={{ padding: 0 }} key={name}>
                <Button {...props} waitType="throttle" wait={300} funcType="text">
                  {child}
                </Button>
              </Menu.Item>
            );
  ...
  return (
    <div className={styles['more-btn-wrap']}>
      {tiledBtns.map(({ child, label, isMenu, name, ...props }) => {
        if (isMenu) {
          return (
          ...
          return (
            <Button {...props} key={name} waitType="throttle" wait={300} funcType="link">
              {child}
            </Button>
          );
        }
