...
import { Table, Button, Modal, Col, Row, Icon } from 'hzero-ui';
import { Bind } from 'lodash-decorators';
import { isEmpty, omit } from 'lodash';
import classNames from 'classnames';
import intl from 'utils/intl';
import formatterCollections from 'utils/intl/formatterCollections';
import { getResponse, getCurrentUserId, tableScrollWidth } from 'utils/utils';
 ...
 */
@formatterCollections({ code: 'hpfm.individuationTable' })
    ...
    return (
      <div>
        <div className={classNames(style['header-title'])}>
          {intl.get(`hpfm.individuationTable.view.title.preview`).d(`自定义效果预览`)}
        </div>
        <Table
          bordered
        ...
        <Row className={classNames(style['header-tips'])}>
          <Col span={1}>{intl.get(`hpfm.individuationTable.view.title.prompt`).d(`提示:`)}</Col>
          <Col span={23}>
            <div>
              {intl
