...
import { queryFlexRuleDetail, deleteFlexRuleDetails } from '@/services/hpfm/flexRuleService';
...
export default class FlexFieldsConfig extends PureComponent {
  ...
  @Bind()
  save(data, cb = () => {}) {
    const { permissionLevelKey } = this.props;
    this.setState({
      saveLoading: true,
    });
    saveFlexRuleDetails(permissionLevelKey, data).then(res => {
      this.setState({
        saveLoading: false,
      });
      if (res && res.failed) {
  ...
  render() {
    const { flexRuleCode, formSchema = {}, code = {}, visible, componentObject } = this.props;
    const {
      dataSource = [],
      pagination = [],
      detailDrawerVisible,
