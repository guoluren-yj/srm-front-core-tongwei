...
const UploadModal = (props) => {
  ...
  const attchmentList = props.record[fieldCode]
    ? props.record[fieldCode].map((n) => {
        ...
        queryAttachmentList({
          ...
          if (res) {
            const updateDataSource = (
              (form && form.getFieldValue(fieldCode)) ||
              list ||
              []
            ).map((n) =>
        ...
        if (!isEmpty(selectedRows)) {
          ...
          if (!isEmpty(createRows)) {
            const newAttachmentList = dataSource.filter(
              (n) => createRows.indexOf(n) > -1 === false
            );
          ...
          if (!isEmpty(updateRows)) {
            const newFormAttachmentList = ((form && form.getFieldValue(fieldCode)) || []).filter(
  ...
  const fileCount = (form && form.getFieldValue('fileCount')) || attachmentTotal;
  return (
    <Fragment>
