...
/**
 * 处理属性转化
 * @param {*} originConfig 对象，原组件必填等属性, 例如:
 * {required: true, disabled: false}
 * @param {*} fieldProperty 数组，例如：
 * [{
 *  fieldPropertyCode: 'required',
 *  fieldPropertyValue: '',
 *  lines: [{}]
 * }]
 * @param {*} config 对象，；例如
 ...
 * @returns
 */
export function handleInvestgConfig(originConfig = {}, fieldProperty = [], config = {}) {
    ...
    fieldProperty.forEach((i) => {
      ...
      if (valueType) {
        ...
        if (conditionFx) {
          ...
          if (!isErr) {
            const conNoReg = /(\d+)/g;
            const result = calculateExpression(i.lineFxList || [], config);
