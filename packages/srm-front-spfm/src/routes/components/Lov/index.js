...
export default class Lov extends React.Component {
  ...
  async selectAndClose() {
    const { changeSelectRows = () => {}, checkData } = this.props;
    const lovRecords =
      this.modalRef && this.modalRef.current && this.modalRef.current.state.selectedRows;
    let modalShow = false;
    if (!this.record) {
      Modal.warning({
    ...
    if (isFunction(checkData)) {
      const result = await checkData(lovRecords);

      // 数据校验不通过时，模态框不允许关闭，input不允许设值
      modalShow = !result;
    }
    if (modalShow) return;
