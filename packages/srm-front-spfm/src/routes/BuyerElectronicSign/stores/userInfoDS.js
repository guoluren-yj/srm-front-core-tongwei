...
// const ID_CARD = /(^[EeKkGgDdSsPpHh]\d{8}$)|(^(([Ee][a-fA-F])|([DdSsPp][Ee])|([Kk][Jj])|([Mm][Aa])|(1[45]))\d{7}$)/;
...
const QysAuthFormDS = () => ({
  ...
  fields: [
    ...
    },
    {
      ...
      type: 'email',
      dynamicProps: {
        required: ({ record }) => record.get('userType') !== 'INNER',
      },
    },
    {
      label: intl.get(`hiam.userInfo.model.phoneNumber`).d('实名手机号'),
...
export { PreFilledDS, AuthOneDS, AuthTwoDS, RealNameAuthDS, RealNamePhoneDS, QysAuthFormDS };
