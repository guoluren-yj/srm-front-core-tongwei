...
const RadarChartComp = (props) => {
   ...
   * @returns
   */
  const getTagList = (title) => {
    let tagDom = [];
    if (indicatorsList.length && title) {
      indicatorsList.forEach((item) => {
        if (item.title === title) {
          tagDom = drawTagList(item.tagList);
        }
      });
    }

    return {
      domLeng: tagDom.length,
      domStr: tagDom.join(''),
    };
  };
        ...
        >
          ...
          <Axis
            ...
            label={{
              ...
              htmlTemplate(text) {
                const domList = getTagList(text);
                return `<div style='width: 60px; text-align: center;'>
                          ${text}
                          <span style='color: red;'>
                            ${domList && domList.domLeng > 0 ? domList.domLeng : ''}
                          </span>
                        </div>`;
              },
            }}
            ...
            htmlContent={(title, items) => {
              return `<div
                    ...
                    >
                      ...
                      <ul style='margin: 0; padding: 0 16px 6px 16px;'>
                        ${getTagList(title).domStr}
                      </ul>
                      <div
                        style='
