...
export default class Goods extends React.Component {
  ...
  handleSearch() {
    const { dispatch } = this.props;
    dispatch({
      type: 'srmCards/querySupNewGoods',
      payload: {
        type: 'Customer',
  ...
  /**
   * 保存选中的行
   * @param {Array} supNewGoodsKeys
   */
  @Bind()
  onSelectChange(supNewGoodsKeys) {
    const { dispatch } = this.props;
    dispatch({
      type: 'srmCards/updateState',
      payload: {
  ...
  @Bind()
  onOk() {
    const { dispatch, srmCards: { allSupNewGoods = [], supNewGoodsKeys = [] } = {} } = this.props;
    if (!isEmpty(supNewGoodsKeys)) {
      const notPurchase = [];
      for (let i = 0; i < allSupNewGoods.length; i++) {
        if (supNewGoodsKeys.indexOf(allSupNewGoods[i].clauseId) === -1) {
          notPurchase.push({
            code: 'SRM_Goods',
  ...
  render() {
    ...
    const rowSelection = {
      selectedRowKeys: supNewGoodsKeys,
        ...
        <Row className={styles['card-row']}>
          <div className={styles['card-img']}>
            <span className={styles['card-title']}>
              {intl.get(`spfm.dashboard.view.goods.supReceiptWorkbench`).d('销售方收货工作台')}
            </span>
