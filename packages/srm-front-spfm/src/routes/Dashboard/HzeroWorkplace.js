...
export default class Workplace extends React.Component {
  ...
  componentDidMount() {
    ...
    // 退出登录时为什么会执行一次此处的did mount
    // 此处判断下token防止退出登录时因为401跳到未登录页面
    if (getAccessToken()) {
      this.initLayout();
    }
    this.mounted = true;
  ...
  loadCards(layouts = [], flag) {
    ...
    this.importCards(
      ...layout
        .map((c) => {
          const cardItem = this.getCard(c.i, flag);
          return !isNil(cardItem) ? cardItem.code : null;
        })
        .filter(Boolean)
    )
      .then((cmps) => {
  ...
  handleAddCard(card) {
    ...
    const layouts = [
      ...
      {
        ...card,
        x: card.x && typeof card.x === 'number' ? card.x : 0,
        y: card.y && typeof card.y === 'number' ? card.y : 0,
        i: String(cardId),
      },
    ];
