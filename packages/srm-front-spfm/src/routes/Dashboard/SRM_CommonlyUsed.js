...
export default class CommonlyUsed extends React.Component {
  ...
  handleJump(menuRoute) {
    ...
    const menuLeafNode = getMenuLeafData();
    const link = menuLeafNode.find((i) => i.path === menuRoute) || {};
    const { icon = '', name = '', type = '', search = '', id = '' } = link;
    if (!isEmpty(link) && (type === 'link' || type === 'inner-link')) {
      // 链接类型，处理实际跳转的Link
      openTab({
        icon,
        title: name,
        ...
        closable: true,
        search,
        type, // 菜单的类型，link类型将会打开iframe嵌入外部网址
      });
    } else {
  ...
  render() {
    ...
    return (
      <Row style={{height: '100%'}}>
        <Row className={styles.commonlyUsed} style={{height: '100%'}}>
          <div>
            <span className={styles['card-title']}>
              {intl.get(`spfm.dashboard.view.message.commonUseFunc`).d('常用功能')}
            </span>
            <span className={styles['set-up']} onClick={this.openModal}>
              {intl.get(`spfm.dashboard.view.message.set`).d('设置')}
            </span>
          </div>
          {commonlyUsedLoading === true ? (
            ...
            <Col style={{ maxHeight: 'calc(100% - 22px)', overflow: 'auto' }}>
              {functions &&
                functions.map(item => (
                  <Button
