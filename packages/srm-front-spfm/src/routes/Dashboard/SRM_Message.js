...
import { isEmpty, size, pullAllBy, isNil } from 'lodash';
...
@formatterCollections({ code: ['spfm.dashboard', 'hmsg.userMessage'] })
export default class Message extends React.Component {
  ...
  handlePlatformNoticeList(currentPage = 0) {
    const { dispatch } = this.props;
    dispatch({
      type: 'srmCards/queryCompanyNotice',
      payload: {
        size: 10,
        page: currentPage,
        pageStatusCode: 'EXHIBITING',
  ...
  transformMessageContent(message) {
    if (isNil(message) || isNil(message.content)) {
      return '';
    }
    const { content, availableOpenLink, availableTenantName } = message;
    if (!window.notification) {
      window.notification = notification;
    }
    let newContent = '';
    if (content.includes('<a ') && availableOpenLink === false) {
      // 检查是否有a标签
      content.split('<a ').forEach((item, index) => {
        ...
        if (item.includes('</a>')) {
          ...
          if (
            ...
            contentPart = `<a onclick="{notification.error({
                message: '${availableTenantName}',
    });}"${linkContent.substr(linkContent.lastIndexOf('>'))}</a>${item
              .split('</a>')
              .slice(1)
              .join(',')}`;
          }
  ...
  handleColor(item, key) {
    const content = this.transformMessageContent(item);
    if (key % 4 === 0) {
      return (
  ...
  render() {
    const {
      srmCards: { noticeLoading, PlatformNoticeLoading, messageLoading, noticeCategory = [] } = {},
    } = this.props;
    const {
      tabBar,
      isMessage,
            ...
            <div className={styles['notice-overflow']}>
              ...
              >
                {announcementList.map(item => {
                  ...
                  const news = ['XWDT', 'JFGG'].indexOf(item.noticeTypeCode);
