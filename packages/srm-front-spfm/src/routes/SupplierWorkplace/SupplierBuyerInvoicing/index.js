...
import { BuyerRecordListDS } from '@/stores/supplier/supplierBuyerInvoicDS';
import { getUrlParam } from '@/utils/utils';

import BuyerRecord from './BuyerRecord';
...
class index extends Component {
  constructor(props) {
    ...
    this.listDS = new DataSet({ ...BuyerRecordListDS() });
    this.state = {
      isPartial: false,
      targetId,
    };
  ...
  render() {
    ...
    return (
      <>
        <Header
          title={intl.get('spfm.supplierInvoic.title.paymentBuyerRecord').d('供应商订阅缴费记录')}
        >
          <div className="payment-record-header-button">
            <ExcelExportPro
              ...
              requestUrl={`${SRM_PLATFORM}/v1/${organizationId}/supplier-payment/purchase-payment-list-export`}
        ...
        <Content>
          <BuyerRecord listDS={this.listDS} targetId={targetId} history={this.props.history} />
        </Content>
      </>
    );
  }
