...
import { ReactComponent as NoPayment } from '@/assets/payment/no_payment.svg';

import './index.less';

const BillRenewal = props => {
  const { location, remote } = props;
  ...
  useEffect(() => {
    const { tenantNum = '', additionInfo = {} } = getCurrentUser() || {};
    const { organizationNum = '' } = additionInfo || {};

    const params = {
      tenantId: tenantId || getCurrentOrganizationId(),
      coreTenantCode: coreTenantCode || tenantNum,
      supplierTenantCode: supplierTenantCode || organizationNum,
      prePayFlag,
      supplierPaymentId,
    };

    fetchPayData({ ...params }).then(res => {
      if (getResponse(res) && res?.data) {
        setPayData({
          ...res.data,
          paymentFee: res.data.paymentFee >= 0 ? res.data.paymentFee.toFixed(2) : '0.00',
        });
      }
    });
  }, [tenantId, coreTenantCode, supplierTenantCode, prePayFlag, supplierPaymentId]);

  /**
   * 前往缴费
   */
  const handleToPay = () => {
    if (paymentData?.supplierPaymentId) {
      handlePay(paymentData.supplierPaymentId);
    }
  };
  ...
  const handlePaying = () => {
    if (paymentData.payUserId === getCurrentUser().id) {
      // 当前用户等于付款用户
      handleToPay();
    } else if (paymentData?.supplierPaymentId) {
      checkAlreadyPay({ supplierPaymentId: paymentData.supplierPaymentId }).then(result => {
        ...
        <div className="content-card">
          ...
          {!(paymentData && Object.keys(paymentData).length) ? (
            ...
            >
              ...
              >
                <NoPayment />
                <div style={{ marginTop: '16px', color: '#1D2129', fontSize: '14px' }}>
                  {intl
                    .get('spfm.billRenewal.view.title.noPaymentAlertMsg')
                    .d('尊敬的用户您好，您当前企业无需缴费')}
                </div>
              </div>
            </div>
