...
import { getMultiLanguage } from '@/services/rulesDefinitionCategoryService';
...
function MultRichText(props) {
  ...
  const renderRichTextModal = (mulName, multLangDs) => {
    const data = multLangDs.current.get(mulName);
    const onEditorChange = (newData) => {
      multLangDs.current.set({ [mulName]: newData });
    };
    ...
    Modal.open({
      ...
      onOk: () => {},
      onCancel: () => {
        multLangDs.current.set({ [mulName]: data });
      },
    });
  };
  ...
  const renderIntlRichTextModal = (fieldName) => {
    ...
    const renderSuffix = (mulName) => {
      return (
        <Tooltip title={intl.get('spfm.rulesCategory.view.richText.edit').d('富文本编辑')}>
          <Icon type="zoom_out_map" onClick={() => renderRichTextModal(mulName, multLangDs)} />
        </Tooltip>
      );
    };
