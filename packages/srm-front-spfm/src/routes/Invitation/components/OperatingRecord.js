...
@connect(({ loading, invitationQuestionnaireOperation }) => ({
  invitationQuestionnaireOperation,
  loading: loading.effects['invitationQuestionnaireOperation/fetchRecordList'],
  reviewLoading: loading.effects['invitationQuestionnaireOperation/fetchReviewList'],
}))
export default class OperatingRecord extends Component {
   ...
   */
  @Bind()
  fetchRecordList(params = {}, activeKey) {
    const {
      dispatch,
      investgHeaderId,
      organizationId,
    ...
    if (currentTabKey === 'all') {
      dispatch({
        type: 'invitationQuestionnaireOperation/fetchRecordList',
        payload: {
          investgHeaderId,
          organizationId,
      ...
      if (isShowReviewRecord) {
        dispatch({
          type: 'invitationQuestionnaireOperation/fetchReviewList',
          payload: {
            organizationId,
            ...params,
            businessKeyList,
  ...
  render() {
    const {
      ...
      invitationQuestionnaireOperation: { recordList = {}, recordPagination = {}, reviewList = {} },
      isShowReviewRecord = true,
      closeOperating = (e) => e,
    } = this.props;
    ...
    return (
      <React.Fragment>
        <Modal visible={operationRecordVisible} onCancel={closeOperating} footer={null} width={800}>
