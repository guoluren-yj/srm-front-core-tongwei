      ...
      <div className={styles['agree-top']}>
        <div className={styles['agree-word']}>
          ...
          {isSupplier ? (
            ...
            <p style={{ fontSize: 16 }}>
              {intl.get(`spfm.disposeInvite.view.message.turnToCertifi`).d('我邀请的供应商企业')}
              <Tooltip
                title={intl
                  .get('spfm.disposeInvite.view.message.noCertificatedTooltip')
                  .d('该企业已注册，但未完成认证，无法查看企业信息')}
              >
                <span style={{ padding: '0 5px' }} className={styles.company}>
                  {inviteReg.supplierName}
                </span>
              </Tooltip>
              {intl.get(`spfm.disposeInvite.view.message.registerCompleted`).d('已完成在线注册')}
            </p>
          )}
          {/* <p>
            <span style={{ paddingRight: 15 }}>
              {intl.get(`spfm.disposeInvite.model.purchaserCooperation.realName`).d('用户名称')}：
              {invitedUser.realName}
            </span>
          </p>
          <p>
            <span style={{ paddingRight: 15 }}>
              {intl.get(`spfm.disposeInvite.model.purchaserCooperation.phone`).d('手机号码')}：
              {invitedUser.phone}
            </span>
          </p>
          <p>
            <span style={{ paddingRight: 15 }}>
              {intl.get(`spfm.disposeInvite.model.purchaserCooperation.email`).d('邮箱')}：
              {invitedUser.email}
            </span>
          </p> */}
