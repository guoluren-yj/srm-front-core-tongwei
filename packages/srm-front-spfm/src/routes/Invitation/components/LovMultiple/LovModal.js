...
class LovModal extends React.Component {
  ...
  @Bind()
  onSelectHandleChange(record, selected, selectedRows) {
    let finalSelectRows = selectedRows;
    if(selected) {
      record.children && this.selectedChildRows(finalSelectRows, record);
    }else{
      let filterArray = [record.categoryId];
      if(record.children) {
        const arr = [];
        this.unSelectedChildRows(arr, record)
        filterArray = [record.categoryId, ...arr]
      }
      finalSelectRows = selectedRows.filter(i => !filterArray.includes(i.categoryId))
    };
    this.setState({ selectedRows: finalSelectRows });
  }

  @Bind()
  selectedChildRows(finalSelectRows, record) {
    if (!record.children) {return;}
    record.children.forEach(i => {
      finalSelectRows.push(i);
      if(i.children) {
        this.selectedChildRows(finalSelectRows, i)
      }
    })
  }

  @Bind()
  unSelectedChildRows(arr, record) {
    if(!record.children) {return}
    record.children.forEach(i => {
        arr.push(i.categoryId)
        this.unSelectedChildRows(arr, i)
    })
  }
  ...
  render() {
    ...
    const rowSelection = {
      ...
      onSelect: this.onSelectHandleChange,
      ...(getCheckboxProps
        ? { getCheckboxProps }
        : {
            getCheckboxProps: (record) => {
              return {
                disabled: +record.hasChild && (parentNodeDisable || record.isCheck === false),
              };
            },
          }),
    };
