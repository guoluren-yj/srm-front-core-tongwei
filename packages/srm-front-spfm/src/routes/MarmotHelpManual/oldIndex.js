...
import { getDocumentMarkDown } from '@/services/marmotHelpManualService';
import ActionImg from '@/assets/action.png';
import { getDocFlowPermissionDs, getQueryFormDs } from './store/marmotHelpManualDs';
...
// 帮助手册默认打开的md的key
const marmotDefaultKey = '1.1';

function MarmotHelpManual(props = {}) {
  const { jsColor, valueData, queryFormDs } = props;
  ...
  const queryDefaultMd = () => {
    getDocumentMarkDown(marmotDefaultKey).then((res) => {
      if (res) {
        handleSource(res);
        handleTableVisible(true);
      } else {
