    ...
    fields: [
      ...
      },
      {
        dynamicProps: {
          ...
          type: ({ dataSet, record }) => {
            const isNumberType = !['EQUALS', 'NOTEQUALS', 'IN', 'NOT_IN'].includes(
              record.get('operator')
            );
            const options =
              (dataSet.getField('leftValue').get('options') &&
                dataSet.getField('leftValue').get('options').records) ||
              [];
            const leftValueOption = options.find(
              option => option.get('name') === record.get('leftValue')
            );
            if (leftValueOption && leftValueOption.get('lovCode')) {
              __valueOption = leftValueOption.get('valueField');
              __newOption = leftValueOption.toData();
            } else {
              __valueOption = undefined;
              __newOption = undefined;
            }
            const isObject = leftValueOption && leftValueOption.get('lovCode');
            if (['IN', 'NOT_IN'].includes(record.get('operator')) && !isObject) {
              return 'auto';
            }
            return isNumberType
              ? 'number'
