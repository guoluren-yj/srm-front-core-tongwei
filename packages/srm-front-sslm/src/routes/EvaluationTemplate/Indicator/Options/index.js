...
const defaultTableRowKey = 'evalTplIndOptId';
...
export default class Editor extends PureComponent {
  ...
  save() {
    const {
      ...
      indicatorRowDataSource: { evalTplIndId },
    } = this.props;
    const { dataSource } = this.state;
    const params = getEditTableData(dataSource, ['evalTplIndOptId']);
    if (Array.isArray(params) && params.length !== 0) {
      saveIndicatorOpls(evalTplIndId, params, () => {
        this.handleFetchOptionsList();
      });
    }
  }
  ...
  add() {
    const {
      indicatorRowDataSource: { evalTplId },
    } = this.props;
    ...
    let result = (
      <React.Fragment>
        ...
        <a
          ...
          onClick={() =>
            deleteIndicatorOpls(
              { evalTplIndOptId: record.evalTplIndOptId, evalTplId },
              this.handleFetchOptionsList
            )
          }
          disabled={deleteIndicatorOplsLoading || saveIndicatorOplsLoading}
  ...
  @Bind()
  getColumns() {
    const { OptionsDrawerStatus } = this.props;
    const columns = [
      {
      ...
      <Drawer {...drawerProps}>
        ...
        >
          ...
          {OptionsDrawerStatus === 'edit' && (
            <Button
              type="primary"
              loading={processing.saveIndicatorOplsLoading}
              onClick={this.save}
            >
