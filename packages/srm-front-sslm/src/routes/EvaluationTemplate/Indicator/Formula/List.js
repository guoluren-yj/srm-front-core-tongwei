...
import { sum, isEmpty, uniqBy } from 'lodash';
...
export default class List extends PureComponent {
  ...
  getColumns() {
    const {
      ...
      editableRows = [],
      formulaDrawerStatus,
      openParamDefinition = (e) => e,
      openParamConfig = (e) => e,
    } = this.props;
    const defaultColumns = [
      {
        title: intl.get(`spfm.supplierKpiIndicator.model.supplier.fmlRemark`).d('公式说明'),
        dataIndex: 'remark',
        width: 150,
      },
      {
        title: intl.get(`spfm.supplierKpiIndicator.model.supplier.serviceName`).d('服务'),
        dataIndex: 'serviceCode',
        width: 180,
      },
      {
        title: intl.get(`spfm.supplierKpiIndicator.model.suKpiIn.formulaUrl`).d('URL'),
      ...
      },
      {
        ...
        dataIndex: 'paramDefinition',
        width: 100,
        render: (_, record) => (
          <a disabled={record._status === 'create'} onClick={() => openParamDefinition(record)}>
      ...
      },
      {
        ...
        width: 100,
        render: (_, record) => (
          <a disabled={record._status === 'create'} onClick={() => openParamConfig(record)}>
  ...
  render() {
    const { dataSource = [], defaultTableRowKey, loading, onChange = (e) => e } = this.props;
    const components = {
      body: {
        row: EditableRow,
        cell: EditableCell,
