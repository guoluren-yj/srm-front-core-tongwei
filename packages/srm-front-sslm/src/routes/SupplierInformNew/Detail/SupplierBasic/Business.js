...
import { Form, Spin, Select, Output } from 'choerodon-ui/pro';
  ...
  ({
    ...
    return (
      <Spin dataSet={dataSet}>
        {customizeForm(
          {
            code: 'SSLM.SUPPLIER_INFORM_CHANGE_NEW_DETAIL.BUSINESS',
            readOnly: isRead,
          },
          <Form
            columns={3}
          ...
          >
            <FormField name="serviceType" componentType="SELECT" isEdit={isEdit} />
            {isEdit ? (
              <Select
                multiple
                searchable
                ...
                renderer={({ record }) => {
                  if (record) {
                    ...
                    return industryNames.map(industryName => (
                      <span className="c7n-pro-output-multiple-block-disabled c7n-pro-output-multiple-block">
                        {industryName}
                      </span>
                    ));
                  }
            ...
            />
            <FormField name="website" isEdit={isEdit} />
            <FormField
              newLine
              rows={3}
              cols={2}
