...
export default class LovMultiModal extends React.Component {
  ...
  processQueryParams(pagination, prefix = 'left') {
    ...
    const filter = {};
    Object.keys(queryFields).forEach(obj => {
      const queryValue = form.getFieldValue(`${prefix === 'left' ? 'left' : 'right'}${obj}`);
      const { field, dataType } = queryFields[`${obj}`] || {};
      if (field) {
        filter[field] =
              ...
              <Row className="header">
                <div className="inputs">
                  <div className="inputs-wrapper">
                    {queryFieldsList.map(queryField => {
                      return (
          ...
          <div className="control">
            ...
            >
              <Button
                ...
                disabled={Number.isNaN(pagination.total) || Number(pagination.total) > 2000}
              >
