  ...
  fields: [
    {
      ...
      transformResponse: (value, data) => {
        ...
        const serviceTypeMeaning = (serviceTypeList || []).join('、');
    ...
    },
    {
      ...
      transformResponse: (value, data) => {
        const { industryList = [], industryReqList = [] } = data;
        const finalIndustryList = isEmpty(industryReqList) ? industryList : industryReqList;
        const industryListMeaning = (finalIndustryList || []).map(i => i.industryName).join('、');
        return industryListMeaning;
      },
    },
    {
      ...
      transformResponse: (value, data) => {
        const { industryCategoryReqList = [], industryCategoryList = [] } = data;
        const finalIndustryCategoryList = isEmpty(industryCategoryReqList)
          ? industryCategoryList
          : industryCategoryReqList;
        const industryCategoryListMeaning = (finalIndustryCategoryList || [])
          .map(i => i.categoryName)
          .join('、');
        return industryCategoryListMeaning;
      },
    },
    {
      ...
      transformResponse: (value, data) => {
        const { serviceAreaList = [] } = data;
        const serviceAreaListMeaning = serviceAreaList.map(i => i.serviceAreaMeaning).join('、');
        return serviceAreaListMeaning;
      },
    },
    {
      name: 'website',
...
const attachmentDS = () => ({
  ...
  fields: [
    ...
    },
    {
      ...
      bucketDirectory: bucketDirectory.searchPurchaser,
