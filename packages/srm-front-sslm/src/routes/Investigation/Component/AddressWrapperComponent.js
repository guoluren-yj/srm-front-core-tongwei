...
export default class AddressAWrapperComponent extends React.PureComponent {
  ...
  handleCountryChange(countryId, lovRecord) {
    ...
    this.setState({ countryId, regionId: undefined });
  }
  ...
  handleBankChange(_, lovRecord) {
    ...
    this.setState({ bankId: lovRecord?.bankId });
  }
  ...
  handleItemChange(itemId, lovRecords) {
    const { setFieldsValue } = this.composeForm?.props.form;
  ...
  getRegionQueryParams() {
    const currentCountryId = this.composeForm?.props.form.getFieldValue('countryId');
    return {
      countryId: currentCountryId || this.state.countryId,
    };
  }

  getProductCategoryParams(field) {
    const itemId = this.composeForm?.props.form.getFieldValue('itemId');
    return {
  ...
  getItemParams(field) {
    const productCategoryId = this.composeForm?.props.form.getFieldValue('productCategoryId');
    return {
  ...
  getBankFirmQueryParams(field = {}) {
    // 特殊处理取采购方租户id，寻求更好的方法
    // eslint-disable-next-line prefer-destructuring
    // const tenantId = this.props.children.props.fields[0].tenantId;
    return {
  ...
  getCityQueryParams() {
    const currentRegionId = this.composeForm?.props.form.getFieldValue('regionId');
    return {
      parentRegionId: currentRegionId || this.state.regionId,
    };
  }
