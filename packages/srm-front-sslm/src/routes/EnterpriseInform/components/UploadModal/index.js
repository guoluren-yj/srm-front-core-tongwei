...
import { isEmpty, compose, isString, uniqBy } from 'lodash';
import {
  onDraggerUploadRemove as onDraggerUploadRemoveFields,
  queryAttachment as onQueryAttachment,
  onAttachmentRemove,
} from '@/services/attachmentEnterpriseInfoChangeServices';
import {
  isReview,
  reviewFile,
  downLoadFile,
...
const UploadModal = props => {
  const {
    ...
    record: {
      ...
      $form,
      _status,
      firmChangeProserviceAttCount,
      firmChangeBeanStateFlag,
      attachmentStateFlag,
    },
    user: {
  ...
  const queryAttachment = useCallback(() => {
    setSpinning(true);
    onQueryAttachment({ organizationId, investgProserviceId }).then(res => {
      if (res) {
        const updateDataSource = ($form?.getFieldValue(fieldCode) || []).map(n =>
          n.proserviceAttachmentId ? n : { ...n, proserviceAttachmentId: uuidv4() }
        );
        const finalyDataSource = uniqBy([...updateDataSource, ...res], 'proserviceAttachmentId');
  ...
  const uploadData = file => {
    return {
      ...
      directory: 'sslm-enterprise-inform',
      fileName: file.name,
    };
  };
      ...
      onOk: () => {
        ...
        if (!isEmpty(selectedRows)) {
          ...
          const attIdList = updateRows.map(n => ({
            proserviceAttachmentId: n.proserviceAttachmentId,
          }));
          ...
          if (!isEmpty(createRows)) {
            ...
            const tableValues = getEditTableData(newAttachmentList, ['proserviceAttachmentId']).map(
              n => {
          ...
          if (!isEmpty(updateRows)) {
            onAttachmentRemove({ organizationId, attIdList }).then(res => {
              if (res) {
                ...
                const tableValues = getEditTableData(newAttachmentList, [
                  'proserviceAttachmentId',
                ]).map(n => {
                  const { dueDate, ...others } = n;
                  return {
                    ...others,
  ...
  const handleEdit = (flag, record) => {
    const newDataSource = dataSource.map(n =>
      n.proserviceAttachmentId === record.proserviceAttachmentId
  ...
  const handleOk = () => {
    if (isViewOnly) {
      ...
      const tableValues = getEditTableData(dataSource, ['proserviceAttachmentId']).map(n => {
        const { dueDate, investgProserviceId: investgProserviceIdRecord, ...others } = n;
        return {
          ...others,

          dueDate: dueDate && moment(dueDate).format(DEFAULT_DATETIME_FORMAT),
          ...(_status === 'create' ? {} : { investgProserviceId: investgProserviceIdRecord }),
        };
      });
      const hasUpdateLineFlag = !isEmpty(
        dataSource.filter(n => n._status === 'create' || n._status === 'update')
      );

      // 无修改直接关闭弹框
      if (!hasUpdateLineFlag) {
        handleCancel();
      } else {
        onUploadData(tableValues);
        handleCancel();
        if ($form) {
          $form.setFieldsValue({ firmChangeProserviceAttCount: dataSource.length });
        }
      }
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      onCell,
      render: (_, record) => {
        const { tenantId, attachmentUrl, uploadUserId } = record;
        return (
          <Fragment>
            ...
            {!isViewOnly && record._status !== 'create' && record._status !== 'update' && (
              <a
  ...
  useEffect(() => {
    if (visible) {
      if (investgProserviceId && _status !== 'create') {
        queryAttachment(investgProserviceId);
      } else {
        const updateDataSource = ($form?.getFieldValue(fieldCode) || []).map(n =>
          n.proserviceAttachmentId
            ? n
            : {
                ...n,
                proserviceAttachmentId: uuidv4(),
  ...
  }, [investgProserviceId, visible, _status]);

  const fileCount =
    $form?.getFieldValue('firmChangeProserviceAttCount') || firmChangeProserviceAttCount;

  const redFlag =
    ['insert', 'CREATE', 'DELETE'].includes(firmChangeBeanStateFlag) ||
  ...
  return (
    <Fragment>
      <a onClick={handleOpenUploadModal}>
        ...
        <span style={{ marginLeft: '4px', color: isViewOnly && redFlag && 'red' }}>
          {renderAttachmentText({ editable: !isViewOnly, fileCount, linkColor })}
        </span>
      </a>
      {visible && (
