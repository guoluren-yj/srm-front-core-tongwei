...
const UploadCard = ({
  ...
  pictureHeight = 180,
  requiredFlag = false,
  acceptList = ['image/jpeg', 'image/jpg', 'image/png', 'image/bmp'],
  ...
  return !filePath && !viewOnly ? (
    <Attachment
      ...
      className={classnames(styles['file-card-uuid'], {
        [styles['file-card-uuid-required']]: requiredFlag,
      })}
    >
      {label}
    </Attachment>
    ...
    filePath && (
      <Spin spinning={spin}>
        <div
          className={styles['upload-img-style']}
