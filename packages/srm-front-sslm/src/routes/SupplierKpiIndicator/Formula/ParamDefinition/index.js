...
export default class ParamDefinition extends PureComponent {
  ...
  queryParamDefinition() {
    ...
    const { indicatorFmlId = '' } = currentRecord;
    fetchParamDefinition({ indicatorFmlId }, (res) => {
      if (res) {
        this.setState({
          dataSource: res,
        });
  ...
  handleClean(record) {
    ...
    const newPlatformContactList = dataSource.filter(
      (n) => n.indFmlParamId !== record.indFmlParamId
    );
  ...
  handleEdit(flag, record) {
    ...
    const newPlatformContactList = dataSource.map((item) => {
      if (item.indFmlParamId === record.indFmlParamId) {
  ...
  handleAdd() {
    const { dataSource } = this.state;
    this.setState({
      dataSource: [{ _status: 'create', indFmlParamId: uuidv4(), enableFlag: 1 }, ...dataSource],
    });
  }
  ...
  handleSave() {
    ...
    const { dataSource } = this.state;
    const { indicatorFmlId = '' } = currentRecord;
    const tableValues = getEditTableData(dataSource, ['indFmlParamId']);
    const payload = {
      indicatorFmlId,
      tableValues,
    };
  ...
  render() {
    const { onClose = () => {}, visible, queryParamDefinitionLoading } = this.props;
    const { dataSource } = this.state;
    const drawerProps = {
