...
export default class ParamConfig extends PureComponent {
  ...
  getSnapshotBeforeUpdate(prevProps) {
    ...
    const { indicatorFmlId } = currentRecord;
    return visible && indicatorFmlId && indicatorFmlId !== prevProps.currentRecord.indicatorFmlId;
  }
  ...
  queryParamConfig() {
    ...
    const { indicatorFmlId = '' } = currentRecord;
    fetchParamConfig({ indicatorFmlId }, (res) => {
      if (res) {
        this.setState({
          dataSource: res,
        });
  ...
  handleClean(record) {
    ...
    const newPlatformContactList = dataSource.filter(
      (n) => n.indFmlConfigId !== record.indFmlConfigId
    );
  ...
  handleEdit(flag, record) {
    ...
    const newPlatformContactList = dataSource.map((item) => {
      if (item.indFmlConfigId === record.indFmlConfigId) {
  ...
  handleAdd() {
    const { dataSource } = this.state;
    this.setState({
      dataSource: [{ _status: 'create', indFmlConfigId: uuidv4() }, ...dataSource],
    });
  }
  ...
  deleteExistRows(selectedRows, newDataSource = []) {
    ...
    deleteParamConfig(selectedRows, (res) => {
      if (res) {
        // 接口删除成功，前端过滤删除成功的数据
        const newList = pullAllBy(newDataSource, selectedRows, 'indFmlConfigId');
  ...
  handleDelete() {
    ...
    if (!isEmpty(selectedRows)) {
      ...
      if (isEmpty(newRows)) {
        ...
        const newList = pullAllBy(dataSource, newRows, 'indFmlConfigId');
  ...
  handleSave() {
    ...
    const { dataSource } = this.state;
    const { indicatorFmlId = '' } = currentRecord;
    const tableValues = getEditTableData(dataSource, ['indFmlConfigId']);
    const payload = {
      indicatorFmlId,
      tableValues,
    };
  ...
  render() {
    ...
    const columns = [
      {
        ...
        dataIndex: 'indFmlParamId',
        width: 150,
        render: (val, record) =>
          ['create', 'update'].includes(record._status) ? (
            <FormItem>
              {record.$form.getFieldDecorator('indFmlParamId', {
                initialValue: val,
                rules: [
                  {
                    required: true,
                ...
                <Lov
                  code="SSLM_KPI_IND_PARAM"
                  queryParams={{ indicatorFmlId, tenantId }}
    ...
    const rowSelection = {
      selectedRowKeys: selectedRows.map((n) => n.indFmlConfigId),
