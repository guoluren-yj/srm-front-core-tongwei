            ...
            header={
              <Fragment>
                <div className="process-detail-title">
                  {`${intl.get('sslm.common.model.condition').d('判断条件')}-${conditionIndex + 1}`}
                </div>
                <div className="process-detail-label">{condition.conditionDesc}</div>
              </Fragment>
            }
            ...
            <div className="process-detail-content">
              ...
              <div>
                {condition.authManualFlag
                  ? intl.get('sslm.workbench.model.platformSupplier.authManual').d('允许手工发起')
                  : intl
              ...
              {condition.authTacticsRule && (
                <div>
                  ...
                  <span className="process-detail-rules">{condition.authTacticsRule}</span>
                </div>
              )}
            </div>
          </Panel>
      ...
      {lifeCycleStrategyNodes.map((node, nodeIndex) => {
        ...
        return (
          <Panel
            header={
              <Fragment>
                ...
                <div className="process-detail-label">{node.nodeDesc}</div>
              </Fragment>
            }
          ...
          >
            <div className="process-detail-content">
              <div>
                <span className="process-detail-content-label">
                  {intl.get('sslm.supplierLifePolicyConfig.modal.config.nodeType').d('节点类型')}
                </span>
                <span className="process-detail-rules">{node.nodeTypeMeaning}</span>
              </div>
              <div>
                <span className="process-detail-content-label">
                ...
                <span className="process-detail-rules">{node.documentTypeMeaning}</span>
              </div>
              {node.nodeType !== 'REGULATION' &&
                Boolean((head(lifeCycleStrategyConditions) || {}).authManualFlag) && (
                  <Fragment>
                    <div>
                      ...
                      <span className="process-detail-rules">{node.controlFlagMeaning}</span>
                    </div>
                    <div>
                      <span className="process-detail-content-label">
                        {intl.get('sslm.common.model.field.queryDocRule').d('查询单据规则')}
                      </span>
                      <span className="process-detail-rules">{node.queryDocRuleMeaning}</span>
                    </div>
                  </Fragment>
                )}
              ...
              {node.nodeTacticsRule && (
                <div>
                  ...
                  <span className="process-detail-rules">{node.nodeTacticsRule}</span>
                </div>
              )}
              {node.nodeType === 'REGULATION' && (
                <div>
                  ...
                  <span className="process-detail-rules">
                    {`${intl
