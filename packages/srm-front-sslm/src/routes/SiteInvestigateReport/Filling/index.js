...
@withCustomize({
  unitCode: [
    'SSLM_SITEINVESTIGATE_FILLING_LIST.LIST',
    'SSLM_SITEINVESTIGATE_FILLING_LIST.SEARCH_FORM',
  ],
})
@connect(({ siteInvestigateReport, loading }) => ({
  siteInvestigateReport,
  queryLoading: loading.effects['siteInvestigateReport/queryFillingList'],
}))
export default class Filling extends Component {
  componentDidMount() {
    const {
      siteInvestigateReport: { fillingListPagination = {} },
    } = this.props;
    this.init();
    this.handleQueryList(fillingListPagination);
  }
  ...
  handleQueryList(page = {}) {
    ...
    dispatch({
      type: 'siteInvestigateReport/queryFillingList',
      payload: {
        page,
        ...filterValue,
        creationDateFrom,
        creationDateTo,
        customizeUnitCode:
          'SSLM_SITEINVESTIGATE_FILLING_LIST.LIST,SSLM_SITEINVESTIGATE_FILLING_LIST.SEARCH_FORM',
      },
    });
  }
  ...
  render() {
    ...
    const filterFormProps = {
      ...
      customizeFilterForm,
      custLoading,
      code: 'SSLM_SITEINVESTIGATE_FILLING_LIST.SEARCH_FORM',
    };
    ...
    return (
      <Fragment>
        <Header
          title={intl.get('sslm.siteInvestigateReport.view.filling.title').d('现场考察报告填制')}
        />
        <Content>
          ...
          {customizeTable(
            {
              code: 'SSLM_SITEINVESTIGATE_FILLING_LIST.LIST',
            },
            <Table
              bordered
              rowKey="evalHeaderId"
              ...
              dataSource={fillingList}
              scroll={{ x: scrollX, y: 'calc(100vh - 339px)' }}
              pagination={fillingListPagination}
