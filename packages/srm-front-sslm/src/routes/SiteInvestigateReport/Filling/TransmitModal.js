...
export default class TransmitModal extends Component {
  ...
  render() {
    ...
    const columns = [
      {
        title: intl.get('sslm.siteInvestigateReport.model.docManage.scoreUser').d('评分用户'),
        dataIndex: 'loginName',
        width: 150,
        render: (val, record) =>
                  ...
                  rules: [
                    {
                      ...
                      message: intl.get('hzero.common.validation.notNull', {
                        name: intl
                          .get('sslm.siteInvestigateReport.model.docManage.scoreUser')
                          .d('评分用户'),
                      }),
                    },
                  ],
      ...
      {
        title: intl.get(`sslm.siteInvestigateReport.model.docManage.userName`).d('评分用户描述'),
        dataIndex: 'userName',
        width: 150,
        render: (val, record) =>
          ['create'].includes(record._status) ? (
      ...
      {
        title: intl.get(`sslm.siteInvestigateReport.model.docManage.scoreWeight`).d('权重'),
                ...
                rules: [
                  {
                    ...
                    message: intl.get('hzero.common.validation.notNull', {
                      name: intl
                        .get(`sslm.siteInvestigateReport.model.docManage.scoreWeight`)
                        .d('权重'),
                    }),
                  },
                ],
      ...
      {
        title: intl.get('sslm.siteInvestigateReport.model.docManage.transmitReason').d('转交原因'),
    ...
    return (
      <Drawer
        ...
        title={intl.get('sslm.siteInvestigateReport.view.title.editScorerInfo').d('编辑评分人信息')}
      >
