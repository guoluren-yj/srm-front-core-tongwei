/**
 * 基于 hzero-front@1.5.x lov组件开发，使用c7n-pro table组件替换原h0 table组件，用于优化数据量较大时表格性能渲染问题
 */
import React from 'react';
import { Modal, Input, Icon, Button, message } from 'hzero-ui';
import { isEmpty, isFunction, omit, isNil, join } from 'lodash';
...
export default class Lov extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.state = {
      // Question: 历史版本中为state.text赋值是否有实际意义，lov组件中state.text存储的应该是人为选择数据后的值
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    let data = {
      // 修改：cacheValue只会在选择数据时变更，用cacheValue非空判断是否要比对cacheValue和props.value
      text: this.cacheValue !== undefined && nextProps.value !== this.cacheValue ? undefined : text,
    };
    ...
    queryLov({ viewCode, tenantId, publicMode })
      .then(oriLov => {
        ...
        if (!isEmpty(lov)) {
          ...
          // 获取独立值集编码
          const valueList = lov.queryFields
            ? lov.queryFields.filter(item => item.dataType === 'SELECT')
            : [];
  ...
  emitEmpty() {
    ...
    const { form, onClear = e => e, value, onChange, changeSelectRows = () => {} } = this.props;
    changeSelectRows([]);
    if (onChange) {
      const record = {};
      this.setState(
