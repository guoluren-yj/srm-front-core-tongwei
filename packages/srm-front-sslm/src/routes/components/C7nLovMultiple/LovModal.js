...
class LovModal extends React.Component {
  ...
  initTable() {
    const {
      lov: { pageSize = 10, tableFields, valueField: rowkey = defaultRowKey },
    ...
    const { selection: cusPropsSelection } = customeTableDsprops || {};
    const tableDsProps = {
      autoQuery: true,
      pageSize,
      selection: 'single',
      fields: dsFields,
      dataToJSON: 'selected',
      cacheSelection: cusPropsSelection === 'multiple',
      primaryKey: rowkey,
      ...(customeTableDsprops || {}),
      transport: {
        read: config => {
      ...
      events: {
        batchSelect: this.handleSelect,
        select: ({ dataSet, record }) => {
          if (record.get('hasChild') === '1' && record.children) {
        ...
        batchUnSelect: this.handleSelect,
        load: ({ dataSet }) => {
          const { selectedRows = [] } = this.props;
          const selectedIds = (selectedRows || []).map(i => i[rowkey]);
          dataSet.forEach(record => {
            const valueField = record.get(rowkey);
            if (!isEmpty(selectedIds)) {
              const flag = selectedIds.includes(valueField);
              Object.assign(record, { isSelected: flag });
            }
          });
        },
      },
