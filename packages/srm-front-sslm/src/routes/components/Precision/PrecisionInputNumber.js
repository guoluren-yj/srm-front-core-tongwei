...
export default class PrecisionInputNumber extends React.Component {
  ...
  componentDidMount() {
    ...
    const { uom, currency, financial, precision } = this.state;
    const hasPrecisionProp = Object.hasOwnProperty.call(this.props, 'precision'); // 传递了精度属性
    if (hasPrecisionProp) {
      // 拥有精度属性直接返回, 根据传入的精度为准
      if (type !== 'hzero') {
        if (dataSet) {
          dataSet.setState('precision', precision);
        } else {
          // eslint-disable-next-line no-unused-expressions
          record?.setState('precision', precision);
        }
      }
      return;
    }
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (hasPrecisionProp && (nextPrecision !== precision || hasChangeProp)) {
      ...
      return {
        ...
        currency: nextCurrency,
        financial: nextFinancial,
        precision: isNil(nextUom) && isNil(nextCurrency) && isNil(nextFinancial) ? 10 : precision,
      };
    }
    return null;
  }
  ...
  componentDidUpdate(preProps, preState) {
    ...
    if (isNil(uom) && isNil(currency) && isNil(financial) && !isNil(prePrecision)) {
      ...
      if (type !== 'hzero') {
        if (dataSet) {
          dataSet.setState('precision', undefined);
        } else {
          // eslint-disable-next-line no-unused-expressions
          record?.setState('precision', undefined);
        }
      }
      return;
    }
