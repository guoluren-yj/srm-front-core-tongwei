...
import { forEach, isEmpty, isString, max, min, isUndefined } from 'lodash';
import {
  DatePicker,
  Input,
  InputNumber,
...
import { getDateFormat, getResponse, getCurrentOrganizationId } from 'utils/utils';
// import Upload from 'components/Upload';
import Upload from 'srm-front-boot/lib/components/Upload/index';
// import { openTreeList } from './../../Investigation/Approval/Detail';
 ...
 * @returns {string}
 */
export function getDefaultValue(props = []) {
  let defaultValueMeaning;
  if (!isEmpty(props)) {
    const { attributeValueMeaning } =
      props.find(item => item.attributeName === 'defaultValue') || {};
    defaultValueMeaning = attributeValueMeaning;
  }
...
export function getDisplayValue({
  ...
  const displayFieldCode = getDisplayFieldCodeAttrValue(
    field.props || field.investigateConfigComponents
  );
  let valueMeaning =
  ...
  const defaultValueMeaning = getDefaultValue(field.props || field.investigateConfigComponents);
  valueMeaning = isUndefined(dataSource[field.fieldCode]) ? defaultValueMeaning : valueMeaning;
  const content = dataSource[displayFieldCode] || dataSource[field.fieldCode];
  let format;
  let dateStr;
  switch (field.componentType) {
    case 'ValueList':
    case 'Lov':
      // if (field.fieldCode === 'productCategoryId' && _status === 'approval') {
      //   return (
      //     <span>
      //       {dataSource[displayFieldCode] ||
      //         dataSource[`${field.fieldCode}Meaning`] ||
      //         dataSource[field.fieldCode]}
      //       <Popover placement="right" content={openTreeList({ dataSource })} trigger="click">
      //         <Icon type="info-circle" style={{ fontSize: 15, marginLeft: 10 }} />
      //       </Popover>
      //     </span>
      //   );
      // } else {
 ...
 * @returns {string}
 */
export async function queryAttachmentTypeList({ organizationId } = {}) {
