...
// 跳转详情
const handleJumpDetail = ({ dispatch, record, openTabFlag }) => {
...
const getColumns = ({ dispatch, progressList = [] }) => {
  return [
    {
      name: 'reportStatus',
      width: 120,
    ...
    },
    {
      ...
      renderer: ({ record }) => {
        ...
        return (
          <a
            onClick={() =>
              operationRecordsModal({ documentType: 'REPORT_EVAL', documentId: evalHeaderId })
            }
          >
    ...
    },
    {
      ...
      renderer: ({ record, value }) => (
        <a onClick={() => handleJumpDetail({ dispatch, record })}>{value}</a>
      ),
    },
    {
      name: 'evalNumRe',
      width: 130,
      renderer: ({ record }) => (
        <a onClick={() => handleJumpDetail({ dispatch, record, openTabFlag: true })}>
          {record.get('evalNum')}
        </a>
      ),
    },
    ...
    },
    {
      name: 'evalType',
      width: 100,
      renderer: ({ record }) => record.get('evalTypeMeaning'),
    },
    {
      name: 'assessType',
      width: 120,
      renderer: ({ record }) => record.get('assessTypeMeaning'),
    },
    {
      name: 'realName',
      width: 120,
