...
import React, { useState, useEffect, createElement, forwardRef, useImperativeHandle } from 'react';
...
const SupplementaryInfo = (
  { tenantId, firstActiveKey, configList, supplierBasicId, tableMaxHeight },
  ref
) => {
  ...
  return (
    <div className="supplier-detail-content" id="supplementaryInfo">
      <Tabs tabPosition="left" activeKey={activeKey || firstActiveKey} onChange={handleTabsChange}>
        {renderTabPane()}
      </Tabs>
    </div>
  );
...
export default forwardRef(SupplementaryInfo);
