...
const getAttachmentDs = ({ isEdit }) => ({
  ...
  transport: {
    read: ({ data }) => {
      const { queryParam: { abilityChangeLineId, ...rest } = {}, ...other } = data;
      return {
        url: `${SRM_SSLM}/v1/${organizationId}/supply-ability-change-att-lns/${abilityChangeLineId}`,
        method: 'GET',
        data: {
          ...other,
          ...rest,
    ...
    submit: ({ dataSet }) => {
      const { customizeUnitCode } = dataSet.getQueryParameter('queryParam') || {};
      return {
        url: `${SRM_SSLM}/v1/${organizationId}/supply-ability-change-att-lns`,
        method: 'POST',
