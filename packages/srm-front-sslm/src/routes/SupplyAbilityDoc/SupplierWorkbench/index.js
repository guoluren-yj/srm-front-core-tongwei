...
import { deleteReq, fetchSupTabCount } from '@/services/supplyAbilityDocService';
import { operationRecordsModal } from '@/routes/components/OperationRecords';

import { getCommonParams, hanldeCountryChange } from '../utils/index';
import { getToBeSubmitDs, getWholeOrderAllDs } from './stores/getWholeOrderListDS';
...
const Index = ({
  ...
  useEffect(() => {
    const { toBeSubmitted, wholeOrderAll } = paneDsList;
  ...
  const getColumns = () => {
    const wholeOrderHidden = ['toBeSubmitted', 'wholeOrderAll'].includes(activeKey);
  ...
  const handleJumpDetail = (record, type) => {
    ...
    dispatch(
      routerRedux.push({
        pathname: `/sslm/supply-ability-doc-supplier/detail/${abilityReqId}/${type}`,
      })
    );
  };
  ...
  const handleCreate = () => {
    dispatch(
      routerRedux.push({
        pathname: '/sslm/supply-ability-doc-supplier/create',
      })
    );
  };
    ...
    const editorProps = {
      abilityReqStatus: {
        optionsFilter: record => {
          ...
          return ['NEW', 'WAIT_APPROVAL', 'REJECTED', 'COMPLETED'].includes(record.get('value'));
        },
      },
    };
    return editorProps;
  ...
  return (
    <Fragment>
      <Header
        title={intl
          .get('sslm.supplyAbilityDoc.view.title.supplyAbilityDocSupplier')
          .d('供货能力申请单（供）')}
      >
        <HeaderBtns
          activeKey={activeKey}
      ...
      <Content>
        <Tabs
          ...
          customizable
          customizedCode="sslm-sup-supply-ability-doc"
        >
...
export default compose(
  ...
  withCustomize({
    unitCode: [
      'SSLM.SUPPLY_ABILITY_DOC.SUPPLIER_WHOLE_ORDER.TO_SUBMIT_LIST', // 整单-待提交
      'SSLM.SUPPLY_ABILITY_DOC.SUPPLIER_WHOLE_ORDER.ALL_LIST', // 整单-全部
      'SSLM.SUPPLY_ABILITY_DOC.SUPPLIER_DETAIL_LIST.ALL_LIST', // 明细-全部
    ],
  }),
  withProps(
    () => {
