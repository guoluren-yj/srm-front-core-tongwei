...
export default class Search extends Component {
  ...
  render() {
    ...
    const { type = [], useFlag = [], exportStatus = [] } = enumMap;
    const { expandForm, tenantId } = this.state;
    return customizeFilterForm(
      { form, code: 'SFIN.SUPPLIER_QUERY.FILTER_FORM_NEW', expand: expandForm },
          ...
          <Col span={18}>
            ...
            <Row style={{ display: expandForm ? 'block' : 'none' }} {...SEARCH_FORM_ROW_LAYOUT}>
              ...
              <Col span={8}>
                <Form.Item
                  {...formItemLayout}
                  label={intl.get(`${commonPrompt}.exportStatus`).d('导入状态')}
                >
              ...
              <Col span={8}>
                <FormItem
                  label={intl.get(`${commonPrompt}.billingDateFrom`).d('记账日期从')}
                  {...formItemLayout}
                >
              ...
              <Col span={8}>
                <FormItem
                  label={intl.get(`${commonPrompt}.billingDateTo`).d('记账日期至')}
                  {...formItemLayout}
                >
