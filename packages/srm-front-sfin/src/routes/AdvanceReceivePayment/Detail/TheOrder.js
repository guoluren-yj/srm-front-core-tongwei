...
@withCustomize({
  unitCode: ['SFIN.RECEIVE_PREPAYMENT_DETAIL.LINE_ORDER'],
})
...
export default class RowTable extends Component {
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: intl.get(`${promptCode}.receivedPaymentAmount`).d('剩余可收金额'),
    ...
    return (
      <React.Fragment>
        {customizeTable(
          {
            code: 'SFIN.RECEIVE_PREPAYMENT_DETAIL.LINE_ORDER',
          },
          <EditTable
            bordered
            loading={loading}
