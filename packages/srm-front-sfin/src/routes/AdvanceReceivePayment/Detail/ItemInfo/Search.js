...
const commonPrompt = 'sfin.advanceReceivePayment.model.common';
...
export default class Search extends PureComponent {
  ...
  render() {
    const { form = {}, isOrderLine } = this.props;
          ...
          <Col span={17}>
            <Row {...SEARCH_FORM_ROW_LAYOUT}>
              <Col span={8}>
                <FormItem
                  {...formItemLayout}
                  label={intl.get(`${commonPrompt}.displayNum`).d('采购订单号')}
                >
              ...
              {isOrderLine && (
                <Col span={8}>
                  <Form.Item
                    label={intl.get(`${commonPrompt}.displayLineNum`).d('订单行号')}
                    {...SEARCH_FORM_ITEM_LAYOUT}
                  >
              ...
              <Col span={8}>
                <Form.Item
                  label={intl.get(`${commonPrompt}.orderTypeName`).d('订单类型')}
                  {...SEARCH_FORM_ITEM_LAYOUT}
                >
            ...
            <Row style={{ display: expandForm ? 'block' : 'none' }} {...SEARCH_FORM_ROW_LAYOUT}>
              ...
              <Col span={8}>
                <Form.Item
                  label={intl.get(`${commonPrompt}.publishDateStart`).d('发布日期从')}
                  {...SEARCH_FORM_ITEM_LAYOUT}
                >
              ...
              <Col span={8}>
                <Form.Item
                  label={intl.get(`${commonPrompt}.publishDateEnd`).d('发布日期至')}
                  {...SEARCH_FORM_ITEM_LAYOUT}
                >
