...
export default class Search extends Component {
  ...
  render() {
    ...
    const { type = [], useFlag = [] } = enumMap;
    const { expandForm, tenantId } = this.state;
    return customizeFilterForm(
      { form, code: 'SFIN.RECEIVED_DEDUCTION_LIST.FILTER', expand: expandForm },
          ...
          <Col span={18}>
            <Row {...SEARCH_FORM_ROW_LAYOUT}>
              ...
              <Col span={8}>
                <Form.Item
                  {...formItemLayout}
                  label={intl.get(`${commonPrompt}.customCompanyName`).d('客户公司')}
                >
                ...
                >
                  {getFieldDecorator('statusCode')(
                    <Select style={{ width: '100%' }} allowClear>
                      {type.map((n) => {
                        if (n.value !== 'SUBMITTED' && n.value !== 'PENDING') {
                          return (
