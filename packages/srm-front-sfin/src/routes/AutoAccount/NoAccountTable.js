 ...
 */
@withCustomize({
  unitCode: ['SFIN.AUTO_ACCOUNT_LIST.NO_ACCOUNT_GRID'],
})
@connect(({ autoAccount, loading }) => ({
  autoAccount,
  loading: loading.effects['autoAccount/fetchNoAccount'],
}))
export default class NoAccountTable extends Component {
  state = {
    modalVisible: false,
  };
  ...
  render() {
    const {
      loading,
      customizeTable,
      autoAccount: { dispatch, noAccountList = [], noAccountpagination = {} },
    } = this.props;
    ...
    return (
      <React.Fragment>
        {customizeTable(
          {
            code: 'SFIN.AUTO_ACCOUNT_LIST.NO_ACCOUNT_GRID',
          },
          <EditTable
            bordered
            loading={loading}
            rowKey="billId"
            columns={columns}
            dataSource={noAccountList}
            pagination={noAccountpagination}
