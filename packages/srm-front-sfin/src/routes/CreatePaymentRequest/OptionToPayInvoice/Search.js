...
@withCustomize({
  unitCode: ['SFIN.PAYMENT_REQUEST_CREATE_DETAIL.FILTER'],
})
...
export default class Search extends PureComponent {
  ...
  render() {
    const { customizeFilterForm, form, enumMap = {} } = this.props;
    const { getFieldDecorator, getFieldValue } = form;
    const { sourceList = [], flagReverse = [] } = enumMap;
    const { expandForm, tenantId } = this.state;
    return customizeFilterForm(
      {
        code: 'SFIN.PAYMENT_REQUEST_CREATE_DETAIL.FILTER',
        form,
        expand: expandForm,
      },
        ...
        <Row {...SEARCH_FORM_ROW_LAYOUT}>
          <Col span={18}>
            ...
            <Row style={{ display: expandForm ? 'block' : 'none' }} {...SEARCH_FORM_ROW_LAYOUT}>
              ...
              <Col span={8}>
                <Form.Item
                  label={intl.get(`${commonPrompt}.needInvoiceFlag`).d('显示已移除数据')}
                  {...SEARCH_FORM_ITEM_LAYOUT}
                >
                  {getFieldDecorator('removeFlag')(
                    <Select allowClear>
                      {flagReverse.map((item) => (
                        <Option key={item.value} value={item.value}>
                          {item.meaning}
                        </Option>
                      ))}
              ...
              <Col span={8}>
                ...
                >
                  {getFieldDecorator('purchaseAgentId')(
                    <Lov code="SFIN.USER_AUTH.PURCHASE_AGENT" queryParams={{ tenantId }} />
                  )}
                </Form.Item>
              </Col>
            </Row>
