...
@remote({
  code: 'SFIN_ADVANCEE_PAYMENT_RECORD_DETAIL_CUX',
  name: 'remote',
})
@withCustomize({
  unitCode: [
    'SFIN.ADVANCE_PAYMENT_RECORD_DETAIL.HEADER',
    'SFIN.ADVANCE_PAYMENT_RECORD_DETAIL.LINE_CONTRACT',
    'SFIN.ADVANCE_PAYMENT_RECORD_DETAIL.LINE_SUPPLIER',
    'SFIN.ADVANCE_PAYMENT_RECORD_DETAIL.LINE_ORDER',
  ],
})
...
@connect(({ loading, advancePaymentRecord }) => ({
  advancePaymentRecord,
  loading,
  searchLoading: loading.effects['advancePaymentRecord/handleSearchHeader'],
  listLoading: loading.effects['advancePaymentRecord/fetchInvoiceLine'],
  modalLoading: loading.effects['advancePaymentRecord/fetchModalList'],
  saveLoading: loading.effects['advancePaymentRecord/saveList'],
  subLoading: loading.effects['advancePaymentRecord/handleSubmit'],
  delLoading: loading.effects['advancePaymentRecord/deleteHeader'],
  sdelLoading: loading.effects['advancePaymentRecord/deleteList'],
...
export default class Detail extends PureComponent {
  ...
  componentDidMount() {
    ...
    if (paymentHeaderId && isNumber(+paymentHeaderId)) {
      dispatch({
        type: 'advancePaymentRecord/handleSearchHeader',
        payload: {
          tenantId,
          paymentHeaderId,
          customizeUnitCode: 'SFIN.ADVANCE_PAYMENT_RECORD_DETAIL.HEADER',
        },
      }).then((res) => {
        if (res) {
          this.setState({
  ...
  fetchInvoiceLine(page = {}) {
    ...
    const customizeUnitCode = ['CONTRACT'].includes(paymentSourceTypeCode)
      ...
      : 'SFIN.PAY_QUERY_DETAIL.LINE_ORDER';
    dispatch({
      type: 'advancePaymentRecord/fetchInvoiceLine',
      payload: {
        paymentHeaderId,
        page,
        customizeUnitCode,
    ...
    form.validateFieldsAndScroll((errs, values) => {
      if (!errs) {
        if (InvoiceLineSource.length === 0 || (Array.isArray(lines) && lines.length !== 0)) {
          ...
          dispatch({
            type: 'advancePaymentRecord/saveList',
            payload: {
              ...headerInfoTest,
              paymentAdvanceLineList,
              customizeUnitCode:
      ...
      if (!errs) {
        if (InvoiceLineSource.length === 0 || (Array.isArray(lines) && lines.length !== 0)) {
          ...
          Modal.confirm({
            ...
            onOk: () => {
              dispatch({
                type: 'advancePaymentRecord/validateSubmit',
                payload: {
                  ...dataList,
                  customizeUnitCode:
                    'SFIN.ADVANCE_PAYMENT_RECORD_DETAIL.HEADER,SFIN.ADVANCE_PAYMENT_RECORD_DETAIL.LINE_SUPPLIER,SFIN.ADVANCE_PAYMENT_RECORD_DETAIL.LINE_CONTRACT,SFIN.ADVANCE_PAYMENT_RECORD_DETAIL.LINE_ORDER',
                ...
                if (r) {
                  if (r.validatedCode === 'SUCCESS') {
                    dispatch({
                      type: 'advancePaymentRecord/handleSubmit',
                      payload: {
                        ...dataList,
                        customizeUnitCode:
                          'SFIN.ADVANCE_PAYMENT_RECORD_DETAIL.HEADER,SFIN.ADVANCE_PAYMENT_RECORD_DETAIL.LINE_SUPPLIER,SFIN.ADVANCE_PAYMENT_RECORD_DETAIL.LINE_CONTRACT,SFIN.ADVANCE_PAYMENT_RECORD_DETAIL.LINE_ORDER',
                      ...
                      if (res) {
                        notification.success();
                        history.push({ pathname: '/sfin/advance-payment-record/list' });
                      }
                    });
                  }
  ...
  handleDelete() {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        dispatch({
          type: 'advancePaymentRecord/deleteHeader',
          payload: delList,
        }).then((res) => {
          if (res) {
            notification.success();
  ...
  fetchModalList(page = {}) {
    ...
    dispatch({
      type: 'advancePaymentRecord/fetchModalList',
      payload: {
        paymentHeaderId,
        headerIdList,
        page,
  ...
  async onItemInfoModalOk() {
    ...
    const newDataSource = modalList.map((item) => {
      // const { taxIncludedAmount, amountPrecision } = item;
      // const paymentAmount = Number(
      //   taxIncludedAmount * (getFieldValue('advanceRatio') / 100)
      ...
      return {
        ...
        _status: 'create',
        edited: true,
        advanceRatio: getFieldValue('advanceRatio'), // 预付比例
        // paymentAmount:
        //   getFieldValue('advanceRatio') === null || getFieldValue('advanceRatio') === undefined
        //     ? item.paymentAmount || {}
        //     : paymentAmount, // 本次付款金额-默认值
      };
    });
    if (remoteProps) {
      const res = await remoteProps.event.fireEvent('beforeAddLineCux', {
        InvoiceLineSource,
        newDataSource,
        headerInfo,
      });
  ...
  deleteModalList() {
    ...
    if (!isEmpty(oldDataSource)) {
      dispatch({
        type: 'advancePaymentRecord/deleteList',
        payload: {
          paymentHeaderId,
          body: oldDataSource,
        },
  ...
  supplierChange(val, record) {
    ...
    registerField('bankId');
    registerField('bankFirm');
    setFieldsValue({
      bankName: record.bankName,
      bankBranchName: record.bankBranchName,
      bankAccountName: record.bankAccountName,
  ...
  bindHeaderAttachmentUuid(attachmentUuid) {
    ...
    dispatch({
      type: 'advancePaymentRecord/bindHeaderAttachmentUuid',
      payload: {
        paymentHeaderId,
        attachmentUuid,
      },
  ...
  render() {
    ...
    return (
      <Fragment>
        <Header
          backPath="/sfin/advance-payment-record/list"
          title={intl
            .get('sfin.advancePaymentRecord.view.message.advancePaymentRecordAdd')
            .d('预付款申请创建')}
        >
          <Button
        ...
        <Content>
          ...
          >
            ...
            >
              ...
              >
                <Alert
                  message={intl
                    .get(`sfin.advancePaymentRecord.view.title.prompt`)
                    .d(
                      '❗当前收款银行默认展示供应商已启用的主账号银行信息，若不符合本次业务场景，可点击【收款银行】字段，重新选择银行信息'
                    )}
                  type="info"
                  closable
                  showIcon
          ...
          <Modal
            title={intl.get(`${common}addPaymentLine`).d('新增付款行')}
            destroyOnClose
            width={1000}
