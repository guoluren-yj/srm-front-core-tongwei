 ...
 */
@withCustomize({
  unitCode: ['SFIN.BILL_CREATE_DETAIL.DETAILED'],
})
...
export default class DetailTable extends Component {
  ...
  render() {
    ...
    const selectedRowKeys = detailSelectedRows.map((n) => n.trxAndLineNum);
      ...
      },
      {
        ...
        render: (text, record) => {
          if (billFlag) {
            return record.priceShieldFlag === 1
              ? '***'
              : thousandBitSeparatorIsNew(record.netAmount, record.amountPrecision);
          } else {
            return record.priceShieldFlag === 1
              ? '***'
              : thousandBitSeparatorIsNew(text, record.amountPrecision);
          }
        },
      },
      {
      ...
      },
      {
        ...
        render: (text, record) => {
          if (billFlag) {
            return record.priceShieldFlag === 1
              ? '***'
              : thousandBitSeparatorIsNew(record.taxIncludedAmount, record.amountPrecision);
          } else {
            return record.priceShieldFlag === 1
              ? '***'
              : thousandBitSeparatorIsNew(text, record.amountPrecision);
    ...
    return customizeTable(
      {
        code: 'SFIN.BILL_CREATE_DETAIL.DETAILED',
      },
      <Table
        bordered
        rowKey="trxAndLineNum"
        columns={columns}
        dataSource={content}
        pagination={detailPagination}
        onChange={onTableChange}
        scroll={{ x: this.scrollWidth(columns, 250) }}
        rowSelection={modalRowSelectedChange ? rowSelection : {}}
        // scroll={{ x: 3250 }}
      />
    );
  }
