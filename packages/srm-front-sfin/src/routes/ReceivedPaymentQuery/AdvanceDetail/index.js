...
@connect(({ receivedPayQuery, payApprove, loading }) => ({
  receivedPayQuery,
  payApprove,
  headerLoading: loading.effects['receivedPayQuery/queryHeader'],
  fetchAdvanceLineLoading: loading.effects['receivedPayQuery/fetchAdvanceLine'],
}))
...
export default class PayDetail extends Component {
  ...
  fetchAdvanceLine(page = {}) {
    ...
    dispatch({
      type: 'receivedPayQuery/fetchAdvanceLine',
  ...
  render() {
    ...
    const { headerLoading = false, fetchAdvanceLineLoading } = this.props;
    const {
      collapseKeys,
      headerInfo = {},
      InvoiceLineSource = [],
    ...
    return (
      <Fragment>
        <Header
          title={intl.get(`sfin.payment.common.AdvancePayDetail`).d('预收款申请明细')}
          backPath="/sfin/receive-pay-query/list"
        >
          <Upload {...uploadProps} />
        </Header>
        <Content>
          ...
          <Spin spinning={false} wrapperClassName={DETAIL_DEFAULT_CLASSNAME}>
            ...
            >
              <Panel
                ...
                header={
                  <React.Fragment>
                    <h3>{intl.get(`${common}beforeReceivePayHeader`).d('预收款头信息')}</h3>
                    <a>
              ...
              <Panel
                ...
                header={
                  <React.Fragment>
                    <h3>{intl.get(`${common}beforeReceivePayLine`).d('预收款行信息')}</h3>
                    <a>
