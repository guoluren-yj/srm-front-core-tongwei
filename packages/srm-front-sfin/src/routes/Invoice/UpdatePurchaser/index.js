...
@formatterCollections({
  code: ['sfin.invoiceBill', 'entity.company', 'entity.supplier'],
})
@withCustomize({
  unitCode: ['SFIN.INVOICE_UPDATE_LIST.GRID', 'SFIN.INVOICE_UPDATE_PURCHASER_LIST.FILTER'],
})
export default class Update extends SearchPage {
  ...
  pageConfig() {
    return {
      ...
      cacheKey: '/sfin/invoice-update-purhcaser/list',
      paramsFilter: (values) => {
        ...
        return {
          ...
          isPurchaser: true,
          customizeUnitCode:
            'SFIN.INVOICE_UPDATE_LIST.GRID,SFIN.INVOICE_UPDATE_PURCHASER_LIST.FILTER',
        };
      },
    };
  }
  ...
  linkDetail(invoiceHeaderId) {
    ...
    history.push(`/sfin/invoice-update-purchaser/detail/${invoiceHeaderId}`);
  }
  ...
  renderHeader() {
    return (
      <Header
        title={intl.get(`${promptCode}.view.title.invoice.updatePurchaser`).d('维护应付发票')}
      />
    );
  }
  ...
  renderForm(form) {
    ...
    return customizeFilterForm(
      {
        code: 'SFIN.INVOICE_UPDATE_PURCHASER_LIST.FILTER',
        form,
        expand: expand.update,
      },
                ...
                <FormItem label={intl.get('hzero.common.status').d('状态')} {...formItemLayout}>
                  {getFieldDecorator('invoiceStatus')(
                    <Select allowClear>
                      ...
                      <Option value="RETURN_TO_SUBMIT">
                        {intl
                          .get('sfin.invoiceBill.model.invoiceBill.returnSubmit')
                          .d('退回至提交人')}
                      </Option>
                    </Select>
                  )}
