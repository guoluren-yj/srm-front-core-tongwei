...
@withCustomize({
  unitCode: [
    'SFIN.INVOICE_REVIEW_DETAIL.TAX_LINE',
...
export default class Detail extends PureComponent {
  ...
  confirm() {
    ...
    const lines = detailLine.review.content;
    const invoiceLines = getEditTableData(lines, ['_status']);
    const { invoiceHeaderId } = this.state;
    this.headerForm.validateFieldsAndScroll((err, values) => {
      const { reviewedRemark } = values;
      if (!err) {
        confirm({
          ...
          onOk: () => {
            dispatch({
              ...
              payload: {
                type: 'review',
                body: [{ invoiceHeaderId, reviewedRemark, checkSource: 'COPE', invoiceLines }],
        ...
        confirm({
          ...
          onOk() {
            dispatch({
              ...
              payload: {
                type: 'review',
                body: [{ invoiceHeaderId, reviewedRemark, invoiceLines }],
              },
            }).then((res) => {
              if (res) {
                notification.success();
  ...
  render() {
    const { recordModal, infDataSource = [], infPagination = {}, tabKey } = this.state;
    const {
      dispatch,
      confirmLoading,
      rejectLoading,
    ...
    return (
      <React.Fragment>
        <Header
          backPath="/sfin/invoice-review/list"
          title={intl.get(`${promptCode}.view.message.title.invoiceReview.detail`).d('发票复核')}
        >
          <Button
            type="primary"
            icon="check"
          ...
          <Button icon="close" loading={confirmLoading || rejectLoading} onClick={this.reject}>
        ...
        <Content>
          ...
          {customizeTabPane(
            ...
            >
              ...
              >
                <TaxDetailTable
                  hcuzCode="SFIN.INVOICE_REVIEW_DETAIL.TAX_LINE"
                  code="SFIN.INVOICE_REVIEW_DETAIL.TAX_LINE"
