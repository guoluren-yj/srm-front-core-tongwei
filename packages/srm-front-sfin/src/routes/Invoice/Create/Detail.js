...
import ItemInfo from '../Update/ItemInfo';
...
const remoteCodeMap = {
  BTNS: 'SFIN_INVOICE_CREATE_DETAIL.BTNS',
  BTN_SAVE: 'SFIN_INVOICE_CREATE_DETAIL.BTNS_SAVE',
  PROCESS: 'SFIN_INVOICE_CREATE_DETAIL_VALIDATE',
  OCR_TITLE: 'SFIN_INVOICE_CREATE_DETAIL_CUX.OCR_TITLE',
  BTN_TICKET_CREATE: 'SFIN_INVOICE_CREATE_DETAIL_CUX.CREATE_SHOW_FLAG',
};

@remote({
  code: 'SFIN_INVOICE_CREATE_DETAIL',
  name: 'remote',
})
...
export default class CreateDetail extends Component {
  ...
  saveInvoice() {
    ...
    const { attachmentUUID, tenantId, invoiceHeaderId } = this.state;
  ...
  invoiceModalOk(invoiceTypeCode) {
    ...
    const { invoiceHeaderId, tenantId } = this.state;
  ...
  hanldeAddModal() {
    ...
    const { businessType } = detailHeader.create || {};
  ...
  @Bind()
  handleDeleteLines() {
    const { invoiceHeaderId, selectedLines, tenantId } = this.state;
    const { dispatch } = this.props;
    dispatch({
  ...
  handleTaxLovChange(taxRate) {
    ...
    if (selectedLines && selectedLines.length > 0) {
      const ratePlusDiv = math.plus(1, math.div(taxRate, 100)); // (1 + taxRate / 100)
              ...
              selectedLines[0].priceUpdFlag === 1
                ? {
                    ...
                    taxIncludedPrice: this.numberFixed(
                      math.multipliedBy(i.netPrice, ratePlusDiv),
                      2
                    ),
            ...
            const pushData =
              selectedRows[0].priceUpdFlag === 1
                ? {
                    ...i,
                    taxRate,
                    taxIncludedPrice: math.multipliedBy(i.netPrice, ratePlusDiv),
                    // taxAmount: Math.round(i.netAmount * taxRate / 100, 2),
            ...
            const pushData =
              selectedRows[0].priceUpdFlag === 1
                ...
                : {
                    ...i,
                    taxRate,
                    taxIncludedPrice: i.netPrice * ratePlusDiv,
                  };
            if (i.invoiceLineId === j.invoiceLineId) {
              data.push(pushData);
            }
  ...
  fetchModalList(page = {}) {
    ...
    const modalList = [];
    const { infDataSource = [], tenantId } = this.state;
