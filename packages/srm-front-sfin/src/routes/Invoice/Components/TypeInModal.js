...
import { Modal, Form, Input, Spin, Button } from 'hzero-ui';
    ...
    if (editable) {
      footer.push(
        <Button
          key="submit"
          type="primary"
          loading={queryLogisticsLoading || submitLogisticsLoading}
      ...
      <Modal
        ...
        title={
          editable
            ...
            : intl.get(`${promptCode}.title.LogisticsInforView`).d('物流信息查看')
        }
      ...
      >
        <Spin spinning={queryLogisticsLoading || submitLogisticsLoading}>
          ...
          <Form.Item
            ...
            {...formItemLayout}
          >
            {getFieldDecorator('logisticsRemark')(<Input disabled={!editable} />)}
          </Form.Item>
          <Form.Item
            label={intl.get(`${promptCode}.model.invoiceBill.accepterName`).d('收件人姓名：')}
            {...formItemLayout}
          >
            {getFieldDecorator('logisticsReceiver')(<Input disabled={!editable} />)}
          </Form.Item>
          <Form.Item
            label={intl.get(`${promptCode}.model.invoiceBill.accepterPhone`).d('收件人电话：')}
            {...formItemLayout}
          >
            {getFieldDecorator('logisticsReceiveTelNum')(<Input disabled={!editable} />)}
          </Form.Item>
          <Form.Item
            label={intl.get(`${promptCode}.model.invoiceBill.accepterAddress`).d('收件人地址：')}
            {...formItemLayout}
          >
            {getFieldDecorator('logisticsReceiveAddress')(<Input disabled={!editable} />)}
          </Form.Item>
        </Spin>
      </Modal>
    );
