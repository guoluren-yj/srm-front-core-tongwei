...
const commonPrompt = 'spcm.supplierChargeEntry.model';
...
export default class Search extends Component {
  ...
  render() {
    ...
    const { type = [], flag = [] } = enumMap;
    const { expandForm, tenantId } = this.state;
    return customizeFilterForm(
      { form, code: 'SFIN.SUPPLIER_APPROVED.FITTER.NEW', expand: expandForm },
          ...
          <Col span={18}>
                ...
                >
                  {getFieldDecorator('accountSubjectId')(
                    <Lov
                      ...
                      lovOptions={{ displayField: 'accountSubjectName' }}
                      textField="accountSubjectName"
                      // textValue={accountSubjectName}
                ...
                >
                  {getFieldDecorator('supplierCompanyId')(
                    <Lov
                      ...
                      queryParams={{ tenantId }}
                      onOk={(record) => {
                        setFieldsValue({ supplierCompanyId: record.supplierCompanyId });
                      }}
                    />
                  )}
                </Form.Item>
            ...
            <Row style={{ display: expandForm ? 'block' : 'none' }} {...SEARCH_FORM_ROW_LAYOUT}>
              ...
              <Col span={8}>
                <Form.Item
                  {...formItemLayout}
                  label={intl.get(`${commonPrompt}.ticketDeductionFlag`).d('是否票扣')}
                >
                  {getFieldDecorator('ticketDeductionFlag')(
                    <Select style={{ width: '100%' }} allowClear>
                      {flag.map((n) => (
                        <Option key={n.value} value={n.value}>
                          {n.meaning}
