...
} from '@/services/advanceReceivePaymentServices';
import { validateSubmit, batchValidateSubmit } from '@/services/paymentServices';

export default {
  namespace: 'advanceReceivePayment',
  state: {
    listQuery: {},
    supplierLovList: [],
  },
  effects: {
    ...
    *listSettle({ payload, setPagination }, { call, put, spawn }) {
      ...
      const response = yield call(listSettle, { ...payload, asyncCountFlag: 'Y' });
    ...
    *batchValidateSubmit({ payload }, { call }) {
    ...
    // 查询供应商lov
    *fetchSupplierLov({ payload }, { call, put }) {
      const response = yield call(fetchSupplierLov, payload);
      const data = getResponse(response);
      if (data) {
        yield put({
          type: 'updateState',
          payload: { supplierLovList: data.content },
        });
      }
      return data;
    },
