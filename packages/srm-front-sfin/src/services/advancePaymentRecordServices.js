...
export async function handleSearchHeader(params) {
  const { customizeUnitCode, paymentHeaderId, ...query } = parseParameters(params);
  return request(
...
export async function saveList(data) {
  const { customizeUnitCode, ...other } = data;
  return request(
    `${SRM_FINANCE}/v1/${organizationId}/payment-headers/advance-save?customizeUnitCode=${customizeUnitCode}`,
    {
      method: 'POST',
      body: other,
    }
...
export async function handleSubmit(data) {
  const { customizeUnitCode, ...other } = data;
  return request(
    `${SRM_FINANCE}/v1/${organizationId}/payment-headers/advance/submit?customizeUnitCode=${customizeUnitCode}`,
    {
      method: 'POST',
      body: other,
    }
