...
function thirdPartyParamDS() {
  return {
    primaryKey: 'thirdPartyParamId',
    autoQuery: true,
    selection: 'multiple',
    ...
    transport: {
      read: ({ data }) => {
        if (data.thirdPartyId) {
          const { thirdPartyId } = data;
          return {
            url: `${SRM_SMBL}/v1/third-party-params/${thirdPartyId}`,
            method: 'get',
          };
        } else {
          return null;
      ...
      destroy: {
        url: `${SRM_SMBL}/v1/third-party-params/`,
        method: 'delete',
      },
      create: {
        url: `${SRM_SMBL}/v1/third-party-params/`,
