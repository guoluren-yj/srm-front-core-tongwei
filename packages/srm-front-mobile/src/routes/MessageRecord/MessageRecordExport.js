...
import { resendMessageRecord, ignoreMessageRecord } from '@/services/messageRecordService';
...
export default class MessageChannelManageList extends PureComponent {
  ...
  async handleResend() {
    ...
    const res = await resendMessageRecord(selectData);
    if (getResponse(res)) {
      dataSet.query();
  ...
  async handleIgnore() {
    ...
    const selectData = dataSet.selected.map((item) => item.toJSONData());
    if (!selectData.length) return;
    const res = await ignoreMessageRecord(selectData);
    if (getResponse(res)) {
      dataSet.query();
      notification.success();
    }
