...
import { srmCustomizeDS } from './dataSet';
...
export default class SrmCustomize extends Component {
  constructor(props) {
    super(props);
    this.customizeDataSet = new DataSet(
      srmCustomizeDS(({ dataSet }) => {
        this.reloadCustomizeConfigs(dataSet);
      })
    );
    this.lovCreateDs = new DataSet({fields: srmCustomizeDS().fields});
  }
  ...
  reloadCustomizeConfigs = (dataSet) => {
    ...
    // 防止死循环
    if (result !== oldResult) {
      this.props.updateCustomizeConfigs(datas, 'srm');
    }
  };
  ...
  render() {
    const { customizeDataSet } = this;
    return (
      <Fragment>
        <div className={styles["srm-customize-content"]}>
          <Table
