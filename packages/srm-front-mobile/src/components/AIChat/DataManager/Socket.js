...
class Socket {
  ...
  heartbeatInfo = {
    count: 0,
    oldSessionId: null, // 旧seesionId
    sessionId: null, // 当前sessionId
  };
  ...
  onOpen(evt) {
    ...
    this.connect?.send?.(JSON.stringify({ key: 'PING' }));
  ...
  onMessage(evt) {
    ...
    if (result.message === 'PONG') {
      if (this.heartbeatInfo.sessionId !== result.sessionId) {
        this.heartbeatInfo = {
          count: 0,
          oldSessionId: this.heartbeatInfo.sessionId,
  ...
  onClose(evt) {
    ...
    // 需要重连的状态码
    const reconnectCodes = [1000, 1003, 1004, 1005, 1006, 1007, 1008, 1009, 1010];
  ...
  send(message) {
    ...
    // if (['roomInMsgHandler', 'jikeRoomInMsgHandler'].includes(message.key)) {
    //   _message.message.preSession = this.heartbeatInfo.oldSessionId || '';
    // }
    _message.message = JSON.stringify(_message.message);
    this.connect?.send?.(JSON.stringify(_message));
  }
