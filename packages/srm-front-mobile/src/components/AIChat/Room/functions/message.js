...
// 处理消息
export function judgeMessageRecords(records) {
  let lastShowTimeStamp = 0;
  return records.map(record => {
    let showTime = true;
    const time = record?.creationDate
      ? Math.floor(Date.parse(record?.creationDate?.replace(/-/g, '/')) / 1000)
      : '';
    if (time && time - lastShowTimeStamp < 300) {
      showTime = false;
    } else {
      showTime = true;
      lastShowTimeStamp = time;
...
export function textReplaceLink(text) {
  ...
  return text
    ? text.replace(exp, `<a href='$1' target="_blank" style="text-decoration: revert;">$1</a >`)
    : text;
}
