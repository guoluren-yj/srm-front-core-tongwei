...
export default class FieldEditor extends Component {
  ...
  renderOtherOptions() {
    ...
    } else if (["SELECT", "LOV", "RADIOGROUP"].includes(component)) {
      moreOptions = (
        <FormItem
          label={intl
          ...
          {getFieldDecorator('modelFieldWidget.sourceCode', {
            ...
            <Lov
              ...
              code={getSingleTenantValueCode(
                component === 'SELECT' ? 'HPFM.CUST.LOV_DETAIL_CODE' : 'HPFM.LOV_VIEW'
              )}
              textField="modelFieldWidget.sourceCode"
              lovOptions={{
                displayField: component === 'SELECT' ? 'lovCodeNew' : 'viewCode',
                valueField: component === 'SELECT' ? 'lovCodeNew' : 'viewCode',
              }}
            />
          )}
        </FormItem>
    ...
    return (
      ...
      >
        ...
        <div className={styles['model-bottom-button']}>
          <Button type="primary" loading={saveFieldLoading} style={{ marginRight: 8 }} onClick={this.saveField}>
            {intl.get('hzero.common.button.save').d('保存')}
          </Button>
          <Button onClick={handleClose} disabled={saveFieldLoading}>
