...
import { Spin, Tooltip, Icon, Menu, Dropdown, Modal } from 'hzero-ui';
...
export default class SearchBarConfig extends Component {
  ...
  @Bind()
  saveFilter(filter) {
    const { dispatch, unitInfo = {}, filterList = [], onRefresh = () => {} } = this.props;
  ...
  handleRankFilterList(targetFilterId, rankType = false) {
    const { dispatch, unitInfo = {}, filterList = [], onChange = () => {} } = this.props;
    // 过滤出启用的筛选器
    ...
    return (
      <Spin spinning={saveLoading || deleteLoading || false}>
        <div className={styles['searchBar-container-left-title']}>
          ...
          <Tooltip
            placement="bottom"
            title={intl.get('hzero.common.button.createFilter').d('新建筛选器')}
          >
            <span>
