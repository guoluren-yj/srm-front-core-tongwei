...
import { Form, Modal, Popconfirm, Icon, Select, Input, Button, Tooltip } from 'hzero-ui';
...
export default class ConditionModal extends React.Component {
  ...
  getRightField(index, i) {
    const { form, ctxParams, unitId: currentUnitId, fieldList, readOnly } = this.props;
      ...
      rightFields.push(
        <FormItem style={{ maxWidth: '151px' }}>
          {form.getFieldDecorator(`targetValue#${index}`, {
            ...
            <Select allowClear style={{ width: '100%' }} key={index} disabled={readOnly}>
  ...
  setFieldOneInfo(id, conCode, config) {
    ...
    if (fieldObj) {
      ...
      form.setFieldsValue({
        ...
        [`sourceModelFieldCode#${conCode}`]: fieldObj.unitFieldCode,
  ...
  clearFields(level, conCode) {
    ...
    switch (level) {
      case 1:
        form.setFieldsValue({
          [`conExpression#${conCode}`]: undefined,
  ...
  addValidator() {
    ...
    const alreadyExitsConNo = validatorList.map(i => i.conCode);
    while(alreadyExitsConNo.includes(conCode)) {
      conCode++;
    }
        ...
        <FormItem style={{ maxWidth: '151px' }}>
          {form.getFieldDecorator(`sourceField#${index}`, {
            ...
            <Select
              ...
              disabled={readOnly || form.getFieldValue(`sourceUnitId#${index}`) === undefined}
              optionLabelProp="title"
              dropdownClassName={styles['condition-dropdown']}
            >
                      ...
                      {relationShip
                        .filter(k =>
                          getFilter(
                            ...
                            form.getFieldValue(`sourceUnitType#${conCode}`)
                          ).includes(
                            k.value
                          )
                        )
