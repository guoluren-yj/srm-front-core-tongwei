...
import { Form, Popconfirm, Icon, Input, InputNumber, DatePicker, Badge } from 'hzero-ui';
...
export default class DefaultValueModal extends BaseCondition {
  getDefaultValueComponent(record) {
    const { externalForm, paramList, ctxParams, readOnly, unitType } = this.props;
    const {
      fieldWidget,
      multipleFlag,
      sourceCode,
    ...
    switch (fieldWidget) {
      ...
      case 'LOV':
        widgetProps = {
          code: sourceCode,
          queryParams: getParams({ paramList, ctxParams }),
          disabled: !sourceCode,
        };
        if (multipleFlag === 1) {
          Widget = LovMulti;
      ...
      case 'DATE_PICKER': {
        ...
        widgetProps = {
          format,
          showTime: true,
          mode: isMonthPicker ? 'month' : isTimeType ? 'time' : 'date',
          style: { width: '100%' },
        };
      ...
      case 'SWITCH':
        ...
        formOptions.initialValue = String(record.value || 0);
        break;
      default:
        Widget = Input;
    ...
    return (
      <FormItem>
        {record.$form.getFieldDecorator(
          'value',
          formOptions
        )(<Widget {...widgetProps} disabled={readOnly} />)}
      </FormItem>
    );
  }
  ...
  openProDefaultModal(form) {
    const { record, unitId, ctxParams, readOnly } = this.props;
    Modal.open({
    ...
    return [
      {
        ...
        render: (val, record) => (
          <FormItem wrapperCol={{ span: 24 }}>
            ...
            })(<Input inputChinese={false} disabled={readOnly} />)}
          </FormItem>
        ),
      },
      {
  ...
  onOk() {
    ...
    form.validateFields((err, values) => {
      ...
      if (typeof updateSelfValidator === 'function') {
        updateSelfValidator('defaultValue', {
          ...selfValidator,
