...
import { getAccessToken, getCurrentOrganizationId, getResponse, getRequestId } from 'utils/utils';
  ...
  if (reg1.test(newUrl) || reg2.test(newUrl)) {
    if (queryParams && Array.isArray(queryParams)) {
      queryParams.forEach(item => {
        if (item.name === 'url') {
          fileUrl = item.value ? decodeURIComponent(item.value) : '';
        }
      });
    }
  ...
  return withTokenAxios(
    ...
    try {
      ...
      const temp = !headers['content-disposition']
        ? undefined
        : headers['content-disposition'].match(/[fF][iI][Ll][Ee][Nn][Aa][Mm][Ee]=(.*)/)[1];
      fileName = temp;
      ...
      try {
        ...
        notificationType(err.type, err.message);
      } catch (error) {
        console.error(e);
      }
      throw err;
 ...
 * @param {array} params.queryParams 下载文件请求的查询参数，参数格式为：[{ name: '', value: '' }]]
 */
export async function downloadFile(params, flag) {
