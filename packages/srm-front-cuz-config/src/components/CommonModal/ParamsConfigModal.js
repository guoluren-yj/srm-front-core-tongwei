...
export default class ConditionModal extends React.Component {
  ...
  onOk() {
    const { paramList } = this.state;
    const { onSave, onClose, unitList = [] } = this.props;
    const newParams = getEditTableData(paramList, ['_id', '_status']) || [];
    if (paramList.length > 0 && newParams.length === 0) {
      return;
    }
    paramList.forEach(line => {
      const unitConfig = unitList.find(
        // eslint-disable-next-line eqeqeq
        unit => unit.unitId == line.$form.getFieldValue('paramUnitId')
      );
      if (unitConfig) {
        // eslint-disable-next-line no-param-reassign
        line.paramUnitCode = unitConfig.unitCode;
      }
    });
  ...
  getParamValueByType(type, form, record) {
    if (type === 'fixed' || type === 'self') {
      return (
        <Input
          trim
          disabled={record.paramId === undefined && record._status !== 'create'}
          placeholder={
            type === 'self'
              ? intl.get('hpfm.customize.common.selfParamKey').d('自定参数名')
              : undefined
          }
        />
      );
  ...
  autoCompleteFieldKey(code, form) {
    const { fieldList = {} } = this.props;
    const modelCode = code ? code.split('#')[0] : undefined;
    const modelFieldCode = code ? code.split('#')[1] : undefined;
    const fieldObj =
      modelCode && modelFieldCode
        ? (fieldList[form.getFieldValue(`paramUnitId`)] || []).find(
            i => i.modelCode === modelCode && i.modelFieldCode === modelFieldCode
          )
        : undefined;
    form.setFieldsValue({
      paramKey: fieldObj.unitFieldCode,
      paramModelCode: fieldObj.modelCode,
      paramFieldCode: fieldObj.modelFieldCode,
    });
  }

  render() {
    const { paramList } = this.state;
    const { visible, onClose, unitList = [], fieldList = {}, id } = this.props;
    // 创建确定后再点开会禁用，待优化
    const columns = [
      {
                  ...
                  rules: [
                    ...
                    },
                    {
                      validator(_, value = '', cb) {
                        if (/^\d+$/.test(value)) {
                          cb(
                            intl.get('hpfm.customize.common.validate.notAllNum').d('不能为纯数字')
                          );
                        }
                        cb();
                      },
              ...
              <FormItem wrapperCol={{ span: 24 }}>
                {$form.getFieldDecorator('paramType', {
                  ...
                  >
                    ...
                    <Option value="self">
                      {intl.get('hpfm.customize.common.self').d('自定参数')}
                    </Option>
                  </Select>
                )}
              </FormItem>
            ...
            return (
              <div style={{ width: '100%', display: 'flex' }}>
                ...
                >
                  ...
                  })(this.getParamValueByType(paramType, $form, record))}
                </FormItem>
                <FormItem
                  style={{
                    display: paramType === 'unit' ? 'inline-block' : 'none',
                ...
                >
                  {$form.getFieldDecorator('paramField', {
                    initialValue:
                      record.paramModelCode && record.paramFieldCode
                        ? `${record.paramModelCode}#${record.paramFieldCode}`
                        : undefined,
                    rules: [
                      {
                    ...
                    >
                      {(fieldList[$form.getFieldValue(`paramUnitId`)] || []).map(f1 => (
                        <Option
                          value={`${f1.modelCode}#${f1.modelFieldCode}`}
                  ...
                  {$form.getFieldDecorator('paramModelCode', {
                    initialValue: record.paramModelCode,
                  })}
                  {$form.getFieldDecorator('paramFieldCode', {
                    initialValue: record.paramFieldCode,
                  })}
                </FormItem>
              </div>
            );
      ...
      },
      {
        ...
        width: 60,
        render: (val, record) =>
          record.paramId === undefined && record._status !== 'create' ? null : (
            <Popconfirm
