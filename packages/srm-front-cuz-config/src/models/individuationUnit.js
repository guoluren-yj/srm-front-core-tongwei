...
export default {
  namespace: 'individuationUnitCuz',
  state: {
    dataSource: [],
    pagination: {},
    tables: [],
  ...
  effects: {
    ...
    *fetchMenu({ params }, { call }) {
    ...
    // 获取菜单树
    *fetchMenuPlatform({ params }, { call }) {
      const response = getResponse(yield call(queryMenuTreePlatform, params));
      return response;
    },
    ...
    *modifyUnit({ params }, { call }) {
      const response = yield call(modifyUnit, params);
      return response;
    },
