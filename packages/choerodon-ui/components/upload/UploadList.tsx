...
export default class UploadList extends Component<UploadListProps, any> {
  ...
  render() {
    ...
    const list = items.map((file, index) => {
      ...
      const actions =
        (stat.isPicture || stat.isPictureCard) && !stat.isUploading ? (
          <span className={actionsClass}>
            {stat.isError || previewIcon}{downloadIcon}{reUploadIconOrText}{removeIcon}{extraAction && extraAction(file)}
          </span>
        ) : (
          removeIcon
        );
      ...
      ) : <span className={`${prefixCls}-list-item-text`}>{
        extraAction ? (
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <div style={{ flex: '1' }}>{icon}{preview}</div>
            <div>{extraAction(file)}</div>
          </div>
        ): <>{icon}{preview}</>
      }</span>;
