...
export default class Alert extends PureComponent<AlertProps, any> {
  ...
  componentDidMount() {
    if (this.element && isOverflow(this.element)) {
      this.setState({
        tooltip: true,
        styles: { opacity: 1, overflow: 'hidden' },
      });
    } else {
      this.setState({
        styles: { opacity: 1 },
  ...
  render() {
    ...
    const { closing, closed, tooltip, styles } = this.state;
    ...
    const messageDom = (<span className={`${prefixCls}-message`} style={styles}>{message}</span>);
    const descriptionDom = (<span className={`${prefixCls}-description`} style={styles}>{description}</span>);
    ...
    return closed ? null : (
      ...
      >
        <div hidden={!closing} className={alertCls} style={style} {...dataOrAriaProps}>
          ...
          <div className={`${prefixCls}-content`} ref={this.saveRef}>
            {message ? (description || !tooltip ? messageDom : (<Tooltip title={message}>{messageDom}</Tooltip>)) : undefined}
            {description ? (tooltip ? (<Tooltip title={description}>{descriptionDom}</Tooltip>) : descriptionDom) : undefined}
          </div>
          {closeIcon}
        </div>
      </Animate>
