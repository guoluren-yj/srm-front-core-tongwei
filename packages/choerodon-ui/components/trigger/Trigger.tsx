import React, { Children, Component, CSSProperties, isValidElement, Key, MouseEventHandler, MouseEvent, ReactElement, ReactNode } from 'react';
...
export default class Trigger extends Component<TriggerProps> {
  ...
  @autobind
  handlePopupMouseLeave(e: MouseEvent<HTMLDivElement>) {
    ...
    if (ownerDocument) {
      ...
      // 当有选择文本时阻止关闭, 修复react在选中文本时会触发mouseleave的问题
      if (selection && !selection.isCollapsed && currentTarget.contains(selection.anchorNode)) {
        const subscribe = () => {
          new EventManager(ownerDocument).addEventListener('selectionchange', () => {
            const newSelection = ownerDocument.getSelection();
            if (newSelection && currentTarget.contains(selection.anchorNode)) {
              subscribe();
            } else {
              emit();
            }
