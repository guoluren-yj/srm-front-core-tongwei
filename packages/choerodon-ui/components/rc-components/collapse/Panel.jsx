...
const CollapsePanel = function CollapsePanel(props) {
  ...
  const compactTrigger = trigger === "text-icon";
  const headerCls = classNames(`${prefixCls}-header`, headerClass, {
    ...
    [`${prefixCls}-header-compact`]: compactTrigger,
  });
  ...
  const icon = showArrow ? (
    <span className={`${prefixCls}-expand-icon-wrapper`} onClick={expandIconPosition === "right" || trigger === 'icon' ? handleItemClick : undefined}>
  ...
  const headerChildren = (
    <>
      {!['text-right', "right"].includes(expandIconPosition) && icon}
      {header}
      {expandIconPosition === 'text-right' && icon}
    </>
  )
  ...
  return (
    <div className={itemCls} style={style} id={id} ref={wrapRef} tabIndex={-1}>
      ...
      >
        {compactTrigger ? <div onClick={handleItemClick} className="compact-left">{headerChildren}</div> : headerChildren}
