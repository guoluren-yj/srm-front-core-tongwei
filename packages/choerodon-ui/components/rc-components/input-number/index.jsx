...
export default class InputNumber extends Component {
  ...
  onChange = (e) => {
    ...
    // 限制数字输入框只能输入数字
    const eventValue = this.getValueFromEvent(e);
    let newValue = this.props.parser(eventValue);
    ...
    if (this.composing) {
      ...
      if (this.deferCancelComposing) {
        this.deferCancelComposing = false;
        this.composing = false;
      }
    } else {
      const newValue = this.toNumber(this.rawInput);
      const newState = { inputValue: this.rawInput };
  ...
  onBlur = (e, ...args) => {
    ...
    const value = this.getCurrentValidValue(isNil(inputValue) ? '' : this.props.parser(String(inputValue)));
    e.persist();
  ...
  onComposition = (e) => {
    if (e.type === 'compositionstart') {
      ...
      e.preventDefault();
      this.deferCancelComposing = true;
    }
  };
  ...
  setValue(v, callback) {
    ...
    const { value: stateValue = null, inputValue = null } = this.state;
    const newValueInString = math.isFinite(newValue)
      ? math.toFixed(newValue, precision) : `${newValue}`;
    const changed = newValue !== ('value' in this.props ? value : stateValue) || newValueInString !== `${inputValue}`;
    if (!('value' in this.props)) {
      this.setState({
        value: newValue,
  ...
  formatWrapper(num) {
    ...
    if (!this.state.focused && math.isNegativeZero(num)) {
      return '-0';
    }
