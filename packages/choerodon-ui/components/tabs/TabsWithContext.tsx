...
const TabsWithContext: FunctionComponent<TabsWithContextProps> = function TabsWithContext(props) {
  ...
  const [activeGroupKey, groupMatch] = useMemo((): [string | undefined, boolean | undefined] => {
    if (groupedPanelsMap.size) {
      ...
      return [getDefaultGroupKey(groupedPanelsMap), undefined];
    }
  ...
  const groupCurrentKeyMap = useMemo(() => new Map<string, string | undefined>(), []);
  if (activeGroupKey && groupMatch) {
    groupCurrentKeyMap.set(activeGroupKey, activeKey);
  }
