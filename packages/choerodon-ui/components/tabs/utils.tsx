...
export function getActiveKeyByGroupKey(
  groupCurrentKeyMap: Map<string, string | undefined>,
  groupedPanelsMap: Map<string, GroupPanelMap>,
  key: string,
): string | undefined {
  const lastActiveKey = groupCurrentKeyMap.get(key);
  if (lastActiveKey !== undefined) {
    return lastActiveKey;
  }
  const map = groupedPanelsMap.get(key);
  if (map) {
    const { group, panelsMap } = map;
    if ('defaultActiveKey' in group) {
      const { defaultActiveKey } = group;
      if (getActiveIndex(panelsMap, defaultActiveKey) !== -1) {
        return defaultActiveKey;
      } 
    }
