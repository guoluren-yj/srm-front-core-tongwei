...
import { DebouncedFunc, DebounceSettings } from 'lodash/index';
...
export default class Button extends DataSetComponent<ButtonProps> {
  ...
  get btnIcon() {
    const { loading, prefixCls, props: { icon } } = this;
    return loading ? (
      <Progress key="loading" type={ProgressType.loading} size={Size.small} className={`${prefixCls}-icon`} />
    ) : (
      icon && <Icon type={icon} className={`${prefixCls}-icon`} />
    );
  }
  ...
  handleMouseEnter(e) {
    ...
    const { element } = this;
    if (tooltip === ButtonTooltip.always || (tooltip === ButtonTooltip.overflow && isOverflow(
      element,
    ))) {
      show(element, {
    ...
    return super.getClassName(
      {
        ...
        [`${prefixCls}-tooltip`]: tooltip && [ButtonTooltip.always, ButtonTooltip.overflow].includes(tooltip) && !help && !icon,
      },
      ...props,
    );
  }
  ...
  render() {
    const { children, href, funcType, help } = this.props;
    const { loading, disabled, btnIcon } = this;
    const autoInsertSpace = this.getContextConfig('autoInsertSpaceInButton') !== false;
    ...
    const button = (
      <Ripple disabled={disabled || funcType === FuncType.link}>
        <Cmp {...omit(props, omits)}>
          {btnIcon}
          {hasString ? <span className={help || btnIcon ? `${this.prefixCls}-overflow-text` : undefined}>{kids}</span> : kids}
          {help ? <Tooltip title={help}><Icon type="help" onMouseEnter={hide as any} className={`${this.prefixCls}-help`} /></Tooltip> : undefined}
        </Cmp>
      </Ripple>
    );
