...
function Picture(props: PictureProps, ref: Ref<PictureForwardRef>) {
  const {
    src, promiseSrc, downloadUrl, previewUrl, previewTarget, lazy, className, width, height, prefixCls, style, sources, alt, title, block = true, preview = true,
    objectFit = 'fill', objectPosition = 'center', status: propStatus, border, index, onClick, children, onPreview, customModal, ...rest
  } = props;
  const modal = useModal();
  const url = previewUrl || src;
  const pictureRef = useRef<PictureRef>({ src: url, promiseSrc: undefined, downloadUrl });
  ...
  const handlePreview = useCallback(() => {
    if (preview && !previewTarget && status === 'loaded' && (url || promiseSrc)) {
      if (context && isNumber(index)) {
        context.preview(index, customModal ? modal : undefined);
      } else {
        modalPreview({ list: [{ src: url, promiseSrc, downloadUrl }], previewModal: customModal ? modal : undefined });
      }
      if (onPreview) {
        onPreview();
      }
    }
  }, [modal, context, index, preview, previewTarget, status, url, downloadUrl, onPreview]);
  ...
  useEffect(() => {
    if (preview && !previewTarget && context && isNumber(index) && (url || promiseSrc)) {
      const { current } = pictureRef;
      current.src = url;
      current.promiseSrc = promiseSrc;
      context.registerPicture(index, current);
      return () => context.unRegisterPicture(index, current);
    }
  }, [index, context, pictureRef, preview, previewTarget, url, promiseSrc]);
