...
export interface PictureViewerProps {
  ...
  defaultIndex?: number;
  previewModal?: ModalContextValue;
}
...
const PictureViewer: FunctionComponent<PictureViewerProps & { modal?: ModalChildrenProps }> = function PictureViewer(props) {
  ...
  const [stateSrcList, setStateSrcList] = useState<PictureRef[]>();
  ...
  useEffect(() => {
    if (list && list.length && list.some(pic => !!getPreviewItem(pic).promiseSrc)) {
      const promises = list.map(pic => {
        const { src, promiseSrc } = getPreviewItem(pic);
        if (isFunction(promiseSrc)) {
          return promiseSrc();
        }
        return Promise.resolve(src);
      });
      Promise.all(promises).then(srcList => {
        const newList = list.map((pic, index) => ({...getPreviewItem(pic), src: srcList[index]}));
        setStateSrcList(newList);
      });
    }
  }, [list, setStateSrcList]);
  const { length } = stateSrcList || list;
  if (length) {
    const previewItem = getPreviewItem((stateSrcList || list)[index]);
    if (!previewItem) {
      return null;
    }
    const { src, promiseSrc, downloadUrl } = previewItem;
    return (
