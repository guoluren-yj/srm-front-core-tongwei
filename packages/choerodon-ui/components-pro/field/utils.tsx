...
const getDateShowFormatByField: typeof Utils.getDateShowFormatByField = Utils.getDateShowFormatByField;
...
export function getTelFormatter(field?: Field, record?: Record, value?: string, pristine = false) {
  if (field && record && value) {
    const regionName = field.get('regionField');
    if (regionName) {
      const regionValue = pristine ? record.getPristineValue(regionName) : record.get(regionName);
      return regionValue ? `${regionValue} ${value}` : value;
    }
  }
  return value;
}
...
export function processFieldValue(value, field: Field | undefined, options: { getProp(name: string): any; getValue?(): any; lang?: string }, showValueIfNotFound?: boolean, record?: Record, getConfig = getConfigDefault, pristine = false) {
  const { getProp, getValue, lang } = options;
  const type = getProp('type');
  const currency = getProp('currency');
  const precision = getProp('precision');
  const padDecimalZeros = getProp('padDecimalZeros');
  if (currency || type === FieldType.currency) {
    ...
    if (padDecimalZeros) {
      return padNumberTextDecimalZeros((formatter || getCurrencyFormatter(getConfig))(value, formatOptions.lang, formatOptions.options), precision);
    }
  ...
  if (type === FieldType.number || type === FieldType.bigNumber) {
    ...
    const formatter = getProp('formatter');
    if (padDecimalZeros) {
      return padNumberTextDecimalZeros((formatter || getNumberFormatter(getConfig))(value, formatOptions.lang, formatOptions.options), precision);
    }
    return (formatter || getNumberFormatter(getConfig))(value, formatOptions.lang, formatOptions.options);
  }
  if (field) {
    const text = field.getText(value, showValueIfNotFound, record);
    if (type === FieldType.tel) {
      return getTelFormatter(field, record, text, pristine);
    }
    return text;
  }
  return value;
...
export function processValue(value: any, options: ProcessValueOptions = {}) {
  if (!isNil(value)) {
    const date = options.isDate && isString(value) && value ? Moment(value) : value;
    if (isMoment(date)) {
      if (date.isValid()) {
        if (options.dateFormat && options.dateFormat.includes('MMM')) {
          return date.clone().locale('en').format(options.dateFormat);
        }
...
// 小数补零
export function padNumberTextDecimalZeros(numStr: string, decimalPlaces: number) {
  if (numStr && decimalPlaces > 0) {
    const [, decimalPart] = numStr.split('.');
    if (!decimalPart) {
      return `${numStr}.${'0'.repeat(decimalPlaces)}`
    }
    if (decimalPart.length < decimalPlaces) {
      return `${numStr}${'0'.repeat(decimalPlaces - decimalPart.length)}`
    }
  }
  return numStr;
}