...
import { isHTMLInputElement, isHTMLTextAreaElement } from 'choerodon-ui/shared/util';
...
export class FormField<T extends FormFieldProps = FormFieldProps> extends DataSetComponent<T, FormContextValue> {
  ...
  get dataSet(): DataSet | undefined {
    ...
    if (isObjectLike(observableProps) && 'dataSet' in observableProps) {
  ...
  get record(): Record | undefined {
    const { observableProps } = this;
    if (isObjectLike(observableProps) && 'record' in observableProps) {
  ...
  get rangeAllRequired(): boolean {
    return this.getProp('rangeAllRequired');
  }
  ...
  isPreventRenderer(): boolean | undefined {
    const { preventRenderer } = this.props;
    return preventRenderer;
  }

  @autobind
  defaultRenderer(renderProps: RenderProps): ReactNode {
    const renderedText = defaultRenderer(renderProps);
    const precision = this.getProp('precision');
    const padDecimalZeros = this.getProp('padDecimalZeros');
    if (precision && isString(renderedText) && padDecimalZeros) {
      return padNumberTextDecimalZeros(renderedText, precision);
    }
    return renderedText;
  }
    ...
    return super.getWrapperClassNames(
      {
        ...
        [`${prefixCls}-float-label-empty`]: this.hasFloatLabel && !this.getLabel(),
  ...
  componentWillReceiveProps(nextProps: T, nextContext) {
    ...
    if (!nextProps.noValidate && !this.record && !this.compare(this.props.value, nextProps.value)) {
  ...
  getValidatorProp(key: string) {
    switch (key) {
      ...
      case 'timeZone':
        return this.getContextConfig('formatter').timeZone;
      default:
  ...
  showTooltip(e): boolean {
    if (!this.hasFloatLabel && this.showValidation === ShowValidation.tooltip) {
      ...
      if (
        !this.isFocus
        && ((isHTMLInputElement(element) && element.type === 'text') || isHTMLTextAreaElement(element))
        && isOverflow(element)
      ) {
        const { context: { getTooltipTheme, getTooltipPlacement } } = this;
        show(element, {
          title: element.value || element.placeholder,
  ...
  handleFloatLabelMouseEnter(e) {
    const { getTooltipTheme, getTooltipPlacement, getTooltip } = this.context;
    const { labelTooltip = getTooltip('label') } = this;
  ...
  getIsDate(field: Field | undefined = this.field): boolean {
    const dateTypes = [FieldType.date, FieldType.dateTime, FieldType.month, FieldType.time];
    return dateTypes.includes(this.getFieldType(field)) || dateTypes.includes(this.getFieldShowType(field));
  }
  ...
  getTextByValue(value: any): ReactNode {
    const text =
      this.isPreventRenderer() || (this.editable && this.isFocused)
  ...
  getFieldShowType(field: Field | undefined = this.field): FieldType {
    return field && field.get('showType', this.record) || FieldType.string;
  }
