...
export class Select<T extends SelectProps = SelectProps> extends TriggerField<T> {
  ...
  renderSearchFieldInner(_props: any): ReactNode {
  ...
  renderSearchField(props?: any): ReactNode {
    ...
    return (
      <div className={`${prefixCls}-search-bar`}>
        ...
        {this.renderSearchFieldInner(props)}
      </div>
    );
  }
  ...
  compare(oldValue, newValue) {
    const { valueField } = this;
    if (valueField && !isArrayLike(oldValue) && !isArrayLike(newValue) && isObject(oldValue) && isObject(newValue)) {
      return oldValue[valueField] === newValue[valueField];
    }
