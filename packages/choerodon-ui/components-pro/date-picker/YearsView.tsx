...
export default class YearsView<T extends DateViewProps> extends DaysView<T> {
  ...
  @autobind
  handlePrevYearClick() {
    const prevYear = this.getCloneDate().subtract(10, 'y');
    this.props.getDisabledYearData(prevYear);
    this.changeCursorDate(prevYear, ViewMode.decade);
  }
  ...
  @autobind
  handleNextYearClick() {
    const nextYear = this.getCloneDate().add(10, 'y');
    this.props.getDisabledYearData(nextYear);
    this.changeCursorDate(nextYear, ViewMode.decade);
  }
