...
import { action, computed, IReactionDisposer, isArrayLike, observable, runInAction, reaction } from 'mobx';
import { TimeStep, TimeZone } from 'choerodon-ui/dataset/interface';
import dateCodeStore, { DateStoreProps } from 'choerodon-ui/dataset/stores/DateCodeStore';
import KeyCode from 'choerodon-ui/lib/_util/KeyCode';
import warning from 'choerodon-ui/lib/_util/warning';
import { DateCode } from 'choerodon-ui/dataset/configure';
...
export default class DatePicker extends TriggerField<DatePickerProps>
  ...
  @observable disabledDate?: DateStoreProps | undefined;

  @observable currentDateCode?: string | undefined;
  ...
  componentDidMount() {
    ...
    this.reaction = reaction(() => {
      const { record, name, field, currentDateCode } = this;
      return record && name && field && field.get('dateCode', record) !== currentDateCode;
    }, () => this.getDisabledData());
  }

  getDisabledData() {
    const date = this.getCursorDate();
    this.getDisabledYearData(date);
    this.getDisabledNearingYearData(date);
    if (moment().year() !== date.year()) {
      this.getDisabledYearData(moment());
    }
  }

  @autobind
  async getDisabledYearData(value: Moment) {
    const autoDisabledDate = this.getProp('autoDisabledDate');
    const dateCode = this.getProp('dateCode');
    const { disabledDate = {} } = this;
    const currentValue = moment(value);
    if (currentValue && autoDisabledDate && dateCode) {
      const currentYear = moment(currentValue).format('YYYY');
      const response = await dateCodeStore.fetchDateConfig(dateCode, currentYear);
      if (response && response.status === 'success' && !disabledDate[response.year]) {
        runInAction(() => {
          disabledDate[currentYear] = response.date;
          this.disabledDate = { ...disabledDate };
          this.currentDateCode = dateCode;
          if (this.field) {
            this.field.set('dateCode', dateCode);
          }
        });
      }
    }
  ...
  @autobind
  getDisabledNearingYearData(value: Moment) {
    const currentValue = moment(value);
    const autoDisabledDate = this.getProp('autoDisabledDate');
    const dateCode = this.getProp('dateCode');
    if (autoDisabledDate && dateCode && currentValue) {
      const currentMonth = moment(currentValue).format('MM');
      if (currentMonth === '12') {
        this.getDisabledYearData(currentValue.clone().add(1, 'y'));
      }
      if (currentMonth === '01') {
        this.getDisabledYearData(currentValue.clone().subtract(1, 'y'));
      }
    }
  }
  ...
  isPreventRenderer() {
    if (super.isPreventRenderer()) {
      return true;
    }
    const { editorInPopup } = this.observableProps;
    return editorInPopup && this.popup;
  }

  @autobind
  getContextShowFormat() {
  ...
  defaultRenderer(props: RenderProps): ReactNode {
    let renderedText = super.defaultRenderer(props);
    const { showFormat = this.getContextShowFormat() } = this.props;
    const enLocale = showFormat && showFormat.includes('MMM');
    if (showFormat && props.value) {
      renderedText = (enLocale ? moment(props.value).clone().locale('en') : moment(props.value)).format(showFormat);
    }
    const mode = this.getDefaultViewMode();
    const { value } = props;
    if (value && [ViewMode.time, ViewMode.dateTime].includes(mode)) {
      ...
      if (timeZone) {
        const renderedTimeZone = typeof timeZone === 'function' ? timeZone(value, {}) : (enLocale ? value.clone().locale('en') : value).format(timeZone);
      ...
      <>
        ...
        }
        {
          createElement(viewComponents[mode], {
            ...
            getDisabledYearData: this.getDisabledYearData,
            getDisabledNearingYearData: this.getDisabledNearingYearData,
          } as DateViewProps)
        }
      </>
    );
  ...
  checkMoment(item) {
    ...
    if (!this.props.useInvalidDate && isString(item) && !moment(item, format, true).isValid()) {
  ...
  handleCursorDateChange(cursorDate: Moment, selectedDate: Moment, mode?: ViewMode) {
    if (
      (mode && mode !== ViewMode.date) || (this.isUnderRange(cursorDate, mode) && this.isDateOutOfFilter(cursorDate, selectedDate, mode))
    ) {
  ...
  exchangeRangeValue(start: Moment, end: Moment) {
    ...
    if (defaultTime && moment.isMoment(start) && moment.isMoment(end)) {
  ...
  isValidDate(currentDate: Moment, selected: Moment, mode?: ViewMode): boolean {
    const range = this.isUnderRange(currentDate, mode) && this.isDateOutOfFilter(currentDate, selected, mode);
    const year = currentDate.year();
    if (range && year && this.disabledDate && this.disabledDate[year]) {
      return !this.disabledDate[year].includes(moment(currentDate).format('YYYY-MM-DD'));
    }
    return range;
  }
  ...
  isValidNowDate(selected: Moment): boolean {
    ...
    if (isValid && autoDisabledDate && this.currentDateCode && this.disabledDate && this.disabledDate[moment().year()]) {
      return !this.disabledDate[moment().year()].includes(moment().format('YYYY-MM-DD'));
    }
    return isValid;
  }
  ...
  getValidatorProp(key: string) {
    ...
    switch (key) {
      ...
      case 'disabledDate':
        return this.disabledDate && this.disabledDate[moment(this.value).format('YYYY')];
      default:
