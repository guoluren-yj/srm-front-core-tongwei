...
import { DateStoreProps } from 'choerodon-ui/dataset/stores/DateCodeStore';
...
export interface DateViewProps extends ViewComponentProps {
  ...
  disabledDate?: DateStoreProps | undefined;
  getDisabledYearData: (date: Moment) => void;
  getDisabledNearingYearData: (date: Moment) => void;
}
...
export default class DaysView<T extends DateViewProps> extends ViewComponent<T>
  ...
  handleBeforeChange(date: Moment, autoNextDay?: boolean) {
    const { disabledDate } = this.props;
    if (disabledDate) {
      const disabledYearDate = disabledDate[moment(date).format('YYYY')];
      if (disabledYearDate) {
        if (autoNextDay) {
          const nextDay = moment(date).add(1, 'd');
          return disabledYearDate.includes(this.getDisableDateFormat(nextDay)) ? this.handleBeforeChange(nextDay, true) : nextDay;
        } 
        const isDisabled = disabledYearDate.includes(this.getDisableDateFormat(date));
        if (isDisabled) {
          const firstDayOfMonth = date.startOf('M');
          return disabledYearDate.includes(this.getDisableDateFormat(firstDayOfMonth)) ? this.handleBeforeChange(firstDayOfMonth, true) : firstDayOfMonth;
        }
      }
    }
    return date;
  ...
  @autobind
  handlePrevYearClick() {
    const prev = this.getCloneDate().subtract(1, 'y');
    this.props.getDisabledYearData(prev);
    this.changeCursorDate(prev, ViewMode.year);
  }

  @autobind
  handlePrevMonthClick() {
    const prev = this.getCloneDate().subtract(1, 'M');
    this.props.getDisabledNearingYearData(prev);
    this.changeCursorDate(this.handleBeforeChange(prev), ViewMode.month);
  }
  ...
  @autobind
  handleNextYearClick() {
    const next = this.getCloneDate().add(1, 'y');
    this.props.getDisabledYearData(next);
    this.changeCursorDate(next, ViewMode.year);
  }

  @autobind
  handleNextMonthClick() {
    const next = this.getCloneDate().add(1, 'M');
    this.props.getDisabledNearingYearData(next);
    this.changeCursorDate(this.handleBeforeChange(next), ViewMode.month);
  }
  ...
  handleCellClick(date: Moment): void {
    this.props.getDisabledYearData(date);
    this.props.getDisabledNearingYearData(date);
    this.choose(date);
  }
  ...
  getDisableDateFormat(date: Moment = this.props.date): string {
