...
import { PatchingConfig } from 'choerodon-ui/dataset/configure';
import { __DATASET_PATCHING_CONFIG__, __DATASET_GET_REAL_COUN_STATUS__, DATASET_GET_REAL_COUNT_STATUS } from 'choerodon-ui/dataset/data-set/DataSet';
...
import PatchingIcon from './PatchingIcon';
...
export default class Pagination extends DataSetComponent<PaginationProps> {
  ...
  @observable patchingExpanded?: boolean;
  ...
  @computed
  get limtCount(): boolean {
    const { dataSet } = this.observableProps;
    return dataSet && dataSet.getState(__DATASET_GET_REAL_COUN_STATUS__) === DATASET_GET_REAL_COUNT_STATUS.READY && !!this.total && this.total > 999;
  }
  ...
  get totalPage(): number {
    ...
    if (dataSet && !this.limtCount) {
      return dataSet.totalPage;
    }
    if (total !== undefined && pageSize !== undefined) {
      if (this.limtCount) {
        return Math.ceil(Math.min(total, 1000) / pageSize);
      }
  ...
  get next(): boolean {
    ...
    return this.limtCount || page < Math.floor((total - 1) / pageSize) + 1;
  }
  ...
  handleChange(page: number, pageSize: number) {
    ...
    if (this.pageSize !== pageSize) {
      ...
      if (dataSet) {
        ...
        dataSet.query(1, dataSet.patchingParams, true);
      }
    } else {
  ...
  @autobind
  async handlePagerClick(page, type) {
    // 判断最后一页是否需要查总数
    if (this.dataSet && this.limtCount && type === 'last') {
      this.dataSet.setState(__DATASET_GET_REAL_COUN_STATUS__, DATASET_GET_REAL_COUNT_STATUS.DOING);
      this.dataSet.query().then(() => {
        this.jumpPage(this.dataSet!.totalPage);
      });
    } else {
      this.jumpPage(page);
    }
  ...
  handleJumpChange(value) {
    ...
    if (!this.limtCount && value > totalPage) {
      value = totalPage;
    }
    if (simple) {
      this.jumpPage(value);
  ...
  @autobind
  @action
  togglePatchingExpanded() {
    this.patchingExpanded = isBoolean(this.patchingExpanded) ? !this.patchingExpanded : false;
  }

  @autobind
  @action
  handleCurrentPatching({ target }) {
    const { patchingConfig, dataSet } = this.props;
    if (patchingConfig && dataSet) {
      const { value } = target;
      const patchingOption = patchingConfig.options.find(option => option.value === value);
      if (patchingOption) {
        const { value } = patchingOption;
        dataSet.unSelectAll();
        dataSet.clearCachedSelected();
        dataSet.setState(__DATASET_PATCHING_CONFIG__, { ...patchingOption, name: patchingConfig.name });
        dataSet.setQueryParameter(patchingConfig.name, value);
        dataSet.query();
      }
    }
  }
  ...
  async count() {
    const { dataSet } = this;
    if (dataSet) {
      dataSet.setState(__DATASET_GET_REAL_COUN_STATUS__, DATASET_GET_REAL_COUNT_STATUS.DOING);
      await dataSet.query();
    }
  }
  ...
  renderTotal(pageSize: number, page: number, total: number): ReactNode {
    ...
    const to = this.limtCount ? pageSize * page : Math.min(pageSize * page, total);
    const counting = dataSet && dataSet.counting;
    const totalNode = this.limtCount ? (
      <Tooltip title={$l('Pagination', 'get_real_total')}>
        <a onClick={this.count}>1000+</a>
      </Tooltip>
    ): total;
  ...
  renderPatching(): ReactNode {
    const { patchingConfig, dataSet } = this.props;
    if (patchingConfig && dataSet) {
      const { prefixCls, patchingExpanded = true } = this;
      return (
        <Button
          funcType={FuncType.link}
          onClick={this.togglePatchingExpanded}
          tooltip={ButtonTooltip.none}
          className={classNames(
            `${prefixCls}-patching`,
            {
              [`${prefixCls}-patching-expanded`]: patchingExpanded,
            })}
        >
          <PatchingIcon />
          {$l('Pagination', 'patching_setting')}
        </Button>
      )
    }
  }

  renderPatchingOptions(): ReactNode {
    const { patchingConfig, dataSet } = this.props;
    if (patchingConfig && dataSet) {
      const { name, options, description } = patchingConfig;
      if (name && options) {
        const { prefixCls, patchingExpanded = true } = this;
        const { value } = dataSet.getState(__DATASET_PATCHING_CONFIG__) || options[0];
        return (
          <div
            className={
              classNames(
                `${prefixCls}-patching-options`,
                {
                  [`${prefixCls}-patching-options-expanded`]: patchingExpanded,
                })}
          >
            <Radio.Group value={value} onChange={this.handleCurrentPatching}>
              {
                options.map(option => <Radio value={option.value} key={`patching-radio-${option.value}`} >{option.meaning}</Radio>)
              }
            </Radio.Group>
            <span className={`${prefixCls}-patching-description`}>
              {$l('Pagination', 'patching_distinction')}
