...
export class Cascader<T extends CascaderProps> extends TriggerField<T> {
    ...
    const treePropsChange = (treeRecord: Record[], isFilterSearch = false) => {
      ...
      if (treeRecord.length > 0) {
        treeRecords = treeRecord.map((recordItem) => {
          ...
          const itemContent = this.getMenuItem({ record: recordItem, text, value }); 
          return (children ? {
  ...
  getMenuItem({ record, text, value }: RenderProps): string | ReactNode {
    const {
      options,
  ...
  getPopupContent(): ReactNode {
    ...
    if (this.multiple) {
      // 超出600条时全选过程会明显卡顿
      const disabledSelectAll = this.cascadeOptions.length > 600;
      const checkAllClass = classNames(`${this.prefixCls}-select-all-none`, {
        [`${this.prefixCls}-select-all-none-disabled`]: disabledSelectAll,
      });
      if (disabledSelectAll) {
        return [
          <Tooltip title={$l('Cascader', 'disabled_select_all')} key="disabled-select--all">
            <div key="check-all" className={checkAllClass}>
              <span>{$l('Cascader', 'select_all')}</span>
              <span>{$l('Cascader', 'unselect_all')}</span>
            </div>
          </Tooltip>,
          menu,
        ];
  ...
  handleKeyDownEnter(e) {
    if (this.popup && !this.editable) {
      ...
      if (this.isSelected(value) && this.multiple) {
        ...
        this.setPopup(true);
      } else if (value && value.get && !value.get(disabledField)) {
        this.choose(value);
      }
    }
    e.preventDefault();
