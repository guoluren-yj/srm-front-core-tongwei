...
export default class Lov extends Select<LovProps> {
  ...
  get searchMatcher(): SearchMatcher {
    ...
    if (viewMode === TriggerViewMode.popup) {
      ...
      if (config) {
        const { lovItems } = config;
        if (lovItems && lovItems.length) {
          const defaultQueryField = lovItems.find(i => i.conditionFieldDefaultSelect);
          if (defaultQueryField && defaultQueryField.gridFieldName) {
            return defaultQueryField.gridFieldName;
          }
        }
      }
  ...
  get patchingConfig(): PatchingConfig | undefined {
    const lovPatching = this.context.getConfig('lovPatching');
    if (lovPatching && isFunction(lovPatching)) {
      return lovPatching().find(code => code.code === this.lovCode);
    }
  }
  ...
  @autobind
  renderSearchFieldInner(props?: TableQueryBarHookProps): ReactNode {
    if (!props || !props.queryDataSet) {
      return null;
    }
    const { queryDataSet } = props;
    const { searchText, prefixCls, searchMatcher } = this;
    const searchFieldProps = this.getSearchFieldProps();
    const { multiple, className } = searchFieldProps;
    const field = isString(searchMatcher) && queryDataSet.getField(searchMatcher as string);
    const fieldType = field ? field.get('type') : undefined;
    if (fieldType === LovFieldType.SELECT && field) {
      ...
      return (
        ...
        >
          {options ? options.map(i => (
            <Option key={i.get(valueField)} value={i.get(valueField)}>{i.get(textField)}</Option>
          )) : null}
        </ObserverSelect>
      );
    }
    return super.renderSearchFieldInner(props);
  }
    ...
    if (config && options) {
      ...
      const mergedTableProps: Partial<TableProps> | undefined = mergeProps<Partial<TableProps>>(tableProps, {
        ...
        pagination: {
          ...
          patchingConfig: this.patchingConfig,
      ...
      const { showSelectedInView } = this;
      return (
        <LovView
          key={this.record?.id}
          {...lovViewProps}
          showSelectedInView={showSelectedInView}
          viewMode={viewMode}
          dataSet={options}
          config={config}
  ...
  beforeOpen(options: DataSet): Partial<LovViewProps> | undefined {
    const { multiple, primitive, valueField, lovCode } = this;
    if (multiple) {
  ...
  afterOpen(options: DataSet, fetchSingle?: boolean) {
    if (this.autoSelectSingle) {
      ...
      const { patchingConfig } = this;
      if (this.resetOptions(noCache) && fetchSingle !== true && !this.multiple) {
        options.query(1, options.patchingParams, true);
      ...
      if (patchingConfig) {
        options.setQueryParameter(patchingConfig.name, patchingConfig.options[0].value);
      }
    }
  }
  ...
  searchRemote(text?: string | string[] | undefined) {
    ...
    if (isString(searchMatcher) && (viewMode === TriggerViewMode.popup || !isSearchTextEmpty(text))) {
      ...
      if (this.isSearchFieldInPopup() || this.props.searchAction === SearchAction.input) {
        options.query(1, options.patchingParams, true).then(() => delete this.searching);
      }
    }
  }
  ...
  getPopupClassName(defaultClassName: string | undefined): string | undefined {
    ...
    return classNames(defaultClassName, {
      ...
      [`${this.prefixCls}-lov-patching`]: !!this.patchingConfig,
    });
  }
  ...
  syncValueOnBlur(value) {
    ...
    if (mode !== ViewMode.button) {
      ...
      if (searchAction === SearchAction.blur && value && !hasRecord) {
        const { options } = this;
        options.query(1, options.patchingParams, true).then(() => {
  ...
  async selectSingle() {
    ...
    await options.query(1, options.patchingParams, true);
    if (options.length === 1) {
      this.choose(options.get(0));
    } else {
      this.openModal();
