...
const Label: FunctionComponent<LabelProps> = (props) => {
  const { getTooltipTheme, getTooltipPlacement, getTooltip } = useContext(ConfigContext);
  const { children, className, tooltip = getTooltip('label'), width } = props;
...
const Item: IItem = observer((props: ItemProps): ReactElement<any> | null => {
  ...
  const { children, useColon: fieldUseColon = useColon, hidden, ...rest } = props;
  ...
  if (isValidElement<FormFieldProps>(child)) {
    ...
    const { className, fieldClassName, hidden: childrenHidden, ...otherProps } = childrenProps;
    ...
    const secretFieldOutput = (child.type as any).displayName === 'SecretField' && (fieldProps as any).displayOutput;
    ...
    const isOutput = labelLayout === LabelLayout.horizontal && ((child.type as any).displayName === 'Output' || intlFieldOutput || secretFieldOutput);
    const labelClassName = classNames(`${prefixCls}-label`, `${prefixCls}-label-${labelAlign}`, fieldClassName, {
      [`${prefixCls}-required`]: required && !((child.type as any).displayName === 'Output' || intlFieldOutput || secretFieldOutput),
    ...
    const style = labelLayout === LabelLayout.float && (child.type as any).displayName === 'SelectBox' ? { marginTop: 16 } : undefined;
    ...
    if (labelLayout === LabelLayout.vertical) {
      return (
        <>
          ...
          <div className={wrapperClassName} style={style}>{cloneElement(child, fieldElementProps)}</div>
        </>
      );
    }
