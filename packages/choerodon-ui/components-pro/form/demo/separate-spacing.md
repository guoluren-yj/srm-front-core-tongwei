...
````jsx
import { Form, TextField, Password, NumberField, EmailField, UrlField, DatePicker, Select, SelectBox, TextArea, Button, TelField } from 'choerodon-ui/pro';
...
ReactDOM.render(
  <Form separateSpacing={{width: 30,height:30}}  labelWidth={[60, 70, 50]} columns={3}>
    <TelField colSpan={3} label="手机号" pattern="1[3-9]\d{9}" name="phone" required />
