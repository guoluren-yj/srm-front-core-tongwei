...
````jsx
import { DataSet, Form, TextField, NumberField, Password, EmailField, UrlField, DatePicker, Select, SelectBox, Lov, Button, TelField } from 'choerodon-ui/pro';
...
class App extends React.Component {
  ds = new DataSet({
    autoCreate: true,
    fields: [
      { name: 'phone', type: 'tel', label: '手机号', required: true },
  ...
  render() {
    return (
      <Form record={this.ds.current} style={{ width: '4rem' }}>
        <TelField pattern="1[3-9]\d{9}" name="phone" />
