...
export default class Form extends DataSetComponent<FormProps, FormContextValue> implements IForm {
  ...
  @autobind
  handleLabelMouseEnter(e, title: string) {
    const { target } = e;
    const { getTooltipTheme, getTooltipPlacement, getTooltip } = this.context;
    const tooltip = getTooltip('label');
    if (tooltip === LabelTooltip.always || (tooltip === LabelTooltip.overflow && isOverflow(target))) {
      show(target, {
        title,
  ...
  rasterizedChildren() {
    ...
    for (let index = 0, len = childrenArray.length; index < len;) {
      ...
      const secretFieldOutput = TagName === 'SecretField' && props && props.displayOutput;
      const {
        rowSpan: propsRowSpan = 1,
        colSpan: propsColSpan = 1,
        newLine,
        className,
        fieldClassName,
      ...
      const rowSpan = columns === 1 ? 1 : propsRowSpan;
      const colSpan = columns === 1 ? 1 : propsColSpan;
      ...
      const isOutput = (type as any).__PRO_OUTPUT || intlFieldOutput || secretFieldOutput;
        ...
        >
          {labelLayout === LabelLayout.vertical && !!label && (
            <>
              <label
                className={labelClassName}
                onMouseEnter={(e) => this.handleLabelMouseEnter(e, label)}
                onMouseLeave={this.handleMouseLeave}
              >
                {toJS(label)}
              </label>
  ...
  getMergedProps(props = {}) {
    ...
    if ((!mergedProps.style || !mergedProps.style.width) && mergedProps.useWidthPercent) {
      const width = this.getContextConfig('formWidthPercent');
      if (width && width !== 100) {
        mergedProps.style = {
          ...mergedProps.style,
          width: `${width}%`,
