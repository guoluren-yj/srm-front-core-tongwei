...
const SelectionTips: FunctionComponent<any> = function SelectionTips() {
  ...
  if (tableStore.props.selectionMode !== SelectionMode.none && dataSet.selection === DataSetSelection.multiple) {
    if (tableStore.showSelectionTips) {
      return (
        <div className={`${prefixCls}-selection-tips`}>
    ...
    if (tableStore.showSelectionCached !== false) {
      return cachedButton;
    }
  }
  return null;
