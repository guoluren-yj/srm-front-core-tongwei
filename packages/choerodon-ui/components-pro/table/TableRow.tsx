...
export const VIRTUAL_HEIGHT = '__VIRTUAL_HEIGHT__';
export const UPDATE_CUSTOM_SIZE = '__UPDATE_CUSTOM_SIZE__';
...
const TableRow: FunctionComponent<TableRowProps> = function TableRow(props) {
  ...
  const {
    ...
    expandIconAsCell, expandedRowRenderer, isTree, canTreeLoadData, fullColumnWidth, aggregation,
  } = context;
  const {
    highLightRow,
  ...
  const setRowHeight = useCallback(action((key: Key, height: number, target: HTMLTableRowElement) => {
    if (inView && columnsInView && height && target.offsetParent) {
      const hasSomeCellInView = virtualCell ? Boolean(target.querySelector('td > *,th > *')) : true;
      if (hasSomeCellInView) {
        if (metaData) {
        ...
        if (tableStore.dataSet.current === record) {
  ...
  const useVirtual = !isFixedRowHeight && virtualCell && !hidden;
  const getCell = (columnGroup: ColumnGroup, columnIndex: number, rest: Partial<TableCellProps>, hasCollapseCell?: boolean): [ReactNode, Record | undefined] => {
  ...
  const [columns, { isCurrent }] = (() => {
    const { customizable, isAggregationCellExpanded } = tableStore;
    const { leafs } = columnGroups;
    const columnLength = leafs.length;
    let hasCollapseCell: boolean | undefined = false;
    return leafs.reduce<[ReactNode[], { isCurrent?: boolean }]>((result, columnGroup, columnIndex) => {
      ...
      if (key !== CUSTOMIZED_KEY) {
        ...
        if (aggregation && useVirtual && !hasCollapseCell) {
          hasCollapseCell = isAggregationCellExpanded(record, getColumnKey(columnGroup.column), 0) === false;
        }
        const [cell, actualRecord] = getCell(columnGroup, columnIndex, rest, hasCollapseCell);
        result[0].push(cell);
  ...
  const height = intersectionRef ? virtualHeight !== undefined ? virtualHeight :
    pxToRem(metaData ? metaData.height : !dataSet.getState(UPDATE_CUSTOM_SIZE) && record.getState(VIRTUAL_HEIGHT) || tableStore.virtualRowHeight, true) : lock ?
