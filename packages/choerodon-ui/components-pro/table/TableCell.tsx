...
const TableCell: FunctionComponent<TableCellProps> = function TableCell(props) {
  ...
  const getInnerNode = useCallback((col: ColumnProps, onCellStyle?: CSSProperties, inAggregation?: boolean, headerGroup?: Group, cellKey?: string) => record ? (
    <TableCellInner
      column={col}
      record={record}
      style={onCellStyle}
  ...
  const aggregationTreeRenderer = useCallback(({ colGroup, style, cellKey }) => {
    return getInnerNode(colGroup.column, style, true, colGroup.headerGroup, cellKey);
  }, [getInnerNode]);
  ...
  const groupCell =
    indexInGroup === 0 
    || (
      tableStore.virtual && indexInGroup > -1
      && (
        tableStore.virtualStartIndex !== 0 && group
          /** 
           * 当tableStore.virtualStartIndex不为0，且是分组的列时，
           * tableStore.virtualStartInde所在行可能会隐藏,即inView为false，会导致分组列无法显示内容，
           * 故此处tableStore.virtualStartIndex+2，此时该行一定是显示的，inView一定为true，可保证分组列正常显示。
           * */  
          ? (tableStore.virtualStartIndex + 2) === rowIndex
          : tableStore.virtualStartIndex === rowIndex
      )
    );
  if (group && !groupCell) {
    return null;
  ...
  const getAggregationTreeGroups = ($aggregation): ReactElement<AggregationTreeProps>[] | undefined => {
    if ($aggregation) {
      ...
      if (childrenInAggregation) {
        ...
        const { headerGroup, headerGroups } = columnGroup;
        return groupedAggregationTree({
          ...
          hideLabel: Boolean(headerGroupExtraColumn && headerGroups && !__tableGroup),
          hideValue: Boolean(headerGroupExtraColumn && !headerGroups && !__tableGroup),
          rowIndex,
        });
      }
    }
