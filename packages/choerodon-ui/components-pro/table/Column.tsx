...
export interface ColumnPropsBase extends ElementProps {
  ...
  /**
   * 列脚汇总，自动计算number类型数据总合，优先级高于footer
   */
...
export function minColumnWidth(col: ColumnProps, store: TableStore): number {
  ...
  const minWidth = (store.isTree ? 40 : 0) + (min === undefined ? store.getConfig(aggregation ? 'tableAggregationColumnDefaultMinWidth' : 'tableColumnDefaultMinWidth') : min);
  if (width === undefined) {
    return minWidth;
  }
