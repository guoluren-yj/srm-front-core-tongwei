...
import { ColumnAlign, SelectionMode, TableCommandType, CellType } from './enum';
...
import isOverflow, { isVerticalOverflow } from '../overflow-tip/util';
...
const TableCellInner: FunctionComponent<TableCellInnerProps> = function TableCellInner(props) {
  const { column, record, children, style, disabled, inAggregation, prefixCls, colSpan, headerGroup, rowGroup, cellKey, cellType } = props;
  ...
  const showEditor = useCallback((cell) => {
    if (name) {
      if (!lock && tableStore.overflowX) {
        const tableBodyWrap = tableStore.virtual && cell.offsetParent && cell.offsetParent.parentNode ? cell.offsetParent.parentNode.parentNode : cell.offsetParent;
        if (tableBodyWrap) {
      ...
      tableStore.showEditor(columnKey);
      const editor = tableStore.editors.get(columnKey);
      if (editor) {
        if (editor.cellNode) {
  ...
  const fieldTypeOrigin = field && field.get('type', record);
  const fieldShowTypeOrigin = field && field.get('showType', record);
  const fieldType = !aggregation && rowHeight !== 'auto' && fieldTypeOrigin;
  const cellLineBreak = !hasEditor && fieldTypeOrigin === FieldType.string && aggregation && aggregationCellLineBreak;
  ...
  const prefixStyle = useMemo(() => {
    if (!aggregation || !tableStore.hasAggregationColumn) {
      if (height !== undefined && rows === 0) {
        return {
          height: cellLineBreak ? undefined : pxToRem(height),
          lineHeight: cellLineBreak ? undefined : 1.5,
          ...style,
        };
      }
      if (rowHeight !== 'auto') {
        ...
        return {
          height: cellLineBreak ? undefined : pxToRem(heightPx),
          lineHeight: cellLineBreak ? undefined : rows > 0 ? 'inherit' : pxToRem(lineHeightPx),
          ...style,
        };
      }
    }
    return style;
  }, [fieldType, key, rows, rowHeight, height, style, aggregation, hasEditor, cellLineBreak]);
  ...
  const aggregationCellStyle = useMemo(() => {
    return aggregation && cellLineBreak ? {
      WebkitLineClamp: aggregationCellLineLimit,
      maxHeight: aggregation && cellLineBreak ? `${pxToRem(aggregationCellLineLimit * (1.5 * 12))}` : undefined,
    } : {};
  },  [aggregation, cellLineBreak, aggregationCellLineLimit]);
  const innerStyle = useMemo(() => {
    if (inAggregation) {
      return { ...prefixStyle, ...colSpanStyle, ...aggregationCellStyle };
    }
    return { textAlign, ...prefixStyle, ...colSpanStyle, ...aggregationCellStyle };
  }, [inAggregation, textAlign, prefixStyle, colSpanStyle, aggregation, cellLineBreak, aggregationCellLineLimit]);
  ...
  const processValue = (v) => {
    if (!isNil(v)) {
      const text = isPlainObject(v) ? v : utilProcessValue(v, {
        dateFormat: getDateShowFormatByField(field, undefined, record),
        ...
        isDate: [FieldType.date, FieldType.dateTime, FieldType.month, FieldType.time].includes(fieldType) || [FieldType.date, FieldType.dateTime, FieldType.month, FieldType.time].includes(fieldShowTypeOrigin),
      });
      ...
      }, true, record, tableStore.getConfig, pristine);
    }
    return '';
  };
  const processRenderer = (value, repeat?: number) => {
    ...
    if (field && (field.getLookup(record) || field.get('options', record) || field.get('lovCode', record))) {
      if (isArrayLike(value)) {
        ...
        if (isCascader) {
          processedValue = field.getCascaderText(value, undefined, record);
        } else {
  ...
  const isOverflowElement = (element) => {
    return isOverflow(element) || (aggregation && cellLineBreak && isVerticalOverflow(element)); 
  };
  const showTooltip = useCallback((e) => {
    if (field && field.get('type', record) !== FieldType.attachment && !(multipleValidateMessageLengthRef.current > 0 || (!field.get('validator', record) && field.get('multiple', record) && toMultipleValue(value, field.get('range', record)).length))) {
    ...
    if (element && !multiLine && (tooltip === TextTooltip.always || (tooltip === TextTooltip.overflow && isOverflowElement(element))) || isOverflowMaxTagCount) {
      const { current } = innerRef;
      if (text && current && current.contains(element)) {
        const tooltipConfig: TooltipProps = isObject(tooltipProps) ? tooltipProps : {};
        const getTooltipTitle = () => {
          if (isOverflowMaxTagCount || field && field.get('multiple') && result && isArrayLike(result) ) {
          ...
          const tags = element.querySelectorAll('.c7n-tag');
          if (tags.length) {
            return Array.from(tags).map((tag: HTMLElement) => tag.textContent).join('、');
          }
  ...
  useEffect(() => {
    if (name && inlineEdit && record === currentEditRecord) {
      const currentEditor = tableStore.editors.get(columnKey);
      if (currentEditor) {
  ...
  if (inValid || tooltip || isOverflowMaxTagCount) {
  ...
  const handleResize = (_, height) => {
    if (cellKey) {
      tableStore.setAggregationCellLineHeightCache(`${cellType ? `${cellType}-` : ''}${cellKey}-${record.index}`, height);
    }
  }
  ...
  const output: ReactNode = cellLineBreak ? (
