...
import ObserverNumberField, { FormatNumberFuncOptions } from '../number-field/NumberField';
import Currency from '../currency/Currency';
import DatePicker from '../date-picker/DatePicker';
import DateTimePicker from '../date-time-picker/DateTimePicker';
import { getNumberFormatter } from '../field/utils';
...
import TelField from '../tel-field/TelField';
...
export function getEditorByColumnAndRecord(
  ...
  if (record) {
    ...
    if (isValidElement(cellEditor)) {
      const { displayOutput } = cellEditor.props as any || {};
      if (displayOutput) {
        return undefined;
      }
      return cellEditor;
  ...
  if (isValidElement(editor)) {
    const { displayOutput } = editor.props as any || {};
    if (displayOutput) {
      return undefined;
    }
    return editor;
...
export function findRow(tableStore: TableStore, record: Record): HTMLTableRowElement | null {
  ...
  const selector = `tr[data-index="${record.id}"], tr .c7n-row[data-index="${record.id}"]`;
  ...
  if (name !== undefined && current && node.element) {
    const wrapperSelector =
      !isStickySupport() && overflowX && lock
        ...
        : '';
    const selector = `${wrapperSelector}tr[data-index="${current.id}"] [data-index="${name}"]`;
...
export function findIndexedSibling(element, direction): HTMLTableRowElement | null | undefined {
  if (element) {
...
export function numberFieldFormat(value: number | BigNumber, numberField: Field) {
  let result = value;
  let precision;
  let formatterOptions: FormatNumberFuncOptions = { options: {}};
  let numberGrouping;
  let formatter;
  if (numberField) {
    precision = numberField.get('precision');
    formatterOptions = numberField.get('formatterOptions') || { options: {} };
    numberGrouping = numberField.get('numberGrouping');
    formatter = numberField.get('formatter');
  }
  const numberFieldFormatterOptions: FormatNumberFuncOptions = getConfig('numberFieldFormatterOptions') || { options: {} };
  const lang = formatterOptions.lang || numberFieldFormatterOptions.lang;
  const options: Intl.NumberFormatOptions = {
    maximumFractionDigits: precision || math.dp(result || 0),
  ...
  result = (formatter || getNumberFormatter(getConfig))(result, lang, options);
  return result;
} 