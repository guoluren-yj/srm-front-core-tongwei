...
import { action, computed, get, IReactionDisposer, observable, ObservableMap, reaction, runInAction, set, toJS } from 'mobx';
...
export const GROUP_EXTRA_KEY = '__group-extra-column__'; // TODO:Symbol
...
export const FOOTER_SUMMARY_HEIGHT = 44;

export const DEFAULT_SELECTION_WIDTH = 50;

export const FOOTER_SELECTION_WIDTH = 76;

export type HeaderText = { name: string; label: string };

export type FooterSummary = boolean | TableFooterSummaryType.all;
...
function footerSummaryColumns(column: ColumnProps[], dataSet: DataSet) {
  if (dataSet && dataSet.selection === DataSetSelection.multiple) {
    return column && column.filter(({ name, footerSummary }) => {
      if (footerSummary && name) {
        const field = dataSet.getField(name);
        if (field && [FieldType.number, FieldType.currency].includes(field.type)) {
          return true;
        }
      }
      return false;
...
function footerSummaryName(type: TableFooterSummaryType) {
  switch (type) {
    case TableFooterSummaryType.checked:
      return $l('Table', 'checked_footer_summary');
    case TableFooterSummaryType.page:
      return $l('Table', 'page_footer_summary');
    case TableFooterSummaryType.all:
      return $l('Table', 'all_footer_summary');
    default:
      return '';
  }
}
...
function getHeaderGroupedColumns(
  tableStore: TableStore,
  groups: Group[],
  tableGroups: TableGroup[],
  columns: ColumnProps[],
  dataSet: DataSet,
  groupedColumns: ColumnProps[],
  customizedColumns?: { [key: string]: ColumnProps },
  parentKey?: any,
): {
  ...
  groups.forEach((group) => {
    ...
    const { headerColumns: subColumns } = subGroups.length ? getHeaderGroupedColumns(tableStore, subGroups, tableGroups, columns, dataSet, groupedColumns, customizedColumns, key) : { headerColumns: columns };
  ...
  result.headerColumns.push(...generatedColumns);
  return result;
}
...
export function normalizeGroupColumns(
  ...
  if (hasHeaderGroup) {
    ...
    const { headerGroupExtraColumn } = tableStore.props;
    if (headerGroupExtraColumn) {
      const { width, children, ...rest } = headerGroupExtraColumn;
      const childrenColumns = merge(subColumns && treeMap(subColumns, ((col, index) => {
        if (aggregation || index === 0) {
          const __originalKey = getColumnKey(col);
          const colKey = `${GROUP_EXTRA_KEY}-${__originalKey}`;
          const newCol: ColumnProps = {
            ...omit(col, ['__groups', 'width']),
            key: colKey,
            __originalKey,
            lock: ColumnLock.left,
          };
          const noChildren = !col.children || !col.children.length;
          if (aggregation && hasAggregationColumn ? col.aggregation : noChildren) {
            if (!col.aggregation) {
              newCol.title = undefined;
              newCol.header = undefined;
            }
            newCol.width = width;
          } else {
            newCol.title = undefined;
          ...
          if (noChildren) {
            newCol.renderer = () => null;
          }
          return newCol;
        }
      }), ({ sort = Infinity }, { sort: sort2 = Infinity }) => sort - sort2), toJS(children));
      const { hidden } = tableGroup || {};
      if (hidden) {
        if (childrenColumns) {
          childrenColumns.forEach((col) => {
            groupedColumns.push({
              ...col,
              ...headerGroupExtraColumn,
              __group: group,
              __tableGroup: tableGroup,
              lock: ColumnLock.left,
            });
        ...
        const extraColumn = {
          ...(subColumns && subColumns.length ? rest : headerGroupExtraColumn),
          key: GROUP_EXTRA_KEY,
        ...
        groupedColumns.push(extraColumn);
      }
    }
    return [
      groupedColumns,
...
export default class TableStore {
  ...
  editors: Map<string | number, TableEditor> = new Map();
  ...
  @observable currentEditorName?: string | number;
  ...
  @observable footerSummaryType: TableFooterSummaryType;

  @observable footerSummaryRenderResponse: FooterSummaryRenderResponse | undefined;

  @observable aggregationCellLineHeightCache: ObservableMap<string, number>;
  ...
  get aggregationCellLineBreak(): boolean | undefined {
    return this.props.aggregationCellLineBreak;
  }

  get aggregationCellLineLimit(): number | undefined {
    return this.props.aggregationCellLineLimit || 4;
  }

  get aggregationExpandType(): 'cell' | 'row' | 'column' {
    return this.props.aggregationExpandType || this.customized.aggregationExpandType || 'cell';
  }
  ...
  get aggregationExpandTypeChangeable(): boolean {
    if ('aggregationExpandTypeChangeable' in this.props) {
      return this.props.aggregationExpandTypeChangeable!;
    }
    return this.getConfig('tableAggregationExpandTypeChangeable') !== false;
  }
  ...
  get size(): Size {
    ...
    return size || this.props.size || this.props.customizedCode && this.getConfig('tableCustomizedSize') || Size.default;
  }
  ...
  get showSelectionCached(): boolean | undefined {
    const { tempCustomized, customized: { showSelectionCached: customizedShowSelectionCached } } = this;
    const tempShowSelectionTips = get(tempCustomized, 'showSelectionCached');
    if (tempShowSelectionTips !== undefined) {
      return tempShowSelectionTips;
    }
    if (customizedShowSelectionCached !== undefined) {
      return customizedShowSelectionCached;
    }
  }

  get showCachedTips(): boolean {
    const { showSelectionCached } = this;
    if (showSelectionCached !== undefined) {
      return showSelectionCached;
    }
  ...
  get selectionColumn(): ColumnProps | undefined {
    if (this.hasRowBox) {
      ...
      const selectionColumn: ColumnProps = {
        ...
        align: ColumnAlign.center,
        width: scaleSize(DEFAULT_SELECTION_WIDTH),
        lock,
      };
      if (dataSet && dataSet.selection === DataSetSelection.multiple) {
        selectionColumn.header = this.multipleSelectionRenderer;
        if (this.footerSummary) {
          selectionColumn.footer = this.renderFooterSummaryMenu;
          selectionColumn.width = scaleSize(FOOTER_SELECTION_WIDTH);
        } else {
  ...
  get footerSummary(): {
    ...
    const { aggregation, computedFooterSummary } = this.props;
    if (aggregation) {
      return false;
    }

    const summaryType = computedFooterSummary ? TableFooterSummaryType.all : true;
    const mergeColumns = [...this.leftOriginalColumns, ...this.originalColumns, ...this.rightOriginalColumns];
    if (mergeColumns.length) {
      const summaryColumns = footerSummaryColumns(mergeColumns, this.dataSet);
    ...
    const { columns, customizable, children } = this.props;
    const customizedColumns = customizable ? this.customized.columns : undefined;
    const generatedColumns: [ColumnProps[], ColumnProps[], ColumnProps[], ChildrenInfo] = columns ? mergeDefaultProps(this, columns, aggregation, customizedColumns)
      : normalizeColumns(this, children, aggregation, customizedColumns);
    const [leftOriginalColumns, originalColumns, rightOriginalColumns] = generatedColumns;
    const summaryColumns = footerSummaryColumns([...leftOriginalColumns, ...originalColumns, ...rightOriginalColumns], this.dataSet);
    if (summaryColumns && summaryColumns.length) {
      return {
        columns: summaryColumns,
        type: summaryType,
  ...
  get hasFooter(): boolean {
    return this.hasRowBox && !!this.footerSummary || this.columnGroups.leafs.some(({ column }) => !!column.footer && column.key !== SELECTION_KEY);
  }
  ...
  constructor(node: Table) {
    runInAction(() => {
      ...
      this.aggregationCellLineHeightCache = observable.map();
      this.setProps(node.props);
      this.footerSummaryType = TableFooterSummaryType.checked;
  ...
  getColumnTooltip(column: ColumnProps): TableColumnTooltip | undefined {
    ...
    if (this.aggregation && this.aggregationCellLineBreak) {
      return TextTooltip.overflow;
    }
  ...
  @action
  setAggregationCellLineHeightCache(item: string, value: number) {
    if (!this.aggregationCellLineHeightCache) {
      this.aggregationCellLineHeightCache = observable.map();
    }
    this.aggregationCellLineHeightCache.set(item, value);
  }

  getAggregationCellLineHeightFromCache(key?: string): number | undefined {
    let lineHeight;
    if (this.aggregationCellLineHeightCache && key) {
      const cache = {};
      this.aggregationCellLineHeightCache.forEach((value, item) => {
        if (item.substring(0, key.length) === key) {
          cache[key] = value;
          if (!lineHeight) {
            lineHeight = value;
          } else {
            lineHeight = Math.max(lineHeight, value);
          }
        }
      });
    }
  ...
  showNextEditor(name: string | number, reserve: boolean): boolean {
    const { dataSet } = this;
    const { currentIndex } = dataSet;
    const indexOfCurrentPage = currentIndex - (dataSet.currentPage - 1) * dataSet.pageSize;
    const index = reserve ? indexOfCurrentPage - 1 : indexOfCurrentPage + 1
    const record = dataSet.get(index);
  ...
  @action
  initColumns(cb?: TableCustomizedChange) {
    ...
    if (cb) {
      cb({ ...customized, customizedColumns: [...leftOriginalColumns, ...rightOriginalColumns, ...originalColumns] });
    }
  }

  isAggregationCellExpanded(record: Record, key: Key, index: number): boolean | undefined {
    const expandedKeys: ObservableMap<Key, boolean> | undefined = record.getState(`${AGGREGATION_EXPAND_CELL_KEY}-${index}`) as ObservableMap<Key, boolean>;
    if (expandedKeys) {
  ...
  @action
  setAggregationCellExpanded(record: Record, key: Key, expanded: boolean, index: number) {
    const expandedKeys: ObservableMap<Key, boolean> = record.getState(`${AGGREGATION_EXPAND_CELL_KEY}-${index}`) || observable.map();
    expandedKeys.set(key, expanded);
    record.setState(`${AGGREGATION_EXPAND_CELL_KEY}-${index}`, expandedKeys);
  }
  ...
  @autobind
  async handleQueryFooterSummary() {
    const { computedFooterSummary } = this.props;
    if (computedFooterSummary) {
      const { footerSummary, dataSet } = this;
      const columns = footerSummary && footerSummary.columns;
      if (columns) {
        const { queryDataSet } = dataSet;
        const queryData = queryDataSet ? queryDataSet.toJSONData() : undefined;
        const result = await computedFooterSummary({ dataSet, columns, queryData });
        runInAction(() => {
          this.footerSummaryRenderResponse = result;
        });
      }
    }
  }
  ...
  @autobind
  @action
  handleFooterSummaryMenuClick({ key }) {
    this.footerSummaryType = key;
    if (key === TableFooterSummaryType.all && !this.footerSummaryRenderResponse) {
      this.handleQueryFooterSummary();
    }
  }
  ...
  @autobind
  renderFooterSummaryMenu() {
    const { prefixCls, footerSummary, footerSummaryType } = this;
    return (
      <Dropdown overlay={
        <Menu
          prefixCls={`${prefixCls}-dropdown-menu`}
          onClick={this.handleFooterSummaryMenuClick}
          defaultSelectedKeys={[TableFooterSummaryType.checked]}
        >
          <Menu.Item key={TableFooterSummaryType.checked}>
            {$l('Table', 'checked_footer_summary')}
          </Menu.Item>
          <Menu.Item key={TableFooterSummaryType.page}>
            {$l('Table', 'page_footer_summary')}
          </Menu.Item>
          {
            footerSummary && footerSummary.type === TableFooterSummaryType.all ? (
              <Menu.Item key={TableFooterSummaryType.all}>
                {$l('Table', 'all_footer_summary')}
              </Menu.Item>
            ) : null
          }
        </Menu>
      }>
        <Button size={Size.small} funcType={FuncType.link} color={ButtonColor.dark}>
          <>{footerSummaryName(footerSummaryType)}</>
  ...
  private multipleSelectionRenderer() {
    const buttons = [
      <Tooltip 
        key='selectAllTip'
        title={this.props.showAllPageSelectionButton ? undefined : $l('Table', 'current_page_selection')}
      >
        <ObserverCheckBox
          key="selectAll"
