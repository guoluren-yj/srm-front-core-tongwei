...
| aggregationExpandType | 聚合单元格展开模式 | 'cell' \| 'row' \| 'column' |  |
| aggregationExpandTypeChangeable | 可设置聚合单元格展开模式 | boolean | [globalConfig.tableAggregationExpandTypeChangeable](/components/configure#API) |
...
| onCustomizedChange | 个性化配置变更钩子， 在个性化配置变更时触发 | (customized) => void | |
...
| computedFooterSummary | 列脚汇总钩子, 需要跨页全部汇总时使用  | { dataSet: DataSet; columns: ColumnProps[], queryData?: { \[key: string\]: any } } => { \[key: string\]: number \| BigNumber } \| Promise<{ \[key: string\]: number \| BigNumber }> |  |  |
| headerGroupExtraColumn | 二维分组额外的列, 在聚合模式下会显示label组  | ColumnProps |  |  |
...
| footerSummary | 列脚汇总，自动计算number类型数据总合，优先级高于footer | boolean |  |  |
