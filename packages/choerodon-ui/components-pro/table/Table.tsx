...
export interface TableCustomizedChangeProps extends TableCustomized {
  customizedColumns: ColumnProps[];
}

export type TableCustomizedChange = (customized: TableCustomizedChangeProps) => void;
...
export type FooterSummaryRenderProps = { dataSet: DataSet; columns: ColumnProps[], queryData?: { [key: string]: any } };
export type FooterSummaryRenderResponse = { [key: string]: number | BigNumber };
...
export interface TableProps extends DataSetComponentProps {
   ...
   * 可设置聚合单元格展开模式
   */
  aggregationExpandTypeChangeable?: boolean;
  /**
   * 显示原始值
   */
  pristine?: boolean;
   ...
   * 聚合视图单元格内容换行展示 
   */
  aggregationCellLineBreak?: boolean;
  /**
   * 聚合视图单元格内容最大显示行数
   */
  aggregationCellLineLimit?: number;
  /**
   * 聚合视图切换钩子
   */
  ...
  groups?: TableGroup[];
  /**
   * 二维分组额外的列, 在聚合模式下会显示label组
   */
  headerGroupExtraColumn?: ColumnProps;
  /**
   * 横向滚动事件
   */
   ...
   * 个性化配置变更钩子
   */
  onCustomizedChange?: TableCustomizedChange;
  /**
   * 列脚汇总渲染器
   */
  computedFooterSummary?: (props: FooterSummaryRenderProps) => FooterSummaryRenderResponse | Promise<FooterSummaryRenderResponse>;
}
...
export default class Table extends DataSetComponent<TableProps> {
        ...
        if (errors.length) {
          ...
          if (parents.length) {
            parents.forEach(parent => {
              if (!tableStore.isRowExpanded(parent)) {
                tableStore.setRowExpanded(parent, true);
              }
            });
          }
          raf(() => {
  ...
  getPagination(position: TablePaginationPosition): ReactElement<PaginationProps> | undefined {
    ...
    } else if (position === TablePaginationPosition.bottom) {
      return (
        <div className={`${prefixCls}-selection-tips-wrapper`}>
          <SelectionTips />
        </div>
      );
    }
