...
function generateRowGroup(props: GenerateRowGroupProps): ReactElement {
  ...
  const { footerSummary } = tableStore;
  const node = (
    <TableRowGroup key={key} columnGroups={columnGroups} lock={lock} level={level} footerSummary={footerSummary && footerSummary.type}>
      {children}
    </TableRowGroup>
  );
  if (statistics) {
      ...
      generateRowGroup({
        ...
        children: (
          <>
            ...
            <span>{$l('Table', showCachedTips ? 'current_page' : 'current_page_records')}</span>
          </>
        ),
      }),
    );
...
const Rows: FunctionComponent<RowsProps> = function Rows(props) {
  ...
  const cachedRows: ReactNode[] = useComputed(() => (
    generateCachedRows({ tableStore, columnGroups, lock, isTree, rowDraggable, virtual: false, isFixedRowHeight, virtualCell, expandIconColumnIndex }, onClearCache)
