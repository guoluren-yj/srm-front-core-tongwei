...
import { UPDATE_CUSTOM_SIZE } from '../TableRow';
...
const CustomizationSettings: FunctionComponent<CustomizationSettingsProps> = function CustomizationSettings(props) {
  ...
  const { prefixCls, tableStore, dataSet: tableDataSet } = context;
    ...
    data: [
      {
        ...
        showSelectionCached: tableStore.showSelectionTips || tableStore.showCachedTips,
      },
    ],
    events: {
      update({ record, name, value }) {
        ...
        if (tempCustomized) {
          ...
          if (name === 'heightType') {
            ...
            tableDataSet.setState(UPDATE_CUSTOM_SIZE, true);
          }
        }
  ...
  useEffect(() => {
    if (handleOk) {
      handleOk(action(() => {
        const { tempCustomized, aggregation, props: { onAggregationChange, onCustomizedChange } } = tableStore;
        tableStore.tempCustomized = { columns: {} };
        tableStore.saveCustomized(tempCustomized);
        tableStore.initColumns(onCustomizedChange && !isEqual(customized, tempCustomized) ? onCustomizedChange : undefined);
        tableStore.node.handleHeightTypeChange();
        const { aggregation: customAggregation, showSelectionCached } = tempCustomized;
        ...
        if (showSelectionCached !== undefined) {
          tableStore.setProps({ ...tableStore.props, showSelectionCached });
        }
      }));
    }
    if (handleCancel) {
  ...
  if (tableRecord.get('aggregation') && tableStore.aggregationExpandTypeChangeable) {
    tableSettings.push(
    ...
    <TableContext.Provider value={context}>
      ...
      >
        ...
        <CollapsePanel
          header={
            <span className={`${prefixCls}-customization-panel-title`}>
              {$l('Table', 'selection_settings')}
            </span>
          }
          key="selection"
        >
          <div className={`${prefixCls}-customization-selection-item`}>
            {$l('Table', 'show_selection_cached')}
            <Switch
              record={tableRecord}
              name="showSelectionCached"
            />
          </div>
        </CollapsePanel>
      </Collapse>
