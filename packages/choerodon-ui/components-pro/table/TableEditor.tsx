...
export default class TableEditor extends Component<TableEditorProps> {
  ...
  componentDidMount() {
    ...
    if (name) {
      ...
      editors.set(getColumnKey(column), this);
    }
  }
  ...
  componentWillUnmount() {
    ...
    if (name) {
      ...
      editors.delete(getColumnKey(column));
  ...
  saveWrap(node) {
    ...
    if (node && (!tableStore.inlineEdit || this.cellNode)) {
  ...
  showNextEditor(reserve: boolean) {
    ...
    if (name) {
      this.blur();
      const { tableStore } = this.context;
      if (tableStore.showNextEditor(getColumnKey(column), reserve)) {
        this.alignEditor();
        this.focus();
      }
    }
