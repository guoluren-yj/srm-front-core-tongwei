...
export default class ModalContainer extends Component<ModalContainerProps> implements IModalContainer {
  ...
  open(props: ModalProps) {
    ...
    const before = beforeOpen();
    if (isPromise(before)) {
      before.then(() => this.updateModals(modals))
    } else {
      this.updateModals(modals);
    }
  }
  ...
  clear(closeByLocationChange?: boolean, destroyAll?: boolean) {
    const { modals } = this.state;
    this.updateModals(modals.map(modal => {
      const hidden = destroyAll ? !modal.ignoreDestroyAll : true;
      return closeByLocationChange && !modal.closeOnLocationChange ? modal : ({
        ...modal,
        destroyOnClose: hidden,
        hidden,
      })
    }));
  }
