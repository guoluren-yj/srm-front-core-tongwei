...
const PopupContent: React.FC<{
  ...
  const { content, prefixCls, theme, translate: { x, y }, showImage } = props;
  ...
  const showImageCls = showImage ? `${contentCls}-show-img` : '';
  const contentPreCls = isString(content) && (content.includes('\n') || content.includes('\\n')) ? `${contentCls}-pre` : undefined;
  const showContent = content && isString(content) ? content.replace(/\\n/g, '\n') : content;
  return (
    <div>
      <div className={`${arrowCls} ${arrowCls}-${theme}`} style={arrowStyle} />
      <div className={`${contentCls} ${contentCls}-${theme} ${showImageCls} ${contentPreCls}`}>
        {showContent}
      </div>
    </div>
  );
...
export default class Tooltip extends Component<TooltipProps, any> {
  ...
  renderPopupContent(...props) {
    ...
    const { theme = getTooltipTheme(), showImage } = this.props;
