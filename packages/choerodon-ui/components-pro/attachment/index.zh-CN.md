...
| afterUpload | 上传文件之后的钩子，参数为上传成功的文件  | (attachment: AttachmentFile[]) => void | 无 |
| afterDelete | 删除文件之后的钩子，参数为删除的文件 | (attachment: AttachmentFile) => void | 无 |
...
| fileReadOnly | 单个文件的只读控制 | (file: AttachmentFile) => boolean | 无 |
| crossTenant | 跨租户标识 | boolean | 无 |
| attachmentLimit | 按文件限制预览、删除、下载能力 | (attachment: AttachmentFile, listType?: AttachmentListType, isAttachmentsInControl?: boolean) => ({ preview?: boolean; download?: boolean; remove?: boolean }) | |
| queryArgs | 查询附件时携带的额外参数 | { query?: any; body?: any; } | 无 |
