...
export interface AttachmentListProps {
  ...
  onFetchAttachments: (props: { bucketName?: string; bucketDirectory?: string; storageCode?: string; attachmentUUID: string; isPublic?: boolean; isAttachmentsInControl?: boolean }) => void;
  ...
  useEffect(() => {
    if (attachmentUUID) {
      ...
      if (!current || current.attachmentUUID !== attachmentUUID || current.record !== record) {
        if (attachments && !hasAttachmentsProps) {
          onAttachmentsChange(undefined);
        }
        oldValues.current = { attachmentUUID, record };
        if (tempAttachmentUUID !== attachmentUUID) {
          onFetchAttachments({ bucketName, bucketDirectory, storageCode, attachmentUUID, isPublic, isAttachmentsInControl: hasAttachmentsProps });
        }
      }
    }
  }, [onFetchAttachments, bucketName, bucketDirectory, storageCode, attachmentUUID, tempAttachmentUUID, isPublic, record]);
        ...
        >
          {
            (provided: DraggableProvided) => (
              <Item
                ...
                isPublic={isPublic}
                isAttachmentsInControl={hasAttachmentsProps}
