...
export default class SecretFieldView extends Component<SecretFieldViewProps> {
  ...
  @autobind
  handleEdit() {
    const { name, token = '', onChange, modal, pattern, required, record } = this.props;
    const { context, formDs } = this;
    const editValue = formDs && formDs.current && formDs.current.get(name);
    // 空值不进行正则校验
    if (editValue && pattern) {
  ...
  render() {
    ...
    return (
      <div className={`${prefixCls}-modal`}>
        {
          flag !== 'slider' && (
            <Form className={`${prefixCls}-modal-form`} dataSet={this.formDs} columns={4} labelWidth="auto" labelLayout={LabelLayout.float}>
              {
                flag === 'verify' && (
                  <>
                    ...
                    <td className={`${prefixCls}-modal-verify`} colSpan={4}>
                      <ObserverTextField name="verifyCode" valueChangeAction={ValueChangeAction.input} style={{flex: 1}} />
              ...
              }
              {
                flag === 'edit' && (
                  <>
                    ...
                    <td colSpan={4}>{!validate && <p className={`${prefixCls}-modal-validate`}>{$l('SecretField', 'type_mismatch', { label })}</p>}</td>
