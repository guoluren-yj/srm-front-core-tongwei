...
import { AutoSizeType, ResizeType } from '../text-area/enum';
...
export default class IntlField extends TextArea<IntlFieldProps> {
  ...
  renderOutput(): ReactNode {
      ...
      return (
        <span {...this.getWrapperProps()}>
          <span className={className}>
            <Output {...this.getOtherProps()}>{text}</Output>
            {suffix}
          </span>
        </span>
      );
    ...
    return (
      <span className={className}>
        <Output {...this.getMergedProps()}>{text}</Output>
        {suffix}
      </span>
    );
  }
