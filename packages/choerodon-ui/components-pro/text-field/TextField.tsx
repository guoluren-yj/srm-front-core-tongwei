...
export interface TextFieldProps<V = any> extends FormFieldProps<V> {
   ...
   */
  border?: boolean;
  onDragEnd?: InputHTMLAttributes<HTMLInputElement>['onDragEnd'];
}
...
export class TextField<T extends TextFieldProps> extends FormField<T> {
  ...
  get addonBefore(): ReactNode | undefined {
    const { addonBefore = this.props.addonBefore } = this.observableProps;
    return addonBefore;
  }

  get addonAfter(): ReactNode | undefined {
    const { addonAfter = this.props.addonAfter } = this.observableProps;
    return addonAfter;
  }
  ...
  getOtherProps() {
    ...
    otherProps.onDragEnd = this.handleDragEnd;
    const keepOriginMaxLength = this.getProp('keepOriginMaxLength');
    if (keepOriginMaxLength === false) {
      return omit(otherProps, ['regionOptions', 'regionField', 'onRegionChange', 'maxLength']);
    }
    return omit(otherProps, ['regionOptions', 'regionField', 'onRegionChange']);
  }

  forceBlur(e) {
    const { addonBefore, addonAfter } = this;
  ...
  @autobind
  handleTextGroupBlur(e) {
    const { onBlur } = this.props;
    setTimeout(() => {
      if (onBlur && !this.isFocused) {
        onBlur(e);
      }
    }, 0);
  }
  ...
  showTooltip(e): boolean {
    ...
    const { tooltip = getTooltip('text-field') } = this.props;
    const { element } = this;
    if (element && !this.isFocus && !this.multiple && (tooltip === TextTooltip.always || (tooltip === TextTooltip.overflow && isOverflow(element)))) {
      const title = this.getRenderedValue() || this.getProp('placeholder') || (this.hasFloatLabel && !this.range && this.getLabel());
      if (title) {
        show(element, {
          title,
          placement: getTooltipPlacement('text-field') || 'bottom',
          theme: getTooltipTheme('text-field'),
        });
        return true;
      }
    }
  ...
  renderGroup(): ReactNode {
    const {
      ...
      addonBefore,
      addonAfter,
      props: { addonBeforeStyle, addonAfterStyle, groupClassName, _inTable, tabIndex },
    } = this;
    ...
    const classString = classNames(`${prefixCls}-group`, groupClassName, {
      ...
      [`${prefixCls}-group-invalid`]: !this.valid,
    });

    return (
      <TextFieldGroup key="wrapper" prefixCls={prefixCls} onBlur={_inTable && (addonBefore || addonAfter) ? this.handleTextGroupBlur : undefined}>
  ...
  renderRangeEditor(props) {
    ...
    let selectionClass = '';
    let startSelectionClass = '';
    let endSelectionClass = '';
    if (rangeTarget === 1) {
      editorStyle.right = 0;
    } else {
      editorStyle.left = 0;
    ...
    if (!editable) {
      selectionClass = `${prefixCls}-selection-transparent`;
    }
    // 筛选条默认宽度处理
    ...
    if (startRenderedValue && (!editable || !isFocused)) {
      startSelectionClass = `${prefixCls}-selection-transparent`;
    }
    if (endRenderedValue && (!editable || !isFocused)) {
      endSelectionClass = `${prefixCls}-selection-transparent`;
    }
    return (
      <span key="text" className={`${prefixCls}-range-text`}>
        ...
        {endRenderedValue}
        {
          !this.disabled && (
            <input
              {...props}
              className={this.getInputClassString(`${prefixCls}-range-input ${selectionClass}`)}
              key="text"
        ...
        <input
          tabIndex={-1}
          className={`${prefixCls}-range-start ${startSelectionClass}`}
          onChange={noop}
        ...
        <input
          tabIndex={-1}
          className={`${prefixCls}-range-end ${endSelectionClass}`}
          onChange={noop}
  ...
  getEditor(wrap: (node: ReactNode) => ReactNode, renderedValue?: ReactNode): ReactNode {
    ...
    if (this.hasFloatLabel && !this.getSuffix() && !this.getInnerSpanButton()) {
      otherProps.style = {
        ...otherProps.style,
        paddingRight: '0.1rem',
      };
    ...
    otherProps.className = this.getInputClassString(`${otherProps.className} ${selectionClass}`);
  ...
  @autobind
  handleDragEnd(e: DragEvent<HTMLInputElement>) {
    ...
    // 修复日语全角输入法下文字框中拖拽后输入的问题
    if (isFunction(currentTarget.setSelectionRange)) {
      currentTarget.setSelectionRange(selectionEnd || 0, selectionEnd || 0);
      raf(() => {
        window.setTimeout(() => {
          currentTarget.setSelectionRange(selectionStart || 0, selectionEnd || 0);
        }, 0);
      });
    }
  }
  ...
  handleChange(e) {
    ...
    const restricted = this.lock ? value : this.restrictInput(value);
    if (restricted !== value) {
    ...
    if (!this.lock && (!this.isFocus || (valueChangeAction === ValueChangeAction.input && this.isValidInput(restricted)))) {
