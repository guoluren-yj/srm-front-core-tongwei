...
import moment, { isDate, isMoment, unitOfTime } from 'moment';
...
import Field, { DynamicProps, FieldProps, Fields } from './Field';
...
function checkLanguageRequired(
  language: string,
  required?: boolean | { lang?: string[], options?: Intl.NumberFormatOptions },
) {
  return isObject(required) ? required.lang && required.lang.includes(language) : required;
}
...
export function processIntlField(
  ...
  if (fieldProps && fieldProps.type === FieldType.intl) {
    ...
    const hasDynamicType = 'type' in (fieldProps.dynamicProps || fieldProps.computedProps || {});
    languages.forEach(language => {
      ...
      if (checkLanguageRequired(language, required)) {
        dynamicProps.required = ({ record, dataSet }) => {
          const value = record.get(name);
          // 当主字段有值时
          if (value || record.intlEditing) {
            const field = hasDynamicType ? dataSet.getField(name) : undefined;
            if (!field || field.get('type', record) === FieldType.intl) {
              const tlsData = record.get(tlsKey);
              if (
                // 新建态
                record.status === RecordStatus.add
                // 编辑态
                // 初始没有值时
                || !record.getPristineValue(name)
              ) {
                // 请求过tls或非主语言
                return Boolean(tlsData || dataSet.lang !== language);
              }
              // 请求过tls时
              if (tlsData && name in tlsData) {
                return true;
              }
            }
          }
...
export function getLimit(limit: any, record: Record, minOrMax: any, type: FieldType) {
  ...
  if (limit === undefined) {
    ...
    if (configLimit && typeof configLimit === 'function') {
      limit = configLimit(type);
    }
  }
  if (!isNil(limit)) {
    let unit: unitOfTime.StartOf | undefined | null;
    switch (type) {
      case 'year':
        unit = 'y';
        break;
    ...
    if (unit !== undefined) {
      ...
      if (unit) {
        if (minOrMax === 'min') {
          return limit.startOf(unit);
        }
        return limit.endOf(unit);
      }
    }
  }
  return limit;
