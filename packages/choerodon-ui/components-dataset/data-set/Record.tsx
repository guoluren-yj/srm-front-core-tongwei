...
function initRecordField(record: Record, name: string, fieldProps?: FieldProps, intlRequired?: boolean | { lang?: string[], options?: Intl.NumberFormatOptions }): [Field, Map<string, Field> | undefined] {
...
export default class Record {
  ...
  @observable intlEditing?: boolean;
  ...
  addField(name: string, fieldProps?: FieldProps): Field {
    ...
    const [field, intlFields] = initRecordField(this, name, fieldProps, this.dataSet.getConfig('intlFieldRequired'));
  ...
  set(item: string | object, value?: any): Record {
    if (isString(item)) {
      ...
      const newValue = processValue(isArrayLike(value) ? value.slice() : value, field, this);
      ...
      if (value && field.get('type', this) === 'intl') {
        const intlFieldRequired = dataSet.getConfig('intlFieldRequired');
        if (intlFieldRequired) {
          ...
          if (languages.length) {
            ...
            languages.forEach(language => {
              if (language !== lang) {
                const langField = dataSet.getField(`${tlsKey}.${oldName}.${language}`);
                if (langField) {
                  langField.checkValidity(this);
                }
              }
            });
          }
