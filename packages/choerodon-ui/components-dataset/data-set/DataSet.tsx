...
import { Config, ConfigKeys, DefaultConfig, getConfig, PatchingConfig } from '../configure';
...
export const __DATASET_PATCHING_CONFIG__ = '__DATASET_PATCHING_CONFIG__';

export const __DATASET_GET_REAL_COUN_STATUS__ = '__DATASET_GET_REAL_COUN_STATUS__';

export const DATASET_GET_REAL_COUNT_STATUS = {
  'READY': 'READY',
  'DOING': 'DOING',
  'DONE': 'DONE',
};
...
const CASCADE_CACHE = '__CASCADE_CACHE__';

const CASCADE_STATUS = '__CASCADE_STATUS__';
...
export function initDataSetField(dataSet: DataSet, name: string, fieldProps?: FieldProps, intlRequired?: boolean | { lang?: string[], options?: Intl.NumberFormatOptions }): [Field, Map<string, Field> | undefined] {
...
function deleteCachedRecord(dataSet: DataSet, record) {
  const cachedIndex = dataSet.cachedRecords.indexOf(record);
  if (cachedIndex !== -1) {
    dataSet.cachedRecords.splice(cachedIndex, 1);
  }
}

function getToken() {
  return Date.now().toString(36).slice(0, 8) + Math.round(Math.random() * 36 * 36).toString(36).padStart(2, '0');
}
...
export default class DataSet extends EventManager {
  ...
  private queryPending: PromiseQueue = new PromiseQueue();
  ...
  @observable lovCaches?: ObservableMap<string, LookupCache>;
  ...
  get unSelected(): Record[] {
    return this.currentUnSelected.concat(this.cachedUnSelected.filter(record => !record.isSelected));
  }
  ...
  @computed
  get cascadeStatus(): DataSetStatus {
    const status = this.getState(CASCADE_STATUS);
    return status === undefined ? DataSetStatus.ready : status;
  }

  set cascadeStatus(status: DataSetStatus) {
    this.setState(CASCADE_STATUS, status);
  }

  @observable cachedRecords: Record[];

  get lovPatching(): PatchingConfig[] | undefined {
    const lovPatching = this.getConfig('lovPatching');
    if (lovPatching && isFunction(lovPatching)) {
      return lovPatching();
    }
  }

  get patchingParams() {
    const dsPatchingConfig = this.getState(__DATASET_PATCHING_CONFIG__);
    if (dsPatchingConfig) {
      return { [dsPatchingConfig.name]: dsPatchingConfig.value };
    }
  }
  ...
  get limitCount(): boolean {
    return this.getState(__DATASET_GET_REAL_COUN_STATUS__) === DATASET_GET_REAL_COUNT_STATUS.READY && !!this.totalCount && this.totalCount > 999;
  }
  ...
  private syncChildrenRemote = debounce((remoteKeys: string[], current: Record) => {
    const { children } = this;
    remoteKeys.forEach(childName => this.syncChild(children[childName], current, childName, undefined, this.getState(CASCADE_CACHE)));
  }, 300);
  ...
  constructor(props?: DataSetProps, context?: DataSetContext) {
    ...
    runInAction(() => {
      ...
      this.initPatchingParams();
      this.initQueryDataSet(queryDataSet, queryFields, queryData);
      if (data) {
        const { length } = data;
        if (length) {
  ...
  doQuery(page, params?: object, cache?: boolean, paging?: boolean): Promise<any> {
    return this.read(page, params, undefined, paging, async (data) => {
  ...
  async locate(index: number): Promise<Record | undefined> {
    ...
    if (paging === true || paging === 'server') {
      ...
      if (index >= 0 && (this.limitCount || index < totalCount)) {
  ...
  async delete(
    ...
    if (records) {
      ...
      if (
        ...
        if (res && res.success) {
          // 移除cachedDestroyed中数据
          runInAction(() => {
            const data = isArrayLike(records) ? records.slice() : [records];
            data.forEach(record => {
              if (record && record.isCached && this.cachedDestroyed.indexOf(record) !== -1) {
                deleteCachedRecord(this, record)
              }
            });
          });
        }
  ...
  initFields(fieldProps: FieldProps[]): [string, Field][] {
    ...
    fieldProps.forEach((props) => {
      const { name } = props;
      if (name) {
        const [field, intlFields] = initDataSetField(this, name, props, this.getConfig('intlFieldRequired'));
  ...
  addField(name: string, fieldProps?: FieldProps): Field {
    ...
    const [field, intlFields] = initDataSetField(this, name, fieldProps, this.getConfig('intlFieldRequired'));
  ...
  commitData(allData: any[], total?: number, onlyDelete?: boolean): DataSet {
    ...
    if (this.dataToJSON === DataToJSON.normal) {
      ...
      } else if (!onlyDelete && autoQueryAfterSubmit) {
...
Then the query method will be auto invoke.`,
      ...
      if (onlyDelete) {
        this.destroyed.forEach(r => r.commit(undefined, this));
        // 仅删除时查询当前页并保留缓存
        this.query(this.currentPage, undefined, true);
      } else {
        this.query();
      }
    }
  ...
  bind(ds: DataSet, name: string) {
    ...
    const { current } = ds;
    if (current) {
      ds.syncChild(this, current, name, undefined, ds.getState(CASCADE_CACHE));
    } else if (this.length) {
      this.loadData([]);
    }
  }
  ...
  loadData(allData: (object | Record)[] = [], total?: number, cache?: boolean): DataSet {
    ...
    if (cacheRecords && this.hasChildren) {
      this.setState(CASCADE_CACHE, cache);
    }
  ...
  private initQueryDataSet(queryDataSet?: DataSet, queryFields?: FieldProps[], queryData?: object) {
    if (queryFields) {
      const queryDataSetProps: DataSetProps = {
        fields: queryFields,
      };
      if (queryData) {
        queryDataSetProps.data = [queryData];
      }
      queryDataSet = new DataSet(queryDataSetProps, this.context);
    }
    if (queryDataSet) {
  ...
  /**
   * 支持通过queryParameter控制patching初始值
   */
  private initPatchingParams() {
    const { lovPatching, props: { queryParameter } } = this;
    if (lovPatching && !isEmpty(queryParameter)) {
      const config = lovPatching.find(code => queryParameter[code.name]);
      if (config) {
        const { name } = config;
        const currentConfig = config.options.find(item => item.value === queryParameter[name]) || config.options[0];
        this.setState(__DATASET_PATCHING_CONFIG__, { ...currentConfig, name });
      }
    }
  }
  ...
  private async read(page = 1, params?: object, more?: boolean, paging?: boolean, callback?: Function): Promise<any> {
    if (this.checkReadable(this.parent)) {
      try {
        if (!more) {
          this.changeStatus(DataSetStatus.loading);
        }
        if (this.getState(__DATASET_GET_REAL_COUN_STATUS__) === DATASET_GET_REAL_COUNT_STATUS.DOING) {
          runInAction(() => {
            this.counting = this.count(page, params);
          });
          const countResult: object = await this.counting;
          const { totalKey } = this;
          ...
          runInAction(() => {
            ...
            this.setState(__DATASET_GET_REAL_COUN_STATUS__, DATASET_GET_REAL_COUNT_STATUS.DONE);
          });
          return countResult;
        }
        ...
        if (newConfig.url) {
          this.changeCascadeStatus(DataSetStatus.loading);
          ...
          if (queryEventResult) {
            ...
            const promise = Promise.all([this.axios(fixAxiosConfig(newConfig)), token]);
            if (!more) {
              this.queryPending.add(promise);
            }
            if (page >= 0) {
              runInAction(() => {
            ...
            const [result, currentToken] = await (more ? promise : this.queryPending.ready(promise));
            ...
            if (currentToken === token) {
                ...
                if (needCount) {
                  this.counting = this.count(page, params)
                    .then(action((countResult: object) => {
                      ...
                      this.setState(__DATASET_GET_REAL_COUN_STATUS__, DATASET_GET_REAL_COUNT_STATUS.READY);
                      return total;
                    })).finally(action(() => {
                      this.counting = undefined;
                    }));
              ...
              const newResult = this.handleLoadSuccess(result);
              if (callback) {
  ...
  @action
  private changeCascadeStatus(status: DataSetStatus) {
    const { parent } = this;
    if (this.hasChildren) {
      if (!status) {
        this.cascadeStatus = status;
      }
    }
    if (parent) {
      if (status === 'ready') {
        const childrenAllReady = Object.values(parent.children).every(ds => ds.status === DataSetStatus.ready);
        if (childrenAllReady) {
          parent.cascadeStatus = status;
        }
      } else if (parent.cascadeStatus !== status) {
        parent.cascadeStatus = status;
      }
    }
  }
  ...
  private handleCascade({
    ...
    if (dataSet.status === DataSetStatus.ready) {
      dataSet.changeCascadeStatus(DataSetStatus.ready);
    }
  }
  ...
  private syncChildren(current?: Record, previous?: Record) {
    ...
    const cache = this.getState(CASCADE_CACHE);
    keys.forEach(childName => {
      ...
      if (current) {
        ...
        if (snapshot instanceof DataSetSnapshot) {
          ds.restore(snapshot);
        } else if (!this.syncChild(ds, current, childName, true, cache)) {
          ds.loadData([], undefined, cache);
  ...
  private syncChild(
    ...
    if (!onlyClient) {
      ...
      ds.doQuery(1, undefined, cache).then(resp => {
        const { current } = this;
        ...
        const dataSetSnapshot = getIf<Record, { [key: string]: DataSetSnapshot }>(currentRecord, 'dataSetSnapshot', {});
        dataSetSnapshot[childName] = ds.loadDataFromResponse(resp, cache).snapshot();
      });
    }
    return false;
  }
  ...
  private generateQueryString(page: number, pageSizeInner?: number, onlyCount?: boolean, paging?: boolean) {
    ...
    if (typeof generatePageQuery === 'function') {
      return generatePageQuery({
        ...
        countLimit: pageQuery.countLimit,
      });
    }
