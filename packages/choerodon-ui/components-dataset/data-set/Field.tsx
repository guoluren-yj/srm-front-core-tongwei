...
import { action, computed, get, IComputedValue, isObservableObject, observable, ObservableMap, remove, runInAction, set, toJS, isArrayLike } from 'mobx';
...
import dateCodeStore from '../stores/DateCodeStore';
...
function hasBatchSideEffectKeys(props: FieldProps & { [key: string]: any }, type: 'lookup' | 'lov'): boolean {
  return !!BATCH_SIDE_EFFECT_KEYS.find(effect => {
    if (props[effect]) {
      if ((effect === 'dynamicProps' || effect === 'computedProps') && isObject(props[effect])) {
        const keys = Object.keys({ ...props[effect] });
        if (type === 'lov') {
          return keys.some(key => ['lovCode'].includes(key));
        }
        if (type === 'lookup') {
          return keys.some(key => ['lookupCode', 'lovPara'].includes(key));
        }
      }
      return true;
    }
...
export default class Field {
  ...
  @observable lovToken?: string | undefined;
  ...
  getLookupText(value?: any, showValueIfNotFound?: boolean, record: Record | undefined = this.record): string | undefined {
    ...
    if (lookup) {
      ...
      const lookupShowTextIfNotFound = this.dataSet.getConfig('lookupShowTextIfNotFound');
      if (lookupShowTextIfNotFound) {
        return lookupShowTextIfNotFound({
          name: this.name,
          value,
          record,
          showValueIfNotFound,
  ...
  getCascaderText(value?: any, showValueIfNotFound?: boolean, record: Record | undefined = this.record): string | undefined {
    if (isArrayLike(value)) {
      const options = this.get('options', record);
      const textField = this.get('textField', record);
      if (options) {
        const valueField = this.get('valueField', record);
        const idField = this.get('idField', record);
        const parentField = this.get('parentField', record);
        if (idField && parentField) {
          return value.map(v => this.getText(v, showValueIfNotFound, record)).join('/');
        }
        const childrenField = this.get('childrenField', record) || 'children';
        let text = '';
        value.reduce((option, v, index) => {
          const optionValue = isObject(v) ? v[valueField] : v;
          if (index === 0) {
            const found = options.find(item => isSameLike(item.get(valueField), optionValue));
            if (found) {
              text = found.get(textField) || '';
              return found;
            }
          } else if (option) {
            const children = option[childrenField] || (option.get ? option.get(childrenField) : option[childrenField]);
            if (children) {
              const found = children.find(item => isSameLike(item.get ? item.get(valueField) : item[valueField], optionValue));
              if (found) {
                text += `/${found.get ? found.get(textField) : found[textField] || ''}`;
                return found;
              }
            }
          }
   ...
   * @return true | false
   */
  get rangeAllRequired(): boolean {
    return this.get('rangeAllRequired');
  }

  /**
   * 设置范围值是否全必输
   * @param rangeAllRequired 范围值是否全必输
   */
  set rangeAllRequired(rangeAllRequired: boolean) {
    this.set('rangeAllRequired', rangeAllRequired);
  }
  ...
  getValidatorPropGetter(record: Record | undefined = this.record): <T extends keyof ValidatorProps>(key: T) => ValidatorProps[T] {
    return (key) => {
      if (record) {
        switch (key) {
          case 'attachment':
            return this.getAttachments(record);
          ...
          case 'fetchAttachmentCount': {
            return this.fetchAttachmentCount(record.get(this.name));
          }
          case 'max':
          case 'min':
          ...
          case 'minLength':
          case 'maxLength': {
            const type = getBaseType(this.get('type', record));
            return type !== FieldType.string && type !== FieldType.auto ? undefined : this.get(key, record);
          }
          case 'format':
          ...
          case 'disabledDate':
            return this.getDisabledDate(record);
          case 'timeZone':
            return this.dataSet.getConfig('formatter').timeZone;
          case 'pattern': {
            const secretFieldEnableConfig = this.dataSet.getConfig('secretFieldEnable');
            // TelField 设置 mode=secert 若未查询脱敏则不校验
            if (record
                && this.get('type', record) === FieldType.tel
                && secretFieldEnableConfig && secretFieldEnableConfig()
                && (record.getState(`_telField-secertMode-${this.name}`) || this.get('telMode') === TelMode.secret)
                && !record.getState(`_secretField_queryFlag_${this.name}`)
            ) {
              return undefined;
            }
  ...
  async checkValidity(record: Record | undefined = this.record): Promise<boolean> {
    if (record) {
      ...
      const fieldType = this.get('type', record);
      if (fieldType === 'secret') {
        const secretFieldEnableConfig = dataSet.getConfig('secretFieldEnable');
        if (secretFieldEnableConfig && secretFieldEnableConfig() && !(!record.get('_token') || !record.getPristineValue(name))) {
          return true;
        }
      }
  ...
  fetchLookup(noCache = false, record: Record | undefined = this.record): Promise<object[] | undefined> {
    ...
    let promise;
    const lookupProps = this.getProps(record);
    if (!noCache && batch && lookupCode && !hasBatchSideEffectKeys(lookupProps, 'lookup')) {
      ...
      if (!promise) {
        promise = lookupStore.fetchLookupDataInBatch(lookupCode, batch, this, record).then(action((result: object[]) => {
          if (result) {
    ...
    if (promise) {
      return promise.then(action((result) => {
        ...
        if (lookup) {
          const { items } = lookup;
          if (items && isArray(items) && oldToken !== getLookupToken(this, record)) {
  ...
  fetchLovConfig(record: Record | undefined = this.record) {
    const lovCode = this.get('lovCode', record);
    if (lovCode) {
      const batch = this.get('lovBatchAxiosConfig', record) || this.dataSet.getConfig('lovBatchAxiosConfig');
      const lovProps = this.getProps(record);
      if (batch && !hasBatchSideEffectKeys(lovProps, 'lov')) {
        let promise;
        const lovCaches = getIf<DataSet, ObservableMap<string, LovCache>>(this.dataSet, 'lovCaches', () => observable.map());
        const cachedLov = getIfForMap<ObservableMap<string, LovCache>, LovCache>(lovCaches, lovCode, () => new LovCache());
        const data = lovCodeStore.lovCodes.get(lovCode);
        if (data) {
          promise = Promise.resolve<LovConfig>(data);
        } else {
          promise = cachedLov.promise;
        }
        if (!promise) {
          promise = lovCodeStore.fetchLovDataInBatch(lovCode, batch, this).then(action((result: LovConfig) => {
            if (result) {
              cachedLov.promise = undefined;
              lovCodeStore.lovCodes.set(lovCode, result);
            }
            return result;
          }));
          cachedLov.promise = promise;
        }
      } else {
  ...
  fetchAttachments(props: { bucketName?: string, bucketDirectory?: string, storageCode?: string, attachmentUUID: string, isPublic?: boolean, query?: any, body?: any }, record: Record | undefined = this.record) {
    const { bucketName, bucketDirectory, attachmentUUID, storageCode, isPublic, query, body } = props;
    const { fetchList } = this.dataSet.getConfig('attachment');
    if (fetchList) {
      fetchList({ bucketName, bucketDirectory, attachmentUUID, storageCode, isPublic, query, body }).then(action((results: FileLike[]) => {
  ...
  fetchAttachmentCount(uuid: string, isPublic?: boolean, record: Record | undefined = this.record) {
    ...
    if (batchFetchCount && !this.attachments) {
      return attachmentStore.fetchCountInBatch({
        attachmentUUID: uuid,
  ...
  getValidationErrorValues(record: Record | undefined = this.record): ValidationResult[] {
    if (record) {
      ...
      if (this.get('type', record) === 'intl') {
        const tlsKey = record.dataSet.getConfig('tlsKey');
        const { supports } = localeContext;
        const { lang } = record.dataSet;
        const languages = Object.keys(supports);
        const errorLanguages = languages.filter(language => {
          if (language !== lang) {
            const langError = record.getValidationError(`${tlsKey}.${name}.${language}`);
            if (langError) {
              return langError.length > 0;
            }
          }
          return false;
        });
        if (errorLanguages.length) {
          return [
            new ValidationResult({
              validationProps: {},
              validationMessage: $l('Validator', 'intl'),
              value: null,
  ...
  setAttachments(attachments: AttachmentFile[] | undefined, record: Record | undefined = this.record, uuid?: string | undefined) {
    if (record) {
      ...
      if (value) {
        value = this.getAttachmentCacheKey(value);
  ...
  getAttachments(record: Record | undefined = this.record, uuid?: string | undefined) {
    if (record) {
      let value = uuid || record.get(this.name) || this.tempAttachmentUUID;
      if (value) {
        value = this.getAttachmentCacheKey(value);
  ...
  getDisabledDate(record: Record | undefined = this.record) {
    if (record) {
      const autoDisabledDate = this.get('autoDisabledDate', record);
      const dateCode = this.get('dateCode', record);
      const value = record.get(this.name);
      if (autoDisabledDate && dateCode && value && isMoment(value)) {
        return dateCodeStore.fetchDateConfig(dateCode, moment(value).format('YYYY'));
      }
    }
  }
  ...
  setAttachmentCount(count: number | undefined, record: Record | undefined = this.record) {
    if (record) {
      ...
      if (uuid) {
        const value = this.getAttachmentCacheKey(uuid);
        const attachmentCaches = getIf<Record, ObservableMap<string, AttachmentCache>>(record, 'attachmentCaches', () => observable.map());
        const cache = attachmentCaches.get(value);
        if (cache) {
          set(cache, 'count', count);
        } else {
          attachmentCaches.set(value, { count });
        }
      }
    } else {
  ...
  getAttachmentCacheKey(uuid?: string) {
    return `${uuid}#${this.get('bucketName') || 'default'}#${this.get('bucketDirectory') || 'default'}`;
  }
}
