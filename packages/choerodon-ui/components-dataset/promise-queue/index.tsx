...
class PromiseTask<T = any> {
  ...
  then<TResult1 = T, TResult2 = never>(onfulfilled?: ((value: T) => (PromiseLike<TResult1> | TResult1)) | undefined | null, onrejected?: ((reason: any) => (PromiseLike<TResult2> | TResult2)) | undefined | null): Promise<TResult1 | TResult2> {
    ...
    return new Promise<TResult1 | TResult2>((resolve, reject) => {
      const resolveCallbacks = getIf<this, Function[]>(this, 'resolveCallbacks', []);
      const rejectCallbacks = getIf<this, Function[]>(this, 'rejectCallbacks', []);
...
export default class PromiseQueue<T = any> {
  ...
  add(promise: Promise<T> | (() => Promise<T>)): Promise<T> {
    const { queue } = this;
    const task = new PromiseTask<T>(promise);
    queue.push(task);
    return task
      .then((v) => v)
      .finally(() => this.$clear(task));
  }
  ...
  private $nextTask = (preResult?: T): Promise<T | undefined> => {
    const task = this.queue.shift();
    if (task) {
      return task.run().then((v) => {
        if (this.queueing) {
          return this.$nextTask(v);
        }
        return v;
      }).catch((error) => {
        this.stop();
  ...
  async ready(defaultResult?: Promise<T | undefined>): Promise<T | undefined> {
    if (this.promise) {
      return this.promise;
    }
    ...
    if (queue.length) {
      this.queueing = true;
      try {
        const promise = Promise.all(Array.from({ length: threads }).map(() => this.$nextTask()))
