...
import { flow as mobxFlow, isArrayLike, toJS } from 'mobx';
...
import attachmentError from './rules/attachmentError';
...
const flow = mobxFlow as <R, Args extends any[]>(generator: (...args: Args) => Generator<any, R, any> | AsyncGenerator<any, R, any>)=> (...args: Args) => CancellablePromise<R>;
...
function* execute(rules: validationRule[], value: any[], props: ValidatorBaseProps, getProp: <T extends keyof ValidatorProps>(key: T) => ValidatorProps[T], validationResults: ValidationResult[]): any {
  // why rules is undefined?  -- SLBrowser-8.0.0.3161
  if (rules) {
...
export default {
  ...
  reportAll: (errors: ValidationErrors[]) => {
    const { length } = errors;
  ...
  reportDataSet: (errors: ValidationSelfErrors[]) => {
    const { length } = errors;
  ...
  report: async (results: ValidationResult[], props: ValidatorBaseProps) => {
...
[field<${name}>]:`,
  ...
  checkValidity: flow(function* (value: unknown = null, props: ValidatorBaseProps = {}, getProp: <T extends keyof ValidatorProps>(key: T) => ValidatorProps[T] = noop) {
    const validationResults: ValidationResult[] = [];
    const attachmentErr: methodReturn = yield attachmentError(value, props, getProp);
    if (attachmentErr !== true) {
      validationResults.push(attachmentErr);
    } else {
      const valueMiss: methodReturn = yield valueMissing(value, props, getProp);
      if (valueMiss !== true) {
