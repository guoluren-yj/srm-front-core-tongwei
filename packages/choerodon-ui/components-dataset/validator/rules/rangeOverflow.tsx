  ...
  if (!isEmpty(value)) {
    ...
    if (!isNil(max)) {
      ...
      if (isOverflow(value, max, range)) {
        ...
        let maxString = toDateString(max, format);
        const type = getProp('type');
        if (type === 'time' || type === 'dateTime') {
          const timeZone = getProp('timeZone');
          if (timeZone) {
            const renderedTimeZone = typeof timeZone === 'function' ? timeZone(moment(max), { boundaryType: 'max' }) : moment(max).format(timeZone);
            if (renderedTimeZone) {
              maxString = `${maxString}${renderedTimeZone}`;
            }
          }
        }
        const injectionOptions = { max: maxString, label };
