...
const locale: Locale = {
  ...
  Validator: {
    ...
    too_long_attachment: '请将该文件数量减少到{maxLength}个或更少文件（目前您上传了{length}个文件）。',
    too_short_attachment: '请将该文件数量增加到{minLength}个或更多文件（目前您上传了{length}个文件）。',
    upload_error: '存在附件上传失败，请检查。',
    uploading: '附件正在上传，请等待上传完成或取消上传后重试。',
