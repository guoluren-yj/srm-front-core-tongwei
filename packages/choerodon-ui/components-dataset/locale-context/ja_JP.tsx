...
const locale: Locale = {
  ...
  Validator: {
    ...
    too_long_attachment: 'ファイル数を「maxLength」以下に減らしてください(現在「length」ファイルをアップロードしています)。',
    too_short_attachment: 'ファイル数を{minLength}個以上に増やしてください(現在、{length}個のファイルをアップロードしています)。',
    upload_error: '添付ファイルのアップロードに失敗がありますのでチェックしてください。',
    uploading: '添付ファイルはアップロード中ですので、アップロード完了またはキャンセル後に再試行してください。',
