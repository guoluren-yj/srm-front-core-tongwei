import moment, { isDate, isMoment, Moment, unitOfTime } from 'moment';
...
export function getDateFormatByField(field?: Field, type?: FieldType, record?: Record): string {
  if (field) {
    const format = field.get('format', record);
    if (format) {
      return format;
    }
    const fieldShowType = field.get('showType', record);
    const fieldType = field.get('type', record);
    return field.get('format', record) || getDateFormatByFieldType(fieldShowType || type || fieldType, field);
  }
  if (type) {
...
export function getDateShowFormatByField(field?: Field, type?: FieldType, record?: Record): string {
  const getGlobalShowFormat = getGlobalConfig('getDateShowFormat');
  ...
  return getDateFormatByField(field, type, record);
}
...
export function getNumberFormatOptions(
  ...
  return { style: 'decimal', minimumFractionDigits: 2, maximumFractionDigits: 20 };
}
...
export function toDateString(min: unknown, format?: string): string {
  if (isDate(min)) {
    min = moment(min);
  }
  if (isMoment(min)) {
