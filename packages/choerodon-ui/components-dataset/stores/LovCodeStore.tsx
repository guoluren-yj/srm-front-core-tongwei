...
import { defaultCache } from '../configure/index';
...
export type callbackArgs = [(codes: string[]) => AxiosRequestConfig, Field | undefined];

export type responseType = LovConfig | undefined;
...
export class LovCodeStore {
  ...
  batchCallback = (codes: string[], args: callbackArgs): Promise<{ [key: string]: LovConfig }> => {
    const [lovBatchAxiosConfig, field] = args;
    if (lovBatchAxiosConfig) {
      const config = processAxiosConfig(lovBatchAxiosConfig, codes);
      return this.getAxios(field)({...config }) as any;
    }
    return Promise.resolve({});
  };

  merger: PromiseMerger<LovConfig, callbackArgs, undefined> = new PromiseMerger<LovConfig, callbackArgs, undefined>(
    this.batchCallback,
    defaultCache,
    undefined,
    30,
  ...
  fetchLovDataInBatch(code: string, lovBatchAxiosConfig: (codes: string[]) => AxiosRequestConfig, field?: Field): Promise<responseType> {
    const getBatchKey = (defaultKey) => {
      const { url } = lovBatchAxiosConfig([code]);
      return url ? url.split('?')[0] : defaultKey;
    };
    return this.merger.add(code, getBatchKey, [lovBatchAxiosConfig, field]);
  }

  async fetchConfig(code: string, field?: Field, record?: Record): Promise<responseType> {
    let config = this.getConfig(code);
    // SSR do not fetch the lov
  ...
  getLovDataSet(code: string, field?: Field, dataSetProps?: DataSetProps | ((p: DataSetProps) => DataSetProps), record?: Record): DataSet | undefined {
    ...
    if (config) {
      ...
      const autoCount = getGlobalConfig('lovQueryAutoCount', field);
      if (!isNil(lovPageSize)) {
        dsProps.autoCount = autoCount;
      }
