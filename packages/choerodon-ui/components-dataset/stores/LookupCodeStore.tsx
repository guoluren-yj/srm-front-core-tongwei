...
export class LookupCodeStore {
  ...
  batchCallback = (codes: string[], args: callbackArgs, lovParas): Promise<{ [key: string]: responseData }> => {
    const [lookupBatchAxiosConfig, field] = args;
    if (lookupBatchAxiosConfig) {
      const config = processAxiosConfig(lookupBatchAxiosConfig, codes, lovParas);
  ...
  fetchLookupDataInBatch(code: string, lookupBatchAxiosConfig: (codes: string[]) => AxiosRequestConfig, field?: Field, record?: Record): Promise<responseType> {
    const getBatchKey = (defaultKey) => {
      const { url } = lookupBatchAxiosConfig([code]);
      return url ? url.split('?')[0] : defaultKey;
    };
    return this.merger.add(code, getBatchKey, [lookupBatchAxiosConfig, field], field ? getLovPara(field, record) : {});
  }
  ...
  getUrl(field: Field, record?: Record | undefined): string | undefined {
    ...
    if (lovCode && type !== FieldType.object && (type !== FieldType.auto || !record || !isObject(record.get(field.name)))) {
