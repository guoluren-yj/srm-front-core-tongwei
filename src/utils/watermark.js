...
export function watermark(text, { watermark_cols, watermark_rows, watermark_x_space, watermark_y_space }, container) {
  ...
  for (let i = 0; i < watermark_rows; i++) {
    ...
    for (let j = 0; j < watermark_cols; j++) {
      ...
      // 调整水印层级，异步登录是 1200000 ，比异步登录大一点
      mask_div.style.zIndex = "1300000";
...
export function cacWaterMark(container) {
  ...
  if (
    watermark_cols === 0 ||
    (parseInt(defaultSettings.watermark_x + defaultSettings.watermark_width * watermark_cols + watermark_x_space * (watermark_cols - 1)) > page_width)
  ) {
  ...
  if (
    watermark_rows === 0 ||
    (parseInt(defaultSettings.watermark_y + defaultSettings.watermark_height * watermark_rows + watermark_y_space * (watermark_rows - 1)) > page_height)
  ) {
    watermark_rows = parseInt((watermark_y_space + page_height - defaultSettings.watermark_y) / (defaultSettings.watermark_height + watermark_y_space));
    watermark_y_space = parseInt(((page_height - defaultSettings.watermark_y) - defaultSettings.watermark_height * watermark_rows) / ((watermark_rows - 1)||1));
  }
